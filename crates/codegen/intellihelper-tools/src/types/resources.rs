...
/// `"namespace.Name"` (e.g., `"intellihelper_build.ReadFile"`). The ID is used as
...
pub trait ResourceType: Any + 'static {
    /// Unique identifier, e.g. `"intellihelper_build.ReadFile"`.
    const ID: &'static str;
...
///
/// ```ignore
/// register_resource!("intellihelper_build", "ReadFile", ReadHistory);
/// ```
///
/// This generates:
/// ```ignore
/// impl ResourceType for ReadHistory {
///     const ID: &'static str = "intellihelper_build.ReadFile";
/// }
/// ```
#[macro_export]
...
struct ResourceEntry {
    type_id: TypeId,
    /// The `ResourceType::ID` string (e.g., `"intellihelper_build.ReadFile"`).
    id: String,
...
impl Resources {
    ...
    /// {
    ///   "params": {
    ///     "intellihelper_build.Edit": { ... },
    ///   },
    ///   "state": {
    ///     "intellihelper_build.ReadFile": { ... },
    ///     "intellihelper_build.Todo": { ... },
    ///   }
    /// }
    /// ```
    ///
...
/// tool falls back to `Cwd/.intellihelper/plan.md`.
#[derive(Debug, Clone)]
...
pub const PLAN_FILE_RELATIVE_PATH: &str = ".intellihelper/plan.md";
...
/// [`Cwd`]`/.intellihelper/plan.md`, else the bare relative `.intellihelper/plan.md`.
...
/// `INTELLIHELPER_SCHEDULER_BACKGROUND_LOOPS` env var, or the
...
register_resource!("intellihelper_build", "WebCitation", WebCitationCounter);
...
/// depending on `intellihelper-mcp` directly.  Follows the same pattern as
...
mod tests {
    ...
    register_resource!("intellihelper_build", "Edit", EditConfig);
    #[derive(Debug, Clone, Default, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
    struct ReadHistory {
    ...
    register_resource!("intellihelper_build", "Todo", TodoData);
    #[test]
    ...
    fn ephemeral_types_silently_skipped_during_serialization() {
        ...
        assert!(state.get("intellihelper_build.ReadFile").is_some());
    ...
    fn load_from_populates_registered_types() {
        ...
        state_map.insert(
            "intellihelper_build.ReadFile".to_string(),
            serde_json::json!({"files_read": ["loaded.rs"]}),
        );
        let mut params_map = HashMap::new();
        params_map.insert(
            "intellihelper_build.Edit".to_string(),
    ...
    fn get_json_returns_registered_value() {
        ...
        let val = res.get_json("params", "intellihelper_build.Edit").unwrap();
    ...
    fn get_json_returns_none_for_missing_value() {
        let mut res = Resources::new();
        res.register_params::<EditConfig>();
        assert!(res.get_json("params", "intellihelper_build.Edit").is_none());
    }
    #[test]
