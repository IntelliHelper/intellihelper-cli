...
fn external_stream_end_to_end() {
    ...
    let mut cfg = intellihelper_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
            "INTELLIHELPER_EXTERNAL_OTEL" => Some("1".into()),
    ...
    cfg.client = intellihelper_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    intellihelper_telemetry::external::init(Some(cfg));
    assert!(intellihelper_telemetry::external::is_active());
    ...
    assert!(!intellihelper_telemetry::is_enabled());
    intellihelper_telemetry::log_event(intellihelper_telemetry::events::SessionNew {
        session_id: "sess-int-1".into(),
        client_identifier: None,
        client_version: None,
        is_git_repo: true,
        permission_mode: intellihelper_telemetry::enums::PermissionMode::Ask,
    });
    intellihelper_telemetry::log_event(intellihelper_telemetry::events::SessionHarness {
        session_id: "sess-int-1".into(),
    ...
    intellihelper_telemetry::log_event(intellihelper_telemetry::events::PromptSubmitted {
    ...
    intellihelper_telemetry::log_event(intellihelper_telemetry::events::ModelResponseReceived {
        ...
        for rl in &req.resource_logs {
            ...
            for sl in &rl.scope_logs {
                assert_eq!(
                    ...
                    Some("ai.xai.intellihelper_code")
                );
    ...
    assert!(
        resource_service_name
            .as_deref()
            .is_some_and(|s| s.contains("intellihelper-cli")),
        "service.name=intellihelper-cli is a wire commitment: {resource_service_name:?}"
    );
    for expected in [
        "intellihelper_code.session_start",
        "intellihelper_code.user_prompt",
        "intellihelper_code.api_request",
    ] {
        assert!(
    ...
    assert_eq!(
        event_names
            .iter()
            .filter(|n| *n == "intellihelper_code.session_start")
            .count(),
        1
    );
            ...
            for sm in &rm.scope_metrics {
                for metric in &sm.metrics {
                    ...
                    if let Some(Data::Sum(sum)) = &metric.data {
                        ...
                        if metric.name == "intellihelper_code.session.count" {
    ...
    assert!(
        metric_names.iter().any(|n| n == "intellihelper_code.session.count"),
        "missing session.count in {metric_names:?}"
    );
    assert!(metric_names.iter().any(|n| n == "intellihelper_code.token.usage"));
    assert_eq!(
    ...
    intellihelper_telemetry::external::shutdown();
    assert!(
