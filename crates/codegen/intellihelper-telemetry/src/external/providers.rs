...
fn build_resource(cfg: &ExternalOtelConfig) -> opentelemetry_sdk::Resource {
    let mut attrs = vec![
        ...
        opentelemetry::KeyValue::new("intellihelper_code.schema.version", super::schema::SCHEMA_VERSION),
    ];
    ...
    opentelemetry_sdk::Resource::builder_empty()
        // RQ6 (final): `intellihelper-cli`, a wire commitment.
...
fn grpc_tls_candidates(
    ...
    // Process-wide `INTELLIHELPER_EXTRA_CA_BUNDLE` roots (fail-open by that crate's
    ...
    if let Some(extra_pem) = ders_to_pem_bundle(intellihelper_extra_ca::extra_root_ders()) {
...
mod tests {
    ...
    /// The DER→PEM re-encode used for `INTELLIHELPER_EXTRA_CA_BUNDLE` must produce a
