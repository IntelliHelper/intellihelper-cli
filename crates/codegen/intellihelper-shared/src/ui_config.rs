...
use intellihelper_config_types::DisplayRefreshSettings;
...
pub struct UiConfig {
    ...
    /// connect time (CLI `--hunk-tracker-mode` / `INTELLIHELPER_HUNK_TRACKER` override
    ...
    /// Speech-to-text language preference for voice dictation. A IntelliHelper STT
...
pub struct ContextualHints {
    ...
    /// SSH wrap session-load tip (recommend `intellihelper wrap ssh` when the session
...
impl Default for UiConfig {
    fn default() -> Self {
        Self {
            ...
            theme: None,
            fork_secondary_model: intellihelper_models::default_model().to_string(),
            yolo: false,
            ui_theme: None,
            compact_mode: false,
            simple_mode: None,
