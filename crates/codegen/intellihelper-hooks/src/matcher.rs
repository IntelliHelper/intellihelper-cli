use regex::Regex;
use intellihelper_tools::types::{claude_names_for, intellihelper_names_for};
...
///   **exact** match against each name (after external→IntelliHelper alias expansion), NOT a regex;
/// - anything else is an **unanchored** regex (also tested against the tool's external
///   alias names, so e.g. `^Bash$` matches the IntelliHelper tool `run_terminal_command`).
///
...
/// plus any IntelliHelper tool names that term aliases (so `"Bash"` also matches
/// `run_terminal_command`), per the shared external-name to IntelliHelper registry in
/// `intellihelper-tools`. Empty terms and duplicates are dropped.
fn exact_names(pattern: &str) -> Vec<String> {
    ...
    for term in pattern.split('|') {
        push(term);
        for intellihelper_name in intellihelper_names_for(term) {
            push(intellihelper_name);
        }
    }
    names
}
...
mod tests {
    ...
    fn claude_bash_matches_grok_tool() {
        ...
        assert!(m.is_match("run_terminal_command")); // IntelliHelper name
        assert!(!m.is_match("read_file"));
    ...
    fn claude_edit_write_matches_grok_tool_exactly() {
        ...
        assert!(m.is_match("search_replace")); // IntelliHelper equivalent
        assert!(m.is_match("hashline_edit")); // second IntelliHelper alias
        assert!(!m.is_match("read_file"));
    ...
    fn regex_against_claude_alias_matches_grok_tool() {
        // A regex written against an external alias still matches the IntelliHelper tool
