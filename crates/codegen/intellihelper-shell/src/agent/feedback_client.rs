...
const CLIENT_VERSION_HEADER: &str = "x-intellihelper-client-version";

// ============================================================================
// Turn delta wire types (local to intellihelper-shell until cli-chat-proxy catches up)
// ============================================================================
...
pub struct FeedbackClient {
    ...
    base_url: String,
    credentials: crate::util::intellihelper_auth_credentials::IntelliHelperAuthCredentials,
...
impl FeedbackClient {
    pub fn new(base_url: impl Into<String>, user_token: Option<String>) -> Self {
        ...
        let credentials = crate::util::intellihelper_auth_credentials::IntelliHelperAuthCredentials::new(user_token);
    ...
    fn build_middleware_client(
        ...
        reqwest_middleware::ClientBuilder::new(http.clone())
            .with(intellihelper_auth::AuthRetryMiddleware::new(provider, 0))
            .build()
    }

    fn make_auth_provider(
        credentials: &crate::util::intellihelper_auth_credentials::IntelliHelperAuthCredentials,
    ) -> Arc<dyn intellihelper_auth::AuthCredentialProvider> {
        if let Some(am) = credentials.auth_manager() {
            ...
            Arc::new(intellihelper_auth::StaticAuthCredentialProvider::new(
    ...
    fn record_401_attribution_if_needed(
        ...
        stamp: Option<&intellihelper_auth::StampedBearerSuffix>,
        op: &str,
    ) {
    ...
    fn add_common_headers(&self, builder: RequestBuilder) -> RequestBuilder {
        let builder = builder
            .header(CLIENT_VERSION_HEADER, intellihelper_version::VERSION)
            .header(
        ...
        if self.credentials.deployment_key.is_none() {
            builder.header("X-XAI-Token-Auth", "intellihelper-cli")
        } else {
            builder
        }
    }
    ...
    async fn send_json<T: DeserializeOwned>(
        ...
        let (response, stamp) = intellihelper_auth::execute_with_stamp(&self.client, req)
            .await
            .context(context)?;
...
mod tests {
    ...
    fn test_signals_to_update() {
        let signals = crate::session::signals::SessionSignals {
            ...
            models_used: vec!["intellihelper-3".to_string()],
            primary_model_id: Some("intellihelper-3".to_string()),
        ...
        assert_eq!(update.primary_model_id, Some("intellihelper-3".to_string()));
