...
//! (the envelope types are canonical in `intellihelper_workspace_types::rpc`).
//!
//! `error_code` uses a non-wildcard match so the compiler enforces
//! coverage of new `WorkspaceError` variants.
use crate::error::WorkspaceError;
pub use intellihelper_workspace_types::rpc::{RpcEnvelope, RpcError};
...
pub fn error_code(err: &WorkspaceError) -> &'static str {
    match err {
        ...
        WorkspaceError::TurnActive(_) => intellihelper_workspace_types::rpc::envelope::TURN_ACTIVE,
...
pub fn rpc_error_to_workspace(err: RpcError) -> WorkspaceError {
    if let Some(kind) =
        intellihelper_workspace_types::rpc::export_github::ExportGithubError::from_wire_code(&err.code)
    {
...
mod tests {
    ...
    fn export_github_codes_round_trip_typed() {
        for kind in intellihelper_workspace_types::rpc::export_github::ExportGithubError::ALL {
