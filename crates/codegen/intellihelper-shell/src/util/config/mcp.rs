...
use intellihelper_tools::types::compat::{CompatConfig, CompatConfigToml};

pub use intellihelper_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types extracted to `intellihelper-config-types` (config
...
pub use intellihelper_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use intellihelper_config_types::PoolConfig;
...
/// Walks from cwd up to the git repo root checking `.intellihelper/config.toml` at each level.
/// Project-scoped `.intellihelper/config.toml` entries override global `~/.intellihelper/config.toml`
...
/// `.intellihelper/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.intellihelper/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.intellihelper/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.intellihelper/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .intellihelper/config.toml"
                );
...
pub(crate) fn mcp_preferences_path() -> PathBuf {
    intellihelper_config::intellihelper_home().join("mcp_preferences.json")
}
...
pub(crate) fn collect_mcp_setup_configs(
    cwd: &std::path::Path,
    plugin_registry: Option<&intellihelper_agent::plugins::PluginRegistry>,
    compat: &CompatConfig,
    ...
    if let Some(registry) = plugin_registry {
        ...
        for plugin in registry.active_plugins() {
            ...
            if let Some(ref inline_value) = plugin.inline_mcp_servers {
                let normalized =
                    intellihelper_agent::plugins::manifest::normalize_inline_mcp_servers(inline_value);
...
/// Upsert an MCP server entry in `~/.intellihelper/config.toml`.
///
...
/// config file, e.g. a project-scoped `.intellihelper/config.toml`.
...
/// Delete an MCP server entry from `~/.intellihelper/config.toml`.
///
...
/// config file, e.g. a project-scoped `.intellihelper/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = intellihelper_mcp::credentials::McpCredentialStore::load_default() {
...
/// transport-less case is steered to `disabled_mcp_servers`, IntelliHelper's real
/// disable mechanism.
    ...
    let message = if !has_command && !has_url {
        format!(
            "`mcp_servers.{name}` has no transport. To run it, set `command = \"...\"` or \
             ...
             See ~/.intellihelper/docs/user-guide/07-mcp-servers.md"
        )
    } else {
        format!(
                    ...
                    problems.push(McpServerConfigProblem {
                        ...
                        message: format!(
                            "`mcp_servers.{name}` has an unrecognized field `{field}`; it is \
                             ignored. See ~/.intellihelper/docs/user-guide/07-mcp-servers.md"
                        ),
                    });
                }
                if config.enabled
                    ...
                    problems.push(McpServerConfigProblem {
                        ...
                        message: format!(
                            "`mcp_servers.{name}` is enabled but its `{field}` is blank. \
                             ...
                             off. See ~/.intellihelper/docs/user-guide/07-mcp-servers.md"
                        ),
                    });
                    continue;
                }
...
// `.mcp.json` discovery moved to `intellihelper-workspace` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use intellihelper_workspace::project_config::{
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.intellihelper/config.toml` files from `cwd` up to the
...
/// [`load_mcp_server_configs_with_project`], for `intellihelper inspect`.
...
/// Names `intellihelper mcp enable`/`disable` may target: user/project TOML (including
...
fn load_cli_plugin_registry(cwd: &std::path::Path) -> intellihelper_agent::plugins::PluginRegistry {
    ...
    let discovered = intellihelper_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    intellihelper_agent::plugins::PluginRegistry::from_discovered(
        discovered,
...
fn config_path() -> PathBuf {
    crate::util::intellihelper_home::intellihelper_home().join("config.toml")
}

/// Path to the user-level config file (`~/.intellihelper/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.intellihelper/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".intellihelper").join("config.toml")
}
...
/// Overrides `[cli] session_registry`; usable before `~/.intellihelper/config.toml` exists.
pub const SESSION_REGISTRY_ENV_VAR: &str = "INTELLIHELPER_SESSION_REGISTRY";

pub(crate) fn session_registry_from_env_opt() -> Option<bool> {
    intellihelper_config::env_bool(SESSION_REGISTRY_ENV_VAR)
}
...
mod tests {
    ...
    fn session_registry_local_override_precedence() {
        ...
        {
            let _g = intellihelper_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "false");
            assert_eq!(
        ...
        {
            let _g = intellihelper_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "bogus");
            assert_eq!(
                session_registry_local_override_sourced(Some(&toml_true)),
        ...
        {
            let _g = intellihelper_test_support::EnvGuard::unset(SESSION_REGISTRY_ENV_VAR);
...
"#;
    ...
    fn test_use_leader_no_cli_section() {
        ...
        let toml_str = r#"
[models]
default = "intellihelper-code-fast-1"
"#;
        let root: TomlValue = toml::from_str(toml_str).unwrap();
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.intellihelper/skills", "~/.intellihelper/skills/special/SKILL.md"]
ignore = ["~/.intellihelper/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.intellihelper/skills", "~/.intellihelper/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.intellihelper/skills/noisy/SKILL.md"]);
    }
...
"#;
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${INTELLIHELPER_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
    ...
    fn mcp_json_all_toml_names_includes_disabled() {
        let tmp = tempfile::tempdir().unwrap();
        let intellihelper_dir = tmp.path().join(".intellihelper");
        std::fs::create_dir_all(&intellihelper_dir).unwrap();
        std::fs::write(
            intellihelper_dir.join("config.toml"),
            r#"
...
"#,
    ...
    async fn enable_unstick_only_touches_nearest_project_definition() {
        ...
        std::fs::create_dir_all(nested.join(".intellihelper")).unwrap();
...
enabled = false
"#;
        let ancestor = tmp.path().join(".intellihelper").join("config.toml");
        let nearer = nested.join(".intellihelper").join("config.toml");
    ...
    async fn restore_mcp_server_enabled_after_enable_scopes_tiers() {
        ...
        // save_mcp_server_enabled_in (that reads ambient config_path / intellihelper_home).
