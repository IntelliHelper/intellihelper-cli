//! `intellihelper wrap` — run any command in a local PTY that forwards its clipboard.
//!
//! Generalizes the `intellihelper ssh` wrapper: spawns an arbitrary command inside a
...
//! stamps `LC_INTELLIHELPER_APPEARANCE` from the local OS theme so a remote
//! `theme = "auto"` can resolve over SSH + tmux.
//!
//! Resolvable programs spawn directly. On Unix, commands a direct spawn cannot
//! run — a single shell-quoted string (`intellihelper wrap "mycli ssh host"`) or a shell
...
/// Run the `intellihelper wrap` command.
///
...
pub fn run(args: &WrapArgs) -> Result<()> {
    ...
    let program = args
        .command
        .first()
        .ok_or_else(|| anyhow::anyhow!("intellihelper wrap: no command given"))?;
    ...
    let (wrapped, fallback) = {
        ...
        let in_path = intellihelper_config::shell::is_command_available(program);
        (
    ...
    if should_wrap() {
        match crate::pty_wrap::run_wrapped_command(&wrapped.program, &wrapped.args) {
            ...
            Err(e) => {
                ...
                eprintln!("intellihelper wrap: wrapped mode failed, running without PTY wrapping: {e}");
...
/// The program and argv `intellihelper wrap` will actually spawn.
#[derive(Clone)]
struct SpawnPlan {
    program: String,
    args: Vec<String>,
...
fn derive_spawn(
    ...
    // (`intellihelper wrap "mycli ssh host"`), not a program name: hand it to the shell
    ...
    // name, and an empty one (`intellihelper wrap "$PROG" ...` with `$PROG` unset)
...
/// deliberately not `intellihelper_config::shell::unix_shell_path`, which coerces
...
///
/// Unlike `intellihelper ssh`, `intellihelper wrap` does not gate on the terminal brand: the user
