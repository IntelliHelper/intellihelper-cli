...
use intellihelper_config::resolve_global_hook_sources;
use intellihelper_hooks::discovery::HookSource;
use intellihelper_hooks::error::HookError;
...
fn include_claude_hooks(compat: &intellihelper_tools::types::compat::CompatConfig) -> bool {
    compat.claude.hooks
        && !crate::claude_import::is_claude_import_marked_with_log("discover_hook_source_paths")
}

fn include_cursor_hooks(compat: &intellihelper_tools::types::compat::CompatConfig) -> bool {
    compat.cursor.hooks
}
...
pub(crate) fn discover_hook_source_paths(
    ...
    // Soft hooks-paths I/O keeps fixed slots; hard resolve omits IntelliHelper globals.
    let mut global: Vec<PathBuf> =
        match resolve_global_hook_sources(intellihelper.as_deref(), /* reject_symlinks */ false) {
            Ok(resolved) => {
                if let Some(e) = &resolved.configured_error {
                    tracing::warn!(
                        error = %e,
                        "hooks-paths unreadable; retaining fixed IntelliHelper hook discovery sources only"
                    );
                }
                resolved
                    .discovery_sources()
            ...
            Err(e) => {
                tracing::warn!(
                    error = %e,
                    "global hook source resolve hard-failed; omitting IntelliHelper global sources"
                );
                Vec::new()
            }
        };
    ...
    if let Some(root) = git_root {
        ...
        project.push(root.join(".intellihelper").join("hooks"));
        if include_cursor {
...
pub(crate) fn discover_hooks(
    ...
    compat: &intellihelper_tools::types::compat::CompatConfig,
    trusted: bool,
) -> (intellihelper_hooks::discovery::HookRegistry, Vec<HookError>) {
...
/// dedup in [`intellihelper_hooks::discovery::registry_from_specs_deduped`], a config
...
pub(crate) fn assemble_hooks(
    config_layers: &[intellihelper_config::HookConfigLayer],
    git_root: Option<&Path>,
    compat: &intellihelper_tools::types::compat::CompatConfig,
    trusted: bool,
) -> (intellihelper_hooks::discovery::HookRegistry, Vec<HookError>) {
    let (mut specs, mut errors) =
        intellihelper_hooks::config::parse_hooks_from_config_layers(config_layers);
    ...
    let (file_specs, file_errors) =
        intellihelper_hooks::discovery::collect_specs_from_sources(&global_sources, &project_sources);
    specs.extend(file_specs);
    errors.extend(file_errors);

    (
        intellihelper_hooks::discovery::registry_from_specs_deduped(specs),
        errors,
    )
}
