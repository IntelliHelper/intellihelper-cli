...
impl AgentView {
    ...
    pub fn new(session: AgentSession, scrollback: ScrollbackState) -> Self {
        ...
        let mut view = Self {
            ...
            context_state: None,
            // Cumulative session spend (input+output) for the status bar badge.
            session_usage_total_tokens: 0,
            hit_session_usage: Default::default(),
            chat_kind: false,
            app_chat_mode: false,
            ...
            hit_optimize_input: Default::default(),
            optimize_generation: None,
            optimize_undo: None,
    ...
    pub fn apply_full_context_info(&mut self, next: intellihelper_shell::session::ContextInfo) {
        if self.chat_kind {
    ...
    pub fn apply_context_used(&mut self, used: u64, total: u64) {
        ...
        match self.context_state.as_mut() {
            ...
            None => {
                self.context_state = Some(intellihelper_shell::session::ContextInfo::from_notification(
                    used, total,
                ));
            }
        }
