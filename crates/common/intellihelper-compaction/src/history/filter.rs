...
//!
//! Behavior is byte-for-byte identical for IntelliHelper chat (`T = Arc<IntelliHelperTurn>`).
...
///   summaries so their `<intellihelper_user_queries>` sections can be split out.
...
/// Extracts **all** `<intellihelper_user_queries>...</intellihelper_user_queries>` blocks
...
pub fn split_prior_compaction_text(text: &str) -> (Option<String>, String) {
    let start_tag = "<intellihelper_user_queries>";
    let end_tag = "</intellihelper_user_queries>";
...
/// Extract a `<intellihelper_user_queries>` XML block from `User` items in `turns`.
///
/// Walks `turns`, finds `User` items, and formats each as a `<intellihelper_query>`
/// element with text content (from [`CompactionItem::text`]) and any
/// `<intellihelper_file id="..." name="..." />` lines for the item's attachment
...
pub fn extract_user_queries_from_turns<T: CompactionItem>(
    ...
    let mut result = String::from("<intellihelper_user_queries>\n");
    ...
    for turn in turns {
        ...
        result.push_str("<intellihelper_query>");
    ...
    result.push_str("</intellihelper_user_queries>");
    Some(result)
}

/// Walk `turns`, find any prior compaction summary items, extract their
/// `<intellihelper_user_queries>` blocks via [`split_prior_compaction_text`], and
/// concatenate them.
///
...
pub struct SeparatedHistoryTurns<T> {
    /// `turns` with the `<intellihelper_user_queries>` block stripped from every
    ...
    /// Concatenation of every `<intellihelper_user_queries>` block found (in
    ...
    /// regardless of whether it contained a `<intellihelper_user_queries>` block.
...
/// `<intellihelper_user_queries>` block (preserved verbatim for the next summary)
...
///   loop, so the LLM never sees `<intellihelper_user_queries>` from earlier rounds.
...
///   would see the prior `<intellihelper_user_queries>` and tend to copy it into the
...
pub fn separate_prior_user_queries<T: CompactionItemBuilder>(
    ...
    for turn in turns {
        if turn.is_compaction_summary() {
            ...
            // `<intellihelper_user_queries>` block (and therefore stripped to an empty
...
mod tests {
    ...
    fn extract_user_queries_wraps_single_user_turn() {
        ...
        assert!(out.starts_with("<intellihelper_user_queries>"));
        assert!(out.ends_with("</intellihelper_user_queries>"));
        assert!(out.contains("<intellihelper_query>hello world</intellihelper_query>"));
    }
    ...
    fn extract_prior_user_queries_concatenates_blocks() {
        let inner = "<intellihelper_user_queries>\n<intellihelper_query>first</intellihelper_query>\n</intellihelper_user_queries>";
        let inner2 = "<intellihelper_user_queries>\n<intellihelper_query>second</intellihelper_query>\n</intellihelper_user_queries>";
    ...
    fn separate_strips_user_queries_from_summary_item() {
        let prior = "<intellihelper_user_queries>\n<intellihelper_query>Q1</intellihelper_query>\n</intellihelper_user_queries>\n\n<chunk_summary index=\"0\">S1</chunk_summary>";
        ...
        assert!(prior.contains("<intellihelper_user_queries>"));
        ...
        match &sep.turns_for_llm[0] {
            MockItem::Developer { text, summary } => {
                ...
                assert!(!text.contains("<intellihelper_user_queries>"));
    ...
    fn separate_drops_summary_item_with_no_rest() {
        let only_queries = "<intellihelper_user_queries>\n<intellihelper_query>Q</intellihelper_query>\n</intellihelper_user_queries>";
