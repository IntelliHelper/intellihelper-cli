...
//! `intellihelper_sampling_types::SamplingConfig` so that the sampler crate
//! avoids transitive dependencies on shell-specific types
//! (`intellihelper-tools`, etc.).
...
/// submit.
///
/// # Construction in `intellihelper-shell`
///
...
/// composing chat-state's `intellihelper_sampling_types::SamplingConfig`
...
pub struct SamplerConfig {
    ...
    /// client itself sends the opt-in `x-intellihelper-doom-loop-check` header on
