...
pub(crate) use intellihelper_pager_pty_harness::{
...
pub(crate) const CONTEXTUAL_HINTS_ENV: &[(&str, &str)] = &[("INTELLIHELPER_CONTEXTUAL_HINTS", "1")];
...
// `intellihelper_pager_pty_harness::flows` (re-exported above).
...
/// would never fetch `/v1/settings`. Spawns WITHOUT `INTELLIHELPER_ANNOUNCEMENTS_OVERRIDE`
...
pub(crate) fn spawn_polling_session_with_env(
    ...
    overrides.push(EnvOp::set("INTELLIHELPER_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS", "1"));
...
/// Explicit overrides for a folder-trust run. A self-built intellihelper auto-trusts,
/// so `INTELLIHELPER_TEST_VERSION` simulates a release; the gate is pinned both ways.
pub(crate) fn trust_env(feature_on: bool) -> [(&'static str, &'static str); 2] {
    [
        ("INTELLIHELPER_TEST_VERSION", "0.0.0-sim"),
        ("INTELLIHELPER_FOLDER_TRUST", if feature_on { "1" } else { "0" }),
    ]
}
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".intellihelper")
        .join(intellihelper_workspace::trust::TRUST_FILE_NAME);
    let store = intellihelper_workspace::trust::TrustStore::load_from(store_path);
    store.is_trusted(&intellihelper_workspace::trust::workspace_key(repo))
}
...
// `wait_for_labels_absent` lives in `intellihelper_pager_pty_harness::flows`
// (re-exported above).
...
pub(crate) fn seed_mcp_server_config(content: &ContentController) {
    ...
    let intellihelper_home = content.home().join(".intellihelper");
    std::fs::create_dir_all(&intellihelper_home).expect("create fake INTELLIHELPER_HOME");
    let config = format!(
        "[mcp_servers.{MCP_TEST_SERVER}]\ncommand = \"{command}\"\nargs = []\nstartup_timeout_sec = 2\n"
    );
    std::fs::write(intellihelper_home.join("config.toml"), config).expect("write config.toml");
}
...
// unit tests in intellihelper-hooks::discovery::tests. The PTY E2E test requires
...
/// Seed `~/.intellihelper/config.toml` with a `[ui]` section body (e.g.
/// `"vim_mode = true"`). Same `{INTELLIHELPER_HOME|HOME}/.intellihelper/config.toml` location
...
pub(crate) fn seed_ui_config(content: &ContentController, ui_body: &str) {
    let intellihelper_home = content.home().join(".intellihelper");
    std::fs::create_dir_all(&intellihelper_home).expect("create .intellihelper");
...
pub(crate) fn seed_mouse_reporting_toggle_config(content: &ContentController, enabled: bool) {
    let intellihelper_home = content.home().join(".intellihelper");
    std::fs::create_dir_all(&intellihelper_home).expect("create .intellihelper");
    // Minimal opt-in only — matches load_config's `{INTELLIHELPER_HOME|HOME}/.intellihelper/config.toml`.
...
pub(crate) fn seed_keep_text_selection_config(content: &ContentController) {
    ...
    std::fs::create_dir_all(&intellihelper_home).expect("create .intellihelper");
    std::fs::write(
        intellihelper_home.join("config.toml"),
...
pub(crate) fn spawn_mouse_toggle_pager(content: &ContentController) -> PtyHarness {
    ...
    PtyHarness::spawn_with_content_env_ops(
        ...
        content,
        &[],
        &[EnvOp::set("INTELLIHELPER_MOUSE_REPORTING_TOGGLE", "true")],
    )
    .expect("spawn pager")
}
...
pub(crate) const ESC_DOUBLE_PRESS_ENV: &str = "INTELLIHELPER_ESC_DOUBLE_PRESS_MS";
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    let binary = pager_binary().expect("resolve pager binary");
    let value = intellihelper_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string();
    PtyHarness::spawn_with_content_env_ops(
        &binary,
        DEFAULT_ROWS,
        DEFAULT_COLS,
...
/// Locate `<intellihelper_home>/sessions/<encoded cwd>/<session id>/`, where the shell
...
pub(crate) fn session_dir(content: &ContentController, harness: &mut PtyHarness) -> PathBuf {
    let sessions = content.home().join(".intellihelper").join("sessions");
    for _ in 0..100 {
...
// ── intellihelper wrap e2e ───────────────────────────────────────────────────────

/// `intellihelper wrap` run budget. Same contention math as the requirements-version
...
/// Run `intellihelper wrap <wrap_args...>` to completion inside a PTY with an isolated
/// `INTELLIHELPER_HOME`, returning the exit code (`None` only while still running at
...
pub(crate) fn run_wrap_driving(
    ...
    let mut env: Vec<(&str, &str)> = vec![("INTELLIHELPER_HOME", &home_str), ("NO_COLOR", "1")];
    env.extend_from_slice(extra_env);

    let mut harness =
        PtyHarness::new_inherited_env(&binary, DEFAULT_ROWS, DEFAULT_COLS, &args, &env, None)
            .expect("spawn intellihelper wrap");
    ...
    let code = match wait_for_exit_status(&mut harness, WRAP_TIMEOUT) {
        ...
        Ok(PtyExitPoll::Running) => {
            harness.quit().expect("kill intellihelper wrap after timeout");
            None
        }
        Ok(PtyExitPoll::PendingStatus) => {
            panic!("intellihelper wrap exited but portable status remained unavailable for {WRAP_TIMEOUT:?}")
        }
        Err(error) => panic!("poll intellihelper wrap exit: {error:#}"),
    };
...
/// `intellihelper wrap` hands to the user's shell without depending on any real
...
/// Dump an asciinema cast of `harness` into `$INTELLIHELPER_PTY_CAST_DIR/<file_name>`
...
pub(crate) fn write_cast_if_requested(harness: &PtyHarness, file_name: &str) {
    let Ok(dir) = std::env::var("INTELLIHELPER_PTY_CAST_DIR") else {
        return;
    };
    if dir.is_empty() {
        return;
...
/// `$INTELLIHELPER_PTY_CAST_DIR/<file_stem>.{txt,html}` when the env var is set.
...
pub(crate) use intellihelper_pager_pty_harness::host_clipboard::{
...
pub(crate) use intellihelper_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
