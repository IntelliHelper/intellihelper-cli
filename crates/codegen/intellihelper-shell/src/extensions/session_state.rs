...
pub(crate) async fn handle_import(args: &acp::ExtRequest) -> ExtResult {
    ...
    if !has_local_session {
        ...
        crate::util::intellihelper_home::ensure_sessions_cwd_dir(&request.cwd)
...
fn sanitize_summary_for_host(summary: &mut serde_json::Map<String, Value>, id: &str, cwd: &str) {
    ...
    set_or_remove(
        summary,
        "intellihelper_home",
        crate::session::persistence::intellihelper_home_string(),
    );
    set_or_remove(
        summary,
        "sandbox_profile",
        intellihelper_sandbox::configured_profile_name().map(String::from),
    );
}
