...
impl TitleManager {
    ...
    pub fn update(&mut self, state: &TitleState<'_>) -> Option<String> {
        ...
        if !has_parts {
            self.composed.clear();
            self.composed.push_str("intellihelper");
        }
    ...
    pub fn reset(&mut self) -> String {
        let esc = build_title_escape("intellihelper");
        self.last_title.clear();
        self.last_title.push_str("intellihelper");
...
fn write_item(
    ...
    match item {
        TitleItem::IntelliHelper => {
            push_separator(buf, has_parts);
            buf.push_str("intellihelper");
        }
        TitleItem::Spinner => {
...
/// strings (e.g. intellihelper.com conversation titles), which must not terminate the
...
mod tests {
    ...
    #[test]
    fn intellihelper_only_produces_just_grok() {
        let cfg = config_with_items(vec![TitleItem::IntelliHelper]);
        let mut mgr = TitleManager::new(&cfg);
        let state = idle_state();
        mgr.update(&state);
        assert_eq!(mgr.last_title, "intellihelper");
    }

    #[test]
    fn session_name_and_grok_joined_with_separator() {
        let cfg = config_with_items(vec![TitleItem::SessionName, TitleItem::IntelliHelper]);
        let mut mgr = TitleManager::new(&cfg);
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "my project - intellihelper");
    }
    ...
    fn spinner_only_shown_when_active() {
        let cfg = config_with_items(vec![TitleItem::Spinner, TitleItem::IntelliHelper]);
        let mut mgr = TitleManager::new(&cfg);
        ...
        mgr.update(&state);
        assert!(mgr.last_title.contains(" - intellihelper"));
    ...
    fn activity_hidden_when_idle() {
        let cfg = config_with_items(vec![TitleItem::Activity, TitleItem::IntelliHelper]);
        let mut mgr = TitleManager::new(&cfg);
        mgr.update(&idle_state());
        assert_eq!(mgr.last_title, "intellihelper");
    }
    ...
    fn activity_shows_waiting_when_busy_without_activity() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Waiting - intellihelper");
    }
    ...
    fn activity_prefers_specific_activity_over_waiting() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Thinking - intellihelper");
    }
    ...
    fn action_required_visible_on_first_tick() {
        let cfg = config_with_items(vec![TitleItem::ActionRequired, TitleItem::IntelliHelper]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
    ...
    fn model_item_shown_when_present() {
        let cfg = config_with_items(vec![TitleItem::Model, TitleItem::IntelliHelper]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
            model: Some("intellihelper-3"),
            ..idle_state()
        };
        mgr.update(&state);
        assert_eq!(mgr.last_title, "intellihelper-3 - intellihelper");
    }
    ...
    fn cwd_shows_last_component() {
        let cfg = config_with_items(vec![TitleItem::Cwd, TitleItem::IntelliHelper]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
    ...
    fn turn_timer_shown_when_above_one_second() {
        let cfg = config_with_items(vec![TitleItem::TurnTimer, TitleItem::IntelliHelper]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "42s - intellihelper");
    }
    ...
    fn reset_clears_state_and_emits_grok() {
        ...
        mgr.update(&state);
        assert_ne!(mgr.last_title, "intellihelper");
    ...
    fn default_config_active_turn_with_permissions() {
        ...
        for t in [&t1, &t2] {
            assert!(t.contains("intellihelper"), "title missing 'intellihelper': {t}");
    ...
    fn all_items_present_in_order() {
        ...
        assert_eq!(
            mgr.last_title,
            "Thinking - proj - intellihelper-3 - workspace - intellihelper"
        );
    }
