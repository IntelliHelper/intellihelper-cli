...
fn render_tab_bar(
    ...
    for (row_idx, row_indices) in rows.iter().enumerate() {
        ...
        for (local_idx, &tab_idx) in row_indices.iter().enumerate() {
            ...
            // but unreadable as text glyphs on intellihelperday.
...
mod tests {
    ...
    fn centered_tip_footer_centers_and_clips() {
        ...
        let theme = Theme::current();
        const TEXT: &str = "Tip · Ask IntelliHelper about the docs";
        ...
        assert!(wide.contains("Ask IntelliHelper about the docs"));
