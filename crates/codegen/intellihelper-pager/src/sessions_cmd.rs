...
use intellihelper_shell::auth::{AuthManager, try_ensure_fresh_auth};
use intellihelper_shell::session::merge::MergedSession;
...
pub async fn run(args: SessionsArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    // If the user has previously run the interactive `intellihelper` TUI (which succeeds
    ...
    let client = intellihelper_shell::agent::session_registry_client::SessionRegistryClient::new(
    ...
    match args.command {
        SessionsCommand::List { limit } => {
            let sessions = intellihelper_shell::session::merge::fetch_merged(
                Some(&client),
                cwd.to_str(),
                intellihelper_shell::session::merge::CwdScope::WithSiblings,
                None,
                limit,
            )
            .await;
        ...
        SessionsCommand::Search { query, limit } => {
            ...
            use intellihelper_shell::session::merge::REMOTE_TIMEOUT;
            use intellihelper_shell::session::storage::search::{SessionSearchRequest, execute_search};
        ...
        SessionsCommand::Delete { id } => {
            ...
            let deletion = intellihelper_shell::session::persistence::delete_session_history(
                &id,
                None,
                needs_remote,
                auth_manager.clone(),
