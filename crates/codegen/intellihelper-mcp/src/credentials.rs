...
//!
//! Credentials are stored in `$INTELLIHELPER_HOME/mcp_credentials.json`, keyed by a
...
///
/// Local helper (not shell-base): `intellihelper-mcp` sits below `config-types` in the
...
/// File name for the credential store inside `$INTELLIHELPER_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$INTELLIHELPER_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$INTELLIHELPER_HOME/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user intellihelper home (set $INTELLIHELPER_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$INTELLIHELPER_HOME/mcp_credentials.json`.
    fn default_path() -> Option<PathBuf> {
        Some(intellihelper_config::user_intellihelper_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$INTELLIHELPER_HOME/mcp_credentials.json`. Existing credential files must keep
    ...
    fn save_and_load_from_file() {
        let dir = std::env::temp_dir().join("intellihelper-mcp-credentials-test");
