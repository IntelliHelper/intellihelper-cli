//! Shim — see `intellihelper_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `intellihelper_shell::instrumentation_timer!`). Keeping the macro here
...
pub use intellihelper_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `intellihelper_shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `intellihelper_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `intellihelper_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `intellihelper_telemetry::instrumentation`.
#[macro_export]
