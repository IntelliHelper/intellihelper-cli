...
pub(crate) const ENV_CRASH_HANDLER: &str = "INTELLIHELPER_CRASH_HANDLER";
...
/// Precedence: requirements > env (`INTELLIHELPER_CRASH_HANDLER`) >
...
/// Merge system-managed policy (`/etc/intellihelper`) under home `managed_config.toml`
...
fn load_managed_toml_layers() -> Option<TomlValue> {
    ...
    match (system, managed) {
        ...
        (None, Some(m)) => Some(m),
        (Some(mut s), Some(m)) => {
            intellihelper_config::deep_merge_toml(&mut s, &m);
            Some(s)
        }
    }
}
...
mod crash_handler_gate_tests {
    ...
    // `INTELLIHELPER_CRASH_HANDLER` is process-global; serialize and force it unset at
