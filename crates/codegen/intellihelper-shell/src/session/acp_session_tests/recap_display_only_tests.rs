...
fn main_turn_input(items: Vec<ConversationItem>) -> Vec<serde_json::Value> {
    let request = intellihelper_sampling_types::ConversationRequest {
        items,
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            cfg.api_backend = intellihelper_sampling_types::ApiBackend::Responses;
            // Low is not the model default, so a fallback would show up in the assert below.
            cfg.reasoning_effort = Some(intellihelper_sampling_types::ReasoningEffort::Low);
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            let mut cfg = actor.chat_state_handle.get_sampling_config().await.unwrap();
            cfg.base_url = server.url();
            cfg.api_backend = intellihelper_sampling_types::ApiBackend::ChatCompletions;
            actor.chat_state_handle.update_sampling_config(cfg);
        ...
        .run_until(async {
            ...
            while let Ok(msg) = persistence_rx.try_recv() {
                if let PersistenceMsg::RecapRequest(artifact) = msg {
                    ...
                    assert!(
                        artifact.x_intellihelper_req_id.starts_with("xai-recap-"),
                        "req id: {}",
                        artifact.x_intellihelper_req_id
                    );
...
/// shape, so it also covers the laxer intellihelper ChatCompletions/Responses shapes.)
#[test]
fn over_budget_recap_serializes_to_well_formed_messages_request() {
    use crate::session::helpers::session_recap;
    use intellihelper_sampling_types::messages::{ContentBlock, MessageContent, MessageRole};
    use intellihelper_sampling_types::{ConversationRequest, ToolCall, rs};
    ...
    // intellihelper backend => strip_reasoning=false; the over-budget branch strips anyway.
    let items = session_recap::budget_recap_items(conv, "system-reminder", false, 8_000);
    let req = ConversationRequest::from_items(items);
    let msg = intellihelper_sampling_types::build_messages_request(&req);
        ...
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            let conv_id = recap_req
                .header("x-intellihelper-conv-id")
                .expect("recap must send x-intellihelper-conv-id");
            assert!(
...
async fn recap_request_sends_hosted_tools_under_backend_search() {
    use intellihelper_sampling_types::HostedTool;
    use intellihelper_test_support::MockInferenceServer;
        ...
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            // Backend-search fixture: agent carries hosted tools and both gates are on.
            {
                ...
                *agent_slot = intellihelper_agent::Agent::new(
...
async fn recap_hosted_tools_reflect_the_active_per_turn_override() {
    use intellihelper_sampling_types::{HostedTool, SearchDateBound, ToolOverrides, XSearchOptions};
    use intellihelper_test_support::MockInferenceServer;
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            let conv_id = btw_req
                .header("x-intellihelper-conv-id")
                .expect("side question must send x-intellihelper-conv-id");
            assert!(
...
async fn side_question_trims_reasoning_orphaned_by_mid_turn_truncation() {
    use intellihelper_sampling_types::conversation::{AssistantItem, ToolCall};
    use intellihelper_test_support::MockInferenceServer;
