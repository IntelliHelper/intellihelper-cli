...
use intellihelper_test_support::{MockInferenceServer, ScriptedResponse};
...
fn drain_gateway(mut rx: tokio::sync::mpsc::UnboundedReceiver<acp_lib::AcpClientMessage>) {
...
async fn actor_with_mock_sampler(
    ...
    let sampling_cfg = intellihelper_sampler::SamplerConfig {
    ...
    let sampler_handle = intellihelper_sampler::SamplerActor::spawn(
        sampling_cfg,
        intellihelper_sampler::RetryPolicy {
            max_retries: 0,
