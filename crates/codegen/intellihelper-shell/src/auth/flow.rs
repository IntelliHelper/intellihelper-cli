...
use crate::auth::{AuthManager, IntelliHelperAuth, GrokComConfig, parse_output};
use crate::http::TransportFailureKind;
use crate::util::intellihelper_home;
use intellihelper_telemetry::events::{LoginFailed, LoginFailureKind};
...
/// session.
fn is_cached_credential_compatible(auth: &IntelliHelperAuth, grok_com_config: &GrokComConfig) -> bool {
...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("INTELLIHELPER_LOGIN_DEVICE_FLOW")
...
///
/// Precedence: CLI (`--oauth`/`--device-auth`) > `INTELLIHELPER_LOGIN_DEVICE_FLOW` env >
/// `[auth] login_device_flow` config > `intellihelper_build_login_device_flow` remote feature flag > loopback.
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("INTELLIHELPER_LOGIN_DEVICE_FLOW");
...
/// Sets no `INTELLIHELPER_AUTH_EXPIRED`: operator binaries, which live outside this
...
async fn run_external_auth_provider(
...
) -> anyhow::Result<(IntelliHelperAuth, bool)> {
    ...
    // Proper fix: pgid-kill via tty-utils.
...
async fn run_auth_flow_inner(
    ...
    {
        intellihelper_telemetry::session_ctx::log_event(event);
    }
    result
}
...
async fn run_auth_flow_steps(
    ...
    // `intellihelper login --devbox` uses run_devbox_login and is not gated here.
    ...
    // --device-auth / INTELLIHELPER_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
    ...
    anyhow::bail!(
        "No OAuth2 configuration available. Run `intellihelper login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
pub async fn try_ensure_fresh_auth(grok_com_config: &GrokComConfig) -> Option<IntelliHelperAuth> {
...
fn build_startup_auth_manager(grok_com_config: &GrokComConfig) -> Arc<AuthManager> {
    let auth_manager = Arc::new(AuthManager::new(
        &intellihelper_home::intellihelper_home(),
...
async fn try_ensure_fresh_auth_with(auth_manager: &Arc<AuthManager>) -> Option<IntelliHelperAuth> {
    match auth_manager.auth().await {
        Ok(auth) => Some(auth),
        Err(e) => {
...
/// injected for tests.
async fn try_noninteractive_auth_no_mint_with(auth_manager: &Arc<AuthManager>) -> Option<IntelliHelperAuth> {
...
fn expired_refreshable_session(auth_manager: &AuthManager) -> Option<IntelliHelperAuth> {
    auth_manager
        .current_or_expired()
...
async fn persist_or_use_minted(auth_manager: &AuthManager, new_auth: IntelliHelperAuth) -> IntelliHelperAuth {
...
fn report_signed_in(auth: &IntelliHelperAuth) {
    eprint!("\r\x1b[K");
    match auth.email {
...
) -> anyhow::Result<IntelliHelperAuth> {
...
pub async fn ensure_authenticated_with_override(
    ...
    let intellihelper_home = intellihelper_home::intellihelper_home();
    let auth_manager = Arc::new(AuthManager::new(&intellihelper_home, grok_com_config.clone()));
...
pub async fn ensure_authenticated_or_noninteractive(
    ...
    message_prefix: Option<&str>,
) -> anyhow::Result<Option<IntelliHelperAuth>> {
...
/// Unified `intellihelper login` handler for CLI entry points (tui, pager).
///
/// Precedence: `--oauth` forces loopback, `--device-auth` forces device,
/// otherwise `INTELLIHELPER_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub async fn run_cli_login(
    ...
    // client, and `intellihelper login` never boots an agent, so without this every
    ...
    intellihelper_telemetry::session_ctx::drain_pending(CLI_TELEMETRY_DRAIN).await;
    result
}
...
async fn run_cli_login_steps(
    ...
    // supports the device flow. Without this guard, `intellihelper login` on an
...
/// config was actually applied; `intellihelper setup` reports the no-config case.
async fn apply_post_login_config(authenticated: IntelliHelperAuth) -> anyhow::Result<()> {
...
pub struct LogoutResult {
    ...
    /// `true` if `XAI_API_KEY` / `INTELLIHELPER_CODE_XAI_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        intellihelper_telemetry::external::set_identity(
            intellihelper_telemetry::external::IdentityAttrs::default(),
        );
        intellihelper_telemetry::external::flush();
...
/// `intellihelper logout` CLI handler. Calls [`perform_logout`] and formats
/// the result to stderr.
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let intellihelper_home = intellihelper_home::intellihelper_home();
    let auth_manager = AuthManager::new(&intellihelper_home, config.grok_com_config.clone());
...
mod tests {
    ...
    /// `intellihelper-http`; what's local is which `LoginFailureKind` each maps to,
    ...
    /// unrelated auth tests, so classification is covered in `intellihelper-http`.
    #[test]
    ...
    /// Run `f` with `INTELLIHELPER_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("INTELLIHELPER_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("INTELLIHELPER_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("INTELLIHELPER_LOGIN_DEVICE_FLOW"),
        };
        f()
    }

    // A intellihelper.com first-party (x.ai-issuer) OIDC session — `is_xai_auth()` true.
    fn oidc_session(key: &str, refresh: Option<&str>) -> IntelliHelperAuth {
        IntelliHelperAuth {
            key: key.into(),
    ...
    fn expired_refreshable_session_gate() {
        ...
        mgr.hot_swap(IntelliHelperAuth {
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only intellihelper_home: reading a missing auth.json succeeds (empty), but
    ...
    async fn interactive_login_carries_no_expired_flag_even_over_a_stale_credential() {
        let echo_env = "printf '%s' \"e=${INTELLIHELPER_AUTH_EXPIRED:-unset}\"";
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
    ...
    async fn a_provider_written_to_the_published_contract_can_sign_in_after_an_expiry() {
        let conforming =
            r#"if [ "$INTELLIHELPER_AUTH_EXPIRED" = "1" ]; then exit 1; else printf '%s' sso-token; fi"#;
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
            AuthManager::new(dir.path(), GrokComConfig::default())
                .with_proxy_base_url(&dead_proxy_url()),
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("INTELLIHELPER_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("INTELLIHELPER_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `intellihelper login` must use loopback, not device —
    ...
    fn legacy_auth() -> IntelliHelperAuth {
        IntelliHelperAuth {
            key: "k".into(),
    ...
    fn oidc_auth(issuer: &str) -> IntelliHelperAuth {
        IntelliHelperAuth {
    ...
    async fn run_auth_flow_uses_valid_disk_token_when_expired() {
        ...
        let valid_disk = IntelliHelperAuth {
    ...
    async fn run_auth_flow_defers_to_consumer_refresh_on_transient_failure() {
        ...
        let expired_with_rt = IntelliHelperAuth {
            key: "expired-access-token".into(),
            auth_mode: AuthMode::Oidc,
            expires_at: Some(Utc::now() - chrono::Duration::hours(1)),
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        let expired_no_rt = IntelliHelperAuth {
        ...
        // INTELLIHELPER_LOGIN_DEVICE_FLOW / the real config file (the CLI override
    ...
    fn extract_url_from_external_provider_stderr() {
        ...
        assert_eq!(
            extract(
                "Visit the following link to sign into IntelliHelper: https://auth.example.com/login?code=abc"
            ),
    ...
    /// CLI `intellihelper login` passes `on_stderr=None`; stderr must be inherited so
