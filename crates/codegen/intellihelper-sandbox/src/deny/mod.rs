...
pub(crate) fn ancestors_within_writable_roots(
    ...
    let mut out = Vec::new();
    for anc in intellihelper_config::existing_ancestor_chain(path) {
...
pub(crate) fn apply_write_deny_paths_to_capability_set(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        tracing::info!(
            count = entries.len(),
            "Applied Seatbelt write-deny for IntelliHelper-owned direct hook sources"
        );
    }
    #[cfg(target_os = "linux")]
    {
...
mod tests {
    ...
    fn ancestors_pin_under_writable_root_not_home() {
        ...
        let intellihelper = tmp.join("intellihelper");
        let sessions = intellihelper.join("sessions");
        let leaf = sessions.join("extra-hooks");
        std::fs::create_dir_all(&leaf).unwrap();
        let ws = tmp.join("ws");
        std::fs::create_dir_all(&ws).unwrap();

        let roots = [intellihelper.clone(), ws.clone()];
        let pin = ancestors_within_writable_roots(&leaf, &roots);
        assert!(
            pin.iter().any(|p| p == &sessions),
            "must pin sessions under INTELLIHELPER_HOME: {pin:?}"
        );
        assert!(
            pin.iter().any(|p| p == &intellihelper),
            "must pin INTELLIHELPER_HOME grant root: {pin:?}"
        );
        assert!(
