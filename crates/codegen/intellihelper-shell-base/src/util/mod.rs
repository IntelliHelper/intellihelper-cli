pub mod changelog;
pub mod event_id;
pub mod intellihelper_home;
pub mod secure_file;
pub mod tips;
pub mod uname;
pub use intellihelper_shared::clipboard;
pub use intellihelper_shared::stderr::{stderr_lock, with_locked_stderr};
...
/// toggles via `INTELLIHELPER_CLI_CHAT_PROXY_BASE_URL` (or similar) pointing at an
...
/// True if `pid` is an intellihelper process; pairs with [`kill_process_by_pid`] to avoid killing a recycled PID.
/// Best-effort on macOS/BSD (liveness-only via `kill -0`), exact on Linux (/proc cmdline) and Windows (image path).
pub fn is_intellihelper_process(pid: u32) -> bool {
    #[cfg(target_os = "linux")]
    {
        let cmdline_path = format!("/proc/{pid}/cmdline");
        match std::fs::read(&cmdline_path) {
            Ok(data) => String::from_utf8_lossy(&data).contains("intellihelper"),
            Err(_) => false,
        }
    }
    #[cfg(windows)]
...
/// Stricter [`is_intellihelper_process`] for the auto-kill zombie path: on macOS/BSD it
...
/// exactly, so this delegates there. Use the permissive [`is_intellihelper_process`] for
/// operator-driven `intellihelper leaders kill`.
pub fn is_intellihelper_process_strict(pid: u32) -> bool {
    #[cfg(all(not(target_os = "linux"), not(windows)))]
    {
        ...
        match cmd.output() {
            Ok(out) if out.status.success() => {
                ...
                comm.lines()
                    ...
                    .is_some_and(|name| name.to_ascii_lowercase().contains("intellihelper"))
            }
            _ => false,
        }
    }
...
mod tests {
    ...
    #[test]
    fn is_intellihelper_process_self_true_impossible_pid_false() {
        assert!(is_intellihelper_process(std::process::id()));
        assert!(!is_intellihelper_process(u32::MAX));
    }
    #[test]
    fn is_intellihelper_process_strict_self_true_impossible_pid_false() {
        assert!(is_intellihelper_process_strict(std::process::id()));
        assert!(!is_intellihelper_process_strict(u32::MAX));
    }
    #[cfg(unix)]
    #[test]
