...
pub(super) fn default_actions(
    ...
    let mut actions = vec![
        ...
        ActionDef {
            ...
            hint_key_display: Some("Ctrl+Shift+P"),
            requires_confirmation: false,
            long_help: Some(
                "Rewrites the current composer draft into clearer natural-language prose for a coding agent (async, Qoder/Roo-style).\nDoes not send the message — only replaces the draft in place.\nUses light workspace identity always; attaches git status/diff only for change/branch drafts so unrelated asks are not rewritten as \"summarize local changes\". After success, use Undo optimize to restore the previous draft.\nEmpty drafts are a no-op. Also available as `/optimize-input` or the sparkle on the model · mode line.",
            ),
        },
        ActionDef {
            id: ActionId::UndoOptimizeInput,
            label: "undo-optimize",
            description: "Undo optimize",
            default_key: key!('z', CONTROL | SHIFT),
            alt_keys: vec![],
            category: Category::Input,
            context: When::PromptFocused,
            hint_priority: None,
            hint_key_display: Some("Ctrl+Shift+Z"),
            requires_confirmation: false,
            long_help: Some(
                "Restores the composer draft from before the last successful Optimize input.\nNo-op when there is nothing to undo.",
            ),
        },
        ActionDef {
    ...
    actions.extend([
        ...
        // rebind any of them through `~/.intellihelper/config.toml`.
        ActionDef {
