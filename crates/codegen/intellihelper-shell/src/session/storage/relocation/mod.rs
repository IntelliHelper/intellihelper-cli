...
impl RelocationStorage {
    pub(crate) fn new(intellihelper_home: PathBuf) -> Self {
        Self {
            intellihelper_home,
            #[cfg(test)]
            fault: None,
    ...
    #[cfg(test)]
    fn with_fault(intellihelper_home: PathBuf, fault: TestFault) -> Self {
        Self {
            intellihelper_home,
            fault: Some(fault),
        }
    ...
    pub(crate) fn acquire(&self, session_id: &str) -> Result<RelocationLease> {
        journal::acquire(&self.intellihelper_home, session_id)
    }

    pub(crate) fn read_journal(&self, session_id: &str) -> Result<RelocationJournal> {
        journal::read(&self.intellihelper_home, session_id)
    }

    fn write_journal(&self, journal: &RelocationJournal) -> std::result::Result<(), WriteFailure> {
        journal::write(&self.intellihelper_home, journal, self.journal_fault(journal.phase))
    }
    ...
    pub(crate) fn sync_journal_namespace(&self) -> Result<()> {
        ...
        journal::sync_namespace(&self.intellihelper_home)
    }
    ...
    pub(crate) fn stage_and_publish(
        ...
        journal.validate(&self.intellihelper_home)?;
        let source =
            journal::session_dir_at(&self.intellihelper_home, &journal.source_cwd, &journal.session_id);
    ...
    pub(crate) fn recover_all(&self) -> Result<()> {
        for session_id in RelocationView::journal_ids(&self.intellihelper_home)? {
    ...
    pub(crate) fn finalize_terminal(
        ...
        proof.journal.validate(&self.intellihelper_home)?;
        ...
        let path = journal::journal_path(&self.intellihelper_home, &lease.session_id);
    ...
    fn finish_rollback(&self, journal: &mut RelocationJournal) -> Result<TerminalRelocation> {
        ...
        let result = (|| {
            ...
            self.remove_transaction_directory(&self.staging_dir(journal))?;
            let target =
                journal::session_dir_at(&self.intellihelper_home, &journal.target_cwd, &journal.session_id);
            if target.exists() {
                self.validate_target(journal)?;
            }
    ...
    fn validate_request(&self, lease: &RelocationLease, request: &RelocationRequest) -> Result<()> {
        ...
        if journal::session_dir_at(&self.intellihelper_home, &request.source_cwd, &request.session_id)
            == journal::session_dir_at(&self.intellihelper_home, &request.target_cwd, &request.session_id)
        {
            return Err(RelocationError::Inconsistent(
    ...
    pub(super) fn validate_authoritative_dir(
        ...
        if path
            != journal::session_dir_at(&self.intellihelper_home, expected_cwd, &journal.session_id).as_path()
        {
            return Err(RelocationError::Inconsistent(
    ...
    fn target_parent(&self, cwd: &str) -> PathBuf {
        self.intellihelper_home
            .join("sessions")
            .join(intellihelper_config::encode_cwd_dirname(cwd))
    }

    fn ensure_target_parent(&self, cwd: &str) -> Result<PathBuf> {
        let sessions = self.intellihelper_home.join("sessions");
        fs::create_dir_durable(&sessions)?;
        let encoded = intellihelper_config::encode_cwd_dirname(cwd);
    ...
    fn staging_dir(&self, journal: &RelocationJournal) -> PathBuf {
        self.intellihelper_home
            .join("sessions")
            .join(intellihelper_config::encode_cwd_dirname(&journal.target_cwd))
