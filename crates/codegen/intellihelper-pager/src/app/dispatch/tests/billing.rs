...
use super::*;
use intellihelper_shell::sampling::error::is_free_usage_exhausted_error;
...
fn is_max_tier_positive_match() {
    ...
    assert!(is_max_tier(Some("SUPERINTELLIHELPER_HEAVY")));
}
...
fn upsell_non_max_upgrade_url_is_supergrok() {
    ...
    assert!(url.contains("referrer=intellihelper-build"), "got: {url}");
}
...
fn upsell_non_max_unified_shows_buy_credits() {
    ...
    assert_eq!(
        ...
        "Purchase credits to keep using IntelliHelper CLI"
    );
}
...
fn is_credit_limit_error_matches_legacy_403_and_pool_402() {
    ...
    assert!(is_credit_limit_error(
        None,
        "API error (status 402 Payment Required): IntelliHelper CLI usage balance exhausted"
    ));
    assert!(is_credit_limit_error(
        None,
        "status 403: run out of credits"
...
fn complete_session_usage(
    app: &mut AppView,
    session_id: &str,
    usage: intellihelper_shell::extensions::notification::PromptUsage,
) -> Vec<Effect> {
    dispatch(
...
#[serial_test::serial(INTELLIHELPER_TEST_OPEN_URL_FILE)]
#[test]
fn manage_billing_gates_on_consumer_billing_surface() {
    let out = std::env::temp_dir().join(format!("intellihelper-manage-billing-{}.txt", std::process::id()));
    let _ = std::fs::remove_file(&out);
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("INTELLIHELPER_TEST_OPEN_URL_FILE", &out) };
    let mut app = test_app_with_agent();
    dispatch(Action::ManageBilling, &mut app);
    let opened = std::fs::read_to_string(&out).unwrap_or_default();
    assert!(opened.contains("intellihelper.com/?_s=usage"), "got: {opened}");
    let _ = std::fs::remove_file(&out);
...
fn session_usage_complete_pushes_block_and_chains_billing() {
    let mut app = test_app_with_agent();
    let before = agent_scrollback_len(&app);
    let usage = intellihelper_shell::extensions::notification::PromptUsage {
        totals: intellihelper_shell::extensions::notification::PromptUsageModel {
            input_tokens: 1_000,
            output_tokens: 100,
            total_tokens: 1_100,
            model_calls: 3,
...
fn session_usage_complete_drops_stale_session() {
    ...
    let effects = complete_session_usage(
        &mut app,
        "old-session",
        intellihelper_shell::extensions::notification::PromptUsage {
            totals: intellihelper_shell::extensions::notification::PromptUsageModel {
                model_calls: 99,
...
fn free_usage_upsell_shows_two_options_with_exact_labels() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: intellihelper_telemetry::events::SuperGrokUpsell::FreeUsagePaywall,
            }
        )
    ));
    ...
    let expected = [
        ...
        (
            ...
            "Get the most out of IntelliHelper CLI. Highest usage limits.",
            Some(UPSELL_URL_UPGRADE),
        ),
    ];
...
fn free_usage_failure_opens_paywall_modal() {
    ...
    use intellihelper_shell::extensions::notification::{RetryState, SessionUpdate};
...
fn restricted_command_submit_opens_two_option_upsell() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: intellihelper_telemetry::events::SuperGrokUpsell::RestrictedCommand,
            }
        )
    ));
    let q = &qv.questions[0];
...
/// `INTELLIHELPER_TEST_OPEN_URL_FILE` seam), `Action::OpenUrl` for a billing CTA
...
fn open_url_shows_manual_url_when_browser_unavailable() {
    ...
    let bad = std::env::temp_dir().join(format!(
        "intellihelper-open-url-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("INTELLIHELPER_TEST_OPEN_URL_FILE", &bad) };
    ...
    unsafe { std::env::remove_var("INTELLIHELPER_TEST_OPEN_URL_FILE") };
}
...
fn open_url_does_not_show_fallback_when_opener_succeeds() {
    let url_file =
        std::env::temp_dir().join(format!("intellihelper-open-url-ok-{}.txt", std::process::id()));
    let _ = std::fs::remove_file(&url_file);
    // SAFETY: serialized via `serial_test`.
    unsafe { std::env::set_var("INTELLIHELPER_TEST_OPEN_URL_FILE", &url_file) };
...
fn open_url_welcome_toasts_single_line_url_when_browser_unavailable() {
    let bad = std::env::temp_dir().join(format!(
        "intellihelper-open-url-welcome-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("INTELLIHELPER_TEST_OPEN_URL_FILE", &bad) };
...
fn credit_limit_upsell_submit_shows_url_when_browser_unavailable() {
    ...
    let bad = std::env::temp_dir().join(format!(
        "intellihelper-open-url-upsell-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test`.
    unsafe { std::env::set_var("INTELLIHELPER_TEST_OPEN_URL_FILE", &bad) };
    ...
    let kind = LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            intellihelper_telemetry::events::CreditLimitChoice::UpgradeTier,
            intellihelper_telemetry::events::CreditLimitChoice::PurchaseCredits,
        ],
    };
