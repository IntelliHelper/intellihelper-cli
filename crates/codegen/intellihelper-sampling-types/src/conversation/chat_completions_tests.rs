...
fn test_conversation_item_roundtrip() {
    ...
    let assistant = ConversationItem::assistant_with_model("Hi there!", "intellihelper-3");
...
fn test_conversation_request_to_chat_completion() {
    ...
    assert_eq!(chat_req.model, Some("intellihelper-3".to_string()));
...
fn test_assistant_with_content_and_tool_calls() {
    ...
    assert_eq!(chat_msg.tool_calls.len(), 1);
    assert_eq!(chat_msg.model_id, Some("intellihelper-3".to_string()));
}
