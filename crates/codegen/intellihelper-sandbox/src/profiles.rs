...
//! `off`. Custom profiles via `~/.intellihelper/sandbox.toml` or `.intellihelper/sandbox.toml`.
...
/// Load sandbox config from `~/.intellihelper/sandbox.toml` and `.intellihelper/sandbox.toml`.
///
...
pub fn load_sandbox_config(workspace: &Path) -> SandboxConfig {
    ...
    // Global config: ~/.intellihelper/sandbox.toml
    let global_path = intellihelper_home().join("sandbox.toml");
    if let Some(global) = load_config_file(&global_path) {
        config = global;
    }

    // Project config: <workspace>/.intellihelper/sandbox.toml (additive only)
    let project_path = workspace.join(".intellihelper").join("sandbox.toml");
...
pub fn sandbox_profile_conflicts(workspace: &Path) -> Vec<String> {
    let global = load_config_file(&intellihelper_home().join("sandbox.toml")).unwrap_or_default();
    let project =
        load_config_file(&workspace.join(".intellihelper").join("sandbox.toml")).unwrap_or_default();
...
impl ProfileName {
    ...
    pub(crate) fn capability_set_from_profile(
        ...
        // directories like ~/.intellihelper/ that may not exist on first run.
        ...
        if !profile.write_deny.is_empty() {
            ...
            {
                let files =
                    intellihelper_config::validated_hook_json_files_for_sources(&profile.write_deny)
    ...
    fn resolve(&self, workspace: &Path, config: &SandboxConfig) -> anyhow::Result<SandboxProfile> {
        match self {
            ...
            Self::Strict => {
                ...
                .chain(std::iter::once(intellihelper_home()))
                .collect();
                ...
                let profile_config = config.profiles.get(name).ok_or_else(|| {
                    anyhow::anyhow!(
                        "Custom sandbox profile '{name}' not found. \
                         Define it in ~/.intellihelper/sandbox.toml or .intellihelper/sandbox.toml:\n\n\
                         [profiles.{name}]\n\
                         extends = \"workspace\"\n\
                         read_only = [\"/data\"]\n"
                    )
...
mod tests {
    ...
    /// Hosts with a retargetable `$INTELLIHELPER_HOME/hooks` symlink (fail-closed under
...
"#;
    ...
    fn enxio_device_file_is_skipped_but_directory_is_not() {
        ...
        let dir = std::env::temp_dir().join(format!("intellihelper-sbx-test-{}", std::process::id()));
