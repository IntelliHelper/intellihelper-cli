...
use intellihelper_tools::implementations::skills::types::SkillInfo;
...
fn is_official_marketplace_source(source: &hooks_plugins_types::MarketplaceScanResult) -> bool {
    source.source_name == intellihelper_plugin_marketplace::OFFICIAL_SOURCE_NAME
        || intellihelper_plugin_marketplace::is_official_source_url(&source.source_url_or_path)
}
...
pub fn plugin_group(plugin: &hooks_plugins_types::PluginInfo) -> PluginGroup {
    ...
    match &plugin.origin {
        ...
        Some(PluginOrigin::MarketplaceInstall {
            ...
            rank: 5,
            key: format!("intellihelper-mp:{source}"),
            label: source.clone(),
        },
        Some(PluginOrigin::MarketplaceInstall {
            source_name: None, ..
...
impl ExtensionsTab {
    ...
    pub fn telemetry_tab(self) -> intellihelper_telemetry::events::ExtensionsModalTab {
        use intellihelper_telemetry::events::ExtensionsModalTab;
        match self {
...
pub enum ButtonAction {
    ...
    AddMcpServer {
        name: String,
        config: Box<intellihelper_shell::util::config::McpServerConfig>,
    },
    ...
    RefreshMcpList,
    /// Open intellihelper.com connectors page (MCP tab: press `o`).
...
fn classify_hook_source(source_dir: &str) -> HookSourceMeta {
    let intellihelper = intellihelper_config::intellihelper_home();
    let source_path = std::path::Path::new(source_dir);
    // Plugin / installed-plugin dirs, under the user intellihelper home (INTELLIHELPER_HOME-aware)
    // or a project-scoped `{cwd}/.intellihelper/<subdir>/`. Returns the first path
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        // User intellihelper home (INTELLIHELPER_HOME-aware).
        if let Ok(rest) = source_path.strip_prefix(intellihelper.join(subdir))
            && let Some(name) = first_comp(rest)
        {
            return Some(name);
        }
        // Project-scoped `.intellihelper/<subdir>/<name>` anywhere in the path.
        ...
        comps
            .windows(3)
            .find(|w| w[0] == ".intellihelper" && w[1] == subdir && !w[2].is_empty())
            .map(|w| w[2].clone())
    };
    ...
    // Global hooks under $INTELLIHELPER_HOME/hooks
    let global_hooks = intellihelper.join("hooks");
    ...
    // Project hooks
    if source_dir.ends_with("/.intellihelper/hooks") || source_dir.contains("/.intellihelper/hooks/") {
        return HookSourceMeta {
    ...
    let display = {
        if let Ok(rest) = source_path.strip_prefix(&intellihelper) {
            let prefix = crate::util::display_intellihelper_home_prefix();
...
fn skill_source_str(skill: &SkillInfo) -> String {
    if let Some(ref cs) = skill.config_source {
        match cs {
            intellihelper_tools::types::config_source::ConfigSource::User { path } => {
                if crate::util::is_under_user_intellihelper_home(path) {
                    crate::util::display_user_intellihelper_path("skills")
            ...
            intellihelper_tools::types::config_source::ConfigSource::Project { path } => {
                let s = path.display().to_string();
                if s.contains("/.intellihelper/") {
                    ".intellihelper/skills".into()
            ...
            intellihelper_tools::types::config_source::ConfigSource::Plugin { plugin_name, .. } => {
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.intellihelper/plugins/<name>/` must label as
        // a (non-removable) plugin, not a removable "Custom" source. The user
        // intellihelper-home branch is INTELLIHELPER_HOME-aware; this covers the project fallback.
        let (label, is_custom) = derive_source_label("/repo/work/.intellihelper/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.intellihelper/installed-plugins/vendor-abc123/skills");
    ...
    fn mcp_collapsed_managed_section_omits_server_rows() {
        ...
        assert!(
            rows.labels
                .iter()
                .any(|l| l.starts_with("Managed by intellihelper.com")),
    ...
    fn make_skill(
        ...
        intellihelper_tools::implementations::skills::types::SkillInfo {
            ...
            metadata: None,
            path: "test".to_string(),
            scope: intellihelper_tools::implementations::skills::types::SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
            plugin_root: None,
    ...
    fn make_plugin_skill(
        ...
        skill.scope = intellihelper_tools::implementations::skills::types::SkillScope::Plugin;
    ...
    fn mcp_add_url_derives_name() {
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                assert!(matches!(
                    config.transport,
                    intellihelper_shell::util::config::McpServerTransportConfig::StreamableHttp { .. }
                ));
            }
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                match config.transport {
                    intellihelper_shell::util::config::McpServerTransportConfig::Stdio {
                        command,
                        args,
                        ..
                    } => {
    ...
    fn plugin_group_fallback_without_origin() {
        ...
        assert_eq!(group.key, "intellihelper-mp:xAI Official");
    ...
    fn plugin_group_unknown_origin_uses_scope_fallback() {
        ...
        assert_eq!(plugin_group(&unknown).key, "intellihelper-mp:xAI Official");
    }
    ...
    fn skills_groups_then_az_by_label() {
        ...
        project_z.scope = intellihelper_tools::implementations::skills::types::SkillScope::Local;
        project_z.config_source = Some(
            intellihelper_tools::types::config_source::ConfigSource::Project {
                path: std::path::PathBuf::from("/repo/.intellihelper/skills/zzz"),
            },
        );
        project_z.display_name = Some("zeta-proj".into());

        let mut project_a = make_skill("aaa-proj", "other project");
        project_a.scope = intellihelper_tools::implementations::skills::types::SkillScope::Repo;
        project_a.config_source = Some(
            intellihelper_tools::types::config_source::ConfigSource::Project {
                path: std::path::PathBuf::from("/repo/.intellihelper/skills/aaa"),
            },
        );
    ...
    fn hooks_group_and_row_order() {
        ...
        let hooks = vec![
            ...
            h_stop,
            make_hook("a", "/repo/.intellihelper/hooks", false),
            h_pre,
            h_notify,
