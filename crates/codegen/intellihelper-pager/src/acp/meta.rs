...
//! intellihelper-shell hasn't been updated or meta is absent.
...
pub struct NotificationMeta {
    ...
    /// (`"{sessionId}-{counter}"`, see `intellihelper-shell util::event_id`). The
...
/// replayed notifications (`_meta.isReplay`, stamped by intellihelper-shell's
...
pub mod user_prompt_meta {
    ...
    /// See [`intellihelper_prompt_queue::COMBINED_DISPLAY_TEXTS_META`].
    pub const COMBINED_DISPLAY_TEXTS: &str = intellihelper_prompt_queue::COMBINED_DISPLAY_TEXTS_META;
}
...
pub mod user_message_chunk_meta {
    ...
    /// ([`intellihelper_shell::session::PromptOrigin::hide_user_echo_from_scrollback`]).
...
mod tests {
    ...
    fn parse_missing_new_fields() {
        // Simulate old intellihelper-shell that doesn't send streamStartMs/turnStartMs
        let meta_json = json!({
            "totalTokens": 1000u64,
            "agentTimestampMs": 1700000000000i64,
        });
