...
mod tests {
    ...
    fn workflow_script_header_hides_rhai_path() {
        let theme = Theme::current();
        let block = EditToolCallBlock::new(".intellihelper/workflows/cc-deep-research.rhai", vec![]);
        let header = block.header_line(
            &theme,
            false,
            false,
        ...
        let block =
            EditToolCallBlock::new(".intellihelper/workflows/triage.rhai", vec![]).with_prefix("Creating ");
        let header = block.header_line(
            &theme,
            false,
            false,
    ...
    fn test_diff_reflow_preserves_banded_syntect_styles_across_wrap() {
        let _guard = pin_intellihelpernight_syntect();
        let theme = Theme::intellihelpernight();
        let config = DiffRenderConfig::default();
    ...
    /// Pins IntelliHelperNight via the shared test-lock guard; hold it for the whole
    /// test so a concurrent theme flip can't skew the compared highlighter walks.
    fn pin_intellihelpernight_syntect() -> std::sync::MutexGuard<'static, ()> {
        let guard = crate::theme::cache::pin_theme();
        assert!(
            !Theme::intellihelpernight().diff_uses_line_fg(),
            "IntelliHelperNight must be banded for Equal-line syntect path in render"
        );
        guard
    }
    ...
    /// Caller pins the theme via `pin_intellihelpernight_syntect` (lock is not reentrant).
...
"
    ...
    fn file_scoped_stale_theme_falls_back_to_hunk_only() {
        ...
        let stale = ThemeKind::IntelliHelperDay;
