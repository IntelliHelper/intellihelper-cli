...
/// auth.json scope key for plain API key auth (desktop login, `intellihelper login --api-key`).
...
/// server enrichment opts in. Single source for `IntelliHelperAuth`, `AuthMeta`, and
...
pub enum AuthMode {
    ...
    External,
    /// Plain API key (e.g. from intellihelper-desktop login or `intellihelper login --api-key`)
    ApiKey,
}
...
pub struct IntelliHelperAuth {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub has_intellihelper_code_access: Option<bool>,
...
impl std::fmt::Debug for IntelliHelperAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("IntelliHelperAuth")
...
impl IntelliHelperAuth {
    ...
    /// `true` when this auth can access intellihelper.com managed MCP connectors.
    ...
    pub(crate) fn carry_user_profile_from(&mut self, prev: &IntelliHelperAuth) {
...
impl Default for IntelliHelperAuth {
    fn default() -> Self {
        Self {
            key: String::new(),
...
#[cfg(test)]
impl IntelliHelperAuth {
    /// Returns a `IntelliHelperAuth` with sensible defaults for tests. Override fields
    /// with struct update syntax:
    /// ```ignore
    /// IntelliHelperAuth { key: "my-key".into(), ..IntelliHelperAuth::test_default() }
    /// ```
...
pub(crate) type AuthStore = BTreeMap<String, IntelliHelperAuth>;
...
///
/// Legacy `WebLogin` tokens (from the pre-OIDC `intellihelper login --legacy`
...
pub fn lookup_auth(map: &AuthStore, scope: &str) -> Option<IntelliHelperAuth> {
...
/// Early-invalidation buffer. Override with `INTELLIHELPER_AUTH_EARLY_INVALIDATION_SECS`
/// for testing (e.g. `=5` to shrink the buffer to 5 seconds).
pub(super) fn early_invalidation() -> Duration {
    std::env::var("INTELLIHELPER_AUTH_EARLY_INVALIDATION_SECS")
        .ok()
...
pub(crate) fn is_expired(auth: &IntelliHelperAuth) -> bool {
...
pub(crate) fn is_expired_with_buffer(auth: &IntelliHelperAuth, buffer: Duration) -> bool {
...
mod tests {
    ...
    fn make_auth(mode: AuthMode) -> IntelliHelperAuth {
        IntelliHelperAuth {
            key: "k".into(),
            auth_mode: mode,
            create_time: Utc::now(),
    ...
    fn is_xai_auth_matrix() {
        ...
        let with_issuer = |mode: AuthMode, issuer: Option<&str>| IntelliHelperAuth {
    ...
    fn missing_coding_data_retention_opt_out_deserializes_opted_out() {
        ...
        let auth: IntelliHelperAuth = serde_json::from_str(json).unwrap();
        assert!(
            auth.coding_data_retention_opt_out,
            "missing field must default to opted-out"
        );
        assert!(default_coding_data_retention_opt_out());
        assert!(IntelliHelperAuth::default().coding_data_retention_opt_out);
    }
}
