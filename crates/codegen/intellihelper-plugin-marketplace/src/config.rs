//! Parse marketplace sources from `~/.intellihelper/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
...
/// `INTELLIHELPER_MARKETPLACE_REQUIRE_SHA=1`. Tighten-only: either source can enable,
...
pub fn env_require_sha() -> bool {
    intellihelper_config::env_bool("INTELLIHELPER_MARKETPLACE_REQUIRE_SHA").unwrap_or(false)
}
...
/// and `known_marketplaces.json` files under `~/.intellihelper/` and `~/.claude/`.
pub fn load_extra_sources_from_settings(existing: &[MarketplaceSource]) -> Vec<MarketplaceSource> {
    let roots: Vec<PathBuf> = [
        intellihelper_config::user_intellihelper_home(),
...
/// instead of `~/.intellihelper`/`~/.claude`. Each root is checked for
...
mod tests {
    ...
    fn require_sha_policy_composition() {
        ...
        unsafe { std::env::remove_var("INTELLIHELPER_MARKETPLACE_REQUIRE_SHA") };
        assert!(!load_require_sha(&empty), "absent everywhere → off");
        assert!(load_require_sha(&enabled), "config alone can enable");

        unsafe { std::env::set_var("INTELLIHELPER_MARKETPLACE_REQUIRE_SHA", "1") };
        assert!(load_require_sha(&empty), "env alone can enable");

        unsafe { std::env::set_var("INTELLIHELPER_MARKETPLACE_REQUIRE_SHA", "0") };
        assert!(
