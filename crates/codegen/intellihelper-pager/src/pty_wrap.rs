//! Local PTY wrapper: the engine behind `intellihelper wrap` (see [`crate::wrap_cmd`]).
//!
...
fn apply_wrap_child_env(
    ...
    cmd.env("INTELLIHELPER_OSC52_SINK", "1");
    cmd.env("LC_INTELLIHELPER_OSC52_SINK", "1");
    if let Some(appearance) = appearance {
        let value = appearance.as_env_value();
        cmd.env("INTELLIHELPER_APPEARANCE", value);
        cmd.env("LC_INTELLIHELPER_APPEARANCE", value);
    }
}

/// Run an arbitrary command inside a local PTY with OSC 52 output filtering.
///
/// This is the engine behind `intellihelper wrap`: it spawns
...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // detached — `intellihelper wrap` is short-lived and exits with the child.
...
mod tests {
    ...
    fn apply_wrap_child_env_dark_overrides_parent_light_on_both_names() {
        ...
        cmd.env("INTELLIHELPER_APPEARANCE", "light");
        cmd.env("LC_INTELLIHELPER_APPEARANCE", "light");
        apply_wrap_child_env(&mut cmd, Some(SystemAppearance::Dark));
        assert_eq!(env_str(&cmd, "INTELLIHELPER_APPEARANCE").as_deref(), Some("dark"));
        assert_eq!(env_str(&cmd, "LC_INTELLIHELPER_APPEARANCE").as_deref(), Some("dark"));
        assert_eq!(env_str(&cmd, "INTELLIHELPER_OSC52_SINK").as_deref(), Some("1"));
        assert_eq!(env_str(&cmd, "LC_INTELLIHELPER_OSC52_SINK").as_deref(), Some("1"));
    }

    #[test]
    fn apply_wrap_child_env_light_overrides_parent_dark_on_both_names() {
        let mut cmd = portable_pty::CommandBuilder::new("true");
        cmd.env("INTELLIHELPER_APPEARANCE", "dark");
        cmd.env("LC_INTELLIHELPER_APPEARANCE", "dark");
        apply_wrap_child_env(&mut cmd, Some(SystemAppearance::Light));
        assert_eq!(env_str(&cmd, "INTELLIHELPER_APPEARANCE").as_deref(), Some("light"));
        assert_eq!(
            env_str(&cmd, "LC_INTELLIHELPER_APPEARANCE").as_deref(),
            Some("light")
        );
    }
    ...
    fn apply_wrap_child_env_none_does_not_stamp_from_parent_snapshot() {
        let mut cmd = portable_pty::CommandBuilder::new("true");
        cmd.env("INTELLIHELPER_APPEARANCE", "dark");
        cmd.env_remove("LC_INTELLIHELPER_APPEARANCE");
        apply_wrap_child_env(&mut cmd, None);
        assert_eq!(env_str(&cmd, "INTELLIHELPER_APPEARANCE").as_deref(), Some("dark"));
        assert_eq!(env_str(&cmd, "LC_INTELLIHELPER_APPEARANCE"), None);
        assert_eq!(env_str(&cmd, "INTELLIHELPER_OSC52_SINK").as_deref(), Some("1"));
    }
}
