...
pub(crate) trait AuthSnapshot: Send + Sync {
    ...
    fn current(&self) -> Option<IntelliHelperAuth>;
    /// Read the expired in-memory bearer (for its `refresh_token`).
    fn expired_auth(&self) -> Option<IntelliHelperAuth>;
    ...
    fn read_disk_auth(&self) -> Option<IntelliHelperAuth>;
...
pub(crate) trait ExternalCommandRunner: Send + Sync {
    ...
    async fn run_external_command(&self, command: &str) -> Option<IntelliHelperAuth>;
}
...
impl RefreshOutcome {
    ...
    pub(crate) fn success(auth: IntelliHelperAuth) -> Self {
    ...
    /// flow). Any refresher holding the [`IntelliHelperAuth`] it sent must use
    ...
    ///
    /// Prefer this wherever the attempted [`IntelliHelperAuth`] is in hand: it captures
...
mod tests {
    use super::*;
    use crate::auth::{AuthMode, IntelliHelperAuth, GrokComConfig};
    ...
    fn token_ttl_expires_external_token_by_create_time() {
        ...
        let old_token = IntelliHelperAuth {
        ...
        let new_token = IntelliHelperAuth {
