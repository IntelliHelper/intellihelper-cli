...
//! are written to `.intellihelper/config.toml`.
...
use intellihelper_shell::claude_import::{ImportPlan, ImportableItem, PathKind, find_project_root};
use intellihelper_workspace::permission::types::RuleAction;
...
fn build_rows(
    ...
    if !plan.global_items.is_empty() {
        ...
        let label = "Global  ~/.intellihelper/config.toml".to_string();
    ...
    if !plan.project_items.is_empty() {
        ...
        let project_config = find_project_root(cwd).join(".intellihelper").join("config.toml");
...
mod tests {
    use super::*;
    use intellihelper_shell::claude_import::PathKind;
    use intellihelper_workspace::permission::types::{PatternMode, PermissionRule, ToolFilter};
    ...
    fn filtered_plan_respects_per_item_selection_after_grouping() {
        use intellihelper_shell::claude_import::ImportableItem;
