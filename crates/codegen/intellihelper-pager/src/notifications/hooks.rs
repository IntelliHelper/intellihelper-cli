...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("INTELLIHELPER_EVENT", event_str)
        .env("INTELLIHELPER_MESSAGE", message)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    if let Some(sid) = session_id {
        cmd.env("INTELLIHELPER_SESSION_ID", sid);
    }
...
mod tests {
    ...
    fn sets_environment_variables() {
        ...
        let command = format!(
            "printf 'INTELLIHELPER_EVENT=%s\\nINTELLIHELPER_MESSAGE=%s\\nINTELLIHELPER_SESSION_ID=%s\\n' \
             \"$INTELLIHELPER_EVENT\" \"$INTELLIHELPER_MESSAGE\" \"$INTELLIHELPER_SESSION_ID\" > {}",
            out.display()
        );
        ...
        assert!(
            content.contains("INTELLIHELPER_EVENT=Turn complete"),
            "missing INTELLIHELPER_EVENT: {content}"
        );
        assert!(
            content.contains("INTELLIHELPER_MESSAGE=hello world"),
            "missing INTELLIHELPER_MESSAGE: {content}"
        );
        assert!(
            content.contains("INTELLIHELPER_SESSION_ID=sess-42"),
            "missing INTELLIHELPER_SESSION_ID: {content}"
        );
    }
    ...
    fn omits_session_id_when_none() {
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(
            !content.contains("INTELLIHELPER_SESSION_ID"),
            "INTELLIHELPER_SESSION_ID should not be set: {content}"
        );
    }
    ...
    fn run_hook_passes_correct_env_via_thread() {
        ...
        assert!(content.contains("INTELLIHELPER_EVENT=Turn complete"));
        assert!(content.contains("INTELLIHELPER_MESSAGE=test body payload"));
        assert!(content.contains("INTELLIHELPER_SESSION_ID=test-session-123"));
    }
}
