...
use crate::auth::model::{IntelliHelperAuth, UserInfo, lookup_auth};
...
pub(super) fn spawn(manager: Arc<AuthManager>, auth: IntelliHelperAuth) {
...
pub(super) async fn enrich_inline(manager: &AuthManager, auth: &mut IntelliHelperAuth) {
...
async fn run_user_info_enrichment(manager: &AuthManager, auth: IntelliHelperAuth) {
    let started = std::time::Instant::now();
    ...
    if disk.key != auth.key || disk.refresh_token != auth.refresh_token {
        intellihelper_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                ...
                "written_key_prefix": intellihelper_auth::bearer_suffix(&auth.key),
                "disk_key_prefix": intellihelper_auth::bearer_suffix(&disk.key),
            })),
        );
        return;
    }
...
pub(super) fn apply_user_info_enrichment(disk: &mut IntelliHelperAuth, user_info: UserInfo) {
