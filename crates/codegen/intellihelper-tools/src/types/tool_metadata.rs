//! `ToolMetadata` — intellihelper-tools-specific metadata for tools.
//!
...
/// IntelliHelper-tools-specific metadata trait.
///
...
pub trait ToolMetadata: Send + Sync {
    ...
    /// Namespace grouping (IntelliHelperBuild, Cursor, OpenCode, ...).
    /// Used to build the fully-qualified tool ID at registration time
    /// (e.g., `"IntelliHelperBuild:grep"`).
