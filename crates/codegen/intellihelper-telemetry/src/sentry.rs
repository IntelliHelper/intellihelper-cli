...
pub struct Config {
    /// Sentry tag `client`, e.g. `"intellihelper-pager"`.
...
impl Scrubber {
    ...
    fn scrub(&self, s: &str) -> String {
        let out = intellihelper_secrets::redact_secrets(s);
    ...
    fn scrub_value(&self, val: &mut Value) {
        intellihelper_secrets::walk_json_strings(val, &mut |s| *s = self.scrub(s));
    }
}
