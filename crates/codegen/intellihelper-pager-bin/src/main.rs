...
use intellihelper_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use intellihelper_pager::client_identity::PAGER_CLIENT_VERSION;
use intellihelper_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use intellihelper_shell::agent::config::Config as AgentConfig;
use intellihelper_shell::leader::{
...
use intellihelper_update::{UpdateConfig, auto_update, enforce_version_policy_or_exit};
...
fn apply_headless_args_to_config(args: &HeadlessArgs, config: &mut AgentConfig) {
    if let Some(v) = &args.intellihelper_ws_origin {
        config.grok_com_config.intellihelper_ws_origin = v.clone();
    }
    if let Some(v) = &args.intellihelper_ws_url {
        config.grok_com_config.intellihelper_ws_url = v.clone();
    }
}
/// Apply global endpoint CLI args to an existing config.
fn apply_agent_endpoint_args(
    agent_args: &intellihelper_pager::app::AgentArgs,
...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   IntelliHelper agent server starting...");
    eprintln!();
...
/// Entrypoint tag for `intellihelper -p`; keys the quiet stderr default in `init_tracing_simple`.
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    use intellihelper_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(intellihelper_telemetry::sampling_log::layer())
        .with(intellihelper_telemetry::instrumentation::layer())
        .with(intellihelper_telemetry::hooks_log::layer())
        .with(intellihelper_telemetry::otel_layer::build_otel_layer(
            intellihelper_telemetry::otel_layer::OtelClientInfo {
                client_name: "intellihelper-pager",
                client_version: intellihelper_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            intellihelper_shell::auth::credential_provider::build_default_otel_layer_config(),
        ));
    intellihelper_telemetry::debug_log::install_firehose(registry, app_entrypoint);
    intellihelper_telemetry::external::init(
        intellihelper_shell::agent::config::resolve_external_otel_config(
            intellihelper_telemetry::external::config::ExternalClientInfo {
                service_version: env!("VERSION_WITH_COMMIT").to_owned(),
                client_version: intellihelper_version::VERSION.to_owned(),
                app_entrypoint: app_entrypoint.to_owned(),
            },
        ),
    );
}
/// `intellihelper setup`: rendering + exit codes only; fetch logic lives in `intellihelper_shell::managed_config`.
/// `json` prints the served configuration instead of installing it.
async fn run_setup_command(json: bool) {
    use intellihelper_shell::managed_config::{self, SetupOutcome};
    if !managed_config::has_principal() {
        eprintln!("No deployment key or team sign-in found.");
        eprintln!();
        eprintln!("To install managed configuration, sign in with a team using `intellihelper login`,");
        eprintln!("or set a deployment key:");
        eprintln!();
        if cfg!(unix) {
            eprintln!("  export INTELLIHELPER_DEPLOYMENT_KEY=<your-key>");
        } else {
            eprintln!("  $env:INTELLIHELPER_DEPLOYMENT_KEY=\"<your-key>\"");
        }
        eprintln!("  intellihelper setup");
        eprintln!();
        eprintln!("Or add the key to ~/.intellihelper/config.toml:");
        eprintln!();
        eprintln!("  [endpoints]");
        eprintln!("  deployment_key = \"<your-key>\"");
        eprintln!();
        eprintln!(
            "If you don't have a deployment key, contact your organization's IntelliHelper administrator."
        );
        std::process::exit(1);
    }
    if json {
    ...
    match managed_config::run_setup().await {
        ...
        SetupOutcome::Skipped => {
            eprintln!(
                "Managed configuration was not applied this run (another process held the apply lock, or the credential changed during the fetch). Run `intellihelper setup` again."
            );
        }
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = intellihelper_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !intellihelper_shell::util::is_intellihelper_process(pid) {
            if let Some(ref lock) = d.lock_path {
                eprintln!("  PID {pid} is not an intellihelper process, removing stale lock");
        ...
        if let Err(e) = intellihelper_shell::util::kill_process_by_pid(pid) {
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(intellihelper_shell::env::IntelliHelperBuildEnvironment::Production),
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, intellihelper_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = intellihelper_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = intellihelper_shell::leader::LeaderClient::connect(
        socket_path.to_path_buf(),
        "intellihelper-pager-leader-cli",
        ClientMode::Stdio,
...
fn leader_info_json(
    ...
    info: Option<&intellihelper_shell::leader::ControlPayload>,
...
/// Env override for the `intellihelper workspace` gate: any truthy value enables the
/// command locally, a falsy one disables it — bypassing the remote settings flag.
const WORKSPACE_COMMAND_ENV: &str = "INTELLIHELPER_WORKSPACE_COMMAND";
/// Resolution of the `intellihelper workspace` gate. `Unknown` is kept separate from
...
/// The `INTELLIHELPER_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`);
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&intellihelper_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
/// Truthy parse for intellihelper on/off env vars: everything enables except the common
...
fn fetch_remote_settings() -> Option<intellihelper_shell::util::config::RemoteSettings> {
    join_early_prefetch(intellihelper_shell::agent::models::start_early_prefetch(None))
}
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    ) && let Some(profile) = intellihelper_sandbox::requested_confinement_profile()
    {
        anyhow::bail!(
            "`intellihelper workspace` start/restart/resume is unavailable under sandbox profile '{profile}': \
    ...
    match workspace_command_gate(env_override, remote_settings.as_ref()) {
        ...
        WorkspaceGate::Disabled => {
            anyhow::bail!(
                "`intellihelper workspace` is not enabled for this account \
             (gated by a server-side feature flag that is currently off)."
            )
        }
        WorkspaceGate::Unknown => {
            anyhow::bail!(
                "Could not load your settings for `intellihelper workspace`. Check your \
             network connection (run `intellihelper login` if you are signed out), then \
             try again."
            )
        }
    }
...
async fn connect_workspace_control(
    ...
    let ws_url = &agent_config.grok_com_config.intellihelper_ws_url;
    let socket = socket_path_for_ws_url(ws_url);
    LeaderClient::connect(
        socket,
        "intellihelper-workspace-cli",
        ClientMode::Stdio,
        ClientCapabilities::default(),
    )
    .await
    .map_err(|e| {
        anyhow::anyhow!(
            "no running leader for this environment ({e}). \
             Start a intellihelper session, or run `intellihelper workspace start`."
        )
    })
}
async fn workspace_control(
    ...
    let raw_config = intellihelper_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use intellihelper_shell::auth::ensure_authenticated;
    intellihelper_shell::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    let raw_config = intellihelper_shell::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
    let agent_config = AgentConfig::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    let (use_leader, _) = resolve_use_leader(
        ...
        true,
        intellihelper_sandbox::requested_confinement_profile(),
    );
    if !use_leader {
        anyhow::bail!(
            "`intellihelper workspace` requires leader mode (the workspace is shared via the leader).\n\
             Enable it with `[cli] use_leader = true` in ~/.intellihelper/config.toml, or pass --leader."
        );
    }
    ensure_authenticated(
        &agent_config.grok_com_config,
        false,
        Some("No cached credentials found. Run `intellihelper login` first."),
    )
    .await?;
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    intellihelper_telemetry::sentry::flush_on_shutdown();
    intellihelper_telemetry::otel_layer::shutdown_otel();
    intellihelper_telemetry::debug_log::flush();
...
/// can't drift.
const PLUGIN_DIR_LEADER_WARNING: &str = "intellihelper: --plugin-dir is ignored in leader mode; run with --no-leader to \
     load per-process plugins";
/// Run the `agent` subcommand, dispatching to the appropriate mode.
async fn run_agent_command(
    agent_args: Box<intellihelper_pager::app::AgentArgs>,
    ...
    let _signal_flush = tokio::spawn(async {
        #[cfg(unix)]
        {
            ...
            use intellihelper_pager::app::signal_handler::next_signal_code;
    ...
    if matches!(
        ...
        intellihelper_shell::agent::app::suppress_otel();
    }
    init_tracing_simple("agent");
    let _otel_guard = intellihelper_telemetry::otel_layer::otel_guard();
    intellihelper_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => intellihelper_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(error = %e, "--trust: failed to resolve cwd; folder not trusted")
            }
        }
    }
    let early_prefetch = intellihelper_shell::agent::models::start_early_prefetch(None);
    intellihelper_shell::agent::mvp_agent::warm_async_http_client();
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "IntelliHelper CLI (pager) - v{}",
            intellihelper_version::display_version_with_commit(
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(intellihelper_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = intellihelper_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    if let Some(warning) = launch_yolo.blocked_warning {
        eprintln!("intellihelper: {warning}");
    }
    agent_config.default_yolo_mode = launch_yolo.yolo;
    agent_config.default_auto_mode = intellihelper_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    ...
    agent_config.resolve_runtime_fields(&intellihelper_shell::agent::config::RuntimeResolutionContext {
    ...
    let requested_confinement = intellihelper_sandbox::requested_confinement_profile();
    let LeaderMode {
        use_leader,
    ...
    let managed_install = is_managed_install(
        ...
        &intellihelper_shell::util::intellihelper_home::intellihelper_home(),
    );
    ...
    if use_leader {
        ...
        let env_urls = intellihelper_shell::leader::LeaderEnvUrls::from(&agent_config.grok_com_config);
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = intellihelper_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            intellihelper_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(intellihelper_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = intellihelper_shell::util::config::load_config().await;
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = intellihelper_shell::managed_config::sync().await
                                        {
                                            tracing::warn!(
...
/// Sets `INTELLIHELPER_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for
/// `intellihelper dashboard`, and clears `args.command` so the regular
...
/// the leader roster — so `intellihelper dashboard` does NOT force leader mode and
/// is compatible with `--no-leader`.
///
/// The only gate is the feature flag: a disabled dashboard
/// (`[dashboard].enabled = false` / `INTELLIHELPER_AGENT_DASHBOARD=0`) is a CLI
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !intellihelper_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.intellihelper/config.toml and \
             unsetting INTELLIHELPER_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("INTELLIHELPER_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
const RUNTIME_SHUTDOWN_GRACE: std::time::Duration = std::time::Duration::from_secs(2);
const INTELLIHELPER_WORKER_THREADS_ENV: &str = "INTELLIHELPER_WORKER_THREADS";
...
/// How `INTELLIHELPER_WORKER_THREADS` resolved.
#[derive(Debug, PartialEq, Eq)]
enum WorkerCount {
    ...
    fn notice(&self) -> Option<String> {
        match self {
            ...
            Self::Clamped {
                ...
                "intellihelper: clamped {INTELLIHELPER_WORKER_THREADS_ENV}={requested} to {used} (valid range is 1..={cores})"
            )),
            Self::Ignored { value, .. } => Some(format!(
                "intellihelper: ignoring {INTELLIHELPER_WORKER_THREADS_ENV}={value:?} (not a valid integer)"
            )),
        }
    }
}
fn cli_worker_threads() -> NonZeroUsize {
    ...
    let resolved = match std::env::var(INTELLIHELPER_WORKER_THREADS_ENV) {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<intellihelper_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(intellihelper_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<intellihelper_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(intellihelper_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    intellihelper_shell::heap_profile::install(intellihelper_shell::heap_profile::HeapProfileHooks {
...
fn dispatch_doctor_if_requested(args: &PagerArgs) -> bool {
    ...
    if let Err(error) = intellihelper_pager::doctor_cmd::run(doctor_args.clone()) {
        eprintln!("Error: {error:#}");
        std::process::exit(1);
    }
    true
...
fn main() {
    if let Some(code) = intellihelper_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    if let Some(code) = intellihelper_pager::voice::maybe_run_capture_subprocess() {
        std::process::exit(code);
    }
    ...
    intellihelper_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    intellihelper_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        intellihelper_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        intellihelper_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    intellihelper_pager::memory_trace::start(
        intellihelper_shell::util::intellihelper_home::intellihelper_home().join("memtrace"),
    );
    raise_fd_limit();
    if let Err(e) = intellihelper_config::validate_requirements() {
        eprintln!("Couldn't start IntelliHelper: {e}");
        eprintln!();
        eprintln!(
            "Update IntelliHelper to a version the policy allows, or ask your administrator \
             to fix the managed requirements."
        );
        std::process::exit(2);
    }
    let _sentry_guard = intellihelper_telemetry::sentry::init(intellihelper_telemetry::sentry::Config {
        client: "intellihelper-pager",
        client_version: PAGER_CLIENT_VERSION,
        release: env!("VERSION_WITH_COMMIT"),
        disabled: intellihelper_shell::agent::config::is_error_reporting_disabled_sync(),
    });
    intellihelper_pager::docs::extract_user_guide_docs(&intellihelper_shell::util::intellihelper_home::intellihelper_home());
    crash_handler::install_terminal_restore_only();
    if intellihelper_shell::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = intellihelper_shell::util::intellihelper_home::intellihelper_home().join("crash");
        if let Some(report) = crash_handler::check_previous_crash(&crash_dir) {
            eprintln!("IntelliHelper crashed during your last session.");
    ...
    let crashed = intellihelper_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
    ...
    let runtime = tokio::runtime::Builder::new_multi_thread()
        ...
        .build()
        .unwrap_or_else(|e| {
            eprintln!("intellihelper: failed to start tokio runtime with {workers} workers: {e}");
...
async fn async_main(args: PagerArgs) -> Result<()> {
    ...
    if let Some(ref mode) = args.compaction_mode {
        unsafe { std::env::set_var("INTELLIHELPER_COMPACTION_MODE", mode) };
    }
    if let Some(ref detail) = args.compaction_detail {
        unsafe { std::env::set_var("INTELLIHELPER_COMPACTION_DETAIL", detail) };
    }
    if args.chat() {
        unsafe {
            std::env::set_var(intellihelper_shell::agent::chat_modes::INTELLIHELPER_CHAT_MODE_ENV, "1");
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(intellihelper_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    if let Some(ref path) = args.debug_file {
        unsafe {
            std::env::set_var("INTELLIHELPER_DEBUG_LOG", path);
            std::env::remove_var("INTELLIHELPER_LOG_FILE");
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("INTELLIHELPER_DEBUG_LOG", "1");
        set_if_unset("INTELLIHELPER_HOOKS_LOG", "1");
    }
    if let Some(Command::Completions { shell }) = &args.command {
        intellihelper_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return intellihelper_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        intellihelper_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        intellihelper_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    intellihelper_shell::config::apply_sandbox(
        None,
    ...
    intellihelper_shell::http::set_client_name(if is_interactive {
        intellihelper_workspace::permission::ClientType::IntelliHelperPager
    } else {
        intellihelper_workspace::permission::ClientType::Generic
    });
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!({
                        ...
                        "channel": intellihelper_update::channel_name().unwrap_or("unknown"),
                    });
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    write_version(
                ...
                if args.leader || args.no_leader {
                    ...
                    anyhow::bail!(
                        "top-level {flag} applies to the pager TUI, not the agent subcommand. \
                         Use `intellihelper-pager agent {flag}` instead."
                    );
                }
            ...
            Command::Inspect { json } => {
                ...
                intellihelper_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return intellihelper_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = intellihelper_telemetry::otel_layer::otel_guard();
                return intellihelper_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = intellihelper_telemetry::otel_layer::otel_guard();
                let config = intellihelper_shell::config::load_effective_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return intellihelper_pager::models::list_available_models(&agent_config).await;
            }
            ...
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return intellihelper_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return intellihelper_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Share(ref share_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return intellihelper_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return intellihelper_pager::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return intellihelper_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return intellihelper_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
            ...
            Command::Login {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                intellihelper_shell::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
                intellihelper_shell::instrumentation::finalize_and_exit(0);
            }
            Command::Logout => {
                init_tracing_simple("cli");
                let config = intellihelper_shell::config::load_effective_config_disk_only()
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                intellihelper_shell::auth::run_cli_logout(&config)?;
                intellihelper_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = intellihelper_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(intellihelper_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some()
            && args.output_format == intellihelper_pager::headless::OutputFormat::Plain
        {
            args.output_format = intellihelper_pager::headless::OutputFormat::Json;
        }
        return intellihelper_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            intellihelper_pager::headless::HeadlessOptions {
    ...
    let result = intellihelper_pager::app::run(args, bg_update_rx).await;
    intellihelper_sandbox::flush();
    match result {
        Ok(true) => {
            let adopted = bg_update_wait.lock().await.take();
            if finish_update_on_exit(adopted, &update_config).await {
                eprintln!("Update installed. Run `intellihelper` to start.");
            } else {
                eprintln!("Update did not complete. Run `intellihelper update` to retry.");
            }
            Ok(())
        }
        Ok(false) => Ok(()),
...
///
/// Prefers awaiting the parked waiter for the background `intellihelper update` child
...
/// fall back to a fresh blocking `intellihelper update`, which itself resolves to
...
fn build_update_config() -> UpdateConfig {
    let environment = intellihelper_shell::env::IntelliHelperBuildEnvironment::from_flags(false, false);
    let mut config = UpdateConfig::from_environment(&environment);
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                intellihelper_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("INTELLIHELPER_NPM_REGISTRY"))
        .ok()
        .or_else(intellihelper_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = intellihelper_shell::config::load_effective_config_disk_only()
        && let Some(ch) = intellihelper_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
fn should_check_for_updates(no_auto_update_flag: bool) -> bool {
    ...
    !std::env::var_os("INTELLIHELPER_DISABLE_AUTOUPDATER")
...
/// True when `exe` is the binary `<intellihelper_home>/bin/intellihelper` resolves to, the
...
/// npm shim hardcodes `~/.intellihelper`, so a custom `INTELLIHELPER_HOME` skips here too.
fn is_managed_install(exe: Option<std::path::PathBuf>, intellihelper_home: &std::path::Path) -> bool {
    if intellihelper_home.as_os_str().is_empty() {
        return false;
    }
    let Some(exe) = exe else {
        return false;
    };
    let managed = intellihelper_config::intellihelper_application_in(intellihelper_home);
...
/// Handle `intellihelper-pager update [--check] [--json] [--force-reinstall] [--version X] [--alpha|--stable|--enterprise]`.
...
/// After a successful `intellihelper update`, ask any running leader on this machine that
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in intellihelper_shell::leader::discover_leaders().await {
        if d.classification != intellihelper_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match intellihelper_shell::leader::LeaderClient::connect(
            socket_path,
            "intellihelper-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        )
        .await
        ...
        match client
            ...
            .await
        {
            Ok(Ok(intellihelper_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(intellihelper_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn override_out_of_range_is_clamped_with_a_notice() {
        ...
        assert_eq!(
            ...
            "intellihelper: clamped INTELLIHELPER_WORKER_THREADS=100000 to 360 (valid range is 1..=360)"
        );
    }
    #[test]
    fn override_unparseable_is_ignored_with_a_notice() {
        ...
        assert_eq!(
            ...
            "intellihelper: ignoring INTELLIHELPER_WORKER_THREADS=\"abc\" (not a valid integer)"
        );
    }
    #[test]
    ...
    impl TempHeapDump {
        fn new(label: &str) -> Self {
            let path = std::env::temp_dir().join(format!(
                "intellihelper-jemalloc-{label}-{}-{}.heap",
                std::process::id(),
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            intellihelper_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: intellihelper_shell::heap_profile::JemallocStats) {
    ...
    fn jemalloc_dump_rejects_interior_nul_path() {
        ...
        let path = Path::new(OsStr::from_bytes(b"/tmp/intellihelper-jemalloc-\0.heap"));
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            intellihelper_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!intellihelper_shell::heap_profile::prof_available());
            return;
        }
        assert!(intellihelper_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(intellihelper_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(intellihelper_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        intellihelper_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    fn is_managed_install_matches_only_the_bin_intellihelper_target() {
        let home =
            std::env::temp_dir().join(format!("intellihelper-pager-managed-install-{}", std::process::id()));
        ...
        let pinned = home.join("bin").join("intellihelper-9.9.9");
    ...
    /// `intellihelper dashboard` flags the startup hook without forcing leader mode —
    ...
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        ...
        assert_eq!(
            std::env::var("INTELLIHELPER_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
            "startup hook flag must be set",
        );
        unsafe { std::env::remove_var("INTELLIHELPER_OPEN_DASHBOARD_AT_STARTUP") };
    }
    /// `intellihelper dashboard --no-leader` is allowed — the dashboard does not
    ...
    /// `INTELLIHELPER_AGENT_DASHBOARD=0` disables the feature — the subcommand
    ...
    fn dashboard_subcommand_errors_when_disabled() {
        ...
        assert!(
            std::env::var("INTELLIHELPER_OPEN_DASHBOARD_AT_STARTUP").is_err(),
            "failure path must not flag the startup hook",
        );
    }
    #[test]
    fn workspace_command_gate_resolution() {
        use intellihelper_shell::util::config::RemoteSettings;
    ...
    #[serial_test::serial(INTELLIHELPER_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("INTELLIHELPER_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("INTELLIHELPER_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("INTELLIHELPER_WORKSPACE_COMMAND", "off") };
