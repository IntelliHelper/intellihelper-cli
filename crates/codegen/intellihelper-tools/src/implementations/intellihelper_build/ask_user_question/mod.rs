...
/// params are injected, `INTELLIHELPER_ASK_USER_QUESTION_TIMEOUT_SECS` (positive
/// integer seconds) still overrides this default directly —
/// e.g. `INTELLIHELPER_ASK_USER_QUESTION_TIMEOUT_SECS=8` for tests / TUI repro.
...
pub const RESPONSE_TIMEOUT_ENV: &str = "INTELLIHELPER_ASK_USER_QUESTION_TIMEOUT_SECS";
...
crate::register_resource!("intellihelper_build", "AskUserQuestion", AskUserQuestionParams);
...
pub struct QuestionOption {
    ...
    /// Opaque id; hidden from the model. IntelliHelper callers leave it `None`.
...
/// session-owned coordinator (in intellihelper-shell), which performs an ACP
