...
//!
//! Runs the built intellihelper binary against the mock inference server with a
//! caller-owned `$INTELLIHELPER_HOME`, then inspects `~/.intellihelper/debug/`:
...
//!   `~/.intellihelper/debug/<sessionId>.txt` with first-party content, and does NOT enable
//!   sampling/instrumentation. Regression for the master switch having bundled
//!   `INTELLIHELPER_LOG_SAMPLING`/`INTELLIHELPER_INSTRUMENTATION`, whose global `TargetFilterLayer`
...
//! - a live `agent` session (explicit `INTELLIHELPER_DEBUG_LOG=1`) writes a per-session
//!   `~/.intellihelper/debug/<sessionId>.txt` with real first-party content + `latest.txt`.
//! - `--debug-file <path>` writes one explicit file and bypasses per-session
//!   routing entirely (no `~/.intellihelper/debug/` files).
//! - `INTELLIHELPER_LOG_FILE=<path>` writes that explicit file (back-compat single file).
//!
...
//! ```bash
//! cargo test -p intellihelper-shell --test test_debug_logging -- --ignored
//! ```
...
/// The per-session firehose directory under a pinned `$INTELLIHELPER_HOME`.
fn debug_dir(home: &Path) -> PathBuf {
    home.join(".intellihelper").join("debug")
}

/// List firehose `*.txt` files under `~/.intellihelper/debug` (excluding the `latest.txt`
...
/// Build a headless `intellihelper -p` command with a pinned `$INTELLIHELPER_HOME` so the firehose
/// lands under `<home>/.intellihelper/debug`. Firehose env knobs are cleared so the test
...
fn debug_cmd(
    ...
    sandbox
        ...
        .set_env("INTELLIHELPER_HOME", home.join(".intellihelper"));
...
/// assert it carries first-party (`intellihelper`) content. Panics with the captured
/// stderr tail if it never fills. Shared by the live-agent tests.
async fn read_session_firehose_when_ready(path: &Path, client: &IntelliHelperStdioClient) -> String {
    ...
    assert!(
        content.contains("intellihelper"),
...
/// headless `intellihelper -p` client is near-silent, so its lazily-opened firehose may
...
async fn debug_flag_enables_firehose_without_crashing() {
    ...
    assert_headless_success(&result, "intellihelper --debug headless", Some(&server));
...
async fn no_debug_flag_writes_no_debug_dir() {
    ...
    assert_headless_success(&result, "intellihelper headless (no --debug)", Some(&server));
    assert!(
...
/// A live `agent` session writes `~/.intellihelper/debug/<sessionId>.txt` with real
...
async fn agent_session_writes_named_session_file() {
    with_local_set(|| async {
        ...
        sandbox.set_env("INTELLIHELPER_DEBUG_LOG", "1");
        let intellihelper_home = sandbox.intellihelper_home().to_path_buf();

        let client =
            IntelliHelperStdioClient::spawn_with_sandbox(&server, workdir.workspace(), sandbox).await;
        ...
        let session_file = intellihelper_home.join("debug").join(format!("{sid}.txt"));
        ...
        // file, so `tail -f ~/.intellihelper/debug/latest.txt` follows the live session.
        #[cfg(unix)]
        {
            let link = intellihelper_home.join("debug").join("latest.txt");
...
/// The `--debug` FLAG (not `INTELLIHELPER_DEBUG_LOG` directly) drives the firehose end to
/// end through the master switch. Regression: the master switch used to also set
/// `INTELLIHELPER_LOG_SAMPLING`/`INTELLIHELPER_INSTRUMENTATION`, whose `TargetFilterLayer` globally
...
async fn debug_flag_master_switch_enables_firehose() {
    with_local_set(|| async {
        ...
        // Drive `intellihelper --debug agent stdio`: the master switch (which runs before
        // the agent dispatch) must be what enables the firehose — NOT a direct
        // INTELLIHELPER_DEBUG_LOG env. The sandbox baseline excludes inherited firehose
        // toggles, so the `--debug` flag is the only thing enabling logging here.
        let client = IntelliHelperStdioClient::spawn_with_sandbox_env_and_args(
            &server,
            workdir.workspace(),
            sandbox,
            &[],
        ...
        // clears INTELLIHELPER_LOG_SAMPLING (hermetic), so the sampling layer stays off and
        // `~/.intellihelper/logs/sampling.jsonl` is never written — the `--debug`
        ...
        // INTELLIHELPER_INSTRUMENTATION=disabled, so that assertion would be vacuous.
        let sampling = intellihelper_home.join("logs").join("sampling.jsonl");
...
/// `--debug-file <path>` writes one explicit file and bypasses per-session
/// routing entirely (no `~/.intellihelper/debug/` files created).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn debug_file_flag_writes_single_file_and_bypasses_routing() {
    ...
    assert_headless_success(&result, "intellihelper --debug-file", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        explicit.exists(),
...
/// `INTELLIHELPER_LOG_FILE=<path>` (no `--debug`) writes that exact file (back-compat).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn intellihelper_log_file_explicit_path_is_written() {
    let server = MockInferenceServer::start()
        .await
        .expect("start mock server");
    let workdir = git_workdir();
    ...
    sandbox.set_env("INTELLIHELPER_LOG_FILE", &custom);
    let result = run_headless_in_sandbox(cmd, sandbox).await;

    assert_headless_success(&result, "intellihelper INTELLIHELPER_LOG_FILE=path", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        custom.exists(),
        "explicit INTELLIHELPER_LOG_FILE path not written: {custom:?}\nstderr tail:\n{}",
        stderr_tail(&result.stderr, 800)
    );
    // Single-file mode bypasses per-session routing.
    assert!(
        firehose_txt_files(home.path()).is_empty(),
        "INTELLIHELPER_LOG_FILE must bypass per-session routing, found: {:?}",
        firehose_txt_files(home.path())
    );
}
