...
//! connection to the intellihelper.com relay server with automatic reconnection.
...
/// Config for the intellihelper.com WebSocket relay. Fields are private so the only
...
impl RelayConfig {
    /// Session gate: builds only for a intellihelper.com first-party session
    ...
    pub(crate) fn for_session(
        ...
        Some(Self {
            ws_url: ctx.intellihelper_ws_url.clone(),
            ws_origin: ctx.intellihelper_ws_origin.clone(),
...
///
/// The relay maintains a persistent WebSocket connection to intellihelper.com with
...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!("Authentication required. Run `intellihelper login` to re-authenticate.");
        cancel.cancel();
        return false;
    };
    ...
    match recovered {
        Ok(new_auth) if new_auth.key == config.auth.key => {
            ...
            intellihelper_telemetry::unified_log::info(
                ...
                Some(serde_json::json!({
                    "context": context,
                    "key_prefix": intellihelper_auth::bearer_suffix(&new_auth.key),
                })),
            );
            false
        }
        Ok(new_auth) => {
            ...
            intellihelper_telemetry::unified_log::info(
                ...
                Some(serde_json::json!({
                    "context": context,
                    "new_key_prefix": intellihelper_auth::bearer_suffix(&new_auth.key),
                })),
            );
    ...
    loop {
        ...
        match connect_to_relay(&config, proxy_url.as_deref(), &cancel).await {
            Ok(ws) => {
                ...
                tprintln!("Disconnected from IntelliHelper WebSocket server");
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        ...
        axum::http::header::HeaderValue::from_static(intellihelper_version::VERSION),
    );
    req.headers_mut().insert(
...
mod tests {
    ...
    /// Helper to create a test IntelliHelperAuth with the given key.
    fn test_auth(key: &str) -> IntelliHelperAuth {
        IntelliHelperAuth {
            key: key.to_string(),
    ...
    fn for_session_builds_only_for_xai_issuer() {
        ...
        let builds = |a: &IntelliHelperAuth| RelayConfig::for_session(a, &cfg, None, None).is_some();
        let xai = IntelliHelperAuth {
        ...
        assert!(builds(&xai));
        let external_xai = IntelliHelperAuth {
        ...
        assert!(!builds(&IntelliHelperAuth {
            key: String::new(),
            ..xai.clone()
        }));
        assert!(!builds(&IntelliHelperAuth {
    ...
    /// Helper: write a IntelliHelperAuth to disk under the given scope.
    fn write_test_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &IntelliHelperAuth) {
    ...
    async fn auth_recovery_refreshes_and_heals_missing_auth_json() {
        ...
        let expired_session = IntelliHelperAuth {
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(XAI_OAUTH2_ISSUER.to_string()),
    ...
    async fn attempt_auth_recovery_same_key_backs_off_without_cancel() {
        ...
        let fresh_session = IntelliHelperAuth {
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(XAI_OAUTH2_ISSUER.to_string()),
