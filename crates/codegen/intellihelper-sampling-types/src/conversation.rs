...
/// tool-call arguments routinely contain. `pub` for `intellihelper-shell`.
...
pub struct ConversationRequest {
    ...
    pub x_intellihelper_conv_id: Option<String>,
    pub x_intellihelper_req_id: Option<String>,
    pub x_intellihelper_session_id: Option<String>,
    pub x_intellihelper_turn_idx: Option<String>,
    pub x_intellihelper_agent_id: Option<String>,
    pub x_intellihelper_deployment_id: Option<String>,
    pub x_intellihelper_user_id: Option<String>,
    ...
    /// Sticky routing key for prompt-cache reuse; overrides `x_intellihelper_conv_id` for routing.
...
pub struct ConversationResponse {
    ...
    /// only, opt-in via the `x-intellihelper-doom-loop-check` header). Empty when the
...
// ---------------------------------------------------------------------------
//
// Part of the IntelliHelper Compaction unification. Lets the shared, transport-agnostic
// engine in `crates/common/intellihelper-compaction` operate over intellihelper-build's
...
impl intellihelper_compaction::CompactionItem for ConversationItem {
    fn role(&self) -> intellihelper_compaction::CompactionRole {
        use intellihelper_compaction::CompactionRole;
        // intellihelper-build has no distinct `Developer` role; everything maps onto
    ...
    fn is_compaction_summary(&self) -> bool {
        // intellihelper-build has no structural marker that uniquely identifies a prior
        ...
        // (add a dedicated marker) before routing intellihelper-build history through
        // the shared `history`/`inter` filter.
        false
    }

    fn attachment_refs(&self) -> Vec<intellihelper_compaction::CompactionFileRef> {
        // intellihelper-build `UserItem`s carry only `Text`/`Image { url }` content
        // parts — there is no id+name attachment-ref concept like the chat harness's
        // `IntelliHelperTurn`. The full-replace path does not read this; revisit if
        // image attachments need to survive into the `<intellihelper_user_queries>`
        // preamble.
        Vec::new()
    }
}

impl intellihelper_compaction::CompactionItemFactory for ConversationItem {
...
/// ([stream/messages.rs:340](crates/codegen/intellihelper-sampler/src/stream/messages.rs))
...
impl ConversationRequest {
    ...
    pub fn with_conv_id(mut self, conv_id: impl Into<String>) -> Self {
        self.x_intellihelper_conv_id = Some(conv_id.into());
        self
    }

    /// Set request ID header
    pub fn with_req_id(mut self, req_id: impl Into<String>) -> Self {
        self.x_intellihelper_req_id = Some(req_id.into());
        self
    }
...
mod compaction_item_bridge_tests {
    use super::*;
    use intellihelper_compaction::{CompactionItem, CompactionItemFactory, CompactionRole};
    ...
    fn metadata_accessors_are_conservative() {
        // intellihelper-build has no structural compaction-summary marker, and no
...
mod tests {
    ...
    fn test_transform_cwd_transforms_tool_call_arguments() {
        ...
        let worktree = "/home/user/.intellihelper/worktrees/project/ab-uuid-a";
    ...
    fn test_transform_cwd_worktree_to_root_syncback() {
        ...
        let worktree = "/home/user/.intellihelper/worktrees/myproject/fork-a";
    ...
    fn test_transform_cwd_assistant_only_tool_calls_no_content() {
        ...
        let worktree = "/home/user/.intellihelper/worktrees/proj/fork-a";
    ...
    fn test_conversation_request_builder() {
        ...
        assert_eq!(req.model, Some("intellihelper-3".to_string()));
        assert_eq!(req.temperature, Some(0.5));
        assert_eq!(req.max_output_tokens, Some(1000));
        assert_eq!(req.x_intellihelper_conv_id, Some("conv-123".to_string()));
        assert_eq!(req.x_intellihelper_req_id, Some("req-456".to_string()));
    ...
    fn test_assistant_item_with_model_id() {
        ...
        .with_model_id("intellihelper-3");

        assert_eq!(item.model_id, Some("intellihelper-3".to_string()));
    }

    #[test]
    fn test_conversation_item_with_model_id() {
        let item = ConversationItem::assistant("Hello").with_model_id("intellihelper-3");

        let ConversationItem::Assistant(a) = item else {
            panic!("Expected Assistant");
        };
        assert_eq!(a.model_id, Some("intellihelper-3".to_string()));

        // Non-assistant should be unchanged
        let user = ConversationItem::user("Hi").with_model_id("intellihelper-3");
    ...
    fn multi_tco_reasoning_items_round_trip_as_siblings() {
        ...
        let response = rs::Response {
            ...
            metadata: None,
            model: "intellihelper-build".to_string(),
    ...
    //      (earlier intellihelper-build / chat-completions written as v1)
    ...
    #[test]
    fn upgrade_legacy_reasoning_singular_intellihelper_build_shape() {
