...
Agent mode runs IntelliHelper as a long-lived server that clients talk to over [ACP](https://agentclientprotocol.com) (JSON-RPC). Use it from IDEs, SDKs, eval harnesses, and custom apps. For a one-shot prompt that prints and exits, use `intellihelper -p` instead ([headless mode](14-headless-mode.md)).
...
The [Agent Client Protocol (ACP)](https://agentclientprotocol.com) defines how clients talk to coding agents over JSON-RPC. With IntelliHelper it covers:
...
```bash
intellihelper agent --always-approve --model intellihelper-build stdio
intellihelper agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```

| Flag | Description |
| ---- | ----------- |
| `-m, --model <MODEL>` | Model ID (for example `intellihelper-build`). |
...
Clients connect over WebSocket and authenticate with the secret token. If you omit `--secret`, the agent prints a generated token at startup, or set `INTELLIHELPER_AGENT_SECRET`. The process keeps state across client reconnects. Permissions match other entry points; see [Permissions and safety](22-permissions-and-safety.md).
...
```bash
intellihelper agent --always-approve headless --intellihelper-ws-url wss://your-relay.example.com/ws
```
...
Beyond the base ACP protocol, IntelliHelper defines extension methods under the `x.ai/` prefix for SpaceXAI-specific functionality. These cover:
...
class IntelliHelperACPChat {
  ...
  async init() {
    this.proc = spawn("intellihelper", ["agent", "--always-approve", "stdio"]);
