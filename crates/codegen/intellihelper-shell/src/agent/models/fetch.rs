...
pub(crate) struct PrefetchEnv {
    pub(crate) auth: Option<IntelliHelperAuth>,
...
pub fn start_early_prefetch_with_auth(auth: Option<IntelliHelperAuth>) -> Option<EarlyPrefetchHandle> {
...
fn start_early_prefetch_impl(
    ...
    let auth = AuthManager::new(&intellihelper_home, grok_com_config.unwrap_or_default()).current();
