//! Tracing layer for `target: "sampling_log"` → `~/.intellihelper/logs/sampling.jsonl`.
//! Enable with `--log-sampling` or `INTELLIHELPER_LOG_SAMPLING=1`.
...
const ENV_VAR: &str = "INTELLIHELPER_LOG_SAMPLING";
    ...
    S: Subscriber + for<'span> LookupSpan<'span> + Send + Sync + 'static,
{
    ...
    let path = intellihelper_home().join(crate::unified_log::LOG_DIR).join(LOG_FILE);
