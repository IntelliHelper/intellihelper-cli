...
fn partition_rules_by_scope(
    files: Vec<intellihelper_agent::prompt::agents_md::AgentConfigFile>,
    intellihelper_home: &std::path::Path,
    vendor_homes: &[(std::path::PathBuf, bool)],
    workspace_root: Option<&std::path::Path>,
) -> (
    Vec<intellihelper_agent::prompt::user_message::RuleEntry>,
    Vec<intellihelper_agent::prompt::user_message::RuleEntry>,
) {
    ...
    for file in files {
        ...
        let entry = intellihelper_agent::prompt::user_message::RuleEntry::from(file);
        if is_user_rule {
            user.push(entry);
        } else {
...
mod partition_rules_by_scope_tests {
    ...
    use std::path::Path;
    use intellihelper_agent::prompt::agents_md::AgentConfigFile;
    ...
    fn paths(entries: &[intellihelper_agent::prompt::user_message::RuleEntry]) -> Vec<&str> {
    ...
    #[test]
    fn intellihelper_home_nested_in_workspace_keeps_direct_surfaces_user_scoped() {
        let files = vec![
            file("/repo/config/AGENTS.md"),
            file("/repo/config/rules/global.md"),
            file("/repo/config/.intellihelper/rules/project.md"),
    ...
    #[test]
    fn nested_intellihelper_home_workspace_files_stay_workspace_scoped() {
        let files = vec![
            file("/custom/intellihelper/rules/global.md"),
            file("/custom/intellihelper/worktrees/repo/.cursor/rules/project.md"),
            file("/custom/intellihelper/worktrees/repo/src/AGENTS.md"),
        ];
        let (workspace, user) = partition_rules_by_scope(
            files,
            Path::new("/custom/intellihelper"),
            &[],
            Some(Path::new("/custom/intellihelper/worktrees/repo")),
        );
        assert_eq!(paths(&user), vec!["/custom/intellihelper/rules/global.md"]);
        assert_eq!(
            paths(&workspace),
            vec![
...
/// end for intellihelper ordering), so a failed offload never
...
impl SessionActor {
                ...
                !matches!(
                    ...
                    ConversationItem::User(u)
                        if u.synthetic_reason
                            == Some(intellihelper_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    ...
    async fn build_templated_user_message(
        ...
        use intellihelper_agent::prompt::agents_md::read_agents_config_with_paths;
        use intellihelper_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use intellihelper_workspace::file_system::{git_status_short, jj_status};
        use intellihelper_workspace::session::git::VcsKind;
    ...
    ) -> Vec<intellihelper_agent::prompt::user_message::McpServerEntry> {
        use intellihelper_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let client = intellihelper_sampler::SamplingClient::new(sampler_config).map_err(|e| {
