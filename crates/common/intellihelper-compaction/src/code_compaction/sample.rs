...
//!
//! The canonical `sample → classify → retry` loop, used by **both** intellihelper-build's
//! full-replace pass ([`sample_full_replace_summary`](super::sample_full_replace_summary))
//! and IntelliHelper chat's intra `Shared` summarizer
...
//! the winning summary (intellihelper-build cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the intellihelper-build host uses to step down its input size).
