#!/bin/bash
#
# IntelliHelper CLI installer (enterprise channel) — https://cli.intellihelper.in/enterprise-install.sh
#
...
#
# Usage:
#   curl -fsSL https://cli.intellihelper.in/enterprise-install.sh | bash            # latest enterprise
#   curl -fsSL https://cli.intellihelper.in/enterprise-install.sh | bash -s 0.1.42  # specific version
#   INTELLIHELPER_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://cli.intellihelper.in/enterprise-install.sh)
#
...
# Read a token from ~/.intellihelper/auth.json for the given scope key.
# Format: {"scope_url": {"key": "token"}, ...}
read_grok_token() {
    local auth_file="$HOME/.intellihelper/auth.json"
    local scope="$1"
...
# Resolve auth: INTELLIHELPER_DEPLOYMENT_KEY > OIDC token > legacy token
OIDC_SCOPE="https://auth.x.ai::b1a00492-073a-47ea-816f-4c329264a828"
LEGACY_SCOPE="https://accounts.x.ai/sign-in"
AUTH_SOURCE=""

if [ -n "$INTELLIHELPER_DEPLOYMENT_KEY" ]; then
    ...
    if [ -n "$OIDC_TOKEN" ]; then
        ...
        echo "Auth: using OIDC token from ~/.intellihelper/auth.json." >&2
    elif [ -n "$LEGACY_TOKEN" ]; then
        AUTH_SOURCE="auth.json (legacy)"
        echo "Auth: using legacy token from ~/.intellihelper/auth.json." >&2
    fi
fi
...
if [ -n "$AUTH_SOURCE" ]; then
    echo "Installing IntelliHelper $version ($platform, $AUTH_SOURCE)..." >&2
else
    echo "Installing IntelliHelper $version ($platform)..." >&2
fi

binary_path="$DOWNLOAD_DIR/intellihelper-$platform"
artifact_base="${BASE_URL}/intellihelper-${version}-${platform}"

if [ "$os" = "windows" ]; then
    binary_path="${binary_path}.exe"
fi

echo "  Downloading intellihelper ${version}..." >&2
if [ "$os" = "windows" ]; then
    if ! download_file_parallel "${artifact_base}.exe" "$binary_path"; then
        if ! download_file_parallel "$artifact_base" "$binary_path"; then
            ...
            if is_not_found "${artifact_base}.exe"; then
                echo "Error: IntelliHelper is not yet available for your system ($platform)." >&2
            else
...
CONFIG_FILE="$HOME/.intellihelper/config.toml"
...
if [ -n "$INTELLIHELPER_DEPLOYMENT_KEY" ]; then
    PROXY_URL="${INTELLIHELPER_PROXY_URL:-https://cli-chat-proxy.grok.com/v1}"
    ...
    if [ -n "$AUTH_HEADER_FILE" ]; then
        ...
        printf 'Authorization: Bearer %s\n' "$INTELLIHELPER_DEPLOYMENT_KEY" > "$AUTH_HEADER_FILE"
    ...
    if [ -n "$DEPLOY_RESPONSE" ]; then
        ...
        if [ -n "$MANAGED_CONFIG" ] && [ "$MANAGED_CONFIG" != "null" ]; then
            printf '%s\n' "$MANAGED_CONFIG" > "$HOME/.intellihelper/managed_config.toml"
            echo "  Managed config applied." >&2
        else
            rm -f "$HOME/.intellihelper/managed_config.toml"
        fi
        if [ -n "$REQUIREMENTS" ] && [ "$REQUIREMENTS" != "null" ]; then
            printf '%s\n' "$REQUIREMENTS" > "$HOME/.intellihelper/requirements.toml"
            echo "  Requirements applied." >&2
        else
            rm -f "$HOME/.intellihelper/requirements.toml"
        fi
    fi
fi
...
# Also update shell config so ~/.intellihelper/bin is on PATH for future sessions
...
fish_add_path $HOME/.intellihelper/bin
# <<< intellihelper installer <<<'
    elif [ "$user_shell" = "zsh" ]; then
        new_block='# >>> intellihelper installer >>>
export PATH="$HOME/.intellihelper/bin:$PATH"
fpath=(~/.intellihelper/completions/zsh $fpath)
...
# <<< intellihelper installer <<<'
    ...
    if grep -qs "intellihelper installer" "$config_file" 2>/dev/null; then
        # Replace existing block in-place (strip old >>> to <<< lines, insert new)
        tmp="$config_file.tmp.$$"
        awk '
            /# >>> intellihelper installer >>>/ { skip=1; next }
            /# <<< intellihelper installer <<</ { skip=0; next }
            !skip { print }
        ' "$config_file" > "$tmp" && mv "$tmp" "$config_file"
    else
