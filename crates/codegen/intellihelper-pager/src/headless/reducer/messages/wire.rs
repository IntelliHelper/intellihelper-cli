...
use intellihelper_shell::extensions::notification::ResponseUsage;
...
/// Map a IntelliHelper permission mode to the Messages `permissionMode` enum, clamping IntelliHelper-only values to `default`.
...
/// The `usage.server_tool_use` sub-object (IntelliHelper counts backend `web_search` only).
#[derive(Clone, Default, Serialize, Deserialize)]
