//! intellihelper.com product Skills catalog — the same REST sources intellihelper-web uses:
...
use intellihelper_tools::implementations::skills::skill::extract_skill_body;
use intellihelper_tools::implementations::skills::types::{SkillInfo, SkillScope};

use crate::auth::AuthManager;

const INTELLIHELPER_WEB_URL: &str = "https://grok.com";
...
/// connect timeout; without this a hung intellihelper.com stalls the session actor.
...
/// Combined product catalog matching intellihelper-web's Skills menu composition.
...
impl ProductSkillsCatalog {
    ...
    ///
    /// Mirrors intellihelper-web: enabled user skills first (and hide same-named
...
pub enum SkillsError {
    #[error("no intellihelper.com credentials")]
    NoAuth,
...
fn primary_is_tenant_tagged(auth: &crate::auth::IntelliHelperAuth) -> bool {
    auth.team_id.is_some() || auth.organization_id.is_some()
}

fn entry_is_untagged(auth: &crate::auth::IntelliHelperAuth) -> bool {
...
fn entry_matches_primary_tenant(
    ...
    entry: &crate::auth::IntelliHelperAuth,
) -> bool {
...
fn skills_auth_alt_candidates<'a>(
    primary: &crate::auth::IntelliHelperAuth,
    entries: impl IntoIterator<Item = &'a crate::auth::IntelliHelperAuth>,
...
/// Stateless transport for product Skills REST (intellihelper-web `skillsApi`).
...
impl SkillsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("INTELLIHELPER_SKILLS_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                std::env::var("INTELLIHELPER_CONVERSATIONS_BASE_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .or_else(|| {
                std::env::var("INTELLIHELPER_CODE_WEB_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .unwrap_or_else(|| INTELLIHELPER_WEB_URL.to_string());
        Self {
    ...
    /// IntelliHelper.com product Skills require first-party session auth (same gate as
    /// managed MCP / sibling intellihelper.com clients — not plain BYOK API keys).
    async fn require_skills_auth(&self) -> Result<crate::auth::IntelliHelperAuth, SkillsError> {
    ...
    /// Credentials to try for intellihelper.com product Skills REST.
    ///
    /// Primary first. When primary is OIDC on the default intellihelper.com host, also
    ...
    fn skills_auth_candidates(&self, primary: &crate::auth::IntelliHelperAuth) -> Vec<SkillsAuthCandidate> {
        ...
        if self.base_url != INTELLIHELPER_WEB_URL {
            return out;
        }
    ...
    /// feature-flag allow-list (same as intellihelper-web attach menu).
    ///
...
mod tests {
    ...
    fn test_auth_manager() -> Arc<AuthManager> {
        use crate::auth::{AuthMode, IntelliHelperAuth, GrokComConfig, XAI_OAUTH2_ISSUER};
    ...
    fn skills_auth_alts_prefer_tagged_then_untagged_recovery() {
        ...
        let same_team = IntelliHelperAuth {
            key: "web-team".into(),
            user_id: "u1".into(),
            team_id: Some("team-a".into()),
        ...
        let untagged = IntelliHelperAuth {
        ...
        let team_plus_org = IntelliHelperAuth {
    ...
    fn entry_matches_primary_tenant_requires_symmetric_tags() {
        ...
        let extra_org = IntelliHelperAuth {
