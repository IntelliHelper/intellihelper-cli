...
fn install_client_hook(
    actor: &SessionActor,
    event: intellihelper_hooks::event::HookEventName,
    callback_ids: &[&str],
) {
...
async fn client_hooks_fire_without_file_registry() {
    ...
    local
        .run_until(async {
            ...
            install_client_hook(
                &actor,
                intellihelper_hooks::event::HookEventName::Stop,
                &["cb_0"],
            );
            ...
            actor.fire_hook(
                intellihelper_hooks::event::HookEventName::Stop,
                None,
                intellihelper_hooks::event::HookPayload::Stop {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
                intellihelper_tools::registry::types::ToolConfig::for_tool::<
                    intellihelper_tools::implementations::use_tool::UseTool,
                >(),
            ])
            .await;
            ...
            client_hooks.insert(
                ...
                vec![crate::extensions::hooks::ClientHookGroup {
                    matcher: Some(
                        intellihelper_hooks::matcher::HookMatcher::new("linear__save_issue").unwrap(),
                    ),
...
async fn post_tool_use_and_failure_never_double_fire() {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_intellihelper_build_agent_with_todo().await;
...
fn file_registry_with_stop_spec(
    event: intellihelper_hooks::event::HookEventName,
    script: &str,
) -> intellihelper_hooks::discovery::HookRegistry {
    let (mut registry, _) = intellihelper_hooks::discovery::load_hooks(None, None);
    registry.append_specs(vec![intellihelper_hooks::config::HookSpec {
...
async fn subagent_session_gates_on_subagent_stop() {
    ...
    local
        .run_until(async {
            ...
            *actor.hook_registry.borrow_mut() =
                Some(std::sync::Arc::new(file_registry_with_stop_spec(
                    intellihelper_hooks::event::HookEventName::SubagentStop,
...
/// `"subagent_stop"` on the wire, matching `INTELLIHELPER_HOOK_EVENT`.
#[tokio::test(flavor = "current_thread")]
async fn alias_envelope_serializes_canonical_event_name() {
    ...
    local
        .run_until(async {
            ...
            let envelope = actor.make_hook_envelope(
                intellihelper_hooks::event::HookEventName::SubagentEnd,
                None,
                intellihelper_hooks::event::HookPayload::SubagentStop {
                    phase: intellihelper_hooks::event::SubagentStopPhase::Observe,
