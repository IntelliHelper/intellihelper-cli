...
) -> (Vec<hooks_plugins_types::MarketplacePluginEntry>, bool) {
    let mut candidates = Vec::new();
    let mut official_source_present = false;
    for source in response.sources {
        let is_official = source.source_name == intellihelper_plugin_marketplace::OFFICIAL_SOURCE_NAME
...
pub(super) fn plugin_cta_phase_for(
    ...
    use intellihelper_plugin_marketplace::matcher::{KeywordCandidate, match_plugin_keyword};
...
pub(super) fn handle_cta_plugin_install_done(
    ...
    log_event(intellihelper_telemetry::events::PluginCtaInstalled {
    ...
    match result {
        Ok(servers) => {
            ...
            if needs_auth {
                ...
                log_event(intellihelper_telemetry::events::ExtensionsModalOpened {
                    trigger: intellihelper_telemetry::events::ExtensionsModalTrigger::AuthHandoff,
    ...
    match result {
        Ok(mut response) => {
            ...
            if let Some(agent) = app.agents.get_mut(&agent_id) {
                ...
                if enabled {
                    agent.plugin_cta.dismissed = intellihelper_shell::config::dismissed_plugin_ctas();
                }
                ...
                if matches!(
                    ...
                    {
                        log_event(intellihelper_telemetry::events::PluginCtaImpression {
