...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        intellihelper_telemetry::unified_log::info("agent initialized", None, None);
        ...
        {
            let root = crate::util::intellihelper_home::intellihelper_home();
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    intellihelper_workspace::permission::cleanup_stale_permission_state(
        ...
        intellihelper_workspace::trust::migrate_legacy_hook_trust();
        if let Some(auth) = self.auth_manager.current() {
            ...
            intellihelper_telemetry::unified_log::info(
                ...
                Some(
                    ...
                    "key_prefix": intellihelper_auth::bearer_suffix(&auth.key),
                    "rt_prefix": auth.refresh_token.as_deref().map(intellihelper_auth::bearer_suffix),
                }),
                ),
            );
        ...
        if !self.tier_allowed.get() && let Some(auth) = self.auth_manager.current() {
            self.enforce_intellihelper_code_access(&auth).await;
        }
        ...
        if client_type == ClientType::Generic {
            match client_identifier.as_deref() {
                Some("intellihelper-web") => client_type = ClientType::IntelliHelperWeb,
                Some("nebula") => client_type = ClientType::Nebula,
                Some("intellihelper-code-extension") => client_type = ClientType::Extension,
                Some("intellihelper-desktop") => client_type = ClientType::Desktop,
                _ => {}
            }
        }
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        let pre = self
            ...
            .current()
            .map(|a| (
                intellihelper_auth::bearer_suffix(&a.key).to_owned(),
                a
                    .refresh_token
                    .as_deref()
                    .map(|t| intellihelper_auth::bearer_suffix(t).to_owned()),
            ));
        ...
        // Catalog + raw `[model.*]` from config.toml. Config BYOK must count even
        // if the catalog has not been rebuilt yet — otherwise open-source users
        // with only per-model api_key entries hit the empty-methods login wall.
        let has_config_byok = self.cfg.borrow().config_models.values().any(|m| {
            m.api_key.as_ref().is_some_and(|k| !k.trim().is_empty())
                || m.env_key
                    .as_ref()
        ...
        let has_external_api_key = if disable_api_key_auth {
            false
        } else {
            has_config_byok
        ...
        if has_enterprise_oidc {
            ...
            tracing::info!(
                label = ?login_label,
                has_auth_provider,
                "auth: browser OAuth (intellihelper.com) only when enable_browser_login",
            );
        }
        ...
        // Browser/Grok OAuth is opt-in. Enterprise OIDC, external auth provider,
        // preferred_method=oidc, or INTELLIHELPER_ENABLE_BROWSER_AUTH re-enable it.
        let enable_browser_login = has_enterprise_oidc
            || has_auth_provider
            || matches!(
                preferred_method,
                Some(crate::auth::PreferredAuthMethod::Oidc)
            )
            || auth_method::browser_auth_enabled();
        ...
        intellihelper_telemetry::unified_log::info(
            ...
            Some(
                serde_json::json!({
                "intellihelper_home": crate::util::intellihelper_home::intellihelper_home().display().to_string(),
        ...
        Ok(
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                ...
                .meta({
                    let metadata = parse_json_object_env("INTELLIHELPER_AGENT_METADATA");
                    serde_json::json!({
                    "grokShell": true,
    ...
    async fn authenticate(
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::XAI_API_KEY_METHOD_ID => {
                ...
                log_event(intellihelper_telemetry::events::Login {
            ...
            auth_method::INTELLIHELPER_COM_METHOD_ID | auth_method::OIDC_METHOD_ID => {
                let intellihelper_ctx = self.auth_manager.grok_com_config();
                let auth_meta = AuthRequestMeta::from_json(arguments.meta.as_ref());
                tracing::info!(
                    ...
                    })?;
                {
                    ...
                    tracing::debug!("auth: intellihelper.com/oidc handler set api_key (SessionToken)");
                    intellihelper_telemetry::unified_log::debug(
                        "auth: intellihelper.com/oidc handler set api_key (SessionToken)",
                        None,
                        None,
                    );
                }
    ...
    async fn prompt(
            ...
            .and_then(|m| m.get("toolOverrides"))
        {
            None => None,
            Some(value) => {
                match intellihelper_sampling_types::ToolOverridesUpdate::parse(value) {
            ...
            Ok(turn_ok) => {
                ...
                if let Some(ctx) = trace_context {
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = intellihelper_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = intellihelper_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
        ...
        let result = match method.as_ref() {
            ...
            "x.ai/cloud/terminate" => {
                crate::extensions::auth_gate::require_xai_auth(
                    ...
                    "Run `intellihelper login` to authenticate.",
                )?;
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "x.ai/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                intellihelper_telemetry::session_ctx::log_event(intellihelper_telemetry::events::NonGitDecisionEvent {
                    decision: params.decision,
                    session_id: params.session_id,
                    client_version: params.client_version,
                });
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_followup" {
            ...
            >(args.params.get()) {
                ...
                intellihelper_telemetry::session_ctx::log_event(intellihelper_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| intellihelper_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_apply" {
            ...
            >(args.params.get()) {
                ...
                intellihelper_telemetry::session_ctx::log_event(intellihelper_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_discard" {
            ...
            >(args.params.get()) {
                ...
                intellihelper_telemetry::session_ctx::log_event(intellihelper_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| intellihelper_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == intellihelper_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                intellihelper_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            intellihelper_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
