#!/usr/bin/env node
// Runs once after npm install/update. Reads the intellihelper binary from the
// matching per-platform optional dependency (@intellihelper/cli-<platform>)
// and installs it to ~/.intellihelper/bin/ using versioned filenames:
//
//   Unix:    intellihelper-<version>  +  intellihelper  (symlink)
//   Windows: intellihelper-<version>.exe  +  intellihelper.exe  (copy)
//
...
// $INTELLIHELPER_HOME (else ~/.intellihelper), matching the Rust intellihelper_home() including its
// canonicalized-home default. Lets fleets relocate the binary off a slow $HOME
// (NFS); old code hardcoded os.homedir().
function defaultIntelliHelperHome() {
    const home = os.homedir();
    try { return path.join(fs.realpathSync(home), '.intellihelper'); } catch { return path.join(home, '.intellihelper'); }
}
const INTELLIHELPER_HOME = process.env.INTELLIHELPER_HOME ?? defaultIntelliHelperHome();
const CANONICAL_DIR = path.join(INTELLIHELPER_HOME, 'bin');
...
if (!SUPPORTED.has(key)) {
    console.error(`@intellihelper/cli: unsupported platform ${key}`);
    process.exit(0);
}
...
function resolvePlatformPackageDir() {
    const platformPkg = `@intellihelper/cli-${key}`;
    try {
...
if (!version) {
    console.error('@intellihelper/cli: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (!fs.existsSync(versionedPath) && !writeVendorBinary(brPath, rawPath, versionedPath)) {
        console.error(`@intellihelper/cli: missing binary at ${brPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@intellihelper/cli: failed to update ${canonicalPath}: ${e2.message}`);
                console.error('Close all running intellihelper processes and try again.');
                return false;
            }
        }
    } else {
    ...
    if (!fs.existsSync(canonicalPath)) {
        console.error(`@intellihelper/cli: ${canonicalName} did not resolve after install`);
        return false;
    }
...
// Uses an exact prefix match + hyphen + digit to avoid intellihelper-* matching intellihelper-pager-*.
...
if (!platformDir) {
    console.error(`@intellihelper/cli: platform package @intellihelper/cli-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @intellihelper/cli');
    process.exit(0);
}
...
const configDir = INTELLIHELPER_HOME;
...
// Persist the npm registry so `intellihelper update` and the trampoline use the same one.
const npmRegistry = process.env.INTELLIHELPER_NPM_REGISTRY
    || (() => {
        try {
