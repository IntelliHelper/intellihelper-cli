...
//!
//! `intellihelper-pager` uses this module to set up OTLP
...
const ENV_OTEL_FILTER: &str = "INTELLIHELPER_OTEL_FILTER";
...
///
/// Built by the binaries (`intellihelper-pager`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `intellihelper_shell::auth::credential_provider::build_otel_credential_provider`.
pub struct OtelLayerConfig {
    ...
    /// Value for the `X-XAI-Token-Auth` header (typically `"intellihelper-cli"`).
...
pub struct OtelClientInfo {
    /// Binary name (`intellihelper-pager`) -> `client.name`.
...
///
/// - `client_name`: binary name (e.g. `"intellihelper-tui"`, `"intellihelper-pager"`) -- stored as
///   `client.name` resource attribute for dashboards to distinguish client types.
/// - `client_version`: `CARGO_PKG_VERSION` -- sent in the `x-intellihelper-client-version` header.
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let tracer = provider.tracer("intellihelper-cli");
    ...
    let otel_filter = tracing_subscriber::filter::EnvFilter::try_new(&otel_filter)
        .unwrap_or_else(|e| {
            eprintln!(
                "[otel] Invalid INTELLIHELPER_OTEL_FILTER '{}': {}. Using default '{}'.",
...
fn build_export_headers(
    ...
    snapshot: &intellihelper_auth::CredentialSnapshot,
...
fn build_server_provider(client: OtelClientInfo, config: OtelLayerConfig) -> SdkTracerProvider {
    ...
    let mut provider = SdkTracerProvider::builder().with_resource(
        opentelemetry_sdk::Resource::builder_empty()
            .with_service_name("intellihelper-cli")
    ...
    if config.exporter.enabled {
        ...
        static_headers.insert(
            "x-intellihelper-client-version".to_string(),
...
mod tests {
    ...
    use std::sync::Mutex;
    use intellihelper_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
    ...
    fn make_exporter(
        ...
        RefreshableSpanExporter {
            ...
            token_header_value: Arc::from("intellihelper-cli"),
