//! Minimal (scrollback-native) render mode — `intellihelper --minimal`.
//!
...
//! # Wiring
//!
//! `intellihelper-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`intellihelper_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`intellihelper-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `intellihelper-pager`.
///
...
/// subsequent calls are ignored (see [`intellihelper_pager::minimal_hook`]).
pub fn install() {
    intellihelper_pager::minimal_hook::install(intellihelper_pager::minimal_hook::MinimalHooks { draw });
}
