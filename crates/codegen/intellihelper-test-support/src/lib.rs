...
//! Shared test utilities for intellihelper-build crates: mock inference server, SSE
...
//! - [`IntelliHelperStdioClient`] — ACP client that drives `intellihelper agent stdio` as a subprocess
//! - [`RawStdioClient`] — raw-wire ACP driver for bytes the typed client can't
//!   produce (Foundation `\/` methods, string UUID ids)
//! - [`leader::LeaderStdioClient`] — ACP client that drives `intellihelper agent --leader stdio` (unix)
...
//! - [`run_headless`] — Run `intellihelper -p` against the mock server and capture output
//! - [`git_workdir`] — Create a git-initialized [`TestSandbox`]
//! - [`intellihelper_binary`] — Resolve the intellihelper binary path (INTELLIHELPER_BINARY env or cargo_bin)
...
/// Multiply a harness timeout by `INTELLIHELPER_TEST_TIMEOUT_SCALE` (positive integer,
/// default 1). CI lanes on shared runner pools raise it so pool load slows
/// tests instead of failing them (see the IntelliHelper CLI merge CI workflow).
pub fn scaled(base: std::time::Duration) -> std::time::Duration {
    let scale = std::env::var("INTELLIHELPER_TEST_TIMEOUT_SCALE")
        .ok()
...
#[cfg(unix)]
pub mod uds_proxy;
pub use acp_client::{IntelliHelperStdioClient, RawStdioClient};
pub use counting_server::spawn_counting_server;
pub use env::{EnvGuard, git_workdir, intellihelper_binary};
pub use headless::{
