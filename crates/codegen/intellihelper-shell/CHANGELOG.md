...
- **intellihelper doctor** now warns when tmux is reducing colors and can fix the config.
...
- **INTELLIHELPER_EXTRA_CA_BUNDLE** env var allows adding custom TLS root certificates.
...
- **IntelliHelper** no longer crashes on startup when the host machine has no free threads.
...
- **MCP servers** can now be enabled or disabled directly from the CLI with `intellihelper mcp enable <name>` and `intellihelper mcp disable <name>`.
...
- **Invalid MCP server entries** in config.toml no longer prevent IntelliHelper from starting; problems are shown in `intellihelper inspect`.
...
- **Fixed credential loss** across multiple intellihelper processes sharing the same auth file.
...
- **New /tutorial slash command** opens an opt-in nine-topic onboarding tour of IntelliHelper.
...
- **`intellihelper doctor fix`** can now repair common tmux clipboard and passthrough problems.
- **Per-provider auth helpers** now work on Windows and can run from a configurable working directory.
- **/resume** now shows only native IntelliHelper sessions by default and shows a hint when external sessions are hidden.
- **`intellihelper --resume`** can now resume a session by its title as well as by ID.
...
- **Web search** now defaults to intellihelper-4.5.
...
- You can now run `intellihelper doctor fix` commands directly from inside the TUI instead of only from the CLI.
...
- **npm package** now installs the native binary under `$INTELLIHELPER_HOME/bin` (honoring the same override as the Rust CLI).
...
- IntelliHelper now automatically stops a turn that keeps repeating the exact same tool call many times in a row.
...
- When the auto-permission classifier times out or fails, IntelliHelper now shows a normal permission prompt instead of silently denying.
...
- **intellihelper doctor fix ssh-wrap** can set up `intellihelper wrap ssh` automatically for Bash, zsh, and fish.
...
- **intellihelper doctor** checks terminal, tmux, clipboard, and keyboard setup without opening the TUI.

## Bug Fixes

- **Image paste** over intellihelper wrap now works on headless remotes.
...
- **Added INTELLIHELPER_CLIPBOARD_NO_OSC52** env var to stop clipboard sequences from appearing as garbage in unsupported terminals.
...
- **/btw** now works inside `intellihelper --minimal`, showing answers in the live area and committing them to scrollback on Esc.
...
- **Default model** is now IntelliHelper 4.5 with high/medium/low reasoning effort and improved compaction settings.
...
- **Global rules** from ~/.intellihelper/rules and compatible vendor homes are now discovered correctly.
...
- **SSH sessions** now show a one-time tip recommending `intellihelper wrap ssh <host>` for clipboard and terminal restore.
...
- **intellihelper** and the agent binary now stay in sync even when no update is installed.
- **Copying** a multiline queued prompt now copies the complete text instead of a collapsed summary.
- **intellihelper wrap** now restores the terminal after SSH disconnects or other abrupt child exits.
- **Voice speech-to-text** now works with per-model API keys in config.toml without requiring `intellihelper login`.
...
- **intellihelper login** now requests IntelliHelper Projects scopes so workspace listing works after consent.
...
- **`intellihelper plugin install`** now accepts a marketplace's registered name as a qualifier.
...
- **Fixed background task tracking** when using intellihelper -p --no-wait-for-background so tasks are properly reaped on exit.
...
- **intellihelper inspect** now shows effective compatibility settings for Cursor, Claude, and Codex sessions.
...
- **Resume hint** after quitting minimal mode now prints the correct intellihelper --minimal --resume command.
...
- **IntelliHelper no longer crashes** when printing resume hints after the terminal pane has closed.
...
- **intellihelper setup --json** prints your team's managed configuration without installing it.
...
- **How-to Guides** modal now shows a tip linking to Ask IntelliHelper above the footer shortcuts.
...
- **`intellihelper worktree ls`** now works as a short alias for `intellihelper worktree list`.
- **MCP tool output truncation** can now be set per-repo in `.intellihelper/config.toml`.
...
- **Resumed intellihelper.com chats** now use the conversation's last model instead of the gateway default.
...
- **File links and media** now work for worktree sessions under ~/.intellihelper/worktrees/.
...
- **Terminal command cards** on intellihelper.com now correctly settle after foreground bash tasks.
...
- **intellihelper inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
- **--minimal** and **--fullscreen** choices are now remembered for future plain intellihelper launches.

## Bug Fixes

- **Queued bash commands** promoted at turn end now render their output instead of disappearing.
- **Xcode / Foundation ACP clients** can now drive intellihelper agent stdio without silent parse drops on session/* calls.
...
- **intellihelper --minimal** now supports native drag-select on classic Windows conhost terminals.
...
- The "Managed by intellihelper.com" link in the Extensions modal is now clickable and underlined.
...
- **intellihelper --minimal** now aligns the prompt, status bar, and messages flush-left with the welcome card.
...
- **intellihelper update** on Windows no longer fails when a previous .old executable is still running.
...
- **intellihelper models** banner now correctly reports per-model API keys and deployment keys.
...
- You can now add a local directory as a plugin marketplace source with `intellihelper plugin marketplace add`.
...
- **Plan files** now default to .intellihelper/plan.md to match IntelliHelper conventions.
...
- **--minimal** flag now shows in `intellihelper --help`.
...
- **Tool result previews** no longer paint opaque panels in `intellihelper --minimal`.
- **intellihelper wrap** now correctly handles quoted strings and shell aliases.
...
- **IntelliHelper.com chat sessions** now integrate fully with the gateway bridge for model catalog and resume.
...
- **Chat sessions show the intellihelper.com model catalog** in the picker.
...
- **`intellihelper update`** downloads have a longer timeout.
...
- **Pasting images** from the local clipboard now works when running commands through `intellihelper wrap`.
...
- **intellihelper agent stdio** now checks for updates in the background like other modes.
...
- **intellihelper wrap** now shows copy success over SSH and suggests native drag-select when paste fails.
...
- **Fixed `intellihelper agent stdio` hangs** on Windows when used with persistent clients such as VS Code.
...
- **Added `intellihelper wrap`** to run any command with local clipboard support.
...
- **Fixed `intellihelper agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or intellihelper-desktop.
...
- You can now pass --json-schema to intellihelper -p and receive a validated JSON object instead of free text.
...
- **Agent-run terminal commands** now set `INTELLIHELPER_AGENT=1` so host tools can tell them apart from interactive shells.
...
- **intellihelper -w --ref <branch>** now creates worktrees based on the specified ref instead of HEAD.
...
- **--disable-web-search** now honored in intellihelper -p and intellihelper agent; auxiliary model routing respects catalog overrides.
...
- The legacy `agent` command is now kept in sync with `intellihelper` after running `intellihelper update`.
- Headless (`intellihelper -p`) runs now wait for background tasks and subagents to finish before exiting.
...
- **`intellihelper plugin install <name>`** now resolves plugins from registered marketplaces instead of only local paths.
...
- **intellihelper sessions delete <id>** command now lets you permanently remove a session from the CLI.
...
- **intellihelper quit** no longer hangs when background git or network tasks are slow.
...
- Session lists (welcome screen, /resume, intellihelper sessions list) are now sorted by the same activity time shown in the UI.
...
- **Added option** to fully disable the hunk tracker via --hunk-tracker-mode, INTELLIHELPER_HUNK_TRACKER, or config.
...
- **IntelliHelper** can now export usage metrics and events to your own OpenTelemetry collector when enabled.
...
- **`intellihelper dashboard`** now reliably opens the dashboard instead of silently falling through to a normal session.
...
- **`intellihelper mcp add`** now accepts positional arguments (e.g. `intellihelper mcp add filesystem -- npx ...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`intellihelper update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.intellihelper.
...
- **MCP servers menu** and slash commands now work when starting intellihelper outside a project directory.
...
- **intellihelper --resume** now correctly finds the real session instead of failing on empty image-only folders.
...
- **Fixed `intellihelper --resume`** failing on empty image-only session folders left by cross-directory pastes.
...
- **Ctrl+C** now cancels a blocking `intellihelper update` cleanly instead of leaving an orphaned download repainting the terminal.
...
- **`intellihelper --debug`** now produces per-session log files under ~/.intellihelper/debug/ even with a leader process.
...
- **`intellihelper update`** now restarts any older running leader so all clients get the new binary.
...
- **`intellihelper login`** now defaults to device code flow, which works reliably in SSH, WSL, VPN, and browser-restricted environments.
...
- **API-key users** can now run `intellihelper agent --leader` without forced interactive login or timeouts.
...
- **image_gen** can now be enabled via the harness model using [features] in config.toml or the INTELLIHELPER_IMAGE_GEN_HARNESS env var.
...
- **intellihelper inspect** now shows the resolved on/off state and source for every Claude/Cursor compatibility toggle.
- **Cursor MCP servers and hooks** are now discovered and can be disabled independently via INTELLIHELPER_CURSOR_MCPS_ENABLED / INTELLIHELPER_CURSOR_HOOKS_ENABLED.
...
- **Cursor skills and rules** are now discovered alongside IntelliHelper and Claude directories.
...
- **Background tasks** started inside subagents now continue running after the subagent session ends.
- **Image description** now reliably uses the intellihelper-build model instead of falling back to the active session model.
...
- **image_gen** now uses the higher-quality intellihelper-imagine-image-quality model.
