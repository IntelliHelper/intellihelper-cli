...
/// **top border** (e.g. `✗ Session no longer exists`, `✓ Theme: IntelliHelper
...
mod tests {
    ...
    ///
    /// ```text
    /// cargo test -p intellihelper-pager --lib \
    ...
    fn header_location_renders_from_staged_cwd() {
        ...
        state.cwd = std::path::PathBuf::from("/intellihelper-staged-cwd-marker");
        ...
        let top_row: String = (0..area.width)
            ...
            .collect();
        assert!(
            top_row.contains("/intellihelper-staged-cwd-marker"),
    ...
    fn feedback_badge_renders_verbatim_in_neutral_color() {
        ...
        state.error_toast = Some(format!("{check} Theme: IntelliHelper Day"));
        let _ = render_dispatch(&mut buf, Rect::new(0, 0, 80, 3), &theme, &mut state, None);

        let top_row: String = (0..80).map(|x| buf[(x, 0)].symbol().to_string()).collect();
        assert!(
            top_row.contains(&format!("{check} Theme: IntelliHelper Day")),
    ...
    fn render_location_picker_shows_worktree_toggle_in_repo() {
        ...
        let repo = std::env::temp_dir().join("intellihelper-loc-wt-toggle-repo-test");
    ...
    fn render_location_picker_hides_worktree_toggle_outside_repo() {
        ...
        let mut modal = LocationPickerState::new(
            vec![],
            std::path::PathBuf::from("/intellihelper-not-a-repo-xyz-12345"),
            std::collections::HashMap::new(),
        );
        let mut buf = Buffer::empty(area);
        render_location_picker(&mut buf, area, &theme, &mut modal);
