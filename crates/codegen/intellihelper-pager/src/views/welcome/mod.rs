...
struct WelcomeLayoutInput<'a> {
    ...
    prompt_compact: bool,
    announcement: Option<&'a intellihelper_announcements::RemoteAnnouncement>,
...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **IntelliHelper CLI** VERSION+channel **Beta** (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | IntelliHelper CLI Beta [channel] (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **IntelliHelper CLI Beta**  VERSION (left-aligned).
    HeroInline,
}
...
pub(super) fn render_version_badge(
    ...
    let channel = intellihelper_update::channel_label();
    match &mode {
        VersionBadgeMode::Full { .. } => {
            ...
            spans.push(Span::styled(
                format!("{}{}", intellihelper_version::VERSION, channel),
                Style::default().fg(theme.gray),
            ));
            spans.push(Span::styled(
                " Beta",
...
pub struct WelcomeRenderParams<'a> {
    ...
    pub announcement: Option<&'a intellihelper_announcements::RemoteAnnouncement>,
    ...
    pub foreign_resume_hint: Option<&'a intellihelper_workspace::foreign_sessions::RecentForeignSession>,
    pub is_api_key_auth: bool,
    pub session_picker_content_results:
        Option<&'a [intellihelper_shell::extensions::session_search::SearchSessionHit]>,
    ...
    pub gate: Option<&'a intellihelper_shell::auth::GateInfo>,
...
pub fn render_welcome(
    ...
    let mut result = match params.auth_state {
        AuthState::Pending { error } => {
            // Only offer browser login when an interactive method is configured.
            // Default (API-key / BYOK) path must not invent "Login with intellihelper.com".
            let login_text;
            let menu: Vec<(&str, &str)> = if let Some(label) = params.login_label {
                login_text = format!("Login with {}", label);
                vec![("l", login_text.as_str()), ("q", "Quit")]
            } else {
                vec![("q", "Quit")]
            };
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "IntelliHelper CLI is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
/// multi-line block showing the workspace path and the warning that IntelliHelper CLI
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "IntelliHelper CLI may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
...
fn render_announcement_section(
    ...
    buf: &mut Buffer,
    theme: &Theme,
    announcement: &intellihelper_announcements::RemoteAnnouncement,
    min_width_hint: u16,
    content_height: u16,
    expanded: bool,
    mouse_pos: Option<(u16, u16)>,
    ...
    let (cursor_pos, post_flush_escapes) = if show_picker {
        ...
        if centered.height > 2 {
            ...
            let gate_link = p
                ...
                .unwrap_or("https://grok.com/__PROTECT_1__?referrer=intellihelper-build");
...
mod tests {
    ...
    fn foreign_resume_tip_names_each_tool_and_age() {
        use intellihelper_workspace::foreign_sessions::ForeignSessionTool;
        ...
        for (tool, label) in [
            ...
            let hint = intellihelper_workspace::foreign_sessions::RecentForeignSession {
                tool,
    ...
    /// IntelliHelper view when scanned foreign rows exist — even when the native list
    ...
    fn hidden_external_hint_renders_outside_chat_mode() {
        ...
        assert!(
            ...
            "default IntelliHelper filter must pin the hidden-external hint:\n{build_mode}"
        );
        assert!(
    ...
    fn command_auth_arm_shows_url_and_waiting() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.x.ai/oauth2/authorize?client_id=intellihelper";
    ...
    fn long_ann() -> intellihelper_announcements::RemoteAnnouncement {
...
the usual channels. "
    ...
    fn announcement_equal_for_short_message() {
        let area = Rect::new(0, 0, 120, 60);
        let a = intellihelper_announcements::RemoteAnnouncement {
            title: Some("FYI".into()),
