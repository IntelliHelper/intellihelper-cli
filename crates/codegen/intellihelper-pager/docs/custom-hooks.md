...
Hooks let you run custom scripts or HTTP requests at key moments during a IntelliHelper session — for example, before or after a tool runs, when a session starts or ends, or when the agent sends a notification.
...
2. Create a simple hook file, e.g. `~/.intellihelper/hooks/session-start.json`:
   ```json
   {
     "hooks": {
       "SessionStart": [
...
|-----------|-----------------------------------|--------------|-------|
| Global    | `~/.intellihelper/hooks/*.json`            | Always       | Best for personal hooks |
...
**Trusting a project**: Open the hooks modal (`Ctrl+L` on non–VS Code family, or `/hooks` on any terminal including VS Code family) or run `/hooks-trust` (the same folder-trust gate as `--trust`, recorded in `~/.intellihelper/trusted_folders.toml`) the first time you open a project with hooks. This prevents untrusted repos from running arbitrary code.
...
**Tool name aliases**: Claude-style names like `Bash`, `Edit`, `Read` automatically match IntelliHelper's internal names (`run_terminal_cmd`, `search_replace`, `read_file`).
...
IntelliHelper injects the following variables into every hook process:

- `INTELLIHELPER_HOOK_EVENT` — the event name (e.g. `pre_tool_use`, `session_start`, `post_tool_use`)
- `INTELLIHELPER_HOOK_NAME` — the full configured name of this hook
- `INTELLIHELPER_SESSION_ID` — the current session identifier
- `INTELLIHELPER_WORKSPACE_ROOT` — absolute path to the workspace root

For hooks provided by plugins, the following are also set:

- `INTELLIHELPER_PLUGIN_ROOT` — absolute path to the plugin's installation directory
- `INTELLIHELPER_PLUGIN_DATA` — absolute path to the plugin's writable data directory

These runner- and plugin-injected variables always take precedence. Attempts to override the reserved runner keys via the `env` field are stripped at load time (with a warning logged). For plugin hooks, `INTELLIHELPER_PLUGIN_ROOT` and `INTELLIHELPER_PLUGIN_DATA` similarly override any user-supplied values for those keys.
...
`INTELLIHELPER_PLUGIN_ROOT` and `INTELLIHELPER_PLUGIN_DATA`. These keys override any user-declared
...
{
  "type": "command",
  "command": "${HOME}/.config/intellihelper-hooks/check.sh"
}
```

Lookup order for each reference:
1. The handler's own `env` map.
2. The current process environment (the env IntelliHelper itself sees).
...
`${INTELLIHELPER_PLUGIN_ROOT}/check` resolve against the plugin's actual path.
...
Hooks from `~/.intellihelper/hooks/` appear under **Global**, project ones under **Project**, etc.
...
- Global hooks (`~/.intellihelper/...`) run with your user permissions — treat them like shell scripts.
...
- **See errors?** → Check the pager logs (usually in the tracing pane or `~/.intellihelper/logs`).

## More Examples

See the built-in examples in the `intellihelper-hooks` crate:

- [Safe Shell Guard](../../../intellihelper-hooks/examples/hooks/safe-shell.json)
- [No Recursive Grep](../../../intellihelper-hooks/examples/hooks/no-recursive-grep.json) — hard-blocks `grep -r`/`grep -R`/`rgrep` (OOM guard)
- [Session Audit Log](../../../intellihelper-hooks/examples/hooks/session-log.json)
- [Tool Activity Logger](../../../intellihelper-hooks/examples/hooks/tool-logger.json)

Copy them to `~/.intellihelper/hooks/` and customize.
