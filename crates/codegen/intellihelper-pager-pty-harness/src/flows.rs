...
/// the shell's `IntelliHelperAuth::is_zdr_team` trigger) — locks the settings modal's
...
/// names must match the shell's `IntelliHelperAuth` serde names in
/// `intellihelper-shell/src/auth/model.rs`).
