//! `pty-bench` — PTY benchmark CLI for `intellihelper-pager`.
//!
...
//! ```bash
//! cargo bench -p intellihelper-pager-pty-harness \
...
//! ```bash
//! PAGER_BINARY=./artifacts/intellihelper-${VERSION}-linux-x86_64 \
//!   cargo bench -p intellihelper-pager-pty-harness \
//!   --bench pty_bench -- --all \
...
#[command(
    name = "pty-bench",
    about = "PTY benchmark harness for intellihelper-pager",
    long_about = None,
)]
struct Cli {
