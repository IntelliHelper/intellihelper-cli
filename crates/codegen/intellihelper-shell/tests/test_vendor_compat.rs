...
//! `.intellihelper`, `.cursor`, and `.claude` vendor dirs, spawns a real `intellihelper agent
//! stdio` process against the mock inference server (toggling the
//! `INTELLIHELPER_<VENDOR>_<SURFACE>_ENABLED` env vars via `cmd.env`), sends one prompt,
//! and asserts on the full inference request bodies:
//!
//! - the IntelliHelper-native skill is always present regardless of toggles
...
//! ```bash
//! cargo test -p intellihelper-shell --test test_vendor_compat -- --ignored
//! ```
...
const MARKER_INTELLIHELPER_SKILL: &str = "ZZ_INTELLIHELPER_SKILL_MARKER";
...
fn seed_fixtures(home: &Path, cwd: &Path) {
    ...
    write_skill(home, ".intellihelper", "intellihelper-skill", MARKER_INTELLIHELPER_SKILL);
...
async fn run_scenario(env: &[(&str, &str)]) -> String {
    ...
    let client = IntelliHelperStdioClient::spawn_with_sandbox(&server, workdir.workspace(), sandbox).await;
...
/// Defaults (all vendors on): intellihelper + cursor-vendor + claude-vendor skills present; the
...
async fn vendor_compat_defaults_include_vendor_skills_but_drop_denylisted() {
    with_local_set(|| async {
        ...
        assert!(
            body.contains(MARKER_INTELLIHELPER_SKILL),
            "intellihelper-skill must always be present"
        );
        assert!(
...
/// `INTELLIHELPER_CURSOR_SKILLS_ENABLED=false` drops the cursor-vendor skill; intellihelper stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("INTELLIHELPER_CURSOR_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_INTELLIHELPER_SKILL),
            "intellihelper-skill always present"
        );
        assert!(
            !body.contains(MARKER_CURSOR_SKILL),
...
/// `INTELLIHELPER_CLAUDE_SKILLS_ENABLED=false` drops the claude-vendor skill; intellihelper stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("INTELLIHELPER_CLAUDE_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_INTELLIHELPER_SKILL),
            "intellihelper-skill always present"
        );
...
/// `INTELLIHELPER_CURSOR_RULES_ENABLED=false` drops cursor-vendor rules; claude-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("INTELLIHELPER_CURSOR_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_RULE),
...
/// `INTELLIHELPER_CLAUDE_RULES_ENABLED=false` drops claude-vendor rules; cursor-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("INTELLIHELPER_CLAUDE_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_RULE),
...
/// `INTELLIHELPER_CURSOR_AGENTS_ENABLED=false` drops cursor-vendor AGENTS.md; claude-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("INTELLIHELPER_CURSOR_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_AGENTS),
...
/// `INTELLIHELPER_CLAUDE_AGENTS_ENABLED=false` drops claude-vendor AGENTS.md; cursor-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("INTELLIHELPER_CLAUDE_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_AGENTS),
...
async fn vendor_compat_all_cursor_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("INTELLIHELPER_CURSOR_SKILLS_ENABLED", "false"),
            ("INTELLIHELPER_CURSOR_RULES_ENABLED", "false"),
            ("INTELLIHELPER_CURSOR_AGENTS_ENABLED", "false"),
        ])
        .await;
        ...
        assert!(body.contains(MARKER_INTELLIHELPER_SKILL), "intellihelper always present");
...
async fn vendor_compat_all_claude_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("INTELLIHELPER_CLAUDE_SKILLS_ENABLED", "false"),
            ("INTELLIHELPER_CLAUDE_RULES_ENABLED", "false"),
            ("INTELLIHELPER_CLAUDE_AGENTS_ENABLED", "false"),
        ])
        .await;
...
/// All vendor compat OFF: only intellihelper-native skill survives.
#[tokio::test]
#[ignore] // requires pre-built binary
