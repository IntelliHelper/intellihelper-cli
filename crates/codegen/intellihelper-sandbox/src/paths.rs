...
// ── IntelliHelper state directory ────────────────────────────────────────────────────

/// IntelliHelper state directory — always writable (`$INTELLIHELPER_HOME` or `~/.intellihelper`).
pub(crate) fn intellihelper_home() -> PathBuf {
...
pub(crate) fn essential_writable_paths(workspace: &Path) -> Vec<PathBuf> {
    let mut paths = vec![workspace.to_path_buf(), intellihelper_home()];
    paths.extend(temp_writable_paths());
    paths
}

/// Writable directory paths for the read-only profile (minimal: just ~/.intellihelper + temp).
/// Device files are handled separately via `allow_file` in `to_capability_set_with_config`.
pub(crate) fn essential_writable_paths_minimal() -> Vec<PathBuf> {
    let mut paths = vec![intellihelper_home()];
    paths.extend(temp_writable_paths());
    paths
}
