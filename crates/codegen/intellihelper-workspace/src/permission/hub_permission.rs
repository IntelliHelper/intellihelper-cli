...
static PERMISSION_REPLY_DURATION: LazyLock<HistogramVec> = LazyLock::new(|| {
    register_histogram_vec!(
        "intellihelper_workspace_permission_reply_seconds",
        "Wall-clock time awaiting chat's reply to a permission_request hook",
        &["outcome"],
        vec![0.5, 1.0, 2.0, 5.0, 10.0, 30.0, 60.0, 120.0, 300.0, 600.0]
    )
    .expect("intellihelper_workspace_permission_reply_seconds must register once")
});
...
static PERMISSION_TIMEOUT_TOTAL: LazyLock<IntCounter> = LazyLock::new(|| {
    register_int_counter!(
        "intellihelper_workspace_permission_timeout_total",
        "permission_request hooks whose reply timed out (backstop deadline fired)"
    )
    .expect("intellihelper_workspace_permission_timeout_total must register once")
});
...
pub const HITL_PERMISSION_LIVE_ENV: &str = "INTELLIHELPER_HITL_PERMISSION_LIVE";
/// Whether the HITL-live permission path is enabled.
///
/// Intended long-term gate: the chat flag `intellihelper_chat_enable_hitl_live_path`,
...
pub fn access_kind_for_hub_tool(tool_name: &str, args: &Value) -> Option<AccessKind> {
    ...
    let name = name.strip_prefix("IntelliHelperBuild:").unwrap_or(name);
    match name {
