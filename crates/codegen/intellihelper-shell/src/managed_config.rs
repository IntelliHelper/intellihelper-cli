...
pub const MANAGED_ARTIFACT_FILES: [&str; 4] = [
    ...
    intellihelper_config::signed_policy::SIGNATURE_SIDECAR_FILE,
    intellihelper_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE,
];
...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    if artifacts_removed {
        remove_synced_file(
            home,
            intellihelper_config::MANAGED_CONFIG_CACHE_FILE,
    ...
    let atomic_write_tmp_prefixes = [
        format!("{}.", intellihelper_config::MANAGED_CONFIG_CACHE_FILE),
        format!(
            "{}.",
            intellihelper_config::signed_policy::SIGNATURE_SIDECAR_FILE
        ),
...
/// would just 401).
fn eligible_team_principal(auth: IntelliHelperAuth) -> Option<IntelliHelperAuth> {
...
/// config is a intellihelper.com feature with one intellihelper.com auth.
fn read_active_team_auth() -> Option<IntelliHelperAuth> {
    let home = crate::util::intellihelper_home::intellihelper_home();
...
pub fn clear_orphan() {
    ...
    if intellihelper_config::fail_closed_policy_armed_at(&home) {
        tracing::info!(
...
/// files (TUI tick vs `intellihelper login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `intellihelper setup`; runs retries to completion.
    Standard,
...
/// Base is 1s; `INTELLIHELPER_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("INTELLIHELPER_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `intellihelper-http` classifier.
...
async fn fetch_managed_config_once(
    ...
    if let Some(nonce) = intellihelper_config::signed_policy::stored_envelope_nonce(
        &crate::util::intellihelper_home::intellihelper_home(),
        echo_principal,
    ) && let Ok(value) = reqwest::header::HeaderValue::from_str(&nonce)
    {
        request = request.header(
            intellihelper_config::signed_policy::MANAGED_CONFIG_NONCE_ECHO_HEADER,
            value,
        );
    }
...
/// Override with `INTELLIHELPER_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("INTELLIHELPER_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
/// snapshots, and OTel: the **server** IntelliHelperBuildDeployment UUID (the id
...
/// Resolve deployment key from `INTELLIHELPER_DEPLOYMENT_KEY` env var, then config files.
...
/// Callers doing auto-fetch should check this; explicit user actions (intellihelper setup) skip it.
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("INTELLIHELPER_MANAGED_CONFIG") {
        return v;
    }
...
/// Fetch managed config + requirements and write to `~/.intellihelper/`, trying the
...
async fn sync_bounded(
    budget: SyncBudget,
    team_override: Option<IntelliHelperAuth>,
...
/// read-only `intellihelper setup --json` both build on this.
...
fn apply_fetched(
    ...
    let verified = if intellihelper_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(intellihelper_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        intellihelper_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
        ...
        {
            clear_squatting_dir(
                &home.join(intellihelper_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE),
            );
    ...
    clear_squatting_dir(&home.join(intellihelper_config::MANAGED_CONFIG_CACHE_FILE));
...
fn served_principal_of(payload: &intellihelper_config::signed_policy::SignedPayload) -> Option<&str> {
    payload
        .deployment_id
        .as_deref()
...
fn verified_claim_sidecar(
...
) -> Option<intellihelper_config::signed_policy::SignatureEnvelope> {
    use intellihelper_config::signed_policy::now_unix;
    ...
    let claim = match intellihelper_config::signed_policy::verify_fetched_claim(&sidecar, now_unix()) {
        Ok(claim) => claim,
        Err(e) => {
...
fn claim_binds_to(
    claim: &intellihelper_config::signed_policy::ManagedIdentityClaim,
    served_principal: Option<&str>,
) -> bool {
...
/// Post-login hook for `intellihelper login` and the ACP/TUI authenticate flow: clear any
...
pub async fn post_login_sync(authenticated: Option<IntelliHelperAuth>) -> ManagedConfigSync {
    clear_orphan();
    if !is_fetch_enabled() {
...
/// Whether a credential exists that `intellihelper setup` could install config for.
...
pub async fn ensure_managed_policy_present(
    ...
    let team = tokio::time::timeout(SESSION_START_AUTH_DEADLINE, auth_manager.auth())
        ...
        .ok()
        .and_then(Result::ok)
        .filter(IntelliHelperAuth::is_team_principal);
    if !has_principal() {
        return;
    }
...
/// are the non-forgeable layers. Recovery: reconnect / `intellihelper setup`; ceasing to serve
...
fn bump_managed_rollback_floor() {
    ...
    if !intellihelper_config::signed_policy::verification_active() {
        return;
    }
    let home = crate::util::intellihelper_home::intellihelper_home();
    match try_lock_managed_config(&home) {
        Some(_lock) => {
            intellihelper_config::bump_rollback_floor(&home);
        }
...
/// Outcome of the `intellihelper setup` sync. The caller renders it — CLI presentation
/// and exit codes stay out of the library.
#[derive(Debug)]
pub enum SetupOutcome {
    /// Config was written to `~/.intellihelper`.
    Installed,
...
/// Result of `intellihelper setup --json`: what the server serves for the current
/// principal, verbatim. `managed_config` may embed the enforced deployment key,
/// exactly as `intellihelper setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `intellihelper setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `intellihelper setup --json` without writing anything:
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `intellihelper setup` would refuse
...
/// Run the `intellihelper setup` sync for the current principal. The caller must check
