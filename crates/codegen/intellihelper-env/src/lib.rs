...
//! Backend environment presets for the IntelliHelper CLI crate family: endpoint URL
//! defaults, environment selection, and env-var test support.
//!
//! Public builds expose production endpoints. Values resolve as a `INTELLIHELPER_*`
//! env-var override when set, else the compiled production default.
/// The endpoint set for one backend environment.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct IntelliHelperBuildEndpoints {
...
const PRODUCTION_ENDPOINTS: IntelliHelperBuildEndpoints = IntelliHelperBuildEndpoints {
...
pub enum IntelliHelperBuildEnvironment {
    #[default]
    Production,
}
impl IntelliHelperBuildEnvironment {
    pub fn from_flags(_dev: bool, _staging: bool) -> Self {
        IntelliHelperBuildEnvironment::Production
    }
    /// Indicator string for display; `None` for Production.
    pub fn indicator(&self) -> Option<&'static str> {
        match self {
            IntelliHelperBuildEnvironment::Production => None,
        }
    }
    pub fn is_production(&self) -> bool {
        matches!(self, IntelliHelperBuildEnvironment::Production)
    }
    fn env_prefix(&self) -> &'static str {
        match self {
            IntelliHelperBuildEnvironment::Production => "INTELLIHELPER_PRODUCTION",
        }
    }
    /// Compiled endpoint set for this environment (production by default).
    pub fn endpoints(&self) -> IntelliHelperBuildEndpoints {
        match self {
            IntelliHelperBuildEnvironment::Production => PRODUCTION_ENDPOINTS,
        }
    }
    ...
    /// The relay WebSocket URL (Web Frontend at `intellihelper.com/code` driving a
    ...
    /// `INTELLIHELPER_GATEWAY_URL` opt-in takes precedence.
    pub fn gateway_ws_url(&self) -> String {
        self.resolve("_GATEWAY_WS_URL", self.endpoints().gateway_ws_url)
    }
}
impl std::fmt::Display for IntelliHelperBuildEnvironment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            IntelliHelperBuildEnvironment::Production => write!(f, "production"),
        }
    }
}
...
mod tests {
    ...
    fn test_env_prefix() {
        assert_eq!(
            IntelliHelperBuildEnvironment::Production.env_prefix(),
            "INTELLIHELPER_PRODUCTION"
        );
    }
    #[test]
    fn env_var_guard_set_value_updates_then_restores_on_drop() {
        const KEY: &str = "INTELLIHELPER_ENV_VAR_GUARD_SET_VALUE_PROBE";
    ...
    fn relay_and_gateway_urls_are_distinct() {
        assert_ne!(
            IntelliHelperBuildEnvironment::Production.relay_ws_url(),
            IntelliHelperBuildEnvironment::Production.gateway_ws_url(),
        );
    }
    #[test]
    fn test_from_flags() {
        assert_eq!(
            IntelliHelperBuildEnvironment::from_flags(false, false),
            IntelliHelperBuildEnvironment::Production
        );
    }
}
