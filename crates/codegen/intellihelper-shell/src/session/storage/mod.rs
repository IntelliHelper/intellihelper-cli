...
//   - production, current intellihelper home:   `load_updates_for_replay`
//   - production, streaming (bounded): `stream_replay_updates_at`
//   - tests, explicit intellihelper home:       `load_updates_for_replay_at` (typed reference)
...
/// intellihelper home. Typed, materialize-all replay reader: collects every update into
...
pub fn load_updates_for_replay_at(
    ...
    let Some(updates_path) = resolve_replay_updates_path(session_id, intellihelper_home)? else {
        return Ok(None);
    };
    Ok(Some(collect_replay_updates(&updates_path)?))
}
...
/// Resolve `updates.jsonl` for `session_id` under `intellihelper_home`, or `None` when
...
/// Invoke `f` once per replay-ready ACP update for a session under `intellihelper_home`,
...
mod tests {
    ...
    fn stream_replay_updates_at_missing_session_is_empty() {
        ...
        std::fs::create_dir_all(intellihelper_home.path().join("sessions")).unwrap();

        let mut count = 0usize;
        let emission =
            stream_replay_updates_at("does-not-exist", intellihelper_home.path(), |_| count += 1).unwrap();
    ...
    fn stream_replay_updates_at_surfaces_read_errors() {
        let intellihelper_home = tempfile::tempdir().unwrap();
        let session_dir = intellihelper_home.path().join("sessions").join("cwd").join("sess");
        std::fs::create_dir_all(&session_dir).unwrap();
        std::fs::write(session_dir.join(SUMMARY_FILE), "{}").unwrap();
        std::fs::create_dir(session_dir.join(UPDATES_FILE)).unwrap();

        let result = stream_replay_updates_at("sess", intellihelper_home.path(), |_| {});
        assert!(
            result.is_err(),
