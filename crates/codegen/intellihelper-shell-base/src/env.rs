//! IntelliHelperBuildEnvironment configuration for the shell crate family.
//!
//! The environment presets (per-environment endpoint URLs, the staging
//! trust check, `EnvVarGuard`) live in the [`intellihelper_env`] leaf crate so
...
//! - `INTELLIHELPER_GATEWAY_URL` — when set to a valid URL, `MvpAgent` spawns a
//!   per-session gateway bridge actor and routes prompts through
//!   it. Unset → falls back to [`IntelliHelperBuildEnvironment::gateway_ws_url`] for
//!   sessions created in gateway mode; otherwise local-mode (unchanged).
pub use intellihelper_env::{
    IntelliHelperBuildEnvironment, PROD_ASSET_SERVER_URL, PROD_CLI_CHAT_PROXY_BASE_URL, PROD_GATEWAY_WS_URL,
...
pub use intellihelper_env::EnvVarGuard;
...
pub const INTELLIHELPER_GATEWAY_URL_ENV: &str = "INTELLIHELPER_GATEWAY_URL";
...
pub const INTELLIHELPER_DISABLE_CUSTOM_BRIDGE_ENV: &str = "INTELLIHELPER_DISABLE_CUSTOM_BRIDGE";
/// `true` when the custom-method bridge passthrough is force-disabled via
/// [`INTELLIHELPER_DISABLE_CUSTOM_BRIDGE_ENV`]. Accepts `1`/`true` (case-insensitive).
pub fn custom_bridge_disabled() -> bool {
    std::env::var(INTELLIHELPER_DISABLE_CUSTOM_BRIDGE_ENV)
        .map(|v| {
            let v = v.trim();
...
/// Parse `INTELLIHELPER_GATEWAY_URL` into a [`url::Url`].
///
/// This build ignores the variable and stays in local mode (warns if set).
pub fn parse_gateway_url() -> Option<url::Url> {
    let raw = std::env::var(INTELLIHELPER_GATEWAY_URL_ENV).ok()?;
    if raw.is_empty() {
        return None;
    }
    tracing::warn!(
        env = INTELLIHELPER_GATEWAY_URL_ENV,
        "INTELLIHELPER_GATEWAY_URL is set but this build does not support gateway-bridge mode; staying in local mode"
    );
    None
}
#[cfg(test)]
mod tests {
    ...
    fn parse_gateway_url_returns_none_when_unset() {
        let _env = EnvVarGuard::remove(INTELLIHELPER_GATEWAY_URL_ENV);
        assert!(parse_gateway_url().is_none());
    }
    #[test]
    fn parse_gateway_url_returns_none_when_empty() {
        let _env = EnvVarGuard::set(INTELLIHELPER_GATEWAY_URL_ENV, "");
        assert!(parse_gateway_url().is_none());
    }
    #[test]
    fn parse_gateway_url_hard_off_without_gateway_bridge() {
        let _env = EnvVarGuard::set(INTELLIHELPER_GATEWAY_URL_ENV, "wss://gateway.example.com/ws");
        assert!(
    ...
    fn custom_bridge_disabled_defaults_false_when_unset() {
        let _env = EnvVarGuard::remove(INTELLIHELPER_DISABLE_CUSTOM_BRIDGE_ENV);
        assert!(!custom_bridge_disabled());
    }
    #[test]
    fn custom_bridge_disabled_true_for_one_and_true() {
        for v in ["1", "true", "TRUE", " true "] {
            let _env = EnvVarGuard::set(INTELLIHELPER_DISABLE_CUSTOM_BRIDGE_ENV, v);
            assert!(
