...
async fn handle_session_delete(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // Mirrored by the `intellihelper sessions delete <id>` CLI path.
...
/// hot-reload watcher when `<cwd>/.intellihelper/config.toml`,
...
/// `~/.intellihelper/config.toml` edits.
...
/// Hot-reload the model catalog from `~/.intellihelper/models_cache.json` after an
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // that `x.ai/commands/list` (the pull used by intellihelper-desktop after session
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .intellihelper/plugins under it, plus
