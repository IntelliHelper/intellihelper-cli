...
fn local_intellihelper_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("intellihelper-pager{}", std::env::consts::EXE_SUFFIX))
}

fn ensure_local_intellihelper_binary(binary: &Path) {
    if binary.exists() {
        return;
    }
    ...
    let output = cmd
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build intellihelper-pager: {e}"));

    assert!(
        output.status.success(),
        "failed to build intellihelper-pager for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
    ...
    assert!(
        binary.exists(),
        "intellihelper-pager build completed but binary missing at {}",
        binary.display()
    );
}

/// Resolve intellihelper binary: `INTELLIHELPER_BINARY` env (CI) or a locally built `intellihelper-pager` binary.
pub fn intellihelper_binary() -> PathBuf {
    if let Ok(path) = std::env::var("INTELLIHELPER_BINARY") {
        let p = PathBuf::from(path);
        assert!(p.exists(), "INTELLIHELPER_BINARY does not exist: {}", p.display());
        // Bazel's INTELLIHELPER_BINARY is runfiles-relative; the harness spawns the child
        // with a different cwd, so absolutize against the (runfiles-root) cwd now.
        return std::path::absolute(&p).unwrap_or(p);
    }

    if let Ok(path) = std::env::var("CARGO_BIN_EXE_intellihelper-pager") {
        let p = PathBuf::from(path);
        if p.exists() {
            return p;
        }
    ...
    let binary = local_intellihelper_binary_path();
    ensure_local_intellihelper_binary(&binary);
    binary
}
