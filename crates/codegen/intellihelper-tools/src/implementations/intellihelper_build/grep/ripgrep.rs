...
const RG_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-rg/rg-",
    env!("INTELLIHELPER_TOOLS_RG_VER"),
    "-",
    env!("INTELLIHELPER_TOOLS_RG_TARGET"),
    ".bin"
));
...
fn resolve_bundled_rg() -> std::io::Result<PathBuf> {
    ...
    let p = crate::util::intellihelper_home().join("vendor").join(concat!(
        "rg-",
        env!("INTELLIHELPER_TOOLS_RG_VER"),
        "-",
        env!("INTELLIHELPER_TOOLS_RG_TARGET")
...
/// binary to ~/.intellihelper/vendor/ and returns that path.
