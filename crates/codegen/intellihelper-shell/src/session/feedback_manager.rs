...
pub(crate) fn new_submission(
    ...
    s.shell_version = Some(intellihelper_version::VERSION.to_string());
    s
}
...
pub(crate) async fn submit_feedback_workflow(
    ...
    if let Some(user_meta) = crate::agent::mvp_agent::parse_json_object_env("INTELLIHELPER_USER_METADATA") {
    ...
    if telemetry_enabled {
        ...
        intellihelper_telemetry::session_ctx::log_event(intellihelper_telemetry::events::UserFeedback {
...
pub struct FeedbackManagerConfig {
    ...
    /// ratings). Gated by `INTELLIHELPER_FEEDBACK_ENABLED`.
    pub feedback_enabled: bool,
    /// Whether session analytics (signal sync, turn deltas) are enabled.
    /// Gated by `INTELLIHELPER_TELEMETRY_ENABLED`. These are analytics data that
...
mod author_identity_tests {
    ...
    fn text_submission() -> FeedbackSubmission {
        ...
        s.model_id = Some("intellihelper-4".to_string());
        s
    }
    ...
    async fn env_var_identity_reaches_the_wire_end_to_end() {
        let _email =
            intellihelper_test_support::env::EnvGuard::set("INTELLIHELPER_TEST_WORK_EMAIL", "ada@corp.example");
        let _name =
            intellihelper_test_support::env::EnvGuard::set("INTELLIHELPER_TEST_WORK_NAME", "Ada Lovelace");

        // The loader expands `$VAR` at load, exactly as a trusted config tier ships it.
        let mut value = toml::from_str::<toml::Value>(
            r#"
[feedback.user]
name = ["$INTELLIHELPER_TEST_WORK_NAME"]
email = ["$INTELLIHELPER_TEST_WORK_EMAIL"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(body["modelId"], "intellihelper-4");
        ...
        assert_eq!(persisted.model_id.as_deref(), Some("intellihelper-4"));
    }

    /// `INTELLIHELPER_USER_METADATA` is merged into the submission and travels with it:
    ...
    async fn workflow_merges_user_metadata_into_submission() {
        let _guard = intellihelper_test_support::env::EnvGuard::set(
            "INTELLIHELPER_USER_METADATA",
