//! Canonical external-settings tool name ↔ IntelliHelper tool correspondence: one table
//! replacing two that drifted apart.
//!
//! Two consumers read it independently. The hook matcher (`intellihelper-hooks`) needs the
//! IntelliHelper tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`intellihelper-agent`) needs the [`ToolKind`] a `tools:`
...
//!
//! The `intellihelper` names are test-checked against the live registry.

use super::tool::ToolKind;
use ToolKind::*;

/// One Claude tool's correspondence to IntelliHelper, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// IntelliHelper [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// IntelliHelper tool names this Claude tool maps to (empty when there is no direct
    /// IntelliHelper tool — the entry then only contributes a `kind`).
    intellihelper: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, intellihelper: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        intellihelper,
...
const fn match_only(claude: &'static str, intellihelper: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        intellihelper,
...
const CLAUDE_TOOLS: &[ClaudeTool] = &[
    ...
    k("Skill",           Read,                 &["skill"]),                           // matcher: opencode's `skill` tool; allowlist Read (intellihelper-build reads SKILL.md)
...
/// The IntelliHelper [`ToolKind`] a Claude allowlist entry resolves to, if any.
...
/// The IntelliHelper tool names a Claude matcher term fires on.
pub fn intellihelper_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.intellihelper)
        .unwrap_or(&[])
        .iter()
        .copied()
}

/// The Claude names that map to `intellihelper_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(intellihelper_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.intellihelper.contains(&intellihelper_name))
        .map(|t| t.claude)
}

/// Every distinct IntelliHelper name the table references, for the `intellihelper-agent` drift-check
/// test that asserts each is a real client tool name.
pub fn intellihelper_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.intellihelper.iter().copied())
...
mod tests {
    ...
    fn every_row_contributes() {
        // A row with neither a kind nor a IntelliHelper name is dead weight (and signals a typo).
        for t in CLAUDE_TOOLS {
            assert!(
                t.kind.is_some() || !t.intellihelper.is_empty(),
                "dead row: {}",
                t.claude
            );
        }
