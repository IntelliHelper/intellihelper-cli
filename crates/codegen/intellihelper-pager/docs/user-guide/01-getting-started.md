...
```powershell
$env:INTELLIHELPER_VERSION="0.1.42"; irm https://cli.intellihelper.in/install.ps1 | iex
```

The PowerShell installer automatically adds `%USERPROFILE%\.intellihelper\bin` to your User PATH. Alternatively, install via [Git for Windows](https://gitforwindows.org/) (Git Bash) or MSYS2 using the bash script above. WSL users get the Linux binary automatically.
...
Start IntelliHelper by running:

```bash
intellihelper
```

On first launch, IntelliHelper opens your browser to authenticate with intellihelper.com. After you sign in, IntelliHelper stores your credentials in `~/.intellihelper/auth.json`, where they persist across sessions. IntelliHelper refreshes your credentials automatically and prompts you to sign in again when they can no longer be renewed.
...
Once authenticated, IntelliHelper presents a full-screen TUI with two main areas:

- **Scrollback** -- the conversation history showing your prompts, IntelliHelper's responses, tool calls, file edits, and more.
- **Prompt** -- the input area at the bottom where you type messages.

Type a message and press `Enter` to send it. IntelliHelper reads files, runs commands, and edits code as needed. Each tool run streams into the scrollback in real time.
...
By default, IntelliHelper asks for permission before executing shell commands or editing files. You can approve individually or toggle always-approve mode:

- Press `Ctrl+O` to toggle always-approve mode
- Use the `--yolo` flag at launch: `intellihelper --yolo`
...
Every conversation is a **session**. Sessions are automatically saved to `~/.intellihelper/sessions/` and can be resumed later. Each session tracks the full conversation history, tool calls, file edits, and task state.

- Start a new session: `Ctrl+N` or `/new`
- Resume a previous session: `/resume` in the TUI, or `--resume <ID>` from the CLI
- Continue the most recent session: `intellihelper -c`
...
- **Agent messages** -- IntelliHelper's responses with full markdown rendering and syntax highlighting
- **Thinking blocks** -- IntelliHelper's reasoning process (collapsible)
...
IntelliHelper has built-in tools for:
...
```
/model intellihelper-build                 # Switch model
...
intellihelper "fix the failing auth test and run it"

# Initial prompt in a new git worktree. Use --worktree=<name> (with `=`) so the
# prompt isn't swallowed as the worktree name — `intellihelper -w "refactor module X"`
# would treat "refactor module X" as the worktree label, not the prompt.
intellihelper --worktree=feat "refactor module X"

# Base the worktree on a specific branch (e.g. main) instead of the current HEAD:
intellihelper -w --ref main "implement feature from main"
...
intellihelper -m intellihelper-build

# Resume a previous session
intellihelper --resume <session-id>

# Continue the most recent session
intellihelper -c

# Experimental scrollback-native render mode. Sticky: plain `intellihelper` reopens in
# the mode last chosen via --minimal/--fullscreen (or /minimal//fullscreen).
intellihelper --minimal

# Back to the standard fullscreen TUI (and make it sticky again)
intellihelper --fullscreen
...
Run IntelliHelper non-interactively for scripting, CI/CD, and automation:
...
```bash
intellihelper -p "Review changes for bugs" --output-format json --yolo | jq -r '.text'
```
...
Add per-project instructions by creating an `AGENTS.md` file in your repository. IntelliHelper reads these files and injects their contents as a project-instructions message at the start of the conversation:

```
~/.intellihelper/AGENTS.md           # Global rules (apply to all projects)
<repo-root>/AGENTS.md       # Repository-level rules
<cwd>/AGENTS.md             # Directory-level rules (highest priority)
```

Deeper files take precedence. IntelliHelper also reads `CLAUDE.md` files for compatibility.
