...
//!
//! Moved from `intellihelper-shell/src/session/worktree.rs` into the workspace
...
// Canonical in intellihelper-workspace-types; re-exported for existing paths.
pub use intellihelper_workspace_types::rpc::worktree::{
...
/// Resolve the intellihelper home for worktree paths via the **same** resolver used for
/// `worktrees.db` (`fast_worktree::resolve_intellihelper_home`), so checkout dirs and
/// the metadata DB always live under the same `.intellihelper` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `intellihelper_config::intellihelper_home()`,
/// so worktree paths also agree with trust/hooks and other intellihelper-home paths.
fn intellihelper_home() -> std::path::PathBuf {
    fast_worktree::resolve_intellihelper_home().unwrap_or_else(|_| {
        dirs::home_dir()
...
/// Returns `~/.intellihelper/worktrees/<repo_slug>` for the given git root.
///
...
pub fn worktree_base_dir(git_root: &Path) -> std::path::PathBuf {
    ...
    intellihelper_home().join("worktrees").join(slug)
}

/// Resolves the worktree base directory (`~/.intellihelper/worktrees/<repo_name>`)
/// for a given source path, correctly handling intellihelper-managed worktrees.
///
/// When `source_path` is already under `~/.intellihelper/worktrees/<repo>/...`, the
...
/// original repo).
///
/// For paths outside the intellihelper worktree directory, falls back to
...
pub fn worktree_base_dir_for_source(source_path: &Path) -> Result<std::path::PathBuf> {
    let worktrees_dir = intellihelper_home().join("worktrees");
...
/// Walk up from `cwd` (staying within `~/.intellihelper/worktrees/`) to its registered
/// worktree record.
///
...
/// The recorded source repo of the intellihelper-managed worktree containing `cwd`, if any.
///
/// Thin wrapper over [`worktree_record_for_cwd`] that drops the DB handle;
/// returns `None` (without DB I/O) for paths outside `~/.intellihelper/worktrees/`.
...
///
/// When the source path is already inside `~/.intellihelper/worktrees/<repo>/`, the
...
///
/// ACP equivalent of `intellihelper -w -r <session_id>` (optionally with `--ref`).
...
use fast_worktree::{
    ...
    rebuild_worktree_db, resolve_intellihelper_home, resolve_worktree_auto_gc_from_layers,
};
...
pub fn worktree_auto_gc_layer_from_settings(
    s: &intellihelper_config_types::WorktreeAutoGcSettings,
) -> WorktreeAutoGcLayer {
    use std::collections::BTreeMap;
    use intellihelper_config_types::WorktreeKindMaxAge;
...
/// Env + `$INTELLIHELPER_HOME/config.toml` only — **`remote=None` is intentional**.
///
...
/// processes; use `INTELLIHELPER_WORKTREE_AUTO_GC=0` / `INTELLIHELPER_WORKTREE_AUTO_GC_DRY_RUN=1`
...
fn resolve_worktree_auto_gc_local() -> fast_worktree::ResolvedWorktreeAutoGc {
    use intellihelper_config_types::WorktreeAutoGcSettings;

    let local = if let Ok(home) = resolve_intellihelper_home() {
...
pub fn worktree_db_rebuild() -> Result<fast_worktree::RebuildReport> {
    let home = resolve_intellihelper_home()?;
...
/// Scan `~/.intellihelper/worktrees/<repo_name>/` for subdirectories not tracked
...
mod tests {
    ...
    async fn snapshot_and_remove_captures_then_deletes() {
        ...
        let ref_name = "refs/intellihelper/subagents/dispose-1";
    ...
    async fn snapshot_and_remove_then_rehydrate_round_trips() {
        ...
        let ref_name = "refs/intellihelper/subagents/dispose-2";
    ...
    async fn snapshot_standalone_worktree_durable_after_removal_round_trips() {
        ...
        let ref_name = "refs/intellihelper/subagents/standalone-1";
    ...
    async fn snapshot_failure_preserves_worktree() {
        ...
        let result = snapshot_and_remove_subagent_worktree(
            &not_a_worktree,
            &not_a_worktree,
            "refs/intellihelper/subagents/dispose-3",
        )
        .await;
    ...
    /// Point `INTELLIHELPER_HOME` at an isolated tempdir (`resolve_intellihelper_home` re-reads
    ...
    /// and restores `INTELLIHELPER_HOME` on drop (before releasing the lock), so the
    ...
    async fn cleanup_cancelled_worktree_removes_dir_and_deregisters() {
        ...
        // can't clobber this row (IntelliHelperHomeFixture is not visible across crates).
