//! Unified PTY harness for intellihelper-pager.
//!
...
//!    `intellihelper-pager/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `INTELLIHELPER_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// (XTVERSION probes and intellihelper wrap). Content-backed launches must use
    /// [`Self::new_in_sandbox`].
    ...
    /// Spawn from a canonical [`intellihelper_test_support::TestSandbox`] baseline
    ...
    pub fn new_in_sandbox(
        ...
        cols: u16,
        args: &[&str],
        sandbox: &intellihelper_test_support::TestSandbox,
        env: &[(&str, &str)],
        cwd: Option<&Path>,
    ) -> Result<Self> {
    ...
    /// # use intellihelper_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
    ...
    /// through intellihelper's stdout. Used to reproduce the doubled-line class of bugs
    /// where intellihelper's diff renderer never re-asserts a region it didn't write
