...
pub use intellihelper_config::shell::UnixShellKind;

const INIT_MARKER: &str = "__INTELLIHELPER_STATIC_SHELL_MARKER__";
...
fn shell_binary(shell: UnixShellKind) -> &'static str {
    intellihelper_config::shell::unix_shell_path(shell)
}
...
impl StaticShellSnapshot {
    ...
    pub async fn init(cwd: &Path) -> Self {
        let shell = intellihelper_config::shell::detect_unix_shell_kind();
...
mod tests {
    ...
    async fn replays_aliases_and_functions() {
        ...
        let output = run_static(
            "alias intellihelper_alias_probe='echo ALIAS_OK'\ngrok_fn_probe() { echo FN_OK; }\n",
            "intellihelper_alias_probe && intellihelper_fn_probe",
        )
        .await;
