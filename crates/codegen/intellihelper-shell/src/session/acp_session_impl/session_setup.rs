...
impl SessionActor {
    ...
    ) -> Option<intellihelper_tools::types::skill_discovery_tracker::SkillUpdateEffects> {
        let bridge = self.agent.borrow().tool_bridge().clone();
    ...
    pub(crate) async fn slash_skills_for_resolve(
        &self,
    ) -> Vec<intellihelper_tools::implementations::skills::types::SkillInfo> {
    ...
    pub(super) fn wrap_skill_reminder(
        &self,
        effects: &intellihelper_tools::types::skill_discovery_tracker::SkillUpdateEffects,
    ) -> Option<ConversationItem> {
        use intellihelper_tools::types::skill_discovery_tracker::SkillUpdateKind;
        let is_cursor = self.is_cursor_harness();
    ...
    pub(super) async fn maybe_refresh_model_metadata_on_resume(&self) {
        ...
        let _ = am.auth().await;
        let provider: Arc<dyn intellihelper_auth::AuthCredentialProvider> = Arc::new(
        ...
        let (response, stamp) =
            match intellihelper_auth::execute_with_stamp(&middleware_client, built).await {
                Ok(r) => r,
                Err(e) => {
    ...
    pub(super) async fn handle_model_metadata_update(
        ...
        let updated_config = intellihelper_sampling_types::SamplingConfig {
    ...
    pub(super) async fn inject_deny_read_globs(&self) {
        ...
        self.agent
            .borrow()
            .tool_bridge()
            .update_resource(intellihelper_tools::types::resources::DenyReadGlobs(
