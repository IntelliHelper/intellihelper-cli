...
pub const LEGACY_XAI_API_KEY_ENV_VAR: &str = "INTELLIHELPER_CODE_XAI_API_KEY";

/// Read the API key from the environment.
///
/// Checks `XAI_API_KEY` first, then falls back to the legacy
/// `INTELLIHELPER_CODE_XAI_API_KEY` for backward compatibility.
...
/// Returns `true` if either `XAI_API_KEY` or `INTELLIHELPER_CODE_XAI_API_KEY` is set.
...
/// `INTELLIHELPER_DISABLE_API_KEY_AUTH`) is the admin kill switch: when true the
...
pub struct AuthMethodsBuildInputs<'a> {
    ...
    /// default `intellihelper.com` method.
    ...
    /// Optional display label for the login method (`intellihelper.com` or `oidc`).
    pub login_label: Option<&'a str>,
    /// True if `grok_com_config.auth_provider_command` is configured (sets
    /// `meta.external_provider = true` on the `intellihelper.com` method).
    ...
    /// Advertise browser OAuth (`intellihelper.com` / former grok.com login).
    ///
    /// **Default off** for open-source IntelliHelper: users authenticate with
    /// `XAI_API_KEY` (or per-model `api_key`/`env_key`), not a Grok browser
    /// login. Opt in via `INTELLIHELPER_ENABLE_BROWSER_AUTH=1`, enterprise
    /// OIDC, external auth provider command, or `preferred_method = oidc`.
    pub enable_browser_login: bool,
}

/// Env var that re-enables browser OAuth (`intellihelper.com`) auth method.
pub const ENABLE_BROWSER_AUTH_ENV_VAR: &str = "INTELLIHELPER_ENABLE_BROWSER_AUTH";

/// Whether browser OAuth should be advertised (opt-in).
pub fn browser_auth_enabled() -> bool {
    intellihelper_config::env_bool(ENABLE_BROWSER_AUTH_ENV_VAR).unwrap_or(false)
}
...
/// 3. interactive login (optional):
///    - `oidc`               (if `has_enterprise_oidc`)
///    - `intellihelper.com`  (if `enable_browser_login` and not enterprise OIDC)
///
...
pub fn build_auth_methods(inputs: AuthMethodsBuildInputs<'_>) -> BuiltAuthMethods {
    ...
    match preferred_method {
        ...
        Some(PreferredAuthMethod::Oidc) => build_pinned_oidc(
            ...
            login_label,
            has_auth_provider_command,
            // OIDC pin implies interactive session login is desired.
            true,
        ),
        None => build_unpinned(
            has_external_api_key,
...
fn push_interactive_login(
    ...
    } else if enable_browser_login {
        methods.push(grok_com_auth_method(login_label, has_auth_provider_command));
    }
    // else: no interactive login — API key / cached token only (open-source default).
}
...
impl AuthMethodKind {
    pub fn from_id(id: &acp::AuthMethodId) -> Self {
        match id.0.as_ref() {
            ...
            INTELLIHELPER_COM_METHOD_ID => Self::GrokCom,
    ...
    /// `true` for session-based methods (cached_token, intellihelper.com, oidc).
...
/// `true` for session-based ACP methods (cached_token, intellihelper.com, oidc).
...
pub const AUTH_ERROR_SESSION_EXPIRED: &str =
    "Session expired. Run `intellihelper login` to re-authenticate.";

pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `intellihelper login`, set XAI_API_KEY, or add api_key to ~/.intellihelper/config.toml.";
...
/// `None` (no automatic browser OAuth fallthrough — open-source default).
///
...
/// No credentials advertised (no API key, no session, browser login disabled).
pub const NO_AUTH_CREDENTIALS: &str = "No API key configured. Set XAI_API_KEY (or model api_key/env_key in ~/.intellihelper/config.toml) and restart.";

/// Error when `preferred_method=oidc` but the session path cannot proceed.
pub const PREFERRED_OIDC_UNAVAILABLE: &str =
    "preferred_method=oidc but no session is available. Run `intellihelper login` to authenticate.";
...
pub(crate) fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.intellihelper/auth.json".to_string())),
    )
}

pub const INTELLIHELPER_COM_METHOD_ID: &str = "intellihelper.com";

/// xAI OAuth2/OIDC auth. Method id `"intellihelper.com"` kept for ACP wire-compat.
pub(crate) fn grok_com_auth_method(
    label: Option<&str>,
    has_auth_provider_command: bool,
) -> acp::AuthMethod {
    let name = label.unwrap_or("IntelliHelper");
    ...
    acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new(acp::AuthMethodId::new(INTELLIHELPER_COM_METHOD_ID), name.to_string())
...
mod tests {
    ...
    /// No advertiseable API-key credentials → no browser OAuth fallthrough.
    #[test]
    fn after_cached_token_unavailable_fails_without_api_key() {
    ...
    fn default_inputs() -> AuthMethodsBuildInputs<'static> {
        AuthMethodsBuildInputs {
            ...
            // Open-source default: no Grok/browser OAuth.
            enable_browser_login: false,
        }
    }

    fn browser_login_inputs() -> AuthMethodsBuildInputs<'static> {
        AuthMethodsBuildInputs {
    ...
    /// `intellihelper.com` — `auth_methods.first()` does NOT need interactive login,
    ...
    /// Brand-new user (no API key, no cached token): no auth methods by default
    /// (browser OAuth disabled). User must set `XAI_API_KEY`.
    #[test]
    fn fresh_user_has_no_methods_without_browser_or_api_key() {
    ...
    /// Opt-in browser OAuth: only `intellihelper.com` is advertised.
    #[test]
    fn fresh_user_with_browser_login_advertises_intellihelper_com() {
        let built = build_auth_methods(browser_login_inputs());
    ...
    /// Enterprise OIDC replaces `intellihelper.com` (mutually exclusive). xai.api_key,
    ...
    fn enterprise_oidc_replaces_grok_com_but_xai_api_key_still_first() {
        ...
        assert!(
            ...
            "intellihelper.com and oidc are mutually exclusive",
        );
    }

    /// `has_auth_provider_command` is plumbed through to the `intellihelper.com` method
    ...
    fn auth_provider_command_sets_external_provider_meta() {
        ...
        let intellihelper = built
            .methods
            .iter()
            .find(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::GrokCom)
            .expect("intellihelper.com must be advertised when browser login enabled");
        assert_eq!(intellihelper.name(), "Acme Corp");
        let meta = intellihelper.meta().expect("meta should be set");
        assert_eq!(
    ...
    /// `~/.intellihelper/config.toml` skeleton from the bug report, walks it through
    ...
    fn enterprise_byok_config_does_not_require_login() {
        ...
        // pager's `startup_auth_metadata()` returns `needs_login = false`.
        {
            ...
            let built = build_auth_methods(AuthMethodsBuildInputs {
                ...
                // intellihelper.com login (no enterprise OIDC).
                has_cached_token: false,
                ..default_inputs()
            });
            assert_eq!(
    ...
    fn disable_api_key_auth_suppresses_xai_api_key_method() {
        ...
        assert_eq!(
            ...
            "with api-key auth disabled, browser login on, and no cached token, \
             the login method must lead so the pager requires interactive login",
        );
        assert!(built.default_auth_method_id.is_none());
    }
    ...
    /// Legacy `INTELLIHELPER_CODE_XAI_API_KEY` env var is accepted as a fallback
    ...
    /// When both `XAI_API_KEY` and `INTELLIHELPER_CODE_XAI_API_KEY` are set,
    ...
    // -- intellihelper login --legacy regression coverage ------------------------
    //
    // `intellihelper login --legacy` produces a IntelliHelperAuth with `auth_mode: WebLogin`,
    // `oidc_issuer: None`, and no `expires_at` (30-day hardcoded TTL).
    // When this token is present via the `INTELLIHELPER_AUTH` env var (or via legacy
    ...
    // to-end. A regression in INTELLIHELPER_AUTH JSON parsing or in auth method
    // ordering would send legacy-token users to the login screen.

    /// END-TO-END REGRESSION TEST: a legacy auth token (WebLogin, no
    /// expires_at) present in the `INTELLIHELPER_AUTH` env var, with no other auth
    ...
    #[test]
    #[serial]
    fn intellihelper_login_legacy_token_does_not_require_login() {
        use crate::auth::{AuthManager, AuthMode, IntelliHelperAuth, GrokComConfig};

        // Ensure clean slate for "no other auth available".
        let _g1 = EnvGuard::unset("INTELLIHELPER_AUTH_PATH");
        let _g2 = EnvGuard::unset(XAI_API_KEY_ENV_VAR);

        // Construct a legacy-style token exactly as `intellihelper login --legacy`
        // produces: WebLogin mode, no OIDC fields, no refresh_token, no
        // expires_at (is_expired falls back to 30-day age check).
        let legacy_token = IntelliHelperAuth {
        ...
        // Provide it via INTELLIHELPER_AUTH env var (highest priority code path in
        ...
        let _g = EnvGuard::set("INTELLIHELPER_AUTH", &legacy_json);
        ...
        assert!(
            current.is_some(),
            "legacy token in INTELLIHELPER_AUTH env MUST be loaded directly -- if this fails, \
    ...
    fn no_legacy_token_means_no_cached_token_advertised() {
        ...
        let _g1 = EnvGuard::unset("INTELLIHELPER_AUTH");
        let _g2 = EnvGuard::unset("INTELLIHELPER_AUTH_PATH");
        ...
        assert!(
            ...
            "no cached token AND no api key AND browser login off: no methods advertised",
        );
    }
    ...
    fn pin_oidc_with_session_hides_api_key() {
        ...
        assert_eq!(
            method_ids(&built),
            vec![CACHED_TOKEN_AUTH_METHOD_ID, INTELLIHELPER_COM_METHOD_ID]
        );
    ...
    fn pin_oidc_without_session_is_interactive_only() {
        ...
        assert_eq!(method_ids(&built), vec![INTELLIHELPER_COM_METHOD_ID]);
        assert!(built.default_auth_method_id.is_none());
    }
}
