...
//! file logger that writes to `~/.intellihelper/logs/memory.log`.
//!
//! ## When to use
//!
...
//! INTELLIHELPER_MEMORY_LOG=0 intellihelper                # disable even when enabled
//! tail -f ~/.intellihelper/logs/memory.log      # watch in another terminal
//! ```
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "INTELLIHELPER_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.intellihelper/logs/memory.log`. Filters to `xai_memory=trace`.
    /// Set `INTELLIHELPER_MEMORY_LOG=0` to disable, `INTELLIHELPER_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || intellihelper_home().join("logs").join("memory.log");
