...
pub(super) fn build_middleware_client(
    ...
    intellihelper_http::with_auth_retry(intellihelper_http::shared_client(), credentials)
}

fn build_static_middleware_client(
    api_key: Option<String>,
) -> reqwest_middleware::ClientWithMiddleware {
    let provider: std::sync::Arc<dyn intellihelper_auth::AuthCredentialProvider> = std::sync::Arc::new(
        intellihelper_auth::StaticAuthCredentialProvider::new(Box::new(NoopHttpAuth), api_key),
    );
...
impl intellihelper_auth::HttpAuth for NoopHttpAuth {
    ...
    async fn embed_batch(
        ...
        for batch in texts.chunks(self.max_batch_size) {
            ...
            for attempt in 0..MAX_RETRIES {
                ...
                let request = intellihelper_http::shared_client()
