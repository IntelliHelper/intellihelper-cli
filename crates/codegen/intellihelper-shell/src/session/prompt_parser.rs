...
use intellihelper_workspace::file_system::{
    FileReference, render_embedded_resource, render_file_reference,
};
/// Parsed prompt with context and query kept separate.
///
/// Some templates put `<user_query>` last (context first); IntelliHelper puts it first.
...
pub struct ParsedPrompt {
    ...
    /// IntelliHelper mode may include editor open/focus metadata; the compat mode does not.
...
impl ParsedPrompt {
    ...
    ///
    /// Layout:
    /// - **IntelliHelper mode:** `<user_query>` + `<skill_information>` + context
...
///
/// When `is_cursor` is false, produces original IntelliHelper-format output:
...
/// IntelliHelper-format resource links: `<focused_files>` / `<open_files>` with
...
mod tests {
    ...
    /// Shorthand: render + assemble for intellihelper mode.
    fn render_grok(
        message: &str,
        embedded: Vec<String>,
    ...
    fn test_grok_render_with_attachments_uses_system_reminder_wrapper() {
        ...
        assert!(
            ...
            "IntelliHelper should start with <user_query>, got: {result}"
        );
    }
    #[test]
    fn test_grok_render_user_query_first() {
        ...
        assert!(
            uq_pos < rr_pos,
            "IntelliHelper: <user_query> ({uq_pos}) should come before resource links ({rr_pos})\ngot: {result}"
        );
