...
//!
//! Wires together a wiremock-mocked GCS bucket + an isolated `INTELLIHELPER_HOME`
//! tempdir so we can verify the full install pipeline:
//!   fetch version → download intellihelper binary → chmod → atomic symlink →
//!   cleanup_old_downloads → persist installer config.
//!
//! The function reads `intellihelper_home()` (a process-wide `OnceLock`), so all
//! tests in this binary share a single `INTELLIHELPER_HOME` and run serially via
//! `#[serial]`.
...
use intellihelper_update::UpdateConfig;
use intellihelper_update::auto_update::{install_internal_from_base, install_internal_from_bases};
...
async fn mount_gcs(version: &str, platform: &str) -> MockServer {
    ...
    // Main intellihelper binary download.
    Mock::given(method("GET"))
        .and(path(format!("/intellihelper-{version}-{platform}")))
...
async fn install_internal_pinned_version_writes_binary_and_symlink() {
    ...
    let downloaded = home
        .join("downloads")
        .join(format!("intellihelper-0.1.181-{platform}"));
...
async fn install_internal_updates_stale_agent_symlink_to_new_version() {
    ...
    let old_binary = download_dir.join(format!("intellihelper-0.1.180-{platform}"));
...
async fn install_internal_cleans_up_stale_pager_symlink() {
    // Old installations shipped a separate intellihelper-pager binary. Verify the
    // update removes the stale symlink from ~/.intellihelper/bin/.
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    let bin_dir = home.join("bin");
    std::fs::create_dir_all(&bin_dir).unwrap();
    let pager_link = bin_dir.join("intellihelper-pager");
    ...
    assert!(
        ...
        "stale intellihelper-pager symlink should be removed"
    );
}
...
async fn install_internal_alpha_channel_resolves_max_of_alpha_and_stable() {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    Mock::given(method("GET"))
        .and(path(format!("/intellihelper-0.1.181-{platform}")))
        .respond_with(ResponseTemplate::new(200).set_body_bytes(b"#!/bin/sh\nexit 0\n".to_vec()))
        .mount(&server)
        .await;
...
#[tokio::test]
#[serial]
async fn install_internal_fails_on_intellihelper_binary_404() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = MockServer::start().await;
...
async fn install_internal_cleans_up_old_versions_keeping_n_minus_one() {
    ...
    assert!(
        downloads.join(format!("intellihelper-0.1.181-{platform}")).exists(),
        "current"
    );
    assert!(
        downloads.join(format!("intellihelper-0.1.180-{platform}")).exists(),
        "N-1 retained"
    );
    assert!(
        !downloads.join(format!("intellihelper-0.1.179-{platform}")).exists(),
        "oldest deleted"
    );

    // Symlink updated to latest.
    let target = std::fs::read_link(home.join("bin").join("intellihelper")).unwrap();
    assert!(
        target
            .file_name()
            .unwrap()
...
async fn install_internal_idempotent_for_same_version() {
    ...
    let target = std::fs::read_link(test_home().join("bin").join("intellihelper")).unwrap();
    assert!(target.to_string_lossy().contains("0.1.181"));
}

#[tokio::test]
#[serial]
async fn install_internal_creates_intellihelper_home_subdirs_if_missing() {
    let _ = test_home();
    reset_home();
