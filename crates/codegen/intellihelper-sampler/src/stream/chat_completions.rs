    ...
    async_stream::stream! {
        ...
        loop {
            ...
            if let Some(u) = chunk.usage.clone() {
                ...
                let chunk_cost = intellihelper_sampling_types::reported_cost_ticks(u.cost_in_usd_ticks);
        ...
        if first_choice_seen {
            if !reasoning_acc.is_empty() {
                items.push(ConversationItem::Reasoning(
                    intellihelper_sampling_types::synthesized_reasoning_item(reasoning_acc),
                ));
            }
