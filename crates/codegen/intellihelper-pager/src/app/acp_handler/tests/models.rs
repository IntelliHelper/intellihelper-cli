#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn models_update_preserves_active_agent_model() {
        ...
        let agent = app.agents.get_mut(&AgentId(0)).unwrap();
        let id_3 = acp::ModelId::new(std::sync::Arc::from("intellihelper-3"));
        agent
            .session
            .models
            .available
            .insert(id_3.clone(), make_model_info("intellihelper-3"));
        agent.session.models.current = Some(id_3);

        let notif = make_models_update_notif("intellihelper-4", &["intellihelper-3", "intellihelper-4"]);
    ...
    fn models_update_uses_shell_default_when_agent_model_removed() {
        ...
        // intellihelper-3 removed from catalog.
        let notif = make_models_update_notif("intellihelper-4.3", &["intellihelper-4.3", "intellihelper-4.5"]);
        handle_models_update(&notif, &mut app);

        assert_eq!(
            app.models.current.as_ref().map(|id| id.0.as_ref()),
            Some("intellihelper-4.3"),
    ...
    fn models_update_noop_when_agent_matches_shell_default() {
        ...
        let agent = app.agents.get_mut(&AgentId(0)).unwrap();
        let id_4 = acp::ModelId::new(std::sync::Arc::from("intellihelper-4"));
        agent
            .session
            .models
            .available
            .insert(id_4.clone(), make_model_info("intellihelper-4"));
        ...
        assert_eq!(
            app.models.current.as_ref().map(|id| id.0.as_ref()),
            Some("intellihelper-4"),
            "app.models.current must be intellihelper-4 when agent and shell agree"
        );
        let agent = app.agents.get(&AgentId(0)).unwrap();
        assert_eq!(
            agent
                ...
                .map(|id| id.0.as_ref()),
            Some("intellihelper-4"),
            "agent model must remain intellihelper-4"
        );
    }
    ...
    fn models_update_non_active_agent_uses_shell_fallback_not_active_model() {
...

        {
            ...
            let id_5 = acp::ModelId::new(std::sync::Arc::from("intellihelper-4.5"));
            agent_b
                .session
                .models
                .available
                .insert(id_5.clone(), make_model_info("intellihelper-4.5"));
            agent_b.session.models.current = Some(id_5);
        }

        // intellihelper-5 removed from catalog.
        let notif = make_models_update_notif("intellihelper-4", &["intellihelper-3", "intellihelper-4"]);
        handle_models_update(&notif, &mut app);
        ...
        // B's intellihelper-5 was removed — must fall back to shell's intellihelper-4, not A's intellihelper-3.
    ...
    fn model_changed_updates_state_silently_on_follower() {
        let mut app = make_app_with_agent("sess-1");
        let agent = app.agents.get_mut(&AgentId(0)).unwrap();
        seed_models(agent, "intellihelper-3", &["intellihelper-3", "intellihelper-4"]);
        let scrollback_before = agent.scrollback.len();
        // Follower: no local switch in flight.
        assert!(!agent.session.model_switch_pending);

        let notif = model_changed_ext("sess-1", "intellihelper-4", None);
    ...
    fn model_changed_dropped_when_model_unknown_to_catalog() {
        ...
        let notif = model_changed_ext("sess-1", "intellihelper-99-unknown", None);
        let changed = handle_ext_notification(&notif, &mut app);
        assert!(
            !changed,
    ...
    fn model_changed_applies_reasoning_effort_on_follower() {
        ...
        let notif = model_changed_ext("sess-1", "intellihelper-4", Some("high"));
    ...
    fn model_changed_dropped_for_unknown_session_id() {
        ...
        let notif = model_changed_ext("sess-OTHER", "intellihelper-4", None);
        let changed = handle_ext_notification(&notif, &mut app);
        assert!(!changed);
