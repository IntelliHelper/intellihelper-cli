...
impl HubHandle {
    ...
    pub(crate) async fn connect(
        ...
        let mut server_builder = ToolServerBuilder::default()
            ...
            .binary_version(intellihelper_version::VERSION)
...
struct CallCompletedGuard {
    ...
    outcome: file_utils::events::ToolOutcome,
}
impl CallCompletedGuard {
    fn new(
        ...
        Self {
            ...
            call_id,
            session_id,
            outcome: file_utils::events::ToolOutcome::Cancelled,
        }
    }
    fn set_outcome(&mut self, outcome: file_utils::events::ToolOutcome) {
            ...
            match transport {
                ...
                None => {
                    tracing::warn!(
                        tool = %self.name(),
                        session = %session_id,
                        "INTELLIHELPER_HITL_PERMISSION_LIVE set but no hub ToolServer; rejecting guarded tool"
                    );
                    return terminal_only(Err(ToolError::new(
                        ToolErrorKind::PermissionDenied,
            ...
            while let Some(item) = inner.next().await {
                match item {
                    ...
                    ToolStreamItem::Terminal(Ok(run_result)) => {
                        ...
                        _guard.set_outcome(file_utils::events::ToolOutcome::Success);
                    ...
                    ToolStreamItem::Terminal(Err(e)) => {
                        ...
                        _guard.set_outcome(file_utils::events::ToolOutcome::Error);
...
mod tests {
    ...
    use intellihelper_tools::types::tool_metadata::ToolMetadata as XaiToolMetadata;
    #[derive(Debug)]
    ...
    fn bg_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("IntelliHelperBuild:monitor", Some(ToolKind::Monitor)),
            ],
    ...
    fn bg_started_notif(task_id: &str) -> ToolNotification {
        use intellihelper_tools::notification::types::{
    ...
    fn task_completed_notif(task_id: &str) -> ToolNotification {
        use intellihelper_tools::computer::types::{TaskKind, TaskSnapshot};
