...
```bash
cargo run -p intellihelper-pager --release --bin pty-bench -- \
  --all \
  --write-baseline crates/codegen/intellihelper-pager-pty-harness/benches/pty_baselines/<platform>.json
```
