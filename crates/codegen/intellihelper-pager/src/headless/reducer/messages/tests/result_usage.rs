...
fn messages_model_usage_maps_and_zero_fills() {
    let rows = json!({
        "intellihelper-4": {"inputTokens": 90, "outputTokens": 7, "cacheReadInputTokens": 10, "cacheCreationInputTokens": 25, "costUSD": 0.02},
    });
    let out = messages_model_usage(Some(&rows), Some("intellihelper-4"), 0, Some(131_072));
    let mu = &out["intellihelper-4"];
...
fn messages_model_usage_attributes_web_search_to_current_model() {
    let rows = json!({
        "intellihelper-4": {"inputTokens": 90, "outputTokens": 7, "costUSD": 0.02},
        "intellihelper-mini": {"inputTokens": 5, "outputTokens": 1},
    });
    let out = messages_model_usage(Some(&rows), Some("intellihelper-4"), 3, Some(131_072));
    assert_eq!(out["intellihelper-4"]["webSearchRequests"], 3);
    assert_eq!(out["intellihelper-mini"]["webSearchRequests"], 0);
    assert_eq!(out["intellihelper-4"]["contextWindow"], 131_072);
    assert!(out["intellihelper-mini"]["contextWindow"].is_null());
}
...
fn messages_partial_error_max_tokens_recovers_real_id_and_usage() {
    ...
    out.extend(r.reduce(StreamEvent::ResponseStarted {
        ...
        model: Some("intellihelper-4".into()),
        input_tokens: 42,
