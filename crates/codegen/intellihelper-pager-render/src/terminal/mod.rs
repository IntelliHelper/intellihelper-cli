...
pub enum TerminalName {
    ...
    JetBrains,
    /// IntelliHelper Desktop (Electron app).
    #[strum(to_string = "IntelliHelper Desktop")]
    IntelliHelperDesktop,
...
pub struct TerminalContext {
    ...
    /// Which embedded editor `:terminal` intellihelper is running inside, if any.
...
impl TerminalContext {
    ...
    /// keyed off this only fires when a FocusGained actually reaches intellihelper, which
    ...
    pub fn telemetry_snapshot(&self) -> intellihelper_telemetry::events::TerminalTelemetry {
        ...
        intellihelper_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: intellihelper_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> intellihelper_shared::session::FeedbackTerminalInfo {
        use intellihelper_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(intellihelper_shared::clipboard::native_tool_name().to_owned()),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // first client, not the current one). Revisit when `intellihelper ssh` can
    // forward env vars.
    let mut ctx = build_terminal_context_from_env(&env);
...
/// `intellihelper-pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
fn terminal_name_from_term_program(value: &str) -> Option<TerminalName> {
    ...
    match normalized.as_str() {
        ...
        "grokdesktop" => Some(TerminalName::IntelliHelperDesktop),
...
///
/// Parsed from `[terminal] alt_screen` in `~/.intellihelper/pager.toml` and
