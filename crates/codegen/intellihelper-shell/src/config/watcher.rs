...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.intellihelper/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** intellihelper process
    /// sharing the same `~/.intellihelper` (the writer may also be this process;
...
/// Watches `~/.intellihelper/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.intellihelper/config.toml`, `.mcp.json`,
...
/// processes (e.g. `intellihelper login` in another terminal).
///
/// Adds two **non-recursive** watches per `cwd` argument:
/// `<cwd>/` (catches `.mcp.json` and `.claude.json` at the project root) and
/// `<cwd>/.intellihelper/` (catches `<cwd>/.intellihelper/config.toml`). Recursing on `<cwd>`
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.intellihelper/`. Use [`Self::watch_path`] later to register additional
    /// project cwds for sessions that open in previously-unwatched
    /// directories.
    pub fn start(
        intellihelper_home: &Path,
        extra_paths: &[PathBuf],
        cwd: Option<&Path>,
        ...
        let intellihelper_home_buf = intellihelper_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(intellihelper_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(intellihelper_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(intellihelper_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(intellihelper_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %intellihelper_home.display(),
                    error = %e,
                    "failed to watch intellihelper home directory"
                )
            })
            .ok()?;
        ...
        // includes `<cwd>/.intellihelper/config.toml` so the loop above
        // watches `<cwd>/.intellihelper/`), the call below installs a
        ...
        tracing::info!(
            intellihelper_home = %intellihelper_home.display(),
    ...
    /// Register `<cwd>/` and `<cwd>/.intellihelper/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.intellihelper/config.toml` trigger a
    ...
    /// `<cwd>/.intellihelper/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.intellihelper/` does not yet
/// exist at session-open time, the `.intellihelper/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.intellihelper/` followed by a
/// write to `<cwd>/.intellihelper/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let intellihelper_dir = cwd.join(".intellihelper");
    if let Err(e) = debouncer
        .watcher()
        .watch(&intellihelper_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .intellihelper directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let intellihelper_dir = cwd.join(".intellihelper");
    if let Err(e) = debouncer.watcher().unwatch(&intellihelper_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .intellihelper directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.intellihelper/` hasn't
...
const VENDOR_CONFIG_ROOT_NAMES: &[&str] = &[".intellihelper", ".agents", ".claude", ".cursor"];

/// Vendor roots (by name or `intellihelper_home`) must use scoped watches — they can
/// contain large non-skill trees (`worktrees/`, etc.).
fn is_vendor_config_root(dir: &Path, intellihelper_home: &Path) -> bool {
    if paths_equal(dir, intellihelper_home) {
        return true;
    }
    dir.file_name()
        .and_then(|n| n.to_str())
...
fn project_grok_refresh_dirs(project_root: &Path) -> Vec<(PathBuf, RecursiveMode)> {
    let project_grok = project_root.join(".intellihelper");
...
fn plan_skills_watch_targets(
    ...
    for dir in dirs_to_watch {
        if is_vendor_config_root(dir, intellihelper_home) {
...
/// Watches project `.intellihelper` skills/commands/workflows for mid-session discovery.
///
...
impl SkillsFileWatcher {
    ...
    /// [`collect_skill_config_dirs`](intellihelper_agent::prompt::skills::collect_skill_config_dirs).
    ///
    ...
    pub fn start(
        ...
        let dirs_to_watch = intellihelper_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &intellihelper_home,
            config_paths,
        ...
        Self::start_with_dirs(&dirs_to_watch, &intellihelper_home, project_root.as_deref())
    }
    ...
    pub fn start_with_dirs(
        ...
        let plan = plan_skills_watch_targets(dirs_to_watch, intellihelper_home, project_root);
...
mod tests {
    ...
    fn is_vendor_config_root_matches_known_names_at_any_tier() {
        ...
        let home = home.path();
        let intellihelper_home = home.join(".intellihelper");

        assert!(is_vendor_config_root(&intellihelper_home, &intellihelper_home));
        assert!(is_vendor_config_root(&home.join(".claude"), &intellihelper_home));
        assert!(is_vendor_config_root(&home.join(".cursor"), &intellihelper_home));
        assert!(is_vendor_config_root(&home.join(".agents"), &intellihelper_home));
        assert!(is_vendor_config_root(
            &home.join("repo").join(".intellihelper"),
            &intellihelper_home
        ));
        assert!(is_vendor_config_root(
        ...
        assert!(!is_vendor_config_root(&home.join("my-skills"), &intellihelper_home));
        assert!(!is_vendor_config_root(&home.join(".config"), &intellihelper_home));
        assert!(!is_vendor_config_root(
            &home.join("repo").join("my-skills"),
            &intellihelper_home
        ));

        let custom_home = home.join("custom-intellihelper-home");
    ...
    fn project_grok_refresh_dirs_matches_vendor_layout() {
        ...
        let intellihelper = project.join(".intellihelper");
        let dirs = project_grok_refresh_dirs(project);

        assert_eq!(dirs.len(), 4);
        assert_eq!(dirs[0], (intellihelper.clone(), RecursiveMode::NonRecursive));
        assert_eq!(
            &dirs[1..],
            [
                (intellihelper.join("skills"), RecursiveMode::Recursive),
                (intellihelper.join("commands"), RecursiveMode::NonRecursive),
                (intellihelper.join("workflows"), RecursiveMode::NonRecursive),
            ]
        );
        assert_eq!(dirs[1..], vendor_skill_refresh_dirs(&intellihelper));
    }
    ...
    fn start_with_dirs_keeps_parent_only_watch() {
        ...
        let intellihelper_home = project.join("home-intellihelper");
        fs::create_dir_all(&intellihelper_home).unwrap();

        let plan = plan_skills_watch_targets(&[], &intellihelper_home, Some(project));
        ...
        let (watcher, _rx) = SkillsFileWatcher::start_with_dirs(&[], &intellihelper_home, Some(project))
    ...
    fn plan_skills_watch_targets_scopes_vendors_and_seeds_refresh() {
        ...
        let project_grok = project.join(".intellihelper");
        ...
        let plan = plan_skills_watch_targets(&dirs, &intellihelper_home, Some(project));
    ...
    fn plan_skills_watch_targets_multi_vendor_refresh_fanout() {
        let intellihelper_home = PathBuf::from("/home/u/.intellihelper");
        let a = PathBuf::from("/repo/.claude");
        let b = PathBuf::from("/repo/.agents");
        let plan = plan_skills_watch_targets(&[a.clone(), b.clone()], &intellihelper_home, None);
    ...
    fn plan_skills_watch_targets_seeds_all_missing_project_vendor_roots() {
        let tmp = TempDir::new().unwrap();
        let project = tmp.path();
        let intellihelper_home = project.join("elsewhere").join(".intellihelper");
        let plan = plan_skills_watch_targets(&[], &intellihelper_home, Some(project));
    ...
    fn plan_skills_watch_targets_does_not_double_seed_present_vendor_roots() {
        ...
        let plan = plan_skills_watch_targets(&present, &intellihelper_home, Some(project));
    ...
    fn plan_skills_watch_targets_partial_vendors_seed_only_missing() {
        ...
        for name in [".intellihelper", ".agents", ".cursor"] {
            let root = project.join(name);
            expected.push((root.clone(), RecursiveMode::NonRecursive));
            expected.extend(vendor_skill_refresh_dirs(&root));
        }
    ...
    #[test]
    fn plan_skills_watch_targets_parent_watches_project_when_intellihelper_present_siblings_missing() {
        let tmp = TempDir::new().unwrap();
        let project = tmp.path();
        let intellihelper_home = project.join("home-intellihelper");
        let project_grok = project.join(".intellihelper");
    ...
    fn workflow_change_classifies_missing_directory_creation() {
        let intellihelper = Path::new("/tmp/project/.intellihelper");
        assert_eq!(
            discovery_change_for_path(intellihelper),
        ...
        assert_eq!(
            discovery_change_for_path(&intellihelper.join("skills")),
            Some(DiscoveryChange::Skills)
        );
        assert_eq!(
            discovery_change_for_path(&intellihelper.join("commands")),
            Some(DiscoveryChange::Skills)
        );
        assert_eq!(
            discovery_change_for_path(&intellihelper.join("workflows")),
            Some(DiscoveryChange::Workflows)
        );
        assert_eq!(
            discovery_change_for_path(&intellihelper.join("workflows/review.rhai")),
            Some(DiscoveryChange::Workflows)
        );
        assert_eq!(
            discovery_change_for_path(&intellihelper.join("skills/review/SKILL.md")),
            Some(DiscoveryChange::Skills)
        );
    }
    ...
    /// A write to `<intellihelper_home>/models_cache.json` must surface as
    /// `ConfigChangeEvent::ModelsCacheChanged` so a long-running leader can
    /// hot-load a catalog fetched by another intellihelper process.
    #[test]
    #[cfg_attr(
        target_os = "macos",
        ignore = "flaky on macOS: FSEvents does not reliably deliver events in test harness"
    ...
    /// A write to `<cwd>/.intellihelper/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        let intellihelper_home = TempDir::new().unwrap();
        let cwd = TempDir::new().unwrap();
        let project_grok = cwd.path().join(".intellihelper");
        fs::create_dir_all(&project_grok).unwrap();
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.intellihelper/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.intellihelper/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_grok = new_cwd.path().join(".intellihelper");
        fs::create_dir_all(&project_grok).unwrap();
        fs::write(project_grok.join("config.toml"), "").unwrap();
