...
pub enum LogSource {
    ...
    #[serde(rename = "shell")]
    Shell,
    #[strum(serialize = "intellihelper-pager")]
    #[serde(rename = "intellihelper-pager")]
    IntelliHelperPager,
    #[strum(serialize = "intellihelper-desktop")]
    #[serde(rename = "intellihelper-desktop")]
...
/// `~/.intellihelper/logs/unified.jsonl` (those bursts inflate exactly the counters
...
fn log_path() -> PathBuf {
    ...
    intellihelper_home().join(LOG_DIR).join(LOG_FILE)
}
...
fn test_log_dir() -> &'static PathBuf {
    ...
    TEST_LOG_DIR.get_or_init(|| {
        ...
        let dir = std::env::temp_dir().join(format!(
            "intellihelper-unified-log-test-{}-{nanos}",
            std::process::id()
        ));
...
/// healing a stale handle rather than re-resolving `$INTELLIHELPER_HOME` — which also
...
mod tests {
    ...
    /// writer onto the developer's real `~/.intellihelper/logs/unified.jsonl`.
    #[ctor::ctor]
    ...
    /// an emit lands in a per-process temp file, never under `intellihelper_home()`.
    #[test]
    fn redirect_routes_writes_and_snapshots_to_process_temp_file() {
        ...
        assert!(
            ...
            "the shared file must live under the temp dir, not intellihelper_home(): {}",
            log_path().display()
        );
    }
    ...
    fn log_entry_serializes_full() {
        let entry = LogEntry {
            ts: "2025-07-14T10:30:00.123Z".into(),
            src: LogSource::IntelliHelperPager,
            pid: Some(4242),
    ...
    fn unknown_src_rejected_at_deserialization() {
        for bad in &[
            r#"{"src":"evil","entries":[]}"#,
            r#"{"src":"","entries":[]}"#,
            r#"{"src":"INTELLIHELPER-PAGER","entries":[]}"#,
        ] {
