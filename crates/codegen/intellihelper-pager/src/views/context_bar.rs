...
/// Compact session cumulative token spend for the status bar (distinct from
/// context-window fill). Example: `12K tokens` / `0 tokens`.
///
/// Label uses the plain word **tokens** (not a cryptic `sess` prefix) so the
/// badge matches common CLI/IDE usage chrome. Click opens full session usage
/// with input / output / cached breakdown.
pub fn session_usage_line(total_tokens: u64, theme: &Theme) -> Line<'static> {
    session_usage_line_with_hover(total_tokens, false, theme)
}

/// Same as [`session_usage_line`], with optional hover emphasis for the
/// clickable status-bar badge.
pub fn session_usage_line_with_hover(
    total_tokens: u64,
    hovered: bool,
    theme: &Theme,
) -> Line<'static> {
    let label = format!("{} tokens", fmt_tokens(total_tokens));
...
mod tests {
    ...
    #[test]
    fn session_usage_line_uses_tokens_label() {
        let theme = Theme::default();
        let line = session_usage_line(12_300, &theme);
        let text = line_text(&line);
        assert!(!text.contains("sess"), "{text}");
        assert!(text.contains("tokens"), "{text}");
        assert!(text.contains("12K"), "{text}");
        let zero = line_text(&session_usage_line(0, &theme));
        assert_eq!(zero, "0 tokens");
    }
