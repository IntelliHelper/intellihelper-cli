//! `intellihelper mcp doctor` -- runtime health check for MCP servers.
...
fn discover_servers(cwd: &Path) -> (Vec<ConfigSourceStatus>, Vec<DiscoveredServer>) {
    ...
    let user_config = intellihelper_home.join("config.toml");
    if user_config.is_file() {
        sources.push(ConfigSourceStatus {
            path: "~/.intellihelper/config.toml".to_string(),
...
// ── Managed (intellihelper.com) server discovery ─────────────────────────

const MANAGED_SOURCE_LABEL: &str = "intellihelper.com";
...
async fn try_discover_managed_servers() -> (ConfigSourceStatus, Vec<DiscoveredServer>) {
    ...
    let auth_manager = Arc::new(crate::auth::AuthManager::new(&intellihelper_home, grok_com_config));
    ...
    let token = match auth_manager.get_valid_token().await {
        Ok(key) => key,
        Err(_) => return managed_skipped("auth expired — run `intellihelper login`"),
    };
...
async fn check_server_start(
    ...
    let noop = file_utils::events::EventWriter::noop();
...
async fn check_tools_list(service: &mcp_servers::McpService) -> Check {
    use intellihelper_mcp::rmcp::model::PaginatedRequestParams;
    match service
...
pub async fn run_doctor(cwd: &Path, name_filter: Option<&str>) -> DoctorReport {
    ...
    // Folder-trust gate: `intellihelper mcp doctor` actually STARTS each server
    ...
    //
    // `remote = None` is intentional: standalone `intellihelper mcp doctor` has no loaded
...
pub fn print_report(report: &DoctorReport) {
    ...
    if report.servers.is_empty() {
        ...
        println!("  Run `intellihelper mcp add --help` to get started.");
        println!();
        return;
    }
    ...
    println!(
        ...
        if report.failing_count > 0 {
            " Run `intellihelper mcp doctor --json` for full diagnostics."
        } else {
            ""
        }
    );
