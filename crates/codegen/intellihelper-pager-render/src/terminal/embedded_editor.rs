//! Detects whether intellihelper is running inside an editor's embedded `:terminal`
...
/// Which embedded editor `:terminal` intellihelper is running inside.
...
/// `HOST_TERMINAL_ENV_VARS` in `intellihelper-pager-pty-harness/src/pty.rs`
/// (test-env hygiene).
