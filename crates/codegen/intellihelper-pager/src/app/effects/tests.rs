...
use super::*;
use intellihelper_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
fn format_acp_error_rate_limit_surfaces_detail_or_fallback() {
    ...
    let rpm_body = "You are sending requests too quickly. Please slow down, or upgrade to a IntelliHelper subscription for higher limits: https://grok.com/__PROTECT_2__";
    let rpm = acp::Error::new(RATE_LIMITED_ERROR_CODE, "Rate limited")
        .data(format!("API error (status 429 Too Many Requests): {rpm_body}"));
    assert!(format_acp_error(&rpm, false).contains("intellihelper.com/supergrok"));
...
/// Untitled intellihelper.com chats must stay listed, rendered as "Untitled".
#[test]
...
fn parse_kill_outcome_reads_result_envelope() {
    use intellihelper_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use intellihelper_workspace::session::git::RestoreDegree;
...
/// Redirect `INTELLIHELPER_HOME` to a tempdir for test isolation.
fn setup_intellihelper_home_in_tempdir() -> tempfile::TempDir {
    let tmp = tempfile::tempdir().expect("tempdir creation");
    unsafe {
        std::env::set_var("INTELLIHELPER_HOME", tmp.path());
    }
    tmp
}
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use intellihelper_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
            intellihelper_shell::active_sessions::list_in(dir.path())
                .expect("list")
                .is_empty(),
...
async fn persist_permission_mode_acp_notification_fires_once_on_best_effort() {
    ...
    let _guard = setup_intellihelper_home_in_tempdir();
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: intellihelper_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
            intellihelper_home: PathBuf::from("/path/that/must/not/be-read"),
...
fn agent_profile_names_are_valid_builtins() {
    use std::str::FromStr;
    use intellihelper_agent::config::BuiltinAgentName;
    let test_cases: &[(SessionFlags, &str)] = &[
        ...
        (
            ...
            "intellihelper-build-plan-no-subagents",
        ),
        (
            SessionFlags {
                plan_mode: true,
...
/// Default flags produce no agent profile (uses intellihelper-build default).
#[test]
fn default_flags_produce_no_profile() {
    let flags = SessionFlags::default();
    assert_eq!(flags.agent_profile(), None);
}
/// --subagents alone produces no profile (intellihelper-build already has TaskTool).
#[test]
...
/// Neutralize `INTELLIHELPER_AGENT` for the profile-matrix tests below: agent-driven
...
/// Callers must be `#[serial_test::serial(INTELLIHELPER_AGENT)]` (process-global env).
fn without_intellihelper_agent() -> crate::test_util::EnvVarGuard {
    crate::test_util::EnvVarGuard::set("INTELLIHELPER_AGENT", "")
}
...
#[serial_test::serial(INTELLIHELPER_AGENT)]
#[test]
fn runtime_default_flags_produce_plan_meta() {
    let _env = without_intellihelper_agent();
    let flags = SessionFlags {
        plan_mode: true,
        subagents: true,
        ask_user: true,
    ...
    assert_eq!(meta["agentProfile"], "intellihelper-build-plan");
...
fn plan_only_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "intellihelper-build-plan-no-subagents");
...
/// --ask-user alone selects the intellihelper-build-ask-user profile.
#[serial_test::serial(INTELLIHELPER_AGENT)]
#[test]
fn ask_user_alone_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "intellihelper-build-ask-user");
    assert!(meta.get("askUserQuestion").is_none());
    assert_eq!(meta["yoloMode"], false);
}
...
/// does not need an `agentProfile` (default `intellihelper-build` already has it).
#[test]
...
/// All three flags on at the runtime default produce intellihelper-build-plan
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> intellihelper_shell::session::SessionInfoResponse {
    use intellihelper_shell::session::acp_types::{ContextInfo, SessionInfoData};
    intellihelper_shell::session::SessionInfoResponse {
...
fn format_session_info_session_auth_ignores_api_key_env() {
    ...
    assert!(!text.contains("intellihelper login"), "{text}");
}
#[test]
fn format_session_info_api_key_without_env() {
    ...
    assert!(!text.contains("Manage account and credits"), "{text}");
    assert!(
            text.contains("Run `intellihelper login` to use your SuperGrok subscription instead."),
            "{text}"
        );
    assert!(!text.contains("intellihelper.com"), "{text}");
}
#[test]
...
fn format_session_info_shows_resolved_when_enabled_and_different() {
    let info = make_session_info("intellihelper-4.5", Some("intellihelper-4.3"), 1000, 10000);
    let text = format_session_info(&info, None, true, false, false);
    assert!(text.contains("Model: intellihelper-4.5 (intellihelper-4.3)"));
}
#[test]
fn format_session_info_hides_resolved_when_disabled() {
    let info = make_session_info("intellihelper-4.5", Some("intellihelper-4.3"), 1000, 10000);
    let text = format_session_info(&info, None, false, false, false);
    assert!(text.contains("Model: intellihelper-4.5"));
    assert!(!text.contains("intellihelper-4.3"));
}
#[test]
fn format_session_info_no_parens_when_resolved_matches_requested() {
    let info = make_session_info("intellihelper-4.5", Some("intellihelper-4.5"), 1000, 10000);
    let text = format_session_info(&info, None, true, false, false);
    assert!(text.contains("Model: intellihelper-4.5"));
    assert!(!text.contains("(intellihelper-4.5)"));
}
#[test]
...
fn format_session_info_shows_model_hash_for_coding_slug_without_flag() {
    let mut info = make_session_info("intellihelper-build", None, 1000, 10000);
    info.data.model_fingerprint = Some("abc123".into());
    info.data.show_model_fingerprint = false;
    let text = format_session_info(&info, None, false, false, false);
    assert!(text.contains("Model Hash: abc123"));
...
fn session_picker_entry_maps_to_dormant_roster_row() {
    ...
    let entry = SessionPickerEntry {
        ...
        model_id: Some("intellihelper-4".to_string()),
        num_messages: 3,
    ...
    assert_eq!(roster.model_id.as_deref(), Some("intellihelper-4"));
