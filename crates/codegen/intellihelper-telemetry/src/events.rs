...
//!
//! These structs were extracted from `intellihelper-shell` so they can be
...
pub enum ContextualTipKind {
    ...
    WordSelect,
    /// SSH session without `intellihelper wrap` → tip to wrap the ssh command locally.
    SshWrap,
}
...
pub struct ToolCallCompleted {
    pub tool_name: String,
    pub outcome: file_utils::events::types::ToolOutcome,
    pub duration_ms: u64,
...
// ---------------------------------------------------------------------------
// Pager events (called from intellihelper-pager via log_event)
// ---------------------------------------------------------------------------
...
pub struct ClipboardCopy {
    ...
    /// An explicit `intellihelper wrap` OSC 52 sink was active.
    pub osc52_sink: bool,
...
/// is lifted — i.e. they subscribed (externally on intellihelper.com) and came back.
/// This is the actual conversion signal for SuperGrok Heavy subscriptions
/// attributed to IntelliHelper CLI: the user saw the gate in IntelliHelper CLI, went and
...
pub struct SubscriptionActivated {
    ...
    /// is strongly attributable to IntelliHelper CLI's upsell surface.
...
/// `intellihelper-shell-manual_auth` (the `manual_auth` binding gets the `intellihelper-shell-`
