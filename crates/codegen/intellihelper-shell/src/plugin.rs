...
use intellihelper_agent::plugins::discovery::PluginScope;
use intellihelper_agent::plugins::git_install::{self, UpdateStatus};
...
use intellihelper_plugin_marketplace::git::{self, SourceCacheLease};
use intellihelper_plugin_marketplace::{
...
/// When `keep_data` is true, `~/.intellihelper/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &intellihelper_agent::plugins::install_registry::MarketplaceProvenance,
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `intellihelper plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `intellihelper plugin install ./{name}`, or add a \
                     source with `intellihelper plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `intellihelper plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `intellihelper plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
pub(crate) fn marketplace_require_sha() -> bool {
    intellihelper_config::load_effective_config_disk_only()
        .map(|c| intellihelper_plugin_marketplace::load_require_sha(&c))
        .unwrap_or_else(|_| intellihelper_plugin_marketplace::env_require_sha())
}
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &intellihelper_workspace::permission::resolution::MarketplaceAllowlist,
...
/// `~/.intellihelper/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    // Resolve user intellihelper via user_intellihelper_home() (None when no home resolves) and
    // home separately, so removal still runs from $INTELLIHELPER_HOME when no home dir
    // exists, and never touches a cwd-relative .intellihelper.
    ...
    if let Some(ref intellihelper) = intellihelper {
        settings_candidates.push(intellihelper.join("settings.local.json"));
        settings_candidates.push(intellihelper.join("settings.json"));
    }
    ...
    if let Some(ref intellihelper) = intellihelper {
        known_candidates.push(intellihelper.join("plugins").join("known_marketplaces.json"));
    }
    if let Some(ref home) = home {
...
mod tests {
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("intellihelper plugin install ./sentry"), "{msg}");
        assert!(msg.contains("intellihelper plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        ...
        assert!(
            msg.contains("intellihelper plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> intellihelper_workspace::permission::resolution::MarketplaceAllowlist {
        intellihelper_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = intellihelper_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
