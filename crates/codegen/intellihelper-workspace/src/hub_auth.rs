//! Hub [`AuthProvider`] from `~/.intellihelper/auth.json` for the standalone
...
//!
//! The in-leader `intellihelper workspace` exposure does NOT use this path — it sources
...
pub fn default_auth_path() -> anyhow::Result<PathBuf> {
    let intellihelper = intellihelper_config::user_intellihelper_home()
        .ok_or_else(|| anyhow::anyhow!("no user intellihelper home (set $INTELLIHELPER_HOME or $HOME)"))?;
    Ok(intellihelper.join("auth.json"))
}
...
fn read_auth_entry(path: &Path) -> anyhow::Result<(String, AuthEntry)> {
    if !path.exists() {
        anyhow::bail!(
            "No auth credentials found at {}. Run `intellihelper login` first.",
            path.display()
        );
    }
    ...
    entries
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "no OIDC auth entry found in {}. Run `intellihelper login` first.",
                path.display()
            )
        })
}
...
/// intellihelper-shell `auth.json` writer takes. Polling `try_lock` rather than a
...
/// the default credential path (`~/.intellihelper/auth.json`).
pub fn provider(
    hub_url: &Url,
...
mod tests {
    ...
    /// the latest `expires_at` wins — that's the entry the user's intellihelper
