...
download_file_with_progress() {
    ...
    if [ "$DOWNLOADER" = "curl" ]; then
        if [ -t 2 ]; then
            # --progress-bar writes the bar to stderr; keep errors fatal (-f).
...
download_file_parallel() {
    local url="$1" output="$2"
    # Interactive install: single-connection + curl/wget progress bar.
    if [ -t 2 ]; then
...
if [ "$os" = "windows" ]; then
    ...
    if ! "$binary_tmp" --version </dev/null >/dev/null 2>&1; then
        echo "Error: downloaded intellihelper failed to run; keeping the existing install." >&2
        rm -f "$binary_tmp"
        exit 1
    fi
    mv -f "$binary_tmp" "$binary_path"
    # Use relative symlinks when BIN_DIR and DOWNLOAD_DIR share a parent
    # (default layout: ~/.intellihelper/bin and ~/.intellihelper/downloads are siblings).
