...
use super::status::{
    ...
    dispatch_optimize_input, dispatch_show_session_info, dispatch_show_tasks,
    dispatch_show_token_usage, dispatch_show_usage, dispatch_undo_optimize_input,
...
pub(crate) fn dispatch(action: Action, app: &mut AppView) -> Vec<Effect> {
    ...
    let effects = match action {
        ...
        Action::OpenBlockViewer => {
            ...
            let mut credit_card: Option<(String, intellihelper_telemetry::events::CreditLimitChoice)> =
                None;
            with_scrollback(app, |s| {
            ...
            if let Some((url, choice)) = credit_card {
                log_event(intellihelper_telemetry::events::CreditLimitUpsellClicked {
        ...
        Action::AnnouncementsOpenCta(surface) => {
            if let Some((promo, url)) = crate::views::announcements::promo_cta_target(
                ...
                log_event(intellihelper_telemetry::events::AnnouncementCtaClicked {
                    id: promo_id,
                    source: surface,
                });
                open_url_or_show(app, &url);
        ...
        Action::ShowTokenUsage => dispatch_show_token_usage(app),
        Action::OptimizeInput => dispatch_optimize_input(app),
        Action::UndoOptimizeInput => dispatch_undo_optimize_input(app),
        ...
        Action::SetCodingDataSharing { opted_in } => set_coding_data_sharing(
            app,
            opted_in,
            intellihelper_telemetry::events::CodingDataConsentSource::Settings,
        ),
