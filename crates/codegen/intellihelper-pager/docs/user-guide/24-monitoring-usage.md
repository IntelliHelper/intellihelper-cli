...
> **Status: alpha.** The schema below is versioned (`intellihelper_code.schema.version = v1`);
> additive changes may occur without notice, renames/removals will bump the
> version and be called out in the changelog.

IntelliHelper CLI can export usage **metrics** and **events** to your organization's
...
|---------|---------------|
| Telemetry master switch | `[features] telemetry` / `INTELLIHELPER_TELEMETRY_ENABLED` |
| Coding data, retention, and training | Settings — `/privacy` opens the row |
| Trace upload | `[telemetry] trace_upload` / `INTELLIHELPER_TELEMETRY_TRACE_UPLOAD` |
| External OpenTelemetry | `INTELLIHELPER_EXTERNAL_OTEL` / `[telemetry] otel_*` (this guide) |
...
```bash
export INTELLIHELPER_EXTERNAL_OTEL=1                  # master switch
...
`INTELLIHELPER_EXTERNAL_OTEL=1` alone enables **nothing** — you must also select at
...
|---|---|---|
| `INTELLIHELPER_EXTERNAL_OTEL` | `0` | Master switch. Distinct from `INTELLIHELPER_TELEMETRY_ENABLED`, which controls SpaceXAI-internal product analytics — the two govern opposite-pointing data flows. |
...
| `OTEL_LOG_USER_PROMPTS` | `0` | Content gate: prompt text on `intellihelper_code.user_prompt` (60 KB cap, secret-scrubbed). |
...
> `INTELLIHELPER_EXTERNAL_OTEL` is set, product analytics ignores those vars, and the
...
standard OTEL names** (`INTELLIHELPER_EXTERNAL_OTEL`, `OTEL_*`) for ecosystem
...
the `[telemetry]` `otel_*` keys through `intellihelper setup` managed config /
...
(`intellihelper --debug`) for `external otel:` lines — they record whether the stream
...
| Attribute | Value |
|---|---|
| `service.name` | `intellihelper-cli` |
...
| `intellihelper_code.schema.version` | `v1` |
...
## Metrics (meter scope `ai.xai.intellihelper_code`)

| Metric | Unit | Attributes |
|---|---|---|
| `intellihelper_code.session.count` | `{session}` | base attrs only |
| `intellihelper_code.token.usage` | `{token}` | `type` = `input` \| `output` \| `reasoning` \| `cache_read`; `model` |
| `intellihelper_code.turn.count` | `{turn}` | `outcome` = `completed` \| `cancelled` \| `error`; `model` |
| `intellihelper_code.tool.decision` | `{decision}` | `tool_name`, `decision` = `allow` \| `deny` \| `cancelled` \| `followup`, `access_kind`, `permission_mode` |
| `intellihelper_code.tool.usage` | `{call}` | `tool_name`, `outcome` |
| `intellihelper_code.error.count` | `{error}` | `error_category`, `model` |

There is no `cost.usage` metric: join `intellihelper_code.token.usage` with your own
...
|---|---|
| `intellihelper_code.session_start` | `model`, `permission_mode`, `mcp_server_count`, `plugin_count`, `skill_count`, `hook_count`, `memory_enabled`, `is_git_repo`, `client_identifier` |
| `intellihelper_code.session_end` | `duration_secs`, `turn_count`, `tool_call_count`, `compaction_count`, `model` |
| `intellihelper_code.user_prompt` | `prompt_length`, `model`, `screen_mode?` (`fullscreen` \| `inline` \| `minimal` \| `headless` \| `other`); `prompt` (**prompts**) |
| `intellihelper_code.turn_completed` | `outcome`, `duration_ms`, `tool_call_count`, `model`, `error_category?`, `cancellation_category?` |
| `intellihelper_code.api_request` | `model`, `duration_ms`, `stop_reason?`, `input_tokens`, `output_tokens`, `reasoning_tokens`, `cache_read_tokens` |
| `intellihelper_code.api_error` | `error_category`, `model`, `status_code?`, `duration_ms?` |
| `intellihelper_code.tool_result` | `tool_name`, `outcome`, `success`, `duration_ms`, `file_extension`; `tool_parameters`, `file_path` (**details**) |
| `intellihelper_code.tool_decision` | `tool_name`, `decision`, `access_kind`, `permission_mode`, `source` |
| `intellihelper_code.mcp_server_connection` | `status`, `transport_type`, `duration_ms`, `tool_count?`, `error_type?`; `mcp_server.name` (**details**; collapsed to `mcp_server` otherwise) |
| `intellihelper_code.permission_mode_changed` | `to_mode`, `trigger` |
| `intellihelper_code.skill_activated` | `skill_source`, `trigger` = `slash_command` \| `skill_md_read` \| `skill_tool`; `skill.name` (**details**) |
| `intellihelper_code.plugin_loaded` | `install_kind?`, `success`, `error_category?`; `plugin_name` (**details**) |
| `intellihelper_code.compaction` | `duration_ms`, `tokens_before`, `tokens_after`, `model?` |
| `intellihelper_code.subagent` | `phase` = `launched` \| `completed`, `subagent_type?`, `outcome?`, `duration_ms?` |
| `intellihelper_code.auth` | `auth_method` |
| `intellihelper_code.internal_error` | `error_type` (class only — no message, no location) |
| `intellihelper_code.model_switched` | `from_model`, `to_model`, `success`, `error_code?` |
...
sum by (model, type) (rate(intellihelper_code_token_usage_total[1h]))

# Sessions per team per day
sum by (team_id) (increase(intellihelper_code_session_count_total[1d]))

# Tool-permission denial ratio
sum(rate(intellihelper_code_tool_decision_total{decision="deny"}[1h]))
  / sum(rate(intellihelper_code_tool_decision_total[1h]))
```
