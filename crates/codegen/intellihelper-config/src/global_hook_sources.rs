//! IntelliHelper-owned direct global hook paths shared by shell discovery and sandbox
//! write-deny: `$INTELLIHELPER_HOME/hooks`, `hooks-paths`, and absolute registry targets.
...
pub enum GlobalHookSourceKind {
    /// `$INTELLIHELPER_HOME/hooks/` (discovered + protected).
    HookDirectory,
    /// `$INTELLIHELPER_HOME/hooks-paths` (protected; never loaded as hook JSON).
    RegistryFile,
...
pub enum GlobalHookSourceError {
    ...
    #[error("symlinked INTELLIHELPER_HOME is not allowed under sandbox write-deny: {path}")]
    SymlinkedIntelliHelperHome { path: PathBuf },
    ...
    #[error("IntelliHelper hooks directory has wrong type (expected real directory): {path}")]
    InvalidHooksDir { path: PathBuf },
    #[error("IntelliHelper hooks-paths registry has wrong type (expected real file): {path}")]
    InvalidRegistryFile { path: PathBuf },
    #[error("cannot create IntelliHelper hooks directory {path}: {source}")]
    CreateHooksDir {
        path: PathBuf,
        #[source]
        source: io::Error,
    },
    #[error("cannot create IntelliHelper hooks-paths registry {path}: {source}")]
    CreateRegistryFile {
        path: PathBuf,
        #[source]
        source: io::Error,
...
/// Ensure real `$INTELLIHELPER_HOME/hooks` dir + `hooks-paths` file (create if missing).
/// Race-resistant create (`create_dir` / `create_new`+`O_NOFOLLOW`); never
/// truncates an existing registry; rejects symlinks/wrong types.
pub fn ensure_intellihelper_hook_slots(intellihelper_home: &Path) -> Result<(), GlobalHookSourceError> {
    if path_has_symlink_component(intellihelper_home) {
        return Err(GlobalHookSourceError::SymlinkedIntelliHelperHome {
            path: intellihelper_home.to_path_buf(),
        });
    }

    match std::fs::create_dir(intellihelper_home) {
        Ok(()) => {}
        Err(e) if e.kind() == io::ErrorKind::AlreadyExists => {}
        Err(e) if e.kind() == io::ErrorKind::NotFound => {
            std::fs::create_dir_all(intellihelper_home).map_err(|source| {
                GlobalHookSourceError::CreateHooksDir {
                    path: intellihelper_home.to_path_buf(),
                    source,
                }
    ...
    let intellihelper_meta = std::fs::symlink_metadata(intellihelper_home).map_err(|source| {
        GlobalHookSourceError::CreateHooksDir {
            path: intellihelper_home.to_path_buf(),
            source,
        }
    })?;
    if intellihelper_meta.file_type().is_symlink() || !intellihelper_meta.file_type().is_dir() {
        return Err(GlobalHookSourceError::SymlinkedIntelliHelperHome {
            path: intellihelper_home.to_path_buf(),
        });
    }

    let hooks = intellihelper_home.join("hooks");
    ...
    let registry = intellihelper_home.join("hooks-paths");
...
/// Resolve IntelliHelper-owned direct global hook sources (`reject_symlinks` for sandbox).
pub fn resolve_global_hook_sources(
    ...
    if let Some(intellihelper) = intellihelper_home {
        if reject_symlinks && path_has_symlink_component(intellihelper) {
            return Err(GlobalHookSourceError::SymlinkedIntelliHelperHome {
                path: intellihelper.to_path_buf(),
            });
        }

        let hooks = intellihelper.join("hooks");
        let hooks_paths = intellihelper.join("hooks-paths");
        if reject_symlinks {
