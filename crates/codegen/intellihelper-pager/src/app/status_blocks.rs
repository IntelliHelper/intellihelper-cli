...
///
/// Layout mirrors `/token-usage` (Input / Output / Total with cached +
/// reasoning, then By model) so the status-bar badge click and slash command
/// read the same way.
pub(crate) fn session_usage_block_text(
    ...
    if t.model_calls == 0 && usage.model_usage.is_empty() {
        return if usage.usage_is_incomplete {
            "Session usage\n\
             · Input:    0 (0 cached)\n\
             · Output:   0 (0 reasoning)\n\
             · Total:    0 · calls: 0\n\
             Note: none recorded yet; tracking is incomplete and may under-count."
                .to_string()
        } else {
            "Session usage\n\
             · Input:    0 (0 cached)\n\
    ...
    // Multi-model only — single-model totals already cover the ledger.
    if usage.model_usage.len() > 1 {
        ...
        for (model, m) in &usage.model_usage {
            rows.push(format!(
                ...
                group_thousands(m.total_tokens),
                format_cost(m),
            ));
        }
    }
    ...
    join_header_rows("Session usage (since start or last resume)".to_string(), rows)
}

/// Cost cell. Ticks are 1e10 per USD; partial sums are scrubbed to absent.
fn format_cost(m: &intellihelper_shell::extensions::notification::PromptUsageModel) -> String {
    use intellihelper_shell::extensions::notification::ticks_to_usd;
...
mod tests {
    use super::*;
    use intellihelper_shell::extensions::notification::{PromptUsage, PromptUsageModel};
    ...
    fn session_usage_block_empty_ledger() {
        ...
        assert!(text.contains("0 (0 cached)"), "{text}");
        ...
        let incomplete_text = session_usage_block_text(&incomplete);
        assert!(incomplete_text.contains("incomplete"), "{incomplete_text}");
        assert!(incomplete_text.contains("Input:"), "{incomplete_text}");
    }
    ...
    fn session_usage_block_lists_models_when_multiple() {
        ...
        usage
            .model_usage
            .insert("intellihelper-build".into(), model_row(100, 10, None));
        usage
            .model_usage
            .insert("intellihelper-4".into(), model_row(50, 5, None));
        let text = session_usage_block_text(&usage);
        assert!(text.contains("By model:"), "{text}");
        assert!(text.contains("intellihelper-build — 100 in / 10 out"), "{text}");
        assert!(text.contains("intellihelper-4 — 50 in / 5 out"), "{text}");
    }
