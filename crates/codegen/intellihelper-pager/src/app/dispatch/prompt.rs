...
/// Start (if needed) and submit the initial prompt from `intellihelper "<prompt>"`.
///
...
pub(super) fn dispatch_show_undo_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(intellihelper_telemetry::events::ContextualTip {
            tip: intellihelper_telemetry::events::ContextualTipKind::Undo,
...
pub(in crate::app) fn show_small_screen_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(intellihelper_telemetry::events::ContextualTip {
            tip: intellihelper_telemetry::events::ContextualTipKind::SmallScreen,
            action: intellihelper_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(in crate::app) fn show_ssh_wrap_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(intellihelper_telemetry::events::ContextualTip {
            tip: intellihelper_telemetry::events::ContextualTipKind::SshWrap,
            action: intellihelper_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(super) fn dispatch_show_plan_nudge(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(intellihelper_telemetry::events::ContextualTip {
            tip: intellihelper_telemetry::events::ContextualTipKind::PlanMode,
            action: intellihelper_telemetry::events::ContextualTipAction::Shown,
        });
    }
    vec![]
...
pub(super) fn dispatch_show_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(intellihelper_telemetry::events::ContextualTip {
            tip: intellihelper_telemetry::events::ContextualTipKind::WordSelect,
            action: intellihelper_telemetry::events::ContextualTipAction::Shown,
        });
    }
...
pub(super) fn dispatch_accept_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    log_event(intellihelper_telemetry::events::ContextualTip {
        tip: intellihelper_telemetry::events::ContextualTipKind::WordSelect,
        action: intellihelper_telemetry::events::ContextualTipAction::Accepted,
    });
...
fn maybe_show_send_now_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(intellihelper_telemetry::events::ContextualTip {
            tip: intellihelper_telemetry::events::ContextualTipKind::SendNow,
            action: intellihelper_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
    ...
    if !literal && trimmed.starts_with('/') {
        ...
        let exec_result = {
            ...
            if let Some(invocation) = parse_invocation(trimmed) {
                ...
                {
                    use intellihelper_telemetry::events::{PagerCommandSource, PagerSlashCommand};
                    use intellihelper_telemetry::session_ctx::log_event;
        ...
        match exec_result {
            ...
            CommandResult::Action(Action::OptimizeInput) => {
                // Typing `/optimize-input` occupies the composer (same as
                // `/edit-prompt`). Prefer Ctrl+Shift+P or the command palette
                // so an existing draft is preserved. Clear the slash text and
                // still run so palette-dispatched OptimizeInput works.
                if consume_input {
                    agent.prompt.set_text("");
                }
                return dispatch(Action::OptimizeInput, app);
            }
...
pub(super) fn handle_prompt_response(
    ...
    if let Some(agent) = app.agents.get_mut(&agent_id) {
        ...
        // Per-prompt usage in PromptResponse `_meta.usage` — fold into the
        // session status-bar badge (cumulative spend for this process).
        if let Ok(pr) = &result
            && let Some(usage_val) = pr.meta.as_ref().and_then(|m| m.get("usage"))
            && let Ok(usage) = serde_json::from_value::<
                intellihelper_shell::extensions::notification::PromptUsage,
            >(usage_val.clone())
        {
            agent.session_usage_total_tokens = agent
                .session_usage_total_tokens
                .saturating_add(usage.totals.total_tokens);
        }
        ...
        let free_usage_blocked = agent.session.free_usage_blocked
            || result
                .as_ref()
                .err()
                .is_some_and(|e| intellihelper_shell::sampling::error::is_free_usage_exhausted_error(e));
        ...
        if let Some((kind, body)) = notification {
            ...
            if kind != NotificationEventKind::TurnComplete || queue_empty {
                ...
                let notif_title = session_name
                    .map(|s| s.to_string())
                    .unwrap_or_else(|| "IntelliHelper".into());
