...
impl ProtectedEditReason {
    pub fn kind(self) -> &'static str {
        match self {
            ...
            Self::Etc => "etc",
            Self::IntelliHelperConfig => "intellihelper_config",
            Self::IntelliHelperSandbox => "intellihelper_sandbox",
    ...
    pub fn description(self) -> Option<&'static str> {
        match self {
            ...
            Self::IntelliHelperConfig => Some(
                "Note: This edit contains changes to IntelliHelper config, which can alter permissions, tools, and other behavior in later sessions.",
            ),
            Self::IntelliHelperSandbox => Some(
                "Note: This edit contains changes to the IntelliHelper sandbox config, which can loosen filesystem and network restrictions on commands.",
            ),
...
pub(crate) fn edit_target_protection(path: &Path) -> Option<ProtectedEditReason> {
    ...
    let lexical = intellihelper_paths::normalize_lexically(path);
...
fn protected_edit_reason(path: &Path) -> Option<ProtectedEditReason> {
    ...
    if protected_intellihelper_hook_root(path, &string_components) {
    ...
    if let Some(reason) = protected_intellihelper_config_file(path, &string_components) {
        return Some(reason);
    }
...
/// IntelliHelper config files that alter permissions (`config.toml`, the
...
/// inside any `.intellihelper` dir (user-global default and workspace overlays) and
/// directly under a custom `$INTELLIHELPER_HOME`, which the component match cannot see.
fn protected_intellihelper_config_file(path: &Path, components: &[&str]) -> Option<ProtectedEditReason> {
    protected_intellihelper_config_file_with_home(
        path,
        components,
        intellihelper_config::user_intellihelper_home().as_deref(),
    )
}

fn protected_intellihelper_config_file_with_home(
    path: &Path,
    components: &[&str],
    user_intellihelper_home: Option<&Path>,
) -> Option<ProtectedEditReason> {
    let reason = match components.last().copied() {
        Some(
            intellihelper_config::USER_CONFIG_FILENAME
            | intellihelper_config::MANAGED_CONFIG_FILENAME
            | intellihelper_config::REQUIREMENTS_FILENAME,
        ) => ProtectedEditReason::IntelliHelperConfig,
        Some("sandbox.toml") => ProtectedEditReason::IntelliHelperSandbox,
        _ => return None,
    };
    let in_dot_grok = components.len() >= 2 && components[components.len() - 2] == ".intellihelper";
    let in_intellihelper_home = || intellihelper_home_matches(user_intellihelper_home, |home| path.parent() == Some(home));
    (in_dot_grok || in_intellihelper_home()).then_some(reason)
}

/// True when `pred` holds for the user intellihelper home in either its lexical or
...
fn intellihelper_home_matches(home: Option<&Path>, pred: impl Fn(&Path) -> bool) -> bool {
    home.is_some_and(|home| {
        let lexical = intellihelper_paths::normalize_lexically(home);
        pred(&lexical)
            || resolve_following_symlinks(&lexical, 0).is_some_and(|resolved| pred(&resolved))
    })
}

fn path_is_under_user_intellihelper_hook_root(path: &Path, intellihelper_home: &Path) -> bool {
    path.starts_with(intellihelper_home.join("hooks")) || path == intellihelper_home.join("hooks-paths")
}

fn protected_intellihelper_hook_root(path: &Path, components: &[&str]) -> bool {
    components.windows(2).any(|pair| pair == [".intellihelper", "hooks"])
        || components.ends_with(&[".intellihelper", "hooks-paths"])
        || intellihelper_home_matches(intellihelper_config::user_intellihelper_home().as_deref(), |home| {
            path_is_under_user_intellihelper_hook_root(path, home)
        })
}
...
mod tests {
    ...
    fn sensitive_edit_targets_and_lexical_aliases_prompt() {
        for path in [
            "/home/user/.zshrc",
            "/etc",
            "/etc/intellihelper-test",
            "/work/subdir/../.git/hooks/pre-commit",
            "/home/user/.intellihelper/sandbox.toml",
            "/work/project/.intellihelper/sandbox.toml",
        ] {
            assert!(
        ...
        for path in [
            "/work/src/main.rs",
            "/work/project/.intellihelper/config.toml/backup",
    ...
    fn edit_target_protection_classifies_reasons() {
        let cases = [
            (
                "/home/user/.intellihelper/hooks/evil.json",
            ...
            (
                "/home/user/.intellihelper/config.toml",
                ProtectedEditReason::IntelliHelperConfig,
            ),
            (
                "/home/user/.intellihelper/sandbox.toml",
                ProtectedEditReason::IntelliHelperSandbox,
            ),
            (
                "/work/project/.intellihelper/sandbox.toml",
                ProtectedEditReason::IntelliHelperSandbox,
            ...
            (
                "/home/user/.intellihelper/managed_config.toml",
                ProtectedEditReason::IntelliHelperConfig,
            ),
            (
                "/home/user/.intellihelper/requirements.toml",
                ProtectedEditReason::IntelliHelperConfig,
            ),
            (
    ...
    fn sensitive_edit_targets_include_hook_roots() {
        for path in [
            "/home/user/.intellihelper/hooks/evil.json",
            "/home/user/.intellihelper/hooks/nested/deep.json",
            "/home/user/.intellihelper/hooks-paths",
            "/home/user/.claude/settings.json",
            "/home/user/.claude/settings.local.json",
            "/home/user/.cursor/hooks.json",
            "/work/project/.intellihelper/hooks/local.json",
            "/work/project/.intellihelper/hooks-paths",
        ] {
            assert!(
                edit_target_protection(Path::new(path)).is_some(),
        ...
        for path in [
            "/home/user/.intellihelper/hooks-disabled/note.json",
            "/home/user/.intellihelper/hooks-evil/note.json",
    ...
    #[test]
    fn path_is_under_user_intellihelper_hook_root_matches_relocated_home() {
        let home = Path::new("/custom/intellihelper-home");
        for path in [
            "/custom/intellihelper-home/hooks/x.json",
            "/custom/intellihelper-home/hooks/nested/deep.json",
            "/custom/intellihelper-home/hooks",
            "/custom/intellihelper-home/hooks-paths",
        ] {
            assert!(
                path_is_under_user_intellihelper_hook_root(Path::new(path), home),
                "must match under custom intellihelper home: {path}"
            );
        }
        for path in [
            "/custom/intellihelper-home/hooks-disabled/note.json",
            "/custom/intellihelper-home/hooks-evil/note.json",
            "/custom/intellihelper-home/config.toml",
            "/custom/other/hooks/x.json",
            "/custom/intellihelper-home-extra/hooks/x.json",
        ] {
            assert!(
                !path_is_under_user_intellihelper_hook_root(Path::new(path), home),
    ...
    fn sensitive_edit_targets_follow_symlinks() {
        ...
        .unwrap();
        let intellihelper_hook = outside.path().join(".intellihelper/hooks/evil.json");
        std::fs::create_dir_all(intellihelper_hook.parent().unwrap()).unwrap();
        std::fs::write(&intellihelper_hook, b"{}").unwrap();
        symlink(&intellihelper_hook, ws.path().join("intellihelper-hook-link")).unwrap();

        for path in [
            ws.path().join("file-link"),
            ws.path().join("hooks-link/new-hook"),
            ws.path().join("module-hooks-link/new-hook"),
            ws.path().join("intellihelper-hook-link"),
        ] {
            assert!(
    ...
    /// A custom `$INTELLIHELPER_HOME` has no `.intellihelper` path component, so the live
    /// `config.toml` / `sandbox.toml` must be caught by the home-prefix branch.
    #[test]
    fn intellihelper_config_files_under_custom_intellihelper_home_are_protected() {
        let home = tempfile::tempdir().unwrap();
        let home_path = home.path();
        for (file, reason) in [
            ("config.toml", ProtectedEditReason::IntelliHelperConfig),
            ("managed_config.toml", ProtectedEditReason::IntelliHelperConfig),
            ("requirements.toml", ProtectedEditReason::IntelliHelperConfig),
            ("sandbox.toml", ProtectedEditReason::IntelliHelperSandbox),
        ] {
            let path = home_path.join(file);
            let components = [file];
            assert_eq!(
                protected_intellihelper_config_file_with_home(&path, &components, Some(home_path)),
                Some(reason),
                "{file} directly under $INTELLIHELPER_HOME must be protected"
            );
        }
    ...
    /// The resolved-symlink arm of the intellihelper-home match must decide: `$INTELLIHELPER_HOME`
    /// points at a symlink while the edit targets the physical home directory,
    /// so the lexical parent-equality arm cannot fire.
    #[test]
    #[cfg(unix)]
    fn intellihelper_config_under_symlinked_intellihelper_home_is_protected() {
        use std::os::unix::fs::symlink;
        ...
        assert_eq!(
            ...
            Some(ProtectedEditReason::IntelliHelperSandbox)
        );
    }
    ...
    fn resolved_root_alias_matches_physical_destination() {
        ...
        assert!(resolved_path_is_within_root(
            &resolved_root.join("intellihelper-test"),
            Path::new("/etc")
        ));
        assert!(!resolved_path_is_within_root(
            Path::new("/tmp/intellihelper-test"),
            Path::new("/etc")
        ));
    }
