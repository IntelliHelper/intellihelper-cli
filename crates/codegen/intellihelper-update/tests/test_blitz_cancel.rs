...
//!
//! > `~/.intellihelper/bin/intellihelper` resolves to a binary that passes the smoke-test, OR it
...
use intellihelper_update::auto_update::install_internal_from_base;
...
/// (`intellihelper` and `agent` — see `swap_managed_bin_links`). Returns the
...
fn seed_previous_good(home: &Path, version: &str, platform: &str) -> PathBuf {
    ...
    let prev = downloads.join(format!("intellihelper-{version}-{platform}"));
    std::fs::write(&prev, small_good_artifact()).unwrap();
    std::fs::set_permissions(&prev, std::fs::Permissions::from_mode(0o755)).unwrap();

    let rel = format!("../downloads/intellihelper-{version}-{platform}");
...
/// What the active `intellihelper` should resolve to after an install attempt.
...
/// to both `intellihelper` and `agent` — `swap_managed_bin_links` moves them together.
...
/// `cargo nextest run -p intellihelper-update --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]
...
async fn blitz_fuzz_stress() {
    ...
    let iterations: usize = std::env::var("INTELLIHELPER_BLITZ_ITERS")
        .ok()
