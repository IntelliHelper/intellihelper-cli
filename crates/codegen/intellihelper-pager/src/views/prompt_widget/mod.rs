...
//!  intellihelper-3 · yolo                             ← info line (optional)
//! ```
//!
...
pub struct PromptInfo<'a> {
    ...
    /// Show a clickable optimize-draft icon (sparkle) after the mode flags.
    pub show_optimize: bool,
    /// Hover highlight for the optimize icon.
    pub optimize_hovered: bool,
}
...
pub struct PromptWidget {
    ...
    /// Screen rect of the optimize-sparkle icon from the last info-line paint
    /// (for click / hover hit-testing). `None` when the icon was not drawn.
    pub(crate) optimize_button_rect: Option<Rect>,
        ...
        let applied = if snap.cursor_in_command {
            ...
            if let Some(ref range) = snap.command_range {
                    ...
                    && self.textarea.text().is_char_boundary(range.end)
                {
                    ...
                    // `/mod intellihelper-4` yields `/model intellihelper-4`, not `/model  intellihelper-4`
            ...
            // Replace the args range (e.g., "gr" → "IntelliHelper 4 Fast").
    ...
    fn handle_key_inner(&mut self, key: &KeyEvent) -> PromptEvent {
        ...
        if changed {
            ...
            if is_backspace_key && !old_text.is_empty() {
                use intellihelper_telemetry::events::BackspaceNoEffect;
...
impl PromptWidget {
    ...
    pub fn draw(
        ...
        // Fresh each paint; `render_info_line` re-sets when the sparkle is drawn.
        self.optimize_button_rect = None;
        ...
        // Bottom divider: ╰──────────intellihelper-3 · flags──╯
    ...
    /// Render the info line: right-aligned `model_name · flag1 · ✦`, optional
    /// `multiline` on the far right.
    ///
    ...
    /// border. Stores the optimize-icon hit rect on `self` when drawn.
    pub(crate) fn render_info_line(
        ...
        // Left side: model name + flags (+ optional optimize sparkle). Wrap
        // with leading/trailing spaces so the rendered cells adjacent to the
        // corner borders (╰ / ╯) are blanked out instead of showing the
        // underlying `─` glyphs from the bottom-border fill.
        let pad_style = Style::default().bg(bg);
        let mut left_spans = vec![Span::styled(" ", pad_style)];
        // Column offset of the optimize glyph within the left line (after
        // the leading pad). Used to compute the screen hit rect after paint.
        let mut optimize_col_in_left: Option<u16> = None;
        let mut left_cols: u16 = 1; // leading pad
        if let Some(warning) = info.usage_warning {
            ...
            left_cols = left_cols.saturating_add(unicode_width::UnicodeWidthStr::width(warning) as u16);
            left_spans.push(Span::styled(" · ", sep_style));
            left_cols = left_cols.saturating_add(3);
        }
        left_spans.push(Span::styled(info.model_name.to_owned(), model_style));
        left_cols = left_cols
            .saturating_add(unicode_width::UnicodeWidthStr::width(info.model_name) as u16);
        for flag in info.flags {
            ...
            left_cols =
                left_cols.saturating_add(unicode_width::UnicodeWidthStr::width(flag.text) as u16);
        }
        if info.show_optimize {
            left_spans.push(Span::styled(" · ", sep_style));
            left_cols = left_cols.saturating_add(3);
            optimize_col_in_left = Some(left_cols);
            let icon = crate::glyphs::optimize_sparkle();
            let opt_style = if info.optimize_hovered {
                Style::default()
                ...
                let dimmed = crate::render::color::blend_color(bg, theme.text_secondary, flag_opacity)
                    .unwrap_or(theme.gray);
                Style::default().fg(dimmed).bg(bg)
            };
            left_spans.push(Span::styled(icon.to_owned(), opt_style));
            left_cols = left_cols
                .saturating_add(unicode_width::UnicodeWidthStr::width(icon) as u16);
        }
        // Trailing pad mirrors the leading pad above.
        left_spans.push(Span::styled(" ", pad_style));
        left_cols = left_cols.saturating_add(1);
        ...
        let (left_x, left_w) = if !right_spans.is_empty() {
        ...
        if let Some(col) = optimize_col_in_left {
            let icon_w = unicode_width::UnicodeWidthStr::width(crate::glyphs::optimize_sparkle())
                as u16;
            // Pad hit target slightly so the larger ★ control is easy to click.
            let hit_w = icon_w.saturating_add(1).max(3);
            // Only register a hit if the glyph fully fits in the painted width.
            if col.saturating_add(icon_w) <= left_w {
                self.optimize_button_rect = Some(Rect {
                    x: left_x.saturating_add(col),
                    y: area.y,
                    width: hit_w.min(left_w.saturating_sub(col)).max(1),
                    height: 1,
                });
            }
        }
