...
impl AgentView {
    ...
    pub fn draw(
        ...
        // Session cumulative spend (in+out), shown next to context window fill.
        // Clickable → `/usage` (input / output / cached breakdown).
        if !self.chat_kind {
            status.push(
                "session_usage",
                context_bar::session_usage_line_with_hover(
                    self.session_usage_total_tokens,
                    self.hit_session_usage.hovered,
                    &theme,
                ),
            );
        }
        ...
        self.hit_session_usage.rect = areas.get("session_usage").copied();
        ...
        if let Some(profile) = intellihelper_sandbox::profile_name() {
        ...
        let show_optimize = matches!(self.prompt_mode, PromptMode::Normal)
            && self.prompt_input_mode.prompt_info_override().is_none();
        let optimize_hovered = self.hit_optimize_input.hovered;
        ...
        if permission_view_h > 0 {
            ...
            // Capture optimize sparkle hit rect painted by the info line.
            self.hit_optimize_input.rect = self.prompt.optimize_button_rect;
        }
        // Overlays that replace the normal prompt leave optimize_button_rect
        // unset; clear any stale hit from the previous frame.
        if self.prompt.optimize_button_rect.is_none() {
            self.hit_optimize_input.rect = None;
        }
...
mod toast_fit_tests {
    ...
    fn long_message_truncates_with_ellipsis_instead_of_vanishing() {
        let msg = "Copied via OSC 52 — also saved to /tmp/intellihelper-0/last-copy.txt. If paste fails, hold Shift (or Fn) and drag to select & copy natively.";
    ...
    fn ctrl_f_hint_follows_the_key_in_every_focus_mode() {
        ...
        perm.options = vec![acp::PermissionOption::new(
            acp::PermissionOptionId::new(Arc::from(
                intellihelper_workspace::permission::ALLOW_EDITS_SESSION_OPTION_ID,
            )),
