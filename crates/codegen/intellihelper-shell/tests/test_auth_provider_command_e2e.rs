...
//! a throwaway `INTELLIHELPER_HOME`, no network, and a provider command that needs no
...
use chrono::Utc;
use intellihelper_shell::auth::{AuthMode, IntelliHelperAuth, GrokComConfig, try_ensure_fresh_auth};

const SEED_TOKEN: &str = "stale-token-that-must-be-replaced";

/// Point the process at a throwaway intellihelper home. `intellihelper_home()` memoizes into a
...
async fn auth_provider_command_mints_the_session_credential() {
    ...
    use_temp_intellihelper_home(home.path());

    // `echo <token>` is valid in both `sh -c` and `cmd /C`, so this phase needs
    // no external binary and runs identically on every platform.
    let token = mint_with_provider(home.path(), "echo intellihelper-ext-token").await;
    assert_eq!(token, "intellihelper-ext-token");
