...
use crate::db::{ListFilter, WorktreeDb, WorktreeKind, now_epoch_secs, resolve_intellihelper_home};
...
pub const ENV_AUTO_GC: &str = "INTELLIHELPER_WORKTREE_AUTO_GC";
/// `1` / `true` / `on` forces age-count without delete.
pub const ENV_AUTO_GC_DRY_RUN: &str = "INTELLIHELPER_WORKTREE_AUTO_GC_DRY_RUN";
/// Default max age in seconds (overrides TOML/remote when set and parseable).
pub const ENV_AUTO_GC_MAX_AGE: &str = "INTELLIHELPER_WORKTREE_AUTO_GC_MAX_AGE";
/// `1` / `true` / `on` enables optional discovery rebuild + stale git prune.
pub const ENV_AUTO_GC_REBUILD: &str = "INTELLIHELPER_WORKTREE_AUTO_GC_REBUILD";
...
/// Parse `INTELLIHELPER_WORKTREE_AUTO_GC_MAX_AGE` as seconds; invalid/absent → None.
...
fn maybe_run_rebuild(
    ...
    let home = match resolve_intellihelper_home() {
        Ok(h) => h,
        Err(e) => {
            tracing::warn!(error = %e, "auto worktree rebuild skipped: intellihelper home unresolved");
            return (None, false);
        }
    };
...
/// Scrub stale intellihelper-owned registrations from each known source repo,
/// scoped to worktrees under the intellihelper home to prove ownership (see
...
fn prune_stale_git_worktree_registrations(repos: &BTreeSet<PathBuf>) -> u64 {
    let Ok(intellihelper_home) = resolve_intellihelper_home() else {
        tracing::warn!("auto worktree registration scrub skipped: intellihelper home unresolved");
        return 0;
    };
    let cleaned: u64 = repos
        .iter()
        .filter(|repo| repo.is_dir())
        .map(|repo| crate::git::remove_stale_worktree_registrations_under(repo, &intellihelper_home))
...
mod tests {
    ...
    #[test]
    fn include_rebuild_true_registers_untracked_under_intellihelper_home() {
        let _g = env_guard();
        clear_auto_gc_env();
        let fx = crate::db::IntelliHelperHomeFixture::new();
        let db = WorktreeDb::open(&fx.home).unwrap();
        ...
        assert!(
            ...
            "untracked dir under intellihelper_home/worktrees must be registered"
        );
    }
    ...
    #[test]
    fn prune_keeps_foreign_registrations_outside_intellihelper_home() {
        let _g = env_guard();
        clear_auto_gc_env();
        let fx = crate::db::IntelliHelperHomeFixture::new();
        let db = WorktreeDb::open(&fx.home).unwrap();
        ...
        assert_eq!(
            count_regs(&repo),
            before,
            "user registrations outside intellihelper home must survive"
        );
    }
