...
//! - Session resume
//!
//! Each test spawns a real `intellihelper agent stdio` process, speaks the full ACP
...
//! Run locally:
//! ```bash
//! cargo test -p intellihelper-shell --test test_agent_type_invariant -- --ignored
//! ```
...
fn invalidate_models_cache(home: &std::path::Path) {
    let cache = home.join(".intellihelper").join("models_cache.json");
    if cache.exists() {
        std::fs::remove_file(&cache).expect("failed to delete models_cache.json");
    }
}
/// Start a mock server with two models:
/// - `default-model`: no agent_type (→ defaults to "intellihelper-build")
...
/// - `model-a`: no agent_type (→ "intellihelper-build")
/// - `model-b`: no agent_type (→ "intellihelper-build")
...
/// `intellihelper-build` harness. The system prompt sent to the LLM should contain
/// the intellihelper-build identity string.
#[tokio::test]
#[ignore]
async fn test_default_model_uses_intellihelper_build_harness() {
    with_local_set(|| async {
        ...
        assert!(
            sys_prompt.contains("IntelliHelper") || sys_prompt.contains("intellihelper"),
            "default model should use intellihelper-build harness\nsystem prompt preview: {}",
...
async fn test_session_resume_preserves_harness() {
    with_local_set(|| async {
        ...
        drop(writer);
        let reader =
            IntelliHelperStdioClient::spawn_with_sandbox(&server, workdir.workspace(), shared_sandbox).await;
        ...
        let original_has_intellihelper =
            original_sys_prompt.contains("IntelliHelper") || original_sys_prompt.contains("intellihelper");
        let resumed_has_intellihelper =
            resumed_sys_prompt.contains("IntelliHelper") || resumed_sys_prompt.contains("intellihelper");
        assert_eq!(
            original_has_intellihelper,
            resumed_has_intellihelper,
            "resumed session should use the same harness as the original\n\
             original identity markers: intellihelper={original_has_intellihelper}\n\
             resumed identity markers: intellihelper={resumed_has_intellihelper}\n\
...
/// default to `"intellihelper-build"`. This exercises the serde default.
#[tokio::test]
#[ignore]
async fn test_model_without_agent_type_defaults_to_intellihelper_build() {
    with_local_set(|| async {
            ...
            assert!(
            sys_prompt.contains("IntelliHelper") || sys_prompt.contains("intellihelper"),
            "model without agent_type should default to intellihelper-build harness\nsystem prompt preview: {}",
            &sys_prompt[..sys_prompt.len().min(500)]
        );
        })
        .await;
}
/// The `INTELLIHELPER_AGENT` escape hatch should override the model's agent_type.
/// Setting `INTELLIHELPER_AGENT=intellihelper-build` with an alternate-agent model should use
/// intellihelper-build harness.
#[tokio::test]
#[ignore]
async fn test_intellihelper_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            let client = IntelliHelperStdioClient::spawn_with_sandbox_env_and_args(
                    ...
                    workdir.workspace(),
                    sandbox,
                    &[("INTELLIHELPER_AGENT", "intellihelper-build")],
                    &[],
                )
                .await;
            client.initialize_with_timeout().await;
            ...
            assert!(
            result.is_ok(),
            "prompt with INTELLIHELPER_AGENT override failed: {:?}\nstderr:\n{}",
            result.err(),
            client.stderr()
        );
            let sys_prompt = server
            ...
            assert!(
            sys_prompt.contains("IntelliHelper") || sys_prompt.contains("intellihelper"),
            "INTELLIHELPER_AGENT=intellihelper-build should override catalog model agent_type\nsystem prompt preview: {}",
            &sys_prompt[..sys_prompt.len().min(500)]
        );
        })
        .await;
