...
/// Env override for the whole feature: `INTELLIHELPER_PROMPT_SUGGESTIONS=0/1`.
/// When unset, the persisted `prompt_suggestions` setting applies.
pub const PROMPT_SUGGESTIONS_ENV: &str = "INTELLIHELPER_PROMPT_SUGGESTIONS";

/// Env override for the model used by the suggestion call:
/// `INTELLIHELPER_PROMPT_SUGGESTIONS_MODEL=<model-id>`.
pub const PROMPT_SUGGESTIONS_MODEL_ENV: &str = "INTELLIHELPER_PROMPT_SUGGESTIONS_MODEL";
...
pub const PREFERRED_SUGGESTION_MODEL: &str = "intellihelper-build-0.1";
...
pub struct PromptSuggestionController {
    ...
    /// `INTELLIHELPER_PROMPT_SUGGESTIONS` env var, falling back to the persisted
...
pub fn resolve_enabled() -> bool {
    ...
    ENV_OVERRIDE
        .get_or_init(|| intellihelper_config::env_bool(PROMPT_SUGGESTIONS_ENV))
...
/// env override > `intellihelper-build-0.1` when the catalog offers it > `None`.
///
/// The hint is one tier of the shell-side resolution (env > config.toml >
/// remote settings > this hint > `intellihelper-build-0.1` default): the shell
