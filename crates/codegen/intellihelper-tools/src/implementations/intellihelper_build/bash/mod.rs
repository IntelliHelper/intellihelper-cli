...
use regex::Regex;
use intellihelper_config::shell::AmpersandSemantics;
...
pub struct BashParams {
    ...
    /// intellihelper-build opts up to 10h via intellihelper-shell's `BashToolConfig`.
    ///
    /// When set:
...
impl crate::types::resources::ResourceType for BashParams {
    const ID: &'static str = "intellihelper_build.Bash";
...
// particular production intellihelper-build sets it to 10h in intellihelper-shell's
...
/// [`intellihelper_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
/// should use `background: true`. Env override: `INTELLIHELPER_MAX_FOREGROUND_BLOCK_MS`.
const MAX_FOREGROUND_BLOCK: Duration = Duration::from_secs(300); // 5 minutes

fn max_foreground_block() -> Duration {
    std::env::var("INTELLIHELPER_MAX_FOREGROUND_BLOCK_MS")
        .ok()
...
// ───────────────────────────────────────────────────────────────────────────
// Bare `echo "<msg>"` detection (for statistics + hints in intellihelper_build bash)
// ───────────────────────────────────────────────────────────────────────────
...
/// communication. We want statistics on this pattern for the intellihelper_build
...
impl BashTool {
    ...
    /// (intellihelper-cursor) uses this ceiling to report the true FG wait in
    ...
    ///   the terminal backend default applies (`INTELLIHELPER_FOREGROUND_BLOCK_BUDGET_MS`
    ...
    /// still honors `INTELLIHELPER_FOREGROUND_BLOCK_BUDGET_MS` via `None` on the request.
    ///
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = intellihelper_config::shell::chain_separator();
...
impl crate::types::tool_metadata::ToolMetadata for BashTool {
    ...
    fn tool_namespace(&self) -> ToolNamespace {
        ToolNamespace::IntelliHelperBuild
    }
...
impl tool_runtime::Tool for BashTool {
    ...
    async fn run(
        ...
        let ampersand = intellihelper_config::shell::ampersand_semantics();
        ...
        if input.is_background {
            ...
            let request = TerminalRunRequest {
                ...
                // existing intellihelper_build callers that never opted in are
                // unaffected.
            ...
            // We tag it for statistics (intellihelper_build backend) and can surface an educational
...
mod tests {
    ...
    mod resolve_effective_timeout_tests {
        ...
        fn foreground_positive_clamped_to_default_max() {
            ...
            // intellihelper-shell opts up to 10h via max_timeout_secs. Background unaffected.
