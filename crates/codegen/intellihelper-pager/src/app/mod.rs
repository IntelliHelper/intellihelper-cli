...
pub use intellihelper_prompt_queue as prompt_queue;
mod acp_handler;
mod csi_filter;
mod dispatch;
...
/// (`[ui] mouse_reporting_toggle` / `INTELLIHELPER_MOUSE_REPORTING_TOGGLE`). Seeded once
...
pub(crate) fn voice_mode_requirement_pin() -> Option<bool> {
    intellihelper_config::load_merged_requirements().and_then(|req| {
        req.get("features")
...
pub(crate) fn voice_mode_config_value() -> Option<bool> {
    intellihelper_shell::config::load_effective_config()
        .ok()
        .and_then(|cfg| {
            cfg.get("features")
                .and_then(|f| f.get("voice_mode"))
...
pub(crate) fn resolve_voice_mode_enabled(
    ...
    use intellihelper_shell::agent::config::{BoolFlag, ConfigSource};
    let resolved = BoolFlag::env("INTELLIHELPER_VOICE_MODE")
...
pub(crate) enum ScreenMode {
    ...
    ///
    /// All minimal-mode rendering lives in the sibling `intellihelper-pager-minimal`
...
impl ScreenMode {
    ...
    /// allowlist (`intellihelper-telemetry`'s `KNOWN_SCREEN_MODES`); renaming one
...
pub fn resolve_leader_mode<'p>(
    ...
    _remote_settings: Option<&intellihelper_shell::util::config::RemoteSettings>,
    eligible: bool,
...
pub fn join_early_prefetch(
    handle: Option<intellihelper_shell::agent::models::EarlyPrefetchHandle>,
) -> Option<intellihelper_shell::util::config::RemoteSettings> {
    let handle = handle?;
...
/// should print a message telling the user to relaunch `intellihelper`.
pub async fn run(
    args: PagerArgs,
    bg_update_rx: Option<
        tokio::sync::oneshot::Receiver<Option<intellihelper_update::auto_update::UpdateAvailable>>,
    >,
) -> anyhow::Result<bool> {
    ...
    let grok_com_config = match intellihelper_shell::agent::config::Config::new_from_toml_cfg(
        &raw_config,
    ) {
        Ok(c) => c.grok_com_config,
        Err(e) => {
            tracing::warn!(error = %e, "failed to parse config for auth refresh, using defaults");
            intellihelper_shell::auth::GrokComConfig::default()
        }
    };
    let refreshed_auth = tokio::time::timeout(
        intellihelper_shell::http::STARTUP_AUTH_REFRESH_TIMEOUT,
        intellihelper_shell::auth::try_ensure_fresh_auth(&grok_com_config),
    )
    .await
    .unwrap_or(None);
    let early_prefetch = match refreshed_auth {
        Some(auth) => intellihelper_shell::agent::models::start_early_prefetch_with_auth(Some(auth)),
        None => intellihelper_shell::agent::models::start_early_prefetch(Some(grok_com_config.clone())),
    };
    ...
    intellihelper_shell::util::config::cache_remote_auto_mode(
    ...
    if let Some(reason) = policy_disable_reason {
        tokio::spawn(intellihelper_shell::leader::kill_stale_reachable_leaders(reason));
    }
    if let Some(err) =
    ...
    {
        let summaries = intellihelper_shell::session::persistence::list_summaries(None).await?;
    ...
    let env_hunk_tracker_mode = std::env::var("INTELLIHELPER_HUNK_TRACKER").ok();
    ...
    let launch_auto = intellihelper_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
    );
...
fn print_exit_resume_hint(info: &ExitInfo, max_width: usize, w: &mut impl Write) {
    ...
    if info.minimal {
        let _ = writeln!(w, "  intellihelper --minimal --resume {}", info.session_id);
    } else {
        let _ = writeln!(w, "  intellihelper --resume {}", info.session_id);
    }
}
...
/// BEL/ESC (titles can arrive from intellihelper.com conversation metadata) would
...
fn terminal_title_string(title: &str) -> String {
    ...
    if sanitized.is_empty() {
        ...
        format!("{} - intellihelper", truncated)
    }
}
...
mod tests {
    ...
    fn terminal_title_strips_control_characters() {
        assert_eq!(
            ...
            "evil]52;c;payloadtitle - intellihelper"
        );
        assert_eq!(terminal_title_string("\x07\x1b\x00"), "intellihelper");
        assert_eq!(terminal_title_string(""), "intellihelper");
        assert_eq!(terminal_title_string("My chat"), "My chat - intellihelper");
    }
    #[test]
    ...
    fn cli_leader_and_no_leader_conflict() {
        let result = try_parse_pager(&["intellihelper-pager", "--leader", "--no-leader"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_leader_flag_parses() {
        let args = try_parse_pager(&["intellihelper-pager", "--leader"]).unwrap();
        assert!(args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    fn cli_no_leader_flag_parses() {
        let args = try_parse_pager(&["intellihelper-pager", "--no-leader"]).unwrap();
        assert!(!args.leader);
        assert!(args.no_leader);
    }
    #[test]
    fn cli_neither_leader_flag_defaults_false() {
        let args = try_parse_pager(&["intellihelper-pager"]).unwrap();
        assert!(!args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    ...
    fn cli_top_level_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["intellihelper-pager", "--leader", "agent"]).unwrap();
        assert!(args.leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    fn cli_top_level_no_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["intellihelper-pager", "--no-leader", "agent"]).unwrap();
        assert!(args.no_leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    ...
    fn remote_settings_leader_mode_true_enables_leader() {
        let rs = intellihelper_shell::util::config::RemoteSettings {
    ...
    fn cli_resume_parses_session_id() {
        let args = try_parse_pager(&["intellihelper-pager", "--resume", "abc-123"]).unwrap();
        assert_eq!(args.session_to_resume(), Some("abc-123"));
    }
    #[test]
    fn cli_short_r_parses_session_id() {
        let args = try_parse_pager(&["intellihelper-pager", "-r", "abc-123"]).unwrap();
        assert_eq!(args.session_to_resume(), Some("abc-123"));
    }
    #[test]
    fn cli_load_alias_parses_session_id() {
        let args = try_parse_pager(&["intellihelper-pager", "--load", "abc-123"]).unwrap();
        assert_eq!(args.session_to_resume(), Some("abc-123"));
    }
    #[test]
    fn cli_resume_preferred_over_load() {
        let mut args = try_parse_pager(&["intellihelper-pager", "--resume", "from-resume"]).unwrap();
        args.load_session = Some("from-load".into());
        assert_eq!(args.session_to_resume(), Some("from-resume"));
    }
    #[test]
    fn cli_continue_flag_parses() {
        let args = try_parse_pager(&["intellihelper-pager", "--continue"]).unwrap();
        assert!(args.continue_last_session);
        assert_eq!(args.session_to_resume(), None);
    }
    #[test]
    fn cli_continue_short_c_parses() {
        let args = try_parse_pager(&["intellihelper-pager", "-c"]).unwrap();
        assert!(args.continue_last_session);
    }
    #[test]
    fn cli_resume_no_id_sets_empty_sentinel() {
        let args = try_parse_pager(&["intellihelper-pager", "--resume"]).unwrap();
    ...
    fn cli_short_r_no_id_sets_empty_sentinel() {
        let args = try_parse_pager(&["intellihelper-pager", "-r"]).unwrap();
        assert_eq!(args.resume_session.as_deref(), Some(""));
        assert!(args.resume_most_recent());
    }
    #[test]
    ...
    fn cli_continue_conflicts_with_resume() {
        let result = try_parse_pager(&["intellihelper-pager", "--continue", "--resume", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_continue_conflicts_with_load() {
        let result = try_parse_pager(&["intellihelper-pager", "--continue", "--load", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    ...
    fn cli_chat_flag_rejected_without_feature() {
        assert!(try_parse_pager(&["intellihelper-pager", "--chat"]).is_err());
    }
    #[cfg(feature = "local-workspace")]
    #[test]
    fn cli_local_workspace_attach_requires_chat() {
        assert!(
            try_parse_pager(&["intellihelper-pager", "--local-workspace-attach=srv"]).is_err(),
            "attach without --chat must clap-error"
        );
        let args =
            try_parse_pager(&["intellihelper-pager", "--chat", "--local-workspace-attach=srv"]).unwrap();
    ...
    fn cli_local_workspace_cwd_requires_chat() {
        assert!(try_parse_pager(&["intellihelper-pager", "--local-workspace-cwd=/tmp/a"]).is_err());
    ...
    fn cli_local_workspace_flags_rejected_without_feature() {
        assert!(try_parse_pager(&["intellihelper-pager", "--local-workspace-attach=srv"]).is_err());
        assert!(try_parse_pager(&["intellihelper-pager", "--local-workspace"]).is_err());
        assert!(try_parse_pager(&["intellihelper-pager", "--local-workspace-cwd=/tmp"]).is_err());
    }
    #[test]
    ...
    fn cli_worktree_flag_parses() {
        let args = try_parse_pager(&["intellihelper-pager", "--worktree"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_short_w_parses() {
        let args = try_parse_pager(&["intellihelper-pager", "-w"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_with_label() {
        let args = try_parse_pager(&["intellihelper-pager", "-w", "my-label"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
    }
    #[test]
    fn cli_worktree_long_with_label() {
        let args = try_parse_pager(&["intellihelper-pager", "--worktree", "fix-bug"]).unwrap();
    ...
    fn cli_worktree_with_resume_parses() {
        let args = try_parse_pager(&["intellihelper-pager", "-w", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    fn cli_worktree_label_with_resume() {
        let args = try_parse_pager(&["intellihelper-pager", "-w", "my-label", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    ...
    fn cli_session_id_parses() {
        let args = try_parse_pager(&["intellihelper-pager", "--session-id", "my-id"]).unwrap();
    ...
    fn cli_session_id_short_s_parses() {
        let args = try_parse_pager(&["intellihelper-pager", "-s", "my-id"]).unwrap();
        assert_eq!(args.session_id.as_deref(), Some("my-id"));
    }
    #[test]
    fn cli_session_id_with_resume_requires_fork() {
        let args = try_parse_pager(&["intellihelper-pager", "-s", "a", "--resume", "b"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_continue_requires_fork() {
        let args = try_parse_pager(&["intellihelper-pager", "-s", "a", "--continue"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_resume_and_fork_ok() {
        let args =
            try_parse_pager(&["intellihelper-pager", "-s", "a", "--resume", "b", "--fork-session"]).unwrap();
    ...
    fn cli_no_alt_screen_flag_parses() {
        let args = try_parse_pager(&["intellihelper-pager", "--no-alt-screen"]).unwrap();
    ...
    #[test]
    fn cli_command_name_is_intellihelper() {
        use clap::CommandFactory;
        assert_eq!(PagerArgs::command().get_name(), "intellihelper");
    }
    #[test]
    fn cli_help_output_header() {
        ...
        assert_eq!(
            first_5,
            vec![
                "IntelliHelper CLI TUI",
                "",
                "Usage: intellihelper [OPTIONS] [PROMPT] [COMMAND]",
                "",
                "Arguments:",
            ]
        );
    ...
    fn cli_completions_parses() {
        ...
        let args = try_parse_pager(&["intellihelper-pager", "completions", "zsh"]).unwrap();
        assert!(matches!(
            args.command,
            Some(Command::Completions { shell: Shell::Zsh })
        ));
        let args = try_parse_pager(&["intellihelper-pager", "completions", "bash"]).unwrap();
        assert!(matches!(
            args.command,
    ...
    fn print_exit_resume_hint_includes_minimal_flag() {
        ...
        assert_eq!(
            String::from_utf8(buf).unwrap(),
            "\nResume this session with:\n  intellihelper --minimal --resume sess-abc\n"
        );
    }
    #[test]
    ...
    fn print_exit_resume_hint_truncates_summary_to_width() {
        ...
        assert!(out.contains(&format!("\n> {}…\n", "p".repeat(17))));
        assert!(out.contains(&format!("\n  {}…\n", "r".repeat(17))));
        assert!(out.contains("  intellihelper --resume sess-abc\n"));
    }
    #[test]
