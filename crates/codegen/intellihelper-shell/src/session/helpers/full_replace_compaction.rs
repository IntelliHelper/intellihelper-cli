//! intellihelper-build's L5 wiring onto the shared full-replace engine
//! (`intellihelper_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](intellihelper_compaction::sample_full_replace_summary);
//! this module adapts intellihelper-build's transport and telemetry to its two seams:
//!
...
//! [`FullReplaceError`](intellihelper_compaction::FullReplaceError).
...
use intellihelper_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use intellihelper_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
...
/// [`CompactionSampler`] for intellihelper-build's full-replace pass.
///
...
/// else the structured intellihelper-build prompt), so the shared `CompactionPrompt`
/// the engine passes is ignored — the engine builds the intellihelper-build prompt,
...
impl CompactionSampler for ShellCompactionSampler {
    ...
    async fn sample_compaction(
        ...
        // message (compat short vs structured intellihelper-build), ignoring the shared
...
/// Map intellihelper-build's [`CompactFailure`] onto the shared engine's
...
/// [`FullReplaceObserver`] that reproduces intellihelper-build's per-attempt telemetry:
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    intellihelper_telemetry::session_ctx::log_event(CompactionRetryDegraded {
