...
pub(super) mod paste_key_tests {
    ...
    fn wrap_host_image_malformed_paste_not_inserted_as_text() {
        ...
        let registry = ActionRegistry::defaults();
        let outcome = agent.handle_input(
            &Event::Paste("INTELLIHELPER_WRAP_IMG\nimage/png\n!!!".to_string()),
            &registry,
        );
        assert!(matches!(outcome, InputOutcome::Unchanged));
        assert!(agent.prompt.text().is_empty());
    ...
    -> crate::views::question_view::QuestionViewState {
        let question = intellihelper_tools::implementations::intellihelper_build::ask_user_question::Question {
            question: "Pick one?".to_string(),
            options: vec![
                intellihelper_tools::implementations::intellihelper_build::ask_user_question::QuestionOption {
                    label: "A".to_string(),
