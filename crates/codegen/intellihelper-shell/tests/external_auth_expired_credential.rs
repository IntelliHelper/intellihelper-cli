...
//!
//! One `#[test]`: the phases share one process-global `INTELLIHELPER_HOME` and env, so
...
use intellihelper_shell::agent::mvp_agent::MvpAgent;
use intellihelper_test_support::{MockInferenceServer, MockModelEntry};
...
fn seed_credential(intellihelper_home: &Path, expires_at: chrono::DateTime<chrono::Utc>) {
    let auth = json!({
    ...
    std::fs::create_dir_all(intellihelper_home).expect("create intellihelper home");
    std::fs::write(
...
fn write_interactive_only_provider(intellihelper_home: &Path) -> String {
    use std::os::unix::fs::PermissionsExt;

    let script = intellihelper_home.join("acme-auth.sh");
    std::fs::write(
        &script,
        "#!/bin/sh\n\
...
fn provider_runs(intellihelper_home: &Path) -> usize {
    std::fs::read_to_string(intellihelper_home.join("provider-runs"))
        .map(|s| s.lines().count())
        .unwrap_or(0)
}
...
fn expired_external_credential_routes_to_the_provider_login_flow() {
    ...
    let intellihelper_home = TempDir::new().expect("intellihelper home");
    let workdir = TempDir::new().expect("workdir");
    seed_credential(
        intellihelper_home.path(),
        chrono::Utc::now() - chrono::Duration::hours(1),
    );
    let provider = write_interactive_only_provider(intellihelper_home.path());
    ...
    unsafe {
        ...
        std::env::set_var("INTELLIHELPER_MODELS_BASE_URL", server.url());
        std::env::set_var("INTELLIHELPER_AUTH_PROVIDER_COMMAND", &provider);
        std::env::set_var("INTELLIHELPER_AUTH_PROVIDER_LABEL", PROVIDER_LABEL);
    ...
    agent_rt.block_on(local.run_until(async move {
        ...
        assert_eq!(
            provider_runs(intellihelper_home.path()),
            1,
        ...
        std::fs::remove_file(intellihelper_home.path().join("auth.json")).expect("remove auth.json");
