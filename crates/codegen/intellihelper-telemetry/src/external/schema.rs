...
/// Wire schema version, exported as resource attr `intellihelper_code.schema.version`.
/// Additive changes (new events/attrs) do not bump it; renames/removals do.
pub const SCHEMA_VERSION: &str = "v1";

/// Meter/logger instrumentation scope name.
pub const SCOPE_NAME: &str = "ai.xai.intellihelper_code";
...
impl ExternalEventName {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SessionStart => "intellihelper_code.session_start",
            Self::SessionEnd => "intellihelper_code.session_end",
            Self::UserPrompt => "intellihelper_code.user_prompt",
            Self::TurnCompleted => "intellihelper_code.turn_completed",
            Self::ApiRequest => "intellihelper_code.api_request",
            Self::ApiError => "intellihelper_code.api_error",
            Self::ToolResult => "intellihelper_code.tool_result",
            Self::ToolDecision => "intellihelper_code.tool_decision",
            Self::McpServerConnection => "intellihelper_code.mcp_server_connection",
            Self::PermissionModeChanged => "intellihelper_code.permission_mode_changed",
            Self::SkillActivated => "intellihelper_code.skill_activated",
            Self::PluginLoaded => "intellihelper_code.plugin_loaded",
            Self::Compaction => "intellihelper_code.compaction",
            Self::Subagent => "intellihelper_code.subagent",
            Self::Auth => "intellihelper_code.auth",
            Self::InternalError => "intellihelper_code.internal_error",
            Self::ModelSwitched => "intellihelper_code.model_switched",
            Self::ContextualTip => "intellihelper_code.contextual_tip",
        }
    }
}
...
pub enum MetricIncrement {
    /// `intellihelper_code.session.count`.
    SessionCount,
    /// `intellihelper_code.token.usage` — `token_type` ∈ `input|output|reasoning|cache_read`.
    TokenUsage {
        token_type: &'static str,
        model: String,
        count: u64,
    },
    /// `intellihelper_code.turn.count`.
    TurnCount {
        outcome: &'static str,
        model: String,
    },
    /// `intellihelper_code.tool.decision`.
    ToolDecision {
        tool_name: String,
        decision: &'static str,
    ...
    /// `intellihelper_code.tool.usage`.
    ToolUsage {
        tool_name: String,
        outcome: &'static str,
    },
    /// `intellihelper_code.error.count`.
    ErrorCount {
...
pub(crate) const METRIC_SESSION_COUNT: &str = "intellihelper_code.session.count";
pub(crate) const METRIC_TOKEN_USAGE: &str = "intellihelper_code.token.usage";
pub(crate) const METRIC_TURN_COUNT: &str = "intellihelper_code.turn.count";
pub(crate) const METRIC_TOOL_DECISION: &str = "intellihelper_code.tool.decision";
pub(crate) const METRIC_TOOL_USAGE: &str = "intellihelper_code.tool.usage";
pub(crate) const METRIC_ERROR_COUNT: &str = "intellihelper_code.error.count";
...
pub(crate) const KNOWN_CLIENT_IDENTIFIERS: &[&str] = &[
    ...
    "intellihelper-shell",
    "intellihelper-web",
    "intellihelper-desktop",
    "intellihelper-code-extension",
    "intellihelper-agent-sdk",
    "nebula",
    "zed",
];
...
/// Built-in tool names (closed enum surface in `intellihelper-tools`) that pass
...
fn tool_outcome_label(o: &file_utils::events::types::ToolOutcome) -> &'static str {
    use file_utils::events::types::ToolOutcome;
    match o {
...
/// `SessionHarness` → `intellihelper_code.session_start`. Emitted from a spawn outside
...
/// `SessionNew` → `intellihelper_code.session.count` (metric only; the flagship
...
/// `SessionEnded` → `intellihelper_code.session_end`.
...
/// `PromptSubmitted` → `intellihelper_code.user_prompt`. Prompt text rides the
...
/// `TurnCompleted` → `intellihelper_code.turn_completed` + `turn.count`
...
/// `ModelResponseReceived` → `intellihelper_code.api_request` + `token.usage`.
...
/// `RateLimitHit` → `intellihelper_code.api_error` (`error_category = rate_limit`).
...
/// `ApiError` → `intellihelper_code.api_error`. Category/class enums only — no
...
/// `ToolCallCompleted` → `intellihelper_code.tool_result` + `tool.usage`.
...
/// `PermissionDecisionPayload` → `intellihelper_code.tool_decision` + `tool.decision`.
...
/// `McpServerConnected` → `intellihelper_code.mcp_server_connection` (`status=connected`).
...
/// `McpServerFailed` → `intellihelper_code.mcp_server_connection` (`status=failed`).
...
/// `PlanModeToggled` → `intellihelper_code.permission_mode_changed`.
...
/// `ContextualTip` → `intellihelper_code.contextual_tip`. Labels only (no user
...
/// `YoloToggled` → `intellihelper_code.permission_mode_changed`.
...
/// `SkillDispatched` → `intellihelper_code.skill_activated`. Skill names are details-gated; source and trigger export by default.
...
/// `PluginInstalled` → `intellihelper_code.plugin_loaded`.
...
/// `PluginUsed` → `intellihelper_code.plugin_loaded` (plugin name details-gated).
...
/// `CompactionCompleted` → `intellihelper_code.compaction`.
...
/// `CompactionTriggered` → `intellihelper_code.compaction` trigger attrs ride on the
/// completion event instead (one event per compaction); not mapped.
/// `SubagentLaunched` → `intellihelper_code.subagent` (`phase=launched`).
...
/// `SubagentCompleted` → `intellihelper_code.subagent` (`phase=completed`).
...
/// `InternalError` → `intellihelper_code.internal_error`. Error class only — no
...
/// `ModelSwitched` → `intellihelper_code.model_switched`.
