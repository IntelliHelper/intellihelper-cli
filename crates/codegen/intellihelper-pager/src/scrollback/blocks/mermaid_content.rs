...
//! the generic [`CodeBlockSpan`](intellihelper_markdown::CodeBlockSpan) API) and
...
use intellihelper_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](intellihelper_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](intellihelper_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a intellihelper_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `IntelliHelperDay` is the only light theme; every other concrete theme (and the
/// `IntelliHelperNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `intellihelper_mermaid::MermaidTheme`; it
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::IntelliHelperDay)
}
...
mod tests {
    ...
    use intellihelper_markdown::StreamingMarkdownRenderer;
    ...
    #[test]
    fn theme_is_dark_maps_intellihelperday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::IntelliHelperDay),
            "IntelliHelperDay is the light theme"
        );
        for dark in [
            ThemeKind::IntelliHelperNight,
