...
// (`~/.intellihelper/trusted_folders.toml`) is the single authority for whether a repo's
...
/// (`<user_intellihelper_home>/trusted-hook-projects`), or `None` when no user intellihelper home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(intellihelper_config::user_intellihelper_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user intellihelper home (set $INTELLIHELPER_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$INTELLIHELPER_HOME/disabled-hooks`, or `None` when no user intellihelper
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(intellihelper_config::user_intellihelper_home()?.join("disabled-hooks"))
}
...
mod tests {
    ...
    fn trust_file_in(dir: &Path) -> PathBuf {
        let intellihelper_dir = dir.join(".intellihelper");
        std::fs::create_dir_all(&intellihelper_dir).unwrap();
        intellihelper_dir.join("trusted-hook-projects")
    }
