// Re-export all types from the standalone intellihelper-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use intellihelper_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `intellihelper-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `intellihelper-sampler` crate can reference them without a circular
// dep on `intellihelper-shell`.

// Tests for the types now live in intellihelper-sampling-types crate.

use intellihelper_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `intellihelper-sampling-types` or
/// `intellihelper-tools`) so neither of those crates needs to depend on
