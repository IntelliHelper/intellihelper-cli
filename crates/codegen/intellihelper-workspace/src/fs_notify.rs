...
/// [`intellihelper_workspace_types::FsEventKind`].
///
...
pub(crate) fn to_workspace_event_kind(kind: FsEventKind) -> intellihelper_workspace_types::FsEventKind {
    match kind {
        FsEventKind::Created => intellihelper_workspace_types::FsEventKind::Created,
        FsEventKind::Modified => intellihelper_workspace_types::FsEventKind::Modified,
        FsEventKind::Removed => intellihelper_workspace_types::FsEventKind::Removed,
        FsEventKind::Renamed => intellihelper_workspace_types::FsEventKind::Renamed,
        // `#[non_exhaustive]` fallback.
        _ => intellihelper_workspace_types::FsEventKind::Modified,
    }
}
...
/// [`WorkspaceEvent::FsChanged`](intellihelper_workspace_types::WorkspaceEvent::FsChanged)
...
pub(crate) fn spawn_fs_event_forwarder(
    ...
    events_tx: tokio::sync::broadcast::Sender<intellihelper_workspace_types::WorkspaceEvent>,
    cwd: PathBuf,
                        ...
                        Ok(FsEvent::FilesChanged { ref paths, kind }) => {
                            ...
                            for path in paths {
                                let _ = events_tx.send(
                                    intellihelper_workspace_types::WorkspaceEvent::FsChanged {
                                        path: path.clone(),
                                        kind: ws_kind,
                                    },
                                );
...
pub(crate) async fn refresh_codebase_graph_after_head_change(
    ...
    intellihelper_tools::util::detach_command(&mut diff_cmd);
    diff_cmd.envs(intellihelper_tools::util::pager_env());
    ...
    if let Some(count) = files_updated {
        let _ = events_tx.send(
            intellihelper_workspace_types::WorkspaceEvent::CodebaseIndexUpdated {
                files_indexed: count,
            },
        );
    }
...
pub(crate) fn ws_event_to_codebase_graph_event(
    path: &std::path::Path,
    kind: intellihelper_workspace_types::FsEventKind,
) -> codebase_graph::FileEvent {
    use codebase_graph::{FileEvent, FileEventKind};
    let graph_kind = match kind {
        intellihelper_workspace_types::FsEventKind::Created => FileEventKind::Created,
        intellihelper_workspace_types::FsEventKind::Modified => FileEventKind::Modified,
        intellihelper_workspace_types::FsEventKind::Removed => FileEventKind::Removed,
        intellihelper_workspace_types::FsEventKind::Renamed => FileEventKind::Renamed,
    };
...
mod tests {
    ...
    fn hidden_dir_positive() {
        assert!(is_under_hidden_dir(
            &PathBuf::from("/workspace/.intellihelper/worktrees/abc/src/main.rs"),
    ...
    fn workspace_event_kind_round_trip() {
        use intellihelper_workspace_types::FsEventKind as WsKind;
        assert_eq!(
