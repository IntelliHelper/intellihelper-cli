...
//! `LspTool` (Tool trait impl) under the `IntelliHelperBuild` namespace.
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.intellihelper/lsp.json or <cwd>/.intellihelper/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }
