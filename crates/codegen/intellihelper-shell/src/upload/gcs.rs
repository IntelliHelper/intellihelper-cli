...
//! `StorageClient` constructed inside `file_utils::gcs::*` helpers.
//!
...
/// shell-side adapter that lets `file_utils::gcs::*` helpers wire
...
/// `INTELLIHELPER_TELEMETRY_GCS_BUCKET`; `None` disables trace uploads until a bucket
/// is configured.
pub(crate) const SESSION_TRACES_BUCKET: Option<&str> =
    option_env!("INTELLIHELPER_SESSION_TRACES_BUCKET_DEFAULT");
