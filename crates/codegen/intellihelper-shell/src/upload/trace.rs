...
use intellihelper_workspace::permission::PermissionEvent;
...
fn record_upload_failure(ctx: &PromptTraceContext, f: UploadFailure<'_>) {
    ...
    if level == UploadFailureLogLevel::Warn {
        intellihelper_telemetry::unified_log::warn(&msg, sid, log_ctx);
    } else {
        intellihelper_telemetry::unified_log::error(&msg, sid, log_ctx);
    }
}
...
pub(crate) fn local_sandbox_telemetry() -> Option<LocalSandboxTelemetry> {
    let profile = intellihelper_sandbox::configured_profile_name()?;
    Some(LocalSandboxTelemetry {
        profile: profile.to_owned(),
        applied: intellihelper_sandbox::is_active(),
    })
}
...
pub(crate) async fn upload_subagent_metadata(
    ...
    if let Err(e) =
        file_utils::gcs::upload_bytes(&config, &gcs_path, &json, "application/json").await
    {
        tracing::warn!(
            session_id = %metadata.child_session_id,
            gcs_path = %gcs_path,
...
pub(crate) async fn upload_plugin_state(
    ctx: &PromptTraceContext,
    registry: Option<&intellihelper_agent::plugins::PluginRegistry>,
) {
...
pub(crate) async fn upload_unified_log(ctx: &PromptTraceContext, wait: UploadWait) {
    ...
    let log_bytes = match tokio::task::spawn_blocking(move || {
        intellihelper_telemetry::unified_log::snapshot_session_log(&session_id)
    })
    .await
    {
        Ok(Some(bytes)) => bytes,
    ...
    .await;
    let full_log_bytes =
        tokio::task::spawn_blocking(intellihelper_telemetry::unified_log::snapshot_log).await;
    let user_id = ctx
        .auth_manager
        .current_or_expired()
        .map(|a| a.user_id)
...
pub(crate) fn build_chat_history_session_state(
    messages: &[intellihelper_sampling_types::conversation::ConversationItem],
...
impl TraceExportSource for DynamicResolver {
    ...
    fn proxy_credentials(&self) -> Option<Arc<dyn intellihelper_auth::AuthCredentialProvider>> {
...
pub(crate) fn spawn_startup_spill_reconcile(
    ...
    tokio::spawn(async move {
        match queue {
            Some(queue) => {
                let report =
                    intellihelper_workspace::recovery::run_startup_recovery(&intellihelper_home, &queue).await;
            ...
            None => {
                let purged = tokio::task::spawn_blocking(move || {
                    intellihelper_workspace::recovery::purge_spilled_items(&intellihelper_home)
                })
                .await;
                match purged {
                    Ok(removed) => {
...
pub(crate) fn spawn_purge_stale_upload_scratch() {
    {
        ...
        let dir = crate::util::intellihelper_home::intellihelper_home()
            .join("upload_queue")
            .join("scratch");
...
pub(crate) fn spawn_upload_queue(
    ...
    let queue = UploadQueue::spawn(intellihelper_home, resolver, UploadRetryPolicy::default());
...
mod tests {
    ...
    fn dynamic_resolver_refreshes_proxy_token() {
        ...
        let initial_auth = IntelliHelperAuth {
        ...
        let refreshed_auth = IntelliHelperAuth {
    ...
    fn dynamic_resolver_rereads_disk_on_expired_token() {
        ...
        let grok_com_config = GrokComConfig::default();
        let scope = grok_com_config.auth_scope();
        let expired_auth = IntelliHelperAuth {
        ...
        let fresh_auth = IntelliHelperAuth {
    ...
    async fn resolve_async_picks_up_disk_refreshed_token() {
        ...
        let grok_com_config = GrokComConfig::default();
        let scope = grok_com_config.auth_scope();
        let valid_auth = IntelliHelperAuth {
    ...
    async fn resolve_async_drives_refresh_chain_when_token_expired() {
        ...
        impl crate::auth::refresh::TokenRefresher for FreshRefresher {
            async fn refresh(
                ...
                crate::auth::refresh::RefreshOutcome::Success(Box::new(crate::auth::IntelliHelperAuth {
    ...
    async fn proactive_refresh_makes_trace_resolve_a_cache_hit() {
        ...
        let grok_com_config = GrokComConfig::default();
        let auth_manager = Arc::new(crate::auth::AuthManager::new(dir.path(), grok_com_config));
        auth_manager.hot_swap(IntelliHelperAuth {
            key: "expired-oidc".into(),
    ...
    async fn dynamic_resolver_auth_recovery_wakes_on_already_rotated_token() {
        ...
        auth_manager.hot_swap(crate::auth::IntelliHelperAuth {
