...
/// attribution.
fn auth_error() -> intellihelper_sampler::SamplingErrorInfo {
    intellihelper_sampler::SamplingErrorInfo {
        kind: intellihelper_sampler::SamplingErrorKind::Auth,
...
async fn make_actor_with_auth_and_credentials(
    ...
    let method_id = match auth_type {
        ...
        chat_state::AuthType::ApiKey => "xai.api_key",
    };
...
fn model_not_found_error() -> intellihelper_sampler::SamplingErrorInfo {
    intellihelper_sampler::SamplingErrorInfo {
            kind: intellihelper_sampler::SamplingErrorKind::Api,
            message: "API error (status 404 Not Found): The model intellihelper-build does not exist or your team does not have access".into(),
    ...
    local
        .run_until(async {
            ...
            assert!(
                ...
                "hint must mention `intellihelper logout`, got: {msg}"
            );
            assert!(
                msg.contains("intellihelper login"),
                "hint must mention `intellihelper login`, got: {msg}"
            );
            assert!(
...
fn unauthorized_401_error() -> intellihelper_sampler::SamplingErrorInfo {
    intellihelper_sampler::SamplingErrorInfo {
            kind: intellihelper_sampler::SamplingErrorKind::Api,
            message: "Unauthorized (401) from https://cli-chat-proxy.grok.com/v1/responses: {\"error\":\"Invalid or expired credentials (auth_kind=bearer, x_xai_token_auth=intellihelper-cli, upstream=Unauthenticated, reason=no auth context)\"}".into(),
            status_code: Some(401),
            is_retryable: false,
            retry_after_secs: None,
            should_retry: None,
...
async fn set_session_model_invalidates_byok_memo_for_same_model_id() {
    ...
    local
        .run_until(async {
            ...
            let cfg = intellihelper_sampler::SamplerConfig {
...
async fn sampler_non_auth_kind_401_on_provider_model_still_recovers() {
    ...
    local
        .run_until(async {
            ...
            error.kind = intellihelper_sampler::SamplingErrorKind::Api;
