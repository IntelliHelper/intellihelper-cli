...
fn turn_end_fetches_prompt_suggestion_when_enabled() {
    ...
    // No `intellihelper-build-0.1` in the test catalog and no env override →
    // `None` on the wire; the shell then uses its own `intellihelper-build-0.1`
...
fn prompt_response_formatted_402_takes_credit_limit_path() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::PromptResponse {
            agent_id: id,
            result: Err(
                "Request failed (402) \u{2014} IntelliHelper CLI usage balance exhausted".to_string(),
            ),
            http_status: None,
            prompt_id: None,
        }),
...
/// The always-on pipeline end to end, with `INTELLIHELPER_SUGGESTIONS` semantics
