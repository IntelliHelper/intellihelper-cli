...
/// env master `INTELLIHELPER_CONTEXTUAL_HINTS` (all-on/off) > user config
...
pub fn resolve_contextual_hints(
    ...
    let resolve_tip = |user: Option<bool>, feature_flag: Option<bool>| -> bool {
        BoolFlag::env("INTELLIHELPER_CONTEXTUAL_HINTS")
            .config(user)
...
fn merge_hints_config_layers(
    ...
    if let Some(m) = managed {
        intellihelper_config::deep_merge_toml(&mut merged, m);
    }
    if let Some(u) = user {
        intellihelper_config::deep_merge_toml(&mut merged, u);
    }
    if let Some(r) = requirements {
        intellihelper_config::deep_merge_toml(&mut merged, r);
    }
    merged
}
...
mod tests {
    ...
    const ENV_CONTEXTUAL_HINTS: &str = "INTELLIHELPER_CONTEXTUAL_HINTS";

    // `INTELLIHELPER_CONTEXTUAL_HINTS` is process-global; serialize the tests reading it
