...
use intellihelper_telemetry::events::{SuperGrokUpsell, SuperGrokUpsellClicked};
...
pub(crate) const UPSELL_URL_UPGRADE: &str = "https://grok.com/__PROTECT_19__?referrer=intellihelper-build";
...
pub(super) fn open_credit_limit_upsell(
    ...
    let (
        ...
        intellihelper_telemetry::events::CreditLimitChoice,
        bool,
    ) = match mode {
        ...
        CreditLimitUpsellMode::LegacyPayg { enabled: true } => (
            ...
            intellihelper_telemetry::events::CreditLimitChoice::PayAsYouGo,
            true,
        ),
    ...
    if max_tier {
        use crate::scrollback::block::RenderBlock;
        log_event(intellihelper_telemetry::events::CreditLimitUpsellShown {
            surface: intellihelper_telemetry::events::CreditLimitUpsellSurface::InlineCard,
            max_tier: true,
    ...
    log_event(intellihelper_telemetry::events::CreditLimitUpsellShown {
        surface: intellihelper_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
        max_tier: false,
        pay_as_you_go: payg_telemetry,
        unified_billing,
    });
...
fn open_supergrok_upsell(
    ...
    log_event(intellihelper_telemetry::events::SuperGrokUpsellShown {
        source,
        auth_method,
    });
    ...
    let options = vec![
        ...
        QuestionOption {
            ...
            description: "Get the most out of IntelliHelper CLI. Highest usage limits.".into(),
            preview: None,
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<intellihelper_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<intellihelper_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
...
pub(super) fn dispatch_open_supergrok_url(app: &mut AppView) -> Vec<Effect> {
    ...
    let url = app
        ...
        .unwrap_or("https://grok.com/__PROTECT_25__?referrer=intellihelper-build");
    // Funnel attribution: tag CLI-originated SuperGrok upsell clicks
    // with `referrer=intellihelper-build`, matching the OAuth consent flow and
    ...
    let url = crate::app::link_opener::ensure_query_param(url, "referrer", "intellihelper-build");
