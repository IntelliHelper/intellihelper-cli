...
use intellihelper_workspace::session::file_state::RewindPoint;
...
use crate::util::intellihelper_home::intellihelper_home;
...
/// A feedback entry persisted to `~/.intellihelper/sessions/.../feedback.jsonl`.
///
...
mod feedback_tests {
    ...
    fn make_submission(thumbs_up: bool) -> FeedbackSubmission {
        FeedbackSubmission {
            ...
            model_id: Some("intellihelper-3-fast".into()),
            resolved_model_id: Some("intellihelper-4.5".into()),
            ..Default::default()
        }
    }
...
pub enum PersistenceMsg {
    ...
    CurrentModel {
        ...
        /// The active agent definition name (e.g. `"intellihelper-build"`).
...
pub use intellihelper_shared::session::session_dir;
...
pub fn session_exists_for_cwd(session_id: &str, cwd: &str) -> bool {
    let sessions_root = crate::util::intellihelper_home::intellihelper_home().join("sessions");
...
fn session_exists_for_cwd_in_root(session_id: &str, cwd: &str, sessions_root: &Path) -> bool {
    let encoded = crate::util::intellihelper_home::encode_cwd_dirname(cwd);
...
/// `intellihelper -r <remote_id>` in the same cwd, this function returns the already-restored
...
fn find_local_child_for_remote_in_root(
    ...
    // `intellihelper -r <remote_id>` before this fix was deployed.
...
/// Searches across ALL cwd directories under `~/.intellihelper/sessions/`.
///
/// Use `session_exists_for_cwd` instead when the target cwd is known
/// (e.g., the `-r` resume path) to avoid false-positive matches.
/// Find a session by ID across **all** CWD directories under `~/.intellihelper/sessions/`.
///
...
pub(crate) fn resolve_local_session_any_cwd_result(session_id: &str) -> io::Result<Option<String>> {
    resolve_local_session_any_cwd_in_root(session_id, &intellihelper_home().join("sessions"))
...
fn resolve_local_session_any_cwd_in_root(
    ...
    Ok(session_path
        .parent()
        .and_then(crate::util::intellihelper_home::decode_cwd_from_dirname))
}
...
pub(crate) fn find_persisted_session_dir_by_id_result(
    ...
    find_persisted_session_dir_by_id_in_root_result(session_id, &intellihelper_home().join("sessions"))
}
...
pub(crate) fn find_any_session_dir_by_id_result(session_id: &str) -> io::Result<Option<PathBuf>> {
    storage_view(&intellihelper_home().join("sessions"))
...
pub(crate) fn find_summary_by_session_id(session_id: &str) -> Option<Summary> {
    find_summary_by_session_id_in_root(session_id, &intellihelper_home().join("sessions"))
}
...
pub fn local_summaries_for_cwd_sync(cwd: &str) -> io::Result<Vec<Summary>> {
    local_summaries_for_cwd_sync_in_root(cwd, &intellihelper_home().join("sessions"))
}
...
pub fn resumed_session_sandbox_profile(
    ...
    resumed_session_sandbox_profile_in_root(session_id, cwd, &intellihelper_home().join("sessions"))
}
...
pub struct Summary {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub request_id: Option<String>,
    /// Absolute path to the `.intellihelper` directory, used by reconstruction.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub intellihelper_home: Option<String>,
...
/// Current `intellihelper_home` as a UTF-8 string, or `None` if the path isn't valid UTF-8.
pub(crate) fn intellihelper_home_string() -> Option<String> {
...
impl Summary {
    pub(crate) fn new(info: &Info, model_id: acp::ModelId) -> std::io::Result<Self> {
        let git_metadata =
            intellihelper_workspace::session::git::resolve_persisted_session_git_metadata_sync(
        ...
        Ok(Self {
            ...
            request_id: None,
            intellihelper_home: intellihelper_home_string(),
            last_active_at: None,
...
/// Collect MCP server stderr logs from `~/.intellihelper/logs/mcp/` for inclusion in the session archive.
fn collect_mcp_stderr_logs(files: &mut Vec<CopiedSessionFile>) {
    let mcp_log_dir = intellihelper_config::intellihelper_home().join("logs").join("mcp");
        ...
        StorageMode::Writeback => {
            let auth_manager = auth_manager.ok_or_else(|| {
                io::Error::new(
                    ...
                    "Writeback storage mode requires authentication. Run 'intellihelper login' first.",
                )
            })?;
...
/// worktree containing this session's cwd so `intellihelper worktree gc` expires by
...
pub(crate) async fn new(
    ...
    let root_dir = intellihelper_home();
...
pub async fn list_summaries(cwd: Option<&str>) -> io::Result<Vec<Summary>> {
    let root_dir = crate::util::intellihelper_home::intellihelper_home();
...
/// Walk `~/.intellihelper/sessions/` and delete files with mtime older than `ttl_days`.
...
pub(crate) fn cleanup_stale_sessions(skip_session_dir: Option<&Path>) {
    CLEANUP_SESSIONS_ONCE.call_once(|| {
        ...
        tracing::info!(
            target: "intellihelper_shell::session::persistence",
        ...
        if metadata.file_type().is_dir() && !metadata.file_type().is_symlink() {
            ...
            let lease = if matches!(level, CleanupLevel::Cwd) {
                ...
                let storage = crate::session::storage::relocation::RelocationStorage::new(
                    intellihelper_home.to_path_buf(),
                );
...
mod session_exists_tests {
    ...
    fn returns_false_when_root_does_not_exist() {
        let root = std::path::PathBuf::from("/nonexistent/intellihelper/sessions");
...
mod find_summary_by_session_id_tests {
    ...
    fn minimal_summary(head_commit: &str, head_branch: &str) -> String {
        serde_json::json!({
            ...
            "updated_at": "2026-01-01T00:00:00Z",
            "num_messages": 0,
            "current_model_id": "intellihelper-3",
...
mod resumed_sandbox_profile_tests {
    ...
    fn most_recent_cwd_skips_corrupt_summary() {
        ...
        let corrupt_dir = root
            .join(crate::util::intellihelper_home::encode_cwd_dirname(cwd))
            .join("corrupt");
...
mod session_exists_for_cwd_tests {
    ...
    fn session_under_different_cwd_is_not_considered_present() {
        ...
        let encoded_a = crate::util::intellihelper_home::encode_cwd_dirname("/project/alpha");
    ...
    fn resolve_local_session_any_cwd_skips_stub_and_finds_real() {
        ...
        let encoded_a = crate::util::intellihelper_home::encode_cwd_dirname(cwd_a);
        let dir_a = root.join(&encoded_a).join(session_id);
        fs::create_dir_all(&dir_a).unwrap();
        fs::write(dir_a.join("summary.json"), b"{}").unwrap();
        ...
        let encoded_b = crate::util::intellihelper_home::encode_cwd_dirname(cwd_b);
...
mod find_local_child_tests {
    ...
    fn returns_none_when_no_child_exists() {
        let tmp = TempDir::new().unwrap();
        let root = tmp.path().join("sessions");
        let encoded = crate::util::intellihelper_home::encode_cwd_dirname("/work");
    ...
    /// Regression: a second `intellihelper -r <remote_id>` must return the existing child
...
mod resolve_local_session_tests {
    ...
    // setting up the on-disk structures under a fake intellihelper home.
    ...
    fn exact_match_returns_original_id() {
        ...
        // (We can't call resolve_local_session directly because it uses intellihelper_home(),
    ...
    fn no_match_returns_none() {
        ...
        fs::create_dir_all(root.join(crate::util::intellihelper_home::encode_cwd_dirname(cwd))).unwrap();
...
mod repo_wide_resolution_tests {
    ...
    fn skips_images_only_stub_and_resolves_real_sibling() {
        ...
        let encoded = crate::util::intellihelper_home::encode_cwd_dirname(exact_cwd);
