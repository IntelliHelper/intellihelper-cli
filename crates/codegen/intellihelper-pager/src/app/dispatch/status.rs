...
fn log_coding_data_consent_selected(
    source: intellihelper_telemetry::events::CodingDataConsentSource,
    opted_in: bool,
    previous_opted_in: bool,
) {
    use intellihelper_telemetry::events::{CodingDataConsentChoice, CodingDataConsentSelected};
    intellihelper_telemetry::session_ctx::log_event(CodingDataConsentSelected {
        source,
...
/// (persists via ACP ext-request, NOT `~/.intellihelper/config.toml`).
...
/// `/token-usage` — durable day/week/month token report (session-independent).
pub(super) fn dispatch_show_token_usage(app: &mut AppView) -> Vec<Effect> {
    let report = intellihelper_shell::token_usage_store::load_report();
    let text = intellihelper_shell::token_usage_store::format_report(&report);
    match app.active_view {
        ActiveView::Agent(id) => {
        ...
        ActiveView::Welcome => {
            // No scrollback yet — open as a doc viewer so the report is readable.
...
/// Optimize the active composer draft in place via async LLM (does not send).
pub(super) fn dispatch_optimize_input(app: &mut AppView) -> Vec<Effect> {
    match app.active_view {
        ActiveView::Agent(id) => {
            let Some(agent) = app.agents.get_mut(&id) else {
                return vec![];
            ...
            if draft.trim().is_empty() {
                agent.show_toast("Nothing to optimize — type a draft first.");
                return vec![];
            }
            let generation = agent.optimize_generation.unwrap_or(0).saturating_add(1);
            agent.optimize_generation = Some(generation);
            agent.optimize_undo = None;
            agent.show_toast("Optimizing…");
        ...
        ActiveView::Welcome => {
            ...
            if draft.trim().is_empty() {
                app.show_toast("Nothing to optimize — type a draft first.");
                return vec![];
            }
            let generation = app
                .welcome_optimize_generation
                .unwrap_or(0)
                .saturating_add(1);
            app.welcome_optimize_generation = Some(generation);
            app.welcome_optimize_undo = None;
            app.show_toast("Optimizing…");
            let cwd = std::env::current_dir().ok();
            vec![Effect::OptimizePrompt {
                agent_id: None,
                session_id: None,
...
/// Apply a finished Optimize LLM rewrite (or surface an error toast).
pub(super) fn handle_optimize_prompt_done(
    app: &mut AppView,
    ...
    use crate::optimize_input::sanitize_model_output;
    ...
    match agent_id {
        Some(id) => {
            ...
            if agent.optimize_generation != Some(generation) {
                return vec![];
            }
            agent.optimize_generation = None;
            // Stale if the user edited the composer while we waited.
            if agent.prompt.text() != original_draft {
                agent.show_toast("Optimize discarded — draft changed");
                return vec![];
            }
            match result {
                Ok(raw) => match sanitize_model_output(&raw) {
                    Ok(optimized) if optimized != original_draft => {
                        agent.optimize_undo = Some(original_draft);
                        agent.prompt.set_text(&optimized);
                        agent.show_toast("Optimized · Undo optimize to restore");
                    }
                    Ok(_) => agent.show_toast("Draft already optimized"),
                    Err(msg) => agent.show_toast(msg),
                },
                Err(err) => agent.show_toast(&format!("Optimize failed: {err}")),
            }
        }
        None => {
            if app.welcome_optimize_generation != Some(generation) {
                return vec![];
            }
            app.welcome_optimize_generation = None;
            if app.welcome_prompt.text() != original_draft {
                app.show_toast("Optimize discarded — draft changed");
                return vec![];
            }
            match result {
                Ok(raw) => match sanitize_model_output(&raw) {
                    Ok(optimized) if optimized != original_draft => {
                        app.welcome_optimize_undo = Some(original_draft);
                        app.welcome_prompt.set_text(&optimized);
                        app.show_toast("Optimized · Undo optimize to restore");
                    }
                    Ok(_) => app.show_toast("Draft already optimized"),
                    Err(msg) => app.show_toast(msg),
                },
                Err(err) => app.show_toast(&format!("Optimize failed: {err}")),
            }
        }
    }
    vec![]
...
/// Restore the draft from before the last successful Optimize.
pub(super) fn dispatch_undo_optimize_input(app: &mut AppView) -> Vec<Effect> {
    match app.active_view {
        ActiveView::Agent(id) => {
            if let Some(agent) = app.agents.get_mut(&id) {
                if let Some(prev) = agent.optimize_undo.take() {
                    agent.prompt.set_text(&prev);
                    agent.show_toast("Restored draft");
                } else {
                    agent.show_toast("Nothing to undo");
                }
            }
        }
        ActiveView::Welcome => {
            if let Some(prev) = app.welcome_optimize_undo.take() {
                app.welcome_prompt.set_text(&prev);
                app.show_toast("Restored draft");
            } else {
                app.show_toast("Nothing to undo");
            }
        }
        _ => {}
    }
...
pub(super) fn commit_session_usage_block(
    ...
    if let Some(total) = session_total_tokens {
        agent.session_usage_total_tokens = total;
    }
    agent.scrollback.push_block(RenderBlock::system(text));
...
pub(super) fn append_consumer_billing_surface(app: &mut AppView, agent_id: AgentId) -> Vec<Effect> {
    ...
    // Remote-settings kill switch (`intellihelper_build_usage_redirect_url`): link out
...
pub(in crate::app::dispatch) fn dispatch_privacy_banner_opt_in(app: &mut AppView) -> Vec<Effect> {
    ...
    let effects = set_coding_data_sharing(
        app,
        true,
        intellihelper_telemetry::events::CodingDataConsentSource::PrivacyBanner,
    );
...
pub(super) fn handle_context_info_complete(
    app: &mut AppView,
    agent_id: AgentId,
    info: Box<intellihelper_shell::session::SessionInfoResponse>,
) -> Vec<Effect> {
    if let Some(agent) = app.agents.get_mut(&agent_id) {
