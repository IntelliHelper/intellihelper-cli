...
IntelliHelper CLI runs as a full-screen TUI. It relies on terminal support for color,
...
Run `/doctor` in IntelliHelper to check the current session and see available fixes. If
IntelliHelper cannot start, run `intellihelper doctor` in your shell. Use `intellihelper doctor --json`
...
`intellihelper doctor --json` reports the same color capability when piped. Microphone
...
`/doctor fix tmux-clipboard` or `intellihelper doctor fix dcs-passthrough --yes`.
...
to guess if that directory is unavailable or unsafe. IntelliHelper preserves the file's
...
IntelliHelper deliberately does **not** run `tmux source-file` or change the live tmux
...
IntelliHelper detects these terminal emulators from environment variables:
...
- Inside tmux, variables that identify the outer terminal may not reach IntelliHelper.
...
Inside tmux there are two separate questions: what color IntelliHelper emits, and what
...
IntelliHelper writes through up to three routes, shown in `/doctor` under **Clipboard**:

- **native** — the local operating-system clipboard.
- **tmux** — the tmux paste buffer when IntelliHelper runs inside tmux.
...
terminal focused. Older compositors may require IntelliHelper to remain focused until
the copy message appears. IntelliHelper shows a startup warning when this applies; run
`/doctor` for the detected status and steps.

`INTELLIHELPER_CLIPBOARD_NO_DATA_CONTROL=1` is an advanced fallback that disables the
data-control route. Copies then use command-line clipboard tools.

#### OSC 52 kill switch

IntelliHelper emits OSC 52 on Linux and across tmux, SSH, or displayless containers when
that route is enabled. A terminal that does not implement OSC 52 may display the
encoded payload as text. Set `INTELLIHELPER_CLIPBOARD_NO_OSC52=1` before starting IntelliHelper to
...
A remote IntelliHelper process normally cannot read the local terminal's selection. Use
...
When IntelliHelper cannot identify the outer terminal over SSH, it predicts that OSC 52
...
clipboard. Each copy is still saved to a backup file (`~/.intellihelper/last-copy.txt` by
default; override with `INTELLIHELPER_COPY_FILE`); the toast names that path when delivery
...
through `intellihelper wrap`, for example `intellihelper wrap ssh user@host`. The same command can
wrap container and pod shells. It also restores terminal modes after a dropped
connection.

When an SSH session is not using `intellihelper wrap`, IntelliHelper shows the one-time tip
...
`[ui.contextual_hints]` in `$INTELLIHELPER_HOME/config.toml`. This setting does not hide
the Doctor recommendation.

For repeated SSH use, Doctor offers `intellihelper doctor fix ssh-wrap`. It also shows
...
> **Warning**: `intellihelper wrap` is experimental and may not work in every setup.
...
Zellij and tmux control mode can limit the alternate screen. IntelliHelper normally uses
inline mode in those environments. Run `/doctor` to see the detected condition.
You can configure `[terminal] alt_screen` in `~/.intellihelper/pager.toml`, or run
`intellihelper --no-alt-screen` to confirm inline mode works.

### Zellij keybindings interfere with IntelliHelper

Zellij can intercept Ctrl/Alt keys before they reach IntelliHelper. On Zellij 0.41 or
...
mode, if `Ctrl+G` still does not reach IntelliHelper, open the command palette and select
...
WezTerm ships with the Kitty keyboard protocol disabled. Run `/doctor` in IntelliHelper.
...
keys. IntelliHelper therefore does not negotiate the protocol there, and Shift+Enter can
...
If IntelliHelper stops receiving mouse input, re-enable mouse reporting in the terminal:
...
After about 10 seconds without a transcript, IntelliHelper stops capture and shows
...
to the terminal hosting IntelliHelper. Open **System Settings → Privacy & Security →
...
Run `intellihelper doctor`, or run `/doctor` while voice mode is on. The **Voice** section
shows the microphone IntelliHelper would use. If no input device is available, Doctor
...
problem, set `INTELLIHELPER_VOICE_CAPTURE=inprocess` to use the in-process fallback for
comparison.
