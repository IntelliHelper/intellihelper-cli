...
//!
//! - IntelliHelper chat has a real tokenizer (`TextTokenizer` / `ImageTokenizer`) and
//!   counts whole turns via `IntelliHelperTurn::get_num_tokens`.
//! - intellihelper-build estimates with `bytes / 4`.
//!
...
//! (IntelliHelper chat: `ItemTokenCounter<Arc<IntelliHelperTurn>>`), so exactly one mechanism
//! is in play.
