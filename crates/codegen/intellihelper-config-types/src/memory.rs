//! Memory-system configuration value types, extracted from intellihelper-shell
...
//! `intellihelper-shell` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.intellihelper/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.intellihelper/memory/` are scanned:
