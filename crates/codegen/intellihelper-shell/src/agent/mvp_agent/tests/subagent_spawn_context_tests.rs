...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    use intellihelper_workspace::permission::types::{
    ...
    local
        .run_until(async {
            ...
            let cwd = intellihelper_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) =
                intellihelper_workspace::permission::spawn_permission_manager(
                    sid.clone(),
                    gateway,
                    cwd,
                    intellihelper_workspace::permission::types::ClientType::Generic,
            ...
            for access in [
                intellihelper_workspace::permission::AccessKind::Read(Some(".env".into())),
                intellihelper_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        intellihelper_workspace::permission::Decision::PolicyDeny(_)
                    ),
...
async fn subagent_spawn_context_inherits_parent_configured_cutoff() {
    ...
    let cutoff = intellihelper_sampling_types::ToolOverrides {
        x_search: Some(intellihelper_sampling_types::XSearchOptions {
            date_bound: Some(
                intellihelper_sampling_types::SearchDateBound::new(None, Some("2020-01-01".to_string()))
                    .unwrap(),
            ),
        }),
        web_search: None,
