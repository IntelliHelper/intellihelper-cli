...
/// `~/.intellihelper/config.toml`. Lenient — corrupted fields fall back to
...
pub struct PendingDispatchModel {
    ...
    pub effort: Option<intellihelper_shell::sampling::types::ReasoningEffort>,
    pub display: String,
}
    ...
    fn handle_mouse(&mut self, mouse: &crossterm::event::MouseEvent) -> InputOutcome {
        ...
        if matches!(mouse.kind, MouseEventKind::Down(MouseButton::Left)) {
            ...
            if self.upgrade_cta_hit.contains(mouse.column, mouse.row) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    intellihelper_telemetry::events::AnnouncementCtaSurface::Dashboard,
                ));
            }
...
fn dashboard_action_for_id(
    ...
    match id {
        ...
        | ActionId::OptimizeInput
        | ActionId::UndoOptimizeInput
...
/// Load the full persisted dashboard from `~/.intellihelper/config.toml`.
///
...
/// `~/.intellihelper/config.toml` (`[ui]`, `[hints]`, `[mcpServers]`, …).
///
...
pub fn write_persisted(p: &PersistedDashboard) -> std::io::Result<()> {
    let path = config_path()
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "no intellihelper home"))?;
...
mod tests {
    ...
    fn compact_cwd_strips_home() {
        let p = Path::new("/Users/alice/projects/intellihelper");
        assert_eq!(compact_cwd(p, Some("/Users/alice")), "~/projects/intellihelper");
    }
    ...
    /// per-key serial lock. The `INTELLIHELPER_AGENT_DASHBOARD` key means this
    /// test runs serially with any other test that decorates itself
    /// with `#[serial_test::serial(INTELLIHELPER_AGENT_DASHBOARD)]` — see the
    ...
    fn env_var_force_disables() {
        ...
        // same `INTELLIHELPER_AGENT_DASHBOARD` key reads it concurrently.
