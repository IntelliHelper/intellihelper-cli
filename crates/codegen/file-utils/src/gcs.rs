...
use intellihelper_auth::{AuthCredentialProvider, StaticAuthCredentialProvider};

use crate::storage_client::{Auth401AttributionCallback, StaticIntelliHelperAuth, StorageClient};
...
/// `StaticIntelliHelperAuth` carrying the inline user / deployment keys from
...
fn build_proxy_client_with_fallback(
    ...
    let provider = credentials.unwrap_or_else(|| {
        let mut creds = StaticIntelliHelperAuth::new(Some(user_token.to_owned()));
