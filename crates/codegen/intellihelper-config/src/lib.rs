//! Config file loading for IntelliHelper.
//!
//! Merge order (lowest → highest priority):
//! 1. `/etc/intellihelper/managed_config.toml`
//! 2. `$INTELLIHELPER_HOME/managed_config.toml`
//! 3. `$INTELLIHELPER_HOME/config.toml`
//! 4. `$INTELLIHELPER_HOME/requirements.toml` (cloud cache; Ed25519-signed at rest once a
//!    key is embedded — see [`signed_policy`] — below the OS-protected layers)
//! 5. `/etc/intellihelper/requirements.toml`
//! 6. macOS MDM managed preferences (`ai.x.intellihelper`, admin-forced) — macOS only
//!
...
pub use global_hook_sources::{
    ...
    ensure_intellihelper_hook_slots, existing_ancestor_chain, is_direct_hook_json_name,
...
pub use paths::{
    CLI_BIN_NAME, claude_managed_settings_path, claude_managed_settings_probe_path,
    decode_cwd_from_dirname, default_intellihelper_home, encode_cwd_dirname, ensure_sessions_cwd_dir,
    intellihelper_application, intellihelper_application_in, intellihelper_home, sessions_cwd_dir,
    system_config_dir, user_intellihelper_home,
};
pub use validation::{
