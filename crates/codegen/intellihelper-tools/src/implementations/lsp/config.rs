//! LSP server configuration from `.intellihelper/lsp.json`.
...
/// This is the canonical merge function — both session startup and `intellihelper inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::intellihelper_home::intellihelper_home().join("lsp.json");
    let project_path = cwd.join(".intellihelper").join("lsp.json");
...
/// Load LSP server configs from `~/.intellihelper/lsp.json` and `<cwd>/.intellihelper/lsp.json`.
