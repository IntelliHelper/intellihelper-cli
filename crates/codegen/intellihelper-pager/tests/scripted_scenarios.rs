...
//!
//! These tests exercise the real `intellihelper-pager` binary through a PTY using
...
/// enterprise deploy report: with `INTELLIHELPER_GOAL=1`, `/goal` must
...
/// explicitly off (`INTELLIHELPER_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `INTELLIHELPER_FOLDER_TRUST=1` and a git repo that
...
/// also pins `INTELLIHELPER_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
