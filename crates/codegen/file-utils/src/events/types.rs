...
pub enum Event {
    ...
    /// `TODO_GATE_*` discriminator constant in `intellihelper-shell::session::events`.
    TodoGateFired {
        fires: u32,
        pending: usize,
        in_progress: usize,
    ...
    /// constants in `intellihelper-shell::session::events`.
    ...
    /// in `intellihelper-shell::session::events`.
    ...
    /// discriminator constants in `intellihelper-shell::session::events`.
    ...
    /// `intellihelper-shell::session::events`.
    ...
    /// `intellihelper-shell::session::events`. Fail-open never pauses the goal.
    ...
    /// enumerated by
    /// `intellihelper-shell::session::goal_stop_detector::PATTERN_LABELS`;
...
/// Telemetry mirror of `intellihelper-shell`'s `GoalClassifierVerdict`. Two
/// crates due to the orphan rule; the conversion lives in
/// `intellihelper-shell/src/session/events.rs`.
#[derive(Debug, Clone, Copy, Serialize)]
#[serde(rename_all = "snake_case")]
...
/// Telemetry mirror of `intellihelper-shell`'s `GoalPauseReason`. The two types
...
mod tests {
    ...
    fn goal_strategist_fired_serializes_cadence_field() {
        ...
        let ev = Event::GoalStrategistFired {
            ...
            every: 3,
            model_id: "intellihelper-4".to_string(),
        };
        let v = serde_json::to_value(&ev).unwrap();
        ...
        assert_eq!(v["every"], 3);
        assert_eq!(v["model_id"], "intellihelper-4");
    }
