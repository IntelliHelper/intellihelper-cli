...
mod tests {
    ...
    fn test_auth() -> crate::auth::IntelliHelperAuth {
    ...
    fn status_only_reports_bundled_cache_not_higher_priority_sources() {
        ...
        std::fs::create_dir_all(project_root.join(".intellihelper/personas")).unwrap();
        std::fs::create_dir_all(project_root.join(".intellihelper/roles")).unwrap();
        std::fs::write(
            project_root.join(".intellihelper/personas/researcher.toml"),
            "instructions = \"project persona\"\n",
        )
        .unwrap();
        std::fs::write(
            project_root.join(".intellihelper/roles/reviewer.toml"),
