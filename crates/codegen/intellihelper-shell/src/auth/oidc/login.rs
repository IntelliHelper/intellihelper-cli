...
use super::super::{AuthManager, IntelliHelperAuth};
use super::protocol::{
    OidcError, build_authorize_url, build_intellihelper_auth, discover, enforce_login_principal,
...
fn callback_response(result: &CallbackResult) -> (StatusCode, Html<String>) {
    let (title, message) = match result {
        Ok(_) => (
            "Signed in",
            "You can close this window and return to IntelliHelper CLI.",
        ),
...
pub async fn run_login_flow_with_config(
    ...
    let mut auth = build_intellihelper_auth(tokens, user_info, &oidc.issuer, &oidc.client_id);
...
mod tests {
    ...
    async fn full_login_flow_via_race() {
        ...
        .await
        .unwrap();
        let auth = build_intellihelper_auth(tokens, user_info, &oidc_cfg.issuer, &oidc_cfg.client_id);
