...
pub fn open_url(url: &str) -> bool {
    ...
    if let Ok(path) = std::env::var("INTELLIHELPER_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "INTELLIHELPER_TEST_OPEN_URL_FILE write failed");
            return false;
        }
        return true;
    }
    ...
    intellihelper_tools::util::detach_std_command(&mut command);
    match command.spawn() {
        Ok(_) => true,
        Err(e) => {
...
///
/// Used by the SuperGrok upsell flow to attribute clicks to `referrer=intellihelper-build`,
...
mod tests {
    ...
    fn open_path_command_passes_path_as_a_single_arg() {
        ...
        let path = std::path::Path::new("/tmp/intellihelper session/image 1.jpg");
    ...
    fn ensure_query_param_appends_when_missing() {
        let out = ensure_query_param("https://grok.com/__PROTECT_1__", "referrer", "intellihelper-build");
        assert_eq!(out, "https://grok.com/__PROTECT_2__?referrer=intellihelper-build");
    }

    #[test]
    fn ensure_query_param_preserves_existing_value() {
        let out = ensure_query_param(
            "https://grok.com/__PROTECT_3__?referrer=other",
            "referrer",
            "intellihelper-build",
        );
        assert_eq!(out, "https://grok.com/__PROTECT_4__?referrer=other");
    }

    #[test]
    fn ensure_query_param_keeps_other_query_pairs() {
        let out = ensure_query_param(
            "https://grok.com/__PROTECT_5__?heavy=1",
            "referrer",
            "intellihelper-build",
        );
        assert_eq!(
            out,
            "https://grok.com/__PROTECT_6__?heavy=1&referrer=intellihelper-build"
        );
    }
    ...
    fn ensure_query_param_preserves_fragment() {
        ...
        // routing (`intellihelper.com/#supergrok`); we still want the referrer attached.
        let out = ensure_query_param("https://grok.com/#__PROTECT_8__", "referrer", "intellihelper-build");
        assert_eq!(out, "https://grok.com/?referrer=intellihelper-build#__PROTECT_9__");
    }

    #[test]
    fn ensure_query_param_returns_unchanged_on_parse_failure() {
        let out = ensure_query_param("not a url", "referrer", "intellihelper-build");
        assert_eq!(out, "not a url");
    }

    #[test]
    fn ensure_query_param_url_encodes_value() {
        let out = ensure_query_param("https://grok.com/__PROTECT_10__", "referrer", "intellihelper build");
        assert_eq!(out, "https://grok.com/__PROTECT_11__?referrer=intellihelper+build");
    }
    ...
    fn browser_unavailable_message_includes_full_url() {
        let url = "https://grok.com/__PROTECT_12__?referrer=intellihelper-build";
        assert_eq!(
    ...
    fn browser_unavailable_line_is_url_first_single_line() {
        let url = "https://grok.com/__PROTECT_13__?referrer=intellihelper-build";
