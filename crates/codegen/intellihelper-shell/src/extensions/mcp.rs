...
// rmcp is quarantined in intellihelper-mcp; see that crate's docs.
use intellihelper_mcp::rmcp;
// `wire::MCP_CALL` is the one cross-SDK contract literal; the agent-only siblings
// live in `mcp_methods` below.
use intellihelper_mcp::wire;
...
/// `intellihelper_mcp::wire`), these methods are private to the agent↔client channel and
...
pub(crate) async fn build_mcp_status(
    ...
    tool_bridge: &Arc<intellihelper_tools::bridge::ToolBridge>,
    event_writer: Option<&file_utils::events::EventWriter>,
) -> McpStatusSnapshot {
    ...
    for client in &clients {
        ...
        if let Some(ew) = event_writer {
            ew.emit(file_utils::events::Event::McpHealthCheck {
...
//
// Implements the `McpResourceProvider` trait from intellihelper-tools so that
// `ListMcpResources` / `FetchMcpResource` tools can access MCP
// servers without depending on `intellihelper-mcp` directly.
...
impl intellihelper_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    async fn list_resources(
        &self,
        server: Option<String>,
    ) -> Result<Vec<intellihelper_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(intellihelper_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<intellihelper_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(intellihelper_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
                content: Some(intellihelper_tools::types::resources::McpResourceContent::Text(
                    text,
                )),
            }),
            rmcp::model::ResourceContents::BlobResourceContents {
                ...
                description: None,
                mime_type,
                content: Some(intellihelper_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),
...
mod tests {
    ...
    fn test_mcp_list_response_serialization() {
            ...
            servers: vec![
                McpServerEntry {
                    ...
                    config: McpServerConfig::Http {
                        ...
                        scope_name: Some("IntelliHelper CLI".to_string()),
                    },
                    source_label: None,
                    setup: None,
                    setup_values: None,
        ...
        assert_eq!(json["servers"][0]["scopeName"], "IntelliHelper CLI");
