...
//!
//! `MemoryBackend` is defined in `intellihelper-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `intellihelper-tools`.
/// Mirrors `intellihelper_shell::session::memory_log::TARGET`.
