...
impl AgentView {
    ...
    pub(in crate::app) fn handle_prompt_key(
            ...
            && self.prompt.prompt_suggestion_visible()
        {
            if key!(Tab).matches(key) || key!(Right).matches(key) {
                if self.prompt.accept_prompt_suggestion() {
                    ...
                    intellihelper_telemetry::session_ctx::log_event(
                        intellihelper_telemetry::events::PromptSuggestion {
                            action: intellihelper_telemetry::events::PromptSuggestionAction::Accepted,
                            chars,
                            words,
                        },
                    );
                ...
                && self.prompt.text().is_empty()
            {
                ...
                intellihelper_telemetry::session_ctx::log_event(
                    intellihelper_telemetry::events::PromptSuggestion {
                        action: intellihelper_telemetry::events::PromptSuggestionAction::Dismissed,
                        chars,
                        words,
                    },
                );
        ...
        if let Some(action_id) = registry.lookup(key, When::PromptFocused) {
            match action_id {
                ...
                ActionId::UndoOptimizeInput => {
                    ...
                    return InputOutcome::Action(Action::UndoOptimizeInput);
                }
                other => {
            ...
            match self.prompt.handle_key(key) {
                PromptEvent::Edited => {
                    if undo_tip_accepted {
                        intellihelper_telemetry::session_ctx::log_event(
                            intellihelper_telemetry::events::ContextualTip {
    ...
    /// The pty-only `INTELLIHELPER_ESC_DOUBLE_PRESS_MS` override can exceed this; no
