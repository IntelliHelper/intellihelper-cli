...
- **Using IntelliHelper**
  ...
  - [SSH Passthrough](#ssh-passthrough-intellihelper-ssh) — Apple Terminal clipboard support
- **Configuration**
...
- **Reference**
  - [Introspection (`intellihelper inspect`)](#introspection)
...
- [Building with IntelliHelper](#building-with-intellihelper) — headless API, ACP SDK integration
...
On first launch, IntelliHelper opens your browser to authenticate with intellihelper.com:

```bash
intellihelper
```

Credentials are stored in `~/.intellihelper/auth.json` and persist across sessions. Tokens expire after 7 days; IntelliHelper will prompt you to re-authenticate when needed.
...
export INTELLIHELPER_OIDC_ISSUER="https://acme.okta.com"
export INTELLIHELPER_OIDC_CLIENT_ID="0oa1b2c3d4e5f6g7h8i9"
```

Customers typically also override the API endpoint to point at their own proxy:
```bash
export INTELLIHELPER_CLI_CHAT_PROXY_BASE_URL="https://intellihelper-proxy.acme.com/v1"
```

**3. Run `intellihelper`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.intellihelper/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
IntelliHelper is provider-agnostic — it doesn't know or care how your binary authenticates. It just runs the command, reads a token from stdout, and stores it. Your binary is a black box that handles the entire auth flow.

#### How It Works

```
┌──────────────┐     sh -c     ┌────────────────────────┐
│     IntelliHelper     │──────────────▶│  your auth binary      │
│              │               │                        │
...
1. IntelliHelper runs your command via `sh -c "<command>"`
...
4. **stdout** → captured by IntelliHelper and saved to `~/.intellihelper/auth.json` as the access token
5. exit 0 → success; exit non-zero → IntelliHelper falls through to interactive login
...
|--------|---------------|-------------|
| **stdout** | The token — nothing else | IntelliHelper (parsed and stored in `auth.json`) |
| **stderr** | Login URLs, status messages, errors, progress | The user (displayed in their terminal) |

**Do not print anything to stdout except the token.** No progress messages, no debug output, no "Login successful!" text. IntelliHelper reads stdout verbatim and tries to parse it as a token. Any extra text will break parsing.
...
Use JSON if your tokens expire and you want IntelliHelper to automatically re-run the binary before expiry. The `expires_in` field (seconds until expiry) tells IntelliHelper when to proactively refresh. Without it, IntelliHelper assumes tokens last 30 days.
...
# Print ONLY the token to stdout (IntelliHelper captures this)
...
```bash
# Or via environment variables
export INTELLIHELPER_AUTH_PROVIDER_COMMAND="/usr/local/bin/my-auth-provider"
export INTELLIHELPER_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export INTELLIHELPER_AUTH_TOKEN_TTL=3600               # optional
```

If your binary outputs a bare token string (not JSON with `expires_in`), set `auth_token_ttl` to the token's expected lifetime in seconds. Without it, IntelliHelper cannot detect expiry proactively and will only refresh after a 401.
...
> **Windows:** write the path as a TOML *literal* string (single quotes) so backslashes survive: `auth_provider_command = 'C:\corp\intellihelper-auth.exe'`. Inside a double-quoted TOML string `\t`, `\n`, `\r`, `\b` and `\f` are escape sequences, so `"C:\temp\auth.exe"` parses into a path containing a tab character and the provider fails to start — after which IntelliHelper falls back to browser login as if the setting were ignored.

When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with intellihelper.com". In headless mode (`intellihelper -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
RESP=$(curl -s -X POST https://auth.acme.com/device/code -d "client_id=intellihelper-cli")
...
IntelliHelper runs your binary on two different contracts, and `INTELLIHELPER_AUTH_EXPIRED` is how it tells them apart:

| | `INTELLIHELPER_AUTH_EXPIRED=1` | unset |
|---|---|---|
| **What it is** | A headless refresh over a credential IntelliHelper already holds — near-expiry rotation, or a token the server rejected | A sign-in: `intellihelper login`, the sign-in screen, or the escalation after a headless run couldn't mint |
...
| **Budget** | A few seconds (7s), then IntelliHelper kills the process | 300s — enough for a browser round trip or a device code |
| **So your binary should** | Mint silently, or **exit non-zero**. Never block | Do the full SSO flow, and always mint fresh |

```bash
#!/bin/sh
if [ "$INTELLIHELPER_AUTH_EXPIRED" = "1" ]; then
    # Headless: silent refresh only. If that can't work — the SSO session
    # lapsed, say — exit non-zero rather than block. IntelliHelper then shows the
...
Exiting promptly on `INTELLIHELPER_AUTH_EXPIRED=1` is what makes the handover to the sign-in screen fast: a binary that blocks instead pays the whole refresh timeout on every start with an expired token.

One case stays ambiguous, and only in **leader mode** (`--leader`, or `[cli] use_leader = true`; off by default): with no credential at all, the leader makes one extra attempt in the background just after startup, and that run has the variable unset, like a sign-in. A binary that mints without help (service account, keytab, mounted token) succeeds there and the session heals itself. One that must prompt just sits, up to the 300s sign-in ceiling — nothing waits on it, the sign-in screen is already up, and its stderr goes to `~/.intellihelper/leader.log` rather than to the user.

`INTELLIHELPER_AUTH_EXPIRED` is optional — if your binary ignores it, IntelliHelper still works. It just runs the same flow for both login and refresh, and a flow that prompts will be killed on the headless run before it can finish.

### Automatic Credential Refresh

IntelliHelper supports automatic credential refresh for external auth providers and OIDC. When IntelliHelper detects that your token is expired (either locally based on `expires_in`, or when the server returns a 401), it automatically re-runs your `auth_provider_command` to obtain new credentials before retrying the request.

This is transparent — you don't need to do anything. IntelliHelper handles it in the background during your session.

**When does refresh happen?**

- **Before expiry:** If your binary returned `expires_in` in its JSON output, or you set `auth_token_ttl` in config, IntelliHelper re-runs the binary ~5 minutes before the token expires, so you never see an auth error.
- **On auth error:** If the server rejects a request with 401/403 (e.g. token was revoked or expired), IntelliHelper re-runs the binary and retries the request once.
- **When the refresh run can't mint:** refreshes are headless (no stdin, short timeout), so a binary that needs you to complete an SSO flow cannot succeed there. IntelliHelper then stops treating the stored credential as usable and runs your binary in its interactive mode instead — at startup that is the same sign-in flow a machine with no credentials gets; mid-session the turn fails with a re-auth prompt and `/login` re-runs the binary.
- **OIDC:** If you're using OIDC and have a `refresh_token`, IntelliHelper silently refreshes via your IdP without re-opening the browser.

**Tuning the refresh buffer:**

```bash
# IntelliHelper refreshes tokens 5 minutes before expiry by default.
# Set to 0 to only refresh on 401. Set higher for very short-lived tokens.
export INTELLIHELPER_AUTH_EARLY_INVALIDATION_SECS=300
```

**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `intellihelper login` before starting — IntelliHelper runs your binary automatically on first launch. You _can_ run `intellihelper login` to explicitly hydrate `auth.json` ahead of time if you prefer.
- If both OIDC and `auth_provider_command` are configured: at **login** time, IntelliHelper tries OIDC silent refresh first (if a `refresh_token` exists), then the external binary, then browser-based login. During a **session**, whichever method is configured is used exclusively — if `auth_provider_command` is set it handles all mid-session refreshes; otherwise OIDC silent refresh is used.
...
```bash
intellihelper --debug-file /tmp/intellihelper-auth.log -p "hello"
tail -f /tmp/intellihelper-auth.log
```
...
|-------------|---------------|
| `auth: running external auth provider (headless refresh)` | Your binary is being called with `INTELLIHELPER_AUTH_EXPIRED=1` and a few seconds to work |
| `auth: running external auth provider (interactive login)` | Your binary is being called on the sign-in contract: no `INTELLIHELPER_AUTH_EXPIRED`, stderr shown, 300s |
...
| `auth: external auth provider timed out (likely needs interactive auth), killing` | Binary didn't exit before the 7s headless-refresh timeout and was killed. Exiting non-zero on `INTELLIHELPER_AUTH_EXPIRED=1` avoids this wait entirely |
| `auth: failed to start external auth provider` | The command couldn't be spawned (e.g. binary not found) |

### Per-Model Auth Providers

`auth_provider_command` above replaces IntelliHelper's *session* auth: it mints the token sent to xAI's backend. If you instead want xAI models on normal xAI login while **other models** route through a gateway (LiteLLM, corporate proxy) whose bearer tokens rotate, use a named auth provider — the rotating-token analogue of a per-model `api_key`/`env_key`.
...
- `INTELLIHELPER_AUTH_EXPIRED=1` is set whenever IntelliHelper re-mints over a token still cached in memory, whether from near-expiry rotation or a rejection. The first mint on a cold cache runs without it.

**Token lifecycle:**

- Tokens are cached in memory per provider and shared by every model referencing the provider; nothing is written to disk. The command is a credential helper: it owns durable storage and OAuth2 refresh (keychain, its own dotdir, etc.), exactly like `gcloud auth print-access-token` or a git credential helper. On an in-session re-mint the last credential is handed back via `INTELLIHELPER_AUTH_PROVIDER_ACCESS_TOKEN` (and, when present, `INTELLIHELPER_AUTH_PROVIDER_REFRESH_TOKEN` / `INTELLIHELPER_AUTH_PROVIDER_EXPIRES_AT`), so a refresh-grant command can refresh instead of re-authenticating. The command must be non-interactive and fast; do any interactive login out of band, and IntelliHelper re-runs the command on restart to re-mint.
- IntelliHelper runs the command before a chat turn when the token is missing or within about a minute of expiring, and once more after the server rejects a token. A token rejected within 30 seconds of being fetched is not refetched again, so a broken helper surfaces one clear error instead of looping.
...
**Security:** provider commands execute code, so they are honored only from trusted config layers (`~/.intellihelper/config.toml`, managed config, requirements). A project's `.intellihelper/config.toml` can never define one. Whatever layer sets a model's `base_url` decides where that model's minted token is sent, and `base_url` (unlike the provider table) is not stripped from remote or campaign patches, the same as for a static `env_key`. Keep provider tables and the model `base_url` in layers you trust. The command inherits IntelliHelper's environment (so it sees `PATH`, `HOME`, and any other secrets there), but IntelliHelper's own first-party credentials (`XAI_API_KEY`, `INTELLIHELPER_DEPLOYMENT_KEY`, and related keys) are removed so a BYOK helper never receives them; write helpers that read only what they need, and prefer the `INTELLIHELPER_AUTH_PROVIDER_*` handback for the prior credential.

### Using auth.json for API Access

If you've authenticated with `intellihelper login`, you can use the stored credentials to call the CLI chat proxy directly via curl. The proxy requires specific headers that mirror what the intellihelper CLI sends internally:

```bash
curl -s -N -X POST "https://cli-chat-proxy.grok.com/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $(jq -r '."https://accounts.x.ai/sign-in".key' ~/.intellihelper/auth.json)" \
  -H "X-XAI-Token-Auth: intellihelper-cli" \
  -H "x-intellihelper-model-override: intellihelper-build" \
  -d '{
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.intellihelper/auth.json` (set by `intellihelper login`)                                                                                                                              |
| `X-XAI-Token-Auth: intellihelper-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `x-intellihelper-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `intellihelper-build` which is on the default route, but always safe to include. |
...
| --------------------- | -------------- | ------------ |
| `intellihelper-build`    | ✅ Supported   | ✅ Supported |

> **Note:** `auth.json` tokens expire after 7 days. Run `intellihelper login` to refresh.
...
intellihelper --cwd ~/projects/my-app

# Start with an initial task
intellihelper --prompt "Review this codebase and suggest improvements"

# Add project-specific rules
intellihelper --rules "Always use TypeScript. Prefer functional components."

# Auto-approve mode for trusted tasks
intellihelper --always-approve --prompt "Format all files"
```
...
Run IntelliHelper non-interactively from the command line. Use headless mode when you need to:
...
```bash
intellihelper -p "Your prompt here"
```
...
| `-m, --model <MODEL>`   | Model to use (e.g., `intellihelper-build`)               |
...
intellihelper -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
intellihelper -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
intellihelper -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
intellihelper -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
intellihelper -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
intellihelper -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny everything else dangerous
intellihelper -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Deny edits outside src/
intellihelper -p "Refactor the code" --deny "Edit(/etc/**)"

# Allow all bash commands (auto-approve without prompting)
intellihelper -p "Build the project" --allow "Bash"

# Combine: allow fetching docs sites, deny other URLs
intellihelper --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
intellihelper -p "What does this project do?"

# Use a specific model
intellihelper -p "Optimize this function" -m intellihelper-build

# Get JSON output for parsing
intellihelper -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
intellihelper -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
intellihelper -p "Remember: the secret number is 42" --output-format json
intellihelper -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
intellihelper -p "Continue where we left off" -c

# Run in a different directory
intellihelper -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
intellihelper -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
intellihelper -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
intellihelper -p "Generate a README" > README.md

# Parse JSON output with jq
intellihelper -p "List files" --output-format json | jq -r '.text'

# CI/CD: automated code review
intellihelper -p "Review changes for bugs and security issues." \
  --output-format json --always-approve | jq -r '.text' > review.md

# Pipeline: chain with other tools
git diff --staged | intellihelper -p "Write a concise commit message for these changes"

# Batch: process multiple files
for file in src/*.js; do
  intellihelper -p "Migrate $file from CommonJS to ES modules." --always-approve
done

# Pre-commit hook
intellihelper -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
Run IntelliHelper as an ACP (Agent Client Protocol) agent for integration with IDEs, editors, and custom tooling.
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `intellihelper-build`)                           |
...
```bash
intellihelper agent headless --intellihelper-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`intellihelper ssh`)

Use `intellihelper ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.

```bash
# Basic usage (same args as ssh)
intellihelper ssh user@host

# With SSH flags
intellihelper ssh -t user@host
intellihelper ssh -L 8080:localhost:8080 user@host

# With remote command
intellihelper ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `intellihelper ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.

This runs entirely locally.

---

## Building with IntelliHelper

IntelliHelper can be used as an OpenAI-compatible chat completion backend. Choose between two integration modes:
...
Use headless mode for simple integrations. Spawns `intellihelper -p` and parses JSON output.
...
class IntelliHelperChat:
    ...
    def _build_cmd(self, prompt, model, stream):
        return ["intellihelper", "-p", prompt, "-m", model, "--cwd", self.cwd,
                "--output-format", "streaming-json" if stream else "json", "--always-approve"]

    async def create(self, messages, model="intellihelper-build", stream=False):
...
class IntelliHelperChat {
  ...
  async create(
    ...
    { model = "intellihelper-build", stream = false } = {},
  ) {
    const prompt =
      messages.length === 1
        ? messages[0].content
  ...
  async *streamResponse(prompt: string, model: string) {
    const proc = execa("intellihelper", this.buildArgs(prompt, model, true));
...
// Usage
const client = new IntelliHelperChat(".");
...
# Usage
async def main():
    client = await IntelliHelperACPChat(cwd=".").init()
...
class IntelliHelperACPChat {
  ...
  async init() {
    this.proc = spawn("intellihelper", ["agent", "stdio"]);
...
// Usage
const client = await new IntelliHelperACPChat(".").init();
...
IntelliHelper implements the [Agent Client Protocol (ACP)](https://agentclientprotocol.com), a standard for AI agent communication.
...
IntelliHelper reads configuration from `~/.intellihelper/config.toml`. If the file doesn't exist, IntelliHelper uses sensible defaults. You only need to specify values you want to override.
...
[models]
default = "intellihelper-4.5"                   # model used for new sessions
web_search = "intellihelper-4.5"                # model used by the web_search tool
...
telemetry = false                      # anonymous usage telemetry (env: INTELLIHELPER_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: INTELLIHELPER_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: INTELLIHELPER_RESPECT_GITIGNORE)
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: INTELLIHELPER_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: INTELLIHELPER_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: INTELLIHELPER_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: INTELLIHELPER_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: INTELLIHELPER_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `INTELLIHELPER_TELEMETRY_BUILD_EVENTS_URL`, `INTELLIHELPER_TELEMETRY_BUILD_EVENTS_API_KEY`, and `INTELLIHELPER_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.

### LSP Servers

IntelliHelper can connect to Language Server Protocol (LSP) servers configured in JSON files. LSP integration gives IntelliHelper language-aware code intelligence while it works in your repository.

LSP support is used in two ways:

- **Passive diagnostics** — after edits, IntelliHelper can surface language-server diagnostics such as errors and warnings.
- **The `lsp` tool** — IntelliHelper can actively query the language server for `goToDefinition`, `findReferences`, `hover`, `goToImplementation`, `documentSymbol`, and `workspaceSymbol`.

Reference: [Language Server Protocol](https://microsoft.github.io/language-server-protocol/)

#### Config locations

IntelliHelper looks for server definitions in:

- project config: `<repo>/.intellihelper/lsp.json`
- user config: `~/.intellihelper/lsp.json`
...
- LSP tools are enabled (`INTELLIHELPER_LSP_TOOLS=1` or `[features] lsp_tools = true`)
- the merged LSP configuration is non-empty

Enable the tool for one run:

```bash
INTELLIHELPER_LSP_TOOLS=1 intellihelper
```
...
If LSP tools are enabled but no usable server config is found, IntelliHelper emits a non-fatal warning in logs and continues without the `lsp` tool. If config exists but every server fails to start, the tool may still be present and will fail on first use with a startup error.
...
IntelliHelper does not bundle language server binaries. You must install the server yourself and make sure the configured `command` is runnable on your machine.
...
- Passive diagnostics do **not** require `INTELLIHELPER_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
[models]
default = "company-intellihelper"

[model.company-intellihelper]
model = "intellihelper-build"
base_url = "https://intellihelper-proxy.acme.com/"
name = "IntelliHelper CLI Latest (Proxy)"
...
With this config, `intellihelper` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.

---

## AGENTS.md

Add project-specific instructions by creating an agent rules file (e.g., `AGENTS.md`). IntelliHelper reads these files and appends their contents to the system prompt.

IntelliHelper scans for agent rules in this order:

1. `~/.intellihelper/` (global rules)
...
Within each directory, IntelliHelper checks for these filenames:
...
Skills are reusable prompt packages that extend IntelliHelper with specialized workflows, domain knowledge, and tool integrations. Use them to encode repeatable procedures that would otherwise require re-explaining each session.

### Skill Locations

IntelliHelper discovers skills from these directories (in priority order):

| Location                    | Scope | Priority |
| --------------------------- | ----- | -------- |
| `./.intellihelper/skills/`           | Local | Highest  |
| `<repo_root>/.intellihelper/skills/` | Repo  | Medium   |
| `~/.intellihelper/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.intellihelper/skills/`) are outside the repo and never filtered.
...
| `description` | What the skill does and when to use it—this is how IntelliHelper decides to invoke it |
...
Users can reference skills as `/skill-name` (e.g., `/commit`). When you see this pattern, IntelliHelper invokes the corresponding skill.

> **Tip:** The `description` field is critical — it determines when IntelliHelper automatically invokes the skill. Be specific about trigger phrases and use cases.
...
IntelliHelper discovers agent definitions from `.intellihelper/agents/` (project), `~/.intellihelper/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `INTELLIHELPER_AGENT` env var
4. Default `intellihelper-build` agent
...
```bash
intellihelper --agent-profile ./my-agent.md
# or
export INTELLIHELPER_AGENT="my-custom-agent"
```
...
```bash
export INTELLIHELPER_SUBAGENTS=0              # Environment variable
```
...
[subagents.models]
explore = "intellihelper-build"              # route explore to a lighter model
```
...
model = "intellihelper-build"
prompt_file = ".intellihelper/prompts/researcher.md"

[subagents.personas.concise]
instructions = "Be extremely concise. No filler words."
# instructions_file = ".intellihelper/personas/concise.md"  # or load from file
```

Both are also discovered from `.intellihelper/roles/*.toml` and `.intellihelper/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).

---

## Plugins

Plugins extend IntelliHelper with additional tools, skills, and MCP servers from external packages.

### Plugin Locations

| Location                    | Scope   |
| --------------------------- | ------- |
| `.intellihelper/plugins/`            | Project |
| `~/.intellihelper/plugins/`          | User    |
...
IntelliHelper discovers hooks from `.intellihelper/hooks/` in the project directory. Manage them with:
...
The `context_window` parameter is used to calculate when auto-compact should trigger. If not specified, IntelliHelper falls back to built-in defaults for known models.
...
[model.intellihelper-4.20-0309-reasoning]
temperature = 0.5
api_key = "sk-custom"
```

**How it works:** When you override a built-in model, IntelliHelper starts with the default configuration (including the correct `base_url` from your `[endpoints]` setting), then applies only the fields you specify. Unspecified fields inherit from the default.
...
**Web search model:** Set `[models] web_search`, `INTELLIHELPER_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
> `intellihelper models` output). You also need a `[model.*]` entry so IntelliHelper knows
...
>
> [model.my-custom-model]              # 2. tell IntelliHelper how to reach it
...
intellihelper -p "Hello" -m my-model
...
Point IntelliHelper at a custom OpenAI-compatible `/v1/models` endpoint instead of the default cli-chat-proxy. Useful when models are served behind a corporate gateway or self-hosted inference stack.

**Environment variables:**

| Variable | Required | Description |
|----------|----------|-------------|
| `INTELLIHELPER_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `XAI_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `INTELLIHELPER_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |

**Setup:**

```bash
export INTELLIHELPER_MODELS_BASE_URL="https://api.acme.com/v1"
export XAI_API_KEY="xai-..."
intellihelper
```

IntelliHelper fetches the model list from `{INTELLIHELPER_MODELS_BASE_URL}/models` on startup and sends inference requests to `INTELLIHELPER_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `INTELLIHELPER_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.intellihelper/config.toml`:
...
**Auth behavior:** When `models_base_url` is set, IntelliHelper uses API key auth (`Authorization: Bearer`) instead of session auth. `intellihelper login` is not required — only the API key.

---

## MCP Servers

Extend IntelliHelper's capabilities with [Model Context Protocol](https://modelcontextprotocol.io) servers.

### Configuration

MCP servers are configured in `~/.intellihelper/config.toml`:
...
MCP servers can also be configured per-project in `.intellihelper/config.toml`. IntelliHelper walks from the current directory up to the git repo root, loading `.intellihelper/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.intellihelper/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.intellihelper/config.toml` | This repository   | ↑        |
| `<cwd>/.intellihelper/config.toml`       | Current directory | Highest  |
...
**Example:** commit a `.intellihelper/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.intellihelper/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.intellihelper/config.toml`. Other config sections (models, etc.) are only read from `~/.intellihelper/config.toml`.
...
> **Experimental:** requires `--experimental-memory` (or `INTELLIHELPER_MEMORY=1` / `[memory] enabled = true` in config).

Cross-session memory lets IntelliHelper remember facts, decisions, code patterns, and debugging workflows across separate sessions in the same project.

### How it works

Memory is stored as Markdown files under `~/.intellihelper/memory/`:
- **Global** (`~/.intellihelper/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.intellihelper/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.intellihelper/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries
...
```bash
# Per-session flag
intellihelper --experimental-memory

# Environment variable (persists for the shell session)
export INTELLIHELPER_MEMORY=1
intellihelper
...
At the end of each session, IntelliHelper saves a **structured metadata summary** to the daily session log:
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.intellihelper/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
IntelliHelper searches memory automatically on the first turn of each session and after compaction. The first-turn injection can be disabled or given its own score threshold under `[memory.initial_injection]`. You can also invoke `memory_search` and `memory_get` directly via the model prompt:
...
intellihelper memory edit

# Open global MEMORY.md
intellihelper memory edit --global

# Show memory statistics: file count, chunk count, and index size
intellihelper memory stats
```

### Configuration reference

Key options under `[memory]` in `~/.intellihelper/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.intellihelper/memory/` for external edits and reindex on search |
...
When first-turn memory injection runs, IntelliHelper emits the `intellihelper-shell-memory_injection`
...
IntelliHelper can restrict what the agent process and its spawned commands can access on
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.intellihelper/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.intellihelper/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.intellihelper/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.intellihelper/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.intellihelper/sandbox.toml` (global) or `.intellihelper/sandbox.toml` (per-project):
...
```bash
intellihelper --sandbox devbox
```

### How It Works

The sandbox is applied to the **entire intellihelper process** at startup using kernel
...
- **Platform support**: Sandbox enforcement uses Landlock on Linux (kernel ≥ 5.13)
  ...
  kernel, missing entitlements), IntelliHelper logs a warning and continues without
  enforcement.
...
Sandbox events (profile applied, violations) are logged to `~/.intellihelper/sandbox-events.jsonl`
...
Use `intellihelper inspect` to see everything IntelliHelper discovers in the current directory:

```bash
intellihelper inspect          # human-readable output
intellihelper inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.intellihelper/skills/`, `~/.intellihelper/skills/`, plugins, and config paths
...
IntelliHelper automatically discovers configuration from Claude Code directories alongside native `.intellihelper/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How IntelliHelper uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.intellihelper/skills/`) |
...
Claude Code plugins can provide skills (`skills/`), commands (`commands/`), agents (`agents/`), hooks (`hooks/hooks.json`), MCP servers (`.mcp.json`), and LSP servers (`.lsp.json`). All component types are discovered and used by IntelliHelper at runtime.

---

## Built-in Tools

IntelliHelper includes these tools by default:
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `INTELLIHELPER_WEB_FETCH=1`. 
...
IntelliHelper automatically persists conversations to disk. This works across all modes: TUI, headless, and agent stdio.

### Storage Layout

Sessions are stored under `~/.intellihelper/sessions/`, organized by URL-encoded working directory:

```
~/.intellihelper/sessions/<encoded-cwd>/<session-id>/
...
intellihelper -p "Hello"

# Create or resume a named session
intellihelper -p "Remember: X=42" -s my-session
intellihelper -p "What is X?" -s my-session

# Resume existing session (errors if not found)
intellihelper -p "Continue" -r my-session

# Continue most recent session in current directory
intellihelper -p "What were we doing?" -c
```

Session ID is returned in JSON output:

```bash
intellihelper -p "Hello" --output-format json | jq -r '.sessionId'
```
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.intellihelper/config.toml` | Configuration file                                  |
| `~/.intellihelper/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.intellihelper/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.intellihelper/memory/`     | Cross-session memory files and index                |
| `~/.intellihelper/skills/`     | User-scoped skill definitions                       |
| `~/.intellihelper/plugins/`    | User-scoped plugins                                 |
| `~/.intellihelper/agents/`     | User-scoped agent definitions                       |
| `.intellihelper/config.toml`   | Project-scoped config (MCP servers)                 |
| `.intellihelper/skills/`       | Project-scoped skill definitions                    |
| `.intellihelper/plugins/`      | Project-scoped plugins                              |
| `.intellihelper/agents/`       | Project-scoped agent definitions                    |
| `.intellihelper/hooks/`        | Project-scoped hooks                                |
| `.intellihelper/lsp.json`      | LSP server configuration                            |
...
| `INTELLIHELPER_CLI_CHAT_PROXY_BASE_URL`  | Override the cli-chat-proxy URL (default: `https://cli-chat-proxy.grok.com/v1`)                          |
| `INTELLIHELPER_MODELS_BASE_URL`          | Custom base URL for inference. Model list auto-fetched from `{base_url}/models` (see [Custom Models Endpoint](#custom-models-endpoint)) |
| `INTELLIHELPER_MODELS_LIST_URL`          | Override the model list URL if it differs from `{INTELLIHELPER_MODELS_BASE_URL}/models`                                              |
| `INTELLIHELPER_AUTH_PROVIDER_COMMAND`     | External auth binary (alternative to config file). See [External Auth Provider](#external-auth-provider) |
| `INTELLIHELPER_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `INTELLIHELPER_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `INTELLIHELPER_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `INTELLIHELPER_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `INTELLIHELPER_HOME`                     | Override config directory (default: `~/.intellihelper`)                                                           |
| `INTELLIHELPER_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `INTELLIHELPER_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `INTELLIHELPER_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `INTELLIHELPER_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `INTELLIHELPER_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `INTELLIHELPER_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `INTELLIHELPER_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `INTELLIHELPER_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `INTELLIHELPER_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `INTELLIHELPER_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.intellihelper/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `INTELLIHELPER_LOG_FILE` log; the `--debug` firehose ignores it |

---

## Shell Completions

Generate completions for your shell and install them to enable tab completion for `intellihelper` commands and flags.
...
intellihelper completions bash > ~/.local/share/bash-completion/completions/intellihelper
```

Reload your shell or run `source ~/.bashrc`.

Alternative (IntelliHelper-managed location):

```bash
mkdir -p ~/.intellihelper/completions/bash
intellihelper completions bash > ~/.intellihelper/completions/bash/intellihelper.bash
```
...
intellihelper completions zsh > ~/.zsh/completions/_grok
```
...
```bash
mkdir -p ~/.intellihelper/completions/zsh
intellihelper completions zsh > ~/.intellihelper/completions/zsh/_grok
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.intellihelper/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Regenerate completions after upgrading `intellihelper` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `intellihelper --debug` or `INTELLIHELPER_LOG_FILE` instead:

```bash
# Per-session debug log (~/.intellihelper/debug/<sessionId>.txt)
intellihelper --debug

# Log to a custom path
INTELLIHELPER_LOG_FILE=/tmp/intellihelper-debug.log intellihelper

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.intellihelper/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `INTELLIHELPER_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
INTELLIHELPER_LOG_FILE=/tmp/intellihelper-debug.log RUST_LOG="info,intellihelper_shell::auth=debug" intellihelper
```
...
intellihelper --debug-file /tmp/intellihelper-auth.log -p "hello"
grep "auth:" /tmp/intellihelper-auth.log
```
...
ls ~/.intellihelper/sessions/

# Read session metadata
cat ~/.intellihelper/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.intellihelper/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.intellihelper/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
