...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: intellihelper_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    /// Open intellihelper.com in the browser for SuperGrok subscription upsell.
    OpenSupergrokUrl,
    ...
    /// Open intellihelper.com managed connectors, appending session teamId when set.
    ...
    /// also `/debug scroll`; `INTELLIHELPER_SCROLL_DEBUG=1` enables it from startup).
    ToggleScrollDebugHud,
    /// Toggle the release-safe FPS HUD (`/debug fps`).
    ToggleFpsHud,
    /// Toggle the scroll flight recorder at runtime (`/debug log`;
    /// `INTELLIHELPER_SCROLL_LOG=1` enables it from startup).
    ToggleScrollLog,
    ...
    OpenTranscriptPager,
    /// Minimal mode (`intellihelper --minimal`): re-print the most-recently committed
    ...
    OpenExtensionsModal {
        ...
        trigger: intellihelper_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    AnnouncementsOpenCta(intellihelper_telemetry::events::AnnouncementCtaSurface),
    ...
    /// Commit the active theme (canonical name, e.g. `"intellihelpernight"`, `"auto"`).
    SetTheme(String),
    ...
    ShowUsage,
    /// `/token-usage` — day / week / month token spend by model (local store).
    ShowTokenUsage,
    /// Rewrite the active composer draft in place (does not send).
    OptimizeInput,
    /// Restore the draft from before the last successful Optimize input.
    UndoOptimizeInput,
    ...
    /// Open the Agent Dashboard view (`/dashboard`, `Ctrl+\`, `intellihelper dashboard`).
    OpenDashboard,
...
///
/// Both variants write to `~/.intellihelper/config.toml` and route ACP
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: intellihelper_workspace::foreign_sessions::EnabledForeignSessionSources,
        intellihelper_home: std::path::PathBuf,
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.intellihelper/config.toml`.
    ...
    /// Persist a typed setting to `~/.intellihelper/config.toml`. On failure,
    ...
    /// Rewrite composer draft via `x.ai/optimize_prompt` (async LLM).
    OptimizePrompt {
        /// `None` for the welcome composer (session-less path).
        agent_id: Option<AgentId>,
        /// When set, shell uses the session actor (cwd + model).
        session_id: Option<acp::SessionId>,
        draft: String,
        /// Bumped per request; stale results are ignored.
        generation: u64,
        /// Workspace root for session-less / welcome optimize.
    ...
    /// registry (`~/.intellihelper/active_sessions.json`).
    ...
    FetchPromptSuggestion {
        agent_id: AgentId,
        generation: u64,
        /// Suggestion model resolved by the pager (`intellihelper-build-0.1` when the
...
pub enum TaskResult {
    ...
    WorktreeForked {
        ...
        restore_degree: Option<intellihelper_workspace::session::git::RestoreDegree>,
    ...
    SessionListLoaded {
        ...
        scope: intellihelper_shell::session::unified_list::ListScope,
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<intellihelper_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<intellihelper_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<intellihelper_shell::util::changelog::ChangelogEntry>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<intellihelper_tools::implementations::skills::types::SkillInfo>, String>,
    },
    WorkflowsListLoaded {
        agent_id: AgentId,
        session_id: acp::SessionId,
    ...
    SessionUsageComplete {
        agent_id: AgentId,
        session_id: acp::SessionId,
        usage: Box<intellihelper_shell::extensions::notification::PromptUsage>,
    },
    ...
    /// Optimize-input LLM rewrite finished.
    OptimizePromptDone {
        agent_id: Option<AgentId>,
        generation: u64,
        /// Original draft at click time (for undo + stale checks).
    ...
    DeepSearchResults {
        results: Vec<intellihelper_shell::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
    ...
    GateRefreshed {
        settings: Option<intellihelper_shell::util::config::RemoteSettings>,
    },
