...
//! `~/.intellihelper/memory/{project-slug}-{hash8}/` to avoid polluting the user's repo.

use std::path::{Path, PathBuf};

use intellihelper_tools::util::intellihelper_home::intellihelper_home;

/// Scope for a memory write operation.
/// Write-operation scope. Distinct from `intellihelper_agent::config::MemoryScope` (agent memory dir).
...
/// `~/.intellihelper/memory/`. Workspace-scoped files live under a directory
/// named `{project-slug}-{hash8}`, e.g. `~/.intellihelper/memory/xai-a3f7b2c9/`.
#[derive(Debug, Clone)]
pub struct MemoryStorage {
    /// `~/.intellihelper/memory/`
    global_dir: PathBuf,
    /// `~/.intellihelper/memory/{project-slug}-{hash8}/`
...
impl MemoryStorage {
    /// Create a new `MemoryStorage` rooted at `~/.intellihelper/memory/`.
    ///
    ...
    fn new_inner(cwd: &Path, root_override: Option<&Path>, use_workspace_hash: bool) -> Self {
        let global_dir = root_override
            .map(|p| p.to_path_buf())
            .unwrap_or_else(|| intellihelper_home().join("memory"));
    ...
    ///
    /// File path: `~/.intellihelper/memory/{project}-{hash8}/sessions/YYYY-MM-DD-{slug}-{sid8}.md`
    ///
    /// - `date`: e.g. `"2026-02-23"`
    ...
    pub fn ensure_initialized(&self) -> std::io::Result<()> {
        ...
        if !global_file.exists() {
            std::fs::write(
                ...
                "# Global Memory\n\
                 \n\
                 > This file is automatically managed by IntelliHelper's memory system.\n\
