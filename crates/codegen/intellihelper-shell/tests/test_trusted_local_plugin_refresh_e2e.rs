...
//! 4. Smoke-validate session JSON under `$INTELLIHELPER_HOME/sessions/` after exit.
//!
//! Requires a built `intellihelper` binary (`INTELLIHELPER_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p intellihelper-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p intellihelper-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
use intellihelper_agent::plugins::SharedPluginRegistryHandle;
use intellihelper_agent::plugins::discovery::DiscoveryConfig;
use intellihelper_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _intellihelper_guard = EnvVarGuard::set("INTELLIHELPER_HOME", &intellihelper_home);
    ...
    let mut registry = InstallRegistry::empty(intellihelper_home.join("installed-plugins"));
    ...
    // intellihelper_home() caches the first INTELLIHELPER_HOME per process; a separate test
    ...
    assert_eq!(
        plugin.scope,
        intellihelper_agent::plugins::PluginScope::CliOverride
    );
...
#[tokio::test]
#[ignore = "requires pre-built intellihelper binary; run with --ignored"]
#[serial]
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    let home = dunce::canonicalize(home_tmp.path()).unwrap();
    let intellihelper_home = home.join(".intellihelper");
    std::fs::create_dir_all(&intellihelper_home).unwrap();
    ...
    // The spawned binary gets HOME/INTELLIHELPER_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via intellihelper_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _intellihelper_guard = EnvVarGuard::set("INTELLIHELPER_HOME", &intellihelper_home);
    ...
    sandbox
        ...
        .set_env("INTELLIHELPER_HOME", &intellihelper_home);
    ...
    // Smoke: session storage under INTELLIHELPER_HOME/sessions has JSON artifacts.
    let sessions_root = intellihelper_home.join("sessions");
    assert!(
