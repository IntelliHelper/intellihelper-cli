...
//! release:
//!
//! 1. TUI startup: `check_update_background` spawns a detached `intellihelper update`
//!    (the Ctrl+U path now adopts this child instead of spawning a second).
//! 2. Explicit `intellihelper update` (incl. the Ctrl+U fallback when there is no
//!    live child).
...
//!   temp-name fix, every `0.1.x` download shared one `intellihelper-0.1.tmp`
...
use intellihelper_update::auto_update::{ensure_latest_on_disk, install_internal_from_base, run_update};
use intellihelper_update::version::installed_on_disk_version;

/// Assert the active `~/.intellihelper/bin/intellihelper` resolves to the expected versioned
...
fn assert_active_binary(home: &Path, version: &str, platform: &str, expected_content: &[u8]) {
    let link = home.join("bin").join("intellihelper");
    assert!(link.is_symlink(), "intellihelper must be a symlink");
    let resolved = dunce::canonicalize(&link)
        .unwrap_or_else(|e| panic!("active intellihelper symlink does not resolve: {e}"));
    assert_eq!(
        resolved.file_name().unwrap().to_string_lossy(),
        format!("intellihelper-{version}-{platform}"),
        "active intellihelper must be the expected version"
    );
    assert_eq!(
    ...
    let ran_ok = std::process::Command::new(&resolved)
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    assert!(ran_ok, "active intellihelper must pass the smoke-test");
}

/// Lay down a managed-install layout in the test INTELLIHELPER_HOME:
/// `bin/intellihelper -> ../downloads/intellihelper-<version>-<platform>` (what
...
fn fake_managed_install(version: &str) {
    ...
    let name = format!("intellihelper-{version}-{}", host_platform());
...
// ─────────────────────────────────────────────────────────────────────────────
// Convergence: explicit `intellihelper update` (the Ctrl+U fallback path) finds the
...
// installers that actually maintain the managed `~/.intellihelper/bin/intellihelper` symlink
...
async fn disk_probe_rejects_dangling_symlink() {
    ...
    // ~/.intellihelper/downloads cleanup), the probe must report None — otherwise
    ...
    std::fs::remove_file(
        home.join("downloads")
            .join(format!("intellihelper-0.2.7-{platform}")),
    )
    .unwrap();
...
// shared one `intellihelper-0.1.tmp`, so a concurrent racer could atomically rename a
...
async fn concurrent_different_version_installs_do_not_corrupt_each_other() {
    ...
    // Pre-fix, BOTH of these wrote to downloads/intellihelper-0.1.tmp concurrently
    ...
    for version in ["0.1.181", "0.1.182"] {
        let path = home
            .join("downloads")
            .join(format!("intellihelper-{version}-{platform}"));
        assert_eq!(
    ...
    let resolved = dunce::canonicalize(home.join("bin").join("intellihelper")).unwrap();
    assert_eq!(std::fs::read(&resolved).unwrap(), artifact);
    let name = resolved.file_name().unwrap().to_string_lossy().to_string();
    assert!(
        !name.contains(".tmp"),
        "active intellihelper must never be a temp file: {name}"
    );

    // No stray shared temp file left behind (the pre-fix collision name).
    assert!(
        !home.join("downloads").join("intellihelper-0.1.tmp").exists(),
