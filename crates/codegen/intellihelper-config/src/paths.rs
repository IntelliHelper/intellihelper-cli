//! Filesystem locations for intellihelper config files and binaries.

use std::path::PathBuf;
use std::sync::OnceLock;

static INTELLIHELPER_HOME: OnceLock<PathBuf> = OnceLock::new();
...
/// The default user intellihelper directory (`~/.intellihelper`, canonicalized) used when
/// `INTELLIHELPER_HOME` is unset. Exposed so callers (e.g. display helpers) can detect
/// whether [`intellihelper_home()`] is the default without duplicating the computation.
///
...
/// `~/.intellihelper/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// `fast_worktree::db::resolve_intellihelper_home` (deliberately standalone crate).
pub fn default_intellihelper_home() -> PathBuf {
    #[allow(deprecated)]
    let home = std::env::home_dir().unwrap_or_else(|| PathBuf::from("."));
    dunce::canonicalize(&home).unwrap_or(home).join(".intellihelper")
}

/// Per-user config directory: `$INTELLIHELPER_HOME` or `~/.intellihelper`. Created if needed.
pub fn intellihelper_home() -> PathBuf {
    INTELLIHELPER_HOME
        .get_or_init(|| {
            let intellihelper_home = if let Ok(v) = std::env::var("INTELLIHELPER_HOME") {
                PathBuf::from(v)
            } else {
                default_intellihelper_home()
            };
...
/// The user-global intellihelper home, but only when one genuinely resolves: `Some` when
/// `$INTELLIHELPER_HOME` is set or a home directory is found, `None` otherwise. Unlike
/// [`intellihelper_home()`], this never falls back to a cwd-relative `.intellihelper`, so callers
/// that *scan* user-global intellihelper resources (hooks, marketplace sources, ...) don't
/// mistake a project's `.intellihelper` tree for the user-global one when no home resolves.
pub fn user_intellihelper_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("INTELLIHELPER_HOME").is_some() || std::env::home_dir().is_some();
...
/// System-wide config directory: `/etc/intellihelper/` on Unix, `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/intellihelper"))
    } else {
        None
    }
}
...
/// `intellihelper_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn sessions_cwd_dir(cwd: &str) -> PathBuf {
    intellihelper_home().join("sessions").join(encode_cwd_dirname(cwd))
}
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = intellihelper_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_intellihelper_home_has_no_verbatim_prefix() {
        ...
        let home = default_intellihelper_home();
        assert!(!home.to_string_lossy().starts_with(r"\\?\"));
        assert!(home.ends_with(".intellihelper"));
    }

    /// Product rebrand: home resolution uses `INTELLIHELPER_HOME` + `.intellihelper`.
    #[test]
    fn intellihelper_home_env_key_and_path_segment() {
        // Drive the real shipped resolver path helpers.
        let home = default_intellihelper_home();
        assert!(
            home.ends_with(".intellihelper"),
        ...
        let env_key = "INTELLIHELPER_HOME";
        assert!(
            reader
                .lines()
                .any(|l| l.contains("std::env::var") && l.contains(env_key)
                    || l.contains("std::env::var_os") && l.contains(env_key)
                    || l.contains(&format!("\"{env_key}\""))),
            "paths.rs must read {env_key}"
        );
    }
