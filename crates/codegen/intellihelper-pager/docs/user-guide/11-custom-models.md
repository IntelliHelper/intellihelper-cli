...
IntelliHelper connects to custom model endpoints for alternative providers, self-hosted models, and overriding built-in settings. This guide explains how to select models, configure endpoints, and integrate third-party providers.
...
IntelliHelper supports three API backends. Set `api_backend` in your `[model.*]` config to choose which protocol the model uses:
...
When you omit `api_backend`, IntelliHelper uses `chat_completions`.

To send provider-specific authentication or version headers -- for example, Anthropic's `x-api-key` -- use the `extra_headers` field described below. IntelliHelper sends those headers verbatim with every request to the endpoint.

---

## Configuring Custom Models

Add custom model endpoints in `~/.intellihelper/config.toml` under `[model.<name>]` sections:
...
IntelliHelper resolves the API key in this order:
...
3. Your signed-in session token (from `intellihelper login`), for a model with no `api_key`/`env_key` of its own
4. The `XAI_API_KEY` environment variable (global fallback; IntelliHelper also accepts `INTELLIHELPER_CODE_XAI_API_KEY` for backward compatibility)

### Context Window

The `context_window` value tells IntelliHelper when to trigger auto-compaction. When you override a known model, IntelliHelper inherits that model's context window. When you define a new model and omit `context_window`, IntelliHelper defaults to 200,000 tokens, so set it explicitly to match your provider.
...
Some gateways route or version on the query string. `query_params` appends percent-encoded query parameters to every request IntelliHelper makes for a model. For example, a gateway that selects an API version this way:
...
When you override a built-in model, IntelliHelper starts with the default configuration (including the correct `base_url`), then applies only the fields you specify. Unspecified fields inherit from the default.
...
The `messages` backend uses the Anthropic Messages protocol. Anthropic authenticates with an `x-api-key` header rather than `Authorization: Bearer`, so pass your key through `extra_headers`, which IntelliHelper sends verbatim.
...
Point IntelliHelper at a custom OpenAI-compatible `/v1/models` endpoint instead of the default. Use this when your models sit behind a corporate gateway or a self-hosted inference service.

### Environment Variables

| Variable | Required | Description |
|----------|----------|-------------|
| `INTELLIHELPER_MODELS_BASE_URL` | Yes | Base URL for inference. IntelliHelper fetches the model list from `{base_url}/models`. |
| `XAI_API_KEY` | Yes | API key sent as `Authorization: Bearer`. IntelliHelper also accepts `INTELLIHELPER_CODE_XAI_API_KEY`. |
| `INTELLIHELPER_MODELS_LIST_URL` | No | Override the model-list URL when it differs from `{base_url}/models`. |
...
When you use `[endpoints]` with partial model overrides, IntelliHelper inherits the `base_url` from the endpoints config, so you do not need to specify it in each `[model.*]` section.

### Auth Behavior

When you set `models_base_url`, IntelliHelper uses API key auth (`Authorization: Bearer`) instead of session auth. You do not need `intellihelper login` -- the API key is enough.
