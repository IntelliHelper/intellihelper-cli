...
use dashmap::DashMap;
use file_utils::events::{Event, EventWriter, ToolCompletedSource, ToolOutcome};
...
/// `INTELLIHELPER_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`.
...
pub struct ActivityTracker {
    ...
    /// `INTELLIHELPER_WORKSPACE_EVENTS_ENABLED` is off). Consumed by
...
impl ActivityTracker {
    ...
    /// idle-hold cap comes from `INTELLIHELPER_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`
...
/// The durability idle-hold cap from `INTELLIHELPER_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`.
fn durability_idle_hold_max_from_env() -> u64 {
    durability_idle_hold_from_raw(std::env::var("INTELLIHELPER_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS").ok())
}
