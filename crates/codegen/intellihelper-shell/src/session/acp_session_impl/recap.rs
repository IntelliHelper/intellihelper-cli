...
use intellihelper_sampling_types::SamplingError;
...
fn build_side_question_attempt(base: &ConversationRequest) -> ConversationRequest {
    ...
    request.x_intellihelper_req_id = Some(format!("xai-btw-{}", uuid::Uuid::new_v4()));
    request
}
impl SessionActor {
    ...
    pub(super) async fn handle_recap(&self, auto: bool) {
        ...
        let x_intellihelper_conv_id = format!("recap-{}", uuid::Uuid::new_v4());
        let x_intellihelper_req_id = format!("xai-recap-{}", uuid::Uuid::new_v4());
        ...
        let request = self
            .side_call_request(&setup, items, x_intellihelper_conv_id.clone(), x_intellihelper_req_id.clone())
            .await;
        ...
        let response = match setup.client.conversation_collect(request).await {
            ...
            Err(e) => {
                ...
                self.persist_recap_request_artifact(
                    ...
                    strip_reasoning,
                    tag,
                    &x_intellihelper_req_id,
                    &x_intellihelper_conv_id,
                    started_at,
                    None,
                    None,
                    Some(&e.to_string()),
        ...
        // Applies to manual `/recap` too: spinner-less clients (e.g. IntelliHelper
    ...
    fn persist_recap_request_artifact(
        ...
        reminder_tag: &str,
        x_intellihelper_req_id: &str,
        x_intellihelper_conv_id: &str,
        started_at: String,
        summary: Option<&str>,
        ...
        let artifact = RecapRequestFile {
            ...
            x_intellihelper_req_id: x_intellihelper_req_id.to_owned(),
            x_intellihelper_conv_id: x_intellihelper_conv_id.to_owned(),
            strip_reasoning,
    ...
    /// Rewrite a composer draft into natural-language prompt prose (Optimize input).
    /// Tool-free; injects a bounded git/cwd snapshot. Does not touch conversation history.
    pub(super) async fn handle_optimize_prompt(
        &self,
        draft: &str,
    ) -> Result<String, SideQuestionError> {
        ...
        if trimmed.is_empty() {
            return Err(SideQuestionError::EmptyResponse);
        }
        ...
        // Git snapshot is sync and bounded; run off the async worker pool.
        // Pass draft so heavy status/diff is only attached for change-related asks.
        let cwd_owned = cwd.to_path_buf();
        let draft_for_ctx = trimmed.to_string();
        ...
        let items = vec![
            ...
            ConversationItem::user(build_user_message(trimmed, &workspace_context)),
        ];
        ...
        let request = ConversationRequest {
            ...
            x_intellihelper_session_id: Some(self.session_info.id.to_string()),
    ...
    pub(super) async fn handle_ai_suggest(
        ...
        let model = match model_override {
            Some(m) => m.to_owned(),
            None => "intellihelper-build".to_owned(),
        };
        ...
        let result = match sampling_client.api_backend() {
            crate::sampling::ApiBackend::ChatCompletions => {
                ...
                let events =
                    intellihelper_sampler::stream_chat_completions(raw, meta, request_id, idle_timeout);
                intellihelper_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Responses => {
                let (raw, meta, doom_loop) = sampling_client
                    ...
                    .await
                    .ok()?;
                let events = intellihelper_sampler::stream_responses(
                    raw,
                    meta,
                    request_id,
                    idle_timeout,
            ...
            crate::sampling::ApiBackend::Messages => {
                let (raw, meta) = sampling_client
                    ...
                    .await
                    .ok()?;
                let events = intellihelper_sampler::stream_messages(raw, meta, request_id, idle_timeout);
                intellihelper_sampler::collect_response(events).await
            }
        };
    ...
    /// (`INTELLIHELPER_PROMPT_SUGGESTIONS_MODEL`) > `[models] prompt_suggestion`
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// [`prompt_suggest::DEFAULT_SUGGEST_MODEL`] (`intellihelper-build-0.1`). Every
    ...
    /// `intellihelper-build-0.1` for OAuth users) the request is **skipped
    ...
    /// a reasoning model's response, and some models (e.g. `intellihelper-build`)
    ...
    pub(super) async fn handle_suggest_prompt(
        ...
        let request = ConversationRequest {
            ...
            model: Some(model),
            temperature: None,
            x_intellihelper_conv_id: Some(format!("promptsuggest-{}", uuid::Uuid::new_v4())),
            x_intellihelper_req_id: Some(format!("xai-promptsuggest-{}", uuid::Uuid::new_v4())),
            x_intellihelper_session_id: Some(self.session_info.id.to_string()),
            x_intellihelper_agent_id: Some(intellihelper_telemetry::id::agent_id()),
            ..Default::default()
        };
        ...
        tracing::debug!(
            raw_preview = %intellihelper_tools::util::truncate_str(raw.trim(), 60),
...
mod tests {
    ...
    fn side_question_attempts_get_fresh_request_ids() {
        let base = ConversationRequest {
            x_intellihelper_conv_id: Some("btw-test".into()),
            ..Default::default()
        };
        let a = build_side_question_attempt(&base);
        let b = build_side_question_attempt(&base);
        let (a_id, b_id) = (a.x_intellihelper_req_id.unwrap(), b.x_intellihelper_req_id.unwrap());
        ...
        assert_eq!(a.x_intellihelper_conv_id, base.x_intellihelper_conv_id);
    }
}
