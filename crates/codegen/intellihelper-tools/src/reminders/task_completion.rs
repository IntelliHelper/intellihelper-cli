...
use crate::implementations::intellihelper_build::task::types::{
...
crate::register_resource!("intellihelper_build", "TaskWakeSuppressed", TaskWakeSuppressed);
...
pub fn format_monitor_events(
    events: &[crate::implementations::intellihelper_build::monitor::types::MonitorEventNotification],
    task_output_name: Option<&str>,
) -> Option<String> {
    use std::fmt::Write as _;
    ...
    match events {
        ...
        [event] => {
            ...
            let label =
                crate::implementations::intellihelper_build::monitor::event::sanitize_monitor_description(
                    label,
                );
            Some(format!(
        ...
        _ => {
            type Event =
                crate::implementations::intellihelper_build::monitor::types::MonitorEventNotification;
            ...
            for (task_id, group) in &groups {
                let description = group
                    ...
                    .filter(|d| !d.is_empty())
                    .unwrap_or("event");
                let description = crate::implementations::intellihelper_build::monitor::event::sanitize_monitor_description(
                    description,
                );
                let _ = write!(
                    buf,
...
/// scope only when it has no recorded owner (legacy / non-intellihelper-build backends)
...
/// - `intellihelper-shell`'s `SessionActor` to sweep matching synthetic
    ...
    async fn collect_reminders(
        ...
        if let Some(terminal) = terminal {
            ...
            let goal_loop_active = res
                .get::<crate::implementations::intellihelper_build::task::types::GoalLoopActive>()
                .is_some_and(|g| g.0);
            let surface_reminders = !goal_loop_active
                && res
                    .get::<crate::types::resources::Params<
                        crate::implementations::intellihelper_build::bash::BashParams,
                    >>()
...
mod tests {
    ...
    fn shared_with_reminders_disabled(tasks: Vec<TaskSnapshot>) -> SharedResources {
        ...
        res.insert(Terminal(backend));
        res.register_state::<ReportedTaskCompletions>();
        let params = crate::implementations::intellihelper_build::bash::BashParams {
    ...
    async fn completions_suppressed_when_goal_loop_active() {
        ...
        res.insert(Terminal(backend));
        res.register_state::<ReportedTaskCompletions>();
        res.insert(crate::implementations::intellihelper_build::task::types::GoalLoopActive(true));
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel();
        res.insert(SubagentEventSender(tx));
        tokio::spawn(async move {
        ...
        shared
            .lock()
            .await
            .insert(crate::implementations::intellihelper_build::task::types::GoalLoopActive(false));
        let second = reminder.collect_reminders(shared, &output).await;
        assert!(
            second.is_empty(),
    ...
    async fn reminder_pipeline_ignores_monitor_event_buffer() {
        use crate::implementations::intellihelper_build::monitor::types::{
    ...
    fn format_monitor_events_single_vs_batched() {
        use crate::implementations::intellihelper_build::monitor::types::MonitorEventNotification;
        ...
        let bare = crate::implementations::intellihelper_build::monitor::types::MonitorEventNotification {
            task_id: "task-9".into(),
    ...
    fn wrap_monitor_event_round_trips_through_split() {
        use crate::implementations::intellihelper_build::monitor::event::wrap_monitor_event;
