...
const XAI_IMAGINE_MODEL: &str = "intellihelper-imagine-image-quality";
// Some Imagine models (e.g. `intellihelper-imagine-image`, selectable via `model_override`)
...
pub use intellihelper_tools_api::slash_commands::{
...
pub(crate) const TIER_RESTRICTED_UPSELL: &str = "Image generation is a SuperGrok feature and isn't available on the free or X Basic tier. Let the user know they can unlock image and video generation by upgrading to SuperGrok: https://grok.com/__PROTECT_8__?referrer=intellihelper-build. Do not retry this tool.";
...
impl ImageGenClient {
    pub fn new(
        ...
        let http = intellihelper_extra_ca::with_extra_root_certificates(
...
pub const SESSION_ID_HEADER: &str = "x-intellihelper-session-id";
...
mod tests {
    ...
    fn per_tool_gates_are_independent() {
        let cfg = ImageGenConfig::Enabled {
            ...
            image_edit_enabled: true,
            model_override: Some("intellihelper-imagine-image".into()),
        ...
        assert_eq!(cfg.model_override(), Some("intellihelper-imagine-image"));
    ...
    fn client_selects_model_from_override() {
        ...
        assert_eq!(
            ImageGenClient::new(&mk(Some("intellihelper-imagine-image")), None)
                .unwrap()
                .model,
            "intellihelper-imagine-image"
        );
    }
    ...
    fn client_selects_edit_model_from_override() {
        ...
        let client = ImageGenClient::new(&mk(Some("intellihelper-imagine-image-v2")), None).unwrap();
        assert_eq!(client.edit_model(), "intellihelper-imagine-image-v2");
    ...
    async fn tier_restricted_short_circuits_with_upsell() {
        ...
        match result {
            ToolOutput::Text(t) => {
                ...
                assert!(t.text.contains("supergrok?referrer=intellihelper-build"));
            }
