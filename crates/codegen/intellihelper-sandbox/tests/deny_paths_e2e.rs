//! E2E path-deny and IntelliHelper hook write-deny (subprocess; arm64-tagged).
...
const INTELLIHELPER_HOME_ENV: &str = "SANDBOX_E2E_INTELLIHELPER_HOME";
...
fn apply_fixture_env(cmd: &mut Command, home: &Path, intellihelper_home: &Path, workspace: &Path) {
    cmd.env(WORKSPACE_ENV, workspace.as_os_str())
        .env(HOME_ENV, home.as_os_str())
        .env(INTELLIHELPER_HOME_ENV, intellihelper_home.as_os_str())
        .env("HOME", home.as_os_str())
        .env("INTELLIHELPER_HOME", intellihelper_home.as_os_str());
}
...
fn run_scenario(
    ...
    apply_fixture_env(&mut cmd, home, intellihelper_home, workspace);
    let output = cmd
...
fn skip_if_enforcement_unavailable() -> bool {
    ...
    let support = intellihelper_sandbox::SandboxManager::support_info();
...
fn profile_from_env() -> intellihelper_sandbox::ProfileName {
    intellihelper_sandbox::ProfileName::Custom(std::env::var(PROFILE_ENV).expect(PROFILE_ENV))
}
...
fn subprocess_entry() {
    ...
    // Isolate HOME/INTELLIHELPER_HOME before any config OnceLock init.
    let home = PathBuf::from(std::env::var(HOME_ENV).expect(HOME_ENV));
    let intellihelper_home = PathBuf::from(std::env::var(INTELLIHELPER_HOME_ENV).expect(INTELLIHELPER_HOME_ENV));
    ...
    match scenario.as_str() {
        ...
        "hook_write_deny_marker_spoof" => subprocess_hook_write_deny_marker_spoof(&intellihelper_home),
        other => {
...
fn subprocess_profile_and_bwrap_reexec(profile: &intellihelper_sandbox::ProfileName, workspace: &Path) {
    #[cfg(target_os = "linux")]
    {
        if !intellihelper_sandbox::is_inside_bwrap() {
            ...
            match intellihelper_sandbox::bwrap_reexec_for_profile(profile, workspace) {
                Some(mut cmd) => {
...
fn subprocess_block_deny(workspace: &Path) {
    ...
    let mut sandbox = intellihelper_sandbox::SandboxManager::new(profile, workspace);
...
fn subprocess_hook_write_deny_marker_spoof(_intellihelper_home: &Path) {
    #[cfg(not(target_os = "linux"))]
    {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        unsafe {
            std::env::set_var("__INTELLIHELPER_INSIDE_BWRAP", "1");
        }
        match intellihelper_sandbox::verify_hook_write_deny_enforced() {
            Ok(()) => {
...
/// Workspace-profile IntelliHelper-owned hook write-deny probes (existing sources + first-run).
fn subprocess_hook_write_deny(workspace: &Path, first_run: bool) {
    let home = PathBuf::from(std::env::var(INTELLIHELPER_HOME_ENV).expect(INTELLIHELPER_HOME_ENV));

    let profile = intellihelper_sandbox::ProfileName::Workspace;
    subprocess_profile_and_bwrap_reexec(&profile, workspace);
    ...
    if first_run {
        ...
        eprintln!("OK: first-run IntelliHelper hook slots denied");
    } else {
    ...
    // Parent grants remain creatable: IntelliHelper runtime sibling, workspace, temp.
    assert_write_ok(
        "intellihelper runtime sibling",
...
/// Create isolated HOME + INTELLIHELPER_HOME fixture dirs for a scenario.
fn fixture_homes(
    tag: &str,
) -> (
    PathBuf,
    ...
    let intellihelper = unique_temp_dir(&format!("{tag}-intellihelper"));
    let workspace = unique_temp_dir(&format!("{tag}-ws"));
    // Empty global sandbox config under fixture INTELLIHELPER_HOME so generic tests do
    // not inherit the developer/runner's ~/.intellihelper/sandbox.toml.
    fs::write(intellihelper.join("sandbox.toml"), "").expect("empty global sandbox.toml");
    (
        home.clone(),
        intellihelper.clone(),
        workspace.clone(),
        TempDirGuard(home),
        TempDirGuard(intellihelper),
...
fn run_deny_case(
    ...
    let (home, intellihelper, tmp, _ch, _cg, _cw) = fixture_homes(tag);
    ...
    let deny_list = deny_entries
        ...
        .collect::<Vec<_>>()
        .join(", ");
    fs::create_dir_all(tmp.join(".intellihelper")).expect("mkdir .intellihelper");
    fs::write(
        tmp.join(".intellihelper").join("sandbox.toml"),
        format!("[profiles.{profile}]\nextends = \"workspace\"\ndeny = [{deny_list}]\n"),
    )
    .expect("write sandbox.toml");

    // Ensure IntelliHelper fixed slots exist so workspace-based custom profiles can
    // resolve hook write-deny without depending on the real user tree.
    fs::create_dir_all(intellihelper.join("hooks")).expect("mkdir fixture hooks");
    fs::write(intellihelper.join("hooks-paths"), b"").expect("write fixture hooks-paths");
    ...
    let (status, stderr) = run_scenario(&home, &intellihelper, &tmp, profile, targets, controls, postlaunch);
    assert!(
        status.success(),
...
fn hardlinked_hooks_paths_refuses_startup() {
    ...
    let (home, intellihelper, workspace, _ch, _cg, _cw) = fixture_homes("hook-hl");
    fs::create_dir_all(intellihelper.join("hooks")).unwrap();
    let reg = intellihelper.join("hooks-paths");
    let alias = intellihelper.join("hooks-paths-alias");
    fs::write(&reg, b"").unwrap();
    fs::hard_link(&reg, &alias).unwrap();

    let (status, stderr) =
        run_hook_write_deny_scenario(&home, &intellihelper, &workspace, "hook_write_deny");
    assert!(
        !status.success(),
...
/// Workspace profile: IntelliHelper-owned direct hook sources are write-denied but readable;
/// create / overwrite / unlink / rename / mkdir fail; absolute hooks-paths
/// targets are denied; parent rename is blocked; IntelliHelper/CWD/temp siblings stay writable.
#[test]
fn workspace_protects_direct_hook_sources() {
    ...
    let (home, intellihelper, workspace, _ch, _cg, _cw) = fixture_homes("hook");

    fs::create_dir_all(intellihelper.join("hooks")).expect("mkdir hooks");
    fs::write(intellihelper.join("hooks").join("keep.json"), r#"{"keep-me":true}"#)
        .expect("write keep.json");
    let dynamic = intellihelper.join("sessions").join("extra-hooks");
    ...
    fs::write(
        intellihelper.join("hooks-paths"),
    ...
    for needle in [
        ...
        "OK: intellihelper runtime sibling writable",
...
fn hardlinked_hooks_json_refuses_startup() {
    ...
    let (home, intellihelper, workspace, _ch, _cg, _cw) = fixture_homes("hook-json-hl");
    fs::create_dir_all(intellihelper.join("hooks")).unwrap();
    fs::write(intellihelper.join("hooks-paths"), b"").unwrap();
    let active = intellihelper.join("hooks").join("active.json");
    let alias = intellihelper.join("hooks").join("active-alias.json");
...
fn symlinked_hooks_json_refuses_startup() {
    ...
    let (home, intellihelper, workspace, _ch, _cg, _cw) = fixture_homes("hook-json-sym");
    fs::create_dir_all(intellihelper.join("hooks")).unwrap();
    fs::write(intellihelper.join("hooks-paths"), b"").unwrap();
    let real = intellihelper.join("real-active.json");
    let active = intellihelper.join("hooks").join("active.json");
...
/// First-run: missing fixed slots are created as real IntelliHelper state before apply,
...
fn workspace_protects_direct_hook_sources_first_run() {
    ...
    let (home, intellihelper, workspace, _ch, _cg, _cw) = fixture_homes("hook-fr");
    // Intentionally leave hooks/ and hooks-paths absent (first-run ensure path).

    let (status, stderr) =
        run_hook_write_deny_scenario(&home, &intellihelper, &workspace, "hook_write_deny_first_run");
    assert!(
        status.success(),
    ...
    for needle in [
        "OK: first-run IntelliHelper hook slots denied",
    ...
    // Post-exit host: IntelliHelper slots exist and are valid; no vendor artifacts.
    assert!(
        intellihelper.join("hooks").is_dir(),
        "post-exit: hooks dir must exist as a real directory"
    );
    assert!(
        intellihelper.join("hooks-paths").is_file(),
        "post-exit: hooks-paths must exist as a real file"
    );
    assert_eq!(
        fs::read(intellihelper.join("hooks-paths")).expect("read hooks-paths"),
        b"",
...
fn hook_write_deny_refuses_marker_spoof() {
    ...
    #[cfg(target_os = "linux")]
    {
        let (home, intellihelper, workspace, _ch, _cg, _cw) = fixture_homes("hook-spoof");
        fs::create_dir_all(intellihelper.join("hooks")).unwrap();
        fs::write(intellihelper.join("hooks").join("x.json"), b"{}").unwrap();
        fs::write(intellihelper.join("hooks-paths"), b"").unwrap();
        let (status, stderr) =
            run_hook_write_deny_scenario(&home, &intellihelper, &workspace, "hook_write_deny_marker_spoof");
        assert!(
            status.success(),
