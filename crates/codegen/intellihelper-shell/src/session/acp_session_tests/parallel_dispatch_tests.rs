...
//!
//! These tests verify the parallel dispatch path (INTELLIHELPER_PARALLEL_TOOL_DISPATCH):
...
/// JSON key. The compat toolset input types use `path`, and intellihelper_build's
...
#[test]
fn lock_path_for_args_matches_intellihelper_build_file_path() {
    // intellihelper_build search_replace / opencode EditTool / WriteTool / etc.
...
#[test]
fn lock_path_for_args_matches_intellihelper_build_target_file() {
    // intellihelper_build read_file uses #[serde(rename = "target_file")].
    let args = serde_json::json!({
...
#[test]
fn lock_path_for_args_buckets_intellihelper_build_and_compat_to_same_lock_for_same_file() {
    // A mixed batch (e.g. intellihelper_build search_replace + StrReplace
    ...
    let intellihelper = serde_json::json!({
        "file_path": "/repo/src/main.rs",
        "old_string": "a",
        "new_string": "b",
    });
    ...
    assert_eq!(lock_path_for_args(&intellihelper), lock_path_for_args(&compat));
}
...
fn test_skill_discovery_deferred_during_parallel_batch() {
    use intellihelper_sampling_types::{ConversationItem, SyntheticReason};
