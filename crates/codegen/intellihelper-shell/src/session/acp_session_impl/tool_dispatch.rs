...
/// - `file_path` — intellihelper_build (`search_replace`), opencode (`EditTool`,
///   `WriteTool`, `ReadTool`), codex (`read_file`), intellihelper_build_hashline
///   (`hashline_edit`)
/// - `path` — alternate edit/read tools
/// - `target_file` — intellihelper_build (`read_file`, via `#[serde(rename)]`)
///
...
impl SessionActor {
    ...
    pub(super) async fn handle_direct_bash_command(
        ...
        let title_command = intellihelper_tools::util::strip_redundant_session_cd(
            &command,
        ...
        let exec_wire = {
            ...
            agent
                .tool_bridge()
                .toolset()
                .tool_name_for_kind(intellihelper_tools::types::tool::ToolKind::Execute)
        };
...
// Re-use the UTF-8-safe truncation helper from intellihelper-sampling-types rather
...
///    [`MAX_ARGS_IN_ERROR`] bytes).  Without this, intellihelper-shell would sanitize
