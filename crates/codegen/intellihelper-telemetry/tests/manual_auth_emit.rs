...
//! `intellihelper-shell-manual_auth` with the `reason`/`trigger`/`token_kind`/`principal`
...
use intellihelper_telemetry::client;
use intellihelper_telemetry::config::{TelemetryConfig, TelemetryMode};
use intellihelper_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn manual_auth_posts_to_events_endpoint_as_intellihelper_shell_manual_auth() {
    ...
    intellihelper_telemetry::log_event(ManualAuth {
    ...
    let event = loop {
        let found = bodies.lock().unwrap().iter().find_map(|b| {
            ...
            (e.get("event_name")?.as_str()? == "intellihelper-shell-manual_auth").then(|| e.clone())
        });
        if let Some(e) = found {
            break e;
        }
        assert!(
            ...
            "no intellihelper-shell-manual_auth POST received"
        );
