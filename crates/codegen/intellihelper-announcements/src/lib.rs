//! Shared announcement types, persistence, and formatting for IntelliHelper CLI apps.
//!
//! This crate provides the common logic used by `intellihelper-shell` and
//! `intellihelper-pager` for handling announcements (banner notifications).
...
/// Read hidden announcement ids from `~/.intellihelper/announcements.json`.
...
/// Write hidden announcement ids to `~/.intellihelper/announcements.json`.
...
fn announcements_state_path() -> PathBuf {
    intellihelper_tools::util::intellihelper_home::intellihelper_home().join("announcements.json")
}
...
///
/// Precedence: `INTELLIHELPER_ANNOUNCEMENTS_OVERRIDE` env var (JSON override) → remote announcements.
...
pub fn resolve_startup(
    ...
    if let Ok(raw) = std::env::var("INTELLIHELPER_ANNOUNCEMENTS_OVERRIDE") {
        match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
            Ok(list) => return Some(list),
            Err(e) => {
                tracing::warn!(error = %e, "invalid INTELLIHELPER_ANNOUNCEMENTS_OVERRIDE JSON; ignoring override");
            }
        }
    }
...
mod tests {
    ...
    fn resolve_startup_env_override() {
        ...
        unsafe {
            std::env::set_var("INTELLIHELPER_ANNOUNCEMENTS_OVERRIDE", r#"[{"id":"test"}]"#);
        }
        ...
        // SAFETY: test-only
        unsafe {
            std::env::remove_var("INTELLIHELPER_ANNOUNCEMENTS_OVERRIDE");
        }
    }
