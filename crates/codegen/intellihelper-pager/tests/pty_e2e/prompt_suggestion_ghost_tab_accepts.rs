...
async fn prompt_suggestion_ghost_tab_accepts() {
    // The suggestion model (`intellihelper-build-0.1`, the shell's built-in default)
    ...
    let content = ContentController::start_with_models(vec![
        ...
        MockModel::new("intellihelper-build-0.1"),
    ])
    .await
    ...
    let overrides = [("INTELLIHELPER_PROMPT_SUGGESTIONS".to_owned(), "true".to_owned())];
