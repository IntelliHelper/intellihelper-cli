...
fn resolve_config(cfg: &AgentConfig, auth_manager: &AuthManager) -> AgentConfig {
    ...
    // env var > remote settings > Local. Skip remote settings for Generic (intellihelper -p, subagents).
    ...
    // A CLI/env-set Writeback still requires intellihelper.com auth.
    if cfg.storage_mode == StorageMode::Writeback && !has_xai_auth {
        tracing::info!("Writeback is disabled: requires auth with intellihelper.com");
...
fn init_process(cfg: &AgentConfig, auth_manager: &AuthManager) {
    ...
    INIT.call_once(|| {
        ...
        crate::builtin::extract_builtin_files(&intellihelper_home);

        crate::extensions::marketplace::purge_default_skills_installs(&intellihelper_home);
        ...
        if cfg.resolve_official_marketplace_auto_register().value {
            crate::extensions::marketplace::ensure_official_marketplace_source(&intellihelper_home);
        }
        ...
        intellihelper_telemetry::session_ctx::log_event(limits.into_event());
    });
}
...
pub fn update_telemetry_config(config: &AgentConfig, auth_manager: &AuthManager) {
    let intellihelper_auth = auth_manager.current().filter(|a| a.is_xai_auth());
    let user_id = intellihelper_auth.as_ref().map(|a| a.user_id.clone());
    let team_id = intellihelper_auth.as_ref().and_then(|a| a.team_id.clone());
