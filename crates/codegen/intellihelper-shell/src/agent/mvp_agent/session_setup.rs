...
fn insert_applied_tool_overrides(
    ...
    echo: Option<&intellihelper_sampling_types::ToolOverrides>,
) {
...
struct RestoredSignals {
    ...
    turn_count: u64,
    tool_call_count: u64,
    plan_mode_state: intellihelper_telemetry::events::PlanModeState,
...
impl RestoredSignals {
    fn read(
        ...
        use intellihelper_telemetry::events::PlanModeState as Reported;
        Self {
...
impl MvpAgent {
    pub(super) async fn new_session_inner(
        ...
        if product_analytics || intellihelper_telemetry::external::is_active() {
            ...
            tokio::spawn(async move {
                let git = intellihelper_telemetry::context::collect_git_context(&cwd_str);
                let ev = intellihelper_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                intellihelper_telemetry::session_ctx::log_event_dual(product_analytics, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) =
            match intellihelper_workspace::session::git::discover_git_root(cwd.as_path()) {
    ...
    async fn attach_session(
        ...
        let git_root =
            intellihelper_workspace::session::git::find_git_root_from_path(cwd.as_path()).ok();
        ...
        let (initial_total_tokens, unfinished_subagents) = self
            ...
            .await?;
        let preloaded_envrc = intellihelper_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
        ...
        let (model_state, response_meta) = self
            ...
            .await;
        intellihelper_telemetry::unified_log::info("session loaded", Some(session_id.0.as_ref()), None);
        ...
        if self.product_analytics_enabled() {
            log_event(intellihelper_telemetry::events::SessionLoad {
                session_id: session_id.0.to_string(),
    ...
    async fn restore_session_code(
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            intellihelper_workspace::session::git::warn_registry_disabled_restore(session_id.0.as_ref());
        }
        let restore_checkout_allowed =
            intellihelper_workspace::session::git::restore_code_checkout_allowed(
                cwd.as_path(),
        ...
        {
            tracing::warn!(
                target: intellihelper_workspace::session::git::RESTORE_CODE_LOG,
                session_id = %session_id.0,
                supplied_cwd = %cwd.as_str(),
                persisted_cwd = %summary.info.cwd,
                target_sha = %target_sha,
                "restore_code: skipping session HEAD checkout — supplied cwd is neither a intellihelper worktree nor the session's persisted cwd (refusing to detach the source repo)"
            );
        ...
        if restore_code_requested
            ...
            && let Some(ref target_sha) = summary.head_commit
        {
            use intellihelper_workspace::session::git::RestoreKind;
            let outcome = intellihelper_workspace::session::git::checkout_session_commit(
                cwd.as_path(),
                target_sha,
                true,
                session_id.0.as_ref(),
    ...
    async fn restore_persisted_model(
        ...
        let is_intellihelper_build = persisted_model.0.starts_with("intellihelper-build");
        let same_family_fallback = if is_intellihelper_build {
            available
                .keys()
                .find(|id| id.0.starts_with("intellihelper-build"))
                .cloned()
        } else {
            available
                .keys()
    ...
    async fn build_attach_response_meta(
        ...
        if summary.head_commit.is_some()
            ...
            && summary.git_root_dir.as_deref().is_none_or(|root| {
                intellihelper_workspace::session::git::find_git_root_from_path(std::path::Path::new(
                    cwd.as_str(),
                ))
                .ok()
            ...
            })
        {
            ...
            let current_head =
                intellihelper_workspace::session::git::git_cli(cwd_path, &["rev-parse", "HEAD"])
                    .await
                    .ok();
            if let Some(divergence) = intellihelper_workspace::session::git::detect_head_divergence(
