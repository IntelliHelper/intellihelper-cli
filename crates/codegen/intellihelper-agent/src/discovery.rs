...
//!
//! Searches `.intellihelper/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.intellihelper/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.intellihelper/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".intellihelper/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.intellihelper/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.intellihelper/agents` / `.claude/agents`) under each
...
pub fn all_subagents(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<SubagentEntry> {
    ...
    all_subagents_with_home(cwd, toggle, dirs::home_dir().as_deref(), intellihelper.as_deref())
}
...
fn all_subagents_with_home(
    ...
    home: Option<&Path>,
    intellihelper_home: Option<&Path>,
) -> Vec<SubagentEntry> {
    let discovered = discover_with_home(cwd, home, intellihelper_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.intellihelper/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.intellihelper/agents/` walking from `cwd` up to repo root
/// 2. `~/.intellihelper/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.intellihelper/bundled/agents/` (bundled, lowest priority)
///
/// Deduplicates by name — higher-priority definitions win.
/// User-level agent directories in priority order: user intellihelper agents, `.claude`
/// compat agents, then bundled. `.intellihelper` dirs resolve from `intellihelper_home`
/// (INTELLIHELPER_HOME-aware) plus the legacy literal `~/.intellihelper` when INTELLIHELPER_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    intellihelper_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.intellihelper, included only when it differs from intellihelper_home
    // (i.e. INTELLIHELPER_HOME points elsewhere) so agents left in the old location are
    // still discovered and stay consistent with scope_from_path classification.
    let legacy_grok = home
        .map(|h| h.join(".intellihelper"))
        .filter(|legacy| intellihelper_home != Some(legacy.as_path()));

    let mut dirs = Vec::new();
    if let Some(g) = intellihelper_home {
...
pub fn discover(cwd: &Path) -> Vec<AgentDefinition> {
    let intellihelper = intellihelper_config::user_intellihelper_home();
    discover_with_home(cwd, dirs::home_dir().as_deref(), intellihelper.as_deref())
}
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, intellihelper_home) {
        if dir.is_dir() {
...
pub fn by_name(name: &str) -> Option<AgentDefinition> {
    let intellihelper = intellihelper_config::user_intellihelper_home();
    by_name_with_home(name, dirs::home_dir().as_deref(), intellihelper.as_deref())
}
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, intellihelper_home);
...
///
/// Project-level `.intellihelper/agents/` has highest priority, then falls back
/// to built-ins, user-level, and finally bundled definitions.
pub fn by_name_in_cwd(name: &str, cwd: &Path) -> Option<AgentDefinition> {
    let intellihelper = intellihelper_config::user_intellihelper_home();
    by_name_in_cwd_with_home(name, cwd, dirs::home_dir().as_deref(), intellihelper.as_deref())
}
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, intellihelper_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, intellihelper_home) {
        return Some(def);
    }
...
/// Expand `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` (and the IntelliHelper
...
/// Load project agent definitions from every `.intellihelper/agents` / `.claude/agents`
...
mod tests {
    ...
    #[test]
    fn user_agent_dirs_includes_legacy_grok_when_intellihelper_home_differs() {
        let home = Path::new("/home/u");
        let intellihelper = Path::new("/custom/grokhome");
        let paths: Vec<_> = user_agent_dirs(Some(home), Some(intellihelper))
            .into_iter()
            .map(|(p, _)| p)
            .collect();
        assert!(paths.contains(&intellihelper.join("agents")));
        assert!(paths.contains(&home.join(".intellihelper").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&intellihelper.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".intellihelper").join("bundled").join("agents")));
    }

    #[test]
    fn user_agent_dirs_dedups_legacy_when_intellihelper_home_is_dot_grok() {
        let home = Path::new("/home/u");
        let intellihelper = home.join(".intellihelper");
        let count = user_agent_dirs(Some(home), Some(&intellihelper))
            .into_iter()
            .filter(|(p, _)| *p == intellihelper.join("agents"))
            .count();
        assert_eq!(
            count, 1,
            "no duplicate ~/.intellihelper/agents when intellihelper_home == ~/.intellihelper"
        );
    }
    ...
    #[test]
    fn test_by_name_builtin_intellihelper_build() {
        let def = by_name("intellihelper-build");
        assert!(def.is_some());
        assert_eq!(def.unwrap().name, "intellihelper-build");
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".intellihelper").join("agents");
        let agents_dir_2 = inner_dir.join(".intellihelper").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".intellihelper").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".intellihelper")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".intellihelper")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".intellihelper").join("agents");
        let bundled_dir = home.join(".intellihelper").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".intellihelper")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".intellihelper")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".intellihelper").join("agents");
        let bundled_dir = home.join(".intellihelper").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_project_shadows_builtin() {
        ...
        // Create a project-level "intellihelper-build" that shadows the built-in
        write_agent_file(
            &agents_dir,
            "intellihelper-build.md",
            "intellihelper-build",
            "Custom intellihelper-build",
        );

        let def = by_name_in_cwd("intellihelper-build", tmp.path());
        assert!(def.is_some());
        let def = def.unwrap();
        assert_eq!(def.name, "intellihelper-build");
        assert_eq!(def.description, "Custom intellihelper-build");
    }

    #[test]
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .intellihelper/agents/ directory — should fall back to built-in
    ...
    fn test_orchestrator_from_str_resolves() {
        use std::str::FromStr;
        let variant = BuiltinAgentName::from_str("intellihelper-build-orchestrator")
            .expect("from_str must resolve intellihelper-build-orchestrator");
        assert_eq!(variant, BuiltinAgentName::IntelliHelperBuildOrchestrator);
        let def = variant.definition();
        assert_eq!(def.name, "intellihelper-build-orchestrator");
        assert!(
    ...
    fn test_orchestrator_by_name_in_cwd() {
        let tmp = tempfile::tempdir().unwrap();
        let def = by_name_in_cwd("intellihelper-build-orchestrator", tmp.path())
            .expect("by_name_in_cwd must find intellihelper-build-orchestrator");
        assert_eq!(def.name, "intellihelper-build-orchestrator");
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.intellihelper/agents/) agent named "explore" should NOT shadow
