...
//! [`MockInferenceServer`] from `intellihelper-test-support` and provides the
...
use intellihelper_test_support::{MockInferenceServer, TestSandbox};

pub use intellihelper_test_support::mock_server::LogEntry;
pub use intellihelper_test_support::mock_server::MockModelEntry as MockModel;
pub use intellihelper_test_support::mock_server::StorageUpload;
pub use intellihelper_test_support::sse;
pub use intellihelper_test_support::{
...
impl ContentController {
    ...
    pub async fn start_with_models(models: Vec<MockModel>) -> Result<Self> {
        ...
        sandbox.set_env("INTELLIHELPER_PROMPT_SUGGESTIONS", "false");
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.intellihelper
...
mod tests {
    ...
    async fn read_foreground(url: String, endpoint: InferenceEndpoint) -> String {
        ...
        reqwest::Client::new()
            ...
            .header("x-intellihelper-turn-idx", "1")
            .header("x-intellihelper-req-id", format!("direct-{endpoint:?}"))
            .json(&body)
            .send()
            .await
