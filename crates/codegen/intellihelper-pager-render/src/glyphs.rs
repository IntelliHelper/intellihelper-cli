...
/// Optimize-input affordance on the prompt info line.
///
/// Prefer a **solid black star** (`★` U+2605) which reads larger than the
/// thin four-pointed `✦` on most terminal fonts, plus a trailing pad so the
/// control occupies **2 columns** and is easier to click. Legacy ConHost
/// falls back to a two-column `"* "` (CP437-safe).
///
/// Avoid emoji (e.g. ✨): many terminals give them two columns inconsistently
/// and shift the chrome.
pub fn optimize_sparkle() -> &'static str {
    if is_legacy_windows_console() {
        "* "
    } else {
        "\u{2605} " // ★ + space → 2 columns, larger hit target
    }
}
...
///
/// `INTELLIHELPER_FORCE_LEGACY_CONSOLE=1` (or `true`) forces this on regardless of
...
/// Read the `INTELLIHELPER_FORCE_LEGACY_CONSOLE` escape hatch from the environment.
fn forced_legacy_console_override() -> Option<bool> {
    parse_forced_legacy_console(std::env::var("INTELLIHELPER_FORCE_LEGACY_CONSOLE").ok().as_deref())
}
