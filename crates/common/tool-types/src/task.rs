...
/// The blocking-wait ceiling in effect, honoring `INTELLIHELPER_MAX_WAIT_BLOCK_MS`.
///
...
pub fn max_wait_block_ms() -> u64 {
    std::env::var("INTELLIHELPER_MAX_WAIT_BLOCK_MS")
        .ok()
...
/// A built-in subagent type shared by the CLI (`intellihelper-agent`) and other
...
mod tests {
    ...
    fn task_tool_input_model_parses_explicit() {
        let input: TaskToolInput =
            serde_json::from_str(r#"{"description": "d", "prompt": "p", "model": "intellihelper-3"}"#)
                .unwrap();
        assert_eq!(input.model.as_deref(), Some("intellihelper-3"));
    }
    ...
    fn sanitize_optional_arg_moves_when_no_trim() {
        assert_eq!(
            sanitize_optional_arg(Some("intellihelper-3".into())).as_deref(),
    ...
    // match what the intellihelper-shell MiniJinja templates render for the default
    // intellihelper-build toolset (monitor + task + bash + read present, POSIX). The
