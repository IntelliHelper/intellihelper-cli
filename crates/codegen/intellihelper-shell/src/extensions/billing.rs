...
//!
//! Fetches the authenticated user's IntelliHelper CLI billing configuration
...
/// Current billing configuration for IntelliHelper CLI coding credits.
///
/// Carries both the newer credits-config fields (`credit_usage_percent`,
/// `current_period`) and the deprecated `IntelliHelperBuildBillingConfig` fields
...
/// against both the new `GetIntelliHelperCreditsConfig` and the legacy
/// `GetIntelliHelperBuildBillingConfig` backend responses.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BillingConfig {
    ...
    /// (`GetIntelliHelperCreditsConfig.prepaid_balance`); absent in the legacy billing
    /// shape.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prepaid_balance: Option<Cent>,
    /// Whether this user is on unified usage billing (shared weekly/monthly
    /// pool). From `IntelliHelperCreditsConfig.is_unified_billing_user`, which billing
    /// sets from remote settings `unified_consumer_billing_enabled`. `None` when
    /// absent (legacy `GetIntelliHelperBuildBillingConfig` shape or older servers).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Top-level response (primarily from `GET /rest/intellihelper/credits` + auto-topup-rule).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
///
/// Keeps history to a count + the most recent period so `~/.intellihelper/logs/unified.jsonl`
...
async fn handle_get_billing(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Billing data requires auth with intellihelper.com. Run `intellihelper login` to authenticate.",
    )?;
    ...
    // forwards to the backend `GetIntelliHelperCreditsConfig`.
...
async fn handle_get_auto_topup_rule(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Auto top-up data requires auth with intellihelper.com. Run `intellihelper login` to authenticate.",
    )?;
...
mod tests {
    ...
    fn billing_config_deserializes_credits_config_shape() {
        // Newer `GetIntelliHelperCreditsConfig` response: percentage-based usage,
        ...
        let json = serde_json::json!({
            "config": {
                ...
                "productUsage": [
                    {"product": "PRODUCT_INTELLIHELPER_BUILD", "usagePercent": 61.2}
                ],
                "history": [
                    {
                        "period": {
