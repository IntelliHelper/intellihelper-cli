...
//! stay in each product host (for example `intellihelper-shell`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `intellihelper-sampling-types`. It is decoupled from both IntelliHelper chat and
//! intellihelper-build hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — intellihelper-build's whole-session **full-replace**
//!   subsystem (prompt/summary/failure/config, assemble, orchestration).
//! - [`intra_compaction`] — IntelliHelper chat's tail-keep, per-step pass.
//! - [`inter_compaction`] — IntelliHelper chat's chunked, between-turn pass.
//!
...
//! (active-agent-state `<system-reminder>` formatting shared by IntelliHelper chat and
//! intellihelper-build; hosts still own snapshotting and host-only sections).
...
// intellihelper-build's full-replace subsystem now lives under `code_compaction`;
