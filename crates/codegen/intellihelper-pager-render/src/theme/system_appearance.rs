...
//! 2. Explicit env stamps — `INTELLIHELPER_APPEARANCE` / `LC_INTELLIHELPER_APPEARANCE`
...
/// 1. Desktop APIs
/// 2. Explicit wrap/SSH stamps (`INTELLIHELPER_APPEARANCE` / `LC_INTELLIHELPER_APPEARANCE`)
...
///
/// Used by `intellihelper wrap` to stamp the *local* OS theme into the child env
...
/// in `config.toml`. When `None`, defaults to `IntelliHelperNight` / `IntelliHelperDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::IntelliHelperDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::IntelliHelperNight),
    }
}
...
mod tests {
    ...
    #[test]
    fn to_theme_kind_dark_defaults_to_intellihelpernight() {
        let result = to_theme_kind(SystemAppearance::Dark, None, None);
        assert_eq!(result, ThemeKind::IntelliHelperNight);
    }

    #[test]
    fn to_theme_kind_light_defaults_to_intellihelperday() {
