...
use intellihelper_shell::extensions::notification::RetryState;
use intellihelper_shell::extensions::notification::SessionUpdate as XaiSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> intellihelper_shell::extensions::notification::ImageCompressedEntry {
    intellihelper_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn xai_hook_execution_notif_for_prompt(
    ...
    use intellihelper_shell::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
...
pub(super) fn xai_hook_execution_notif_with_runs(
    ...
    prompt_id: Option<&str>,
    is_replay: bool,
    runs: Vec<intellihelper_shell::extensions::notification::HookRunEntryDto>,
) -> acp::ExtNotification {
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
/// Shared temp `INTELLIHELPER_HOME` for disk-replay tests. `intellihelper_home()` uses a
/// process-wide `OnceLock`, so parallel tests must not each set `INTELLIHELPER_HOME`
...
/// Runs `f` with a thread-local intellihelper home override so disk replay tests do not
/// depend on process-wide `intellihelper_home()` cache order when the full suite runs.
pub(super) fn with_replay_disk_home<R>(f: impl FnOnce(&std::path::Path) -> R) -> R {
    let home = replay_disk_test_home();
    crate::app::subagent::set_replay_intellihelper_home_for_tests(Some(home.to_path_buf()));
    let out = f(home);
    crate::app::subagent::set_replay_intellihelper_home_for_tests(None);
    out
}
pub(super) fn write_child_updates_jsonl(
    ...
    let sessions_dir = intellihelper_home
        .join("sessions")
...
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = intellihelper_workspace::session::git::GitHeadChanged {
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: intellihelper_shell::extensions::mcp::McpServerStatus,
    tools: Option<serde_json::Value>,
) -> acp::ExtNotification {
    use intellihelper_shell::extensions::mcp::{
...
pub(super) fn make_tools_changed_notif_post_h2(
    ...
    let payload = intellihelper_shell::extensions::mcp::McpToolsChanged {
        session_id: session_id.to_string(),
