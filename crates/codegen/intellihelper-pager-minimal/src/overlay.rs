...
use intellihelper_pager::app::agent_view::AgentView;
use intellihelper_pager::app::app_view::{ActiveView, AppView};
use intellihelper_pager::appearance::LayoutConfig;
use intellihelper_pager::minimal_api;
use intellihelper_pager::render::SafeBuf as _;
use intellihelper_pager::theme::Theme;
use intellihelper_pager::views::prompt_widget::{PromptBg, PromptStyle, PromptWidget};
...
fn active(prompt: &PromptWidget, items_width: u16) -> Option<(Kind, u16)> {
    use intellihelper_pager::views::completion_dropdown::MAX_VISIBLE_ROWS;
    use intellihelper_pager::views::file_search::dropdown::MAX_DROPDOWN_ROWS as FILE_MAX;
    use intellihelper_pager::views::slash_dropdown::desired_item_rows;
...
fn compute_target(app: &mut AppView, term_h: u16, width: u16) -> u16 {
    ...
    let (input_mode, multiline) = app
        ...
        .unwrap_or_default();
    let theme = intellihelper_pager::theme::Theme::current();
    ...
    let raw_btw = if minimal_api::minimal_btw_surface_available(agent) {
        intellihelper_pager::views::btw_overlay::btw_panel_height(agent.btw_state.as_ref(), width)
    } else {
        0
    };
...
pub fn render(
    ...
    match kind {
        Kind::FileSearch => {
            intellihelper_pager::views::file_search::dropdown::render_dropdown(
                buf,
                items_rect,
                &prompt.file_search,
                theme,
        ...
        Kind::Slash => {
            ...
            intellihelper_pager::views::slash_dropdown::render_dropdown(
                buf, items_rect, &snap, hovered, theme,
            );
        }
        Kind::Completion => {
            intellihelper_pager::views::completion_dropdown::render_dropdown(
                buf,
                items_rect,
...
pub fn modal_height(modal: Modal, agent: &mut AgentView, screen_h: u16, content_w: usize) -> u16 {
    match modal {
        Modal::Permission => agent
            ...
            .front()
            .map(|p| {
                intellihelper_pager::views::permission_view::permission_view_height(
        ...
        Modal::Question => {
            let input_mode = minimal_api::question_view(agent).is_some_and(|qv| {
                qv.focus == intellihelper_pager::views::question_view::QuestionFocus::InputMode
            });
            ...
            minimal_api::question_view_mut(agent)
                .map(|qv| {
                    intellihelper_pager::views::question_view::question_view_height(
        ...
        Modal::Rewind => minimal_api::rewind_state(agent)
            .map(|rw| intellihelper_pager::views::rewind::rewind_overlay_height(&rw.phase, screen_h))
            .unwrap_or(0),
        Modal::Cancel => {
            if minimal_api::cancel_turn_view(agent).is_some() {
                intellihelper_pager::views::modal::cancel_turn_panel_height(screen_h)
            } else {
                0
            }
        }
    ...
    match modal {
        ...
        Modal::Rewind => {
            if let Some(rw) = minimal_api::rewind_state(agent) {
                intellihelper_pager::views::rewind::render_rewind_overlay(buf, area, &rw.phase, true);
            }
            None
        }
        Modal::Cancel => {
            ...
            let drawn = if let Some(ctv) = minimal_api::cancel_turn_view(agent) {
                intellihelper_pager::views::modal::render_cancel_turn_panel(
                    buf,
                    area,
                    ctv,
                    true,
...
fn render_permission(
    ...
    let result = intellihelper_pager::views::permission_view::render_permission_view(
        buf,
        area,
        perm,
        &followup,
...
fn render_question(
    ...
    use intellihelper_pager::views::question_view::{QUESTION_VIEW_HPAD, QuestionFocus};
    ...
    if let Some(qv) = minimal_api::question_view_mut(agent) {
        let vis = qv.questions.get(qv.active_tab).map(|question| {
            intellihelper_pager::views::question_view::visible_options_height(
                question,
                q_area.height,
                content_w,
                qv.focused_preview(),
    ...
    if let Some(qv) = minimal_api::question_view(agent) {
        intellihelper_pager::views::question_view::render_question_view(
            buf,
            q_area,
            qv,
    ...
    if input_mode {
        ...
        let freeform_sel = minimal_api::question_view(agent)
            ...
            .copied()
            .unwrap_or(false);
        let embed = intellihelper_pager::views::modal_window::embedded_row_style(theme, true);
        ...
        let marker = if is_multi {
            ...
            format!("({})", intellihelper_pager::glyphs::filled_dot())
        } else {
            "(\u{25cb})".to_string()
        };
        ...
        buf.set_span_safe(
            area.x + 9,
            row_y,
            &ratatui::text::Span::styled(intellihelper_pager::glyphs::prompt_arrow(), accent),
            2,
        );
        ...
        let editor = Rect {
            x: area.x + 11,
            y: row_y,
            width: intellihelper_pager::views::question_view::inline_text_width(area.width),
            height: input_h,
        };
        return agent
            .prompt
...
fn question_editor_h(agent: &AgentView, area_w: u16, cap: u16, theme: &Theme) -> u16 {
    let text_w = intellihelper_pager::views::question_view::inline_text_width(area_w);
    agent
        .prompt
...
mod tests {
    use super::*;
    use intellihelper_pager::views::suggestion_controller::CompletionItemParsed;
    ...
    fn question_input_agent(text: &str) -> AgentView {
        use intellihelper_pager::views::prompt_widget::StashedPrompt;
        use intellihelper_pager::views::question_view::{Question, QuestionOption, QuestionViewState};
