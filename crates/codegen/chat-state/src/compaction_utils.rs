...
//! this crate and `intellihelper-shell` can share them without duplication.
use std::collections::BTreeSet;
use intellihelper_sampling_types::{ContentPart, ConversationItem, ToolResultItem};
...
///
/// Stored here (rather than only in `intellihelper-shell`) so that query-extraction
...
/// `ActiveSubagentSummary` in intellihelper-tools. The mapping between them
/// happens in `run_compact_inner()` (intellihelper-shell).
#[derive(Clone)]
...
/// A dependency-free mirror of `TodoStatus` (intellihelper-tools), kept here so
...
impl TodoSummaryStatus {
    ...
    /// Mirrors `TodoStatus::tag()` in intellihelper-tools.
...
/// `TodoItem` in intellihelper-tools.
#[derive(Clone)]
...
/// handled by the consumer (e.g. `intellihelper-shell`), which has access to
...
impl CompactionStateContext {
    ...
    /// model to re-read the same files. intellihelper-build retains
...
/// `chat-state` and `intellihelper-shell` share the same format.
...
pub struct CompactedHistoryInput<'a> {
    ...
    /// When `false` (the default), recent messages come first (intellihelper-build
    /// ordering).
    ...
    /// carrier and the intellihelper-build summary.
    ...
    /// "Total summaries generated so far …" footer. Ignored by the intellihelper-build
...
/// `run_compact_inner` in `intellihelper-shell` assembles inline, but is
...
pub fn repair_history(items: &mut Vec<ConversationItem>) -> HistoryRepairReport {
    let duplicates_removed = intellihelper_sampling_types::dedup_duplicate_tool_results(items);
    let stripped_tool_result_ids = strip_displaced_tool_results(items);
    let synthetic_results_inserted = intellihelper_sampling_types::repair_dangling_tool_calls(
        items,
        intellihelper_sampling_types::DanglingToolCallReason::HarnessHalted {
...
mod tests {
    use super::*;
    use intellihelper_sampling_types::SyntheticReason;
    #[test]
...
actual user question";
    ...
    fn call(id: &str) -> intellihelper_sampling_types::ToolCall {
        intellihelper_sampling_types::ToolCall {
            id: id.into(),
            name: "read_file".to_string(),
            arguments: "{}".into(),
        }
    ...
    async fn build_compacted_history_transcript_hint() {
        ...
        let summary = build_compacted_history(input(Some(
            "/home/user/.intellihelper/sessions/abc/updates.jsonl".to_string(),
        )))
        .last()
        .unwrap()
        .text_content();
        assert!(summary.contains("/home/user/.intellihelper/sessions/abc/updates.jsonl"));
    ...
    ///
    /// Simulates the exact conversation shape produced by intellihelper-shell:
    ///
    ...
    async fn build_compacted_history_multi_turn_with_parallel_tool_calls() {
        use intellihelper_sampling_types::{AssistantItem, ToolCall};
        let conversation = vec![
            // [0] System prompt
            ...
            ConversationItem::Assistant(AssistantItem {
                ...
                model_id: Some("intellihelper-3".to_string()),
...
The user asked to read main.rs and lib.rs. main.rs prints hello world, lib.rs has an add function. The user then asked to fix a typo in main.rs and run tests. The typo was fixed and tests passed.";
    ...
    fn conversation_item_preserves_reasoning_siblings() {
        use intellihelper_sampling_types::{AssistantItem, rs};
        let result = strip_tool_messages_for_conversation_item(vec![
            ConversationItem::system("system"),
    ...
    fn prepare_for_summarization_drops_reasoning_sibling_on_mutated_assistant() {
        use intellihelper_sampling_types::{AssistantItem, ToolCall, rs};
        let mk_reasoning = || {
            ConversationItem::Reasoning(rs::ReasoningItem {
                id: "r_123".to_string(),
                summary: vec![rs::SummaryPart::SummaryText(rs::SummaryTextContent {
    ...
    fn verbatim_reasoning_kept_unless_messages_backend() {
        use intellihelper_sampling_types::{ToolCall, rs};
        let mk = || {
            vec![
                ConversationItem::system("sys"),
                ConversationItem::Reasoning(rs::ReasoningItem {
        ...
        assert!(
            ...
            "reasoning must be kept when strip_reasoning = false (IntelliHelper backends)"
        );
    ...
    fn fit_counts_user_images_against_budget() {
        use intellihelper_sampling_types::ContentPart;
    ...
    fn fit_counts_encrypted_reasoning_against_budget() {
        use intellihelper_sampling_types::rs;
