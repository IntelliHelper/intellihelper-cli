...
//! elected leader) plus a fixed leader socket beneath its `INTELLIHELPER_HOME`; clients
...
impl LeaderCluster {
    ...
    /// one leader) and a fixed leader socket under its `INTELLIHELPER_HOME`.
    pub async fn start(rows: u16, cols: u16) -> Result<Self> {
        let content = ContentController::start()
            .await
            .context("start content controller")?;
        // One shared INTELLIHELPER_HOME => one leader; the socket lives beneath it so
        // every client (sharing the same env) elects/attaches to the same one.
        let intellihelper_home = content.home().join(".intellihelper");
        std::fs::create_dir_all(&intellihelper_home).context("create intellihelper home")?;
    ...
    /// The cluster's sessions root: `INTELLIHELPER_HOME/sessions` (layout below is
    /// `sessions/<encoded-cwd>/<session-id>/updates.jsonl`).
    fn sessions_dir(&self) -> PathBuf {
        self.content.home().join(".intellihelper").join("sessions")
    }
