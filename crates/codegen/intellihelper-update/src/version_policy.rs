...
use crate::version::get_installed_intellihelper_version;
use semver::Version;
use tracing::warn;
use intellihelper_shell::util::config::VersionPolicy;
...
pub(crate) enum VersionPolicyError {
    #[error(
        "Cannot install IntelliHelper {target}: the minimum allowed version is {minimum}. \
         Run `intellihelper update` to install the latest allowed version."
    )]
...
fn required_range_message(decision: &RequiredRangeDecision) -> Option<String> {
    match decision {
        ...
        RequiredRangeDecision::Below { current, minimum } => Some(format!(
            "This version of IntelliHelper ({current}) is older than the minimum required \
             by your organization ({minimum}).\n\n\
             Update to an approved version through your organization's approved \
             method (for example, run `intellihelper update`)."
        )),
        RequiredRangeDecision::Above { current, maximum } => Some(format!(
            "This version of IntelliHelper ({current}) is newer than the maximum allowed \
             by your organization ({maximum}).\n\n\
             Install an approved version through your organization's approved \
             method (for example, run `intellihelper update --version {maximum}`)."
        )),
    }
}
