...
//! - `Default`: the legacy IntelliHelper CLI prefix (built by the shell layer).
...
use intellihelper_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy IntelliHelper CLI prefix (`<user_info>` + optional `<git_status>`), built directly by the
...
/// One discovered rule file (AGENTS.md / Claude.md / .intellihelper/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `intellihelper_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.intellihelper/, ~/.claude/).
    ...
    /// (`~/.intellihelper/projects/<encoded-cwd>/mcps`). Surfaced in
