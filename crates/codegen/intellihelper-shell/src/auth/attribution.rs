...
//!
//! 1. [`intellihelper_telemetry::unified_log::warn`] for the local
//!    `~/.intellihelper/logs/unified.jsonl` file (best-effort; ships to GCS
...
//!
//! [`intellihelper_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`intellihelper_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`intellihelper_sampler::SamplerConfig::attribution_callback`] at every
...
use intellihelper_sampler::{Auth401AttributionCallback, SamplingConsumer};
use intellihelper_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`intellihelper_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `intellihelper_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn intellihelper_tools::Auth401AttributionCallback>` for the
...
/// in `intellihelper-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `intellihelper-shell` go through this helper -- the
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.intellihelper/logs/unified.jsonl) + scrubbed
...
mod tests {
    ...
    use crate::auth::{AuthManager, IntelliHelperAuth, GrokComConfig};

    use super::*;

    /// Test helper: build a fresh `AuthManager` rooted at a tempdir so
    /// nothing from a developer's actual `~/.intellihelper/auth.json` leaks in.
    ...
    fn fresh_auth(key: &str) -> IntelliHelperAuth {
        IntelliHelperAuth {
            key: key.to_string(),
            create_time: Utc::now(),
    ...
    fn hard_expired_auth(key: &str) -> IntelliHelperAuth {
        IntelliHelperAuth {
            key: key.to_string(),
    ...
    /// `ShellAttribution` implements `intellihelper_tools::Auth401AttributionCallback`
