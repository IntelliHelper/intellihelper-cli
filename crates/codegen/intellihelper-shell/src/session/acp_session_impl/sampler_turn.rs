...
/// `SamplingError::is_auth_error` in intellihelper-sampling-types: 403 is
...
pub(super) async fn call_with_auth_retry<F, Fut>(
    ...
    tool_name: &str,
    mut call: F,
) -> Result<intellihelper_tools::types::output::ToolRunResult, tool_runtime::ToolError>
where
    F: FnMut() -> Fut,
    Fut: std::future::Future<
            Output = Result<
                intellihelper_tools::types::output::ToolRunResult,
...
impl SessionActor {
    ...
    fn resolve_hosted(
        ...
        Vec<intellihelper_sampling_types::HostedTool>,
        intellihelper_sampling_types::ToolOverrides,
    ) {
        let mut tools = self.agent.borrow().hosted_tools().to_vec();
        let applied = intellihelper_sampling_types::apply_tool_overrides(
            &mut tools,
    ...
    pub(crate) fn effective_hosted_tools(&self) -> Vec<intellihelper_sampling_types::HostedTool> {
        self.resolve_hosted().0
    }
    pub(crate) fn hosted_tools_for_turn(&self) -> Vec<intellihelper_sampling_types::HostedTool> {
    ...
    pub(crate) fn effective_tool_overrides(
        &self,
    ) -> Option<intellihelper_sampling_types::ToolOverrides> {
        if !self.backend_search_active() {
            return None;
        }
    ...
    /// inherits it.
    pub(crate) fn set_tool_overrides(&self, overrides: intellihelper_sampling_types::ToolOverrides) {
    ...
    pub(crate) fn apply_tool_overrides_update(
        &self,
        update: Option<intellihelper_sampling_types::ToolOverridesUpdate>,
    ) {
    ...
    pub(super) async fn reconstruct_full_config(&self) -> SamplingConfig {
        ...
        impl intellihelper_sampler::HeaderInjector for TraceContextInjector {
        ...
        let cfg = self
            ...
            .get_sampling_config()
            .await
            .unwrap_or_else(|| intellihelper_sampling_types::SamplingConfig {
                base_url: String::new(),
                model: String::new(),
    ...
    pub(crate) async fn wire_permission_auto_llm_classifier(self: &Arc<Self>) {
        ...
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel::<(
            Vec<intellihelper_workspace::permission::ClassifierMessage>,
            tokio::sync::oneshot::Sender<
                Result<String, intellihelper_workspace::permission::ClassifierFailure>,
            >,
        )>();
            ...
            while let Some((messages, respond_to)) = rx.recv().await {
                let result = async {
                    let (sampling_client, model) = match &aux_classifier_sampler {
                        ...
                        None => {
                            let client = session
                                ...
                                .await
                                .map_err(|e| intellihelper_workspace::permission::ClassifierFailure::TransportError(
                                    e.to_string(),
                                ))?;
                            let model = session
                                .chat_state_handle
                    ...
                    let items = messages
                        .into_iter()
                        .map(|m| match m.role {
                            intellihelper_workspace::permission::ClassifierMessageRole::System => {
                                ConversationItem::system(m.text)
                            }
                            intellihelper_workspace::permission::ClassifierMessageRole::User => {
                    ...
                    let request = ConversationRequest {
                        ...
                        json_schema: Some(
                            intellihelper_workspace::permission::classifier_output_json_schema(),
                        ),
                        reasoning_effort: classifier_reasoning_effort,
                        x_intellihelper_conv_id: Some(
                            format!("perm-classifier-{}", uuid::Uuid::new_v4()),
                        ),
                        x_intellihelper_req_id: Some(
                            format!("xai-perm-auto-{}", uuid::Uuid::new_v4()),
                        ),
                        x_intellihelper_session_id: Some(session_id),
                        x_intellihelper_agent_id: Some(intellihelper_telemetry::id::agent_id()),
                    ...
                    let response = tokio::time::timeout(classify_timeout, fut)
                        .await
                        .map_err(|_| {
                            intellihelper_workspace::permission::ClassifierFailure::Timeout
                        })?
                        .map_err(|e| intellihelper_workspace::permission::ClassifierFailure::TransportError(
                            e.to_string(),
                        ))?;
        ...
        let clf =
            intellihelper_workspace::permission::LlmPermissionClassifier::with_channel(tx, prompt_type);
        debug_assert!(
            clf.has_side_query(),
    ...
    pub(super) async fn resolve_aux_sampler_config(
        &self,
        slug: &str,
    ) -> Option<intellihelper_sampler::SamplerConfig> {
        let creds = self.chat_state_handle.get_credentials().await;
    ...
    async fn resolve_auto_classifier_sampler(
        &self,
        slug: &str,
    ) -> Option<(intellihelper_sampler::SamplingClient, String)> {
        ...
        let client = intellihelper_sampler::SamplingClient::new(cfg)
            .map_err(|e| {
    ...
    pub(super) async fn prepare_chat_completion(
        &self,
        force_http1: bool,
    ) -> Result<intellihelper_sampler::SamplingClient, acp::Error> {
        self.refresh_token_if_expired().await;
        let mut full_config = self.reconstruct_full_config().await;
        full_config.force_http1 = force_http1;
        let sampling_client =
            intellihelper_sampler::SamplingClient::new(full_config).map_err(|e| self.to_acp_error(e))?;
        Ok(sampling_client)
    }
    ...
    /// `intellihelper-sampler` instead of constructing a new
    /// `OaiCompatClient`.
    ///
    ...
    fn log_terminal_failure(&self, error_type: &str, status_code: Option<u16>, message: &str) {
        ...
        intellihelper_telemetry::unified_log::warn(
            ...
            Some(serde_json::json!({
                ...
                "key_prefix": auth.as_ref().map(|a| intellihelper_auth::bearer_suffix(&a.key).to_owned()),
                "expires_at": auth
                    .as_ref()
    ...
    pub(crate) async fn handle_sampling_failure(
        self: &Arc<Self>,
        error: intellihelper_sampler::SamplingErrorInfo,
    ) -> Result<SamplerFailureRecovery, acp::Error> {
        use intellihelper_sampler::SamplingErrorKind;
        if self.tool_context.task_output_token_budget.is_some() {
        ...
        let client_version = intellihelper_version::VERSION;
        if auth_mode == crate::auth::AuthMode::WebLogin {
            let msg = format!(
                "{detailed_message}\n\n\
                 ...
                 To fix: run `intellihelper logout` then `intellihelper login` to re-authenticate with OAuth2.\n\n\
        ...
        let error_type = if intellihelper_sampling_types::is_context_length_error(&error.message) {
            "context_length"
        } else {
            error.kind.as_str()
        };
    ...
    pub(crate) async fn run_turn_via_sampler(
        ...
        let request_id = intellihelper_sampler::RequestId::random();
            ...
            .await
        {
            ...
            Err(rich_err) => {
                self.turn_stream_drained.lock().take();
                let info = intellihelper_sampler::SamplingErrorInfo::from(&rich_err);
    ...
    pub(crate) fn record_response_token_usage(
        ...
        if let Some(ref u) = response.usage {
            ...
            crate::token_usage_store::record_model_call(
...
fn resolve_configured_cutoff(
    seed: Option<intellihelper_sampling_types::ToolOverrides>,
    base: Option<&intellihelper_sampling_types::ToolOverrides>,
) -> intellihelper_sampling_types::ToolOverrides {
    use intellihelper_sampling_types::{ToolOverrides, WebSearchOptions, XSearchOptions};
    let ToolOverrides {
        x_search: seed_x,
        web_search: seed_w,
...
mod configured_cutoff_tests {
    ...
    fn inherited_cutoff_agrees_with_the_wire_echo() {
        use intellihelper_sampling_types::{HostedTool, apply_tool_overrides};
