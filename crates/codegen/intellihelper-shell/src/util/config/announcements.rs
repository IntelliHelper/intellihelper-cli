...
/// Re-exported from `intellihelper-announcements` for backward compatibility.
pub use intellihelper_announcements::RemoteAnnouncement;
...
/// Dev/test override for announcements via `INTELLIHELPER_ANNOUNCEMENTS_OVERRIDE` (a JSON
...
pub(crate) fn announcements_override() -> Option<Vec<RemoteAnnouncement>> {
    let raw = std::env::var("INTELLIHELPER_ANNOUNCEMENTS_OVERRIDE").ok()?;
    match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
        Ok(list) => Some(list),
        Err(_) => {
            tracing::warn!("invalid INTELLIHELPER_ANNOUNCEMENTS_OVERRIDE JSON; ignoring");
            None
        }
    }
}
...
/// `INTELLIHELPER_ANNOUNCEMENTS_OVERRIDE` env var overrides everything (dev-only escape hatch).
