...
//! lookup must be cheaper than re-loading from `~/.intellihelper/config.toml`.
...
//! `intellihelper_shell::util::config::set_theme()` (and friends), invoked
...
static CURRENT: AtomicU8 = AtomicU8::new(ThemeKind::IntelliHelperNight as u8);
...
/// back to `IntelliHelperNight` if the byte is somehow out of range (which
...
fn theme_kind_from_u8(byte: u8) -> ThemeKind {
    match byte {
        x if x == ThemeKind::IntelliHelperNight as u8 => ThemeKind::IntelliHelperNight,
        x if x == ThemeKind::IntelliHelperDay as u8 => ThemeKind::IntelliHelperDay,
        ...
        _ => ThemeKind::IntelliHelperNight,
    }
}
...
/// When `None`, `to_theme_kind()` defaults to `IntelliHelperNight` / `IntelliHelperDay`.
...
///
/// On the first call, reads from `~/.intellihelper/config.toml` (via the shell's
...
pub fn current_kind() -> ThemeKind {
    ...
    if terminal_native_locked() {
        return ThemeKind::IntelliHelperNight;
    }
...
pub fn set_terminal_native_lock(locked: bool) {
    ...
    intellihelper_markdown::set_color_level_cap(if locked {
        intellihelper_markdown::ColorLevel::Basic
    } else {
        intellihelper_markdown::ColorLevel::TrueColor
    });
    intellihelper_markdown::set_polarity_safe_syntax(locked);
}
...
///
/// Precedence:
/// 1. Environment variable (`INTELLIHELPER_THEME` / `LC_INTELLIHELPER_THEME`)
/// 2. Config file (`[ui].theme`)
/// 3. Default: `IntelliHelperNight`
#[must_use]
...
fn env_theme_name_from(env: &HashMap<String, String>) -> Option<&str> {
    for key in ["INTELLIHELPER_THEME", "LC_INTELLIHELPER_THEME"] {
        let Some(raw) = env
            .get(key)
            .map(String::as_str)
...
fn resolve_from_config(config_theme: Option<ThemeKind>, osc11_fallback: bool) -> ThemeKind {
    ...
    // Default: IntelliHelperNight
    ThemeKind::IntelliHelperNight
}
...
fn resolve_from_appearance(appearance: Option<system_appearance::SystemAppearance>) -> ThemeKind {
    ...
    appearance
        ...
        .unwrap_or(ThemeKind::IntelliHelperNight)
}
...
/// and the user's dark/light theme mapping. Falls back to `IntelliHelperNight`
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `intellihelper_shell::util::config::set_theme()` (and
...
fn load_from_disk() -> Option<ThemeKind> {
    let root = intellihelper_config::load_effective_config_disk_only().ok()?;
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = intellihelper_config::load_effective_config_disk_only() else {
...
pub fn reset_for_test() {
    ...
    CURRENT.store(ThemeKind::IntelliHelperNight as u8, Ordering::Relaxed);
...
pub fn pin_theme() -> std::sync::MutexGuard<'static, ()> {
    ...
    set(ThemeKind::IntelliHelperNight);
...
mod tests {
    ...
    fn terminal_native_lock_pins_kind_and_blocks_apply_kind() {
        with_test_env(|| {
            set(ThemeKind::IntelliHelperDay);
            set_terminal_native_lock(true);
            assert!(terminal_native_locked());
            assert_eq!(current_kind(), ThemeKind::IntelliHelperNight, "nominal kind");

            let applied = super::super::Theme::apply_kind(ThemeKind::IntelliHelperDay);
            assert_eq!(applied, ThemeKind::IntelliHelperNight, "apply_kind must no-op");
            assert_eq!(current_kind(), ThemeKind::IntelliHelperNight);

            set_terminal_native_lock(false);
            assert_eq!(
                current_kind(),
                ThemeKind::IntelliHelperDay,
    ...
    fn terminal_native_lock_serves_terminal_default_palette() {
        with_test_env(|| {
            ...
            assert_ne!(
                theme.text_primary,
                super::super::Theme::intellihelperday().text_primary,
                "must not serve the cached (IntelliHelperDay) theme"
            );
        });
    }
    ...
    fn terminal_native_lock_enables_polarity_safe_syntax() {
        with_test_env(|| {
            assert!(!intellihelper_markdown::polarity_safe_syntax());
            set_terminal_native_lock(true);
            assert!(
                intellihelper_markdown::polarity_safe_syntax(),
    ...
    fn terminal_native_lock_caps_quantize_at_ansi16() {
        ...
        with_test_env(|| {
            ...
            for input in [
                Color::Rgb(0x26, 0x26, 0x26), // intellihelperday text_primary
    ...
    fn resolve_no_osc11_explicit_auto_and_default() {
        with_test_env(|| {
            assert_eq!(
                resolve_from_config(Some(ThemeKind::IntelliHelperDay), false),
                ThemeKind::IntelliHelperDay
            );
            assert!(!is_auto_mode());
            ...
            assert_eq!(resolve_from_config(None, false), ThemeKind::IntelliHelperNight);
        });
    }
    ...
    #[test]
    fn resolve_auto_dark_system_returns_intellihelpernight() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Dark));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::IntelliHelperNight);
        });
    }

    #[test]
    fn resolve_auto_light_system_returns_intellihelperday() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Light));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::IntelliHelperDay);
        });
    }

    #[test]
    fn resolve_auto_detection_failure_returns_intellihelpernight() {
        with_test_env(|| {
    ...
    #[test]
    fn resolve_from_config_no_config_returns_intellihelpernight() {
        with_test_env(|| {
    ...
    fn resolve_from_config_explicit_theme_returns_it() {
        with_test_env(|| {
            let result = resolve_from_config(Some(ThemeKind::IntelliHelperDay), true);
            assert_eq!(result, ThemeKind::IntelliHelperDay);
            assert!(
                !is_auto_mode(),
    ...
    fn env_theme_overrides_config() {
        with_test_env(|| {
            assert_eq!(
                resolve_initial_theme_from(Some("intellihelperday"), Some(ThemeKind::TokyoNight), false),
                ThemeKind::IntelliHelperDay
            );
            assert!(!is_auto_mode());
        });
    ...
    fn unknown_env_theme_falls_through_to_config() {
        with_test_env(|| {
            assert_eq!(
                resolve_initial_theme_from(Some("not-a-theme"), Some(ThemeKind::IntelliHelperDay), false),
                ThemeKind::IntelliHelperDay
            );
        });
    }
    ...
    #[test]
    fn intellihelper_theme_wins_over_lc_and_config() {
        with_test_env(|| {
            let env = theme_env(&[("INTELLIHELPER_THEME", "intellihelperday"), ("LC_INTELLIHELPER_THEME", "tokyonight")]);
            assert_eq!(
    ...
    fn empty_or_absent_grok_theme_falls_through_to_lc() {
        with_test_env(|| {
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[
                        ("INTELLIHELPER_THEME", ""),
                        ("LC_INTELLIHELPER_THEME", "intellihelperday")
                    ])),
                    Some(ThemeKind::TokyoNight),
                    false,
                ),
            ...
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[("LC_INTELLIHELPER_THEME", "intellihelperday")])),
                    Some(ThemeKind::TokyoNight),
                    true,
                ),
                ThemeKind::IntelliHelperDay
    ...
    fn unknown_grok_theme_falls_through_to_valid_lc() {
        with_test_env(|| {
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[
                        ("INTELLIHELPER_THEME", "not-a-theme"),
                        ("LC_INTELLIHELPER_THEME", "intellihelperday"),
                    ])),
                    Some(ThemeKind::TokyoNight),
                    false,
        ...
        with_test_env(|| {
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[
                        ("INTELLIHELPER_THEME", "not-a-theme"),
                        ("LC_INTELLIHELPER_THEME", "")
                    ])),
                    Some(ThemeKind::IntelliHelperDay),
                    false,
                ),
                ThemeKind::IntelliHelperDay
            );
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[("INTELLIHELPER_THEME", ""), ("LC_INTELLIHELPER_THEME", "")])),
                    Some(ThemeKind::IntelliHelperDay),
                    true,
                ),
                ThemeKind::IntelliHelperDay
    ...
    fn env_theme_system_alias_arms_auto_on_both_resolve_paths() {
        with_test_env(|| {
            ...
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[("INTELLIHELPER_THEME", "system")])),
                    Some(ThemeKind::TokyoNight),
                    false,
                ),
                ThemeKind::IntelliHelperDay
        ...
        with_test_env(|| {
            ...
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[("LC_INTELLIHELPER_THEME", "auto")])),
                    Some(ThemeKind::TokyoNight),
                    true,
                ),
                ThemeKind::IntelliHelperNight
    ...
    fn set_then_current_kind_round_trips() {
        with_test_env(|| {
            ...
            set(ThemeKind::IntelliHelperDay);
            assert_eq!(current_kind(), ThemeKind::IntelliHelperDay);
        });
    }
