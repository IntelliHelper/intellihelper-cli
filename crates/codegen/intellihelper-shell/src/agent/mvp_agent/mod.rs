...
use intellihelper_telemetry::id::{agent_id, agent_instance_id};
use intellihelper_telemetry::session_ctx::log_event;
use intellihelper_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use intellihelper_workspace::permission::{ClientType, PermissionEvent};
...
use intellihelper_workspace::session::git::GitDiscoveryResult;
...
pub(crate) fn resolve_subscription_tier_for_telemetry(
    ...
    auth: Option<&crate::auth::IntelliHelperAuth>,
) -> Option<String> {
...
pub(crate) fn jwt_claim_matches_user_subscription_tier(
    ...
    match user_subscription_tier {
        "IntelliHelperPro" => jwt_claim == "supergrok",
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a intellihelper_sampling_types::TokenUsage>,
...
fn announcements_push_payload(
    stored: Option<&[intellihelper_announcements::RemoteAnnouncement]>,
    last_emitted: &[intellihelper_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<intellihelper_announcements::RemoteAnnouncement>> {
    let current = intellihelper_announcements::filter_expired_at(
...
/// Override with `INTELLIHELPER_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn announcements_refresh_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("INTELLIHELPER_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS")
...
pub struct MvpAgent {
    ...
    /// intellihelper.com chat-product catalog (`/rest/modes`) for chat sessions; distinct
    ...
    /// list. Set by `enforce_intellihelper_code_access`; defaults to `true` (API-key and
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.intellihelper/`).
    ///
    ...
    /// Local session-registry override: `INTELLIHELPER_SESSION_REGISTRY` env, else
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        intellihelper_tools::implementations::intellihelper_build::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: intellihelper_tools::implementations::intellihelper_build::monitor::types::MonitorEventBuffer,
    ...
    pub(crate) plugin_registry_handle: intellihelper_agent::plugins::SharedPluginRegistryHandle,
    ...
    /// cwd→git root plus user/marketplace dirs and stalled intellihelper-desktop's first
    ...
    workspace_ops: RefCell<Option<intellihelper_workspace::WorkspaceOps>>,
    ...
    last_emitted_announcements: RefCell<Vec<intellihelper_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &intellihelper_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<intellihelper_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !intellihelper_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = intellihelper_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
/// unchanged — compatible stock switches (e.g. `intellihelper-build` →
/// `intellihelper-build-plan`) intentionally preserve the session's original ACP
/// `agentProfile`.
...
/// default wire format and toolset, so switching e.g. `intellihelper-build` →
/// `intellihelper-build-plan` doesn't require rebuilding the harness and would
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use intellihelper_agent::config::is_strict_harness_agent_type;
    match (
...
///
/// Headers injected:
///  - `x-intellihelper-client-version` -- required by the proxy's version-gate check.
...
fn inject_proxy_headers(
    ...
    headers
        .entry("x-intellihelper-client-version".to_string())
        .or_insert_with(|| {
            client_version
                .map(String::from)
    ...
    headers
        .entry("x-intellihelper-client-identifier".to_string())
...
impl MvpAgent {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = intellihelper_tools::types::TaskSnapshot {
    ...
    pub(super) async fn enforce_intellihelper_code_access(&self, auth: &crate::auth::IntelliHelperAuth) {
        if !auth.is_xai_auth() {
        ...
        *self.allow_access_resolved_for.borrow_mut() = Some(auth.user_id.clone());
        if !allow {
            tracing::info!(
                "auth: user blocked by allow_access (remote settings intellihelper_build_access_gate)"
            );
    ...
    /// cached_token, intellihelper.com/oidc); the cheap pre-checks below absorb repeats
    ...
    ///    against `~/.intellihelper/bundled/` and the manifest.
