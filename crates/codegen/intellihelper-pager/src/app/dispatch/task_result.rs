...
use super::status::{
    ...
    handle_coding_data_sharing_updated, handle_context_info_complete, handle_optimize_prompt_done,
...
/// Whether a completed clipboard probe should fall through to the `intellihelper wrap`
/// host-image request. A clean `FullMiss` always qualifies; a remote read
/// *error* (`AttachmentRead`) also qualifies because inside `intellihelper wrap` the
...
/// `osc52_sink_active()`, so this is inert outside `intellihelper wrap`.
    ...
    match result {
        ...
        TaskResult::ForeignResumeCwdCanonicalized {
                ...
                && let Some(canonical_cwd) = accepted_cwd
            {
                vec![Effect::DetectForeignResumeHint {
                    ...
                    intellihelper_home: intellihelper_tools::util::intellihelper_home::intellihelper_home(),
                    launch_token,
                }]
            } else {
                vec![]
        ...
        TaskResult::ChangelogFetched { markdown, entries } => {
            ...
            app.changelog_bullets =
                intellihelper_shell::util::changelog::bullets_from_entries(&entries, 3);
            vec![]
        }
        ...
        TaskResult::SessionUsageComplete {
            ...
            let total = usage.totals.total_tokens;
        ...
        TaskResult::OptimizePromptDone {
            ...
            original_draft,
            result,
        } => handle_optimize_prompt_done(app, agent_id, generation, original_draft, result),
