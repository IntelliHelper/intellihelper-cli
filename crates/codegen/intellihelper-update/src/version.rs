...
use intellihelper_shell::env::IntelliHelperBuildEnvironment;
...
///
/// Constructed once from `IntelliHelperBuildEnvironment` at startup and threaded through the
/// update call chain so that `auto_update` and `version` never need to know
/// about the `IntelliHelperBuildEnvironment` enum directly.
#[derive(Debug, Clone)]
pub struct UpdateConfig {
    /// Chat API proxy base URL (versioned `https://cli-chat-proxy.grok.com/v1` endpoint).
    pub proxy_base_url: String,
    /// Auth scope key for `~/.intellihelper/auth.json`.
    pub auth_scope: String,
    /// Enterprise deployment key (INTELLIHELPER_DEPLOYMENT_KEY).
...
impl UpdateConfig {
    pub fn from_environment(env: &IntelliHelperBuildEnvironment) -> Self {
        Self {
            proxy_base_url: env.cli_chat_proxy_base_url(),
            auth_scope: intellihelper_shell::auth::GrokComConfig::default().auth_scope(),
            deployment_key: None,
            alpha_test_key: None,
...
pub async fn write_version_cache(version: &str, stable_version: Option<&str>) {
    let version_path = intellihelper_home().join("version.json");
    let now = time::OffsetDateTime::now_utc();
    let json = IntelliHelperVersion::new(
        version.to_string(),
...
pub async fn is_version_cache_fresh() -> bool {
    ...
    if let Ok(version_str) = fs::read_to_string(&version_path).await
        && let Ok(version) = serde_json::from_str::<IntelliHelperVersion>(&version_str)
...
/// link whose target binary was deleted (e.g. manual `~/.intellihelper/downloads`
...
pub fn installed_on_disk_version() -> Option<String> {
    #[cfg(unix)]
    {
        let app = intellihelper_shell::util::intellihelper_home::intellihelper_application();
...
///
/// Handles the internal layout (`intellihelper-0.1.150-macos-aarch64`, including
/// pre-releases: `intellihelper-0.1.150-alpha.1-linux-x86_64` → `0.1.150-alpha.1`)
/// and the npm layout without a platform suffix (`intellihelper-0.1.150`,
/// `intellihelper-0.1.150-alpha.1`): everything between the `{bin_prefix}-` prefix
/// and the first platform-OS component is the version, validated as semver
/// so unknown layouts (`intellihelper-latest`, `intellihelper-pager-*` when `bin_prefix` is
/// `intellihelper`) return `None` instead of garbage.
///
...
/// Read the cached stable version from `~/.intellihelper/version.json` (sync, for display).
///
...
pub fn cached_stable_version() -> Option<String> {
    ...
    let gv: IntelliHelperVersion = serde_json::from_str(&content).ok()?;
    gv.stable_version
}
...
pub fn channel_name() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        derive_channel(intellihelper_version::VERSION, &stable)
    })
}
...
/// `~/.intellihelper/version.json` (written by the auto-updater):
...
pub fn channel_label() -> &'static str {
    ...
    LABEL.get_or_init(|| {
        ...
        match derive_channel(intellihelper_version::VERSION, &stable) {
            Some("alpha") => " [alpha]",
            Some(_) => " [stable]",
            None => "",
        }
...
mod tests {
    ...
    fn test_is_fresh_rejects_future_timestamp() {
        ...
        let v = IntelliHelperVersion::new("0.1.200".to_string(), None, future);
        assert!(
    ...
    /// symlink-target file name (`intellihelper-<version>-<platform>`).
    #[test]
    fn test_version_from_versioned_binary_name() {
        let cases: &[(&str, Option<&str>)] = &[
            ("intellihelper-0.2.46-darwin-arm64", Some("0.2.46")),
            ("intellihelper-0.1.220-linux-x86_64", Some("0.1.220")),
            ("intellihelper-0.2.5-windows-x86_64.exe", Some("0.2.5")),
            ...
            ("intellihelper-0.1.220-alpha.4-linux-x86_64", Some("0.1.220-alpha.4")),
            ("intellihelper-0.1.220-alpha.4", Some("0.1.220-alpha.4")), // npm layout
            ("intellihelper-pager-0.1.5-darwin-arm64", None),           // "pager" is not a version
            ("intellihelper-garbage-darwin-arm64", None),               // unparseable version
            ("intellihelper-0.2.46", Some("0.2.46")),                   // no platform suffix
            ("other-0.2.46-darwin-arm64", None),               // wrong prefix
            ("intellihelper-latest", None),                             // symlink alias, not a version
            ("intellihelper", None),                                    // bare name
            ("", None),
        ];
        for (name, expected) in cases {
            assert_eq!(
                version_from_versioned_binary_name(name, "intellihelper").as_deref(),
                *expected,
        ...
        // prefix but not under "intellihelper".
        assert_eq!(
            version_from_versioned_binary_name("intellihelper-pager-0.1.5-darwin-arm64", "intellihelper-pager")
                .as_deref(),
            Some("0.1.5")
        );
    }
    ...
    // ──────────────────────────────────────────────────────────────────────
    // IntelliHelperVersion JSON shape — backward compatibility invariants
    // ──────────────────────────────────────────────────────────────────────
    ...
    fn test_version_json_backward_compat() {
        ...
        let v: IntelliHelperVersion = serde_json::from_str(old).unwrap();
        assert_eq!(v.version, "0.1.180");
        assert!(v.stable_version.is_none());

        // New format with stable_version round-trips correctly.
        let now = time::OffsetDateTime::now_utc();
        let new = IntelliHelperVersion::new("0.2.5".to_string(), Some("0.2.3".to_string()), now);
        let json = serde_json::to_string(&new).unwrap();
        let parsed: IntelliHelperVersion = serde_json::from_str(&json).unwrap();
        ...
        assert!(serde_json::from_str::<IntelliHelperVersion>(future).is_ok());

        // Missing required field (checked_at) is rejected.
        let missing = r#"{"version":"0.1.180"}"#;
        assert!(serde_json::from_str::<IntelliHelperVersion>(missing).is_err());
    }
    ...
    fn test_is_fresh_ttl_boundaries() {
        let now = time::OffsetDateTime::now_utc();
        let v = IntelliHelperVersion::new("0.1.200".to_string(), None, now);
    ...
    fn test_update_config_default_channel_is_stable() {
        use intellihelper_shell::env::IntelliHelperBuildEnvironment;
        let cfg = UpdateConfig::from_environment(&IntelliHelperBuildEnvironment::Production);
