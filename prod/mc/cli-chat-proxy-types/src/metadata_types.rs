...
/// v1.18: Added `shell_version`: the intellihelper-shell agent binary version, distinct
///        from `client_version` (the UI client's version). They coincide for the
///        TUI but differ for embedding clients like intellihelper-desktop.
...
/// OS-level sandbox state for a trace turn (local `intellihelper-sandbox`, not cloud sandbox).
...
pub struct PromptMetadata {
    ...
    /// The agent type / harness name for this session (e.g. "intellihelper-build", "codex").
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub agent_type: Option<String>,
    /// Version of the intellihelper-shell agent binary that handled this turn
    /// (`intellihelper_version::VERSION`). Self-reported by the agent, so it reflects
    ...
    /// like intellihelper-desktop the bundled shell differs from the app version.
    #[serde(default, skip_serializing_if = "Option::is_none")]
