...
//!
//! Knobs: INTELLIHELPER_PERF_GIT_FILES (default 300), INTELLIHELPER_PERF_GIT_PICKS (default 6).
...
fn install_global_scan_counter() -> ScanCounter {
    // INTELLIHELPER_E2E_LOG=<filter> tees shell logs to stderr for local debugging.
    let filter = std::env::var("INTELLIHELPER_E2E_LOG").ok();
    ScanCounter(
...
async fn run_storm(
    ...
    let files = env_usize("INTELLIHELPER_PERF_GIT_FILES", 300);
    let picks = env_usize("INTELLIHELPER_PERF_GIT_PICKS", 6);
