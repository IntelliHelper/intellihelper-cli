...
pub use self::types::*;
pub use intellihelper_sampler::ApiBackend;
pub use intellihelper_sampler::SamplingClient as Client;

// Re-export async-openai Responses API types under `rs` namespace
pub use async_openai::types::responses as rs;

// ---------------------------------------------------------------------------
// intellihelper-sampler re-exports
// ---------------------------------------------------------------------------
//
// The actual streaming / retry / HTTP-client logic lives in the
// `intellihelper-sampler` crate. We re-export the public surface here so
...
// `intellihelper_sampler::*`. The shell-side `sampling::client::Config`
