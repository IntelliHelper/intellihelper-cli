...
use intellihelper_tools::types::{
    output::{MCPOutput, MCPOutputDetails, ToolOutput},
    tool::{ToolKind, ToolNamespace},
    tool_metadata::ToolMetadata,
};
use intellihelper_tools::util::{ProcessGroup, ProcessScope};

/// MCP tool name delimiter: server names are qualified as `"server__tool"`.
/// Canonical definition lives in `intellihelper_workspace_types`; re-exported here
/// for callers that historically imported it from this module.
pub use intellihelper_workspace_types::MCP_TOOL_NAME_DELIMITER;

/// Reqwest 0.13 adapter over `intellihelper_extra_ca::extra_root_ders` (DER is version-neutral).
fn with_extra_root_certificates(mut builder: reqwest::ClientBuilder) -> reqwest::ClientBuilder {
    for der in intellihelper_extra_ca::extra_root_ders() {
        match reqwest::Certificate::from_der(der) {
            Ok(cert) => builder = builder.add_root_certificate(cert),
            Err(e) => tracing::warn!(
                error = %e,
                "INTELLIHELPER_EXTRA_CA_BUNDLE: validated DER rejected by reqwest 0.13; skipping cert"
            ),
        }
    }
    builder
...
pub struct McpState {
    ...
    /// Persisted to `~/.intellihelper/config.toml` under `[mcp_servers.<name>].disabled_tools`.
    ...
    event_writer: file_utils::events::EventWriter,
...
impl McpState {
    ...
    pub fn new_with_meta(configs: Vec<acp::McpServer>, meta_config_map: McpMetaConfigMap) -> Self {
        Self {
            ...
            event_writer: file_utils::events::EventWriter::noop(),
    ...
    /// `IntelliHelperClientHandler` cloned during `try_handshake` reads
    ...
    pub fn set_event_writer(&mut self, writer: file_utils::events::EventWriter) {
        self.event_writer = writer;
    }

    pub fn event_writer(&self) -> &file_utils::events::EventWriter {
        &self.event_writer
    }
...
/// MCP initialization strategy. Defined in `intellihelper-telemetry`; re-exported
/// here so existing call sites continue to work.
pub use intellihelper_telemetry::enums::McpInitStrategy;
...
impl McpError {
    ...
    pub fn error_category(&self) -> file_utils::events::McpErrorCategory {
        use file_utils::events::McpErrorCategory;
        match self {
...
impl tool_runtime::Tool for McpErasedTool {
    ...
    async fn run(
        ...
        event_writer.emit(file_utils::events::Event::McpToolCallStarted {
            ...
            .await
        {
            ...
            Err(first_err) if client.has_auth() => {
                ...
                ew.emit(file_utils::events::Event::McpAuthRetry {
                    server_name: server.clone(),
        ...
        let call_result = match dispatch_result {
            Ok(result) => result,
            Err(e) => {
                ew.emit(file_utils::events::Event::McpToolCallCompleted {
                    server_name: server.clone(),
                    tool_name: tool.clone(),
        ...
        event_writer.emit(file_utils::events::Event::McpToolCallCompleted {
            server_name: server.clone(),
            tool_name: tool.clone(),
            call_id: qualified_name.clone(),
            duration_ms,
        ...
        intellihelper_telemetry::session_ctx::log_event(intellihelper_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
impl McpErasedTool {
    async fn try_call_tool(
        ...
        is_timeout: &mut bool,
        ew: &file_utils::events::EventWriter,
    ...
    async fn recover_and_retry(
        ...
        *reconnect_attempted = true;
        ...
        ew.emit(file_utils::events::Event::McpTransportError {
        ...
        let mcp_service = match client.recover().await {
            Ok(service) => {
                ew.emit(file_utils::events::Event::McpTransportReconnect {
                    server_name: self.tool.server_name.clone(),
                    success: true,
                    error: None,
                });
    ...
    W: AsyncWrite + Send + Unpin + 'static,
{
    ...
    fn record_decode_error(&self, line: &[u8], err: &serde_json::Error) {
        ...
        self.event_writer
            .emit(file_utils::events::Event::McpTransportDecodeError {
...
/// Uses [`IntelliHelperClientHandler`] rather than rmcp's default `ClientInfo`
...
pub type McpService = Arc<RunningService<RoleClient, IntelliHelperClientHandler>>;
...
///    (`TransportClosed`).
/// 2. [`IntelliHelperClientHandler`] when the server pushes a notification we
...
pub struct McpClient {
    ...
    /// [`IntelliHelperClientHandler`]** constructed by
...
/// and the [`IntelliHelperClientHandler`] it constructs during
...
/// External-config overrides for an MCP server, surfaced to intellihelper-mcp
...
impl McpClient {
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: intellihelper_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
    ...
    async fn try_handshake(
        &self,
        pending: PendingTransport,
    ) -> Result<rmcp::service::RunningService<RoleClient, IntelliHelperClientHandler>, McpError> {
    ...
    fn make_client_info(server_name: &str) -> ClientInfo {
        ...
        ClientInfo::new(
            capabilities,
            Implementation::new(
                format!("intellihelper-shell-{server_name}"),
    ...
    /// Build the [`IntelliHelperClientHandler`] that drives `client.serve(...)`.
    ///
    ...
    fn make_client_handler(&self) -> IntelliHelperClientHandler {
        IntelliHelperClientHandler {
    ...
    /// references (the [`IntelliHelperClientHandler`] handed to
    ...
    pub async fn get_tool_registrations(
        ...
        let _ensure_init_timer =
            intellihelper_telemetry::instrumentation::timer("mcp_ensure_initialized");
        let mcp_service = self.ensure_initialized().await?;

        let mut all_tools = Vec::new();
        let mut cursor: Option<String> = None;

        let _list_tools_timer = intellihelper_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
/// Copy an MCP server's stderr to `~/.intellihelper/logs/mcp/<server>.stderr.log`
/// in a background task. Truncated per spawn.
fn drain_mcp_stderr_to_log(server_name: &str, mut stderr: tokio::process::ChildStderr) {
    let log_dir = intellihelper_config::intellihelper_home().join("logs").join("mcp");
...
fn ensure_figma_user_agent(headers: &mut reqwest::header::HeaderMap, server_name: &str, url: &str) {
    ...
    headers.insert(
        ...
        reqwest::header::HeaderValue::from_static("intellihelper-cli"),
    );
}
...
pub struct McpSpawnCtx<'a> {
    ...
    pub(crate) event_writer: &'a file_utils::events::EventWriter,
...
impl<'a> McpSpawnCtx<'a> {
    pub fn for_session(
        session_id: &'a str,
        event_writer: &'a file_utils::events::EventWriter,
        mode: OauthInteractivity,
    ...
    pub fn session_less(event_writer: &'a file_utils::events::EventWriter) -> Self {
        Self {
            session_id: None,
            event_writer,
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = intellihelper_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = intellihelper_telemetry::instrumentation::timer("mcp_stdio_spawn");
            ...
            .map_err(|e| {
                ...
                intellihelper_telemetry::session_ctx::log_event(
                    intellihelper_telemetry::events::McpServerFailed {
                        server_name: name.clone(),
                        error_type: intellihelper_telemetry::events::McpErrorType::SpawnFailed,
        ...
        | acp::McpServer::Sse(acp::McpServerSse {
            ...
            let auth_prep = if has_existing_auth {
                ...
                let _auth_discovery_timer =
                    intellihelper_telemetry::instrumentation::timer("mcp_http_auth_discovery");
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = intellihelper_telemetry::instrumentation::timer("mcp_start_servers");
...
#[derive(Debug)]
pub struct IntelliHelperClientHandler {
...
impl IntelliHelperClientHandler {
...
impl ClientHandler for IntelliHelperClientHandler {
    ...
    // See the [`IntelliHelperClientHandler`] doc-comment for the full RPIT
    // contract.
...
mod tests {
    ...
    async fn resilient_transport_skips_undecodable_line_and_keeps_stream_alive() {
        ...
        let mut transport = ResilientRwTransport::new(
            ...
            tokio::io::sink(),
            "fwbuild".to_string(),
            file_utils::events::EventWriter::noop(),
        );
    ...
    fn is_figma_mcp_matches_name_and_host() {
        ...
        assert!(is_figma_mcp("INTELLIHELPER_COM_FIGMA", "https://example.com/mcp"));
        assert!(is_figma_mcp("grok_com_FIGMA", "https://example.com/mcp"));
    ...
    #[test]
    fn ensure_figma_user_agent_sets_intellihelper_cli_when_missing() {
        let mut headers = reqwest::header::HeaderMap::new();
    ...
    async fn recover_and_retry_surfaces_original_error_when_recover_fails() {
        ...
        let mut reconnect_attempted = false;
        let mut is_timeout = false;
        let ew = file_utils::events::EventWriter::noop();
    ...
    async fn try_call_tool_http_mcperror_recovers_then_retry_succeeds() {
        ...
        let ew = file_utils::events::EventWriter::open(tmp.path());
    ...
    async fn try_call_tool_reconnects_then_succeeds_after_retriable_transport_error() {
        ...
        async fn dead_service() -> McpService {
            ...
            let handler = IntelliHelperClientHandler {
    ...
    fn auth_required_records_as_auth_not_init_failed_and_maps_category() {
        ...
        assert!(matches!(
            err.error_category(),
            file_utils::events::McpErrorCategory::AuthRequired
        ));
    }
