...
use intellihelper_sampler::RetryPolicy;
use intellihelper_sampling_types::doom_loop::{DoomLoopSignalKind, SAMPLE_CHECK_EVENT_DATA_CUMULATIVE};
...
use intellihelper_test_support::{MockInferenceServer, MockModelEntry, ScriptedResponse};
...
/// `x-intellihelper-turn-idx`) carries the opt-in header. Aux side-queries the binary
...
/// ```bash
/// cargo test -p intellihelper-shell --test test_doom_loop_recovery -- --ignored
/// ```
#[tokio::test]
...
async fn headless_config_enables_doom_loop_check_header() {
    ...
    let workdir = intellihelper_test_support::git_workdir();
    let sandbox = intellihelper_test_support::TestSandbox::builder()
        .mock_url(server.url())
        .build();
    ...
    let mut cmd = tokio::process::Command::new(intellihelper_test_support::intellihelper_binary());
    ...
    let result = intellihelper_test_support::run_headless_in_sandbox(cmd, sandbox).await;
    intellihelper_test_support::assert_headless_success(&result, "doom-loop header e2e", Some(&server));
    ...
    let responses_posts: Vec<_> = requests
        ...
        .filter(|e| e.method == "POST" && e.path.contains("/responses"))
        .collect();
    // The session turn carries `x-intellihelper-turn-idx`; aux side-queries (session
    // title, etc.) do not.
    let (turns, aux): (Vec<_>, Vec<_>) = responses_posts
        .into_iter()
        .partition(|e| e.header("x-intellihelper-turn-idx").is_some());
    assert!(
        !turns.is_empty(),
    ...
    for turn in turns {
        assert_eq!(
            turn.header("x-intellihelper-doom-loop-check"),
            Some("true"),
    ...
    for side_query in aux {
        assert_eq!(
            side_query.header("x-intellihelper-doom-loop-check"),
            None::<&str>,
