...
IntelliHelper asks before doing anything risky — and can plan before it codes.

## Permissions

When IntelliHelper wants to run a risky command or edit a file, it pauses and asks:
...
For bigger or more ambiguous tasks, use **plan mode**: IntelliHelper explores the
...
`s` requests changes — IntelliHelper iterates until you're happy, then implements.
...
background — IntelliHelper keeps working and you're notified when it finishes
