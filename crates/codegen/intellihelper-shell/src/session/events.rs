...
//! `file-utils`. The orphan-rule items (`From<&permission::Decision>`
//! and the doom-loop categorizer) stay here since they need shell-local
//! types.

pub(crate) use file_utils::events::tracker::EventTracker;
pub(crate) use file_utils::events::types::{
...
) -> Option<intellihelper_sampling_types::PriorTurnInterrupt> {
    use intellihelper_sampling_types::PriorTurnInterrupt;
    match category {
...
/// committing it would silently run intellihelper-build flavor. Distinct from
...
/// (`file-utils`). The two enums are intentional mirrors and live in
