...
/// after we stop waiting. Mirrors the bound the `intellihelper-tools` local terminal
...
fn notification_interval() -> Duration {
    std::env::var("INTELLIHELPER_TERMINAL_NOTIFICATION_INTERVAL_MS")
        .ok()
...
#[async_trait::async_trait]
impl SessionNotificationSender for acp_lib::AcpAgentGatewaySender {
    async fn session_notification(
        &self,
        notification: acp::SessionNotification,
    ) -> Result<(), acp::Error> {
