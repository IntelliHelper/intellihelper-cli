...
//! agent-session config types in `intellihelper-config` and friends.
...
/// `intellihelper-tools` once the wire surface is firm.
...
///
/// TODO(workspace): align with `intellihelper_config::ProjectConfig`.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
