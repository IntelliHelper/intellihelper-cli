...
Sample hooks for IntelliHelper. Copy to `~/.intellihelper/hooks/` to enable globally, or to `<project>/.intellihelper/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.intellihelper/hooks/bin
cp examples/hooks/safe-shell.json ~/.intellihelper/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.intellihelper/hooks/bin/
chmod +x ~/.intellihelper/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.intellihelper/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.intellihelper/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.intellihelper/hooks/bin/
chmod +x ~/.intellihelper/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.intellihelper/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.intellihelper/hooks/bin
cp examples/hooks/session-log.json ~/.intellihelper/hooks/
cp examples/hooks/bin/session-log.sh ~/.intellihelper/hooks/bin/
chmod +x ~/.intellihelper/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.intellihelper/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.intellihelper/hooks/bin
cp examples/hooks/tool-logger.json ~/.intellihelper/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.intellihelper/hooks/bin/
chmod +x ~/.intellihelper/hooks/bin/tool-logger.sh
```
...
```sh
mkdir -p ~/.intellihelper/hooks/bin
cp examples/hooks/stop-verify.json ~/.intellihelper/hooks/
cp examples/hooks/bin/stop-verify.sh ~/.intellihelper/hooks/bin/
chmod +x ~/.intellihelper/hooks/bin/stop-verify.sh
```
...
- **Event names:** `SessionStart`, `PreToolUse`, `PostToolUse`, `Stop`, `SubagentStop`, `SessionEnd` (see the [user guide](../../intellihelper-pager/docs/user-guide/10-hooks.md) for the full set)
- **Matcher:** regex on tool name. Claude names like `Bash`, `Read`, `Edit` are auto-expanded to also match IntelliHelper names (`run_terminal_cmd`, `read_file`, `search_replace`)
...
Remove the JSON file from `~/.intellihelper/hooks/`. The hook stops running on the next session.