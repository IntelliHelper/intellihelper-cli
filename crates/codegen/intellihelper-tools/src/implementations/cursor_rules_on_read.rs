...
mod tests {
    ...
    use crate::implementations::intellihelper_build::read_file::{
        ReadFileInput, ReadFileParams, ReadFileTool as IntelliHelperReadFileTool,
    };
    ...
    #[tokio::test]
    async fn intellihelper_read_file_output_includes_matching_rule_once() {
        let temp = TempDir::new().unwrap();
        let root = temp.path();
        ...
        let first = tool_runtime::Tool::run(&IntelliHelperReadFileTool, test_ctx(shared), input)
            .await
            .unwrap();
