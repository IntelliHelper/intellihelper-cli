...
pub enum NotificationProtocol {
    ...
    Osc9,
    /// Kitty: `\x1b]99;i=intellihelper;{message}\x1b\\`
    Osc99,
...
pub fn select_protocol(ctx: &TerminalContext) -> NotificationProtocol {
    ...
    match ctx.brand {
        ...
        TerminalName::IntelliHelperDesktop => NotificationProtocol::None,
...
fn notification_sequence(
    ...
    Some(match protocol {
        ...
        // OSC 777 already uses the tab title as subtitle, so keep "IntelliHelper".
        NotificationProtocol::Osc9 => format!("\x1b]9;{body} \u{b7} {title}\x07").into(),
        NotificationProtocol::Osc99 => format!("\x1b]99;i=intellihelper;{body} \u{b7} {title}\x1b\\").into(),
        NotificationProtocol::Osc777 => format!("\x1b]777;notify;IntelliHelper;{body}\x1b\\").into(),
...
mod tests {
    ...
    fn select_grok_desktop_uses_none() {
        assert_eq!(
            select_protocol(&ctx_with_brand(TerminalName::IntelliHelperDesktop)),
    ...
    fn emit_none_is_noop() {
        let ctx = ctx_with_brand(TerminalName::IntelliHelperDesktop);
    ...
    fn notification_sequence_osc99_and_osc777_strip_controls() {
        ...
        assert_eq!(osc99.as_ref(), "\x1b]99;i=intellihelper;b \u{b7} t\x1b\\");

        let osc777 = notification_sequence(NotificationProtocol::Osc777, "ignored\x1b", "b\x1body")
            .expect("osc777 yields a sequence");
        assert_eq!(osc777.as_ref(), "\x1b]777;notify;IntelliHelper;body\x1b\\");
    }
