...
/// Env lane (desktop `INTELLIHELPER_SESSION_LIST_CONVERSATIONS`) OR process-wide
/// `--chat` (`INTELLIHELPER_CHAT_MODE`); hard-off in release builds.
...
mod tests {
    ...
    fn local(session_id: &str, updated_at: &str) -> MergedSession {
        MergedSession {
            ...
            source: "local".into(),
            model_id: Some("intellihelper-build".into()),
            num_messages: 7,
    ...
    fn xai_auth_manager(dir: &std::path::Path) -> std::sync::Arc<crate::auth::AuthManager> {
        ...
        am.hot_swap(crate::auth::IntelliHelperAuth {
    ...
    async fn forced_kind_serves_conversations_only() {
        let addr = spawn_conversations_stub(
            ...
            .await;
        let _env = intellihelper_test_support::EnvGuard::set(
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = intellihelper_test_support::EnvGuard::unset("INTELLIHELPER_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = intellihelper_test_support::EnvGuard::set("INTELLIHELPER_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = intellihelper_test_support::EnvGuard::set("INTELLIHELPER_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        use crate::agent::chat_modes::INTELLIHELPER_CHAT_MODE_ENV;
        let _chat_off = intellihelper_test_support::EnvGuard::unset(INTELLIHELPER_CHAT_MODE_ENV);
        let _desktop_off =
            intellihelper_test_support::EnvGuard::unset("INTELLIHELPER_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _desktop =
                intellihelper_test_support::EnvGuard::set("INTELLIHELPER_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = intellihelper_test_support::EnvGuard::set(INTELLIHELPER_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        .to_string();
        {
            let _off = intellihelper_test_support::EnvGuard::unset(INTELLIHELPER_CHAT_MODE_ENV);
        ...
        {
            let _on = intellihelper_test_support::EnvGuard::set(INTELLIHELPER_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
    ...
    fn relax_rows_scopes_to_repo_and_requires_messages() {
        ...
        let repo_url = intellihelper_workspace::session::git::normalize_repo_url(this_repo).unwrap();
