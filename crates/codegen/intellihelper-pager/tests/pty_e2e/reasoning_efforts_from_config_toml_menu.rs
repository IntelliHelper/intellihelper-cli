...
async fn reasoning_efforts_from_config_toml_menu() {
    let content = ContentController::start_with_models(vec![
        MockModel::new("intellihelper-4.5").with_supports_reasoning_effort(true),
    ])
    .await
    .expect("start content");
    content.set_response(format!("{MOCK_RESPONSE_SENTINEL} turn."));

    // Seed `~/.intellihelper/config.toml` with a per-model reasoning-effort menu.
    ...
    // Quote the dotted model id: bare `[model.intellihelper-4.5]` is TOML key-path syntax (model.intellihelper-4.5), not the id "intellihelper-4.5".
    std::fs::write(
