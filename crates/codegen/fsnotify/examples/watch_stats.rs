...
//! startup latency, under either strategy (`INTELLIHELPER_FSNOTIFY_PER_DIR=0|1`).
//!
//! ```bash
...
//! INTELLIHELPER_FSNOTIFY_PER_DIR=0 cargo run --release -p fsnotify --example watch_stats -- run /tmp/js-repo 5
//! INTELLIHELPER_FSNOTIFY_PER_DIR=1 cargo run --release -p fsnotify --example watch_stats -- run /tmp/js-repo 5
//! ```
//!
...
//!   "intellihelper holds 55k inotify watches" report).
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("run") => {
            ...
            let strategy = std::env::var("INTELLIHELPER_FSNOTIFY_PER_DIR").unwrap_or_default();
