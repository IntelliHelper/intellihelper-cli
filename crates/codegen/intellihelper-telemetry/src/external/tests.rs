...
fn event_names_are_pinned() {
    ...
    let expected: &[(E, &str)] = &[
        (E::SessionStart, "intellihelper_code.session_start"),
        (E::SessionEnd, "intellihelper_code.session_end"),
        (E::UserPrompt, "intellihelper_code.user_prompt"),
        (E::TurnCompleted, "intellihelper_code.turn_completed"),
        (E::ApiRequest, "intellihelper_code.api_request"),
        (E::ApiError, "intellihelper_code.api_error"),
        (E::ToolResult, "intellihelper_code.tool_result"),
        (E::ToolDecision, "intellihelper_code.tool_decision"),
        (E::McpServerConnection, "intellihelper_code.mcp_server_connection"),
        (
            E::PermissionModeChanged,
            "intellihelper_code.permission_mode_changed",
        ),
        (E::SkillActivated, "intellihelper_code.skill_activated"),
        (E::PluginLoaded, "intellihelper_code.plugin_loaded"),
        (E::Compaction, "intellihelper_code.compaction"),
        (E::Subagent, "intellihelper_code.subagent"),
        (E::Auth, "intellihelper_code.auth"),
        (E::InternalError, "intellihelper_code.internal_error"),
        (E::ModelSwitched, "intellihelper_code.model_switched"),
        (E::ContextualTip, "intellihelper_code.contextual_tip"),
    ];
...
fn client_identifier_allowlist_is_pinned() {
    ...
    assert_eq!(
        schema::sanitize_client_identifier("intellihelper-pager"),
        "intellihelper-pager"
    );
    assert_eq!(
...
fn sentinel_session_harness() -> events::SessionHarness {
    events::SessionHarness {
        ...
        client_identifier: Some("intellihelper-pager".into()),
        model_id: "intellihelper-4".into(),
        agent_name: "intellihelper-build-plan".into(),
...
fn session_start_snapshot_counts_not_names() {
    ...
    let ev = &events[0];
    assert_eq!(ev.0, "intellihelper_code.session_start");
...
fn session_new_increments_session_count_only() {
    ...
    assert_eq!(names, vec!["intellihelper_code.session.count".to_owned()]);
}
...
fn api_request_snapshot_and_token_usage() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "intellihelper_code.api_request");
    ...
    assert_eq!(
        ...
        vec!["intellihelper_code.token.usage"]
    );
}
...
fn one_failed_turn_increments_error_count_exactly_once() {
    ...
    let names: Vec<String> = exported_events(&stream)
        ...
        .collect();
    assert_eq!(
        names.iter().filter(|n| *n == "intellihelper_code.api_error").count(),
        2
    );
    ...
    let total: u64 = stream
        ...
        .filter(|m| m.name() == "intellihelper_code.error.count")
        .map(|m| match m.data() {
...
fn turn_error_increments_error_count() {
    ...
    names.sort();
    assert_eq!(names, vec!["intellihelper_code.error.count", "intellihelper_code.turn.count"]);
}
...
fn tool_result_gates_off_collapses_and_reduces() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "intellihelper_code.tool_result");
...
fn user_prompt_gates_off_drops_text() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "intellihelper_code.user_prompt");
...
fn mcp_connection_collapses_server_name_by_default() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "intellihelper_code.mcp_server_connection");
...
fn tool_decision_snapshot() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "intellihelper_code.tool_decision");
    ...
    assert_eq!(
        exported_metric_names(&stream),
        vec!["intellihelper_code.tool.decision"]
    );
}
...
fn contextual_tip_maps_every_tip_and_action() {
    ...
    for (tip, action, tip_label, action_label) in cases {
        ...
        let events = exported_events(&stream);
        let ev = &events[0];
        assert_eq!(ev.0, "intellihelper_code.contextual_tip");
...
/// intellihelper-workspace sampler events, which live outside this crate and have
...
fn validating_metric_exporter_drops_export_on_bad_attr_key() {
    ...
    let rogue = meter.u64_counter("intellihelper_code.session.count").build();
    rogue.add(
        1,
...
fn redacting_log_exporter_drops_record_with_closed_gate_key() {
    ...
    record.set_event_name("intellihelper_code.user_prompt");
...
fn redacting_log_exporter_drops_record_with_unknown_key() {
    ...
    let logger = stream.ext.logger.as_ref().unwrap();
    let mut record = logger.create_log_record();
    record.set_event_name("intellihelper_code.api_request");
...
fn metric_increments_pass_model_through_scrub_and_attach_session_id() {
    ...
    let exported = stream.metrics.get_finished_metrics().unwrap();
    let blob = format!("{exported:?}");
    assert!(blob.contains("intellihelper_code.token.usage"));
    assert!(
