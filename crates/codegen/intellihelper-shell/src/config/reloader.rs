...
use crate::auth::{IntelliHelperAuth, read_auth_json};
...
pub enum ConfigUpdate {
    ...
    ///
    /// 1. The global `[mcp_servers]` table in `~/.intellihelper/config.toml`
    ///    changed.
    ...
    ///
    /// Project-scoped changes (`<cwd>/.intellihelper/config.toml`,
    ...
    /// (`<cwd>/.intellihelper/config.toml`, `<cwd>/.mcp.json`, or
    ...
    /// intellihelper process sharing the home dir). The agent should consult the cache
    /// thrash unrelated sessions.
    ProjectMcpServersChanged {
        /// The project root whose `.intellihelper/`, `.mcp.json`, or
    ...
    Skills(intellihelper_agent::prompt::skills::SkillsConfig),
    /// Updated `[compat]` vendor-compatibility config. Applied on the
    /// next agent (re)build, which re-resolves `compat_resolved`.
    Compat(Box<intellihelper_tools::types::compat::CompatConfigToml>),
    ...
    ModelsChanged,
    /// `~/.intellihelper/models_cache.json` was rewritten on disk (possibly by another
...
impl ConfigReloader {
    ...
    pub(crate) async fn run(
        ...
        loop {
            ...
            if has_auth {
                ...
                match result {
                    Ok(Err(e)) => {
                        ...
                        let path = self.intellihelper_home.join("auth.json");
            ...
            // same tick (e.g. `~/.intellihelper/config.toml` AND
    ...
    pub(crate) fn reload_auth(&mut self) -> anyhow::Result<()> {
        let auth_path = self.intellihelper_home.join("auth.json");
    ...
    fn reload_config(&mut self) -> anyhow::Result<()> {
        ...
        // config (`~/.intellihelper/config.toml`) via toml::Value. Project-
        // scoped changes (`<cwd>/.intellihelper/config.toml`,
...
/// |----------------------------|-------------------------|-------------------|
/// | `ProjectConfigChanged`     | `<cwd>/.intellihelper/config.toml` | `<cwd>`           |
    ...
    for evt in batch {
        let cwd = match evt {
            ConfigChangeEvent::ProjectConfigChanged { path } => {
                // <cwd>/.intellihelper/config.toml → <cwd>
                path.parent()
                    .and_then(|p| p.parent())
                    .map(|p| p.to_path_buf())
            }
...
/// for `.intellihelper/config.toml`, `find_mcp_json_files` for `.mcp.json`) so
...
/// (`~/.intellihelper/config.toml`, `~/.claude.json`, `~/.cursor/mcp.json`)
...
/// `ConfigUpdate::Skills`) and `intellihelper inspect` (via the `crate::config`
...
pub(crate) fn parse_skills_config(
    config: &toml::Value,
) -> intellihelper_agent::prompt::skills::SkillsConfig {
    config
        .get("skills")
...
fn parse_compat_config(config: &toml::Value) -> intellihelper_tools::types::compat::CompatConfigToml {
    config
        .get("compat")
        .and_then(|v| v.clone().try_into().ok())
        .unwrap_or_default()
...
mod tests {
    ...
    fn make_auth(key: &str) -> IntelliHelperAuth {
        IntelliHelperAuth {
            key: key.to_string(),
    ...
    async fn reloader_detects_new_auth_key() {
        ...
        assert!(
            matches!(update, ConfigUpdate::Auth(a) if a.key == "new-key"), // a is Box<IntelliHelperAuth>, Deref coercion
    ...
    /// The hash must reflect ancestor `.intellihelper/config.toml` and `.mcp.json`
    ...
    fn hash_project_mcp_config_covers_ancestors() {
        ...
        std::fs::write(tmp.path().join(".intellihelper").join("config.toml"), "x = 1").unwrap();
        let h3 = hash_project_mcp_config(&child).expect("readable");
        assert_ne!(
            h2, h3,
            "ancestor .intellihelper/config.toml create must change the hash"
        );
    }
    ...
    fn parse_skills_config_with_paths() {
        let config: toml::Value = toml::from_str(
            r#"
[skills]
paths = ["/home/user/.intellihelper/skills"]
ignore = ["/tmp"]
"#,
        )
        .unwrap();
        let skills = parse_skills_config(&config);
        assert_eq!(skills.paths, vec!["/home/user/.intellihelper/skills".to_string()]);
...
theme = "dark"
yolo = true
fork_secondary_model = "intellihelper-4.5"
"#,
        )
        .unwrap();
        let (theme, yolo, fork) = extract_ui_fields(&config);
        ...
        assert!(yolo);
        assert_eq!(fork.as_deref(), Some("intellihelper-4.5"));
    }
...
"#,
    ...
    fn models_changed_detects_default_change() {
        let a: toml::Value = toml::from_str("[models]\ndefault = \"intellihelper-code-fast-1\"").unwrap();
        let b: toml::Value = toml::from_str("[models]\ndefault = \"intellihelper-code-slow-1\"").unwrap();
...
"#,
    ...
    /// `ProjectConfigChanged` (`<cwd>/.intellihelper/config.toml`) and
    ...
    fn collect_project_cwds_excludes_home_claude_json() {
        let batch = vec![
            ...
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/x/.intellihelper/config.toml"),
            },
        ];
    ...
    fn collect_project_cwds_dedupes_and_extracts() {
        let batch = vec![
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/a/.intellihelper/config.toml"),
            },
            ConfigChangeEvent::McpConfigChanged {
                path: PathBuf::from("/repo/a/.mcp.json"),
            },
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/b/.intellihelper/config.toml"),
            },
        ];
        let cwds = collect_project_cwds(&batch);
        assert_eq!(
