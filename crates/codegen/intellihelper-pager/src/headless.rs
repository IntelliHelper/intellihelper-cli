//! Headless single-turn mode (`intellihelper -p "prompt"`).
//!
//! Runs the agent in-process via `spawn_intellihelper_shell`, drives the ACP lifecycle
...
use intellihelper_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
...
use intellihelper_shell::util::config as cli_config;

use crate::acp::model_state::{EffortTokenError, ModelState};
use crate::acp::spawn::{AgentShutdownGuard, spawn_intellihelper_shell};
...
pub(crate) fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    intellihelper_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
fn mcp_server_names(cwd: &Path) -> Vec<McpServer> {
    let servers =
        cli_config::load_mcp_servers(cwd, &intellihelper_tools::types::compat::CompatConfig::default());
    servers
        .iter()
        .filter_map(|s| {
            let name = match s {
...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Run `intellihelper login` to authenticate \
         (or `intellihelper login --device-code` if no browser is available)."
            .to_string()
    } else {
        "Not signed in. To authenticate without a browser, run:\n  \
         intellihelper login --device-code\n\n\
         Alternatively, set the XAI_API_KEY environment variable \
         or run `intellihelper login` on a machine with a browser."
            .to_string()
    }
}
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive = std::io::stdin().is_terminal()
                && !intellihelper_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !intellihelper_shell::util::clipboard::is_remote_session();
...
async fn apply_headless_model_and_effort(
    ...
    .map_err(|e| {
        if let Some(name) = model_name {
            anyhow::anyhow!(
                "Couldn't set model '{}': {}. Run 'intellihelper models' to see available models.",
                name,
                e
            )
        } else {
...
pub async fn run_single_turn(
    ...
    intellihelper_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = intellihelper_shell::agent::config::AgentMode::Headless;
    ...
    agent_config.cli_agent_overrides = intellihelper_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        intellihelper_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }

    let cancel = CancellationToken::new();
    let memory_config = agent_config.memory_config.clone();
    let spawned = match spawn_intellihelper_shell(agent_config, &cancel, memory_config).await {
        Ok(s) => s,
        Err(e) => {
    ...
    let track_active = std::env::var("INTELLIHELPER_TRACK_HEADLESS").is_ok();
    if track_active {
        let _ = intellihelper_shell::active_sessions::register(
            intellihelper_shell::active_sessions::ActiveSession {
    ...
    if track_active {
        // Non-blocking flock so a slow/network ~/.intellihelper can't hang exit.
        let _ = intellihelper_shell::active_sessions::try_unregister(&session_id);
    }
    ...
    let outcome: Result<()> = match prompt_result {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = intellihelper_shell::sampling::error::prompt_usage_from_error(&err) {
            ...
            let stop_reason_override =
                (intellihelper_shell::sampling::error::stop_reason_for_turn_error(&err) == "MaxTokens")
