...
//! `.intellihelper/config.toml` including `[permission]` / `[mcp_servers]` /
//! `[plugins].paths`, `~/.claude.json` `projects.<cwd>`, project `.intellihelper/lsp.json`).
...
//! interactive prompt, and the durable [`intellihelper_workspace::trust::TrustStore`]
//! reads/writes — lives in `intellihelper-workspace` (client-side); this module keeps
...
//! on [`intellihelper_workspace::folder_trust::decide`]; the consume-side nuance is
...
use intellihelper_workspace::trust::{TrustStore, is_unsafe_trust_root, workspace_key};

// Decision-side (scan/decide/prompt/store) relocated to `intellihelper-workspace`
// (client crate). `grant_folder_trust` is the ONLY moved item referenced from
// OUTSIDE this module (shell call sites + the pager's
// `intellihelper_shell::agent::folder_trust::grant_folder_trust`), so only it is
...
pub use intellihelper_workspace::folder_trust::grant_folder_trust;
use intellihelper_workspace::folder_trust::{
...
// NOTE: this folder-trust store (`~/.intellihelper/trusted_folders.toml`) is SEPARATE
// from the pre-existing per-plugin trust store
// (`intellihelper_agent::plugins::TrustStore` at `~/.intellihelper/trusted-plugins`, plus the
...
/// [`intellihelper_workspace::folder_trust::revoke_folder_trust_store`].
///
...
pub(crate) fn revoke_folder_trust(cwd: &Path) -> bool {
    ...
    let was_trusted = intellihelper_workspace::folder_trust::revoke_folder_trust_store(cwd);
...
/// ([`intellihelper_workspace::folder_trust::repo_config_kinds`] /
...
/// folder is gated (so an `.intellihelper/lsp.json`-only repo still has a non-empty reason
...
pub(crate) fn detected_config_kinds(cwd: &Path) -> Vec<String> {
    intellihelper_workspace::folder_trust::repo_config_kinds(cwd)
        .into_iter()
        .map(str::to_string)
        .collect()
}
...
pub(crate) fn agent_inline_hooks_allowed(
    scope: intellihelper_agent::config::AgentScope,
    trusted: impl FnOnce() -> bool,
) -> bool {
    scope != intellihelper_agent::config::AgentScope::Project || trusted()
}
...
/// whose cwd differs from the launch dir, `intellihelper mcp doctor`) passes `false`, so
...
///   `store_trusted`: a `intellihelper --trust` grant issued AFTER this workspace was
...
/// (session merge, the session-less agent pool, `intellihelper mcp doctor`). It MUST
...
///
/// Sources: project `.intellihelper/config.toml [mcp_servers]` (NOT the user-tier global
...
/// `~/.intellihelper/config.toml` is dropped when untrusted. This is intended — untrusted
...
pub(crate) fn project_scoped_mcp_names(cwd: &Path) -> HashSet<String> {
    ...
    // `.intellihelper/config.toml [mcp_servers]` entries tagged project (the loader's key
...
pub(crate) fn filter_untrusted_project_lsp(
    ...
    sourced: std::collections::BTreeMap<
        String,
        (
            intellihelper_tools::implementations::lsp::config::LspServerConfig,
            intellihelper_tools::types::config_source::ConfigSource,
        ),
    >,
) -> std::collections::BTreeMap<String, intellihelper_tools::implementations::lsp::config::LspServerConfig>
{
    intellihelper_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
        sourced,
...
mod tests {
    ...
    use intellihelper_workspace::folder_trust::repo_configs_present;
    ...
    fn simulate_release_build() -> EnvGuard {
        EnvGuard::set(intellihelper_version::TEST_VERSION_ENV, "0.0.0-sim")
    }
    ...
    fn revoke_folder_trust_downgrades_cache() {
        ...
        // true; INTELLIHELPER_HOME-isolated so the seed can't touch the real user file and
        // `#[serial]` because INTELLIHELPER_HOME is process-global.
    ...
    fn revoke_never_trusted_folder_writes_no_deny_and_preserves_cascade() {
        ...
        // mid-session untrust. INTELLIHELPER_HOME-isolated so the grant writes to a temp
        // store; `#[serial]` because INTELLIHELPER_HOME is global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("INTELLIHELPER_HOME", home.path());
    ...
    fn revoke_ancestor_cascade_trusted_child_writes_explicit_untrust() {
        ...
        // the cache. INTELLIHELPER_HOME-isolated so the grant writes to a temp store;
        // `#[serial]` because INTELLIHELPER_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("INTELLIHELPER_HOME", home.path());
    ...
    fn revoke_on_unrecordable_home_root_records_no_deny() {
        ...
        // home; INTELLIHELPER_HOME-isolated store; INTELLIHELPER_FOLDER_TRUST unset so the
        ...
        std::fs::create_dir_all(home.path().join(".intellihelper").join("hooks")).unwrap();
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        // actually reads `.envrc`. INTELLIHELPER_HOME-isolated so the trust store is empty;
        // INTELLIHELPER_FOLDER_TRUST unset so the default-on feature flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("INTELLIHELPER_HOME", home.path());
        let _flag = EnvGuard::unset("INTELLIHELPER_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let env = intellihelper_workspace::envrc::load_envrc_or_empty(tmp.path());
        assert_eq!(
            env.get("GATED_ENVRC"),
            Some(&"1".to_string()),
    ...
    fn claude_env_gate_drops_project_env_when_untrusted() {
        ...
        // folder merges it. INTELLIHELPER_HOME-isolated so the trust store is empty;
        // INTELLIHELPER_FOLDER_TRUST unset so the default-on feature flag applies.
        use intellihelper_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("INTELLIHELPER_HOME", home.path());
        let _flag = EnvGuard::unset("INTELLIHELPER_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn claude_env_gate_drops_subdir_project_env_when_untrusted() {
        ...
        // this). INTELLIHELPER_HOME-isolated so the trust store is empty.
        use intellihelper_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("INTELLIHELPER_HOME", home.path());
        let _flag = EnvGuard::unset("INTELLIHELPER_FOLDER_TRUST");
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        // predicate used at mvp_agent/subagent. INTELLIHELPER_HOME-isolated (empty store).
        use intellihelper_agent::config::AgentScope;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("INTELLIHELPER_HOME", home.path());
        let _flag = EnvGuard::unset("INTELLIHELPER_FOLDER_TRUST");
        let tmp = repo_tmp();
        let agents = tmp.path().join(".intellihelper").join("agents");
        ...
        .unwrap();
        let def = intellihelper_agent::discovery::by_name_in_cwd("explore", tmp.path())
    ...
    fn project_scope_allowed_denies_untrusted_repo_with_configs() {
        ...
        // code-exec config (here `.intellihelper/hooks`), and no store grant must be
        // DENIED — even though no verdict was recorded first (the gate re-resolves
        // fail-closed rather than defaulting open). INTELLIHELPER_HOME-isolated (empty
        // store); INTELLIHELPER_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("INTELLIHELPER_HOME", home.path());
        let _flag = EnvGuard::unset("INTELLIHELPER_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".intellihelper").join("hooks")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn project_scope_allowed_allows_repo_without_configs() {
        ...
        // would wrongly deny it. Release-stamped + INTELLIHELPER_HOME-isolated so the
    ...
    fn project_scope_allowed_allows_store_trusted_repo() {
        ...
        // configs present. INTELLIHELPER_HOME-isolated so the seeded store is the temp one;
        // INTELLIHELPER_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("INTELLIHELPER_HOME", home.path());
        let _flag = EnvGuard::unset("INTELLIHELPER_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_inert_local_build() {
        ...
        // INTELLIHELPER_TEST_VERSION unset so `is_local_build()` is genuinely true.
        let _unset_ver = EnvGuard::unset(intellihelper_version::TEST_VERSION_ENV);
        if option_env!("INTELLIHELPER_VERSION").is_some() {
    ...
    fn project_scope_allowed_denies_untrusted_plugin_only_repo() {
        // A plugin-only untrusted repo (just `.intellihelper/plugins/<x>/`, no
        ...
        // discover_plugins/build_for_cwd/reload. INTELLIHELPER_HOME-isolated (empty store);
        // INTELLIHELPER_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("INTELLIHELPER_HOME", home.path());
        let _flag = EnvGuard::unset("INTELLIHELPER_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".intellihelper").join("plugins").join("evil")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn project_scope_allowed_denies_untrusted_permission_only_repo() {
        // Bridge: a clone whose ONLY repo-local config is `.intellihelper/config.toml`
        ...
        let _flag = EnvGuard::unset("INTELLIHELPER_FOLDER_TRUST");
        let tmp = repo_tmp();
        let intellihelper = tmp.path().join(".intellihelper");
        std::fs::create_dir_all(&intellihelper).unwrap();
        std::fs::write(
            intellihelper.join("config.toml"),
            "[permission]\nallow = [\"Bash(*)\"]\n",
        )
        .unwrap();
    ...
    fn kill_switch_allows_untrusted_repo_after_authoritative_resolve() {
        ...
        // INTELLIHELPER_HOME-isolated (empty store); INTELLIHELPER_FOLDER_TRUST unset so the kill-switch
        // is the only signal.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("INTELLIHELPER_HOME", home.path());
        ...
        let cold = repo_tmp();
        std::fs::create_dir_all(cold.path().join(".intellihelper").join("hooks")).unwrap();
        assert!(
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        // un-gating here.
        //
        // INTELLIHELPER_HOME-isolated so both the folder-trust store and the plugin trust
        // store start empty (deterministic untrusted); INTELLIHELPER_FOLDER_TRUST unset so
        // the default-on flag applies; `#[serial]` because both are process-global.
        use intellihelper_agent::plugins::discovery::DiscoveryConfig;
        use intellihelper_agent::plugins::{PluginRegistry, SharedPluginRegistryHandle};
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("INTELLIHELPER_HOME", home.path());
        let _flag = EnvGuard::unset("INTELLIHELPER_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let plugin = tmp.path().join(".intellihelper").join("plugins").join("trustgate");
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        // INTELLIHELPER_HOME-isolated so the grant writes to a temp store; INTELLIHELPER_FOLDER_TRUST unset
        // so the default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("INTELLIHELPER_HOME", home.path());
        let _flag = EnvGuard::unset("INTELLIHELPER_FOLDER_TRUST");
        let tmp = repo_tmp();
        let hooks_dir = tmp.path().join(".intellihelper").join("hooks");
        ...
        let has_project_hook = |reg: &intellihelper_hooks::discovery::HookRegistry| {
            reg.all_hooks()
                .iter()
        ...
        let git_root = intellihelper_workspace::session::git::find_git_root_from_path(tmp.path()).ok();
        let (reg, _errs) = crate::util::hooks::discover_hooks(
            git_root.as_deref(),
            &intellihelper_tools::types::compat::CompatConfig::default(),
            untrusted,
        );
        assert!(
    ...
    fn filter_untrusted_project_lsp_drops_only_project() {
        ...
        use intellihelper_tools::implementations::lsp::config::LspServerConfig;
        use intellihelper_tools::types::config_source::ConfigSource;
            ...
            m.insert(
                ...
                (
                    ...
                    ConfigSource::Project {
                        path: PathBuf::from("/repo/.intellihelper/lsp.json"),
                    },
                ),
            );
            m.insert(
                ...
                (
                    LspServerConfig::default(),
                    ConfigSource::User {
                        path: PathBuf::from("/home/.intellihelper/lsp.json"),
                    },
                ),
            );
            m
    ...
    fn load_servers_sourced_tags_project_lsp_json() {
        use intellihelper_tools::implementations::lsp::config::load_servers_with_plugins_sourced;
        use intellihelper_tools::types::config_source::ConfigSource;

        // A `<cwd>/.intellihelper/lsp.json` server must be tagged `Project` so the gate
        // can distinguish it from user/plugin servers. Asserts on the specific
        // key, so any real `~/.intellihelper/lsp.json` on the test host is irrelevant.
        let tmp = repo_tmp();
        let intellihelper = tmp.path().join(".intellihelper");
        std::fs::create_dir_all(&intellihelper).unwrap();
        std::fs::write(intellihelper.join("lsp.json"), r#"{"projlsp": {"command": "true"}}"#).unwrap();
    ...
    fn untrusted_workspace_drops_loaded_project_lsp() {
        ...
        // from `<cwd>/.intellihelper/lsp.json` is dropped once the workspace is untrusted.
        let tmp = repo_tmp();
        let intellihelper = tmp.path().join(".intellihelper");
        std::fs::create_dir_all(&intellihelper).unwrap();
        std::fs::write(intellihelper.join("lsp.json"), r#"{"projlsp": {"command": "true"}}"#).unwrap();
    ...
    /// (`.mcp.json`) and `projtoml` (`.intellihelper/config.toml [mcp_servers]`).
    ...
    /// `.intellihelper/config.toml`, `.mcp.json`, and `.cursor/mcp.json` must appear in
    ...
    fn cached_untrusted_is_upgraded_by_a_later_grant() {
        ...
        record(&key, false);
        assert!(!project_scope_allowed(tmp.path()));
        // Simulate a `intellihelper --trust` grant landing in the store after the
    ...
    fn explicit_child_untrust_survives_reload_despite_ancestor_trust() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let store_path = tmp.path().join(intellihelper_workspace::trust::TRUST_FILE_NAME);
    ...
    fn no_configs_trust_is_provisional_and_regated_when_configs_appear() {
        ...
        unsafe { std::env::set_var("INTELLIHELPER_FOLDER_TRUST", "1") };
        // Simulate a release-stamped build: an unstamped local/dev build (as in CI,
        // no INTELLIHELPER_VERSION) auto-trusts, so the gate would never engage without this.
        unsafe { std::env::set_var(intellihelper_version::TEST_VERSION_ENV, "0.0.0-sim") };
        let tmp = repo_tmp();
        ...
        unsafe { std::env::remove_var(intellihelper_version::TEST_VERSION_ENV) };
        unsafe { std::env::remove_var("INTELLIHELPER_FOLDER_TRUST") };
    }
    ...
    fn resolve_launch_dir_trust_matches_resolve_and_record() {
        ...
        // INTELLIHELPER_HOME so the store is empty/seeded in temp. `#[serial]` because both
        // vars are process-global.
        let _feature = EnvGuard::set("INTELLIHELPER_FOLDER_TRUST", "1");
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("INTELLIHELPER_HOME", home.path());
        ...
        let untrusted = repo_tmp();
        std::fs::create_dir_all(untrusted.path().join(".intellihelper").join("hooks")).unwrap();
        ...
        let trusted = repo_tmp();
        std::fs::create_dir_all(trusted.path().join(".intellihelper").join("hooks")).unwrap();
        let mut store = TrustStore::load();
    ...
    fn local_build_is_inert_launch_trust_auto_trusts() {
        ...
        // `is_local_build_honors_test_version_override`), with INTELLIHELPER_TEST_VERSION
        // unset so `is_local_build()` is genuinely true. INTELLIHELPER_HOME-isolated so the
        // real store is never touched.
        let _sim = EnvGuard::unset(intellihelper_version::TEST_VERSION_ENV);
        if option_env!("INTELLIHELPER_VERSION").is_some() {
            return; // a release-stamped test binary is not a local build
        }
        let home = tempfile::tempdir().unwrap();
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        // => the GUI prompt is warranted. INTELLIHELPER_HOME-isolated so the store starts
        // empty; `#[serial]` because INTELLIHELPER_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("INTELLIHELPER_HOME", home.path());
        let tmp = repo_tmp();
        ...
        // INTELLIHELPER_FOLDER_TRUST unset: env outranks the remote flag, so an ambient
        // opt-out would otherwise false-fail the Prompt assertion.
        let _sim = EnvGuard::set(intellihelper_version::TEST_VERSION_ENV, "0.0.0-sim");
        let _flag = EnvGuard::unset("INTELLIHELPER_FOLDER_TRUST");
        let remote = RemoteSettings {
    ...
    fn prompt_warranted_false_when_feature_disabled() {
        ...
        // local-build path is not what's under test; INTELLIHELPER_HOME-isolated and
        // INTELLIHELPER_FOLDER_TRUST unset so the kill-switch is the only signal.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("INTELLIHELPER_HOME", home.path());
        let _flag = EnvGuard::unset("INTELLIHELPER_FOLDER_TRUST");
        let _sim = simulate_release_build();
    ...
    fn detected_config_kinds_summarizes_present_markers() {
        ...
        std::fs::write(tmp.path().join(".mcp.json"), "{}").unwrap();
        std::fs::create_dir_all(tmp.path().join(".intellihelper").join("hooks")).unwrap();
        std::fs::write(tmp.path().join(".intellihelper").join("lsp.json"), "{}").unwrap();
    ...
    fn detected_config_kinds_reports_lsp_only_repo() {
        ...
        // `.intellihelper/lsp.json` must still produce a non-empty reason list.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".intellihelper")).unwrap();
        std::fs::write(tmp.path().join(".intellihelper").join("lsp.json"), "{}").unwrap();
        let kinds = detected_config_kinds(tmp.path());
