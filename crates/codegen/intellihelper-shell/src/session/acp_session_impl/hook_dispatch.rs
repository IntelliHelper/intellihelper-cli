...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> intellihelper_hooks::runner::RunContext<'_> {
        intellihelper_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[intellihelper_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, intellihelper_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
    ...
    pub(super) async fn dispatch_hook(
        ...
        payload: intellihelper_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
        ...
        let results =
            intellihelper_hooks::dispatcher::dispatch_non_blocking(&registry, event, &envelope, &ctx)
                .await;
        ...
        for r in results {
            let (hook_name, elapsed, outcome) = match r {
                intellihelper_hooks::result::HookRunResult::Success {
                    ...
                    hook_name,
                    elapsed,
                    intellihelper_telemetry::events::HookOutcome::Success,
                ),
                intellihelper_hooks::result::HookRunResult::Blocked {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    intellihelper_telemetry::events::HookOutcome::Blocked,
                ),
                intellihelper_hooks::result::HookRunResult::Failed {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    intellihelper_telemetry::events::HookOutcome::Error,
                ),
                intellihelper_hooks::result::HookRunResult::Skipped { .. } => continue,
            };
            intellihelper_telemetry::session_ctx::log_event(intellihelper_telemetry::events::HookExecuted {
...
mod notification_hook_filter_tests {
    ...
    fn task_completed_does_not_fire_via_filter() {
        let update = XaiSessionUpdate::TaskCompleted {
            task_snapshot: intellihelper_tools::types::TaskSnapshot {
                task_id: "task-1".into(),
