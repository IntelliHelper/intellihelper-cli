...
use crate::implementations::intellihelper_build::grep::ripgrep::rg_path;
...
///
/// Shares `ToolKind::Search` with the intellihelper-build `GrepTool`. These tools are
/// namespace-exclusive — consumers enable either `IntelliHelperBuild` or `Codex` search,
