//! Wire types for the `intellihelper-workspace` API.
//!
...
//! `intellihelper-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `intellihelper-workspace-types` (instead of `intellihelper-workspace` or
/// `intellihelper-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `intellihelper_workspace::permission`
