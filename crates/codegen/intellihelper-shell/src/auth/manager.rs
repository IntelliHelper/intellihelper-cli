...
use intellihelper_auth::bearer_suffix;
...
use intellihelper_telemetry::events::ManualAuthSurface;

#[cfg(test)]
use super::model::UserInfo;
use super::model::{
    AuthMode, IntelliHelperAuth, early_invalidation, is_expired, is_expired_with_buffer, lookup_auth,
};
...
pub struct AuthManager {
    ...
    inner: Arc<RwLock<Option<IntelliHelperAuth>>>,
    path: PathBuf,
    scope: String,
...
enum LockOutcome {
    Held(AuthFileLock),
    Adopted(Box<IntelliHelperAuth>),
}

// ── Construction + builders ──────────────────────────────────────────

impl AuthManager {
    pub fn new(intellihelper_home: &Path, grok_com_config: GrokComConfig) -> Self {
        ...
        intellihelper_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                "scope": &scope,
                "intellihelper_home": intellihelper_home.display().to_string(),
                "HOME": std::env::var("HOME").unwrap_or_else(|_| "(unset)".into()),
                "INTELLIHELPER_HOME": std::env::var("INTELLIHELPER_HOME").unwrap_or_else(|_| "(unset)".into()),
                "INTELLIHELPER_AUTH_PATH": std::env::var("INTELLIHELPER_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "INTELLIHELPER_AUTH": std::env::var("INTELLIHELPER_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // INTELLIHELPER_AUTH_PATH: custom file path (overrides default $INTELLIHELPER_HOME/auth.json).
        // Resolved before the INTELLIHELPER_AUTH branch so inline-credential managers
        ...
        let path = std::env::var("INTELLIHELPER_AUTH_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| intellihelper_home.join("auth.json"));

        // INTELLIHELPER_AUTH: inline JSON credentials (highest priority, read-only).
        if let Ok(inline_json) = std::env::var("INTELLIHELPER_AUTH") {
            if let Ok(auth) = serde_json::from_str::<IntelliHelperAuth>(&inline_json) {
            ...
            tracing::warn!("INTELLIHELPER_AUTH set but failed to parse as JSON, falling back to file");
        }
    ...
    /// (inline `INTELLIHELPER_AUTH` vs. on-disk `auth.json`), which differ only in the
    /// threaded fields. One literal means a newly added field can't be silently
    /// dropped from one branch.
    fn assemble(
        inner: Option<IntelliHelperAuth>,
        path: PathBuf,
        scope: String,
        grok_com_config: GrokComConfig,
        proxy_base_url: String,
    ...
    /// else allowed.
    pub(crate) fn cached_token_policy_error(&self, auth: &IntelliHelperAuth) -> Option<AuthError> {
    ...
    fn vet_cached(&self, auth: IntelliHelperAuth) -> Option<IntelliHelperAuth> {
    ...
    pub(crate) fn current(&self) -> Option<IntelliHelperAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    #[inline]
    pub(crate) fn with_inner_write<R>(&self, f: impl FnOnce(&mut Option<IntelliHelperAuth>) -> R) -> R {
    ...
    #[inline]
    pub(crate) fn with_inner_read<R>(&self, f: impl FnOnce(Option<&IntelliHelperAuth>) -> R) -> R {
    ...
    pub(crate) fn current_or_expired(&self) -> Option<IntelliHelperAuth> {
    ...
    pub(crate) fn current_wire_valid(&self) -> Option<IntelliHelperAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    pub(crate) fn expired_auth(&self) -> Option<IntelliHelperAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    fn is_token_expired(&self, auth: &IntelliHelperAuth) -> bool {
    ...
    fn is_token_hard_expired(&self, auth: &IntelliHelperAuth) -> bool {
        self.token_expired_with_buffer(auth, Duration::zero())
    }

    fn token_expired_with_buffer(&self, auth: &IntelliHelperAuth, buffer: Duration) -> bool {
    ...
    ///
    /// Returns the input `IntelliHelperAuth` BEFORE enrichment lands; callers
    /// needing the post-enrichment view re-read `current()`.
    pub(crate) async fn update(self: &Arc<Self>, auth: IntelliHelperAuth) -> std::io::Result<IntelliHelperAuth> {
        ...
        match &write_result {
            Ok(()) => intellihelper_telemetry::unified_log::info(
            ...
            Err(e) => intellihelper_telemetry::unified_log::error(
    ...
    pub(crate) async fn save_without_enrichment(
        &self,
        auth: IntelliHelperAuth,
    ) -> std::io::Result<IntelliHelperAuth> {
    ...
    fn spawn_user_info_enrichment(self: &Arc<Self>, auth: IntelliHelperAuth) {
        enrichment::spawn(Arc::clone(self), auth);
    }

    /// Blocking `/user` enrichment for login flows that exit before the background task lands.
    pub(crate) async fn enrich_auth_inline(&self, auth: &mut IntelliHelperAuth) {
        enrichment::enrich_inline(self, auth).await;
    }

    /// Path to the `auth.json` this manager reads/writes (respects
    /// `INTELLIHELPER_AUTH_PATH` / constructor home). Prefer this over
    /// `intellihelper_home()/auth.json` so temp-home tests and custom stores stay isolated.
    ...
    pub(crate) async fn run_external_refresh_command(&self, command: &str) -> Option<IntelliHelperAuth> {
    ...
    pub(crate) fn hot_swap(&self, new_auth: IntelliHelperAuth) {
    ...
    pub(crate) fn try_use_disk_token(
        &self,
        disk_auth: Option<&IntelliHelperAuth>,
    ...
    fn try_adopt_disk_token(&self, reason: RefreshReason, msg: &str) -> Option<IntelliHelperAuth> {
    ...
    fn inner_auth_or_external_default(&self) -> IntelliHelperAuth {
        self.inner.read().clone().unwrap_or_else(|| IntelliHelperAuth {
    ...
    #[cfg(test)]
    fn persist_and_swap(&self, auth: IntelliHelperAuth) -> Option<IntelliHelperAuth> {
    ...
    pub(crate) fn read_disk_auth(&self) -> Option<IntelliHelperAuth> {
    ...
    fn read_disk_auth_silent(&self) -> Option<IntelliHelperAuth> {
    ...
    pub(crate) fn read_disk_auth_with_state(&self) -> (Option<IntelliHelperAuth>, DiskAuthState) {
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in INTELLIHELPER_AUTH mode).
            DiskAuthState::Ok => {
                intellihelper_telemetry::unified_log::info(
    ...
    pub async fn auth(self: &Arc<Self>) -> Result<IntelliHelperAuth, AuthError> {
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<IntelliHelperAuth, AuthError> {
        // Snapshot inner ONCE for dispatch atomicity (closes a TOCTOU
        // where a concurrent `clear()` raced `token_type()` + `inner.read()`).
        let snapshot: Option<IntelliHelperAuth> = self.with_inner_read(|inner| inner.cloned());
        ...
        let result = match token_type {
            ...
            TokenType::LegacySession => {
                ...
                // assumption that a sibling process (`intellihelper login` from
    ...
    pub(crate) async fn try_devbox_recovery(
        self: &Arc<Self>,
        unusable: Option<&str>,
    ) -> Result<IntelliHelperAuth, AuthError> {
        if self.grok_com_config.blocks_automatic_oidc() {
            tracing::debug!(
        ...
        intellihelper_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    pub(crate) async fn refresh_chain(
        ...
        let outcome = {
            ...
            let _awake = if self.is_dark_wake() {
                ...
                system_power::hold_awake("intellihelper: OIDC token refresh")
            } else {
                None
            };
    ...
    pub(crate) fn is_different_token(&self, candidate: &IntelliHelperAuth) -> bool {
        let current_key = self.inner.read().as_ref().map(|a| a.key.clone());
    ...
    pub(crate) fn unauthorized_recovery(
        self: &Arc<Self>,
        rejected: Option<IntelliHelperAuth>,
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<intellihelper_telemetry::events::ManualAuth> {
...
pub(crate) fn compute_proactive_sleep(this: &AuthManager) -> StdDuration {
    ...
    match this.inner.read().as_ref().and_then(|a| a.expires_at) {
        ...
        // `[intellihelper_com] auth_token_ttl` to drive a real schedule.
...
impl intellihelper_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
) -> intellihelper_tools::types::SharedApiKeyProvider {
