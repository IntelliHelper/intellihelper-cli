...
async fn run_reasoning_turn(collapse_thinking: bool) -> Turn {
    ...
    std::fs::create_dir_all(content.home().join(".intellihelper")).expect("mk .intellihelper");
    std::fs::write(
        content.home().join(".intellihelper/config.toml"),
        "[ui]\nshow_thinking_blocks = true\n",
    )
    .expect("write config");
    if collapse_thinking {
        let intellihelper_home = content.sandbox().intellihelper_home().to_path_buf();
        std::fs::create_dir_all(&intellihelper_home).expect("mk intellihelper home");
        std::fs::write(
