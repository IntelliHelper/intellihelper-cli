...
mod tests {
    ...
    struct FakeRunner {
        results: Mutex<Vec<Option<IntelliHelperAuth>>>,
        calls: AtomicU32,
    }
    impl FakeRunner {
        fn new(results: Vec<Option<IntelliHelperAuth>>) -> Self {
            Self {
    ...
    impl ExternalCommandRunner for FakeRunner {
        async fn run_external_command(&self, _command: &str) -> Option<IntelliHelperAuth> {
