...
use crate::auth::model::IntelliHelperAuth;
use crate::auth::token_type::TokenType;
use intellihelper_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};
...
impl RejectedAuth {
    pub(crate) fn capture(auth: Option<&IntelliHelperAuth>) -> Self {
        Self {
...
impl UnauthorizedRecovery {
    ...
    pub(crate) async fn next(&mut self) -> Result<IntelliHelperAuth, AuthError> {
    ...
    async fn resolve_next(&mut self) -> Result<IntelliHelperAuth, AuthError> {
    ...
    async fn next_step_loop(&mut self) -> Result<IntelliHelperAuth, AuthError> {
        loop {
            match self.step {
    ...
    async fn try_reload_from_disk(&self) -> Option<IntelliHelperAuth> {
        let _lock = self
            .auth_manager
        ...
        let Some(disk_auth) = self.auth_manager.read_disk_auth() else {
            ...
            intellihelper_telemetry::unified_log::debug("auth recovery: no disk entry", None, None);
            return None;
        };
        if crate::auth::is_expired(&disk_auth) {
            ...
            intellihelper_telemetry::unified_log::debug(
                ...
                Some(serde_json::json!({
                    "disk_key_prefix": intellihelper_auth::bearer_suffix(&disk_auth.key),
        ...
        if self.is_different_token(&disk_auth) {
            ...
            intellihelper_telemetry::unified_log::info(
                ...
                None,
                Some(serde_json::json!({
                    "adopted_key_prefix": intellihelper_auth::bearer_suffix(&disk_auth.key),
                    "expires_at": disk_auth.expires_at.map(|e| e.to_rfc3339()),
                })),
            );
    ...
    fn fresh_mint_guard(&self) -> Option<IntelliHelperAuth> {
    ...
    async fn try_refresh_from_authority(&self) -> Result<IntelliHelperAuth, AuthError> {
                    ...
                    Ok(auth) => {
                        intellihelper_telemetry::unified_log::info(
                            ...
                            Some(serde_json::json!({
                                ...
                                "new_key_prefix": intellihelper_auth::bearer_suffix(&auth.key),
                                "expires_at": auth.expires_at.map(|e| e.to_rfc3339()),
                            })),
                        );
                    }
    ...
    fn is_different_token(&self, candidate: &IntelliHelperAuth) -> bool {
...
mod tests {
    ...
    use crate::auth::model::{AuthMode, IntelliHelperAuth};
    ...
    fn rejected_cred() -> Option<IntelliHelperAuth> {
        Some(IntelliHelperAuth {
    ...
    async fn recover_with_ok_refresher(m: &Arc<AuthManager>) -> (Result<IntelliHelperAuth, AuthError>, u32) {
        let calls = Arc::new(AtomicU32::new(0));
        m.set_refresher(Arc::new(OkRefresher {
            calls: calls.clone(),
        }));
    ...
    async fn reload_from_disk_rejects_expired_different_token() {
        ...
        let scope = m.grok_com_config().auth_scope();
        let expired_different = IntelliHelperAuth {
