...
async fn unknown_ssh_clipboard_delivery_is_unverified() {
    ...
    let mut harness = PtyHarness::spawn_with_content_env_ops_in_dir(
        ...
        &[
            ...
            // launched under `intellihelper wrap`.
            EnvOp::remove("INTELLIHELPER_OSC52_SINK"),
            EnvOp::remove("LC_INTELLIHELPER_OSC52_SINK"),
        ],
        Some(content.home()),
    )
    .expect("spawn pager");
    ...
    harness
        .wait_for_text("intellihelper wrap ssh <host>", Duration::from_secs(10))
