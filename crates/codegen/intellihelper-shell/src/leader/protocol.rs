...
pub enum ClientMode {
    /// Headless mode (intellihelper agent, intellihelper agent headless) - uses websocket relay.
    /// Leader connects to websocket relay once and forwards messages.
    Headless,
    /// Stdio mode (intellihelper agent stdio, intellihelper -p) - uses local IPC.
...
pub struct LeaderCapabilities {
    ...
    /// (driven by `intellihelper update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `intellihelper update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `intellihelper update` just installed; the leader uses
...
mod tests {
    ...
    fn workspace_status_payload_roundtrip() {
        let payload = ControlPayload::WorkspaceStatus {
            ...
            uptime_ms: 4200,
            active_tool_calls: 2,
            sessions: vec!["intellihelper-a".into(), "intellihelper-b".into()],
            pid: 4242,
        };
