...
use bytes::Bytes;
use std::borrow::Cow;
use intellihelper_tools::util::image_compress::{FilterType, ReEncodeParams, re_encode_under_limit};
...
/// `intellihelper_tools::reminders` (free-fn shape because this module has no
...
pub(crate) fn persisted_image_reject_reason(bytes: &[u8]) -> Option<String> {
    ...
    use intellihelper_tools::util::image_validate as iv;
...
pub(crate) fn inline_attach_verdict(data_b64: &str) -> InlineAttachVerdict {
    ...
    let Ok((w, h, _)) =
        intellihelper_tools::util::image_validate::validate_image_bytes_with(&raw, false)
    else {
        return InlineAttachVerdict::Unreadable;
    };
...
fn compute_normalized_blocking(
    ...
    let (orig_w, orig_h, orig_mime) =
        intellihelper_tools::util::image_validate::validate_image_bytes_with(&raw_bytes, false)
            .map_err(|e| NormalizeError(format!("validate: {e}")))?;
    if !intellihelper_tools::util::image_validate::image_structurally_complete(&raw_bytes) {
    ...
    if !exceeded_size && !exceeded_dimensions {
        if let Err(e) = intellihelper_tools::util::image_validate::validate_image_bytes(&raw_bytes) {
...
mod tests {
    ...
    fn re_encode_fallback_notice_picks_tag_per_harness() {
        ...
        let intellihelper = render_re_encode_fallback_notice(&notes, false);
        assert!(intellihelper.contains("<system-reminder>"));
        assert!(intellihelper.contains("</system-reminder>"));
        assert!(!intellihelper.contains("<system_reminder>"));
        assert!(intellihelper.contains("<image_re_encode_fallback>"));
    }
    #[test]
    fn display_format() {
    ...
    fn image_dropped_notice_picks_tag_per_harness() {
        ...
        let intellihelper = render_image_dropped_notice(&notes, false);
        assert!(intellihelper.contains("<system-reminder>"));
        assert!(intellihelper.contains("</system-reminder>"));
        assert!(!intellihelper.contains("<system_reminder>"));
        assert!(intellihelper.contains("<image_dropped_notice>"));
        assert!(intellihelper.contains("Image 5"));
