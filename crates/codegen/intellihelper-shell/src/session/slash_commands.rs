...
use intellihelper_tools::implementations::skills::skill::format_skill_name;
...
pub(crate) struct ListCommandsRequest {
    ...
    /// Product lane: `"chat"` filters to IntelliHelper Chat / IntelliHelper Computer first-party
...
/// list, and per-turn resolve do not stampede intellihelper.com.
...
fn product_skills_cache_entry_after_fetch(
    primary: &crate::auth::IntelliHelperAuth,
    skills: Vec<SkillInfo>,
    _used_untagged_recovery: bool,
) -> ProductSkillsCacheEntry {
    product_skills_cache_entry(primary, skills)
}
fn product_skills_negative_stamp(auth: &crate::auth::IntelliHelperAuth) -> ProductSkillsIdentityStamp {
...
fn clear_degraded_cache_for_auth(auth: &crate::auth::IntelliHelperAuth) {
...
fn clear_negative_cache_for_auth(auth: &crate::auth::IntelliHelperAuth) {
...
/// Product (intellihelper.com) Skills catalog as SkillInfo rows for slash advertising
...
pub(crate) async fn product_skill_infos(
    ...
    let intellihelper_auth = match auth.auth().await {
        Ok(a) => a,
        Err(err) => {
    ...
    if let Some(skills) = product_skills_cache_lookup(&intellihelper_auth) {
        return skills;
    }
    ...
    match client.try_list_catalog(PRODUCT_SKILLS_LOCALE).await {
        Ok((catalog, used_untagged_recovery)) if catalog.user_list_failed => {
            let skills = catalog.to_skill_infos();
            {
                ...
                if let Some(entry) = guard.as_ref()
                    && product_skills_cache_matches(entry, &intellihelper_auth)
                {
                    tracing::warn!(
            ...
            *PRODUCT_SKILLS_DEGRADED_CACHE.lock() = Some(product_skills_cache_entry_after_fetch(
            ...
            clear_negative_cache_for_auth(&intellihelper_auth);
            tracing::warn!(
        ...
        Ok((catalog, used_untagged_recovery)) => {
            ...
            *PRODUCT_SKILLS_CACHE.lock() = Some(product_skills_cache_entry_after_fetch(
            ...
            clear_degraded_cache_for_auth(&intellihelper_auth);
            clear_negative_cache_for_auth(&intellihelper_auth);
            Some(skills)
        }
        Err(err) => {
            ...
            *PRODUCT_SKILLS_NEGATIVE_CACHE.lock() = Some(product_skills_negative_stamp(&intellihelper_auth));
            tracing::warn!(
                error = %err,
...
fn product_skills_cache_lookup(
    intellihelper_auth: &crate::auth::IntelliHelperAuth,
    ...
    }
    {
        ...
        if let Some(entry) = guard.as_ref()
            && product_skills_negative_matches(entry, intellihelper_auth)
...
///   catalog** (same as intellihelper-web) + builtins — not Build disk skills. Without
...
///   matching today's IntelliHelper CLI flow that calls our dedicated `skill` tool.
...
pub(super) async fn build_skill_information_for_refs(
    ...
    use intellihelper_tools::implementations::skills::skill::{
...
/// `intellihelper-tools` so it stays identical to the pager's `LoopCommand` and the
...
mod tests {
    ...
    fn product_skills_cache_matches_identity_and_team() {
        ...
        let other_user = IntelliHelperAuth {
            key: "tok-c".into(),
            user_id: "user-2".into(),
            team_id: Some("team-a".into()),
            auth_mode: AuthMode::Oidc,
        ...
        let personal_same_user = IntelliHelperAuth {
    ...
    fn product_skills_cache_after_untagged_recovery_keeps_primary_tenant() {
        ...
        let personal = IntelliHelperAuth {
    ...
    fn listing(name: &str) -> crate::session::workflow::registry::WorkflowListing {
        crate::session::workflow::registry::WorkflowListing {
            ...
            when_to_use: None,
            source: "project",
            path: Some(format!(".intellihelper/workflows/{name}.rhai")),
        }
    }
    #[test]
