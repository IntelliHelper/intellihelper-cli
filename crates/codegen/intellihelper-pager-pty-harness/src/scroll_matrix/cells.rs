...
pub struct ExpectedProfile {
    ...
    pub invert: bool,
    /// Speed multiplier (NOT the 1-100 setting): `INTELLIHELPER_SCROLL_SPEED=100`
...
pub struct MatrixCell {
    ...
    /// appends `INTELLIHELPER_SCROLL_LOG`; the harness's env strips guarantee the
...
const SPEED100: (&str, &str) = ("INTELLIHELPER_SCROLL_SPEED", "100");
const MODE_WHEEL: (&str, &str) = ("INTELLIHELPER_SCROLL_MODE", "wheel");
const MODE_TRACKPAD: (&str, &str) = ("INTELLIHELPER_SCROLL_MODE", "trackpad");
const LINES1: (&str, &str) = ("INTELLIHELPER_SCROLL_LINES", "1");
const INVERT: (&str, &str) = ("INTELLIHELPER_INVERT_SCROLL", "1");
...
mod tests {
    ...
    /// pager would (`from_terminal_context` + the `INTELLIHELPER_SCROLL_*` env
    ...
    fn derive_expected(env: &'static [(&'static str, &'static str)]) -> ExpectedProfile {
        ...
        if let Some(lines) = get("INTELLIHELPER_SCROLL_LINES") {
        ...
        let speed = match get("INTELLIHELPER_SCROLL_SPEED") {
            None | Some("50") => 1.0,
            Some("100") => 6.0,
            Some(other) => panic!("unmapped INTELLIHELPER_SCROLL_SPEED {other:?}"),
        };
        ExpectedProfile {
            mode: get("INTELLIHELPER_SCROLL_MODE").unwrap_or("auto"),
            ept,
            wheel_lpt,
            trackpad_lpt,
            invert: get("INTELLIHELPER_INVERT_SCROLL") == Some("1"),
            speed,
        }
    }
