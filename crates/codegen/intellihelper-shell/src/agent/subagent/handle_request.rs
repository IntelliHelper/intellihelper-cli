...
use intellihelper_tools::implementations::{intellihelper_build, opencode};
...
/// `intellihelper-tools` coordinator actor and reached only through `reporter`.
#[tracing::instrument(
...
pub(crate) async fn run_shell_child(
    ...
    let mut effective_runtime = intellihelper_subagent_resolution::resolve_runtime_config(
        &request.subagent_type,
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = intellihelper_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
    ...
    let worktree_path = if let Some(ref source) = resume_source {
        ...
        let dest = match crate::session::worktree::worktree_base_dir_for_source(&source_cwd) {
            ...
            Err(e) => {
                ...
                std::env::temp_dir()
                    .join("intellihelper-subagent-worktrees")
                    .join(&request.id)
            }
        };
    ...
    effective_runtime.capability_mode = intellihelper_subagent_resolution::intersect_capability_modes(
    ...
    intellihelper_subagent_resolution::apply_child_tool_policy(
        &mut definition,
        effective_runtime.capability_mode,
    ...
    {
        let reminder = intellihelper_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let covered_by_parent = fsnotify::watch_root_covers(&ctx.parent_cwd, &child_cwd);
    ...
    let child_cwd_abs = intellihelper_paths::AbsPathBuf::new(child_cwd).unwrap_or_else(|_| {
        intellihelper_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    let attribution_callback: Option<intellihelper_sampler::SharedAttributionCallback> =
    ...
    let yolo_policy_block = intellihelper_workspace::permission::resolution::yolo_disabled_by_policy();
    ...
    if let Some(scope) = agent_memory_scope {
        let memory_tools: Vec<intellihelper_tools::registry::types::ToolConfig> = vec![
            ...
            && let Ok(content) = std::fs::read_to_string(&memory_md)
        {
            ...
            let truncated: String = content
                ...
                .join("\n");
            let truncated =
                intellihelper_tools::util::truncate::truncate_str(&truncated, MAX_BYTES).to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == intellihelper_hooks::event::HookEventName::Stop {
                            s.event = intellihelper_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if !agent_owned_mcp_servers_allowed(is_plugin_agent) {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    intellihelper_agent::config::McpServerRef::Named(name) => {
                    ...
                    intellihelper_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            intellihelper_agent::prompt::skills::list_skills_with_plugins(
                Some(&parent_cwd_str),
    ...
    intellihelper_telemetry::session_ctx::log_event(intellihelper_telemetry::events::SubagentLaunched {
    ...
    let spawn_result = session::spawn_session_on_thread(
        ...
        intellihelper_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
        ...
        std::sync::Arc::new(parking_lot::Mutex::new(
            intellihelper_workspace::file_system::CodebaseIndexManager::new(),
        )),
        false,
        subagent_fs_watch,
        None,
        ...
        if inherit_skills {
            ...
            intellihelper_agent::prompt::skills::SkillsConfig::default()
        },
        if inherit_skills {
        ...
        ctx.yolo_mode
            || matches!(
                ...
                intellihelper_agent::config::PermissionMode::BypassPermissions
            ),
        false,
        None,
        ...
        Vec::new(),
        intellihelper_agent::prompt::context::PromptAudience::Subagent,
    ...
    let outcome = if result.success {
        intellihelper_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        intellihelper_telemetry::events::Outcome::Cancelled
    } else {
        intellihelper_telemetry::events::Outcome::Error
    };
    intellihelper_telemetry::session_ctx::log_event(intellihelper_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        outcome,
    ...
    if let Some(ref wt_path) = worktree_path {
        if snapshot_dispose_enabled {
            let ref_name = format!("refs/intellihelper/subagents/{}", request.id);
            let source_repo = resolve_subagent_source_repo(&ctx);
