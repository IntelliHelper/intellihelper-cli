...
#[cfg(test)]
pub(crate) async fn test_agent_default() -> intellihelper_agent::Agent {
    test_agent_with_tools(vec![]).await
}
#[cfg(test)]
pub(crate) async fn test_agent_backend_search(
    hosted_tools: Vec<intellihelper_sampling_types::HostedTool>,
) -> intellihelper_agent::Agent {
    let base = test_agent_default().await;
    intellihelper_agent::Agent::new(
        base.definition().clone(),
        intellihelper_agent::PromptContext::default(),
        String::new(),
        base.tool_bridge().clone(),
        intellihelper_agent::ReminderPolicy::default(),
        intellihelper_agent::CompactionPolicy::default(),
        hosted_tools,
        true,
    )
}
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> intellihelper_agent::Agent {
    use intellihelper_tools::implementations::intellihelper_build::update_goal::UpdateGoalTool;
    use intellihelper_tools::registry::types::ToolConfig;
    test_agent_with_tools(vec![ToolConfig::for_tool::<UpdateGoalTool>()]).await
}
/// IntelliHelper-build agent with the real `TodoWriteTool` (id `todo_write`, kind
/// `Plan`) registered, so `tool_for_kind(ToolKind::Plan)` resolves through the
/// live toolset instead of the literal fallback.
#[cfg(test)]
pub(crate) async fn test_intellihelper_build_agent_with_todo() -> intellihelper_agent::Agent {
    use intellihelper_tools::implementations::intellihelper_build::todo::TodoWriteTool;
    use intellihelper_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> intellihelper_agent::Agent {
    use intellihelper_tools::implementations::intellihelper_build::enter_plan_mode::EnterPlanModeTool;
    use intellihelper_tools::implementations::intellihelper_build::exit_plan_mode::ExitPlanModeTool;
    use intellihelper_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<intellihelper_tools::registry::types::ToolConfig>,
) -> intellihelper_agent::Agent {
    test_agent_from_config(
        intellihelper_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        intellihelper_agent::AgentDefinition::default_intellihelper_build(),
        std::sync::Arc::new(intellihelper_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
pub(crate) async fn test_agent_with_user_message_template(
    template: intellihelper_agent::prompt::user_message::UserMessageTemplate,
) -> intellihelper_agent::Agent {
    let mut definition = intellihelper_agent::AgentDefinition::default_intellihelper_build();
...
async fn test_agent_from_config(
    ...
    definition: intellihelper_agent::AgentDefinition,
    backend: std::sync::Arc<dyn intellihelper_tools::computer::types::TerminalBackend>,
) -> intellihelper_agent::Agent {
    use intellihelper_tools::computer::local::LocalFs;
    use intellihelper_tools::computer::types::AsyncFileSystem;
    ...
    let ctx = SessionContext {
        ...
        session_folder: std::env::temp_dir().join("intellihelper-test"),
...
pub(crate) async fn create_test_actor_with_terminal(
    ...
    let cwd = intellihelper_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
    let fs = Arc::new(intellihelper_workspace::file_system::MockFs::new(
        cwd.to_path_buf(),
    ));
    ...
    tool_context.task_completion_reservations =
        Some(intellihelper_tools::reminders::task_completion::TaskCompletionReservations::default());
    tool_context.task_wake_suppressed =
        Some(intellihelper_tools::reminders::task_completion::TaskWakeSuppressed::default());
    ...
    let actor = SessionActor {
        ...
        permissions: intellihelper_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
