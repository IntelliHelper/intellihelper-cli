...
//! after every tool call to check for SKILL.md files in `.intellihelper/skills/`,
...
/// Directories that contain skill definitions (`.intellihelper/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".intellihelper", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.intellihelper/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.intellihelper/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.intellihelper/) are outside the git root so the walk breaks early.
