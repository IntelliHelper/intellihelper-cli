...
/// Default scopes for the xAI OAuth2 provider. Includes `intellihelper-cli:access`
...
pub enum PreferredAuthMethod {
    ...
    ApiKey,
    /// OIDC / OAuth2 session (`cached_token`, interactive `intellihelper.com` / `oidc`,
...
pub struct GrokComConfig {
    ...
    /// Login button label (env: `INTELLIHELPER_AUTH_PROVIDER_LABEL`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// refresh works. Env: `INTELLIHELPER_AUTH_TOKEN_TTL`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// can't bypass the deployment's IdP login. Env: `INTELLIHELPER_DISABLE_API_KEY_AUTH`.
...
/// OAuth2 provider configuration (`INTELLIHELPER_OAUTH2_ISSUER` / `INTELLIHELPER_OAUTH2_CLIENT_ID`).
///
...
const DEFAULT_OAUTH2_REFERRER: &str = "intellihelper-build";
/// Returns `true` when `INTELLIHELPER_LOCAL_AUTH=1` is set,
/// indicating the local accounts-app should be used as the OAuth2 issuer.
pub(crate) fn use_local_auth() -> bool {
    std::env::var("INTELLIHELPER_LOCAL_AUTH")
        .map(|v| !v.is_empty() && v != "0")
        .unwrap_or(false)
}
/// Returns the active xAI OAuth2 issuer — the local-dev issuer when
/// `INTELLIHELPER_LOCAL_AUTH=1` is set, otherwise the production issuer.
...
/// auth.json scope key used by the pre-OIDC `intellihelper login --legacy` flow.
...
impl GrokComConfig {
    ...
    /// `INTELLIHELPER_DISABLE_API_KEY_AUTH` env lockdown is sticky: because the env value
    ...
    /// pin is fail-closed. Explicit `intellihelper login --devbox` / `--api-key` bypass
...
impl OAuth2ProviderConfig {
    ...
    pub fn from_env() -> Option<Self> {
        let issuer = std::env::var("INTELLIHELPER_OAUTH2_ISSUER").ok()?;
        let client_id = std::env::var("INTELLIHELPER_OAUTH2_CLIENT_ID").ok()?;
        let principal_type = std::env::var("INTELLIHELPER_OAUTH2_PRINCIPAL_TYPE").ok();
        let principal_id = std::env::var("INTELLIHELPER_OAUTH2_PRINCIPAL_ID").ok();
        ...
        Some(Self {
            issuer,
            client_id,
            scopes: std::env::var("INTELLIHELPER_OAUTH2_SCOPES")
                .map(|s| s.split(',').map(|s| s.trim().to_owned()).collect())
                .unwrap_or(default_scopes),
            principal_type,
            principal_id,
            referrer: Some(
                std::env::var("INTELLIHELPER_OAUTH2_REFERRER")
...
impl Default for GrokComConfig {
    fn default() -> Self {
        ...
        Self {
            intellihelper_ws_origin: std::env::var("INTELLIHELPER_WS_ORIGIN")
                .unwrap_or_else(|_| PROD_WS_ORIGIN.to_owned()),
            intellihelper_ws_url: std::env::var("INTELLIHELPER_WS_URL")
                .unwrap_or_else(|_| PROD_RELAY_WS_URL.to_owned()),
            token_header: "intellihelper-cli".to_owned(),
            oidc,
            oauth2,
            auth_provider_command: std::env::var("INTELLIHELPER_AUTH_PROVIDER_COMMAND").ok(),
            auth_provider_label: std::env::var("INTELLIHELPER_AUTH_PROVIDER_LABEL").ok(),
            auth_token_ttl: std::env::var("INTELLIHELPER_AUTH_TOKEN_TTL")
                .ok()
                .and_then(|v| v.parse().ok()),
            disable_api_key_auth: std::env::var("INTELLIHELPER_DISABLE_API_KEY_AUTH")
                .ok()
...
/// Parse a boolean env-var value for intellihelper's on/off flags. A bare presence
/// enables the flag, but the common falsy spellings (`0`, `false`, `off`,
/// `no`, empty) count as disabled — so e.g. `INTELLIHELPER_DISABLE_API_KEY_AUTH=false`
...
/// True when the admin has set `INTELLIHELPER_DISABLE_API_KEY_AUTH` to a truthy value in
...
fn env_lockdown_forced() -> bool {
    std::env::var("INTELLIHELPER_DISABLE_API_KEY_AUTH")
        .ok()
        .is_some_and(|v| env_flag_enabled(&v))
}
impl OidcAuthConfig {
    pub fn from_env() -> Option<Self> {
        let issuer = std::env::var("INTELLIHELPER_OIDC_ISSUER").ok()?;
        let client_id = std::env::var("INTELLIHELPER_OIDC_CLIENT_ID").ok()?;
        Some(Self {
            issuer,
            client_id,
            scopes: std::env::var("INTELLIHELPER_OIDC_SCOPES")
                .map(|s| s.split(',').map(|s| s.trim().to_owned()).collect())
                .unwrap_or_else(|_| default_oidc_scopes()),
            audience: std::env::var("INTELLIHELPER_OIDC_AUDIENCE").ok(),
        })
    }
}
#[cfg(test)]
mod tests {
    ...
    fn team_auth_scope_is_base_scope() {
        let cfg = OAuth2ProviderConfig {
            ...
            referrer: Some("intellihelper-build".into()),
        };
