...
///
/// Does **not** invent `intellihelper.com` when no interactive method is advertised
...
fn no_login_method_error(app: &AppView) -> String {
    if app.auth_methods.is_empty() {
        intellihelper_shell::agent::auth_method::NO_AUTH_CREDENTIALS.to_string()
    } else {
...
pub(super) fn handle_auth_complete(
        ...
        && *current_seq == request_seq
    {
        if let Some(meta_val) = meta.as_ref()
            && let Ok(auth_meta) =
                serde_json::from_value::<intellihelper_shell::auth::AuthMeta>(meta_val.clone())
        {
