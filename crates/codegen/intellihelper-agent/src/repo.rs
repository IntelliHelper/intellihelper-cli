...
//! primitive consumed cross-crate by `intellihelper-workspace`, not agent-definition
//! discovery.
...
/// code-exec markers (`.mcp.json`, `.intellihelper/config.toml`, `.claude/settings.json`,
...
impl RepoDirChain {
    ...
    pub fn resolve(cwd: &Path) -> Self {
        let git_root = git2::Repository::discover(cwd)
            ...
            // home-level `.intellihelper`/`.mcp.json`/plugins would look repo-local. Drop
...
/// from `intellihelper-workspace`, which depends on THIS crate) to keep the dep edge
