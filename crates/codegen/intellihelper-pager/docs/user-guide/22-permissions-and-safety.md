...
Control what IntelliHelper can access and do: permission modes, allow/ask/deny rules, hooks, and the optional OS-level sandbox.

- **Modes** set how often IntelliHelper asks for approval (always-approve, auto, ask, and related).
...
When IntelliHelper edits a file, runs a command, or calls an external tool, it may pause for approval. Permission modes control how often that happens.
...
```bash
intellihelper -p "Run the tests" --always-approve
intellihelper agent --always-approve stdio
intellihelper agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```
...
```bash
intellihelper --always-approve -p "Run the test suite"
intellihelper --permission-mode auto
intellihelper agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```
...
```toml
# project .intellihelper/config.toml
[ui]
...
```bash
intellihelper -p "Deploy the service" --always-approve --deny 'Bash(rm -rf *)'
```
...
Organizations can prevent always-approve from being enabled via CLI, TUI, or `/always-approve`. Set this in `requirements.toml` (user-level under `~/.intellihelper/`, or system-wide under `/etc/intellihelper/` for enforcement users cannot remove):
...
IntelliHelper can still load Claude-style permission **rules** from managed settings; always-approve is locked with `requirements.toml` as shown above.
...
IntelliHelper reads permission rules from three compatible sources. Rules from all sources are merged into one set; a rule's effect depends on its action (`deny` > `ask` > `allow`), not on which file it came from.
...
|-------|------|-----------------------|
| Global (all projects) | `~/.intellihelper/config.toml` | No |
| Project (committed) | `<project>/.intellihelper/config.toml` | Yes (commit it) |
| Project (personal) | `<project>/.claude/settings.local.json` | No (gitignore it) |
| Interactive grants | Stored internally by IntelliHelper, per project | No |

Notes on scoping:

- IntelliHelper discovers a `.intellihelper/config.toml` at every directory level from the repository root down to your working directory, so a subdirectory can add rules on top of the repo root's.
...
- IntelliHelper has no native `config.local.toml`. For personal, uncommitted rules in a project, use `.claude/settings.local.json`; IntelliHelper reads it directly (see [Claude Code Compatibility](#3-claude-code-compatibility-claudesettingsjson)).
...
To stop prompts for a specific command in one project, add a narrow allow rule to that project's `.intellihelper/config.toml` (or `.claude/settings.json`):
...
```bash
intellihelper -p "Review the API changes" \
  --allow 'Bash(git *)' \
  --allow 'Bash(gh *)' \
  --allow 'Read' \
  --allow 'Grep' \
...
### 2. Native Configuration (`~/.intellihelper/config.toml` and `.intellihelper/config.toml`)
...
Rules from the global `~/.intellihelper/config.toml` and every project `.intellihelper/config.toml` (from the repo root down to your working directory) are merged into one rule set, alongside any `.claude/settings.json` rules.

Managed configuration deployed by your organization also contributes `[permission]` rules: the system `/etc/intellihelper/managed_config.toml`, and a user-level copy that IntelliHelper maintains automatically at `~/.intellihelper/managed_config.toml`. Managed rules merge like rules from any other source, with two properties specific to managed `allow` rules: your own `deny` and `ask` rules win over a managed `allow` (severity ordering), and a catch-all managed `allow` is ignored when always-approve is locked off. For rules that users cannot edit away, use the root-owned system `/etc/intellihelper/requirements.toml`.
...
IntelliHelper reads `~/.claude/settings.json` and `~/.claude/settings.local.json`, plus the project-level `<project>/.claude/settings.json` and `settings.local.json` (walking up to the repo root). The native `.intellihelper` source for permission rules is `config.toml`, described in the section above.
...
Supported `defaultMode` values include `default`, `auto`, `acceptEdits`, `bypassPermissions`, `dontAsk`, and `plan`. IntelliHelper reads `defaultMode` from its canonical location under `permissions`; a top-level `defaultMode` is also accepted when the nested key is absent.
...
**Chained commands.** IntelliHelper parses each command like a shell and splits it on `&&`, `||`, `;`, `|`, and newlines. The rule actions treat segments differently:
...
`MCPTool(...)` patterns match the full IntelliHelper tool name in `server__tool` form, with glob support: `MCPTool(linear__*)` matches every tool from the `linear` server. IntelliHelper tool names carry no `mcp__` prefix, so a rule written as `mcp__server__tool` never matches an MCP call; write `MCPTool(server__tool)` instead.
...
Interactive grants are stored in IntelliHelper's own state directory under your home directory, scoped to the directory you launched IntelliHelper from. A grant made in one project never applies in another, grants are not written into the repository, and they are not meant to be hand-edited.

Interactive grants are personal, per-machine state. For an allowlist you can review in code review and share with teammates, use declarative rules in the project's `.intellihelper/config.toml` instead.
...
**`~/.intellihelper/hooks/git-gh-only.json`**
...
**`~/.intellihelper/hooks/git-gh-only.sh`**
...
```bash
chmod +x ~/.intellihelper/hooks/git-gh-only.sh
```
...
```bash
intellihelper -p "Implement the feature using only git and GitHub CLI" \
  --allow 'Read' \
  --allow 'Grep' \
  --allow 'Bash(git *)' \
  --allow 'Bash(gh *)'
...
3. **Review project configuration from unfamiliar sources.** Project permission rules in `.intellihelper/config.toml` and `.claude/settings.json`, including `allow` rules, apply without a separate trust prompt. Review them, and any project hooks, before working in an unfamiliar checkout (see the security notes in [10-hooks.md](10-hooks.md)).
