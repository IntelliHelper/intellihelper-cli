...
//! `intellihelper_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `INTELLIHELPER_POOL_*` /
//! `INTELLIHELPER_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `INTELLIHELPER_SAMPLER_SHARED_CLIENT=0` falls back to
...
//! `INTELLIHELPER_EXTRA_CA_BUNDLE` via `intellihelper_extra_ca` (see env-var registry).

use std::sync::OnceLock;

use intellihelper_workspace::permission::ClientType;
...
///
/// Replaces `intellihelper_shell::instrumentation_timer!`, which cannot be referenced
/// here (it lives in the shell crate, which now depends on this one). This is a
/// behavior-preserving copy: it routes to the same
/// `intellihelper_telemetry::instrumentation` API and keeps the Chrome trace
/// span for these startup timings.
macro_rules! startup_timer {
    ($name:literal) => {{
        use intellihelper_telemetry::instrumentation::{
...
// `OriginClientInfo` is owned by `intellihelper-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `intellihelper-shell`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `intellihelper-telemetry` consumes
// the same type via `intellihelper_sampler::OriginClientInfo`. The shell-specific
// constructors that depended on `ClientType` (a shell-only type) are free
// functions below.
pub use intellihelper_sampler::OriginClientInfo;

/// Construct an [`OriginClientInfo`] from `INTELLIHELPER_CLIENT_NAME` /
/// `INTELLIHELPER_CLIENT_VERSION` env vars. Returns `None` when
/// `INTELLIHELPER_CLIENT_NAME` is unset.
pub fn origin_client_info_from_env() -> Option<OriginClientInfo> {
    std::env::var("INTELLIHELPER_CLIENT_NAME")
        .ok()
        .map(|product| OriginClientInfo {
            product,
            version: std::env::var("INTELLIHELPER_CLIENT_VERSION").ok(),
        })
}
...
pub fn process_user_agent_string() -> String {
    ...
    UserAgent {
        origin,
        agent_product: "intellihelper-shell",
        agent_version,
...
/// Process-level client identifier (`INTELLIHELPER_CLIENT_NAME` env var, default `"intellihelper-shell"`).
...
/// (`intellihelper -p`) run or an interactive session; feeds the `client_mode`
/// metric label.
pub const CLIENT_MODE_HEADER: &str = "x-intellihelper-client-mode";

/// One-way latch: set to `"headless"` at startup by the non-TUI entry points
/// (`run_single_turn` for `intellihelper -p`, `run_headless_inner` for
/// `intellihelper agent [headless]`), `"interactive"` otherwise.
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn intellihelper_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(intellihelper_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
...
pub fn shared_startup_blocking_client() -> reqwest::blocking::Client {
    ...
    BLOCKING_CLIENT
        .get_or_init(|| {
            ...
            intellihelper_extra_ca::with_extra_root_certificates_blocking(
...
mod tests {
    ...
    fn origin_client_info_from_meta_extracts_identifier_and_version() {
        let meta = serde_json::json!({
            "clientIdentifier": "intellihelper-desktop",
            "clientVersion": "1.2.3",
        })
        .as_object()
        .cloned()
        ...
        assert_eq!(
            ...
            Some(OriginClientInfo {
                product: "intellihelper-desktop".to_string(),
    ...
    fn origin_client_info_from_meta_uses_client_type_when_identifier_absent() {
        let meta = serde_json::json!({
            "clientType": "intellihelper_pager",
            "clientVersion": "0.1.2",
        })
        .as_object()
        .cloned()
        ...
        assert_eq!(
            origin_client_info_from_meta(Some(&meta)),
            Some(OriginClientInfo {
                product: "intellihelper-pager".to_string(),
    ...
    fn merge_origin_client_info_preserves_primary_product_and_backfills_version() {
        let merged = merge_origin_client_info(
            Some(OriginClientInfo {
                product: "intellihelper-web".to_string(),
                version: None,
            }),
            Some(OriginClientInfo {
                product: "intellihelper-desktop".to_string(),
    ...
    fn session_user_agent_string_renders_expected_variants() {
        ...
        assert!(with_version.starts_with("intellihelper-desktop/1.2.3 intellihelper-shell/"));
        ...
        assert!(without_version.starts_with("intellihelper-web intellihelper-shell/"));
        assert!(!without_version.starts_with("intellihelper-web/"));
    }
    ...
    fn user_agent_render_collapses_duplicate_origin_and_agent_identity() {
        let ua = UserAgent {
            origin: OriginClientInfo {
                product: "intellihelper-shell".to_string(),
        ...
        assert_eq!(ua.render(), "intellihelper-shell/0.1.171 (macos; aarch64)");
    }
}
