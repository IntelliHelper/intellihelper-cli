...
//! run) — and can be forced with `INTELLIHELPER_VOICE_CAPTURE=inprocess`.
...
/// Env escape hatch: `INTELLIHELPER_VOICE_CAPTURE=inprocess` forces the legacy
/// in-process cpal backend (accepting its permanent footprint cost).
const CAPTURE_BACKEND_ENV: &str = "INTELLIHELPER_VOICE_CAPTURE";
