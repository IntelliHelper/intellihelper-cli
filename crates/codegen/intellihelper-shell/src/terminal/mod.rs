...
/// Resolved absolute path to bash. On Unix uses the `intellihelper_config` shell
/// resolution cascade (`$INTELLIHELPER_SHELL` > `$SHELL` > `which` > common dirs >
...
pub(crate) fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        intellihelper_config::shell::unix_shell_path(intellihelper_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `intellihelper-tools`.
pub use intellihelper_tools::util::pager_env;
...
/// intellihelper-pager which renders its own UI and doesn't need raw ANSI codes).
///
