...
use super::super::IntelliHelperAuth;
use super::protocol::{OidcError, OidcUserInfo, build_intellihelper_auth, discover, refresh_tokens};
...
pub(crate) async fn oidc_token_exchange(auth: &IntelliHelperAuth) -> OidcRefreshResult {
                ...
                && let Some(reason) = classify_terminal(&error_code)
            {
                ...
                crate::unified_log::error(
                    ...
                    Some(serde_json::json!({
                        ...
                        "client_id": client_id,
                        "tried_rt_prefix": auth.refresh_token.as_deref().map(intellihelper_auth::bearer_suffix),
    ...
    let mut new_auth = build_intellihelper_auth(tokens, user_info, issuer, client_id);
