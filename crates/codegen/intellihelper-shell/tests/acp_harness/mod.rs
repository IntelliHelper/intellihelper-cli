...
fn set_test_env(intellihelper_home: &std::path::Path, server_url: &str) {
    ...
    unsafe {
        ...
        std::env::set_var("INTELLIHELPER_CLI_CHAT_PROXY_BASE_URL", server_url);
        std::env::set_var("INTELLIHELPER_XAI_API_BASE_URL", server_url);
...
/// Runs `body` against a mock inference server with `INTELLIHELPER_HOME` isolated to a
...
where
    F: FnOnce(std::path::PathBuf, std::rc::Rc<intellihelper_test_support::MockInferenceServer>) -> Fut,
    Fut: std::future::Future<Output = ()>,
{
    ...
    let server = std::rc::Rc::new(
        mock_rt
            .block_on(intellihelper_test_support::MockInferenceServer::start())
            .expect("mock server"),
    );
    let intellihelper_home = tempfile::TempDir::new().expect("intellihelper home");
    let workdir = tempfile::TempDir::new().expect("workdir");
    set_test_env(intellihelper_home.path(), &server.url());
