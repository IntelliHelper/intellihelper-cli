...
use intellihelper_tools::implementations::intellihelper_build::web_fetch::{
    DomainMatcher, config::DEFAULT_ALLOWED_DOMAINS, domain::normalize_domain,
};
use intellihelper_tools::types::resources::resolve_model_path;
...
fn permission_mode_artifact_str(mode: intellihelper_telemetry::enums::PermissionMode) -> &'static str {
    use intellihelper_telemetry::enums::PermissionMode;
    match mode {
...
fn spawn_permission_manager_with_pin(
    ...
    let _task = tokio::task::spawn_local(async move {
        ...
        // persist it to ~/.intellihelper/sessions/<cwd>/permission.toml. This caused
        // the allow to survive full restarts (new intellihelper process, new agent
                ...
                PermissionCommand::Request {
                    ...
                    if matches!(&access, AccessKind::Bash(_))
                        && sandbox_may_auto_allow_bash(
                            bash_evaluation.as_ref(),
                            intellihelper_sandbox::should_auto_allow_bash(),
                        )
                        && !policy_forced_prompt
                        && !auto_forced_prompt
                    {
                    ...
                    let (decision, outcome_str, user_prompted) = match &access {
                        ...
                        _ => {
                            ...
                            let (decision, outcome_str) = match &prompt_outcome {
                                ...
                                PromptOutcome::AllowAlways => {
                                    // Fallback clients (Generic / IntelliHelperWeb /
...
mod tests {
    ...
    /// always-approve option is only offered for `IntelliHelperTUI | IntelliHelperPager | Desktop`).
    ...
    async fn auto_classifier_block_denies_then_escalates_to_prompt() {
        ...
        local
            .run_until(async {
                ...
                let client = RecordingClient::default();
                let prompts = client.prompts.clone();
                // IntelliHelperPager wires the always-approve option through to its YOLO
                // toggle; it is the option set the auto path prompts under.
                let (mgr, _e) =
                    manager_with_recording_client(&cwd, None, client, ClientType::IntelliHelperPager);
                mgr.set_auto_mode(true);
                mgr.set_classifier(Some(LlmPermissionClassifier::with_fixed_model_text(
