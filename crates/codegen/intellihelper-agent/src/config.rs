//! Agent definition types — parsed from `.intellihelper/agents/*.md` files.
...
use intellihelper_tools::implementations::codex;
use intellihelper_tools::implementations::intellihelper_build;
use intellihelper_tools::implementations::intellihelper_build_concise;
use intellihelper_tools::implementations::memory;
use intellihelper_tools::implementations::opencode;
use intellihelper_tools::implementations::search_tool;
use intellihelper_tools::implementations::use_tool;
use intellihelper_tools::registry::types::{ToolConfig, ToolServerConfig};
...
/// Orchestrator-specific prompt body appended to the standard IntelliHelperBuild
...
fn bash_tool_config() -> ToolConfig {
    ToolConfig::from(&intellihelper_build::BashTool)
...
fn task_tool_config() -> ToolConfig {
    ToolConfig::from(&intellihelper_build::TaskTool)
...
fn task_output_tool_config() -> ToolConfig {
    ToolConfig::from(&intellihelper_build::TaskOutputTool).with_name("get_command_or_subagent_output")
}
/// `wait_tasks` → `wait_commands_or_subagents`.
fn wait_tasks_tool_config() -> ToolConfig {
    ToolConfig::from(&intellihelper_build::WaitTasksTool).with_name("wait_commands_or_subagents")
}
/// `kill_task` → `kill_command_or_subagent`.
fn kill_task_tool_config() -> ToolConfig {
    ToolConfig::from(&intellihelper_build::KillTaskTool).with_name("kill_command_or_subagent")
}
/// Complete workspace-executable toolset for hub registration.
///
/// Extends `default_intellihelper_build_toolset()` with tools that are dynamically
...
/// zero local dispatch.
pub fn workspace_intellihelper_build_toolset() -> ToolServerConfig {
    let mut tools = default_intellihelper_build_toolset().tools;
    tools.push((&opencode::OpenCodeWriteTool).into());
    tools.push((&intellihelper_build::EnterPlanModeTool).into());
    tools.push((&intellihelper_build::ExitPlanModeTool).into());
    tools.push((&intellihelper_build::AskUserQuestionTool).into());
    tools.push((&intellihelper_build::WebSearchTool).into());
    tools.push((&intellihelper_build::ImageGenTool).into());
    tools.push((&intellihelper_build::ImageToVideoTool).into());
    tools.push((&intellihelper_build::ReferenceToVideoTool).into());
    tools.push((&intellihelper_build::WebFetchTool).into());
    tools.push((&memory::search_tool::MemorySearchImpl).into());
    tools.push((&memory::get_tool::MemoryGetImpl).into());
    tools.push((&intellihelper_build::LspTool).into());
    ToolServerConfig {
        tools,
        behavior_preset: None,
    }
}
/// Toolset for the `intellihelper-computer` (workspace/sandbox) preset.
fn intellihelper_computer_toolset() -> ToolServerConfig {
    #[allow(unused_mut)]
    let mut tools = vec![
        bash_tool_config(),
        (&intellihelper_build::ReadFileTool).into(),
        (&intellihelper_build::SearchReplaceTool).into(),
        (&opencode::OpenCodeWriteTool).into(),
        (&intellihelper_build::ListDirTool).into(),
        (&intellihelper_build::GrepTool).into(),
        (&intellihelper_build::KillTerminalCommandTool).into(),
        (&intellihelper_build::GetTerminalCommandOutputTool).into(),
    ];
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn native_toolset_presets() -> Vec<(&'static str, ToolServerConfig)> {
    vec![
        ("intellihelper-build", workspace_intellihelper_build_toolset()),
        ("intellihelper-build-concise", intellihelper_build_concise_toolset()),
        ("intellihelper-build-plan", intellihelper_build_plan_toolset()),
        ("codex", codex_toolset()),
        ("explore", explore_toolset()),
        ("plan", plan_toolset()),
        ("intellihelper-computer", intellihelper_computer_toolset()),
    ]
}
...
fn default_intellihelper_build_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&intellihelper_build::ReadFileTool).into(),
            ...
            (&intellihelper_build::TodoWriteTool).into(),
            task_output_tool_config(),
            wait_tasks_tool_config(),
            task_tool_config(),
            (&intellihelper_build::SchedulerCreateTool).into(),
            (&intellihelper_build::SchedulerDeleteTool).into(),
            (&intellihelper_build::SchedulerListTool).into(),
            (&intellihelper_build::MonitorTool).into(),
            (&search_tool::SearchTool).into(),
            (&use_tool::UseTool).into(),
            (&intellihelper_build::UpdateGoalTool).into(),
            (&intellihelper_build::WorkflowTool).into(),
        ],
        behavior_preset: None,
    }
}
fn intellihelper_build_concise_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            (&intellihelper_build_concise::BashConciseTool).into(),
            (&intellihelper_build_concise::ReadFileConciseTool).into(),
            (&intellihelper_build_concise::SearchReplaceConciseTool).into(),
            (&intellihelper_build::ListDirTool).into(),
            (&intellihelper_build::GrepTool).into(),
            kill_task_tool_config(),
            (&intellihelper_build::TodoWriteTool).into(),
...
pub fn intellihelper_build_hashline_toolset(
    hashline_tools: Vec<intellihelper_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<intellihelper_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
    ...
    tools.extend([
        ...
        wait_tasks_tool_config(),
        task_tool_config(),
        (&intellihelper_build::WebSearchTool).into(),
        (&intellihelper_build::SchedulerCreateTool).into(),
        (&intellihelper_build::SchedulerDeleteTool).into(),
        (&intellihelper_build::SchedulerListTool).into(),
        (&intellihelper_build::MonitorTool).into(),
...
fn plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            ...
            (&intellihelper_build::ListDirTool).into(),
            (&intellihelper_build::GrepTool).into(),
            // (&intellihelper_build::SkillTool).into(),
            (&intellihelper_build::TodoWriteTool).into(),
...
/// IntelliHelper CLI + plan mode toolset.
///
/// Extends the default `intellihelper-build` toolset with plan mode tools:
...
fn intellihelper_build_plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            // Standard intellihelper-build tools
            bash_tool_config(),
            (&intellihelper_build::ReadFileTool).into(),
            (&intellihelper_build::SearchReplaceTool).into(),
            (&intellihelper_build::ListDirTool).into(),
            ...
            (&intellihelper_build::WorkflowTool).into(),
            // Plan mode tools
            (&intellihelper_build::EnterPlanModeTool).into(),
            (&intellihelper_build::ExitPlanModeTool).into(),
            (&intellihelper_build::AskUserQuestionTool).into(),
        ],
        behavior_preset: None,
    }
}
...
fn orchestrator_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            ...
            // Web tools
            (&intellihelper_build::WebSearchTool).into(),
            (&intellihelper_build::WebFetchTool).into(),
            // Imagine
            (&intellihelper_build::ImageGenTool).into(),
            (&intellihelper_build::ImageToVideoTool).into(),
            (&intellihelper_build::ReferenceToVideoTool).into(),
            // Memory
...
/// IntelliHelper CLI + plan mode toolset WITHOUT subagent tools.
///
/// Same as `intellihelper_build_plan_toolset` but excludes `TaskTool`,
/// `TaskOutputTool`, and `KillTaskTool`. Use this when the shell
/// does not have subagent infrastructure wired up.
fn intellihelper_build_plan_no_subagents_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            // Standard intellihelper-build tools (minus TaskTool only — KillTaskTool and
...
/// Default IntelliHelper CLI toolset + `ask_user_question`.
///
/// Same as `default_intellihelper_build_toolset` with the `AskUserQuestionTool` added,
/// allowing the agent to ask structured questions without full plan mode.
fn intellihelper_build_ask_user_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&intellihelper_build::ReadFileTool).into(),
...
/// - `model: intellihelper-3-fast` → `Override("intellihelper-3-fast")`
...
pub enum BuiltinAgentName {
    ...
    IntelliHelperBuildPlan,
    IntelliHelperBuildPlanNoSubagents,
    IntelliHelperBuildAskUser,
    Codex,
    Opencode,
    GeneralPurpose,
    Explore,
    Plan,
    BrowserUse,
    #[strum(serialize = "intellihelper-build-orchestrator")]
    IntelliHelperBuildOrchestrator,
}
...
impl BuiltinAgentName {
    ...
    pub fn definition(self) -> AgentDefinition {
        match self {
            Self::IntelliHelperBuild => AgentDefinition::default_intellihelper_build(),
            Self::IntelliHelperBuildConcise => AgentDefinition::intellihelper_build_concise(),
            Self::IntelliHelperBuildPlan => AgentDefinition::intellihelper_build_plan(),
            Self::IntelliHelperBuildPlanNoSubagents => AgentDefinition::intellihelper_build_plan_no_subagents(),
            Self::IntelliHelperBuildAskUser => AgentDefinition::intellihelper_build_ask_user(),
            ...
            Self::IntelliHelperBuildOrchestrator => AgentDefinition::intellihelper_build_orchestrator(),
        }
    }
...
/// Portable agent identity — parsed from .intellihelper/agents/*.md.
...
pub struct AgentDefinition {
    ...
    #[serde(default = "default_intellihelper_build_toolset")]
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub tool_overrides: Option<intellihelper_sampling_types::ToolOverrides>,
...
pub enum AgentScope {
    /// .intellihelper/agents/ (project-level, highest priority)
    Project,
    /// ~/.intellihelper/agents/ (user-level)
    User,
    /// ~/.intellihelper/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
    /// Built-in agent (e.g., default_intellihelper_build(), browser_use()).
    #[default]
    BuiltIn,
}
impl AgentScope {
...
pub enum MemoryScope {
    /// `~/.intellihelper/agent-memory/<name>/`
    User,
    /// `<project>/.intellihelper/agent-memory/<name>/`
    Project,
    /// `<project>/.intellihelper/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: intellihelper_config::intellihelper_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".intellihelper/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".intellihelper/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// these into `intellihelper_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        let intellihelper = intellihelper_config::user_intellihelper_home();
        let home = dirs::home_dir();
        for (dir, scope) in crate::discovery::user_agent_dirs(home.as_deref(), intellihelper.as_deref()) {
            if path.starts_with(&dir) {
                return scope;
            }
        }
        if path_str.contains(".intellihelper/agents/") || path_str.contains(".intellihelper\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".intellihelper/bundled/agents/")
            || path_str.contains(".intellihelper\\bundled\\agents\\")
        {
...
impl AgentDefinition {
    ...
    /// (`!inject_default_tools`). Stock `intellihelper-build*` agents leave all
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<intellihelper_tools::registry::types::ToolConfig>,
    ) {
        const FILE_TOOL_SLOTS: &[[&str; 2]] = &[
            ["IntelliHelperBuild:read_file", "IntelliHelperBuildHashline:hashline_read"],
            [
                "IntelliHelperBuild:search_replace",
                "IntelliHelperBuildHashline:hashline_edit",
            ],
            ["IntelliHelperBuild:grep", "IntelliHelperBuildHashline:hashline_grep"],
        ];
    ...
    pub fn builtin_defaults(name: &str, description: &str) -> Self {
        Self {
            ...
            tool_config: default_intellihelper_build_toolset(),
    ...
    pub fn default_intellihelper_build() -> Self {
        Self::base(
            BuiltinAgentName::IntelliHelperBuild,
            "IntelliHelper CLI agent for software engineering tasks.",
        )
    }
    /// IntelliHelper CLI Concise agent definition — concise output format for SFT/RL.
    pub fn intellihelper_build_concise() -> Self {
        Self {
            tool_config: intellihelper_build_concise_toolset(),
            agents_md: false,
            ..Self::base(
                BuiltinAgentName::IntelliHelperBuildConcise,
                "IntelliHelper CLI agent with concise output format.",
            )
        }
    }
    /// IntelliHelper CLI agent with plan mode tools.
    pub fn intellihelper_build_plan() -> Self {
        Self {
            tool_config: intellihelper_build_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::IntelliHelperBuildPlan,
                "IntelliHelper CLI agent with plan mode support.",
            )
        }
    }
    /// IntelliHelper CLI + plan mode WITHOUT subagent tools.
    pub fn intellihelper_build_plan_no_subagents() -> Self {
        Self {
            tool_config: intellihelper_build_plan_no_subagents_toolset(),
            ..Self::base(
                BuiltinAgentName::IntelliHelperBuildPlanNoSubagents,
                "IntelliHelper CLI agent with plan mode (no subagents).",
            )
        }
    }
    /// Default IntelliHelper CLI agent with the `ask_user_question` tool.
    pub fn intellihelper_build_ask_user() -> Self {
        Self {
            tool_config: intellihelper_build_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::IntelliHelperBuildAskUser,
                "IntelliHelper CLI agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    /// IntelliHelper CLI Orchestrator — GBL model with full IntelliHelperBuild tools
    ...
    pub fn intellihelper_build_orchestrator() -> Self {
        Self {
            tool_config: orchestrator_toolset(),
            inject_default_tools: false,
            prompt_body: Some(ORCHESTRATOR_PROMPT_BODY.to_string()),
            ..Self::base(
                BuiltinAgentName::IntelliHelperBuildOrchestrator,
                "IntelliHelperBuild orchestrator that delegates coding to specialized subagents",
            )
        }
    }
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, AgentBuildError> {
        ...
        if !value.get("toolConfig").is_some_and(|v| v.is_object()) {
            def.tool_config = default_intellihelper_build_toolset();
        }
...
mod tests {
    ...
    fn toolset_for_preset_resolves_known_names() {
        for name in [
            "intellihelper-build",
            "intellihelper_build",
            "intellihelper-build-concise",
            "intellihelper-build-plan",
            "codex",
            "explore",
            "plan",
            "intellihelper-computer",
            "intellihelper_computer",
        ] {
            assert!(
    ...
    fn presets_select_distinct_toolsets_by_size() {
        let gb = toolset_for_preset("intellihelper-build").unwrap();
    ...
    fn intellihelper_computer_exclusive_ids() -> Vec<String> {
        #[allow(unused_mut)]
        let mut ids: Vec<String> = vec![
            ToolConfig::from(&intellihelper_build::GetTerminalCommandOutputTool).id,
            ToolConfig::from(&intellihelper_build::KillTerminalCommandTool).id,
        ];
        ids
    }
    #[test]
    fn intellihelper_computer_preset_is_curated_intellihelper_build_subset() {
        let gc = toolset_for_preset("intellihelper-computer").unwrap();
        let gb = toolset_for_preset("intellihelper-build").unwrap();
        let gb_ids: std::collections::HashSet<&str> =
            gb.tools.iter().map(|t| t.id.as_str()).collect();
        let exclusive_ids = intellihelper_computer_exclusive_ids();
        ...
        for t in &gc.tools {
            ...
            assert!(
                ...
                "intellihelper-computer tool `{}` must also ship in the intellihelper-build preset",
                t.id
            );
        }
        assert!(
            gc.tools.len() < gb.tools.len(),
            "intellihelper-computer should be a curated subset of intellihelper-build"
        );
    }
    #[test]
    fn intellihelper_computer_uses_subagent_free_background_task_tools() {
        let gc = toolset_for_preset("intellihelper-computer").unwrap();
        let ids: std::collections::HashSet<&str> = gc.tools.iter().map(|t| t.id.as_str()).collect();
        assert!(
            ids.contains(
                ToolConfig::from(&intellihelper_build::GetTerminalCommandOutputTool)
                    .id
                    .as_str()
            )
        );
        assert!(
            ids.contains(
                ToolConfig::from(&intellihelper_build::KillTerminalCommandTool)
                    .id
                    .as_str()
            )
        );
        assert!(!ids.contains(ToolConfig::from(&intellihelper_build::TaskOutputTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&intellihelper_build::KillTaskTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&intellihelper_build::TaskTool).id.as_str()));
        for t in &gc.tools {
            if t.id == ToolConfig::from(&intellihelper_build::GetTerminalCommandOutputTool).id
                || t.id == ToolConfig::from(&intellihelper_build::KillTerminalCommandTool).id
            {
                assert!(t.name_override.is_none(), "tool `{}` must not rename", t.id);
            }
        }
    }
    /// The intellihelper-computer preset must ship a full-file write tool (legacy
    /// `write_file` parity) — the same OpenCode `write` tool the intellihelper-build
    ...
    #[test]
    fn intellihelper_computer_preset_includes_write_tool() {
        let gc = toolset_for_preset("intellihelper-computer").unwrap();
        let write_id = ToolConfig::from(&opencode::OpenCodeWriteTool).id;
        assert!(
            gc.tools.iter().any(|t| t.id == write_id),
            "intellihelper-computer preset must include the `{write_id}` tool"
        );
    }
    #[test]
    fn intellihelper_computer_preset_excludes_plan_and_lsp() {
        let gc = toolset_for_preset("intellihelper-computer").unwrap();
        let gc_ids: std::collections::HashSet<&str> =
            gc.tools.iter().map(|t| t.id.as_str()).collect();
        for excluded in [
            ToolConfig::from(&intellihelper_build::LspTool).id,
            ToolConfig::from(&intellihelper_build::EnterPlanModeTool).id,
            ToolConfig::from(&intellihelper_build::ExitPlanModeTool).id,
        ] {
            assert!(
                !gc_ids.contains(excluded.as_str()),
                "intellihelper-computer preset must not advertise `{excluded}`"
            );
        }
        let full = workspace_intellihelper_build_toolset();
        ...
        for present in [
            ...
            assert!(
                ...
                "workspace_intellihelper_build_toolset must ship `{present}`"
            );
        }
    }
    ...
    fn expected_strict_harness(name: BuiltinAgentName) -> bool {
        match name {
            BuiltinAgentName::Codex | BuiltinAgentName::IntelliHelperBuildOrchestrator => true,
            BuiltinAgentName::IntelliHelperBuild
            | BuiltinAgentName::IntelliHelperBuildConcise
            | BuiltinAgentName::IntelliHelperBuildPlan
            | BuiltinAgentName::IntelliHelperBuildPlanNoSubagents
    ...
    fn is_strict_harness_agent_type_classifies_by_name() {
        for strict in ["codex", "intellihelper-build-orchestrator"] {
            assert!(
        ...
        for non_strict in [
            ...
            "intellihelper-build-plan",
            "intellihelper-build-concise",
            "intellihelper-build-ask-user",
            "opencode",
            "browser-use",
            "custom-user-agent",
            "",
            "intellihelper-build-totally-made-up",
        ] {
            assert!(
...
"#;
    ...
    fn test_model_override_display_shows_id() {
        ...
        assert_eq!(
            ModelOverride::Override("intellihelper-3-fast".to_string()).to_string(),
            "intellihelper-3-fast"
        );
    }
    #[test]
...
"#;
        ...
        assert_eq!(def.model, ModelOverride::Override("intellihelper-3".to_string()));
    }
    #[test]
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.intellihelper/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.intellihelper/agent-memory-local/a")
        );
...
"#;
    ...
    #[test]
    fn test_default_tool_config_has_intellihelper_build_tools() {
        let content = r#"---
name: default-tools
...
"#;
        ...
        assert!(
            ...
            "default tool_config should have intellihelper_build tools"
        );
    }
    #[test]
    ...
    fn test_from_json_has_default_toolset_with_task_tool() {
        ...
        let def = AgentDefinition::from_json(&json).unwrap();
        let task_tool_id = "IntelliHelperBuild:task";
        assert!(
    ...
    fn test_model_override_serde_explicit_model_id() {
        let yaml = "\"intellihelper-3-fast\"";
        let m: ModelOverride = serde_yaml::from_str(yaml).unwrap();
        assert_eq!(m, ModelOverride::Override("intellihelper-3-fast".to_string()));
    }
    #[test]
    ...
    fn test_model_override_serialize_override() {
        let m = ModelOverride::Override("intellihelper-3-fast".to_string());
        let s = serde_json::to_string(&m).unwrap();
        assert_eq!(s, "\"intellihelper-3-fast\"");
    }
    #[test]
    fn test_model_override_in_frontmatter() {
        let content = "---\nname: test\ndescription: Test\nmodel: intellihelper-3-fast\n---\n";
        let def = AgentDefinition::parse(content).unwrap();
        assert_eq!(
            def.model,
            ModelOverride::Override("intellihelper-3-fast".to_string())
        );
    }
    #[test]
    ...
    fn test_model_override_in_json() {
        let json = serde_json::json!({
            "name": "test",
            "description": "Test",
            "model": "intellihelper-code-fast-1"
        });
        let def = AgentDefinition::from_json(&json).unwrap();
        assert_eq!(
            def.model,
            ModelOverride::Override("intellihelper-code-fast-1".to_string())
        );
    }
    #[test]
    fn test_builtin_agent_name_strum_round_trip() {
        use std::str::FromStr;
        for (s, expected) in [
            ("intellihelper-build", BuiltinAgentName::IntelliHelperBuild),
            ("intellihelper-build-concise", BuiltinAgentName::IntelliHelperBuildConcise),
            ("intellihelper-build-ask-user", BuiltinAgentName::IntelliHelperBuildAskUser),
    ...
    fn def_with_template(tpl: crate::prompt::context::TemplateOverride) -> AgentDefinition {
        let mut def = AgentDefinition::default_intellihelper_build();
