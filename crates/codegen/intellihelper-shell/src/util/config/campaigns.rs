...
use intellihelper_config::campaigns::{
    CampaignEntry, filter_active_campaigns, ids_touching_paths, merge_campaign_entries,
};
use intellihelper_config::config_override::{PatchPath, patch_touches_any};
use intellihelper_config::{
    CampaignsState, ConfigLayers, campaigns_state_path, load_dismissed_ids_from_home,
    user_intellihelper_home,
};
use intellihelper_config_types::{CampaignOverride, RemoteSettings};
...
pub(crate) fn dismiss_campaign_ids(ids: impl IntoIterator<Item = String>) {
    let Some(home) = user_intellihelper_home() else {
        return;
    };
...
fn dismiss_campaign_ids_at(
    ...
    // several intellihelper processes share `$INTELLIHELPER_HOME`; the in-process mutex alone would
...
/// `INTELLIHELPER_CAMPAIGNS_OVERRIDE` JSON array replaces all sources (`[]` = none; beats
...
pub(crate) fn campaigns_override() -> Option<Vec<CampaignEntry>> {
    let json = std::env::var("INTELLIHELPER_CAMPAIGNS_OVERRIDE").ok()?;
    match serde_json::from_str::<Vec<CampaignOverride>>(&json) {
        ...
        Err(e) => {
            tracing::warn!(error = %e, "invalid INTELLIHELPER_CAMPAIGNS_OVERRIDE JSON; suppressing all campaigns");
            Some(Vec::new())
        }
    }
}
...
/// The single campaign-resolution path: `INTELLIHELPER_CAMPAIGNS_OVERRIDE` (replaces all
...
/// `INTELLIHELPER_CAMPAIGNS_OVERRIDE`. For one-shot CLI entrypoints that never fetch
...
/// without touching `INTELLIHELPER_HOME` / the process-global cache).
...
pub async fn persist_models_default(
    value: Option<String>,
    reasoning_effort: Option<intellihelper_sampling_types::ReasoningEffort>,
) -> anyhow::Result<()> {
...
mod tests {
    ...
    use intellihelper_config::ConfigLayers;
    ...
    /// `INTELLIHELPER_CAMPAIGNS_OVERRIDE` applies despite the kill switch; without it the
    ...
    fn override_beats_kill_switch() {
...

        {
            let _env = EnvGuard::set(
                "INTELLIHELPER_CAMPAIGNS_OVERRIDE",
        ...
        let _env = EnvGuard::unset("INTELLIHELPER_CAMPAIGNS_OVERRIDE");
        let active = resolve_active_campaigns_from_layers(&layers, &base, &[], &HashSet::new());
        assert!(
            active.is_empty(),
    ...
    /// Invalid `INTELLIHELPER_CAMPAIGNS_OVERRIDE` JSON fails toward *no campaigns*: the
    ...
    fn invalid_override_json_suppresses_all_campaigns() {
        let _env = EnvGuard::set("INTELLIHELPER_CAMPAIGNS_OVERRIDE", "{ not json");
    ...
    /// made while `INTELLIHELPER_CAMPAIGNS=0` must still record the dismissal, or a
    /// later re-enabled campaign would override the user's explicit choice.
    #[test]
    #[serial]
    fn dismiss_resolution_ignores_kill_switch() {
        let _over = EnvGuard::unset("INTELLIHELPER_CAMPAIGNS_OVERRIDE");
        let _kill = EnvGuard::set("INTELLIHELPER_CAMPAIGNS", "0");
    ...
    fn campaign_driven_models_default_tracks_remote_and_dismissals() {
        let _over = EnvGuard::unset("INTELLIHELPER_CAMPAIGNS_OVERRIDE");
        let _kill = EnvGuard::unset("INTELLIHELPER_CAMPAIGNS");
    ...
    /// `INTELLIHELPER_CAMPAIGNS_OVERRIDE="[]"` replaces all sources with nothing — even
    /// layer + remote campaigns resolve to empty.
    #[test]
    #[serial]
    fn override_empty_means_none() {
        let _env = EnvGuard::set("INTELLIHELPER_CAMPAIGNS_OVERRIDE", "[]");
