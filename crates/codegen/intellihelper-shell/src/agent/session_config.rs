...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct IntelliHelperSessionDetail {
...
impl IntelliHelperSessionDetail {
    pub(crate) fn build(
        session_id: String,
        cwd: String,
...
mod tests {
    ...
    fn options_have_one_selected_model_and_a_mode_per_effort() {
        let models = [
            model("intellihelper-build", "IntelliHelper CLI"),
            model("intellihelper-4.5", "IntelliHelper 4.5"),
        ];
        let current = acp::ModelId::from("intellihelper-build");
        ...
        assert_eq!(selected_models[0].id, "intellihelper-build");
    ...
    fn none_effort_is_not_a_user_selectable_mode() {
        ...
        let models = [model("intellihelper-build", "IntelliHelper CLI")];
        let current = acp::ModelId::from("intellihelper-build");
        let opts = build_session_config_options(
            &models,
            &current,
    ...
    fn model_label_falls_back_to_id_when_name_empty() {
        let models = [model("intellihelper-build", "")];
        let current = acp::ModelId::from("intellihelper-build");
        let opts = build_session_config_options(&models, &current, &[], None);
        assert_eq!(opts[0].label, "intellihelper-build");
    }

    #[test]
    fn session_config_option_serializes_camel_case() {
        let opt = SessionConfigOption {
            id: "intellihelper-build".to_string(),
            category: "model".to_string(),
            label: "IntelliHelper CLI".to_string(),
            description: None,
            selected: true,
        };
        let v = serde_json::to_value(&opt).expect("serialize");
        assert_eq!(v["id"], "intellihelper-build");
        assert_eq!(v["category"], "model");
        assert_eq!(v["label"], "IntelliHelper CLI");
        assert_eq!(v["selected"], true);
        assert!(v.get("description").is_none());
    }

    #[test]
    fn intellihelper_session_detail_serializes_camel_case() {
        let detail = IntelliHelperSessionDetail::build(
            "sess-1".to_string(),
        ...
        assert_eq!(v["currentModelId"], "intellihelper-build");
