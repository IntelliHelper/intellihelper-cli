...
pub enum Command {
    /// Run IntelliHelper without the interactive UI
    Agent(Box<AgentArgs>),
    /// Show the configuration IntelliHelper discovers for this directory
    Inspect {
        /// Emit machine-readable JSON output.
        #[arg(long)]
        json: bool,
    },
    /// Check terminal, clipboard, color, and input support without starting IntelliHelper
    Doctor(crate::doctor_cmd::DoctorArgs),
    /// Manage running leader processes
    Leader(LeaderMgmtArgs),
    /// Sign out and clear cached credentials
    Logout,
    /// Sign in to IntelliHelper
    Login {
        /// Ignored (kept for backwards compatibility). OAuth2 is now the only auth method.
        #[arg(long, hide = true)]
        legacy: bool,
        /// Use IntelliHelper OAuth via auth.x.ai.
    ...
    Setup {
        ...
        /// writes nothing to ~/.intellihelper.
        #[arg(long)]
        json: bool,
    },
...
Examples:
  intellihelper wrap docker exec -it my-container bash
  intellihelper wrap kubectl exec -it my-pod -- bash

See ~/.intellihelper/README.md for more information.
")]
    Wrap(WrapArgs),
    ...
    /// `INTELLIHELPER_WORKSPACE_COMMAND=1` to enable it locally for testing.
    #[command(hide = true)]
    ...
    /// `~/.intellihelper/config.toml` or when the `INTELLIHELPER_AGENT_DASHBOARD=0` env
    /// var is set.
    Dashboard,
}
...
/// Targets a running leader process by PID (used by `intellihelper leader` / `intellihelper workspace`).
#[derive(Debug, clap::Args, Clone, Default)]
pub struct LeaderTargetArgs {
    /// Leader process ID from `intellihelper leader list`.
    #[arg(long)]
    pub pid: Option<u32>,
}
#[derive(Debug, clap::Args, Clone)]
    ...
    pub fn canonical_plugin_dirs(&self) -> Vec<PathBuf> {
        self.plugin_dirs
            ...
            .filter_map(|p| match dunce::canonicalize(p) {
                ...
                Ok(_) => {
                    eprintln!(
                        "intellihelper: --plugin-dir {}: not a directory; skipping",
                        p.display()
                    );
                    None
                }
                Err(e) => {
                    eprintln!("intellihelper: --plugin-dir {}: {e}; skipping", p.display());
                    None
                }
            })
            .collect()
...
pub enum AgentCmd {
    ...
    Stdio,
    /// Run the agent headlessly over the IntelliHelper WebSocket relay
...
pub struct HeadlessArgs {
    #[arg(long = "intellihelper-ws-origin")]
    pub intellihelper_ws_origin: Option<String>,
    #[arg(long = "intellihelper-ws-url")]
    pub intellihelper_ws_url: Option<String>,
}
...
pub struct ServeArgs {
    ...
    #[arg(long, env = "INTELLIHELPER_AGENT_SECRET")]
...
pub struct LeaderArgs {
    ...
    /// Defer the intellihelper.com relay WebSocket until the first headless IPC client
...
pub struct PagerArgs {
    ...
    #[arg(long)]
    pub cwd: Option<PathBuf>,
    /// Use a custom leader socket path instead of the default `~/.intellihelper/leader.sock`.
    #[arg(
        long = "leader-socket",
        value_name = "PATH",
        global = true,
    ...
    /// persists per-segment markdown to grep. Sets `INTELLIHELPER_COMPACTION_MODE`.
    ...
    /// `INTELLIHELPER_COMPACTION_DETAIL`.
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            intellihelper_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
    ...
    /// Default for all `intellihelper -p` runs is to wait (up to `--background-wait-timeout`)
    ...
    #[arg(long, env = "INTELLIHELPER_SANDBOX", value_name = "PROFILE")]
    ...
    /// Session-scoped only — does not write config. To default plain `intellihelper` to
    /// minimal, set `[ui] screen_mode = "minimal"` in ~/.intellihelper/config.toml.
    #[arg(long = "minimal")]
    pub minimal: bool,
    ...
    pub fullscreen: bool,
    /// Write sampling events to ~/.intellihelper/logs/sampling.jsonl.
    #[arg(long = "log-sampling", env = "INTELLIHELPER_LOG_SAMPLING", hide = true)]
    ...
    pub no_leader: bool,
    /// Initial prompt for the interactive session, e.g. `intellihelper "fix the bug"` or `intellihelper --worktree=feat "create this feature"`.
    #[arg(
        value_name = "PROMPT",
        conflicts_with_all = &["single",
        "prompt_json",
...
impl PagerArgs {
    ...
    pub fn parse_cli() -> Self {
        let bin_name = std::env::args()
            ...
            .and_then(|n| n.to_str())
            .filter(|n| *n == "intellihelper" || *n == "agent")
            .unwrap_or("intellihelper")
            .to_owned();
    ...
    /// explicit `--sandbox`/`INTELLIHELPER_SANDBOX` that differs from the saved profile is
    ...
    pub fn saved_resume_profile_for_cwd(&self, cwd: Option<&str>) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                intellihelper_shell::session::persistence::resumed_session_sandbox_profile(
                    Some(&id),
                    cwd,
                )
            }
            ResumeTarget::MostRecentForCwd => {
                intellihelper_shell::session::persistence::resumed_session_sandbox_profile(None, cwd)
            }
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<intellihelper_sandbox::ProfileName>().ok()
                    != s.parse::<intellihelper_sandbox::ProfileName>().ok() =>
            {
    ...
    /// whitespace. This is the `intellihelper "<prompt>"` launch form; the headless
...
mod tests {
    ...
    fn version_flags_parse_as_early_intent_without_exiting() {
        for flag in ["--version", "-v", "-V"] {
            let args = PagerArgs::try_parse_from(["intellihelper", flag]).expect("version flag parses");
    ...
    fn ordinary_and_doctor_parsing_do_not_set_version_intent() {
        assert!(!PagerArgs::try_parse_from(["intellihelper"]).unwrap().version);
        assert!(
            !PagerArgs::try_parse_from(["intellihelper", "doctor"])
                .unwrap()
                .version
        );
        assert!(matches!(
            PagerArgs::try_parse_from(["intellihelper", "version"])
                .unwrap()
                .command,
    ...
    fn doctor_accepts_report_and_explicit_fix_forms() {
        let bare = PagerArgs::try_parse_from(["intellihelper", "doctor"]).expect("bare doctor parses");
        assert!(matches!(
            bare.command,
        ...
        let json =
            PagerArgs::try_parse_from(["intellihelper", "doctor", "--json"]).expect("doctor --json parses");
        assert!(matches!(
            json.command,
            Some(Command::Doctor(crate::doctor_cmd::DoctorArgs {
                json: true,
        ...
        for id in [
            ...
            let fix = PagerArgs::try_parse_from(["intellihelper", "doctor", "fix", id, "--yes"])
        ...
        let list = PagerArgs::try_parse_from(["intellihelper", "doctor", "fix"])
        ...
        for unsupported in [
            vec!["intellihelper", "doctor", "all"],
            vec!["intellihelper", "doctor", "fix", "ssh-wrap", "extra"],
            vec!["intellihelper", "doctor", "fix", "--yes"],
            vec!["intellihelper", "doctor", "--json", "fix", "terminal.ssh-wrap"],
        ] {
    ...
    fn resume_target_classifies_flags() {
        assert_eq!(
            PagerArgs::try_parse_from(["intellihelper"]).unwrap().resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["intellihelper", "-c"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["intellihelper", "--resume"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["intellihelper", "--resume", "sess-1"])
                .unwrap()
                .resume_target(),
            ResumeTarget::SessionId("sess-1".to_string())
        );
        assert_eq!(
            PagerArgs::try_parse_from(["intellihelper", "-s", "sess-2"])
                .unwrap()
                .resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["intellihelper", "-r", "old", "--fork-session"])
                .unwrap()
                .resume_target(),
    ...
    fn minimal_and_fullscreen_flags_conflict() {
        let args = PagerArgs::try_parse_from(["intellihelper", "--minimal"]).unwrap();
        assert!(args.minimal && !args.fullscreen);
        let args = PagerArgs::try_parse_from(["intellihelper", "--fullscreen"]).unwrap();
        assert!(args.fullscreen && !args.minimal);
        let err = PagerArgs::try_parse_from(["intellihelper", "--minimal", "--fullscreen"]).unwrap_err();
    ...
    fn startup_sandbox_profile_no_resume() {
        assert_eq!(
            PagerArgs::try_parse_from(["intellihelper", "--sandbox", "strict"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(Some("strict".to_string()))
        );
        assert_eq!(
            PagerArgs::try_parse_from(["intellihelper", "--sandbox", ""])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(None)
        );
        assert_eq!(
            PagerArgs::try_parse_from(["intellihelper"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(None)
        );
    ...
    fn leader_socket_flag_parses_at_root() {
        let args = PagerArgs::try_parse_from(["intellihelper", "--leader-socket", "/tmp/leader-x.sock"])
    ...
    fn leader_socket_flag_defaults_to_none() {
        let args = PagerArgs::try_parse_from(["intellihelper"]).expect("bare intellihelper parses");
        assert!(args.leader_socket.is_none());
    }
    #[test]
    fn leader_mgmt_list_info_kill_parse() {
        let list = PagerArgs::try_parse_from(["intellihelper", "leader", "list", "--json"])
            .expect("intellihelper leader list --json");
        assert!(matches!(
            list.command,
        ...
        let info = PagerArgs::try_parse_from(["intellihelper", "leader", "info", "--pid", "42"])
            .expect("intellihelper leader info --pid");
        assert!(matches!(
            info.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        let kill = PagerArgs::try_parse_from(["intellihelper", "leader", "kill"]).expect("intellihelper leader kill");
        assert!(matches!(
            kill.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        assert!(PagerArgs::try_parse_from(["intellihelper", "leader", "profile"]).is_err());
    }
    #[test]
    fn debug_file_flag_parses_and_is_global() {
        let root = PagerArgs::try_parse_from(["intellihelper", "--debug-file", "/tmp/fire.txt"])
        ...
        let sub =
            PagerArgs::try_parse_from(["intellihelper", "agent", "stdio", "--debug-file", "/tmp/f.txt"])
    ...
    fn positional_prompt_seeds_interactive_session() {
        let args =
            PagerArgs::try_parse_from(["intellihelper", "fix the bug"]).expect("positional prompt parses");
    ...
    fn initial_prompt_trims_and_ignores_whitespace_only() {
        let args = PagerArgs::try_parse_from(["intellihelper", "  spaced  "]).expect("padded prompt parses");
        assert_eq!(args.initial_prompt(), Some("spaced"));
        let blank = PagerArgs::try_parse_from(["intellihelper", "   "]).expect("blank prompt parses");
        assert_eq!(blank.initial_prompt(), None);
    }
    #[test]
    fn subcommand_takes_precedence_over_positional_prompt() {
        let args = PagerArgs::try_parse_from(["intellihelper", "logout"]).expect("subcommand parses");
        assert!(matches!(args.command, Some(Command::Logout)));
        assert!(args.prompt.is_none());
    }
    #[test]
    fn positional_prompt_conflicts_with_headless_single() {
        let err = PagerArgs::try_parse_from(["intellihelper", "-p", "headless", "interactive"])
            .expect_err("positional prompt + --single must conflict");
        assert_eq!(err.kind(), clap::error::ErrorKind::ArgumentConflict);
    }
    #[test]
    fn worktree_flag_and_initial_prompt_combine() {
        let a = PagerArgs::try_parse_from(["intellihelper", "do the thing", "-w"])
            .expect("prompt then bare -w parses");
        assert_eq!(a.initial_prompt(), Some("do the thing"));
        assert_eq!(a.worktree.as_deref(), Some(""));
        let b = PagerArgs::try_parse_from(["intellihelper", "--worktree=feat", "do the thing"])
        ...
        let c = PagerArgs::try_parse_from(["intellihelper", "-w", "x"]).expect("-w x parses");
        assert_eq!(c.worktree.as_deref(), Some("x"));
        assert_eq!(c.initial_prompt(), None);
    }
    #[test]
    fn trust_flag_parses_on_pager_and_alias() {
        let bare = PagerArgs::try_parse_from(["intellihelper"]).expect("bare intellihelper parses");
        assert!(!bare.trust);
        let long = PagerArgs::try_parse_from(["intellihelper", "--trust"]).expect("--trust parses");
        assert!(long.trust);
        let alias =
            PagerArgs::try_parse_from(["intellihelper", "--trust-folder"]).expect("--trust-folder parses");
        assert!(alias.trust);
    }
    #[test]
    fn reasoning_effort_and_effort_alias_parse_same_field() {
        let long = PagerArgs::try_parse_from(["intellihelper", "--reasoning-effort", "high"])
            .expect("--reasoning-effort parses");
        assert_eq!(long.reasoning_effort.as_deref(), Some("high"));
        let alias =
            PagerArgs::try_parse_from(["intellihelper", "--effort", "high"]).expect("--effort alias parses");
        assert_eq!(alias.reasoning_effort.as_deref(), Some("high"));
    }
    #[test]
    fn reasoning_effort_accepts_max_and_remapped_ids() {
        let max = PagerArgs::try_parse_from(["intellihelper", "--effort", "max"]).expect("max parses");
        assert_eq!(max.reasoning_effort.as_deref(), Some("max"));
        let deep =
            PagerArgs::try_parse_from(["intellihelper", "--reasoning-effort", "deep"]).expect("deep parses");
        assert_eq!(deep.reasoning_effort.as_deref(), Some("deep"));
    }
    #[test]
    fn reasoning_effort_last_flag_wins_when_both_names_set() {
        let args =
            PagerArgs::try_parse_from(["intellihelper", "--reasoning-effort", "low", "--effort", "high"])
                .expect("both effort flag names parse");
        assert_eq!(args.reasoning_effort.as_deref(), Some("high"));
        let reverse =
            PagerArgs::try_parse_from(["intellihelper", "--effort", "high", "--reasoning-effort", "low"])
    ...
    fn agent_args_effort_alias_parses() {
        let args = PagerArgs::try_parse_from(["intellihelper", "agent", "--effort", "max", "stdio"])
