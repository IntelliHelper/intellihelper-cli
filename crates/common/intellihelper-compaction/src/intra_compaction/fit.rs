...
//!                          that alone exceed budget (intellihelper-build style:
...
//!   (one-shot, same `tokens * 4` budget as intellihelper-build)
...
/// One-shot payload shrink (intellihelper-build: `max_bytes = max_tokens * 4`).
///
...
///
/// Matches intellihelper-build's focus: in-place truncate is for oversized tool
...
pub fn fit_turns_for_summarizer<T: CompactionItemBuilder>(
    ...
    // One-shot prefix clip on tool results only (intellihelper-build style).
    ...
    // intellihelper-build: when even the newest unit alone exceeds budget, keep it
...
///
/// Mirrors intellihelper-build `truncate_text_to_bytes` / `truncate_item_to_tokens`:
