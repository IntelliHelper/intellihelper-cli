//! Headless markdown analysis sharing IntelliHelper CLI's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `intellihelper-markdown` so analysis matches what IntelliHelper
//! Build actually renders 1:1.
//!
//! After parsing, IntelliHelper applies [`offset_events`]: only `~~…~~` is strikethrough.
...
/// The exact `pulldown-cmark` option set IntelliHelper CLI uses to render markdown.
///
...
/// Offset event stream from IntelliHelper's parser, with single-tilde strikethrough demoted.
///
...
/// Parse `text` with IntelliHelper CLI's options; count elements and flag structural issues.
