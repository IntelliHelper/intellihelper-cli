...
const RG_BYTES: &[u8] = include_bytes!(concat!(
    env!("INTELLIHELPER_SHELL_RG_GEN_DIR"),
    "/rg-",
    env!("INTELLIHELPER_SHELL_RG_VER"),
    "-",
    env!("INTELLIHELPER_SHELL_RG_TARGET"),
    ".bin"
));
...
fn resolve_bundled_rg() -> std::io::Result<PathBuf> {
    ...
    let p = intellihelper_tools::util::intellihelper_home::intellihelper_home()
        .join("vendor")
        .join(concat!(
            "rg-",
            env!("INTELLIHELPER_SHELL_RG_VER"),
