...
impl SleepGate {
    pub(super) fn raise(&self) {
        *self.raised_at.write() = Some(DualClock::now());
        intellihelper_telemetry::unified_log::warn("auth.sleep.gate_set", None, None);
    }
...
impl AuthManager {
    ...
    /// [`system_power::PowerState`] for the canonical explanation of what a
    ...
    ///
    /// `INTELLIHELPER_AUTH_FORCE_DARK_WAKE=1|0` forces the answer for testing (unset
    ...
    /// `INTELLIHELPER_AUTH_EARLY_INVALIDATION_SECS` for a seconds-long repro.
    pub(crate) fn is_dark_wake(&self) -> bool {
        #[cfg(test)]
        if let Some(forced) = *self.dark_wake_override.lock() {
            return forced;
        }
        match std::env::var("INTELLIHELPER_AUTH_FORCE_DARK_WAKE").ok().as_deref() {
            Some("1") => return true,
            Some("0") => return false,
            _ => {}
        }
