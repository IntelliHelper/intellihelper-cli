...
This crate extracts a first-class `Agent` type from `intellihelper-shell`.
...
`intellihelper-shell`, another in-process host, or a headless batch runner.
...
in `.intellihelper/agents/` (project-level) or `~/.intellihelper/agents/` (user-level).

```rust
use intellihelper_agent::{AgentDefinition, AgentBuilder};
use intellihelper_tools::notification::ToolNotificationHandle;

// 1. Parse the definition file
let def = AgentDefinition::from_file(".intellihelper/agents/code-reviewer.md")?;
...
```rust
use intellihelper_agent::discovery;

// Find all .md files in .intellihelper/agents/ directories
...
```
┌──────────────────┐
│  intellihelper-agent  │  ← This crate
│  (Agent, Builder, │
│   Definition)     │
└────────┬─────────┘
         │ depends on
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  intellihelper-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`intellihelper-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `intellihelper-agent` depends on it for tool setup.
- **`intellihelper-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `intellihelper-agent` (AGENTS.md
...
|---|---|---|
| `intellihelper-build` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p intellihelper-agent

# Test
cargo test -p intellihelper-agent

# Clippy
cargo clippy -p intellihelper-agent --fix --allow-dirty
