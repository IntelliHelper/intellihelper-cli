...
//! Delegates to `intellihelper-plugin-marketplace` crate for scanning and install logic.
...
fn load_filtered_marketplace_sources() -> Vec<intellihelper_plugin_marketplace::MarketplaceSource> {
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                intellihelper_plugin_marketplace::SourceKind::Git { url, .. } => url.as_str(),
                intellihelper_plugin_marketplace::SourceKind::Local { path } => {
...
fn refresh_sources(
    sources: &[intellihelper_plugin_marketplace::MarketplaceSource],
    ...
    for source in sources {
        if let Some(filter) = source_url_or_path {
            let identity = match &source.kind {
                ...
                intellihelper_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
            };
            if identity != filter {
                continue;
            }
        }
        if let intellihelper_plugin_marketplace::SourceKind::Git { url, branch } = &source.kind {
            let cache_root = intellihelper_plugin_marketplace::git::default_cache_root();
            if let Err(e) = intellihelper_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                &cache_root,
            ) {
...
async fn handle_update(
    ...
    use intellihelper_plugin_marketplace::installer;
    use hooks_plugins_types::{ActionOutcome, OutcomeStatus};

    let sources = load_filtered_marketplace_sources();

    let source_identity = |s: &intellihelper_plugin_marketplace::MarketplaceSource| -> String {
        match &s.kind {
            intellihelper_plugin_marketplace::SourceKind::Local { path } => path.display().to_string(),
            intellihelper_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
        }
    };
    ...
    let plugin_path =
        match intellihelper_plugin_marketplace::MarketplaceRelativePath::parse(plugin_relative_path) {
            Ok(path) => path,
            Err(e) => {
                return ActionOutcome {
    ...
    let marketplace_root = match &source.kind {
        ...
        intellihelper_plugin_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = intellihelper_plugin_marketplace::git::default_cache_root();
            match intellihelper_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
                intellihelper_plugin_marketplace::git::SyncMode::Force,
            ) {
                Ok(lease) => {
    ...
    let scan = intellihelper_plugin_marketplace::scan_marketplace(&marketplace_root);
    ...
    let provenance = intellihelper_agent::plugins::install_registry::MarketplaceProvenance {
    ...
    let mut registry = intellihelper_agent::plugins::install_registry::InstallRegistry::load();
    ...
    match update_result {
        ...
        Err(intellihelper_agent::plugins::install_registry::InstallError::PluginNotFound { .. }) => {
            ActionOutcome {
                status: OutcomeStatus::NotFound,
                message: format!(
        ...
        let marketplace_root = match &source.kind {
            ...
            intellihelper_plugin_marketplace::SourceKind::Git { url, branch } => {
                ...
                match intellihelper_plugin_marketplace::git::sync_source_cache_with_mode(
                    ...
                    branch.as_deref(),
                    &cache_root,
                    intellihelper_plugin_marketplace::git::SyncMode::UseTtl,
                ) {
                    Ok(lease) => {
...
fn scan_source(
    source: &intellihelper_plugin_marketplace::MarketplaceSource,
    ...
    let scan = intellihelper_plugin_marketplace::scan_marketplace(&root);
    let catalog_loaded = scan.catalog_loaded;
    let discovered = scan.entries;
    drop(lease);

    // Cross-reference with install registry.
    let registry = intellihelper_agent::plugins::install_registry::InstallRegistry::load();
    let plugins = discovered
        .into_iter()
        .map(|p| {
            let (install_status, installed_version) =
                match intellihelper_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: intellihelper_plugin_marketplace::MarketplaceEntry,
...
/// Add a new git or local-path marketplace source to `~/.intellihelper/config.toml`.
async fn handle_add_source(url: &str) -> hooks_plugins_types::ActionOutcome {
    ...
    let allowlist =
        &intellihelper_workspace::permission::resolution::managed_settings().marketplace_allowlist;
    ...
    let existing = intellihelper_plugin_marketplace::load_sources(&config);
    let already_configured = match &input {
        MarketplaceAddInput::GitUrl(git_url) => {
            let normalized = git_url.trim_end_matches(".git");
            existing.iter().any(|s| {
                matches!(&s.kind, intellihelper_plugin_marketplace::SourceKind::Git { url: u, .. }
                    if u.trim_end_matches(".git") == normalized)
            })
        }
        MarketplaceAddInput::LocalPath(path) => existing.iter().any(|s| {
            matches!(&s.kind, intellihelper_plugin_marketplace::SourceKind::Local { path: p }
                if p == path)
        }),
    };
    ...
    if let MarketplaceAddInput::GitUrl(git_url) = &input {
        ...
        let probe = tokio::task::spawn_blocking(move || {
            intellihelper_plugin_marketplace::git::probe_git_remote(&probe_url)
        })
        .await;
        match probe {
            Ok(Ok(())) => {}
                ...
                return ActionOutcome {
                    ...
                    message: format!(
                        "{e}. Not a reachable git repository — to add it anyway (e.g. a \
                         VPN-gated host), run: intellihelper plugin marketplace add {url} --force"
                    ),
                    requires_reload: false,
                    requires_restart: false,
                };
    ...
    let is_official = matches!(&input, MarketplaceAddInput::GitUrl(u)
        if intellihelper_plugin_marketplace::is_official_source_url(u));
    let name = if is_official {
        intellihelper_plugin_marketplace::OFFICIAL_SOURCE_NAME.to_string()
    } else {
        match &input {
    ...
    let config_path = intellihelper_config::intellihelper_home().join("config.toml");
    let intellihelper_home = intellihelper_config::intellihelper_home();
    let _save_guard = crate::util::config::lock_config_writes().await;
    let write = {
        let name = name.clone();
        tokio::task::spawn_blocking(move || {
            let _flock = acquire_init_lock(&intellihelper_home).ok();
...
/// Remove a marketplace source from `~/.intellihelper/config.toml` and uninstall all
...
fn remove_source_locked(source_url_or_path: &str) -> hooks_plugins_types::ActionOutcome {
    ...
    let config_path = intellihelper_home.join("config.toml");
    let is_official = intellihelper_plugin_marketplace::is_official_source_url(source_url_or_path);
...
/// Acquire an advisory exclusive `flock` on `<intellihelper_home>/.config-init.lock`,
...
fn acquire_init_lock(intellihelper_home: &std::path::Path) -> std::io::Result<std::fs::File> {
    use fs2::FileExt;
    let _ = std::fs::create_dir_all(intellihelper_home);
    let lock_path = intellihelper_home.join(".config-init.lock");
...
fn default_skills_repo_keys<'a>(
    repos: impl IntoIterator<
        Item = (
            &'a str,
            &'a intellihelper_agent::plugins::install_registry::InstalledRepo,
        ),
    >,
) -> Vec<&'a str> {
    repos
...
pub(crate) fn purge_default_skills_installs(intellihelper_home: &std::path::Path) {
    purge_default_skills_installs_impl(intellihelper_home, || {
        intellihelper_agent::plugins::install_registry::InstallRegistry::try_load_from(
            intellihelper_agent::plugins::install_registry::InstallRegistry::resolve_install_dir(),
        )
    });
}

fn purge_default_skills_installs_impl(
    intellihelper_home: &std::path::Path,
    load_registry: impl FnOnce() -> Result<
        intellihelper_agent::plugins::install_registry::InstallRegistry,
        intellihelper_agent::plugins::install_registry::InstallError,
    >,
) {
    let config_path = intellihelper_home.join("config.toml");
    ...
    let _lock = match acquire_init_lock(intellihelper_home) {
        Ok(f) => f,
        Err(e) => {
            tracing::warn!(
                error = %e,
                path = %intellihelper_home.join(".config-init.lock").display(),
...
pub(crate) fn ensure_official_marketplace_source(intellihelper_home: &std::path::Path) {
    let config_path = intellihelper_home.join("config.toml");
    ...
    // (settings.json / known_marketplaces.json) under intellihelper_home. Scoped to
    // intellihelper_home only (not ~/.claude) to keep tests hermetic; a user with the URL
    // solely in ~/.claude gets one duplicate entry that the UI dedupes by URL.
    let toml_sources = intellihelper_plugin_marketplace::load_sources(&parsed);
    let json_sources = intellihelper_plugin_marketplace::load_extra_sources_from_settings_in(
        &toml_sources,
        std::slice::from_ref(&intellihelper_home.to_path_buf()),
    );
    let already_present = toml_sources.iter().chain(json_sources.iter()).any(|s| {
        matches!(&s.kind, intellihelper_plugin_marketplace::SourceKind::Git { url, .. }
            if intellihelper_plugin_marketplace::is_official_source_url(url))
    });
    ...
    let write_result = if already_present {
        ...
        add_marketplace_source(
            &config_path,
            intellihelper_plugin_marketplace::OFFICIAL_SOURCE_NAME,
            &crate::plugin::MarketplaceAddInput::GitUrl(
                intellihelper_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string(),
            ),
            true,
        )
    };
    ...
    match write_result {
        Ok(()) if !already_present => {
            tracing::info!(
                url = intellihelper_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
...
mod official_source_tests {
    ...
    fn read_sources(
        config_path: &std::path::Path,
    ) -> Vec<intellihelper_plugin_marketplace::MarketplaceSource> {
        let raw = std::fs::read_to_string(config_path).unwrap_or_default();
        let parsed: toml::Value =
            toml::from_str(&raw).unwrap_or_else(|_| toml::Value::Table(Default::default()));
        intellihelper_plugin_marketplace::load_sources(&parsed)
    }
    ...
    fn add_marketplace_source_local_path_writes_path_entry() {
        ...
        assert!(matches!(
            &sources[0].kind,
            intellihelper_plugin_marketplace::SourceKind::Local { path } if path == &dir
        ));
    ...
    fn removing_last_nonofficial_source_preserves_flag_and_blocks_readd() {
        ...
        assert!(
            !sources.iter().any(|s| matches!(&s.kind,
                intellihelper_plugin_marketplace::SourceKind::Git { url, .. }
                    if intellihelper_plugin_marketplace::is_official_source_url(url))),
    ...
    fn first_run_creates_source_and_sets_flag() {
        ...
        assert!(matches!(
            &sources[0].kind,
            intellihelper_plugin_marketplace::SourceKind::Git { url, .. }
                if url == intellihelper_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL
        ));
    ...
    fn pre_existing_official_source_just_sets_flag() {
        ...
        std::fs::write(
            ...
            format!(
                "[[marketplace.sources]]\nname = \"{}\"\ngit = \"{}\"\n",
                intellihelper_plugin_marketplace::OFFICIAL_SOURCE_NAME,
                intellihelper_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
...
mod default_skills_purge_tests {
    use super::*;
    use intellihelper_agent::plugins::install_registry::{
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = intellihelper_plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
