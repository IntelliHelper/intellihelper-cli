...
/// (`intellihelper-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (intellihelper-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in intellihelper-tools, spawns processes)
/// - `AcpTerminalBackend` (in intellihelper-shell, calls ACP protocol)
#[async_trait::async_trait]
