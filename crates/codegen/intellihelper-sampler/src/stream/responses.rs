    ...
    async_stream::stream! {
        ...
        loop {
            ...
            match event {
                ...
                // split (intellihelper has no HostedTool::CodeInterpreter, so these events
        ...
        let mut items = intellihelper_sampling_types::response_to_conversation_items(response);
        intellihelper_sampling_types::inject_streaming_reasoning_fallback(&mut items, reasoning_acc);
...
mod tests {
    ...
    async fn doom_loop_collector_signals_land_on_completed_response() {
        use intellihelper_sampling_types::doom_loop::{
