...
//!
//! Used by both the ACP `x.ai/session/list` handler and the `intellihelper sessions`
...
pub(crate) async fn fetch_lanes(
    ...
    let repo_urls_fut = async {
        ...
        cwd.map(|c| {
            intellihelper_workspace::session::git::resolve_normalized_remote_urls(std::path::Path::new(
                c,
            ))
        })
        .unwrap_or_default()
    ...
    // `intellihelper --resume <uuid>` resolves across every cwd. `/resume` search was
