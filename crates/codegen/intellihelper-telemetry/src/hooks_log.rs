...
//! file logger that writes to `~/.intellihelper/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `intellihelper_hooks` or `intellihelper_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! INTELLIHELPER_HOOKS_LOG=1 intellihelper              # enable, write to ~/.intellihelper/logs/hooks.log
//! INTELLIHELPER_HOOKS_LOG=/tmp/h.log intellihelper     # write to custom path
//! INTELLIHELPER_HOOKS_LOG=0 intellihelper              # explicitly disable
//! tail -f ~/.intellihelper/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "INTELLIHELPER_HOOKS_LOG";
...
///
/// Writes to `~/.intellihelper/logs/hooks.log` (or custom path via `INTELLIHELPER_HOOKS_LOG`).
/// Filters to hooks (`intellihelper_hooks`) and plugins (`intellihelper_agent::plugins`) targets.
/// Set `INTELLIHELPER_HOOKS_LOG=0` to disable, `INTELLIHELPER_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "intellihelper_hooks=debug,intellihelper_agent::plugins=debug",
    );
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || intellihelper_home().join("logs").join("hooks.log");
