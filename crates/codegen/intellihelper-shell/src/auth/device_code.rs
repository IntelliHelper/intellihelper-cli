...
use crate::auth::{AuthChannels, AuthManager, AuthMode, AuthUrlInfo, AuthUrlMode, IntelliHelperAuth};
...
pub(crate) enum DeviceCodeError {
    #[error(
        "Device-code login is not available for this deployment. \
         Try `intellihelper login` or set XAI_API_KEY instead."
    )]
    NotEnabled,
}
...
/// `x-intellihelper-client-surface` header so device-flow metrics can separate logins a
...
pub(crate) async fn request_device_code(
    ...
    let resp = with_alpha_test_key(
        client
            ...
            .header("x-intellihelper-client-surface", surface.as_str())
            .form(&[
                ("client_id", client_id),
                ("scope", scope_str.as_str()),
                ("referrer", "intellihelper-build"),
            ]),
        &url,
    )
    .send()
...
///
/// On success, persists credentials to `~/.intellihelper/auth.json` and returns
/// the authenticated `IntelliHelperAuth`.
///
...
pub(crate) async fn complete_device_code_login(
    ...
    loop {
        ...
        if tokio::time::Instant::now() > deadline {
            anyhow::bail!("Device code expired. Run `intellihelper login --device-auth` again.");
        }
...
pub(crate) mod tests {
    ...
    fn auth_manager_with_intellihelper_home(
        intellihelper_home: &std::path::Path,
        proxy_base_url: &str,
    ) -> Arc<AuthManager> {
        Arc::new(
            AuthManager::new(intellihelper_home, GrokComConfig::default())
    ...
    fn build_auth_persists_credentials_without_proxy_fetch() {
        ...
        let intellihelper_home = temp_dir.path().join(".intellihelper");
        std::fs::create_dir_all(&intellihelper_home).unwrap();
        let auth_manager = auth_manager_with_intellihelper_home(&intellihelper_home, "http://127.0.0.1:9");
        let tokens = super::TokenOk {
            access_token: "access-token".to_string(),
            refresh_token: Some("refresh-token".to_string()),
            expires_in: Some(900),
            scope: Some("openid email offline_access intellihelper-cli:access".to_string()),
            id_token: Some(
    ...
    fn build_auth_seeds_team_metadata_from_access_token() {
        ...
        let claims = serde_json::json!({
            ...
            "exp": 9999999999u64,
            "iat": 1000000000u64,
            "scope": "offline_access intellihelper-cli:access team:read",
            "principal_type": "Team",
        ...
        let tokens = super::TokenOk {
            ...
            expires_in: Some(900),
            scope: Some("offline_access intellihelper-cli:access team:read".to_owned()),
            id_token: None,
        };
    ...
    fn assert_build_auth_rejected(cfg: GrokComConfig, token_principal: &str, expected_err: &str) {
        ...
        std::fs::create_dir_all(&intellihelper_home).unwrap();
        let auth_manager =
            Arc::new(AuthManager::new(&intellihelper_home, cfg).with_proxy_base_url("http://127.0.0.1:9"));
        ...
        assert!(
            !intellihelper_home.join("auth.json").exists(),
    ...
    async fn run_poll(
        responses: Vec<(u16, serde_json::Value)>,
    ) -> anyhow::Result<(super::IntelliHelperAuth, bool)> {
        let (issuer, server) = spawn_token_server(responses).await;
        let temp_dir = tempfile::tempdir().unwrap();
        let auth_manager = auth_manager_with_intellihelper_home(temp_dir.path(), "http://127.0.0.1:9");
