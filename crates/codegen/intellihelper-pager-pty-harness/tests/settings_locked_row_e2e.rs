...
//! `IntelliHelperAuth` → auth meta → `AppView::coding_data_sharing_lock()` →
...
//! `INTELLIHELPER_PRIVACY_NOTICE_ROLLOUT=1` — asserted on the authenticated welcome
//! screen before opening settings. Row/input details are unit-tested in
//! `intellihelper-pager` (`views/settings_modal/tests.rs`, `locked_coding_*`);
//! this suite covers the auth-to-render pipeline.
//!
//! ```bash
//! cargo test -p intellihelper-pager-pty-harness --test settings_locked_row_e2e \
//!   -- --ignored --nocapture
//! ```
...
use intellihelper_pager_pty_harness::{
...
const ROWS: u16 = 50;
const COLS: u16 = 120;
const BANNER_TITLE: &str = "Help improve IntelliHelper";
...
fn locked_row_env_ops() -> [EnvOp<'static>; 3] {
    [
        EnvOp::set("INTELLIHELPER_PRIVACY_NOTICE_ROLLOUT", "1"),
        EnvOp::set("INTELLIHELPER_ZDR_ACCESS_ENABLED", "1"),
