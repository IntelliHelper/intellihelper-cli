...
///
/// [`SubagentRuntimeOverrides::harness_agent_type`]: intellihelper_tools::implementations::intellihelper_build::task::types::SubagentRuntimeOverrides::harness_agent_type
...
pub(crate) struct RoleSpawnOverride {
    ...
    /// Resolved harness `agent_type` (e.g. `"intellihelper-build-plan"`)
...
mod tests {
    ...
    fn planner_template_default_render_preserves_wording_and_has_no_placeholders() {
        ...
        // (intellihelper-build) tool names; guards against accidental wording drift.
    ...
    async fn fail_open_retry_emits_spawn_failed_event() {
        ...
        let writer = file_utils::events::EventWriter::open(dir.path());
        let ov = RoleSpawnOverride {
            model: Some("m".into()),
            agent_type: Some("t".into()),
        };
