//! Integration tests for intellihelper-hooks.
//!
...
use intellihelper_hooks::discovery::load_hooks;
use intellihelper_hooks::dispatcher;
use intellihelper_hooks::event::*;
use intellihelper_hooks::result::HookDecision;
use intellihelper_hooks::runner::RunContext;
...
async fn non_blocking_dispatch() {
    ...
    assert!(matches!(
        &results[0],
        intellihelper_hooks::result::HookRunResult::Success { .. }
    ));
}
    ...
    let cases = vec![
        ...
        Case {
            ...
            payload: HookPayload::StopFailure {
                error: intellihelper_hooks::event::StopFailureKind::RateLimit,
...
/// runner-reserved keys (`INTELLIHELPER_HOOK_EVENT`, `INTELLIHELPER_HOOK_NAME`,
/// `INTELLIHELPER_SESSION_ID`, `INTELLIHELPER_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$INTELLIHELPER_HOOK_EVENT" > {f}; echo "NAME=$INTELLIHELPER_HOOK_NAME" >> {f}; echo "SESSION=$INTELLIHELPER_SESSION_ID" >> {f}; echo "ROOT=$INTELLIHELPER_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "INTELLIHELPER_HOOK_EVENT": "spoofed_event",
                                "INTELLIHELPER_HOOK_NAME": "spoofed_name",
                                "INTELLIHELPER_SESSION_ID": "spoofed_session",
                                "INTELLIHELPER_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "INTELLIHELPER_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed INTELLIHELPER_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "INTELLIHELPER_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed INTELLIHELPER_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "INTELLIHELPER_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed INTELLIHELPER_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
...
async fn http_hook_url_env_expansion_end_to_end() {
    ...
    use intellihelper_hooks::result::HookRunResult;
    let info = match run {
