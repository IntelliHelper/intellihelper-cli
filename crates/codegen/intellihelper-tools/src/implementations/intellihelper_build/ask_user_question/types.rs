...
//!
//! - **`intellihelper-tools`** — tool blocks on a oneshot, formats the result.
//! - **`intellihelper-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`intellihelper-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `intellihelper-tools`.
...
register_resource!("intellihelper_build", "UserQuestionSender", UserQuestionSender);
