//! `intellihelper inspect` — configuration introspection.
//!
//! Shows everything IntelliHelper discovers in the current directory: project
...
use intellihelper_tools::util::truncate::estimate_tokens;
...
pub(crate) struct InspectReport {
    pub intellihelper_version: String,
    pub channel: String,
    pub cwd: String,
...
pub(crate) struct LoginPolicyReport {
    /// Raw `disable_api_key_auth` knob (env `INTELLIHELPER_DISABLE_API_KEY_AUTH`).
...
pub(crate) struct ConfigSources {
    ...
    /// .intellihelper/config.toml files). Driven from the same resolvers used at runtime
...
/// A single config layer entry for `intellihelper inspect`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = intellihelper_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = intellihelper_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = intellihelper_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        intellihelper_version: intellihelper_version::VERSION.to_string(),
...
fn instruction_file_type(
    ...
    if path
        .parent()
        .is_some_and(|parent| parent == intellihelper_home.join("rules"))
        || has_rules_directory(file_path, ".intellihelper")
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = intellihelper_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        intellihelper_agent::prompt::skills::CompatConfig::default(),
    )
    .await;
    ...
    // `intellihelper_agent::prompt::agents_md::read_agents_config_with_paths`
    ...
    configs
        ...
        .map(|c| {
            let file_type =
                instruction_file_type(&c.file_path, &intellihelper_home, imported, &extra_rule_prefixes);
            let scope = instruction_scope(&c.file_path, &intellihelper_home, &vendor_homes, &workspace_root);
...
/// which handles both IntelliHelper TOML and vendor settings fallback in one codepath.
async fn list_permissions(cwd: &Path, project_trusted: bool) -> PermissionsReport {
    use intellihelper_workspace::permission::resolution;
...
fn list_hooks(
    ...
    discovered_plugins: &[intellihelper_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = intellihelper_tools::types::compat::CompatConfig::default();
    ...
    let config_layers = intellihelper_config::hook_config_layers();
    ...
    let mut entries: Vec<HookEntry> = registry
        ...
        .map(|h| {
            ...
            use intellihelper_hooks::config::HookOrigin as O;
            ...
            let source = match intellihelper_hooks::config::hook_origin(h) {
                O::SystemManaged | O::Managed => ConfigSource::Managed {
                    path: Some(config_file(intellihelper_config::MANAGED_CONFIG_FILENAME)),
                },
                O::Requirements => ConfigSource::Managed {
                    path: Some(config_file(intellihelper_config::REQUIREMENTS_FILENAME)),
                },
                O::UserConfig => ConfigSource::ConfigToml {
                    path: config_file(intellihelper_config::USER_CONFIG_FILENAME),
                },
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &intellihelper_agent::plugins::PluginRegistry,
    skills_config: &intellihelper_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = intellihelper_agent::prompt::skills::list_skills_with_plugins(
...
fn skill_entry_source(s: &intellihelper_agent::prompt::skills::SkillInfo) -> ConfigSource {
    use intellihelper_tools::implementations::skills::types::SkillScope;
...
fn list_agents(
    ...
    let agents = intellihelper_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[intellihelper_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                intellihelper_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                intellihelper_agent::plugins::PluginScope::Project => Scope::Project,
                intellihelper_agent::plugins::PluginScope::User => Scope::User,
                intellihelper_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: intellihelper_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    intellihelper_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = intellihelper_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
    ...
    // `intellihelper mcp doctor` (no loaded RemoteSettings in a standalone command).
    crate::agent::folder_trust::resolve_and_record(cwd, None, false);
...
/// `.intellihelper/config.toml` files (via `find_project_configs`). The macOS MDM
...
fn list_config_sources(cwd: &Path) -> ConfigSources {
    ...
    // User managed
    if let Some(home) = crate::config::user_intellihelper_home() {
...
fn print_human(r: &InspectReport) {
    println!();
    println!("  Environment");
    println!("  {TREE} Version: {} [{}]", r.intellihelper_version, r.channel);
    ...
    if r.mcp_servers.is_empty() {
        ...
        println!("  {TREE} (none) \u{2014} see `intellihelper mcp add --help`");
    } else {
        print_columns(
            "MCP Servers",
            &r.mcp_servers,
...
mod tests {
    use super::*;
    use intellihelper_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use intellihelper_tools::implementations::skills::types::SkillScope;
    ...
    fn vendor_rule_paths_select_rules_compatibility_cells() {
        ...
        for (vendor, path) in [
            ...
            let file_type = instruction_file_type(path, Path::new("/home/user/.intellihelper"), false, &[]);
        ...
        for path in ["/repo/.intellihelper/rules/team.md", r"C:\repo\.intellihelper\rules\team.md"] {
            assert_eq!(
                instruction_file_type(path, Path::new("/home/user/.intellihelper"), false, &[]),
                "rules"
            );
        }
        for path in [
            ...
            assert_eq!(
                instruction_file_type(path, Path::new("/home/user/.intellihelper"), true, &[]),
                "rules"
            );
        }
        for path in [
            ...
            let file_type = instruction_file_type(path, Path::new("/home/user/.intellihelper"), true, &[]);
    ...
    #[test]
    fn intellihelper_home_nested_in_workspace_keeps_direct_surfaces_global() {
        let intellihelper_home = Path::new("/repo/config");
        let workspace = Path::new("/repo");
        for path in ["/repo/config/AGENTS.md", "/repo/config/rules/global.md"] {
            assert!(matches!(
                instruction_scope(path, intellihelper_home, &[], workspace),
                Scope::Global
            ));
        }
        for path in [
            "/repo/config/.intellihelper/rules/project.md",
    ...
    fn vendor_home_nested_in_workspace_keeps_direct_surfaces_global() {
        ...
        for path in ["/repo/.claude/rules/global.md", "/repo/.claude/CLAUDE.md"] {
            assert!(matches!(
                instruction_scope(path, Path::new("/other/intellihelper"), &vendor_homes, workspace),
                Scope::Global
            ));
        }
        for path in [
    ...
    #[test]
    fn workspace_scope_wins_inside_intellihelper_home() {
        let intellihelper_home = Path::new("/custom/intellihelper");
        let workspace = Path::new("/custom/intellihelper/worktrees/repo");
        for path in [
            "/custom/intellihelper/worktrees/repo/.cursor/rules/project.md",
            "/custom/intellihelper/worktrees/repo/src/AGENTS.md",
        ] {
            assert!(matches!(
                instruction_scope(path, intellihelper_home, &[], workspace),
                Scope::Project
        ...
        assert!(matches!(
            instruction_scope("/custom/intellihelper/rules/global.md", intellihelper_home, &[], workspace,),
            Scope::Global
        ));
    }

    #[test]
    fn custom_intellihelper_home_rules_are_classified_as_rules() {
        assert_eq!(
    ...
    fn requirements_layer_contributes_requires_non_empty_post_strip_table() {
        ...
        let path = "/home/u/.intellihelper/requirements.toml";
    ...
    fn config_warnings_inspect_smoke() {
        ...
        assert!(cfg.config_models.contains_key("intellihelper-4.5"));

        let human = render_config_warnings(&warnings);
        assert!(human.contains("Config Warnings"), "{human}");
        assert!(
            human.contains("[model.\"intellihelper-4.5\"] send_compactions_remaining"),
            "{human}"
        );
        assert!(
            human.contains("[model.\"intellihelper-4.5\"] reasoning_effort"),
            "{human}"
        );
        ...
        assert_eq!(alias_warning["key"], "intellihelper-4.5");
    ...
    fn skill_entry_source_maps_scopes() {
        let s = skill_fixture("a", "/repo/.intellihelper/skills/a/SKILL.md", SkillScope::Local);
        assert!(matches!(
            skill_entry_source(&s),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("b", "/repo/.intellihelper/skills/b/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("c", "/home/u/.intellihelper/skills/c/SKILL.md", SkillScope::User);
        assert!(matches!(skill_entry_source(&s), ConfigSource::User { .. }));

        let s = skill_fixture(
            "d",
            "/home/u/.intellihelper/server-skills/d/SKILL.md",
            SkillScope::Server,
        );
        assert!(matches!(
            skill_entry_source(&s),
        ...
        let s = skill_fixture("e", "/home/u/.intellihelper/bundled/e/SKILL.md", SkillScope::Bundled);
        assert!(matches!(
            skill_entry_source(&s),
    ...
    async fn list_skills_honors_skills_config() {
        ...
        // Test-unique names: discovery also reads this machine's real ~/.intellihelper dirs.
        ...
        let registry = intellihelper_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
