...
use intellihelper_tools::implementations::intellihelper_build::LoopFireMode;
...
impl UsageDrainOutcome {
    ...
    pub(super) fn from_outstanding_reply(
        reply: Option<
            &intellihelper_tools::implementations::intellihelper_build::task::types::SubagentOutstandingReply,
        >,
    ) -> Self {
        match reply {
            None => Self {
...
impl SessionActor {
    ...
    pub(super) async fn handle_prompt(
        ...
        *self.turn_prompt_mode.lock() = prompt_mode;
        ...
        let prompt_blocks = match slash_commands::resolve(
            ...
            Err(SlashCommandOutcome::Builtin(action)) => {
                ...
                let slash_used = intellihelper_telemetry::events::SlashCommandUsed {
                ...
                match action {
                    BuiltinAction::GoalSet {
                        ...
                        intellihelper_telemetry::session_ctx::log_event(slash_used);
            ...
            Err(SlashCommandOutcome::InvokeSkill {
                ...
                for sk in &parsed_skills {
                    intellihelper_telemetry::session_ctx::log_event(
                        intellihelper_telemetry::events::SlashCommandUsed {
                    ...
                    intellihelper_telemetry::session_ctx::log_event(
                        intellihelper_telemetry::events::SkillDispatched {
                            ...
                            trigger: intellihelper_telemetry::events::SkillTrigger::SlashCommand,
                        },
                    );
                    ...
                    if let Some(ref pname) = sk.plugin_name {
                        intellihelper_telemetry::session_ctx::log_event(
                            intellihelper_telemetry::events::PluginUsed {
        ...
        intellihelper_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::Turn {
            session_id: self.session_info.id.0.to_string(),
            turn_number: turn_idx,
        });
        let current_prompt_index = self.chat_state_handle.get_prompt_index().await;
        intellihelper_telemetry::session_ctx::begin_prompt_id();
        let origin = super::super::PromptOrigin::from_prompt_id(prompt_id);
        let mut chunk_meta = serde_json::Map::new();
        ...
        *self.tool_context.prompt_index.lock().await = current_prompt_index;
        ...
        let (query, extra_images) = if !self.is_cursor_harness() {
            let extraction = intellihelper_tools::util::base64_images::extract_base64_images(query);
        ...
        let model_id = self
            ...
            .map(|c| c.model)
            .unwrap_or_default();
        if self.telemetry_enabled || intellihelper_telemetry::external::is_active() {
            let effective_client_identifier =
                prompt_client_identifier.or_else(|| self.client_identifier.clone());
            let ev = intellihelper_telemetry::events::PromptSubmitted {
            ...
            intellihelper_telemetry::session_ctx::log_event_dual(self.telemetry_enabled, ev);
        }
        ...
        self.tool_bridge_handle()
            .update_resource(intellihelper_tools::types::resources::AttachedImages(
                attached_image_refs,
            ))
            .await;
        ...
        self.dispatch_hook(
            intellihelper_hooks::event::HookEventName::UserPromptSubmit,
            intellihelper_hooks::event::HookPayload::UserPromptSubmit {
        ...
        match &result {
            Ok(TurnOutcome::Completed { refusal, .. }) => {
                ...
                if let Some(explanation) = refusal {
                    ...
                    self.dispatch_hook(
                        intellihelper_hooks::event::HookEventName::StopFailure,
                        intellihelper_hooks::event::HookPayload::StopFailure {
                            error: intellihelper_hooks::event::StopFailureKind::InvalidRequest,
                ...
                .await;
                intellihelper_telemetry::session_ctx::log_event(
                    intellihelper_telemetry::events::TurnCompleted {
                        outcome: intellihelper_telemetry::events::Outcome::Completed,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
            ...
            Ok(TurnOutcome::Cancelled { category, context }) => {
                ...
                intellihelper_telemetry::session_ctx::log_event(
                    intellihelper_telemetry::events::TurnCompleted {
                        outcome: intellihelper_telemetry::events::Outcome::Cancelled,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
                        model_id: turn_model_id,
            ...
            Err(err) => {
                ...
                intellihelper_telemetry::session_ctx::log_session_event(
                    intellihelper_telemetry::events::ApiError {
                ...
                intellihelper_telemetry::session_ctx::log_event(
                    intellihelper_telemetry::events::TurnCompleted {
                        outcome: intellihelper_telemetry::events::Outcome::Error,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
                        model_id: turn_model_id,
                        cancellation_category: None,
    ...
    pub(crate) async fn inject_pending_monitor_events(&self) {
        ...
        let mine = intellihelper_tools::implementations::intellihelper_build::monitor::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        );
        if mine.is_empty() {
        ...
        let Some(body) = intellihelper_tools::reminders::task_completion::format_monitor_events(
            &mine,
            Some(&self.tool_context.task_output_tool_name),
        ) else {
            return;
        };
        let wrapped = intellihelper_tools::reminders::wrap_reminder(&body);
        self.chat_state_handle
    ...
    pub(crate) async fn first_turn_memory_reminder(&self) -> Option<String> {
        ...
        use intellihelper_tools::types::memory_backend::MemoryBackend as _;
        ...
        intellihelper_telemetry::session_ctx::log_event(
            intellihelper_telemetry::memory_telemetry::MemoryInjection {
    ...
    async fn handle_structured_output_tool_call(
        &self,
        tool_calls: &mut Vec<intellihelper_sampling_types::conversation::ToolCall>,
        validator: &Result<jsonschema::Validator, String>,
        retries: &mut u32,
    ...
    async fn is_run_true_step(
        &self,
        tool_calls: &[intellihelper_sampling_types::conversation::ToolCall],
    ) -> bool {
    ...
    async fn finalize_turn_bookkeeping(
        ...
        if let Some(snap) = snapshot.as_mut() {
            ...
            for pr in &snap.delta.prs_created_this_turn {
                intellihelper_telemetry::session_ctx::log_event(intellihelper_telemetry::events::PrCreated {
                    source: pr.source,
    ...
    async fn process_conversation_turn(
        ...
        loop {
            ...
            if identical_tool_calls.run_len >= identical_tool_calls.hard_stop_threshold() {
                ...
                intellihelper_telemetry::session_ctx::log_event(
                    intellihelper_telemetry::events::ActionStationarityStop {
                        true_noop,
                        run_len,
            ...
            request.x_intellihelper_session_id = Some(self.session_info.id.to_string());
            request.x_intellihelper_turn_idx =
                Some(self.chat_state_handle.get_prompt_index().await.to_string());
            request.x_intellihelper_agent_id = Some(intellihelper_telemetry::id::agent_id());
            if request.x_intellihelper_deployment_id.is_none() {
                request.x_intellihelper_deployment_id = crate::managed_config::resolve_deployment_id(
            ...
            let model_duration_ms = model_timer.elapsed().as_millis() as u64;
            {
                let model_id = self.current_model_id().await;
                intellihelper_telemetry::session_ctx::log_event(
                    intellihelper_telemetry::events::ModelResponseReceived {
                        model_id,
            ...
            let turn_refused =
                stop_reason == Some(intellihelper_sampling_types::StopReason::ContentFilter);
            ...
            for item in response.items {
                match item {
                    intellihelper_sampling_types::ConversationItem::Assistant(_) => {
            ...
            if is_true_noop {
                intellihelper_telemetry::session_ctx::log_event(
                    intellihelper_telemetry::events::ShellTrueNoop {
