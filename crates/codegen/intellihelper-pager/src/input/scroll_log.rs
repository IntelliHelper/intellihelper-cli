//! Scroll flight recorder — `INTELLIHELPER_SCROLL_LOG` JSONL log of scroll-stream
...
//! (see [`super::mouse`]).
//!
//! Enablement: `INTELLIHELPER_SCROLL_LOG=1` (or set-but-empty) logs to
//! `~/.intellihelper/logs/scroll-log-<timestamp>.jsonl`; any other non-`0` value is
//! used as the target path. Unset (or `0`, matching `INTELLIHELPER_SCROLL_DEBUG`)
...
/// Appends [`ScrollLogRecord`]s to the `INTELLIHELPER_SCROLL_LOG` file. Owned as
...
impl ScrollLogRecorder {
    /// Build from `INTELLIHELPER_SCROLL_LOG` (see module docs for value semantics);
    /// `None` when unset or `0`.
    pub(crate) fn from_env_at(base: Instant) -> Option<Self> {
        let raw = std::env::var("INTELLIHELPER_SCROLL_LOG").ok()?;
        let value = raw.trim();
        if value == "0" {
            return None;
        }
...
/// `~/.intellihelper/logs/scroll-log-<utc-ts>.jsonl` — the input-debug dump's dir
