...
//!
//! Extracted from `intellihelper-shell::agent::telemetry`.
...
pub enum EmitterOrigin {
    /// `intellihelper-shell` (and the pager/TUI that emit through it).
    Shell,
    /// `intellihelper-workspace` (remote sampler / workspace server).
    Workspace,
}
...
impl EmitterOrigin {
    ...
    pub fn event_prefix(self) -> &'static str {
        match self {
            EmitterOrigin::Shell => "intellihelper-shell-",
            EmitterOrigin::Workspace => "intellihelper-workspace-",
        }
    }
}
...
/// event-name prefix (`intellihelper-shell-*` vs `intellihelper-workspace-*`).
///
...
mod tests {
    ...
    /// What a command exiting right after emitting (`intellihelper login`) relies on.
    ...
    fn event_prefix_is_stable_per_origin() {
        assert_eq!(EmitterOrigin::Shell.event_prefix(), "intellihelper-shell-");
        assert_eq!(EmitterOrigin::Workspace.event_prefix(), "intellihelper-workspace-");
    }

    /// The `Shell` reroute must reproduce the historical
    /// `format!("intellihelper-shell-{suffix}")` event name byte-for-byte, since every
    ...
    fn shell_origin_event_name_matches_legacy_format() {
        ...
        let legacy = format!("intellihelper-shell-{suffix}");
    ...
    fn workspace_origin_event_name_uses_workspace_prefix() {
        ...
        assert_eq!(name, "intellihelper-workspace-turn");
    }
