...
//! ```bash
//! export XAI_API_KEY=...
//! cargo run -p intellihelper-voice --bin voice-probe -- --seconds 5
//! ```

use std::path::PathBuf;

use intellihelper_voice::{
...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(code) = intellihelper_voice::maybe_run_capture_subprocess() {
...
async fn run() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,intellihelper_voice=debug".into()),
        )
        .init();
    ...
    if args.mic_only {
        ...
        {
            let (bytes, chunks) =
                intellihelper_voice::run_mic_only_probe(config.sample_rate, args.seconds)?;
...
fn load_config(path: Option<&std::path::Path>) -> VoiceConfig {
    ...
    let env_base = std::env::var("INTELLIHELPER_XAI_API_BASE_URL").ok();
    ...
    if let Ok(home) = std::env::var("INTELLIHELPER_HOME")
    ...
    if let Ok(raw) = std::fs::read_to_string(
        std::env::var("HOME")
            .map(PathBuf::from)
            .unwrap_or_default()
            .join(".intellihelper/config.toml"),
    ) && let Ok(table) = toml::from_str::<toml::Table>(&raw)
    {
        return VoiceConfig::from_config_table(&table, env_base.as_deref());
    }
...
Environment:
  ...
  RUST_LOG        optional (default info,intellihelper_voice=debug)

Reads [voice] from ~/.intellihelper/config.toml unless --config PATH is set.
"#
    );
}
