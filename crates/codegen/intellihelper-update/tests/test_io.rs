...
//!
//! These tests touch global process state — `INTELLIHELPER_HOME` (a `OnceLock` in
//! `intellihelper-config`), `INTELLIHELPER_TEST_VERSION`, and `NPM_TOKEN` — so they
//! must run serially. Once `INTELLIHELPER_HOME` is initialized for a process, it can't
//! be changed; we set it from a single shared `OnceLock` and reset the
//! contents of the directory between tests.
//!
//! The patterns here mirror the INTELLIHELPER_HOME isolation used in other
//! integration tests.
...
use intellihelper_update::write_version_cache;
...
#[tokio::test]
#[serial]
async fn write_version_cache_creates_file_at_intellihelper_home() {
    let _ = test_home();
    reset();
...
/// Write a `IntelliHelperVersion`-shaped JSON file with an arbitrary timestamp.
...
/// `IntelliHelperVersion` JSON layout.
async fn cache_is_fresh() -> bool {
    // Mirror the implementation: look at version.json under INTELLIHELPER_HOME,
...
// ─────────────────────────────────────────────────────────────────────────────
// version.json wire format — the on-disk file is read by every intellihelper launch.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// get_installed_intellihelper_version env override
//
// The function honors `INTELLIHELPER_TEST_VERSION` for testing. We exercise it
// via the public re-export only — no private items leaked.
// ─────────────────────────────────────────────────────────────────────────────
//
// Note: `get_installed_intellihelper_version` is not re-exported from `lib.rs`, but
...
async fn get_installed_version_uses_env_var_override() {
    ...
    unsafe {
        std::env::set_var("INTELLIHELPER_TEST_VERSION", "9.9.9");
    }
    let v = intellihelper_update::version::get_installed_intellihelper_version();
    assert_eq!(v, "9.9.9");
    unsafe {
        std::env::remove_var("INTELLIHELPER_TEST_VERSION");
    }
}
...
async fn get_installed_version_with_env_var_takes_precedence() {
    ...
    let real = {
        ...
        intellihelper_update::version::get_installed_intellihelper_version()
    };

    unsafe {
        std::env::set_var("INTELLIHELPER_TEST_VERSION", "0.0.0-test");
    }
    let overridden = intellihelper_update::version::get_installed_intellihelper_version();
...
async fn get_installed_version_handles_alpha_prerelease_in_env() {
    ...
    unsafe {
        std::env::set_var("INTELLIHELPER_TEST_VERSION", "0.1.200-alpha.5");
    }
    let v = intellihelper_update::version::get_installed_intellihelper_version();
...
async fn get_installed_version_does_not_validate_env_var_format() {
    ...
    unsafe {
        std::env::set_var("INTELLIHELPER_TEST_VERSION", "not-a-version");
    }
    let v = intellihelper_update::version::get_installed_intellihelper_version();
