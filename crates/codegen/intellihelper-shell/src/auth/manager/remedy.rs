...
mod tests {
    ...
    fn external_credential(expires_at: chrono::DateTime<Utc>) -> IntelliHelperAuth {
        IntelliHelperAuth {
            key: "external".into(),
    ...
    fn provider_manager(dir: &std::path::Path, credential: IntelliHelperAuth) -> Arc<AuthManager> {
    ...
    async fn renewed_carries_the_wire_valid_bearer_the_buffer_hides() {
        ...
        manager.hot_swap(IntelliHelperAuth {
            key: "wire-valid".into(),
            auth_mode: AuthMode::Oidc,
            refresh_token: Some("rt-live".into()),
