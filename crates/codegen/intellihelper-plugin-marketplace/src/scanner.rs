...
//!    the lookup order — `.intellihelper-plugin/marketplace.json` is preferred), use it.
...
fn scan_single_plugin(plugin_dir: &Path, relative_path: &str) -> MarketplaceEntry {
    ...
    let manifest_result = intellihelper_agent::plugins::manifest::load_manifest(plugin_dir);
    let manifest = match &manifest_result {
        Ok(intellihelper_agent::plugins::manifest::ManifestLoadResult::Found(m)) => Some(m.as_ref()),
        _ => None,
    };
...
mod tests {
    ...
    fn url_sourced_entry_carries_keywords() {
        let dir = tempfile::tempdir().unwrap();
        let intellihelper_dir = dir.path().join(".intellihelper-plugin");
        std::fs::create_dir_all(&intellihelper_dir).unwrap();
        std::fs::write(
            intellihelper_dir.join("marketplace.json"),
            r#"{
                "name": "kw-marketplace",
                "plugins": [{
                    "name": "remote-plugin",
    ...
    #[test]
    fn intellihelper_plugin_dir_index_drives_scan_end_to_end() {
        let dir = tempfile::tempdir().unwrap();
        make_plugin(dir.path(), "intellihelper-plugin", "1.0.0");
        ...
        std::fs::write(
            intellihelper_dir.join("marketplace.json"),
            r#"{
                "name": "intellihelper-marketplace",
                "plugins": [{
                    "name": "intellihelper-plugin",
                    "description": "From the .intellihelper-plugin index",
                    "category": "design",
                    "source": { "type": "local", "path": "./plugins/intellihelper-plugin" },
                    "tags": ["intellihelper"]
                }]
            }"#,
        )
        ...
        let plugins = scan_marketplace(dir.path()).entries;
        assert_eq!(plugins.len(), 1);
        assert_eq!(plugins[0].name, "intellihelper-plugin");
        assert_eq!(plugins[0].category.as_deref(), Some("design"));
        assert_eq!(plugins[0].tags, vec!["intellihelper"]);
        assert!(plugins[0].keywords.is_empty());
    }
    ...
    fn write_intellihelper_file(dir: &Path, file: &str, content: &str) {
        let intellihelper_dir = dir.join(".intellihelper-plugin");
        std::fs::create_dir_all(&intellihelper_dir).unwrap();
        std::fs::write(intellihelper_dir.join(file), content).unwrap();
    }
    ...
    fn url_entry_gets_components_when_sha_matches() {
        ...
        write_intellihelper_file(dir.path(), "plugin-index.json", URL_CATALOG);
    ...
    fn url_entry_without_pinned_sha_gets_no_components() {
        let dir = tempfile::tempdir().unwrap();
        write_intellihelper_file(dir.path(), "marketplace.json", &url_marketplace_index(""));
        write_intellihelper_file(dir.path(), "plugin-index.json", URL_CATALOG);
    ...
    fn malformed_catalog_degrades_to_no_components() {
        ...
        write_intellihelper_file(dir.path(), "plugin-index.json", "not json");
