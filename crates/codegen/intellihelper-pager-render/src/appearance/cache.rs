...
//!
//! Disk writes live in `intellihelper_shell::util::config::set_<field>()`.
...
use intellihelper_shared::ui_config::UiConfig;
...
/// Default ON when unset. The `INTELLIHELPER_PROMPT_SUGGESTIONS` env var overrides
...
/// first call. `INTELLIHELPER_SCROLL_SPEED` overrides config.toml — matches the
/// legacy `appearance::persist::load_scroll_speed` behaviour.
pub fn load_scroll_speed() -> u8 {
    SCROLL_SPEED_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("INTELLIHELPER_SCROLL_SPEED")
                .ok()
...
/// `INTELLIHELPER_SCROLL_MODE` overrides `[ui].scroll_mode` (the `INTELLIHELPER_SCROLL_SPEED`
...
pub fn load_scroll_mode() -> ScrollMode {
    SCROLL_MODE_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("INTELLIHELPER_SCROLL_MODE")
                .ok()
...
/// `INTELLIHELPER_INVERT_SCROLL` (`1`/`true`/`0`/`false`) overrides `[ui].invert_scroll`.
pub fn load_invert_scroll() -> bool {
    INVERT_SCROLL_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("INTELLIHELPER_INVERT_SCROLL")
                .ok()
...
/// its own lines-per-tick. `INTELLIHELPER_SCROLL_LINES` overrides `[ui].scroll_lines`.
pub fn load_scroll_lines() -> Option<u8> {
    SCROLL_LINES_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("INTELLIHELPER_SCROLL_LINES")
                .ok()
...
fn load_bool_from_effective_config(key: &str, default: bool) -> bool {
    let root = match intellihelper_config::load_effective_config_disk_only() {
        Ok(r) => r,
