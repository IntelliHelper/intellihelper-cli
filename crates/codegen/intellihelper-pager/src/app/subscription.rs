...
/// `intellihelper_build_settings.subscription_watch_interval_secs` field.
...
/// Floor for the `INTELLIHELPER_SUBSCRIPTION_WATCH_INTERVAL_SECS` env override
...
impl AppView {
    ...
    pub fn subscription_watch_interval(&self) -> Option<std::time::Duration> {
        if let Ok(v) = std::env::var("INTELLIHELPER_SUBSCRIPTION_WATCH_INTERVAL_SECS")
    ...
    #[must_use]
    pub fn impose_gate(&mut self, gate: intellihelper_shell::auth::GateInfo) -> Vec<Effect> {
    ...
    #[must_use]
    fn defer_gate_for_verification(&mut self, gate: intellihelper_shell::auth::GateInfo) -> Vec<Effect> {
...
mod tests {
    ...
    fn watch_gate() -> intellihelper_shell::auth::GateInfo {
    ...
    fn impose_gate_direct_for_non_consumer_and_already_gated() {
        ...
        let new_copy = intellihelper_shell::auth::GateInfo {
