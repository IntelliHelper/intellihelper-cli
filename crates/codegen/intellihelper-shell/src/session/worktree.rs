...
//!
//! Core worktree lifecycle logic lives in [`intellihelper_workspace::worktree`].
...
use std::path::Path;
use intellihelper_workspace::session::git::find_git_root_from_path;
pub use intellihelper_workspace::worktree::*;
...
/// manual `create_from_worktree_sync` path used by `intellihelper -w --ref`).
async fn create_worktree_for_resume(
    ...
    if find_git_root_from_path(source)
        .ok()
        .is_some_and(|root| intellihelper_workspace::session::git::detect_vcs_kind(&root).is_jj())
    {
...
pub(crate) async fn checkout_persisted_head_in_worktree(
    ...
    session_id: &str,
) -> intellihelper_workspace::session::git::CheckoutSessionOutcome {
    let sha = match head_commit {
        Some(s) if !s.is_empty() => s,
        _ => return intellihelper_workspace::session::git::CheckoutSessionOutcome::default(),
    };
    intellihelper_workspace::session::git::checkout_session_commit(
...
pub(crate) struct WorktreeRestoreDecision {
    ...
    pub restore_degree: Option<intellihelper_workspace::session::git::RestoreDegree>,
}
/// Thin wire-format adapter over the shared
/// [`intellihelper_workspace::session::git::build_restore_decision`] helper.
pub(crate) fn build_worktree_restore_outcome(
    head_commit: Option<&str>,
    outcome: &intellihelper_workspace::session::git::CheckoutSessionOutcome,
    kind: intellihelper_workspace::session::git::RestoreKind,
) -> WorktreeRestoreDecision {
    let d = intellihelper_workspace::session::git::build_restore_decision(head_commit, outcome, kind);
    WorktreeRestoreDecision {
...
pub(crate) async fn resume_session_in_worktree(
    ...
    ops: &intellihelper_workspace::WorkspaceOps,
    ...
    use intellihelper_workspace::session::git::effective_worktree_path;
    tracing::info!(
        target: WORKTREE_LOG,
    ...
    let restore_degree = if codebase_ok {
        Some(intellihelper_workspace::session::git::RestoreDegree::Full)
    } else {
        None
    };
...
async fn resume_local_session_in_worktree(
    req: &ResumeSessionInWorktreeRequest,
    #[allow(unused_variables)] ops: &intellihelper_workspace::WorkspaceOps,
    ...
    if req.restore_code.unwrap_or(restore_code_default) {
        let is_jj = find_git_root_from_path(std::path::Path::new(resolved_source_cwd))
            ...
            .is_some_and(|root| intellihelper_workspace::session::git::detect_vcs_kind(&root).is_jj());
        if !is_jj {
            if intellihelper_workspace::session::git::should_warn_registry_disabled(
                is_jj,
                registry_client.is_some(),
            ) {
                intellihelper_workspace::session::git::warn_registry_disabled_restore(
                    resolved_session_id,
                );
            }
...
pub(crate) async fn rehydrate_session_in_worktree(
    ...
    let session_summary_exists = crate::util::intellihelper_home::sessions_cwd_dir(&req.source_cwd)
        .join(&req.session_id)
        .join("summary.json")
        .exists();
...
mod tests {
    ...
    fn resume_response_round_trips() {
        let resp = ResumeSessionInWorktreeResponse {
            ...
            restore_degree: Some(intellihelper_workspace::session::git::RestoreDegree::Full),
        };
    ...
    fn resume_response_serializes_degree_when_set() {
        let resp = ResumeSessionInWorktreeResponse {
            ...
            restore_degree: Some(intellihelper_workspace::session::git::RestoreDegree::HeadOnly),
        };
        let json = serde_json::to_string(&resp).unwrap();
        ...
        assert_eq!(
            deser.restore_degree,
            Some(intellihelper_workspace::session::git::RestoreDegree::HeadOnly)
        );
    ...
    fn repo_wide_resolve_finds_session_in_sibling_cwd_skipping_remote() {
        ...
        let encoded = crate::util::intellihelper_home::encode_cwd_dirname(sibling_cwd);
    ...
    async fn checkout_persisted_head_stashes_dirty_worktree_state() {
        ...
        assert!(
            list_out.contains("intellihelper: pre-restore-code sess-dirty-wt"),
