...
use std::sync::Arc;
/// Credentials for authenticating with intellihelper backend services.
///
...
/// User token (xAI users) sends `Bearer` + `X-XAI-Token-Auth: intellihelper-cli`.
/// Deployment key takes precedence when both are present.
#[derive(Clone)]
pub struct IntelliHelperAuthCredentials {
...
impl std::fmt::Debug for IntelliHelperAuthCredentials {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("IntelliHelperAuthCredentials")
            .field(
                "user_token",
...
impl IntelliHelperAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your INTELLIHELPER_DEPLOYMENT_KEY is invalid or expired. Please contact a team admin."
        } else if self.user_token.is_some() {
            "Your auth token is invalid or expired. Run `intellihelper login` to re-authenticate."
        } else {
            "Not authenticated."
        }
    }
    ...
    pub fn resolve(&self) -> IntelliHelperAuthCredentials {
    ...
    /// the bearer.
    pub async fn resolve_async(&self) -> IntelliHelperAuthCredentials {
        ...
        let builder = if let Some(ref key) = self.deployment_key {
            ...
            builder
                ...
                .header(
                    ...
                    obfstr::obfstr!("intellihelper-cli"),
                )
        } else {
            builder
        };
...
impl intellihelper_auth::HttpAuth for IntelliHelperAuthCredentials {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        IntelliHelperAuthCredentials::apply(self, builder, base_url)
    }
}
#[cfg(test)]
mod tests {
    ...
    fn resolve_returns_token_when_not_expired() {
        ...
        let creds = IntelliHelperAuthCredentials::new(None).with_auth_manager(mgr);
    ...
    fn resolve_returns_token_during_early_invalidation_window() {
        ...
        let creds = IntelliHelperAuthCredentials::new(None).with_auth_manager(mgr.clone());
    ...
    fn resolve_returns_static_token_when_no_auth_manager() {
        let creds = IntelliHelperAuthCredentials::new(Some("static-token".into()));
