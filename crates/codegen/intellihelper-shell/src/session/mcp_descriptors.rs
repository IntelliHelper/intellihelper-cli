...
/// sanitizer shared with `intellihelper-mcp` so the advertised folder matches disk.
...
fn is_gateway_owned_descriptor(path: &Path) -> bool {
    ...
    value
        .get("x-intellihelper-managed-gateway")
...
mod tests {
    ...
    fn server_dir_is_joined_under_root() {
        let root = Path::new("/home/u/.intellihelper/projects/enc/mcps");
    ...
    async fn gateway_descriptor_collision_prunes_only_gateway_owned_files() {
        ...
        std::fs::write(
            ...
            serde_json::to_vec_pretty(&serde_json::json!({
                ...
                "inputSchema": {"type": "object"},
                "x-intellihelper-managed-gateway": true,
            }))
            .unwrap(),
        )
        .unwrap();
