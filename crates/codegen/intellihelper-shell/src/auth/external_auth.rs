...
/// Parse stdout into a session-credential `IntelliHelperAuth`.
pub(crate) fn parse_output(output: &std::process::Output) -> anyhow::Result<IntelliHelperAuth> {
    let parsed = parse_token_output(output)?;
    Ok(IntelliHelperAuth {
...
pub(crate) async fn run_external_refresh(command: &str) -> Option<IntelliHelperAuth> {
    ...
    cmd.env("INTELLIHELPER_AUTH_EXPIRED", "1");
...
pub(crate) async fn refresh_with_command(command: &str, prev_auth: &IntelliHelperAuth) -> Option<IntelliHelperAuth> {
...
mod tests {
    ...
    #[tokio::test]
    async fn sets_intellihelper_auth_expired_env_on_refresh() {
        let auth = run_external_refresh("echo $INTELLIHELPER_AUTH_EXPIRED")
            .await
            .unwrap();
