...
//!
//! Extracted from `intellihelper-shell::agent::telemetry::track`. The HTTP client is
...
/// whichever [`EmitterOrigin`] prefix it carries (`intellihelper-shell-` /
/// `intellihelper-workspace-`). Unprefixed names pass through unchanged. Kept in
...
pub async fn track(event_name: &str, request_id: &str, ctx: &UserContext, mut metadata: Metadata) {
    ...
    if let Some(ref mixpanel) = client.mixpanel {
        ...
        props.insert("app_name".into(), json!("IntelliHelper Code"));
...
mod tests {
    ...
    /// identical to the previous `strip_prefix("intellihelper-shell-")` behavior.
    #[test]
    fn event_value_strips_shell_prefix() {
        assert_eq!(event_value("intellihelper-shell-turn"), "turn");
        assert_eq!(
            event_value("intellihelper-shell-trace_upload_attempted"),
    ...
    fn event_value_strips_workspace_prefix() {
        assert_eq!(event_value("intellihelper-workspace-turn"), "turn");
    }
    ...
    fn event_value_strips_only_leading_prefix() {
        assert_eq!(event_value("intellihelper-shell-workspace-x"), "workspace-x");
    }
    ...
    /// (e.g. a future `"intellihelper-shell-ext-"` origin), an earlier `ALL` entry could
