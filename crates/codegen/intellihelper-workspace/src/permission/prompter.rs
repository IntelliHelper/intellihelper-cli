...
use intellihelper_mcp::servers::parse_mcp_qualified_name;
use intellihelper_tools::implementations::intellihelper_build::web_fetch::domain_from_url;

const REJECT_ONCE_LABEL: &str = "No, and tell IntelliHelper what to do differently";
...
///        `~/.intellihelper/config.toml` via the `Effect::PersistPermissionMode` effect
...
/// (global yolo) option that is prepended for IntelliHelperTUI / IntelliHelperPager / Desktop.
///
...
/// (`IntelliHelperTUI`, `IntelliHelperPager`, `Desktop`) that wire the option id through
...
fn client_supports_enable_always_approve(client_type: ClientType) -> bool {
    matches!(
        client_type,
        ClientType::IntelliHelperTUI | ClientType::IntelliHelperPager | ClientType::Desktop
    )
}
...
/// Canonical definition lives in `intellihelper_workspace_types` (so both the
/// permission-validation layer and the MCP transport in `intellihelper-mcp` can
...
/// reached `intellihelper_workspace::permission::MCP_TOOL_NAME_DELIMITER`.
...
/// `"IntelliHelper Com Notion"`, and `"getMyTaskList"` → `"GetMyTaskList"`.
...
impl AcpPrompter {
    pub fn new(
        ...
        // Bash options for IntelliHelperTUI - interactive selection with expandable/contractable terms
    ...
    fn build_options_inner(
        ...
        match access {
            ...
            AccessKind::Bash(bash_command) => {
                // For IntelliHelperTUI clients, use the fancy interactive options with term selection
...
mod tests {
    ...
    fn gate_off_strips_bash_always_allow_keeps_yes_no() {
        let p = prompter_with_gate(ClientType::IntelliHelperPager, false);
    ...
    fn gate_on_includes_bash_always_allow() {
        let p = prompter_with_gate(ClientType::IntelliHelperPager, true);
        let access = AccessKind::Bash("kubectl get pods".to_owned());
        let opts = p.build_options(&access);
        assert!(
            has_option(&opts, "allow-always-command"),
    ...
    fn gate_off_strips_generic_bash_always_and_reject_always() {
        let p = prompter_with_gate(ClientType::IntelliHelperWeb, false);
        let access = AccessKind::Bash("kubectl get pods".to_owned());
        let opts = p.build_options(&access);
    ...
    fn bash_meta_present_only_when_gate_on_for_fancy_clients() {
        ...
        let on = prompter_with_gate(ClientType::IntelliHelperPager, true);
        ...
        let off = prompter_with_gate(ClientType::IntelliHelperPager, false);
        assert!(off.bash_selection_meta(&access).is_none());
        // Generic client never gets the fancy-UI meta, even with the gate on.
        let generic = prompter_with_gate(ClientType::IntelliHelperWeb, true);
    ...
    fn bash_reject_always_command_maps_selected_words() {
        let p = prompter(ClientType::IntelliHelperPager);
    ...
    fn generic_client_multi_command_keeps_broad_options() {
        let p = prompter(ClientType::IntelliHelperWeb);
    ...
    fn mcp_prompt_includes_allow_always_with_meta() {
        let p = prompter(ClientType::IntelliHelperTUI);
        for (name, server) in [
            ("linear__list", "linear"),
            ("123__lookup", "123"),
    ...
    fn mcp_fallback_client_returns_plain_allow_always() {
        // non-TUI clients (Generic / IntelliHelperWeb / Extension / …) see `fallback_options`. The
    ...
    fn mcp_titleize_segment_handles_snake_camel_kebab() {
        ...
        assert_eq!(mcp_titleize_segment("grok_com_notion"), "IntelliHelper Com Notion");
    ...
    /// Same pin as above for `IntelliHelperTUI` and `Desktop` — both client types
    ...
    fn enable_always_approve_is_first_for_tui_and_desktop() {
        for ct in [ClientType::IntelliHelperTUI, ClientType::Desktop] {
            let p = prompter(ct);
