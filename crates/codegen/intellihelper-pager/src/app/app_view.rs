...
/// [`intellihelper_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<intellihelper_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: intellihelper_shell::util::config::WorktreeHintMode) -> Self {
        use intellihelper_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        intellihelper_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) =
            intellihelper_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// [`intellihelper_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// Cap for the `INTELLIHELPER_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `INTELLIHELPER_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("INTELLIHELPER_ESC_DOUBLE_PRESS_MS").ok()))
}
...
/// (toolset) gate via [`intellihelper_shell::tier::is_restricted_tier_name`] so
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => intellihelper_shell::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    /// stays sans-IO.
    pub current_ui: intellihelper_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    ...
    /// Current appearance config (hot-reloadable from ~/.intellihelper/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`INTELLIHELPER_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// Release-safe FPS HUD (`/debug fps`; `INTELLIHELPER_FPS` env on release
    /// builds, where the dev overlay is compiled out) — see the module doc.
    pub fps_hud: crate::views::fps_hud::FpsHud,
    pub active_announcements: Vec<intellihelper_announcements::RemoteAnnouncement>,
    ...
    pub announcement: Option<intellihelper_announcements::RemoteAnnouncement>,
    ...
    /// Whether the plugin marketplace CTA is enabled. Env `INTELLIHELPER_PLUGIN_CTA`
    ...
    /// No intellihelper.com billing session exists; `/usage` and credit UI stay off.
    ...
    /// Dashboard entry points (`/dashboard`, `Ctrl+\`, `intellihelper dashboard`, the
    ...
    /// non-selectable headers. Gated by `INTELLIHELPER_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `INTELLIHELPER_CANCEL_REWIND` env /
    ...
    /// Welcome-screen Optimize generation id (`None` when idle).
    pub welcome_optimize_generation: Option<u64>,
    /// Welcome draft before last successful Optimize (for Undo).
    pub welcome_optimize_undo: Option<String>,
    ...
    /// empty and inert outside `--minimal`; the `intellihelper-pager-minimal` crate
    ...
    pub session_picker_content_results:
        Option<Vec<intellihelper_shell::extensions::session_search::SearchSessionHit>>,
    ...
    pub(crate) foreign_session_compat:
        intellihelper_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// `intellihelper_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// at startup and on settings toggles from `INTELLIHELPER_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: intellihelper_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<intellihelper_shell::util::config::ContextualHintsRemote>,
    ...
    /// One-shot gate for the SSH `intellihelper wrap` tip: set after the first
    ...
    /// Login button label from `AuthMethod.name` (e.g., "intellihelper.com", "Acme Corp").
    ...
    /// `intellihelper_build_usage_redirect_url`, targeted at personal-team users).
    ...
    pub announcement_cta_impressions_logged:
        std::collections::BTreeSet<(String, intellihelper_telemetry::events::AnnouncementCtaSurface)>,
    /// Access gate from `intellihelper_build_access_gate`. `Some` = blocked.
    pub gate: Option<intellihelper_shell::auth::GateInfo>,
    ...
    pub pending_gate_verification: Option<intellihelper_shell::auth::GateInfo>,
    ...
    /// `~/.intellihelper/config.toml`. `None` when the file/section is absent
    ...
    /// kill switch or `INTELLIHELPER_VOICE_MODE=0` — the STT pipeline is not started and
    ...
    pub voice_config: intellihelper_voice::VoiceConfig,
    /// Auth for STT (OAuth session via shell `AuthManager`, or `XAI_API_KEY`).
    /// `None` until the pipeline is first started (lazy on `/voice`).
    pub voice_auth: Option<intellihelper_voice::SharedVoiceAuth>,
    /// Commands into the voice pipeline (start/stop capture — toggle, not hold).
    pub voice_cmd_tx: Option<tokio::sync::mpsc::Sender<intellihelper_voice::VoiceCommand>>,
...
impl AppView {
    ...
    pub fn gate_from_settings(
        rs: &intellihelper_shell::util::config::RemoteSettings,
    ) -> Option<intellihelper_shell::auth::GateInfo> {
        let msg = rs.gate_message.as_ref()?;
        if msg.is_empty() {
            return None;
        }
        Some(intellihelper_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    pub fn apply_auth_meta(&mut self, meta: &intellihelper_shell::auth::AuthMeta) {
        ...
        if was_gated && self.gate.is_none() {
            ...
            intellihelper_telemetry::session_ctx::log_event(
                intellihelper_telemetry::events::SubscriptionActivated {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: intellihelper_shell::agent::config::UiConfig::default(),
            ...
            tip: None,
            welcome_prompt,
            welcome_optimize_generation: None,
            welcome_optimize_undo: None,
            slash_mru,
            command_tags,
            ...
            auto_mode_gate: intellihelper_shell::util::config::auto_permission_mode_enabled_from_disk(),
            ...
            voice_config: intellihelper_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && intellihelper_voice::AUDIO_SUPPORTED
    }
    ...
    fn voice_send(&self, cmd: intellihelper_voice::VoiceCommand) {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        self.voice_send(intellihelper_voice::VoiceCommand::PttPress);
    ...
    pub(crate) fn voice_stop_keeping_final(&mut self) {
        ...
        self.voice_send(intellihelper_voice::VoiceCommand::PttRelease);
    ...
    /// Rows the dev `INTELLIHELPER_FPS` overlay occupies (0 in non-dev builds), so
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results:
        &'a Option<Vec<intellihelper_shell::extensions::session_search::SearchSessionHit>>,
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    intellihelper_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
    ...
    if let Event::Mouse(mouse) = ev {
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        intellihelper_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
impl AppView {
    ...
    fn draw_inner(&mut self, terminal: &mut PagerTerminal) {
            ...
            )
        {
            ...
            if crate::terminal::terminal_context()
                ...
                .osc22_cursor
            {
                intellihelper_shell::util::with_locked_stderr(|stderr| {
                ...
                match *active_view {
                    ActiveView::Welcome => {
                        ...
                        if !has_access && !self.access_gate_shown_logged {
                            ...
                            intellihelper_telemetry::session_ctx::log_event(
                                intellihelper_telemetry::events::SuperGrokUpsellShown {
                                    source:
                                        intellihelper_telemetry::events::SuperGrokUpsell::WelcomeScreen,
                                    auth_method: self
                                        .login_method_id
                                        .as_ref()
    ...
    /// Log [`intellihelper_telemetry::events::AnnouncementCtaShown`] for each
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        use intellihelper_telemetry::events::AnnouncementCtaSurface;
        ...
        let key = intellihelper_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
            ...
            {
                intellihelper_telemetry::session_ctx::log_event(
                    intellihelper_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: intellihelper_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    /// One-shot SSH `intellihelper wrap` tip trigger, run at the top of every `draw`
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            intellihelper_telemetry::session_ctx::log_event(intellihelper_telemetry::events::ContextualTip {
                tip: intellihelper_telemetry::events::ContextualTipKind::ImageInput,
                action: intellihelper_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
...
pub(crate) mod tests {
    ...
    /// including after the fast foreign scan lands rows the default IntelliHelper
    ...
    fn tick_demand_fast_while_modal_session_picker_loads() {
        ...
        assert_eq!(
            app.tick_demand(),
            TickDemand::Fast,
            "foreign rows hidden by the IntelliHelper filter must not end the loading spinner"
        );
    ...
    fn slash_gate_resyncs_when_critical_expires_between_pushes() {
        ...
        app.active_announcements = vec![intellihelper_announcements::RemoteAnnouncement {
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![intellihelper_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: intellihelper_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn external_auth_provider_keeps_billing_off_after_auth_meta() {
        ...
        app.apply_auth_meta(&intellihelper_shell::auth::AuthMeta::default());
    ...
    fn apply_auth_meta_disables_billing_surface_for_team_users() {
        let mut app = test_app();
        assert!(app.usage_visible);
        let meta = intellihelper_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_enables_billing_surface_for_personal_users() {
        let mut app = test_app();
        app.usage_visible = false;
        let meta = intellihelper_shell::auth::AuthMeta::default();
        app.apply_auth_meta(&meta);
        assert!(app.usage_visible);
    }
    #[test]
    ...
    fn apply_auth_meta_api_key_enables_voice_and_skips_tier_gate() {
        ...
        app.apply_auth_meta(&intellihelper_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_clears_gate_on_subscription() {
        let mut app = test_app();
        app.gate = Some(intellihelper_shell::auth::GateInfo {
            message: "Subscribe to use IntelliHelper CLI".into(),
            url: Some("https://grok.com/__PROTECT_8__?referrer=intellihelper-build".into()),
            label: None,
        });
    ...
    fn apply_auth_meta_gate_unchanged_when_still_gated() {
        let mut app = test_app();
        let gate = intellihelper_shell::auth::GateInfo {
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(intellihelper_workspace::foreign_sessions::RecentForeignSession {
    ...
    fn esc_owned_before_agent_covers_app_level_owners() {
        ...
        app.import_claude_modal = Some(
            crate::views::import_claude_modal::ImportClaudeModalState::new(
                intellihelper_shell::claude_import::ImportPlan::default(),
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![intellihelper_workspace::file_system::FuzzyMatchResult {
    ...
    fn welcome_picker_f_cycle_disabled_under_chat_mode() {
        ...
        assert_eq!(
            ...
            crate::views::session_picker::SourceFilter::IntelliHelper,
