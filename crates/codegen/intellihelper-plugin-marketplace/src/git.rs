...
//! Cache root: `~/.intellihelper/marketplace-cache/<url-hash>/`
...
pub fn sync_source_cache_with_mode(
    ...
    let url = intellihelper_agent::plugins::git_install::validate_git_url(url)?;
    let branch = branch
        .map(intellihelper_agent::plugins::git_install::validate_git_ref)
        .transpose()?;
...
pub fn default_cache_root() -> PathBuf {
    intellihelper_config::intellihelper_home().join("marketplace-cache")
}
