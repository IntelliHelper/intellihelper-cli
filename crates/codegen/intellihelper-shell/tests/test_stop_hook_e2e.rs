...
//! default since they need the intellihelper binary (`INTELLIHELPER_BINARY` or a local debug build):
//! ```bash
//! cargo test -p intellihelper-shell --test test_stop_hook_e2e -- --ignored
//! ```
...
async fn stop_block_loop_ends_at_continuation_cap() {
    ...
    assert_eq!(
        run.invocations(),
        intellihelper_shell::session::MAX_STOP_HOOK_CONTINUATIONS_PER_TURN,
