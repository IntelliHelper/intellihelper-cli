...
use intellihelper_workspace::permission::resolution::McpServerAllowlist;
...
pub(crate) fn reenableable_disabled_stubs(
    ...
    let settings = intellihelper_workspace::permission::resolution::managed_settings();
...
mod tests {
    ...
    use intellihelper_workspace::permission::resolution::AllowedMcpServer;
    ...
    /// Git repo with project `.intellihelper/config.toml` so discovery is bounded to cwd.
    fn project_repo(toml: &str) -> tempfile::TempDir {
        let tmp = tempfile::tempdir().unwrap();
        git2::Repository::init(tmp.path()).unwrap();
        // Pin trust so ambient INTELLIHELPER_CLI_VERSION cannot drop project MCP names.
        crate::agent::folder_trust::record_for_test(tmp.path(), true);
        std::fs::create_dir_all(tmp.path().join(".intellihelper")).unwrap();
        std::fs::write(tmp.path().join(".intellihelper").join("config.toml"), toml).unwrap();
        tmp
    }

    /// Isolate HOME/INTELLIHELPER_HOME so ambient user MCP config cannot pad discovery.
    fn isolated_home() -> (
        tempfile::TempDir,
        intellihelper_test_support::EnvGuard,
        intellihelper_test_support::EnvGuard,
    ) {
        let home = tempfile::tempdir().unwrap();
        let intellihelper_home = home.path().join(".intellihelper");
        std::fs::create_dir_all(&intellihelper_home).unwrap();
        std::fs::write(intellihelper_home.join("config.toml"), "").unwrap();
        let home_guard = intellihelper_test_support::EnvGuard::set("HOME", home.path());
        let intellihelper_guard = intellihelper_test_support::EnvGuard::set("INTELLIHELPER_HOME", &intellihelper_home);
        (home, home_guard, intellihelper_guard)
    }
