...
async fn get_authenticated_json<T: serde::de::DeserializeOwned>(
    ...
    let resp = match intellihelper_http::shared_client()
        .get(url)
...
impl ManagedMcpState {
    ...
    /// connector re-authorized on intellihelper.com can retry. The reactive path must NOT
...
///
/// Servers with names starting with this prefix are managed by intellihelper.com —
...
///
/// Examples:
///   `grok_com_linear`  → managed by intellihelper.com
///   `grok_com_slack`   → managed by intellihelper.com
///   `my_company_api`   → user-managed (local)
///
/// Single source of truth lives in `intellihelper-workspace` (which matches policy
/// `serverName`s against it); re-exported here so the two never drift.
pub use intellihelper_workspace::permission::resolution::MANAGED_MCP_PREFIX;
...
pub fn to_managed_name(display_name: &str) -> String {
    ...
    intellihelper_shell_base::util::truncate(&raw, MANAGED_MCP_NAME_MAX_CHARS).to_string()
}
