...
use intellihelper_announcements::RemoteAnnouncement;
...
pub struct DoomLoopRecoverySettings {
    /// Send the `x-intellihelper-doom-loop-check` header, parse the reported
...
pub struct WorktreeAutoGcSettings {
    ...
    /// Optional discovery rebuild + intellihelper-scoped stale `.git/worktrees/` scrub (default off).
    #[serde(
        default,
        deserialize_with = "de_opt_bool_tolerant",
        skip_serializing_if = "Option::is_none"
...
pub struct RemoteSettings {
    ...
    /// `app::subscription` module). Forwarded from the `intellihelper_build_settings`
    ...
    /// default (on). Sits below env `INTELLIHELPER_FOLDER_TRUST`, user
    ...
    /// remote settings `intellihelper_build_settings.max_mcp_output_bytes` — global default
    ...
    /// `intellihelper_build_settings` remote settings key) to enable it. See
    ...
    /// `intellihelper_telemetry::external::apply_remote_policy`.
    #[serde(default)]
    ...
    /// `INTELLIHELPER_TWO_PASS_COMPACTION` / default (off).
    #[serde(default)]
    ...
    /// Controlled via remote settings (`non_git_warning` in `intellihelper_build_settings`).
    ...
    /// ghost text), from the `intellihelper_build_settings` remote settings flag. Sits below
    /// env (`INTELLIHELPER_PROMPT_SUGGESTIONS_MODEL`) and `[models] prompt_suggestion`
    /// in config.toml, above the client hint and the built-in
    /// `intellihelper-build-0.1` default. The effective model is catalog-guarded: when
    ...
    /// (e.g. `intellihelper-imagine-image`) instead of the default quality model
    /// (`intellihelper-imagine-image-quality`). Absent/empty → default model.
    #[serde(default)]
    ...
    /// Overridable locally via `INTELLIHELPER_VOICE_MODE`. Free-tier SuperGrok upsell
    ...
    /// remain coding-data opted-out. From `intellihelper_build_settings`.
    ...
    /// User's subscription tier from remote settings `intellihelper_build_access_gate`.
    ...
    /// Whether the user is allowed to use IntelliHelper CLI. Set by remote settings
    /// `intellihelper_build_access_gate` targeting rules. `None` = no server response
    ...
    /// Server-controlled via the remote settings `intellihelper_build_usage_redirect_url`
    ...
    /// `intellihelper_build_settings`. Per-model override on `ModelInfo`
    /// (`intellihelper_build_models`) takes precedence; user config and env var
    /// further override per the resolver chain.
    #[serde(default)]
    pub auto_compact_threshold_percent: Option<u8>,
    /// Max subagent nesting depth (`intellihelper_build_settings.subagents_max_depth`).
    #[serde(default)]
    ...
    /// `0` disables. Env (`INTELLIHELPER_COMPACTION_WALL_CLOCK_SECS`) overrides it.
    ...
    /// Env (`INTELLIHELPER_COMPACTION_MODE`) and user config override it.
    #[serde(default)]
    pub compaction_mode: Option<String>,
    /// Segments verbatim detail (`none` | `minimal` | `balanced` | `verbose`)
    /// from remote settings. Env (`INTELLIHELPER_COMPACTION_DETAIL`) and config override it.
    #[serde(default)]
    pub compaction_detail: Option<String>,
    /// remote settings verbatim-input flag; env (`INTELLIHELPER_COMPACTION_VERBATIM_INPUT`) and config override it. `None` = default (true).
    #[serde(default)]
    ...
    /// remote settings gate for the `intellihelper workspace` CLI command (Computer Hub
    /// workspace exposure), from `intellihelper_build_settings.workspace_command_enabled`.
...
pub struct ContextualHintsRemote {
    ...
    /// SSH wrap session-load tip (recommend `intellihelper wrap ssh` for remote sessions).
    #[serde(default)]
...
/// only guaranteed to work with a compatible harness (cursor vs intellihelper-build).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GoalRoleModel {
    /// Model id, e.g. "intellihelper-4". Resolved against available models at
    /// spawn time; unknown/unauthorized ⇒ fail-open to current model.
    pub model: String,
    /// Harness `agent_type` (e.g. "cursor", "intellihelper-build-plan") whose
    /// `AgentDefinition` decides the role subagent's harness flavor (system
    /// prompt + cursor-vs-intellihelper-build toolset), applied REGARDLESS of the
...
mod tests {
    ...
    fn remote_settings_image_description_model_round_trip() {
        let json = r#"{"image_description_model": "intellihelper-build"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.image_description_model.as_deref(), Some("intellihelper-build"));
        let out = serde_json::to_string(&s).unwrap();
        let s2: RemoteSettings = serde_json::from_str(&out).unwrap();
    ...
    fn remote_settings_prompt_suggestion_model_round_trip() {
        let json = r#"{"prompt_suggestion_model": "intellihelper-build-0.1"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.prompt_suggestion_model.as_deref(), Some("intellihelper-build-0.1"));
        let out = serde_json::to_string(&s).unwrap();
        let s2: RemoteSettings = serde_json::from_str(&out).unwrap();
    ...
    fn remote_settings_goal_planner_model_round_trip() {
        let json =
            r#"{"goal_planner_model": {"model": "intellihelper-4", "agent_type": "general-purpose"}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_strategist_model_round_trip() {
        let json = r#"{"goal_strategist_model": {"model": "intellihelper-4.5", "agent_type": "cursor"}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_skeptic_models_fully_valid_pool_round_trips() {
        let json = r#"{"goal_skeptic_models": [
            {"model": "intellihelper-4", "agent_type": "general-purpose"},
            {"model": "intellihelper-3", "agent_type": "cursor"}
        ]}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
            ...
            vec![
                ...
                GoalRoleModel {
                    model: "intellihelper-3".to_string(),
                    agent_type: "cursor".to_string(),
                },
            ]
        );
    ...
    fn remote_settings_goal_skeptic_models_extra_unknown_fields_kept() {
        let json = r#"{"goal_skeptic_models": [
            {"model": "intellihelper-4", "agent_type": "general-purpose", "reasoning_effort": "high"}
        ]}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
            s.goal_skeptic_models,
    ...
    fn remote_settings_goal_role_models_malformed_pair_does_not_drop_other_fields() {
        let json = r#"{
            ...
            "goal_strategist_model": {"model": "intellihelper-4.5", "agent_type": "cursor"},
            "default_model": "intellihelper-4"
        }"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.goal_planner_model, None);
        assert_eq!(
        ...
        assert_eq!(s.default_model.as_deref(), Some("intellihelper-4"));
    }
    #[test]
    fn remote_settings_goal_role_model_extra_unknown_fields_kept_single_pair() {
        let json = r#"{"goal_planner_model": {"model": "intellihelper-4", "agent_type": "general-purpose", "future": true}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
            s.goal_planner_model,
            Some(GoalRoleModel {
