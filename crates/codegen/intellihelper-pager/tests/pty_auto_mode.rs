...
//!
//! Uses `intellihelper-pager-pty-harness` (`PtyHarness`) + Shift+Tab (CSI Z,
...
//! Always-Approve.
//!
//! Auth: seeds `HOME/.intellihelper/auth.json` from `INTELLIHELPER_AUTH_JSON` (path) or the
//! developer's `~/.intellihelper/auth.json` so the pager skips device-login when
...
//!
//! Run with:
//! `cargo test -p intellihelper-pager --test pty_auto_mode -- --ignored --nocapture`

use std::path::PathBuf;
use std::time::Duration;

use intellihelper_pager_pty_harness::{PtyHarness, pager_binary};
use intellihelper_test_support::TestSandbox;
...
/// Prefer explicit path, else the user's real `~/.intellihelper/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("INTELLIHELPER_AUTH_JSON") {
    ...
    dirs_next_home()
        .map(|h| h.join(".intellihelper/auth.json"))
        .filter(|p| p.is_file())
}
...
fn prepare_sandbox(sandbox: &mut TestSandbox, gate_on: bool) -> Vec<(String, String)> {
    ...
    let intellihelper = sandbox.intellihelper_home();
    let _ = std::fs::create_dir_all(intellihelper);
    if let Some(src) = auth_json_source() {
        let dest = intellihelper.join("auth.json");
        ...
        eprintln!("pty_auto_mode: no ~/.intellihelper/auth.json — may hit device login");
    }
    ...
    // of the developer's shell. `INTELLIHELPER_AUTO_PERMISSION_MODE` is the highest gate
    // layer below requirements; "1"/"0" parse to on/off (intellihelper_config::
    ...
    env.push((
        "INTELLIHELPER_AUTO_PERMISSION_MODE".into(),
...
fn require_auth() -> bool {
    std::env::var("INTELLIHELPER_PTY_REQUIRE_AUTH").is_ok_and(|v| v == "1" || v == "true")
}
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness =
        PtyHarness::new_in_sandbox(&binary, ROWS, COLS, &[], &sandbox, &env_refs, None)
            .expect("spawn pager in PTY (intellihelper-pager-pty-harness)");
    ...
    if is_login_screen(&early) {
        assert!(
            !require_auth(),
            "INTELLIHELPER_PTY_REQUIRE_AUTH set but pager hit the login screen — auth seeding broke"
        );
