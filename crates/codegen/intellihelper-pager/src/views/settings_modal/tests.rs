...
fn editor_render_fixture(buffer: &str, cursor_byte: usize) -> SettingsModalState {
    ...
    let snapshot = PagerLocalSnapshot {
        available_models: vec![(
            "IntelliHelper Test".to_string(),
            acp::ModelId::new(Arc::from("intellihelper-test")),
        )],
        ..PagerLocalSnapshot::default()
    };
...
fn render_editing_value_cursor_at_logical_position_when_buffer_fits() {
    let mut s = editor_render_fixture("IntelliHelper Test", 4); // cursor between "IntelliHelper" and " Test"
    let area = Rect {
        x: 0,
        y: 0,
        width: 60,
...
fn picking_enum_esc_dispatches_preview_revert_for_each_key() {
    let cases: &[(&str, &str)] = &[
        ("theme", "intellihelpernight"),
        ("auto_dark_theme", "intellihelpernight"),
...
fn picking_enum_esc_returns_to_browse() {
    let mut s = make_state();
    s.transition_to_picking_enum("theme", 0, SettingValue::Enum("intellihelpernight"), true);
    let outcome = handle_settings_key(&mut s, &KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE));
    match outcome {
        SettingsKeyOutcome::Action(Action::PreviewTheme(name)) => {
            assert_eq!(
                name, "intellihelpernight",
                "Esc revert must dispatch the original canonical"
            );
        }
        other => panic!("expected Action::PreviewTheme(\"intellihelpernight\") on Esc, got {other:?}"),
    }
    assert!(matches!(s.mode(), SettingsModalMode::Browse));
}
...
fn fork_secondary_model_picker_opens_on_persisted_model() {
    ...
    let slug = "intellihelper-4.5-fast";
    assert_ne!(slug, intellihelper_shell::models::default_model());
    let snapshot = PagerLocalSnapshot {
        available_models: vec![
            ("IntelliHelper 3".to_string(), acp::ModelId::new(Arc::from("intellihelper-3"))),
            (
                "IntelliHelper 4.5 Fast".to_string(),
    ...
    assert_eq!(
        ...
        Some(SettingValue::String("IntelliHelper 4.5 Fast".to_string())),
    );
    ...
    match s.mode() {
        SettingsModalMode::PickingEnum {
            ...
            // Choices: [(no override), IntelliHelper 3, IntelliHelper 4.5 Fast] → idx 2.
            assert_eq!(
                choices_idx, 2,
            ...
            assert_eq!(
                original_value,
                &SettingValue::String("IntelliHelper 4.5 Fast".to_string()),
...
fn editing_value_chars_mutate_buffer_and_invalid_enter_is_noop() {
    ...
    assert!(
        ...
        "validation_error must be Some for unknown model 'a' \
         (catalog has 'IntelliHelper 4 Fast' only)",
    );

    // Enter on a buffer that fails the KnownModel validator
    // (catalog has 'IntelliHelper 4 Fast'; "a" doesn't match) is
...
fn string_editor_uses_canonical_edits_policy_and_live_validation() {
    ...
    let mut state = editor_render_fixture("IntelliHelper Tes", "IntelliHelper Tes".len());
    ...
    assert_eq!(state.editing_buffer(), Some("IntelliHelper Test"));
...
//    the "Tip · Ask IntelliHelper…" docs footer and the keybindings hints.
//
...
fn string_editor_paste_sanitizes_validates_and_consumes_rejected_text() {
    let mut state = editor_render_fixture("IntelliHelper Tst", "IntelliHelper T".len());
...
fn docs_footer_tip_is_centered() {
    ...
    // SHORT = "Tip · Ask IntelliHelper to change a setting" (34 cells);
    ...
    assert!(
        !row_short.contains("intellihelperday"),
        "width=40 must NOT render LONG path (no `intellihelperday`): {row_short:?}",
    );
    assert!(
...
fn tip_line_has_blank_row_above() {
    ...
    for y in 0..area.height {
        let txt = buf_row_text(&buf, y, area.x, area.width);
        if txt.contains("Tip") && txt.contains("Ask IntelliHelper") {
            tip_y = Some(y);
            break;
        }
    }
...
fn click_settings_breadcrumb_ignores_close_on_picker_exit() {
    ...
    match outcome {
        SettingsKeyOutcome::Action(Action::PreviewTheme(orig)) => {
            assert_eq!(orig, "intellihelpernight");
        }
...
fn click_settings_breadcrumb_after_nav_reverts_to_original() {
    ...
    // Pick a different index. The default theme is `intellihelpernight`
...
fn d_key_in_picking_enum_dispatches_open_reset_confirm() {
    ...
    match outcome {
        SettingsKeyOutcome::ActionPair(
            ...
            // Default theme is `intellihelpernight`; entering the picker
            // captures `original_value = current value = intellihelpernight`,
...
fn consent_chooser_drops_tip_and_reset() {
    ...
    assert!(
        !text.contains("Ask IntelliHelper"),
    ...
    assert!(
        text.contains("d reset") && text.contains("Ask IntelliHelper"),
...
/// misleading: on dark themes (IntelliHelperNight, TokyoNight, RosePine
/// Moon) `bg_visual` is actually *lighter* than `bg_highlight`;
/// only on the IntelliHelperDay light theme is title darker. The
...
fn max_thoughts_width_preview_title_styling_distinguishes_from_content() {
    ...
    let raw_theme = match crate::theme::Theme::current_kind() {
        crate::theme::ThemeKind::IntelliHelperNight => crate::theme::Theme::intellihelpernight(),
        crate::theme::ThemeKind::TokyoNight => crate::theme::Theme::tokyonight(),
        crate::theme::ThemeKind::IntelliHelperDay => crate::theme::Theme::intellihelperday(),
        ...
        crate::theme::ThemeKind::Auto => crate::theme::Theme::intellihelpernight(),
    };
    assert_ne!(
