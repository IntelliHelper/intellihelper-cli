...
/// emitter in `intellihelper-shell` and the upsert/detection here — a drift would
...
pub enum AuthType {
    /// From AuthManager (intellihelper login, OIDC, external binary). Refreshable.
    #[default]
    SessionToken,
...
/// `intellihelper_sampling_types::SamplingConfig` (which is secret-free).
