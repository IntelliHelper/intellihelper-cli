...
use crate::auth::{AuthManager, AuthMode, IntelliHelperAuth, GrokComConfig, run_auth_flow};
...
/// or live subagent). The second signal covers relay-driven (intellihelper.com
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = intellihelper_agent::prompt::workspace_user::optional_workspace_user_dir();
...
pub async fn run_stdio_agent(
    ...
    // whoever spawned it (intellihelper-desktop, IDE clients, the agent SDKs, a parent
    ...
    intellihelper_telemetry::unified_log::set_version(intellihelper_version::VERSION);
    ...
    // Log the client that launched us (set by intellihelper-desktop when spawning `intellihelper agent stdio`).
    // This appears early in unified.jsonl and is extremely useful for auth diagnostics.
    if let Ok(version) = std::env::var("INTELLIHELPER_CLIENT_VERSION") {
        crate::unified_log::info(
            "INTELLIHELPER_CLIENT_VERSION",
            None,
    ...
    // blocking stdin reads (see `acp_lib::spawn_stdin_line_reader`): on
    ...
    let result = local_set
        .run_until(async move {
            ...
            // `intellihelper agent stdio` is a local/interactive entrypoint (spawned by
            // intellihelper-desktop), so it needs the gate like the leader and pager paths;
...
pub async fn run_headless(
    ...
    register_fs_watch_runtime();
    intellihelper_telemetry::unified_log::set_version(intellihelper_version::VERSION);
    // `intellihelper agent [headless]` serves non-TUI automation; stamp proxy requests
    // as headless. IDE-facing `intellihelper agent stdio` stays interactive.
    ...
    const HEADLESS_NO_SESSION: &str = "Headless mode requires a intellihelper.com session. \
        Run `intellihelper login` to sign in, or use `intellihelper agent stdio` for API-key access.";
    ...
    let (mut auth, did_browser_flow) = if reauthenticate {
        let auth_manager = Arc::new(AuthManager::new(&intellihelper_home::intellihelper_home(), ctx.clone()));
        run_auth_flow(
            &auth_manager,
            ctx,
            true,
    ...
    // Capture the intellihelper build URL for the first-connection callback
    let intellihelper_code_url = format!("{}/build", ctx.intellihelper_ws_origin);
    ...
    let on_first_connect: Box<dyn FnOnce() + Send + 'static> = Box::new(move || {
        if !did_browser_flow {
            ...
            eprintln!();
            eprintln!(
                "Open IntelliHelper CLI: {} (press Enter to open in browser)",
                intellihelper_code_url
            );
            eprintln!();
            let url_for_open = intellihelper_code_url.clone();
...
fn should_seed_shared_session(existing: Option<&IntelliHelperAuth>, session: &IntelliHelperAuth) -> bool {
    match existing {
        None => true,
        Some(existing) => {
...
/// Start the leader's intellihelper.com relay connection according to the start policy,
/// parking the [`RelayHandle`](crate::agent::relay::RelayHandle) in `slot`
/// once the connection task is running.
///
/// * `relay_on_demand == false` (default — explicit `intellihelper agent leader`
...
///   duplication of mirroring every agent message to intellihelper.com.
///
...
/// Everything needed to arm the leader's intellihelper.com relay *after* startup.
///
...
impl DeferredRelayArm {
    ...
    /// [`spawn_leader_relay`]).
    fn arm_if_eligible(self, session: &IntelliHelperAuth, auth_manager: &Arc<AuthManager>) -> Option<Self> {
        ...
        info!("Relay-eligible auth token appeared after startup — arming intellihelper.com relay");
        spawn_leader_relay(
            self.slot,
            relay_config,
...
/// When a intellihelper.com session is present, the leader connects to the websocket relay
...
/// * `relay_on_demand` - If true, defer the intellihelper.com relay WebSocket until the
...
pub async fn run_leader(
    ...
    let control_state = LeaderServerControlState::new(LeaderServerMetadata {
        ...
        leader_binary_version: intellihelper_version::VERSION.to_string(),
    })
    ...
    let auth: Option<IntelliHelperAuth> = crate::auth::try_noninteractive_auth_no_mint(ctx).await;

    // ── Phase 6b: Legacy devbox auth migration ─────────────────────────────
    let auth: Option<IntelliHelperAuth> = migrate_devbox_auth_if_legacy(auth, &agent_config).await;

    // A session-less leader that can still mint one (auth provider / devbox) will
    // acquire a intellihelper.com session post-readiness whose fleet policy governs
    ...
    local_set
        .run_until(async move {
            ...
            // Bridge websocket messages to agent (from intellihelper.com relay)
            ...
            // Start (or arm) the intellihelper.com relay. Eager by default — a bare
            // `intellihelper agent leader` (devbox / systemd) has no local IPC clients
            ...
            if let Some(relay_config) = relay_config {
                ...
                info!(
                    "Relay not started: no intellihelper.com session token \
            ...
            // Gated on user_intellihelper_home() so a cwd-relative .intellihelper/auth.json is never
            // read as the user auth store when no home resolves.
            let initial_auth_key_hash = intellihelper_config::user_intellihelper_home()
                ...
                while let Some(update) = config_update_rx.recv().await {
                    match update {
                        ...
                        ConfigUpdate::ModelsCacheChanged => {
                            // External write to ~/.intellihelper/models_cache.json
                            // (another intellihelper process fetched a fresher /v1/models
...
mod tests {
    ...
    fn oidc_session(key: &str, create_time: chrono::DateTime<chrono::Utc>) -> IntelliHelperAuth {
        IntelliHelperAuth {
            key: key.into(),
    ...
    fn test_relay_config(addr: std::net::SocketAddr) -> crate::agent::relay::RelayConfig {
        ...
        let cfg = crate::auth::GrokComConfig {
            intellihelper_ws_url: format!("ws://{addr}"),
            intellihelper_ws_origin: format!("http://{addr}"),
            ..Default::default()
        };
    ...
    fn embedded_otel_gate_keeps_a_session_user_fail_closed() {
        ...
        use intellihelper_telemetry::external::{
        ...
        const PROXY_ENV_VAR: &str = "INTELLIHELPER_CLI_CHAT_PROXY_BASE_URL";
        ...
        let session = IntelliHelperAuth {
            // Far-future expiry so `current()` accepts it regardless of clock skew
            // or a leaked `INTELLIHELPER_AUTH_EARLY_INVALIDATION_SECS` buffer; the gate reads
    ...
    /// `intellihelper agent leader` (devbox/systemd — no local IPC clients,
    /// `relay_on_demand == false`) must connect the intellihelper.com relay eagerly.
        ...
        local
            .run_until(async {
                ...
                spawn_leader_relay(
                    slot.clone(),
                    config,
                    false, // eager: explicit `intellihelper agent leader` invocation
                    demand_rx,
                    ws_to_agent_tx,
                    agent_to_ws_tx.clone(),
                    cancel.clone(),
    ...
    async fn deferred_arm_connects_relay_when_auth_appears() {
        ...
        local
            .run_until(async {
                ...
                let ineligible = IntelliHelperAuth::test_default();
                let arm = arm
                ...
                let eligible = IntelliHelperAuth {
                    auth_mode: AuthMode::Oidc,
                    oidc_issuer: Some(crate::auth::XAI_OAUTH2_ISSUER.to_string()),
                    ..IntelliHelperAuth::test_default()
                };
