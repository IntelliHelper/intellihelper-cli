...
/// Shared INTELLIHELPER_HOME boundary fixture for the resume-by-title startup and
/// pre-sandbox tests.
///
/// `intellihelper_home()` is OnceLock-cached process-wide, so summaries land under the
/// *resolved* home (possibly the real `~/.intellihelper` when another test pinned the
...
pub struct IntelliHelperHomeFixture {
...
impl Default for IntelliHelperHomeFixture {
    fn default() -> Self {
        Self::new()
    }
}
