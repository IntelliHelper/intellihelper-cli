...
pub(super) fn handle_settings_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    if let Some(campaigns) = update.campaigns.clone() {
        ...
        intellihelper_shell::util::config::set_remote_campaigns_from_settings(Some(&rs));
    }
    ...
    if let Some(v) = update.auto_permission_mode_enabled {
        ...
        // without a restart.
        intellihelper_shell::util::config::cache_remote_auto_permission_mode_enabled(Some(v));
        app.auto_mode_gate = intellihelper_shell::util::config::auto_permission_mode_enabled_from_disk();
        ...
        && app.permission_mode_from_soft_default
    {
        ...
        let root = intellihelper_shell::config::load_effective_config().ok();
    ...
    if let Some(v) = update.privacy_notice_rollout {
        app.privacy_notice_rollout =
            intellihelper_config::env_bool("INTELLIHELPER_PRIVACY_NOTICE_ROLLOUT").unwrap_or(v);
    }
    if let Some(v) = update.privacy_banner_reshow_days {
        app.privacy_banner_reshow_days = Some(
            std::env::var("INTELLIHELPER_PRIVACY_BANNER_RESHOW_DAYS")
                .ok()
    ...
    if let Some(v) = update.subscription_tier_display {
        ...
        if was_api_key
            ...
            && app
                .subscription_tier
                .as_deref()
                .is_some_and(intellihelper_shell::tier::is_restricted_tier_name)
        {
            app.voice_reset();
    ...
    if let Some(remote_val) = update.session_picker_grouped {
        let resolved = std::env::var("INTELLIHELPER_SESSION_PICKER_GROUPED")
            .ok()
        ...
        && !msg.is_empty()
    {
        ...
        let effs = app.impose_gate(intellihelper_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    let (requirements, user_config, managed_config) = (
        intellihelper_shell::config::load_merged_requirements(),
        intellihelper_shell::config::load_from_disk().ok(),
        intellihelper_shell::config::load_managed_config().ok(),
    );
    ...
    // until restart.
    let remote = intellihelper_shell::util::config::RemoteSettings {
        group_tool_verbs: update.group_tool_verbs,
        ..Default::default()
    };
    let resolved = intellihelper_shell::util::config::resolve_group_tool_verbs(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    let resolved = intellihelper_shell::util::config::resolve_collapsed_edit_blocks(
        requirements.as_ref(),
        user_config.as_ref(),
        managed_config.as_ref(),
        Some(&remote),
    ...
    if let Some(remote_tips) = update.tips {
        use intellihelper_shell::util::config::resolve_tips;
    ...
    if let Some(remote_tags) = update.slash_command_tags.as_ref() {
        use intellihelper_shell::util::config::resolve_slash_command_tags;
        let effective_config = intellihelper_shell::config::load_effective_config().ok();
...
pub(super) fn apply_soft_default_permission_mode(
    ...
    let mode = intellihelper_shell::util::config::resolve_permission_mode(effective_ui, remote);
    ...
    app.current_ui.permission_mode = Some(if auto {
        ...
        intellihelper_shell::util::config::resolved_display_permission_mode(effective_ui, remote)
            .to_string()
    });
}
...
pub(super) fn handle_announcements_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) =
        serde_json::from_str::<intellihelper_announcements::AnnouncementsRefreshed>(notif.params.get())
    else {
        return false;
    };
...
/// `resolve_announcements` honors `INTELLIHELPER_ANNOUNCEMENTS_OVERRIDE` first, so a
/// backend push can't reintroduce announcements when the override is set.
pub(super) fn apply_announcements_update(
    app: &mut AppView,
    next_gen: u64,
    remote: &[intellihelper_announcements::RemoteAnnouncement],
    requirements: Option<&toml::Value>,
    user_config: Option<&toml::Value>,
    managed_config: Option<&toml::Value>,
) {
    let merged = intellihelper_shell::util::config::resolve_announcements(
        requirements,
        user_config,
        managed_config,
        Some(remote),
    );
    let announcements = intellihelper_announcements::filter_expired(merged);
    ...
    if intellihelper_announcements::prune_hidden_announcement_ids(
...
pub(super) fn pick_random_announcement(
    announcements: &[intellihelper_announcements::RemoteAnnouncement],
) -> Option<intellihelper_announcements::RemoteAnnouncement> {
...
/// `intellihelper-shell/src/agent/mvp_agent.rs`. The shell side derives `Serialize`
...
/// `intellihelper-shell/src/agent/mvp_agent.rs` when adding fields that both sides
/// need.
...
pub(super) struct PagerSettingsUpdate {
    ...
    #[serde(default)]
    campaigns: Option<Vec<intellihelper_shell::util::config::CampaignOverride>>,
    #[serde(default)]
