...
pub struct SubagentRuntimeOverrides {
    ...
    /// `"intellihelper-build-plan"`) whose `AgentDefinition` decides the child's harness
...
/// defense-in-depth check in `intellihelper-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "IntelliHelperBuild:run_terminal_cmd" | "IntelliHelperBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
register_resource!("intellihelper_build", "SubagentEventSender", SubagentEventSender);
...
register_resource!("intellihelper_build", "SubagentDepthCounter", SubagentDepthCounter);

/// Host-injected max nesting depth; absent → [`super::MAX_SUBAGENT_DEPTH`].
#[derive(Debug, Clone, Copy)]
pub struct MaxSubagentDepth(pub u32);

register_resource!("intellihelper_build", "MaxSubagentDepth", MaxSubagentDepth);
...
register_resource!("intellihelper_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("intellihelper_build", "SessionIdResource", SessionIdResource);
...
///
/// Set by intellihelper-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by intellihelper-shell at turn start.
...
register_resource!("intellihelper_build", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("IntelliHelperBuild:run_terminal_cmd", ToolKind::Execute),
                tc("IntelliHelperBuild:read_file", ToolKind::Read),
                tc("IntelliHelperBuild:list_dir", ToolKind::List),
                tc("IntelliHelperBuild:grep", ToolKind::Search),
                tc("IntelliHelperBuild:kill_task", ToolKind::KillTaskAction),
                tc("IntelliHelperBuild:get_task_output", ToolKind::BackgroundTaskAction),
            ],
    ...
    fn read_only_filter_keeps_background_task_tools_when_task_tool_remains() {
        let mut config = ToolServerConfig {
            tools: vec![
                ...
                tc("IntelliHelperBuild:kill_task", ToolKind::KillTaskAction),
                tc("IntelliHelperBuild:get_task_output", ToolKind::BackgroundTaskAction),
                tc("IntelliHelperBuild:task", ToolKind::Task),
            ],
            behavior_preset: None,
        };
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("IntelliHelperBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
