...
use intellihelper_tools::implementations::intellihelper_build::task::backend::SubagentBackend;
...
impl MvpAgent {
    ...
    /// Aux sampling client for Optimize-input when no session is loaded yet
    /// (welcome composer). Uses the session-summary model slug when resolvable.
    pub(crate) fn build_optimize_client(&self) -> Result<(OaiCompatClient, String), acp::Error> {
        let slug = self.resolve_session_summary_model();
        let session_key = self.auth_manager.current_or_expired().map(|a| a.key.clone());
        let models = self.models_manager.models();
        let endpoints = self.models_manager.endpoints();
        ...
        let config = crate::agent::config::resolve_aux_model_sampling_config(
            &slug,
            &models,
            &endpoints,
            session_key.as_deref(),
        ...
        .ok_or_else(|| {
            acp::Error::internal_error().data(
                "Could not build sampling client for optimize (sign in or check model config).",
            )
        })?;
        let model = config.model.clone();
        let client = OaiCompatClient::new(config).map_err(map_sampling_err_to_acp)?;
    ...
    pub(super) fn current_or_buffered_auth(&self) -> Option<crate::auth::IntelliHelperAuth> {
        self.auth_manager
            .current()
            .or_else(|| {
                if self.is_session_based_auth() {
                    ...
                    if auth.is_some() {
                        intellihelper_telemetry::unified_log::info(
    ...
    /// must not block the ACP response (intellihelper-desktop sends `initialize` immediately).
    ...
    /// plus user/marketplace walks stalled intellihelper-desktop's first `initialize`),
    ...
    pub(super) fn ensure_telemetry_client(&self) {
        ...
        if !mode.is_disabled() {
            ...
            intellihelper_telemetry::client::init_if_needed(
                cfg.telemetry.clone(),
                mode,
                user_id,
                team_id,
                ...
                intellihelper_version::VERSION.to_owned(),
                subscription_tier,
    ...
    /// Return the current IntelliHelperAuth credentials, if authenticated and not expired.
    pub(crate) fn current_auth(&self) -> Option<crate::auth::IntelliHelperAuth> {
    ...
    pub(crate) fn plugin_registry_handle(
        &self,
    ) -> &intellihelper_agent::plugins::SharedPluginRegistryHandle {
    ...
    fn arm_local_workspace_watcher(
        ...
        let auth_path = intellihelper_workspace::hub_auth::default_auth_path().ok();
    ...
    fn ensure_attach_fs_only_advertised_tools() -> Result<(), String> {
        const ENV: &str = "INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS";
        const ALLOW: &[&str] = &[
            "workspace.fs_list",
            "workspace.fs_exists",
        ...
        let Some(raw) = std::env::var(ENV)
            ...
            return Err(
                "attached workspace_server advertised toolset is uncheckable; refuse attach \
                 (set INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS to a comma-separated FS-only catalog)"
                    .into(),
            );
        };
        let ids: Vec<&str> = raw
    ...
    /// discovery on intellihelper-desktop Windows).
    fn ensure_local_workspace_ops(
        &self,
    ) -> Result<intellihelper_workspace::WorkspaceOps, acp::Error> {
        ...
        let workspace_identity = self
            ...
            .map(|a| match a.team_id.filter(|t| !t.is_empty()) {
                Some(team) => {
                    intellihelper_workspace::WorkspaceIdentity::team(a.user_id, team)
                }
                None => {
                    intellihelper_workspace::WorkspaceIdentity::new(
                        a.user_id,
                        a.principal_type,
                        a.principal_id,
                    )
            ...
            .unwrap_or_default();
        let ops = match intellihelper_workspace::handle::WorkspaceHandle::new_minimal(
            cwd.to_path_buf(),
            workspace_identity,
            project_lsp_trusted,
        ) {
            Ok(handle) => intellihelper_workspace::WorkspaceOps::local(handle),
            Err(e) => {
                tracing::error!(
                    error = %e,
    ...
    ///     `intellihelper.com`, `oidc`) -- even if the in-memory token is currently
    ///     expired or missing.
    ///
    ...
    /// else `intellihelper.com`. `None` when `preferred_method` is pinned.
    ...
    /// `use_oauth` only when the target is interactive `intellihelper.com`. When
    ...
    pub(super) async fn authenticate_after_cached_token_unavailable(
        ...
        let Some(method_id) = self.cached_token_fallthrough_method_id() else {
            ...
            intellihelper_telemetry::unified_log::warn(
        ...
        let meta = if method_id.0.as_ref() == auth_method::INTELLIHELPER_COM_METHOD_ID {
    ...
    pub(super) async fn refresh_remote_settings(&self, auth: &crate::auth::IntelliHelperAuth) {
        ...
        let (
            telemetry_config,
            telemetry_mode,
            intellihelper_user_id,
            intellihelper_team_id,
            deployment_key,
            subscription_tier,
        ) = {
            let cfg = self.cfg.borrow();
            ...
            let intellihelper_user_id = is_xai.then(|| user_id.clone());
            let intellihelper_team_id = is_xai.then(|| team_id.clone()).flatten();
        ...
        intellihelper_telemetry::client::init(
            telemetry_config,
            telemetry_mode,
            intellihelper_user_id,
            intellihelper_team_id,
    ...
    pub(super) fn spawn_post_auth_settings(&self, auth: crate::auth::IntelliHelperAuth) {
        let agent_ref = LocalRef::new(self);
        let _spawned = self
            .spawn_coalesced_settings_task(
    ...
    pub(super) fn apply_polled_announcements(
        ...
        pre_fetch: Option<Vec<intellihelper_announcements::RemoteAnnouncement>>,
    ) {
        let mut cfg = self.cfg.borrow_mut();
    ...
    /// Whether the current session is a personal intellihelper.com account on a gated
    ...
    pub(super) fn prepare_image_gen_config(
        &self,
    ) -> intellihelper_tools::implementations::intellihelper_build::image_gen::ImageGenConfig {
        ...
        let version = cfg
            .client_version
            .clone()
            .unwrap_or_else(|| intellihelper_version::VERSION.to_string());
        let alpha_test_key = cfg.endpoints.alpha_test_key.clone();
        let mut headers = indexmap::IndexMap::new();
        headers.insert("user-agent".to_string(), format!("intellihelper-build/{version}"));
    ...
    pub(super) fn prepare_app_builder_deployer_config(
        ...
        use intellihelper_tools::implementations::intellihelper_build::deploy_app::AppBuilderDeployerConfig;
    ...
    pub(super) fn prepare_video_gen_config(
        &self,
    ) -> intellihelper_tools::implementations::intellihelper_build::video_gen::VideoGenConfig {
    ...
    ///
    /// Enabled gate: `disable_web_search` kill-switch > `INTELLIHELPER_WEB_FETCH` env >
    /// remote settings `web_fetch_enabled` > default (false).
    ///
    /// Params resolution (TOML > env > remote settings > default):
    /// - `proxy_endpoint`: `[toolset.web_fetch] proxy_endpoint` > `INTELLIHELPER_WEB_FETCH_PROXY` > remote settings > None
    /// - `allowed_domains`: `[toolset.web_fetch] allowed_domains` > remote settings > built-in defaults
    /// - `allow_local`: `[toolset.web_fetch] allow_local` > `INTELLIHELPER_WEB_FETCH_ALLOW_LOCAL` > false
    pub(super) fn prepare_web_fetch_config(
        &self,
    ) -> intellihelper_tools::implementations::intellihelper_build::web_fetch::WebFetchConfig {
    ...
    pub fn with_models(
        ...
        if relay_sync_enabled {
            tracing::info!("[intellihelper] Relay sync: ENABLED");
        } else if tui_mode && relay_config_enabled && !has_xai_auth {
            tracing::info!("[intellihelper] Relay sync: DISABLED (no auth - run 'intellihelper login' first)");
        ...
        let instance = Self {
            ...
            plugin_registry_handle: intellihelper_agent::plugins::SharedPluginRegistryHandle::new(
                None,
            ...
            monitor_event_buffer: intellihelper_tools::implementations::intellihelper_build::monitor::types::MonitorEventBuffer::default(),
    ...
    pub(crate) async fn kill_background_task(
        ...
        session_id: &str,
        task_id: &str,
    ) -> Result<intellihelper_tools::types::KillOutcome, String> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub(crate) async fn cancel_subagent(
        &self,
        subagent_id: &str,
    ) -> intellihelper_tools::implementations::intellihelper_build::task::types::SubagentCancelOutcome {
        intellihelper_tools::implementations::intellihelper_build::task::backend::ChannelBackend::new(
    ...
    pub(crate) async fn list_running_subagents(
        ...
        intellihelper_tools::implementations::intellihelper_build::task::types::SubagentInspection,
    > {
        intellihelper_tools::implementations::intellihelper_build::task::backend::ChannelBackend::new(
                self.subagent_event_tx.clone(),
            )
    ...
    pub(crate) async fn query_subagent(
        ...
        intellihelper_tools::implementations::intellihelper_build::task::types::SubagentSnapshot,
    > {
        intellihelper_tools::implementations::intellihelper_build::task::backend::ChannelBackend::new(
                self.subagent_event_tx.clone(),
            )
    ...
    pub async fn list_tasks(
        &self,
        session_id: &str,
    ) -> Option<Vec<intellihelper_tools::types::TaskSnapshot>> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    /// 2. Config file/env enables it ([relay] enabled or INTELLIHELPER_RELAY_SYNC_ENABLED)
    ...
    pub(crate) async fn execute_hooks_action(
            ...
            && let Some(cwd) = self.get_session_cwd(session_id)
        {
            self.interactive_trust_prompted
                .borrow_mut()
                .remove(&intellihelper_workspace::trust::workspace_key(&cwd));
        }
        let handle = self.get_session_handle(session_id)?;
    ...
    pub(crate) fn plugin_registry_snapshot(
        &self,
    ) -> Option<std::sync::Arc<intellihelper_agent::plugins::PluginRegistry>> {
    ...
    pub(super) fn insert_session_config_meta(
        ...
        let detail = session_config::IntelliHelperSessionDetail::build(
    ...
    pub(super) async fn upload_harness_trace_turns(
        ...
        model: &str,
        turns: Vec<Vec<intellihelper_sampling_types::conversation::ConversationItem>>,
    ) {
    ...
    pub(super) async fn build_harness_trace_uploads(
        ...
        for (offset, items) in turns.into_iter().enumerate() {
            ...
            let metadata = PromptMetadata {
                ...
                agent_type: None,
                shell_version: Some(intellihelper_version::VERSION.to_string()),
                workspace_type: None,
    ...
    pub(crate) async fn get_trace_context(
        ...
        let upload_method = match upload_method {
            Some(method) => method,
            None => {
                intellihelper_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::TraceUploadSkipped {
        ...
        let queue = session_handle
            .upload_queue
            .get_or_init(|| {
                let intellihelper_home = crate::util::intellihelper_home::intellihelper_home();
    ...
    /// 5. `INTELLIHELPER_AGENT` env var.
    /// 6. Built-in default agent.
    ///
    /// `INTELLIHELPER_AGENT` and an explicit `[agent] name` bypass step 1.
    /// Strict-harness classification is structural — see
    /// [`intellihelper_agent::config::is_strict_harness_agent_type`].
    ///
    ...
    pub fn resolve_agent_definition(
        ...
        acp_agent_profile: Option<intellihelper_agent::AgentDefinition>,
        model_agent_type: Option<&str>,
    ) -> intellihelper_agent::AgentDefinition {
        use intellihelper_agent::AgentDefinition;
        let intellihelper_agent_env_set = std::env::var("INTELLIHELPER_AGENT")
            .ok()
            .is_some_and(|s| !s.trim().is_empty());
        let config_agent_explicitly_set = agent_config.name.is_some();
        let model_requires_strict_harness = model_agent_type
            .is_some_and(intellihelper_agent::config::is_strict_harness_agent_type);
        if !intellihelper_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && let Some(def) = intellihelper_agent::discovery::by_name_in_cwd(required, cwd)
        {
            tracing::info!(
                agent_name = %def.name,
        ...
        if let Some(ref name) = agent_config.name {
            ...
            if let Some(def) = intellihelper_agent::discovery::by_name_in_cwd(name, cwd) {
                return def;
            }
            tracing::warn!(
                agent_name = %name,
        ...
        let agent_name = std::env::var("INTELLIHELPER_AGENT").ok();
        let resolved = match agent_name.as_deref() {
            Some("browser-use") | Some("browser_use") => AgentDefinition::browser_use(),
            Some("intellihelper-build-concise") | Some("intellihelper_build_concise") => {
                AgentDefinition::intellihelper_build_concise()
            }
            Some(path) if std::path::Path::new(path).is_absolute() => {
                match AgentDefinition::from_file(path) {
                    ...
                    Err(e) => {
                        ...
                        AgentDefinition::intellihelper_build_plan()
                    }
                }
            }
            Some(name) => {
                intellihelper_agent::discovery::by_name_in_cwd(name, cwd)
                    .unwrap_or_else(AgentDefinition::intellihelper_build_plan)
            }
            None => AgentDefinition::intellihelper_build_plan(),
        };
        if !intellihelper_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && resolved.name != required
        {
            ...
            if let Some(def) = intellihelper_agent::discovery::by_name_in_cwd(required, cwd) {
                return def;
            }
            tracing::warn!(
                model_agent_type = %required,
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let fs: Arc<dyn intellihelper_workspace::file_system::AsyncFileSystem> = if use_acp_fs {
        ...
        let loc_tracking_enabled = hunk_tracking_enabled && has_xai_auth
            && (self
                ...
                .unwrap_or(false)
                || std::env::var("INTELLIHELPER_LOC_TRACKING")
        ...
        let mut session_env = intellihelper_workspace::permission::claude_settings::load_claude_env_with_project(
            cwd.as_path(),
            project_env_trusted,
        );
        let envrc = match preloaded_envrc {
            Some(env) => env,
            None => {
                intellihelper_workspace::envrc::load_envrc_or_empty_when_trusted(
                    cwd.as_path(),
        ...
        let pinned_model: Option<(acp::ModelId, ModelEntry)> = match &agent_definition
            .model
        {
            intellihelper_agent::config::ModelOverride::Override(id) => {
            ...
            intellihelper_agent::config::ModelOverride::Inherit => None,
        };
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            let (
                ...
                .unzip();
            let sourced = intellihelper_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
            ...
            if servers.is_empty() {
                let user_path = intellihelper_tools::util::intellihelper_home::intellihelper_home()
                    .join("lsp.json");
                let project_path = tool_ctx.cwd.as_path().join(".intellihelper").join("lsp.json");
                tracing::debug!(
                    cwd = %tool_ctx.cwd,
        ...
        let (mut handle, permission_events_rx, agent_system_prompt, session_thread) = {
            ...
            let attribution_callback: Option<
                intellihelper_sampler::SharedAttributionCallback,
            > = Some(
            ...
            let agent_hook_registry_override = agent_definition
                ...
                .and_then(|hooks_config| {
                    ...
                    let (specs, errors) = intellihelper_hooks::config::parse_hooks_from_value_with_dir(
                        &hooks_val,
                        &format!(
                        "{}{}",
                        intellihelper_hooks::config::AGENT_HOOK_PREFIX,
                    ...
                    let git_root = intellihelper_workspace::session::git::find_git_root_from_path(
                            cwd,
                        )
                        .ok();
            ...
            spawn_session_on_thread(
                    ...
                    subagent_toggle,
                    Vec::new(),
                    intellihelper_agent::prompt::context::PromptAudience::Primary,
                    None,
                    None,
                    disable_web_search,
