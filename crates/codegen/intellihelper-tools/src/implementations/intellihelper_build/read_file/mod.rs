...
mod versions;
use crate::types::schema::IntelliHelperIntegerSchema;
...
crate::register_resource!("intellihelper_build", "ReadFile", ReadFileParams);
...
pub struct ReadFileInput {
    ...
    #[schemars(
        with = "IntelliHelperIntegerSchema",
...
/// an empty window (harness-aligned; not a IntelliHelper-line clamp).
...
";
    ...
    async fn skill_file_ignores_offset_and_limit() {
        let tmp = TempDir::new().unwrap();
        let skill_dir = tmp.path().join(".intellihelper/skills/commit");
        ...
        let resources = test_resources(tmp.path());
        let input = ReadFileInput {
            path: ".intellihelper/skills/commit/SKILL.md".to_string(),
            offset: Some(3),
            limit: Some(1),
            pages: None,
            format: None,
    ...
    async fn md_in_skills_dir_ignores_model_offset_and_limit() {
        let tmp = TempDir::new().unwrap();
        let skill_dir = tmp.path().join(".intellihelper/skills/my-skill");
        std::fs::create_dir_all(&skill_dir).unwrap();
        let content = (1..=1200)
            .map(|n| format!("line{n}"))
            .collect::<Vec<_>>()
        ...
        let resources = test_resources(tmp.path());
        let input = ReadFileInput {
            path: ".intellihelper/skills/my-skill/reference.md".to_string(),
            offset: Some(3),
            limit: Some(1),
            pages: None,
            format: None,
    ...
    /// phantom field only (start past any `split_inclusive` line), so IntelliHelper
