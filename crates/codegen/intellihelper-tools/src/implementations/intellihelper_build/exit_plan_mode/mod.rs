...
//! ## Plan File
//!
//! The plan file path defaults to `.intellihelper/plan.md` relative to the session
...
Use this after you have finished writing your plan to the plan file in plan mode."#
    ...
    fn requires_expr(&self) -> Expr<ToolRequirement> {
        ...
        use crate::implementations::intellihelper_build::enter_plan_mode::EnterPlanModeTool;
        Expr::Value(ToolRequirement::Tool {
            namespace: crate::types::tool_metadata::ToolMetadata::tool_namespace(
                &EnterPlanModeTool,
            )
            .to_string(),
            id: tool_runtime::Tool::id(&EnterPlanModeTool).to_string(),
            if_params: None,
        })
    }
}

impl tool_runtime::Tool for ExitPlanModeTool {
    ...
    ) -> Result<ExitPlanModeOutput, tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_is_read_only() {
        assert!(tool_runtime::Tool::capabilities(&ExitPlanModeTool).is_read_only);
    }
    ...
    async fn exit_with_plan_content() {
        ...
        let plan_dir = tmp.path().join(".intellihelper");
        ...
        match result {
            ExitPlanModeOutput::PlanReady {
                ...
                assert!(plan_file_path.ends_with(".intellihelper/plan.md"));
            }
    ...
    async fn sends_plan_mode_exited_notification_with_content() {
        ...
        match notification {
            ToolNotification::PlanModeExited(exited) => {
                ...
                assert!(exited.plan_file_path.ends_with(".intellihelper/plan.md"));
            }
    ...
    async fn prompt_format_includes_plan_content() {
        ...
        assert!(prompt.contains(".intellihelper/plan.md"));
