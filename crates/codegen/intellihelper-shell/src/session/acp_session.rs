...
use intellihelper_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use intellihelper_agent::prompt::skills::SkillsConfig;
use intellihelper_sampler::SamplerConfig as SamplingConfig;
use intellihelper_sampling_types::truncate_bytes;
...
use intellihelper_tools::types::compat::CompatConfig;
use intellihelper_tools::types::output::{
    BashOutput, ReadFileOutput, ToolOutput as ToolsToolOutput, ToolRunResult,
};
use intellihelper_workspace::file_system::CodebaseIndexManager;
use intellihelper_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use intellihelper_workspace::session::file_state::{FileStateHandle, FileStateTracker};
...
pub(crate) struct InputItem {
    ...
    pub(crate) tool_overrides_update: Option<intellihelper_sampling_types::ToolOverridesUpdate>,
...
impl intellihelper_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        intellihelper_tools::types::resources::ManagedGatewayToolCallResponse,
...
pub(crate) struct SessionActor {
    ...
    /// `intellihelper-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<intellihelper_sampler::SharedAttributionCallback>,
    ...
    pub(crate) tool_overrides: std::cell::RefCell<Option<intellihelper_sampling_types::ToolOverrides>>,
    ...
    pub(crate) resolved_tool_overrides:
        std::sync::Arc<arc_swap::ArcSwapOption<intellihelper_sampling_types::ToolOverrides>>,
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<intellihelper_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<intellihelper_sampling_types::CompactionAtTokens>>,
    ...
    /// sampler itself sends the matching `x-intellihelper-doom-loop-check` header.
    pub(crate) doom_loop_recovery: Option<intellihelper_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    pub(crate) agent: std::cell::RefCell<intellihelper_agent::Agent>,
    ...
    /// Kill-switch (`INTELLIHELPER_GOAL_USE_CURRENT_MODEL_ONLY` / `[features]
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<intellihelper_tools::implementations::intellihelper_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, intellihelper_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<intellihelper_hooks::discovery::HookRegistry>>>,
    ...
    /// and INTELLIHELPER_WORKSPACE_ROOT env var.
    pub(crate) hook_resolved_workspace_root: String,
    /// The detected VCS kind for this session's workspace.
    pub(crate) vcs_kind: intellihelper_workspace::session::git::VcsKind,
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<intellihelper_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<intellihelper_agent::plugins::SharedPluginRegistryHandle>,
    ...
    /// Handle to the per-session `intellihelper-sampler` actor.
    ///
    ...
    pub(crate) sampler_handle: intellihelper_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`intellihelper_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`intellihelper_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: intellihelper_workspace::WorkspaceOps,
    ...
    fn build_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names.iter().any(|n| {
                n == intellihelper_tools::implementations::intellihelper_build::SCHEDULER_CREATE_TOOL_NAME
            }),
            hooks: self.hook_registry.borrow().is_some(),
            plugins: self.plugin_registry.borrow().is_some(),
            goal,
            workflows: tool_names.iter().any(|n| {
                n == intellihelper_tools::implementations::intellihelper_build::workflow::WORKFLOW_TOOL_NAME
            }),
...
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<intellihelper_tools::bridge::ToolBridge> {
...
/// The saved JSON enables deterministic re-rendering, `intellihelper prompt --json`
/// inspection, and post-hoc debugging of what went into a session's system prompt.
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &intellihelper_agent::PromptContext) {
...
) -> Option<intellihelper_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use intellihelper_tools::types::output::{MCPOutput, ToolOutput};
    use intellihelper_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl intellihelper_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
...
mod tool_meta_stamp_tests {
    ...
    use super::*;
    use tokio::sync::mpsc;
    use intellihelper_tools::registry::types::ToolConfig;
    use intellihelper_tools::tool_taxonomy::TOOL_META_KEY;
    use intellihelper_workspace::permission::PermissionCommand;
            ...
            .run_until(async {
                ...
                fixture.actor.agent = std::cell::RefCell::new(
                    test_agent_with_tools(vec![ToolConfig::from_id(
                        "IntelliHelperBuild:read_file".to_string(),
                    )])
                    .await,
                );
                ...
                assert_eq!(t["namespace"], "intellihelper_build");
...
mod managed_gateway_tool_tests {
    ...
    use intellihelper_tools::types::output::{MCPOutput, ToolOutput};
    use intellihelper_tools::types::tool::{ToolKind, ToolNamespace};
    use intellihelper_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<intellihelper_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
