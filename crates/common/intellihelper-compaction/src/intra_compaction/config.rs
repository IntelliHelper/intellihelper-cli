...
///
/// - `FullReplace` (default): intellihelper-build's full-replace strategy — summarize
...
///
/// Uses **percentage** thresholds (borrowed from intellihelper-shell's
...
/// time (token threshold only, matching intellihelper-build) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as intellihelper-build full-replace auto-compact). Worthless early passes are
    ...
    /// `<intellihelper_user_queries>` preamble prepended to the history compaction
...
pub const DEFAULT_COMPACTION_MODEL_NAME: &str = "intellihelper-4.20";
