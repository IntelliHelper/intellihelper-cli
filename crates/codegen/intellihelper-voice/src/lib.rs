//! Voice input for IntelliHelper CLI: an xAI streaming STT client and the
...
mod intercept_tests {
    ...
    fn capture_subcommand_matches_only_argv1() {
        assert!(is_capture_subcommand(&argv(&["intellihelper", "__mic-capture"])));
        ...
        assert!(!is_capture_subcommand(&argv(&["intellihelper"])));
        assert!(!is_capture_subcommand(&argv(&["intellihelper", "chat"])));
        assert!(!is_capture_subcommand(&argv(&[
            "intellihelper",
            "chat",
            "__mic-capture"
