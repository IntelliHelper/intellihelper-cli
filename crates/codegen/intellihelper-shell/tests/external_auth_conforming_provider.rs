...
//!
//! Operator binaries live outside this repo and read `INTELLIHELPER_AUTH_EXPIRED=1` as
...
use chrono::Utc;
use intellihelper_shell::auth::{
    AuthMode, IntelliHelperAuth, GrokComConfig, ensure_authenticated, try_ensure_fresh_auth,
};
...
fn write_conforming_provider(home: &Path) -> String {
    ...
    std::fs::write(
        ...
        format!(
            "#!/bin/sh\n\
             echo \"expired=${{INTELLIHELPER_AUTH_EXPIRED:-unset}}\" >> {log}\n\
...
fn seed_expired_credential(home: &Path, scope: &str) {
    let expired = IntelliHelperAuth {
        ...
        ..IntelliHelperAuth::default()
    };
    let store: BTreeMap<String, IntelliHelperAuth> = [(scope.to_owned(), expired)].into_iter().collect();
    std::fs::write(
        home.join("auth.json"),
...
async fn a_provider_that_declines_the_headless_run_can_still_sign_the_user_in() {
    let home = tempfile::tempdir().expect("intellihelper home");
    ...
    // environment is spawned. `intellihelper_home()` memoizes, so this must stay the
    // only test in the binary.
    unsafe {
        std::env::set_var("INTELLIHELPER_HOME", home.path());
        std::env::set_var("INTELLIHELPER_CLI_CHAT_PROXY_BASE_URL", &dead);
        std::env::set_var("INTELLIHELPER_XAI_API_BASE_URL", &dead);
