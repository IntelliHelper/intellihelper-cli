...
impl RelocationJournal {
    ...
    pub(super) fn validate(&self, intellihelper_home: &Path) -> Result<()> {
        ...
        if session_dir_at(intellihelper_home, &self.source_cwd, &self.session_id)
            == session_dir_at(intellihelper_home, &self.target_cwd, &self.session_id)
        {
            return Err(RelocationError::Inconsistent(
...
pub(super) fn acquire(intellihelper_home: &Path, session_id: &str) -> Result<RelocationLease> {
    validate_component("session id", session_id)?;
    let dir = relocation_dir(intellihelper_home);
...
pub(super) fn read(intellihelper_home: &Path, session_id: &str) -> Result<RelocationJournal> {
    validate_component("session id", session_id)?;
    let path = journal_path(intellihelper_home, session_id);
    ...
    journal.validate(intellihelper_home)?;
    Ok(journal)
}
...
pub(super) fn write(
    ...
    journal
        .validate(intellihelper_home)
        .map_err(WriteFailure::NotCommitted)?;
    let path = journal_path(intellihelper_home, &journal.session_id);
...
pub(super) fn relocation_dir(intellihelper_home: &Path) -> PathBuf {
    intellihelper_home.join(RELOCATION_DIR)
}

pub(super) fn journal_path(intellihelper_home: &Path, session_id: &str) -> PathBuf {
    relocation_dir(intellihelper_home).join(format!("{session_id}.json"))
}

pub(super) fn session_dir_at(intellihelper_home: &Path, cwd: &str, session_id: &str) -> PathBuf {
    intellihelper_home
        .join("sessions")
...
pub(super) fn sync_namespace(intellihelper_home: &Path) -> Result<()> {
    let dir = relocation_dir(intellihelper_home);
