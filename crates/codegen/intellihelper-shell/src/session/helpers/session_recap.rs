...
///    (intellihelper/SGLang via ChatCompletions/Responses) keeps reasoning VERBATIM so
...
/// `max_prompt_length` for current `intellihelper-build` / `intellihelper-4.5` product backends
...
/// that unlikely for normal intellihelper-build sessions).
///
/// * Fast path — if the whole snapshot already fits, returns
///   `build_instruction_items(...)` verbatim (keeps the intellihelper prefix KV cache
...
pub(crate) fn budget_instruction_items(
    ...
    // verbatim path keeps the intellihelper prefix cache warm.
...
mod tests {
    ...
    fn main_turn_count_counts_real_users_only() {
        use std::sync::Arc;
        use intellihelper_sampling_types::{ContentPart, SyntheticReason, ToolCall, UserItem};
    ...
    fn mk_tool_call(id: &str, args: &str) -> intellihelper_sampling_types::ToolCall {
        use std::sync::Arc;
    ...
    fn budget_over_budget_strips_reasoning_even_on_grok() {
        ...
        // intellihelper backend => strip_reasoning=false, but the over-budget branch must
    ...
    fn budget_fast_path_keeps_reasoning_on_grok() {
        ...
        // Fits under a large window on intellihelper (strip_reasoning=false) => verbatim,
        // reasoning kept so the prefix KV cache stays warm.
        let out = budget_recap_items(conv, "system-reminder", false, 256_000);
        assert!(
            out.iter()
                .any(|i| matches!(i, ConversationItem::Reasoning(_))),
            "fits path on intellihelper must keep reasoning verbatim"
        );
    }
