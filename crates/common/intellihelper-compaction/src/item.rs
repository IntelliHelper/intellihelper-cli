...
//! harness implements [`CompactionItem`] for its `IntelliHelperTurn`;
//! intellihelper-build implements it for `intellihelper_sampling_types::ConversationItem`.
//!
...
//! `<intellihelper_user_queries>` blocks, drop tool content from assistant turns,
//! wrap an LLM summary into a carrier item).

/// Harness-agnostic role of a single conversation item.
///
/// This is the common denominator of `IntelliHelperRole` (IntelliHelper chat) and the
/// `ConversationItem` variants (intellihelper-build).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (IntelliHelper chat) — maps to System on harnesses without a
...
/// extraction (`<intellihelper_file id=".." name=".." />` lines in the
/// `<intellihelper_user_queries>` preamble).
...
///
/// Implementors:
/// - IntelliHelper chat: `IntelliHelperTurn`
/// - intellihelper-build: `ConversationItem`
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (IntelliHelper chat's
    /// `IntelliHelperTurn`) compute the flattened text on demand rather than storing a
    /// borrowable slice.
    ...
    /// Whether this item carries a *prior compaction summary* (IntelliHelper chat: a
    ...
    /// strips their `<intellihelper_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<intellihelper_file>` lines in the
    /// `<intellihelper_user_queries>` preamble. Empty for items without attachments.
    ///
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (IntelliHelper chat: a `Developer` turn with category
    ...
    /// requests/results (IntelliHelper chat: keep only `Channel` contents of an
    ...
    /// `max_tokens` (intellihelper-build style: `max_bytes = max_tokens * 4`, prefix
...
/// constructs the typed harness items that make up intellihelper-build's rebuilt
/// history.
///
/// This is a sibling of [`CompactionItemBuilder`], not a part of it, on
/// purpose. `CompactionItemBuilder` is already implemented by IntelliHelper chat's
/// `IntelliHelperTurn`; adding these constructors to it as required methods would break
/// that impl. They are also intellihelper-build-specific (IntelliHelper chat's tail-keep path
...
///
/// The intellihelper-build implementor (`ConversationItem`) maps each constructor to the
...
/// operate over `&[Arc<T>]` (IntelliHelper chat stores turns as `Arc<IntelliHelperTurn>`).
