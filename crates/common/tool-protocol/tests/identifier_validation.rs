...
fn tool_id_accepts_bare_and_namespaced_names() {
    ...
    let namespaced = ToolId::new("IntelliHelperBuild:read_file").unwrap();
    assert_eq!(namespaced.as_str(), "IntelliHelperBuild:read_file");
...
fn tool_id_try_from_string_works() {
    let id: ToolId = "IntelliHelperBuild:read_file".to_owned().try_into().unwrap();
    assert_eq!(id.as_str(), "IntelliHelperBuild:read_file");
}
...
fn server_id_synthesis_starts_with_auto_prefix() {
    ...
    assert_eq!(synth_ns.as_str(), "auto:tool:IntelliHelperBuild:read_file");
}

#[test]
fn server_id_synthesis_is_deterministic() {
    let conn = ConnectionId::new("conn-abc").unwrap();
    let tool = ToolId::new("IntelliHelperBuild:read_file").unwrap();
