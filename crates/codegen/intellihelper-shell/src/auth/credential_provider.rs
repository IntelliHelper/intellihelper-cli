...
use crate::util::intellihelper_auth_credentials::IntelliHelperAuthCredentials;
use reqwest::RequestBuilder;
use std::sync::Arc;
use intellihelper_auth::{
...
fn api_key_id_for(auth: Option<&crate::auth::IntelliHelperAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| intellihelper_telemetry::config::deployment_id_from_key(&a.key))
}
/// Sampler [`BearerResolver`](intellihelper_sampler::BearerResolver) over a live
...
impl WireValidBearerResolver {
    ...
    pub(crate) fn shared(auth_manager: Arc<AuthManager>) -> intellihelper_sampler::SharedBearerResolver {
...
impl intellihelper_sampler::BearerResolver for WireValidBearerResolver {
...
pub(crate) struct ShellAuthCredentialProvider {
    ...
    static_credentials: IntelliHelperAuthCredentials,
}
impl ShellAuthCredentialProvider {
    pub(crate) fn new(
        ...
        let mut static_credentials = IntelliHelperAuthCredentials::new(None);
...
pub(crate) fn embedding_session_credentials(
...
) -> intellihelper_memory::EndpointScopedCredentials {
    ...
    intellihelper_memory::EndpointScopedCredentials::for_endpoint(
        embed_base_url,
...
///
/// - `"intellihelper-shell"`   — classic IntelliHelper CLI / TUI (intellihelper-shell)
/// - `"intellihelper-pager"`   — new IntelliHelper Pager / TUI (intellihelper-pager)
/// - `"intellihelper-desktop"` — IntelliHelper Desktop app
/// - `"intellihelper-extension"` — VS Code / browser extension
///
/// The factory forwards both:
/// - `x-intellihelper-client-version`   (e.g. "0.1.210-alpha.5 (279ffacddb)")
/// - `x-intellihelper-client-identifier`
///
...
pub fn build_storage_client_for_proxy(
    ...
    if let Some(am) = auth_manager {
        ...
        .with_client_identity(intellihelper_version::VERSION, client_identifier)
        .with_client_mode(crate::http::process_client_mode())
        .with_attribution(bridge)
    } else {
        let mut creds = IntelliHelperAuthCredentials::new(user_token);
...
impl HttpAuth for OtelAuthCredentialProvider {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        ...
        let mut creds = IntelliHelperAuthCredentials::new(None);
...
pub(crate) fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        intellihelper_telemetry::external::set_identity(
            intellihelper_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `intellihelper-pager` and `intellihelper-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> intellihelper_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let grok_com_config = crate::auth::GrokComConfig::default();
    let exporter = intellihelper_telemetry::otel_layer::OtelExporterConfig {
    ...
    let bootstrap = Arc::new(AuthManager::new(&intellihelper_home, grok_com_config));
    let provider = Arc::new(OtelAuthCredentialProvider::new(bootstrap));
    let _ = OTEL_PROVIDER.set(provider.clone());
    intellihelper_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    use std::sync::Mutex;
    use intellihelper_auth::AuthCredentialProvider;
    /// Serializes tests that pin `INTELLIHELPER_AUTH_EARLY_INVALIDATION_SECS`, since
    /// env vars are process-global and parallel tests would race.
    static EARLY_INVALIDATION_LOCK: Mutex<()> = Mutex::new(());
    /// RAII guard: pins `INTELLIHELPER_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("INTELLIHELPER_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("INTELLIHELPER_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("INTELLIHELPER_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("INTELLIHELPER_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
    fn make_auth(key: &str, expires_in: ChronoDuration) -> IntelliHelperAuth {
        IntelliHelperAuth {
            key: key.to_string(),
    ...
    fn make_manager(dir: &tempfile::TempDir, initial: Option<IntelliHelperAuth>) -> Arc<AuthManager> {
    ...
    /// pinned in intellihelper-sampler's resolver tests): over a real
    ...
    fn wire_valid_resolver_tracks_manager_across_expiry_and_refresh() {
        use intellihelper_sampler::BearerResolver;
    ...
    async fn refresh_after_unauthorized_drives_recovery_state_machine() {
        ...
        impl crate::auth::refresh::TokenRefresher for OkRefresher {
            async fn refresh(
                ...
                crate::auth::refresh::RefreshOutcome::Success(Box::new(IntelliHelperAuth {
                    key: "fresh".into(),
                    auth_mode: crate::auth::AuthMode::Oidc,
    ...
    fn embedding_session_credentials_scopes_to_first_party() {
        ...
        let api_key_provider: intellihelper_tools::types::SharedApiKeyProvider =
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use intellihelper_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
    ...
    async fn otel_live_refresh_after_unauthorized_drives_recovery() {
        ...
        live_mgr.hot_swap(IntelliHelperAuth {
            key: "stale".into(),
            auth_mode: crate::auth::AuthMode::Oidc,
            create_time: chrono::Utc::now() - ChronoDuration::hours(2),
            user_id: "u".into(),
    ...
    /// user token, matching the precedence in `IntelliHelperAuthCredentials::apply`.
