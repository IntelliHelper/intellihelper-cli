...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] intellihelper_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `intellihelper-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `intellihelper_tools::ToolError`) happens
