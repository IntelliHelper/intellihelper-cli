...
//! live in `intellihelper_tools::implementations::skills::discovery`.

use std::collections::{HashMap, HashSet};
use std::path::{Path, PathBuf};

use crate::plugins::discovery::PluginScope;
use intellihelper_tools::implementations::skills::types::skill_name_from_path;
pub use intellihelper_tools::implementations::skills::types::{SkillInfo, SkillScope};
/// Re-export so agent-side discovery (and the shell) can name the resolved
/// vendor-compat config without reaching into `intellihelper_tools` directly.
pub use intellihelper_tools::types::compat::CompatConfig;

use intellihelper_tools::implementations::skills::discovery::{
...
///
/// Priority order: Local (cwd/.intellihelper/skills, cwd/.agents/skills, cwd/.claude/skills) → Intermediate dirs →
/// Repo (repo_root/.intellihelper/skills, repo_root/.agents/skills, repo_root/.claude/skills) → User (~/.intellihelper/skills, ~/.agents/skills, ~/.claude/skills)
/// → additional paths from `config.paths`
/// → Server (injected `config.server_skill_dirs`)
/// → Bundled (injected `config.bundled_skill_dirs` + `~/.intellihelper/bundled`; lowest precedence).
///
...
/// (e.g. `Repo` for `.intellihelper/plugins/`). Native skills always win bare-name
...
pub async fn list_skills_with_plugins(
    ...
    let mut skills = list_skills_with_options(
        ...
        &intellihelper_tools::util::intellihelper_home::intellihelper_home(),
        compat,
    )
    .await;
...
pub fn collect_skill_config_dirs(
    ...
    let intellihelper_home = global_dir.to_path_buf();
    ...
    // config; `.intellihelper` and `.agents` are always present. When all cells are on
    // this list equals the historical `[".intellihelper", ".agents", ".claude", ".cursor"]`.
...
fn scope_for_config_dir(dir: &Path, cwd: Option<&Path>, git_root: Option<&Path>) -> SkillScope {
    // Home-level dirs (e.g. ~/.intellihelper/, ~/.agents/, ~/.claude/) are User scope.
    #[allow(deprecated)]
    if let Some(home) = std::env::home_dir()
...
/// only visits known config roots (`.intellihelper`, `.agents`, `.claude`, `.cursor`),
...
fn collect_config_skills(config_paths: &[String], git_root: Option<&Path>) -> Vec<SkillInfo> {
    ...
    for skill in &mut skills {
        skill.config_source = Some(
            intellihelper_tools::types::config_source::ConfigSource::ConfigToml {
...
fn stamp_plugin_fields(skills: &mut [SkillInfo], plugin: &crate::plugins::LoadedPlugin) {
    ...
    for skill in skills.iter_mut() {
        ...
        skill.config_source = Some(intellihelper_tools::types::config_source::ConfigSource::Plugin {
...
pub(crate) fn format_skill_for_injection(skill: &SkillInfo) -> Option<String> {
    skill.body.as_ref().filter(|b| !b.is_empty()).map(|body| {
        intellihelper_tools::implementations::skills::skill::build_skill_message(skill, body)
    })
}
    ...
    for name in names {
        ...
        let skill = discovered.iter().find(|s| {
            s.name.eq_ignore_ascii_case(name)
                || intellihelper_tools::implementations::skills::skill::format_skill_name(s)
        ...
        match intellihelper_tools::implementations::skills::skill::load_skill_with_body(skill).await {
...
mod tests {
    ...
    async fn server_skills_discovered_and_shadowed_by_local() {
        ...
        write_skill_md(&cwd.path().join(".intellihelper").join("skills").join("dup"), "dup");
    ...
    fn find_skill_paths_flat_layout() {
        ...
        write_skill_md(&intellihelper_dir.join("skills").join("alpha"), "alpha");
        write_skill_md(&intellihelper_dir.join("skills").join("beta"), "beta");

        let paths = find_skill_paths(&intellihelper_dir);
    ...
    fn find_skill_paths_nested_layout() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let intellihelper_dir = tmp.path().join(".intellihelper");
        let skills = intellihelper_dir.join("skills");
    ...
    fn find_skill_paths_no_skills_dir() {
        // .intellihelper exists but no skills/ subdirectory
        let tmp = tempfile::tempdir().unwrap();
        let intellihelper_dir = tmp.path().join(".intellihelper");
        fs::create_dir_all(&intellihelper_dir).unwrap();
    ...
    fn parse_model_and_effort() {
        ...
        let parsed = parse_skill_frontmatter(content, None).unwrap();
        assert_eq!(parsed.model.as_deref(), Some("intellihelper-3"));
    ...
    async fn list_skills_includes_workspace_user_skills() {
        ...
        write_skill_md(
            &user_dir.join(".intellihelper").join("skills").join("my-tool"),
            "my-tool",
        );
    ...
    async fn list_skills_workspace_user_dedup_when_cwd_inside_user_dir() {
        ...
        let user_dir = repo_root.join("x").join("testuser");
        write_skill_md(
            &user_dir.join(".intellihelper").join("skills").join("dedup-skill"),
            "dedup-skill",
        );
    ...
    async fn list_skills_no_workspace_user_dir_no_extra_skills() {
        ...
        write_skill_md(
            &user_dir.join(".intellihelper").join("skills").join("ghost-skill"),
            "ghost-skill",
        );
    ...
    async fn list_skills_workspace_user_dir_with_recursive_skills() {
        ...
        let skills_base = user_dir.join(".intellihelper").join("skills");
    ...
    fn collect_config_skills_stamps_config_toml_source() {
        ...
        match &skills[0].config_source {
            Some(intellihelper_tools::types::config_source::ConfigSource::ConfigToml { path }) => {
    ...
    async fn list_skills_deduplicates_auto_and_config_overlap() {
        ...
        let auto_dir = repo_root.join(".intellihelper").join("skills").join("dup-skill");
    ...
    async fn list_skills_auto_and_config_overlap_keeps_config_toml_source() {
        ...
        let auto_dir = repo_root.join(".intellihelper").join("skills").join("overlap-skill");
        ...
        assert!(
            matches!(
                ...
                Some(intellihelper_tools::types::config_source::ConfigSource::ConfigToml { .. })
            ),
    ...
    async fn list_skills_ignore_allows_lower_priority_same_name_fallback() {
        ...
        write_skill_md(&cwd.join(".intellihelper").join("skills").join("same"), "same");
        let repo_skill_dir = repo_root.join(".intellihelper").join("skills").join("same");
    ...
    // these functions now live in intellihelper-tools::implementations::skills::discovery
    // and intellihelper-tools::types::skill_discovery_tracker, tested there.
    ...
    async fn disabled_config_marks_skill_enabled_false() {
        ...
        write_skill_md(
            &repo_root.join(".intellihelper").join("skills").join("commit"),
            "commit",
        );
        write_skill_md(
            &repo_root.join(".intellihelper").join("skills").join("review"),
            "review",
        );
    ...
    async fn disabled_config_empty_leaves_all_enabled() {
        ...
        write_skill_md(
            &repo_root.join(".intellihelper").join("skills").join("deploy"),
            "deploy",
        );
    ...
    fn collect_skill_config_dirs_gates_vendor_dirs() {
        ...
        for name in [".intellihelper", ".agents", ".claude", ".cursor"] {
        ...
        assert!(ends_with(&dirs, ".intellihelper"), "intellihelper must remain: {dirs:?}");
    }
    ...
    fn dedupe_same_scope_cross_harness_loser_resurfaces() {
        // A `.claude` skill claiming a `.intellihelper`-owned name (both User scope)
        // was silently hidden before; it now re-keys to its dir basename.
        let out = dedupe_skills(vec![
            named_skill(
                "review",
                "/u/.intellihelper/skills/review/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "review",
    ...
    fn dedupe_frontmatter_owner_evicts_rekeyed_squatter() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/u/.intellihelper/skills/japandi/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "japandi",
                "/u/.intellihelper/skills/japandi2/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "japandi2",
    ...
    fn dedupe_cross_scope_shadowing_unchanged() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/repo/.intellihelper/skills/japandi/SKILL.md",
                SkillScope::Repo,
            ),
            named_skill("japandi", "/u/skills/japandi2/SKILL.md", SkillScope::User),
        ]);
    ...
    fn dedupe_same_scope_same_basename_still_drops() {
        ...
        // (e.g. ~/.intellihelper/skills and ~/.agents/skills): first-seen wins.
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/u/.intellihelper/skills/japandi/SKILL.md",
        ...
        assert_eq!(out.len(), 1);
        assert!(out[0].path.contains(".intellihelper"));
    }
    ...
    async fn copied_skill_dir_with_stale_frontmatter_name_surfaces_both() {
        ...
        // scans intellihelper_home).
        let tmp = tempfile::tempdir().unwrap();
        let repo_root = tmp.path().join("repo");
        fs::create_dir_all(&repo_root).unwrap();
        init_git_repo(&repo_root);

        let skills_dir = repo_root.join(".intellihelper").join("skills");
