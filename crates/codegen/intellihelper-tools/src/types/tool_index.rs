...
//!
//! `ToolSearchIndex` is defined in `intellihelper-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `intellihelper-shell`
...
/// in `intellihelper-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `intellihelper-shell` after MCP initialization.
#[derive(Clone)]
