...
use crate::auth::{IntelliHelperAuth, GrokComConfig};
...
const INTELLIHELPER_CODE_BACKEND_URL: &str = "https://code.grok.com";
const DEFAULT_TIMEOUT: Duration = Duration::from_secs(30);
const INTELLIHELPER_CODE_WEB_URL: &str = "https://grok.com";
/// Build a share URL from a permission ID
pub fn share_url(permission_id: &str) -> String {
    let web_url =
        std::env::var("INTELLIHELPER_CODE_WEB_URL").unwrap_or_else(|_| INTELLIHELPER_CODE_WEB_URL.to_string());
...
fn add_cli_chat_proxy_headers_blocking(
    ...
    let mut builder = builder
        ...
        .header("x-intellihelper-client-version", intellihelper_version::VERSION);
...
async fn add_bundle_fetch_headers(
    ...
    let mut credentials = crate::util::intellihelper_auth_credentials::IntelliHelperAuthCredentials::new(
...
async fn fetch_bundle_inner(
    ...
    let client: reqwest_middleware::ClientWithMiddleware = if let Some(am) = auth_manager {
        let provider: std::sync::Arc<dyn intellihelper_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
...
impl BackendClient {
    fn build_default_client() -> reqwest::Client {
        intellihelper_extra_ca::with_extra_root_certificates(
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            reqwest_client,
            base_url: std::env::var("INTELLIHELPER_CODE_BACKEND_URL")
                .unwrap_or_else(|_| INTELLIHELPER_CODE_BACKEND_URL.to_string()),
            auth_manager: None,
        }
    }
    ...
    /// instead of requiring the caller to pass `&IntelliHelperAuth`.
    pub(crate) fn with_auth_manager(
        mut self,
        manager: std::sync::Arc<crate::auth::AuthManager>,
    ) -> Self {
        let credentials: std::sync::Arc<dyn intellihelper_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
                crate::auth::credential_provider::ShellAuthCredentialProvider::new(
                    manager.clone(),
                    None,
    ...
    async fn resolve_auth(&self) -> Result<IntelliHelperAuth, BackendError> {
        let manager = self
            .auth_manager
            .as_ref()
    ...
    /// Must include X-XAI-Token-Auth so nginx auth subrequest routes to authenticate_intellihelper_cli_token.
    /// See: crates/codegen/intellihelper-shell/src/agent/app.rs:run_headless
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        if let Ok(v) = HeaderValue::from_str(&crate::http::process_client_identifier()) {
            headers.insert("x-intellihelper-client-identifier", v);
        }
        headers.insert(
            crate::http::CLIENT_MODE_HEADER,
        ...
        headers.insert(
            "x-intellihelper-client-version",
            HeaderValue::from_static(intellihelper_version::VERSION),
        );
        Ok(headers)
    }
...
/// Fetch `intellihelper_build_login_device_flow` from cli-chat-proxy `GET /v1/login-config`.
///
/// Unauthenticated (pre-login); `x-intellihelper-agent-id` is the per-install bucketing key.
...
pub async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(intellihelper_telemetry::id::agent_id)
        .await
        .ok()?;
    ...
    let response = client
        ...
        .header("x-intellihelper-agent-id", agent_id)
        .header("x-intellihelper-client-version", intellihelper_version::VERSION)
        .header(
            "x-intellihelper-client-identifier",
            crate::http::process_client_identifier(),
...
pub(crate) fn fetch_models_blocking(
    ...
    match source.auth {
        ...
        EndpointAuth::Session => {
            ...
            request = request
                .header("Authorization", format!("Bearer {}", &auth.key))
                .header("X-XAI-Token-Auth", "intellihelper-cli")
                .header("x-userid", &auth.user_id)
                .header("x-intellihelper-client-version", intellihelper_version::VERSION)
                .header(
                    crate::http::CLIENT_MODE_HEADER,
...
pub(crate) fn parse_remote_model_value(
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        reasoning_efforts: obj
            ...
            .map(|arr| intellihelper_sampling_types::parse_reasoning_effort_options(arr))
            .unwrap_or_default(),
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(intellihelper_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
fn parse_compaction_at_tokens(
    v: &serde_json::Value,
) -> Option<intellihelper_sampling_types::CompactionAtTokens> {
...
fn parse_compactions_remaining(
    v: &serde_json::Value,
) -> Option<intellihelper_sampling_types::CompactionsRemaining> {
...
mod tests {
                ...
                get(
                    |State(state): State<LoginConfigServerState>, headers: HeaderMap| async move {
                        state.seen.lock().unwrap().push(LoginConfigHeaders {
                            ...
                            agent_id: header_str(&headers, "x-intellihelper-agent-id"),
                            client_identifier: header_str(&headers, "x-intellihelper-client-identifier"),
                            client_version: header_str(&headers, "x-intellihelper-client-version"),
                        });
    ...
    async fn fetch_login_device_flow_sends_only_unauthenticated_headers() {
        ...
        assert!(
            ...
            "must send x-intellihelper-agent-id (the bucketing key)"
        );
        assert!(
            h.client_identifier.is_some(),
            "must send x-intellihelper-client-identifier"
        );
        assert!(
            h.client_version.is_some(),
            "must send x-intellihelper-client-version"
        );
    ...
    async fn settings_fetch_maps_status_to_outcome() {
        let auth = IntelliHelperAuth::test_default();
                    ...
                    |State(state): State<BundleServerState>, headers: HeaderMap| async move {
                        state.seen_headers.lock().unwrap().push(SeenHeaders {
                            ...
                            client_version: headers
                                .get("x-intellihelper-client-version")
                                .and_then(|v| v.to_str().ok())
                                .map(str::to_owned),
                        });
    ...
    fn test_auth() -> IntelliHelperAuth {
        IntelliHelperAuth {
            key: "token".to_string(),
    ...
    async fn fetch_subagent_bundle_success() {
        ...
        assert_eq!(headers.token_auth.as_deref(), Some("intellihelper-cli"));
    ...
    fn parse_openai_format_uses_id_field() {
        ...
        assert_eq!(result.model, "intellihelper-3");
        assert_eq!(result.base_url, "https://api.x.ai/v1");
        assert_eq!(result.name.as_deref(), Some("intellihelper-3"));
    }
    #[test]
    ...
    /// REGRESSION: `intellihelper setup` must send the deployment key to
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        unsafe { std::env::set_var("INTELLIHELPER_DEPLOYMENT_KEY", "xai-token-ENTERPRISE") };
        ...
        unsafe { std::env::remove_var("INTELLIHELPER_DEPLOYMENT_KEY") };
    }
    #[derive(Clone)]
