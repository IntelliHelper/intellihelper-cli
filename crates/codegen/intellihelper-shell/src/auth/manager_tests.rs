...
fn make_auth(expires_at: Option<DateTime<Utc>>, create_time: DateTime<Utc>) -> IntelliHelperAuth {
    IntelliHelperAuth {
...
fn legacy_scope_fallback_reads_old_auth_json() {
    ...
    // a machine that was authenticated with an older intellihelper version).
...
fn new_scope_takes_precedence_over_legacy() {
    ...
    let legacy_auth = IntelliHelperAuth {
        key: "legacy-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
    let new_auth = IntelliHelperAuth {
        key: "new-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
...
fn near_expiry_token_invisible_to_current_visible_to_expired_auth() {
    ...
    let near_expiry = IntelliHelperAuth {
...
async fn update_preserves_other_scope_entries() {
    ...
    let external = IntelliHelperAuth {
...
async fn update_preserves_team_fields_when_proxy_omits_them() {
    ...
    let team_auth = IntelliHelperAuth {
        key: "team-token".into(),
        auth_mode: AuthMode::Oidc,
...
async fn team_login_then_personal_evicts_team_token() {
    ...
    let personal_auth = IntelliHelperAuth {
...
fn clear_does_not_remove_legacy_scope() {
    ...
    let oauth_auth = IntelliHelperAuth {
        key: "oauth-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
...
fn manager_collection_predicates_fail_directions() {
    ...
    mgr.hot_swap(IntelliHelperAuth::test_default());
    assert!(!mgr.is_data_collection_disabled());
...
fn try_use_disk_token_accepts_different_key_on_server_rejected() {
    ...
    let mem_auth = IntelliHelperAuth {
        key: "old-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
    mgr.hot_swap(mem_auth);

    let disk_auth = IntelliHelperAuth {
        key: "new-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
...
async fn disk_refresh_wins_over_expired_in_memory() {
    ...
    let fresh_disk = IntelliHelperAuth {
...
async fn auth_returns_expired_api_key_consistently_with_current() {
    ...
    let expired_key = IntelliHelperAuth {
    ...
    // funnel the user back through `intellihelper login`.
    let err = mgr.auth().await.unwrap_err();
    assert!(
    ...
    let fresh_key = IntelliHelperAuth {
...
async fn start_proactive_refresh_is_idempotent() {
    ...
    let stale_api_key = IntelliHelperAuth {
        key: "stale-api-key".into(),
        auth_mode: AuthMode::ApiKey,
        create_time: Utc::now() - Duration::days(60),
        expires_at: None,
...
async fn refresh_chain_demotes_when_attributed_tried_rt_differs_from_disk() {
    ...
    struct AttributedRejection(IntelliHelperAuth);
    #[async_trait::async_trait]
    ...
    assert!(
        ...
        "demotion must not record a sticky verdict that locks out every \
         sibling process until the user re-runs `intellihelper login`",
    );
}
...
async fn permanent_failure_reads_absent_after_clear_so_auth_reports_not_logged_in() {
    ...
    // User runs `intellihelper logout` which calls clear().
    mgr.clear().unwrap();

    // The diagnostic the user now sees on the next request should be
    // "Not logged in. Run `intellihelper login`.", not the stale invalid_grant.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
async fn oidc_refresh_not_blocked_by_model_api_key() {
    ...
    let expired_oidc = IntelliHelperAuth {
...
async fn auth_returns_cached_token_when_refresh_fails_within_real_expiry() {
    ...
    let in_buffer = IntelliHelperAuth {
...
async fn enrichment_task_preserves_interleaved_token_rotation() {
    ...
    let auth_v1 = IntelliHelperAuth {
        key: "key-v1".into(),
        refresh_token: Some("rt-v1".into()),
        user_id: "stable-user".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
    let auth_v2 = IntelliHelperAuth {
        key: "key-v2".into(),
...
async fn enrichment_aborts_when_disk_user_changes_mid_flight() {
    ...
    tokio::time::sleep(std::time::Duration::from_millis(50)).await;
    let intruder = IntelliHelperAuth {
...
async fn enrichment_overlays_team_login_placeholder_user_id() {
    ...
    let team_login = IntelliHelperAuth {
        key: "team-key".into(),
...
/// `has_intellihelper_code_access`. The `&mut IntelliHelperAuth` signature already
...
/// contributor adding a token-like field to both `IntelliHelperAuth` and
/// `UserInfo` is forced to look here.
#[test]
fn apply_user_info_enrichment_preserves_token_fields() {
    let mut disk = IntelliHelperAuth {
        key: "ROT_KEY".into(),
        ...
        has_intellihelper_code_access: Some(true),
    ...
    assert_eq!(disk.has_intellihelper_code_access, snapshot.has_intellihelper_code_access);
...
async fn current_api_key_async_drives_refresh_chain() {
    ...
    use intellihelper_tools::types::ApiKeyProvider;

    let _xai = EnvGuard::unset("XAI_API_KEY");
    let _legacy = EnvGuard::unset("INTELLIHELPER_CODE_XAI_API_KEY");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), GrokComConfig::default()));
    mgr.hot_swap(IntelliHelperAuth {
...
async fn sibling_different_rt_with_expired_at_is_still_sibling() {
    ...
    let original = IntelliHelperAuth {
    ...
    let successor = IntelliHelperAuth {
...
async fn refresh_chain_server_rejected_bypasses_valid_token_double_check() {
    ...
    let valid_but_rejected = IntelliHelperAuth {
...
async fn refresh_chain_server_rejected_concurrent_skips_redundant_refresh() {
    ...
    let rejected = IntelliHelperAuth {
...
async fn enrich_auth_inline_populates_zdr_flags() {
    ...
    let mut auth = IntelliHelperAuth {
        key: "tok".into(),
        ..IntelliHelperAuth::test_default()
    };
...
fn oidc_session_for_team(principal_id: &str) -> IntelliHelperAuth {
    IntelliHelperAuth {
...
async fn cached_api_key_session_rejected_when_api_key_auth_disabled() {
    let api_key_session = || IntelliHelperAuth {
...
fn expired_oidc() -> IntelliHelperAuth {
    IntelliHelperAuth {
        key: "expired-key".into(),
        auth_mode: AuthMode::Oidc,
        refresh_token: Some("rt-old".into()),
...
#[test]
#[serial_test::serial(force_dark_wake_env)] // reads INTELLIHELPER_AUTH_FORCE_DARK_WAKE
fn is_dark_wake_false_when_power_listener_not_started() {
    let _unset = intellihelper_test_support::EnvGuard::unset("INTELLIHELPER_AUTH_FORCE_DARK_WAKE");
    let dir = tempfile::tempdir().unwrap();
    let mgr = AuthManager::new(dir.path(), GrokComConfig::default());
    assert!(
        !mgr.is_dark_wake(),
...
/// `INTELLIHELPER_AUTH_FORCE_DARK_WAKE` forces the dark-wake answer for manual and
...
fn is_dark_wake_env_override_forces_both_states() {
    ...
    {
        let _g = EnvGuard::set("INTELLIHELPER_AUTH_FORCE_DARK_WAKE", "1");
        assert!(
            mgr.is_dark_wake(),
    ...
    {
        let _g = EnvGuard::set("INTELLIHELPER_AUTH_FORCE_DARK_WAKE", "0");
    ...
    }
    {
        ...
        let _g = EnvGuard::set("INTELLIHELPER_AUTH_FORCE_DARK_WAKE", "yes");
...
fn manual_auth_reason_maps_terminal_and_skips_non_forcing() {
    ...
    use intellihelper_telemetry::events::ManualAuthReason as R;
...
async fn manual_auth_capture_attributes_and_recorder_debounces() {
    ...
    use intellihelper_telemetry::events::{AuthTokenKind, ManualAuthSurface};
...
async fn manual_auth_emits_only_for_user_facing_source() {
    ...
    use intellihelper_telemetry::events::{
