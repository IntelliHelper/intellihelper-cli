...
//! its own `OnceLock<INTELLIHELPER_HOME>`. The helpers below ensure the per-binary
//! initialization is identical: same env-var set, same isolation guarantees,
//! same reset between tests.
//!
//! Mirrors the INTELLIHELPER_HOME isolation pattern used in other integration tests.
//!
//! ## Usage
//!
//! ```ignore
...
//! #[serial_test::serial]
//! async fn my_test() {
//!     let _ = test_home();   // initializes INTELLIHELPER_HOME once per binary
...
// ─────────────────────────────────────────────────────────────────────────────
// INTELLIHELPER_HOME isolation
// ─────────────────────────────────────────────────────────────────────────────

/// Returns a process-wide test `INTELLIHELPER_HOME`, initialized exactly once per test
/// binary. Once initialized, `intellihelper_config::intellihelper_home()` will resolve to
...
pub fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe {
            std::env::set_var("INTELLIHELPER_HOME", &path);
            std::env::remove_var("INTELLIHELPER_TEST_VERSION");
            std::env::remove_var("NPM_TOKEN");
            std::env::remove_var("INTELLIHELPER_INSTALLER");
            std::env::remove_var("INTELLIHELPER_MANAGED_BY_NPM");
            std::env::remove_var("INTELLIHELPER_MANAGED_BY_INTERNAL");
        }
        path
    })
}

/// Wipe state in `INTELLIHELPER_HOME` between tests so each test sees a clean home.
...
/// Override the version reported by `get_installed_intellihelper_version()` for the
/// duration of the test (until [`reset_home`] or process exit).
pub fn set_test_version(v: &str) {
    // SAFETY: tests using this helper must be `#[serial]`.
    unsafe { std::env::set_var("INTELLIHELPER_TEST_VERSION", v) };
}
...
/// (`intellihelper-{version}-{platform}`).
...
/// Minimal [`intellihelper_update::UpdateConfig`] for install tests.
pub fn make_update_config(channel: &str) -> intellihelper_update::UpdateConfig {
    intellihelper_update::UpdateConfig {
...
/// Backdate every file in `INTELLIHELPER_HOME/downloads` by ~2 hours.
///
