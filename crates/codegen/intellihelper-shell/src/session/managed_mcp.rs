...
//! `intellihelper_shell_session_support::managed_mcp` and is re-exported here so
//! `crate::session::managed_mcp::…` paths keep resolving unchanged.

pub use intellihelper_shell_session_support::managed_mcp::*;
...
pub(crate) fn merge_managed_mcp_servers_with_policy(
    ...
    let allowlist = &intellihelper_workspace::permission::resolution::managed_settings().mcp_allowlist;
...
impl McpDisabledReason {
    ...
    pub(crate) fn for_blocked_server(
        policy: &intellihelper_workspace::permission::resolution::McpServerAllowlist,
...
fn apply_mcp_server_policy(
    ...
    allowlist: &intellihelper_workspace::permission::resolution::McpServerAllowlist,
) -> Vec<McpServerWithPolicy> {
    merged
        .into_iter()
        .filter_map(|server| {
...
pub(crate) fn merge_managed_mcp_servers_sourced(
    ...
    let config_source = ConfigSource::ConfigToml {
        path: intellihelper_tools::util::intellihelper_home::intellihelper_home().join("config.toml"),
    };
...
pub(crate) struct McpDiscoveryInputs<'a> {
    ...
    pub plugin_registry: Option<&'a intellihelper_agent::plugins::PluginRegistry>,
    pub compat: &'a intellihelper_tools::types::compat::CompatConfig,
}
...
fn load_plugin_mcp_servers_from_value(
    ...
    let normalized = intellihelper_agent::plugins::manifest::normalize_inline_mcp_servers(root);
...
fn load_plugin_mcp_servers_from_config(
    ...
    let sub = |s: &str| -> String {
        let s = intellihelper_agent::plugins::manifest::substitute_env_vars(s, plugin_root, plugin_data);
...
mod tests {
    ...
    fn toml_claim_survives_when_client_cursor_insert_skipped() {
        let cwd = empty_cwd();
        write_cursor_project_mcp(cwd.path(), "killswitch-cache");
        std::fs::create_dir_all(cwd.path().join(".intellihelper")).unwrap();
        std::fs::write(
            cwd.path().join(".intellihelper").join("config.toml"),
            r#"
...
"#,
    ...
    fn merge_drops_denied_server_and_classifies_as_denylist() {
        use intellihelper_workspace::permission::resolution::{AllowedMcpServer, McpServerAllowlist};
...
"#,
    ...
    fn load_plugin_mcp_creates_stdio_server_with_env_substitution() {
        let config: crate::util::config::McpConfig = serde_json::from_value(serde_json::json!({
            "mcpServers": {
                "echo-mcp": {
                    "command": "python3",
                    "args": ["${INTELLIHELPER_PLUGIN_ROOT}/mcp-echo-server.py"]
                }
            }
        }))
        ...
        let (servers, _) = load_plugin_mcp_servers_from_config(
            &config,
            "team-tool",
            "/home/user/.intellihelper/plugins/team-tool",
            "/home/user/.intellihelper/plugin-data/team-tool",
        );
        ...
        match &servers[0] {
            acp::McpServer::Stdio(acp::McpServerStdio {
                ...
                assert_eq!(
                    args.as_slice(),
                    &["/home/user/.intellihelper/plugins/team-tool/mcp-echo-server.py"]
                );
            }
    ...
    fn plugin_server_deduped_across_file_and_inline() {
        use intellihelper_agent::plugins::PluginRegistry;
        use intellihelper_agent::plugins::PluginScope;
        use intellihelper_agent::plugins::discovery::{DiscoveredPlugin, PluginId};
        use intellihelper_agent::plugins::manifest::{PathOrInline, PluginManifest};
        ...
        let dp = DiscoveredPlugin {
            ...
            origin: intellihelper_agent::plugins::PluginOrigin::UserGrok,
            trusted: true,
            skill_dirs: vec![],
            command_dirs: vec![],
            agent_dirs: vec![],
    ...
    fn collect_plugin_oauth_configs_reads_byo_client_id_from_mcp_json() {
        ...
        use intellihelper_agent::plugins::PluginScope;
        use intellihelper_agent::plugins::discovery::{DiscoveredPlugin, PluginId};
        use intellihelper_agent::plugins::manifest::PluginManifest;
