...
/// [`intellihelper_config::env_bool`] parser (`INTELLIHELPER_TOOLS_FIND_BFS` /
/// `INTELLIHELPER_TOOLS_GREP_UGREP`, plus the `INTELLIHELPER_FIND_BFS` / `INTELLIHELPER_GREP_UGREP` aliases).
///
/// Pass the **merged** requirements ([`crate::config::load_merged_requirements`])
/// so an org policy in any requirements layer — not only
/// `~/.intellihelper/requirements.toml` — is honored. Returns `(find_bfs, grep_ugrep)`,
/// which the caller bakes into a
/// [`intellihelper_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub(crate) fn resolve_search_tools_enabled(
    ...
    let disable = intellihelper_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = intellihelper_config::env_bool(primary).or_else(|| intellihelper_config::env_bool(alias));
    ...
    (
        resolve("INTELLIHELPER_TOOLS_FIND_BFS", "INTELLIHELPER_FIND_BFS", "find_bfs"),
        resolve("INTELLIHELPER_TOOLS_GREP_UGREP", "INTELLIHELPER_GREP_UGREP", "grep_ugrep"),
    )
}
...
) -> Option<intellihelper_tools::util::ShellEnvironmentPolicy> {
    let value = effective_cfg?.get("shell_environment_policy")?.clone();
    match value.try_into::<intellihelper_tools::util::ShellEnvironmentPolicy>() {
...
const ENV_LOGIN_SHELL_CAPTURE: &str = "INTELLIHELPER_LOGIN_ENV";
...
const ENV_SCHEDULER_BACKGROUND_LOOPS: &str = "INTELLIHELPER_SCHEDULER_BACKGROUND_LOOPS";
...
///
/// Precedence: requirements > env (`INTELLIHELPER_SCHEDULER_BACKGROUND_LOOPS`) > user
...
/// the shared [`intellihelper_config::env_bool`] via `BoolFlag`). The secs env var
/// lives in the tools crate (`RESPONSE_TIMEOUT_ENV`), parsed once there.
const ENV_ASK_USER_QUESTION_TIMEOUT_ENABLED: &str = "INTELLIHELPER_ASK_USER_QUESTION_TIMEOUT_ENABLED";
...
///
/// Precedence: requirements > env (`INTELLIHELPER_ASK_USER_QUESTION_TIMEOUT_ENABLED`)
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            intellihelper_tools::implementations::intellihelper_build::ask_user_question::RESPONSE_TIMEOUT
                .as_secs(),
        )
}
...
///
/// Precedence: requirements > env (`INTELLIHELPER_ASK_USER_QUESTION_TIMEOUT_SECS`,
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        intellihelper_tools::implementations::intellihelper_build::ask_user_question::response_timeout_env_secs(),
...
) -> intellihelper_tools::implementations::intellihelper_build::ask_user_question::AskUserQuestionParams {
    let requirements = crate::config::load_merged_requirements();
    let layers = match crate::config::ConfigLayers::load() {
        Ok(l) => Some(l),
        Err(e) => {
...
mod ask_user_question_timeout_tests {
    ...
    use intellihelper_tools::implementations::intellihelper_build::ask_user_question::RESPONSE_TIMEOUT_ENV;
    ...
    fn timeout_secs_tier_precedence() {
        let d = intellihelper_tools::implementations::intellihelper_build::ask_user_question::RESPONSE_TIMEOUT
            .as_secs();
...
mod tests {
    ...
    // Assumes INTELLIHELPER_TOOLS_* / DISABLE_EMBEDDED_SEARCH_TOOLS are unset in the test env.
    #[test]
...
mod shell_env_policy_tests {
    use super::*;
    use intellihelper_tools::util::{EnvironmentVariablePattern, ShellEnvironmentPolicyInherit};
