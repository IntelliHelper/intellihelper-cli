...
//!
//! Plugins from project directories (`.intellihelper/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.intellihelper/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.intellihelper/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.intellihelper/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_intellihelper_home() so a project's `.intellihelper/trusted-plugins` is never
        // read as the user trust store when neither INTELLIHELPER_HOME nor a home dir resolves.
        let Some(intellihelper) = intellihelper_config::user_intellihelper_home() else {
            return Self {
                trusted: HashSet::new(),
                file_path: PathBuf::new(),
            };
        };
        let file_path = intellihelper.join(TRUST_FILE_NAME);
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.intellihelper/trusted-plugins`.
    ...
    /// rewrites `~/.intellihelper/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.intellihelper/trusted-plugins`.
