...
Commands come from two places: **shell builtins**, handled by the agent backend (intellihelper-shell), and **pager builtins**, handled by the pager frontend (intellihelper-pager). Both show up in the same menu, and any enabled skill with `user-invocable: true` appears there too.
...
Not `/config-agents` (alias `/agents`), which manages agent *definitions* and personas. Hidden in minimal mode; disable with `INTELLIHELPER_AGENT_DASHBOARD=0` or `[dashboard].enabled = false`.

### `/compact [context]`

Compress conversation history to reclaim context-window space. Pass a note to tell IntelliHelper what to keep:
...
IntelliHelper also auto-compacts once the context window hits 85% (tune it with `[session] auto_compact_threshold_percent`).
...
In minimal mode, open an external editor for an empty composer. IntelliHelper resolves `$VISUAL`, then `$EDITOR`, then `vi`; command values may include quoted arguments. Saving replaces the draft without sending it, and saving an empty file clears it. The command is hidden outside minimal mode.
...
Every copy is also written to a backup file — `~/.intellihelper/last-copy.txt` by default, or `INTELLIHELPER_COPY_FILE` if set. Confirmed copies toast briefly (e.g. `Copied!`). Unverified OSC 52 deliveries and clipboard-unreachable fallbacks name the backup path so you can recover the text.
...
`/flush`, `/dream`, and `/memory` require memory to be enabled (`--experimental-memory` or `INTELLIHELPER_MEMORY=1`); `/memory` also needs a configured memory backend. `/remember` is always available.
...
Run a prompt on a recurring interval. Give the interval as `30m`, `1 hour`, or `every 2 days`; leave it out and IntelliHelper will ask.
...
Set, manage, or check an autonomous goal. IntelliHelper works across rounds and only marks the goal complete after an independent evidence review confirms the claim; if that review can't reproduce the result or has no usable evidence, the goal stays active or pauses with concrete gaps.
...
Project workflows live in `.intellihelper/workflows/*.rhai`; user workflows live in `~/.intellihelper/workflows/*.rhai`. A same-process pause/resume continues the original immutable script, args, and `agent_budget` cap from committed host-call results — to iterate, edit the returned script copy and launch it as a new run.
...
Show durable token usage for **day**, **week**, and **month** (local calendar windows), with **totals** and a **per-model** breakdown. Works from the welcome screen and inside a session. Alias: `/tokens`.

Windows use the machine’s local timezone: day is today from midnight, week is Monday 00:00 through now, month is the 1st of the month through now. Counts come from a local ledger under `~/.intellihelper/token-usage.jsonl` (or `$INTELLIHELPER_HOME`), updated on each model call.

The top-right status bar also shows **session** cumulative spend (`sess …`) next to the context-window fill (`used / total`).
...
Rewrite the current composer draft into clearer **natural-language** prose for a coding agent (async). Style is Qoder/Roo/Cline-like: a short multi-sentence expansion of *your* ask — not a Goal/Requirements template and not a forced “summarize git status” rewrite.

Workspace context is **identity-first** (cwd, branch, package). Full git status/diff stats are attached only when the draft is about branch/changes/PR/review so the model can ground those asks without hijacking unrelated prompts. Does **not** send a turn — it only replaces the draft. Alias: `/optimize`.

After a successful rewrite you can restore the previous draft with **Undo optimize** (`Ctrl+Shift+Z`).

Also available as:
- **✦** sparkle icon on the prompt info line (next to the model · mode flags) — click to optimize
- **Ctrl+Shift+P** while the prompt is focused
- **Ctrl+Shift+Z** — undo the last optimize
...
Any enabled skill with `user-invocable: true` in its SKILL.md frontmatter shows up as a slash command. (Turn a skill off via `/skills` and it stops being advertised.) So a skill at `~/.intellihelper/skills/commit/SKILL.md` runs as:
