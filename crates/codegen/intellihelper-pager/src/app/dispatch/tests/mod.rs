...
fn test_app() -> AppView {
    ...
    AppView {
        ...
        auth_methods: vec![acp::AuthMethod::Agent(acp::AuthMethodAgent::new(
            acp::AuthMethodId::new("intellihelper.com"),
...
/// Plant a Build session under the process `intellihelper_home()` (OnceLock-cached;
/// do not rely on setting `INTELLIHELPER_HOME` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    let home = intellihelper_shell::util::intellihelper_home::intellihelper_home();
    let encoded = intellihelper_shell::util::intellihelper_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: intellihelper_tools::implementations::intellihelper_build::ask_user_question::AskUserQuestionMode::Default,
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    intellihelper_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
...
const POLICY_WARNING: &str =
    intellihelper_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
