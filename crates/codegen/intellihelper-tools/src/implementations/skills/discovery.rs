...
///
/// `skills` is the standard layout (`.intellihelper/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into IntelliHelper CLI sessions.
...
/// name (e.g. `~/.intellihelper/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into IntelliHelper CLI.
...
/// `cwd` (exclusive). At each directory, checks for `.intellihelper/skills/`,
...
/// visits known config roots (`.intellihelper`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.intellihelper` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".intellihelper", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.intellihelper is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.intellihelper/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.intellihelper/ → kept (user content).
        let intellihelper_shell = tmp.path().join(".intellihelper").join("skills").join("shell");
        std::fs::create_dir_all(&intellihelper_shell).unwrap();
        std::fs::write(
            intellihelper_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (intellihelper_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.intellihelper/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .intellihelper skill in an intermediate dir.
        ...
        .unwrap();
        let intellihelper_skill = sub.join(".intellihelper").join("skills").join("intellihelper-dyn");
        std::fs::create_dir_all(&intellihelper_skill).unwrap();
        std::fs::write(intellihelper_skill.join("SKILL.md"), "---\nname: intellihelper-dyn\n---\n").unwrap();
        ...
        assert!(
            names_on.contains(&"intellihelper-dyn"),
            "intellihelper-dyn missing: {names_on:?}"
        );
        assert!(
        ...
        // claude.skills OFF → only intellihelper-dyn discovered.
        ...
        assert!(
            names_off.contains(&"intellihelper-dyn"),
            "intellihelper-dyn missing: {names_off:?}"
        );
        assert!(
