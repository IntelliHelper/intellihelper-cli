...
fn pager_binary() -> Result<std::path::PathBuf, String> {
    for key in ["PAGER_BINARY", "CARGO_BIN_EXE_intellihelper-pager"] {
    ...
    Err("PAGER_BINARY/CARGO_BIN_EXE_intellihelper-pager not set".to_owned())
}
...
fn doctor_json_bypasses_unrelated_startup_state() {
    ...
    let intellihelper_home = temp.path().join("intellihelper-home");
    std::fs::create_dir_all(&home).expect("create HOME");
    std::fs::create_dir_all(&intellihelper_home).expect("create INTELLIHELPER_HOME");
    ...
    let before = directory_entries(&intellihelper_home);
    ...
    assert!(!String::from_utf8_lossy(&output.stdout).contains("IntelliHelper Doctor"));

    let after = directory_entries(&intellihelper_home);
    ...
    for absent in ["docs", "crash", "memtrace", "active_sessions.json"] {
        assert!(
            !intellihelper_home.join(absent).exists(),
...
fn doctor_fix_without_id_lists_tmux_fixes_from_current_probe_evidence() {
    ...
    let home = temp.path().join("home");
    let intellihelper_home = temp.path().join("qhome");
...
fn doctor_hostile_home_and_byobu_create_no_config_files() {
    ...
    for (key, value) in [("HOME", "."), ("BYOBU_CONFIG_DIR", "relative")] {
        let mut command = base_pager_command(&binary, &home, &intellihelper_home, "/bin/bash");
        command
...
fn doctor_fix_without_id_lists_only_applicable_automatic_fixes() {
    ...
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(
        stdout.contains("On your local computer, run: intellihelper doctor fix ssh-wrap"),
        "{stdout}"
    );
    assert!(!home.join(".bashrc").exists());
...
fn doctor_tmux_fix_yes_writes_only_actual_home_tmux_config() {
    ...
    assert_eq!(
        ...
        "# >>> intellihelper doctor >>>\n# >>> terminal.tmux-clipboard >>>\nset -g set-clipboard on\n# <<< terminal.tmux-clipboard <<<\n# <<< intellihelper doctor <<<"
    );
    assert!(!intellihelper_home.join(".tmux.conf").exists());
}
...
fn doctor_fix_yes_writes_only_actual_home_shell_rc() {
    ...
    assert_eq!(
        ...
        "# >>> intellihelper doctor >>>\n# >>> terminal.ssh-wrap >>>\nalias ssh='intellihelper wrap ssh'\n# <<< terminal.ssh-wrap <<<\n# <<< intellihelper doctor <<<"
    );
    assert!(!intellihelper_home.join(".bashrc").exists());
}
...
fn doctor_fix_safety_boundaries_are_process_isolated() {
    ...
    assert!(
        stderr.contains("IntelliHelper found an existing SSH alias or function")
...
fn run_pager(
    ...
    let mut command = base_pager_command(binary, home, intellihelper_home, shell);
...
fn base_pager_command(
    ...
    command
        .env_clear()
        .env("HOME", home)
        .env("INTELLIHELPER_HOME", intellihelper_home)
        .env("SHELL", shell)
