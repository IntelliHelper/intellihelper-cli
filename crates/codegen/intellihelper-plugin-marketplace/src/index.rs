...
//!
//! Catalog file lookup, in order: `.intellihelper-plugin/marketplace.json` (preferred),
//! `.intellihelper-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.intellihelper-plugin/marketplace.json` (preferred xAI convention)
/// 2. `.intellihelper-plugin/plugin.json`
...
pub fn load_index(marketplace_root: &Path) -> Result<Option<MarketplaceIndex>, String> {
    let intellihelper_dir = marketplace_root.join(".intellihelper-plugin");
    let claude_dir = marketplace_root.join(".claude-plugin");
    let candidates = [
        intellihelper_dir.join("marketplace.json"),
        intellihelper_dir.join("plugin.json"),
...
mod tests {
    ...
    #[test]
    fn load_index_valid_intellihelper_dir() {
        let dir = tempfile::tempdir().unwrap();
        let intellihelper_dir = dir.path().join(".intellihelper-plugin");
        std::fs::create_dir_all(&intellihelper_dir).unwrap();
        std::fs::write(
            intellihelper_dir.join("marketplace.json"),
            r#"{"name": "intellihelper", "plugins": []}"#,
        )
        .unwrap();
        let result = load_index(dir.path()).unwrap();
        assert!(result.is_some());
        assert_eq!(result.unwrap().name, "intellihelper");
    }

    #[test]
    fn load_index_intellihelper_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".intellihelper-plugin", "intellihelper"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "intellihelper");
    }
