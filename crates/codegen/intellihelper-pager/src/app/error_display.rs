...
impl WireErrorType {
    pub(crate) fn parse(raw: Option<&str>) -> Self {
        match raw {
            ...
            Some(s) if s == intellihelper_shell::extensions::notification::DISK_FULL_ERROR_TYPE => {
                Self::DiskFull
            }
            Some("api") => Self::Api,
            Some("http") => Self::Http,
...
mod tests {
    ...
    fn drops_model_catalog_dump() {
        let formatted = format_request_failure(
            None,
            Some("api"),
            "API error (status 404 Not Found): model does not exist\n\n  Model:     intellihelper-foo\n  Auth:      ApiKey\n  Version:   0.1.0\n  Available: intellihelper-build\n\n  'intellihelper-foo' is not in your available models.\n  Switch models with /model or start a new session.",
        );
        assert_eq!(formatted.status, Some(404));
        assert_eq!(
            formatted.message(),
            "Not found (404) \u{2014} 'intellihelper-foo' is not in your available models. \
