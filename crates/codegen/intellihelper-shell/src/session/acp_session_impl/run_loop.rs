...
pub(super) async fn run_session(
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<intellihelper_mcp::servers::McpClientEvent>();
        {
        ...
        tokio::select! {
                ...
                _ = &mut idle_flush_sleep, if session.idle_flush_timeout.is_some()
                    ...
                    if current_len > last_len {
                        tracing::info!(target: intellihelper_telemetry::memory_log::TARGET,
                        ...
                        tracing::debug!(target: intellihelper_telemetry::memory_log::TARGET,
                ...
                maybe_cmd = cmd_rx.recv() => {
                    let Some(cmd) = maybe_cmd else {
                        ...
                        let envelope = session.fire_hook(
                            intellihelper_hooks::event::HookEventName::SessionEnd,
                            None,
                            intellihelper_hooks::event::HookPayload::SessionEnd {
                        ...
                        if let Some(registry) = session.hook_registry.borrow().clone() {
                            ...
                            let results = intellihelper_hooks::dispatcher::dispatch_non_blocking(
                                &registry,
                                intellihelper_hooks::event::HookEventName::SessionEnd,
                                &envelope,
                                &ctx,
                        ...
                        }
                        {
                            ...
                            if let Some(signals) = session.signals_handle().snapshot().await {
                                intellihelper_telemetry::session_ctx::log_event(
                                    intellihelper_telemetry::events::SessionEnded {
                    ...
                    match cmd {
                        ...
                        SessionCommand::OverrideModelName { model_name, extra_headers, context_window } => {
                            ...
                            if let Some(mut cfg) = session.chat_state_handle.get_sampling_config().await {
                                ...
                                // the agent-level default (e.g. "intellihelper-4.5").
                        ...
                        SessionCommand::InjectNotification { prompt_id, prompt_blocks, priority, source } => {
                            ...
                            if is_turn_active && priority == NotificationPriority::Next {
                                ...
                                if let Some(buffer) = &session.tool_context.monitor_event_buffer {
                                    ...
                                    buffer.push_capped(
                                        intellihelper_tools::implementations::intellihelper_build::monitor::types::MonitorEventNotification {
                        ...
                        SessionCommand::RefreshSkillBaseline => {
                            ...
                            tokio::task::spawn_local(async move {
                                ...
                                let new_skills = intellihelper_agent::prompt::skills::list_skills_with_plugins(
                                    Some(&cwd),
                                    &skills_config,
                                    pr.as_deref(),
                        ...
                        SessionCommand::UpdateMcpServers { mcp_servers, respond_to } => {
                            ...
                            // also fires for `<cwd>/.intellihelper/config.toml` edits,
                            ...
                            {
                                let _ = tx.send(
                                    intellihelper_mcp::servers::McpClientEvent::ConfigDiff {
                        ...
                        SessionCommand::ToggleMcpServer { server_name, enabled, server_config, respond_to } => {
                            session.events.emit(file_utils::events::Event::McpServerToggled {
                        ...
                        SessionCommand::DispatchSessionStartHook { source } => {
                            let envelope = session.fire_hook(
                                intellihelper_hooks::event::HookEventName::SessionStart,
                                None,
                                intellihelper_hooks::event::HookPayload::SessionStart {
                                    source,
                                    model_id: None,
                                    agent_type: None,
                                },
                        ...
                        SessionCommand::OptimizePrompt { draft, respond_to } => {
                            let s = session.clone();
                            tokio::task::spawn_local(async move {
                                let result = s.handle_optimize_prompt(&draft).await;
                                let _ = respond_to.send(result);
                            });
                        }
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[intellihelper_sampling_types::ConversationItem],
    turn_number: usize,
