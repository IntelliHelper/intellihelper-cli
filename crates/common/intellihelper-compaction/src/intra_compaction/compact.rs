...
// The `Shared` summarizer reuses intellihelper-build's full-replace summarization core
...
/// `FullReplace` strategy (default): intellihelper-build's full-replace — summarize the
...
/// `<intellihelper_user_queries>` preamble: the shared `code_compaction` summarizer
/// (always [`IntraSummarizer::Shared`] here, regardless of `policy.summarizer`)
/// preserves user intent itself, matching intellihelper-build. The reduction and
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 2. Summarize (possibly fitted) turns through intellihelper-build's shared core.
    ...
    //     Shared with IntelliHelper chat inter-compaction via `append_reminder_block` so
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 3a. For `History` target, split prior `<intellihelper_user_queries>` blocks
    //     out of any prior compaction summary items before sampling — same
    //     primitive inter-compaction uses, so the LLM never sees
    //     `<intellihelper_user_queries>` and won't re-emit it (which would snowball
    ...
    //     and the `<intellihelper_user_queries>` preamble below — stays intra's.
    let summary_text = match policy.summarizer {
        ...
        // New (default): intellihelper-build's shared summarization core from
    ...
    // 3c. For `History` target, prepend a `<intellihelper_user_queries>` preamble so
...
/// — intellihelper-build's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and intellihelper-build stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // intellihelper-build appends the summarization prompt as the final user message;
    ...
    .await
    {
        // intellihelper-build returns the raw summary and cleans it in its assembler;
...
/// metric label. Keep these in sync with the doc string on IntelliHelper chat's
...
/// the literal error messages produced by the IntelliHelper chat sampler —
...
mod tests {
    ...
    fn compaction_sample_error_to_intra_maps_empty_response() {
        // The literal message emitted by the IntelliHelper chat sampler when the
    ...
    /// — guards the forwarding that IntelliHelper chat (`Arc<IntelliHelperTurn>`) relies on.
    #[test]
