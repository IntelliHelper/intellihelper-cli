...
fn write_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &IntelliHelperAuth) {
...
async fn oidc_refresh_picks_up_valid_disk_token() {
    ...
    let fresh_on_disk = IntelliHelperAuth {
...
fn expired_oidc_for(base_url: &str) -> IntelliHelperAuth {
    IntelliHelperAuth {
        key: "old-expired-token".into(),
        create_time: Utc::now() - Duration::hours(2),
        user_id: "user-42".into(),
...
/// produce a `PermanentFailure` ("Run `intellihelper login`") verdict, because they
