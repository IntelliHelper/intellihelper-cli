...
//!
//! > After any install attempt, `$BIN_DIR/intellihelper` resolves to a binary that runs,
...
//!
//! The installer lives in the sibling `intellihelper-pager` crate; it is resolved by
...
fn script_path(name: &str) -> Option<PathBuf> {
    dunce::canonicalize(
        Path::new(env!("CARGO_MANIFEST_DIR")).join(format!("../intellihelper-pager/scripts/{name}")),
    )
    .ok()
    .filter(|p| p.exists())
}
...
const INSTALLER_BLOCK_START: &str = "# >>> intellihelper installer >>>";
...
fn seed_previous_good(home: &Path, platform: &str) -> PathBuf {
    let downloads = home.join(".intellihelper").join("downloads");
    let bin = home.join(".intellihelper").join("bin");
    std::fs::create_dir_all(&downloads).unwrap();
    std::fs::create_dir_all(&bin).unwrap();
    let prev = downloads.join(format!("intellihelper-{platform}"));
    ...
    std::os::unix::fs::symlink(format!("../downloads/intellihelper-{platform}"), &link).unwrap();
    dunce::canonicalize(&prev).unwrap()
}

/// Re-resolve `$BIN_DIR/intellihelper` from disk and re-run it: the active intellihelper must
/// always execute, and never be a `.tmp`/partial file.
fn assert_active_grok_runs(home: &Path) {
    let link = home.join(".intellihelper").join("bin").join("intellihelper");
    assert!(link.is_symlink(), "intellihelper must remain a symlink");
    let resolved =
        dunce::canonicalize(&link).unwrap_or_else(|e| panic!("intellihelper symlink dangles: {e}"));
    let name = resolved.file_name().unwrap().to_string_lossy().to_string();
    assert!(
        !name.contains(".tmp"),
        "active intellihelper must not be a temp file: {name}"
    );
    let ok = Command::new(&resolved)
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    assert!(ok, "active intellihelper must run: {}", resolved.display());
}
...
fn run_installer(install_sh: &Path, home: &Path, fakebin: &Path, mode: &str, shell: &str) -> bool {
    ...
    let status = Command::new("/bin/bash")
        ...
        .env("PATH", path_env)
        .env("SHELL", shell)
        .env("INTELLIHELPER_BIN_DIR", home.join(".intellihelper").join("bin"))
        .env("INTELLIHELPER_CHANNEL", "stable")
        .env("FAKE_MODE", mode)
        .status()
...
fn assert_single_installer_block(path: &Path, preserved: Option<&str>) {
    ...
    assert_eq!(
        n,
        1,
        "{} must contain exactly one intellihelper installer block, got {n}:\n{body}",
        path.display()
    );
...
fn install_sh_blitz_keeps_grok_runnable_under_corruption() {
    ...
    for (mode, expect_ok) in cases {
        ...
        // intellihelper always runs (new good binary on success, previous-good on
        // rejection).
