...
use std::sync::Arc;
use intellihelper_agent::prompt::skills::SkillsConfig;
use intellihelper_sampler::{AuthScheme, SamplerConfig};
use intellihelper_sampling_types::{
...
use intellihelper_tools::types::compat::{
...
pub const DEFAULT_AGENT_TYPE: &str = "intellihelper-build-plan";
...
pub struct EndpointsConfig {
    ...
    /// Env: `INTELLIHELPER_MODELS_BASE_URL`. Enables custom endpoint mode.
    /// List URL defaults to `{models_base_url}/models`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub models_base_url: Option<String>,
    /// Env: `INTELLIHELPER_MODELS_LIST_URL`. Overrides the default `{base}/models` list URL.
    #[serde(alias = "models_endpoint", skip_serializing_if = "Option::is_none")]
    pub models_list_url: Option<String>,
    /// Env: `INTELLIHELPER_FEEDBACK_BASE_URL`. Where feedback submissions go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub feedback_base_url: Option<String>,
    /// Env: `INTELLIHELPER_TRACE_UPLOAD_URL`. Where trace uploads go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_url: Option<String>,
    /// Env: `INTELLIHELPER_TRACE_UPLOAD_BUCKET`. Direct bucket (`gs://` or `s3://`), bypasses proxy.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_bucket: Option<String>,
    /// Env: `INTELLIHELPER_TRACE_UPLOAD_REGION`. AWS region (S3 only).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_region: Option<String>,
    /// Env: `INTELLIHELPER_TRACE_UPLOAD_CREDENTIALS_FILE`. Path to GCS SA key or AWS credentials file.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Env: `INTELLIHELPER_TRACE_UPLOAD_ENDPOINT_URL`. Custom S3-compatible endpoint.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_endpoint_url: Option<String>,
    /// Env: `INTELLIHELPER_DEPLOYMENT_KEY`. Management API key for enterprise deployments.
    /// Sent on telemetry and service requests for deployment-level attribution.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub deployment_key: Option<String>,
    /// Env: `INTELLIHELPER_MANAGED_CONFIG_URL`. Override the managed config endpoint.
    ...
    /// favor of `INTELLIHELPER_INTERNAL_OTLP_TRACES_ENDPOINT`, and ignored by the internal
    /// pipeline when `INTELLIHELPER_EXTERNAL_OTEL` is set (the standard `OTEL_*` vars then
    ...
    /// Env: `INTELLIHELPER_INTERNAL_OTLP_TRACES_ENDPOINT`. Full INTERNAL traces endpoint,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub intellihelper_internal_otlp_traces_endpoint: Option<String>,
    /// Env: `INTELLIHELPER_INTERNAL_OTLP_HEADERS`. `k=v,k2=v2` extra headers for the
    /// internal export (debug). Wins over the legacy `OTEL_EXPORTER_OTLP_HEADERS`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub intellihelper_internal_otlp_headers: Option<String>,
    /// External-OTEL master switch, captured at construction via
    /// [`external_otel_master_switch_resolved`] — the same layered resolution
    /// (requirement pin > `INTELLIHELPER_EXTERNAL_OTEL` env > `[telemetry].otel_enabled`
...
/// format, shared with `INTELLIHELPER_INTERNAL_OTLP_HEADERS`): split on `,`,
...
impl EndpointsConfig {
    ...
    /// Managed deployment-config URL (`intellihelper setup`): explicit `managed_config_url`,
    ...
    /// 1. `intellihelper_internal_otlp_traces_endpoint` (verbatim)
    ...
    pub(crate) fn resolve_otlp_traces_endpoint(&self) -> String {
        if let Some(full) = blank_as_unset(&self.intellihelper_internal_otlp_traces_endpoint) {
            ...
            && let Some(legacy) = self.legacy_internal_otlp_traces_endpoint()
        {
            tracing::warn!(
                "Repointing the internal trace pipeline via OTEL_EXPORTER_OTLP_ENDPOINT / \
                 ...
                 INTELLIHELPER_INTERNAL_OTLP_TRACES_ENDPOINT instead — the standard OTEL_* vars will \
    ...
    /// Extra headers for the INTERNAL export: `intellihelper_internal_otlp_headers`
    ...
    pub(crate) fn resolve_otlp_headers(&self) -> Vec<(String, String)> {
        if let Some(headers) = blank_as_unset(&self.intellihelper_internal_otlp_headers) {
    ...
    /// `intellihelper_internal_otlp_*` override shadowed that half.
    ///
    ...
    pub(crate) fn internal_otlp_consumed_standard_vars(&self) -> bool {
        ...
        let endpoint_consumed = blank_as_unset(&self.intellihelper_internal_otlp_traces_endpoint).is_none()
            && self.legacy_internal_otlp_traces_endpoint().is_some();
        let headers_consumed = blank_as_unset(&self.intellihelper_internal_otlp_headers).is_none()
    ...
    /// still honored by the internal pipeline even with `INTELLIHELPER_EXTERNAL_OTEL`
    ...
    pub fn resolve_trace_bucket_url(&self) -> Option<Resolved<String>> {
        resolve_string_flag(
            None,
            "INTELLIHELPER_TELEMETRY_GCS_BUCKET",
...
impl Default for EndpointsConfig {
    fn default() -> Self {
        Self {
            cli_chat_proxy_base_url: std::env::var("INTELLIHELPER_CLI_CHAT_PROXY_BASE_URL").ok(),
            xai_api_base_url: std::env::var("INTELLIHELPER_XAI_API_BASE_URL")
                .unwrap_or_else(|_| XAI_API_BASE_URL_DEFAULT.to_owned()),
            alpha_test_key: None,
            models_base_url: env_string("INTELLIHELPER_MODELS_BASE_URL"),
            models_list_url: env_string("INTELLIHELPER_MODELS_LIST_URL"),
            feedback_base_url: env_string("INTELLIHELPER_FEEDBACK_BASE_URL"),
            trace_upload_url: env_string("INTELLIHELPER_TRACE_UPLOAD_URL"),
            trace_upload_bucket: env_string("INTELLIHELPER_TRACE_UPLOAD_BUCKET"),
            trace_upload_region: env_string("INTELLIHELPER_TRACE_UPLOAD_REGION"),
            trace_upload_credentials_file: env_string("INTELLIHELPER_TRACE_UPLOAD_CREDENTIALS_FILE"),
            trace_upload_credentials: None,
            trace_upload_endpoint_url: env_string("INTELLIHELPER_TRACE_UPLOAD_ENDPOINT_URL"),
            deployment_key: env_string("INTELLIHELPER_DEPLOYMENT_KEY"),
            managed_config_url: env_string("INTELLIHELPER_MANAGED_CONFIG_URL"),
            ...
            intellihelper_internal_otlp_traces_endpoint: env_string("INTELLIHELPER_INTERNAL_OTLP_TRACES_ENDPOINT"),
            intellihelper_internal_otlp_headers: env_string("INTELLIHELPER_INTERNAL_OTLP_HEADERS"),
...
pub use intellihelper_config_types::{BoolFlag, ConfigSource, LazinessDetectorPerModelConfig, Resolved};
...
/// environment so it can't inherit the keys IntelliHelper uses for its own first-party
/// requests. Keep in sync with every first-party credential env read across the
/// crate: `auth::manager` (`INTELLIHELPER_AUTH`/`INTELLIHELPER_AUTH_PATH`), `auth_method`
...
pub(crate) const FIRST_PARTY_CREDENTIAL_ENV_VARS: &[&str] = &[
    ...
    "INTELLIHELPER_AUTH",
    "INTELLIHELPER_AUTH_PATH",
    "INTELLIHELPER_DEPLOYMENT_KEY",
    "INTELLIHELPER_EXTRA_AUTH_KEY",
    "INTELLIHELPER_TRACE_UPLOAD_CREDENTIALS_FILE",
    "OTEL_EXPORTER_OTLP_HEADERS",
    "INTELLIHELPER_INTERNAL_OTLP_HEADERS",
];
...
pub use intellihelper_config::env_bool;
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(intellihelper_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: intellihelper_tools::types::compat::CompatCell,
...
pub(crate) use intellihelper_telemetry::config::env_telemetry_mode;
pub use intellihelper_telemetry::config::{TelemetryConfig, TelemetryMode};
...
impl PluginsConfig {
    ...
    /// Native `.intellihelper/config.toml` entries already present take precedence:
    ...
    pub(crate) fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                intellihelper_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
    ...
    pub(crate) fn to_discovery_config(
        &self,
    ) -> intellihelper_agent::plugins::discovery::DiscoveryConfig {
        intellihelper_agent::plugins::discovery::DiscoveryConfig {
...
pub struct CliConfig {
    ...
    /// Env `INTELLIHELPER_MINIMUM_VERSION`. See [`crate::util::config::VersionPolicy`] for
    /// the version-policy knobs. (Unrelated to
    /// `version_overrides[].maximum_version`, which gates config patches.)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub minimum_version: Option<String>,
    /// Env `INTELLIHELPER_MAXIMUM_VERSION`. See [`crate::util::config::VersionPolicy`].
    #[serde(skip_serializing_if = "Option::is_none")]
    pub maximum_version: Option<String>,
    /// Env `INTELLIHELPER_REQUIRED_MINIMUM_VERSION`. See [`crate::util::config::VersionPolicy`].
    #[serde(skip_serializing_if = "Option::is_none")]
    pub required_minimum_version: Option<String>,
    /// Env `INTELLIHELPER_REQUIRED_MAXIMUM_VERSION`. See [`crate::util::config::VersionPolicy`].
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct ModelsConfig {
    ...
    /// Unset = remote pin, then the client hint / built-in `intellihelper-build-0.1`
...
/// (`intellihelper workspace` / leader `with_default_hub_url`). Does **not** enable
...
pub struct HubConfig {
    ...
    /// `intellihelper workspace start` when the CLI does not pass `--hub-url`.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl SandboxSettingsConfig {
    ...
    pub fn resolve_profile(
        ...
        resolve_string_flag(cli_arg, "INTELLIHELPER_SANDBOX", self.profile.as_deref(), None)
    ...
    pub(crate) fn resolve_auto_allow_bash(&self, requirement: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("INTELLIHELPER_SANDBOX_AUTO_ALLOW_BASH")
...
impl SuggestionsConfig {
    pub fn resolve_enabled(
        ...
        BoolFlag::env("INTELLIHELPER_SUGGESTIONS")
            .config(self.enabled)
    ...
    pub fn resolve_ai_enabled(
        ...
        BoolFlag::env("INTELLIHELPER_SUGGESTIONS_AI")
    ...
    pub fn resolve_ai_model(&self) -> String {
        resolve_string_flag(
            None,
            "INTELLIHELPER_SUGGESTIONS_AI_MODEL",
            self.ai_model.as_deref(),
            None,
        )
        .map(|r| r.value)
        .unwrap_or_else(|| "intellihelper-build".to_owned())
    }
}
...
///
/// These supplement the built-in scan locations (`.intellihelper/skills/`,
/// `.agents/skills/`, `~/.intellihelper/skills/`). They're written by `/import-claude`
...
pub struct Config {
    ...
    /// `[desktop]` section — owned by intellihelper-desktop (Electron app), opaque to the CLI agent.
    #[serde(default, skip_serializing)]
    pub desktop: Option<toml::Value>,
    /// Top-level `announcements` array — consumed by `resolve_announcements`.
    #[serde(default, skip_serializing)]
    pub announcements: Vec<intellihelper_announcements::RemoteAnnouncement>,
    ...
    /// `[marketplace]` — also read by `intellihelper_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    /// Defaults to reading from INTELLIHELPER_STORAGE_MODE env var.
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<intellihelper_agent::config::AgentDefinition>,
    #[serde(skip)]
    pub cli_agent_overrides: CliAgentOverrides,
    /// Whether subagent (task tool) support is enabled. Enabled by default;
    /// disabled only via `INTELLIHELPER_SUBAGENTS=0` or `[subagents] enabled = false`.
    /// Not remotely gated.
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub subagent_roles:
        std::collections::HashMap<String, intellihelper_subagent_resolution::config::SubagentRole>,
    /// Trust-independent personas from inline, user, and bundled sources.
    #[serde(skip)]
    pub subagent_personas:
        std::collections::HashMap<String, intellihelper_subagent_resolution::config::SubagentPersona>,
    ...
    pub zdr_video_output_s3:
        Option<intellihelper_tools::implementations::intellihelper_build::video_gen::ZdrVideoOutputS3Config>,
    ...
    /// Image describe model (`intellihelper-build` default via `ModelOverrideConfig::resolve`).
    #[serde(skip)]
...
pub struct CliAgentOverrides {
    ...
    pub permission_rules: Vec<intellihelper_workspace::permission::types::PermissionRule>,
    pub max_turns: Option<u32>,
    pub permission_mode: Option<intellihelper_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub(crate) fn apply_to_definition(&self, def: &mut intellihelper_agent::config::AgentDefinition) {
    ...
    pub(crate) fn apply_to_subagent_definition(
        &self,
        def: &mut intellihelper_agent::config::AgentDefinition,
    ) {
...
pub use intellihelper_agent::config::AgentDefinition;
pub use intellihelper_agent::config::Effort;
pub use intellihelper_agent::config::PermissionMode;
pub use intellihelper_shared::ui_config::{ContextualHints, UiConfig};
...
/// ```toml
/// [agent]
/// # Use a named agent (looked up via discovery: .intellihelper/agents/, ~/.intellihelper/agents/, built-ins)
...
/// 4. `INTELLIHELPER_AGENT` env var
/// 5. Default `intellihelper-build` agent
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct AgentSelectionConfig {
    /// Name of a built-in or discovered agent definition.
    /// Looked up via `intellihelper_agent::discovery::by_name_in_cwd()`.
    /// Examples: "intellihelper-build", "browser-use", or a custom agent name.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
    /// Path to an agent definition file (.md with YAML frontmatter).
    /// When set, the agent is loaded from this file.
    /// Supports environment variable expansion (e.g., `$HOME/.intellihelper/agents/my-agent.md`).
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mut cfg = Self {
            ...
            client_version: Some(intellihelper_version::VERSION.to_string()),
...
/// (surfaced by `intellihelper inspect`) and is skipped, so it fails closed for the
...
impl Config {
    ...
    pub fn create_auth_manager(&self) -> AuthManager {
        AuthManager::new(
            &crate::util::intellihelper_home::intellihelper_home(),
    ...
    pub fn resolve_runtime_fields(&mut self, ctx: &RuntimeResolutionContext<'_>) {
        ...
        self.auto_wake_enabled = BoolFlag::env("INTELLIHELPER_AUTO_WAKE")
    ...
    fn apply_env_overrides(&mut self) {
        ...
        if let Some(mode) = env_telemetry_mode("INTELLIHELPER_TELEMETRY_ENABLED") {
    ...
    /// config.toml key, or `INTELLIHELPER_TWO_PASS_COMPACTION` env.
    ...
    pub(crate) fn resolve_trace_upload(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("INTELLIHELPER_TELEMETRY_TRACE_UPLOAD")
    ...
    pub(crate) fn trace_upload_decision_debug(&self) -> serde_json::Value {
        ...
        serde_json::json!({
            ...
            "in_env_trace_upload": std::env::var("INTELLIHELPER_TELEMETRY_TRACE_UPLOAD").ok(),
            "in_env_telemetry_enabled": std::env::var("INTELLIHELPER_TELEMETRY_ENABLED").ok(),
    ...
    pub(crate) fn resolve_feedback(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("INTELLIHELPER_FEEDBACK_ENABLED")
    ...
    pub(crate) fn resolve_two_pass_compaction(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("INTELLIHELPER_TWO_PASS_COMPACTION")
            .config(self.features.two_pass_compaction)
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    /// Server-side doom-loop check policy (the `x-intellihelper-doom-loop-check`
    ...
    /// `INTELLIHELPER_DOOM_LOOP_RECOVERY` > TOML `enabled` > remote `enabled` >
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<intellihelper_sampling_types::DoomLoopRecoveryPolicy> {
        use intellihelper_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
            .and_then(|s| s.doom_loop_recovery.as_ref());
        let enabled = BoolFlag::env("INTELLIHELPER_DOOM_LOOP_RECOVERY")
    ...
    /// Precedence: env `INTELLIHELPER_OFFICIAL_MARKETPLACE_AUTO_REGISTER` > remote settings >
    ...
    pub(crate) fn resolve_official_marketplace_auto_register(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("INTELLIHELPER_OFFICIAL_MARKETPLACE_AUTO_REGISTER")
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    pub(crate) fn resolve_lsp_tools(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("INTELLIHELPER_LSP_TOOLS")
    ...
    pub(crate) fn resolve_web_fetch(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("INTELLIHELPER_WEB_FETCH")
    ...
    pub(crate) fn resolve_ask_user_question(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("INTELLIHELPER_ASK_USER_QUESTION")
    ...
    ///
    /// Precedence: requirements > `INTELLIHELPER_VOICE_MODE` > config/managed
    ...
    pub(crate) fn resolve_voice_mode(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("INTELLIHELPER_VOICE_MODE")
    ...
    pub(crate) fn resolve_image_gen(&self) -> Resolved<bool> {
        use intellihelper_tools::implementations::intellihelper_build::IMAGE_GEN_TOOL_NAME;
        ...
        BoolFlag::env("INTELLIHELPER_IMAGE_GEN")
    ...
    pub(crate) fn resolve_image_edit(&self) -> Resolved<bool> {
        use intellihelper_tools::implementations::intellihelper_build::IMAGE_EDIT_TOOL_NAME;
        ...
        BoolFlag::env("INTELLIHELPER_IMAGE_EDIT").default(true).resolve()
    }
    ...
    pub(crate) fn resolve_video_gen(&self) -> Resolved<bool> {
        use intellihelper_tools::implementations::intellihelper_build::{
        ...
        BoolFlag::env("INTELLIHELPER_VIDEO_GEN")
    ...
    /// Precedence: env `INTELLIHELPER_IMAGE_GEN_MODEL_OVERRIDE` > `[features]
    /// image_gen_model_override` config > remote settings `image_gen_model_override`.
    /// `None` → default model (`intellihelper-imagine-image-quality`).
    pub(crate) fn resolve_image_gen_model_override(&self) -> Option<String> {
        resolve_string_flag(
            None,
            "INTELLIHELPER_IMAGE_GEN_MODEL_OVERRIDE",
    ...
    pub(crate) fn resolve_image_edit_model_override(&self) -> Option<String> {
        resolve_string_flag(
            None,
            "INTELLIHELPER_IMAGE_EDIT_MODEL_OVERRIDE",
    ...
    pub(crate) fn resolve_goal(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("INTELLIHELPER_GOAL")
            .config(self.goal.enabled)
            .feature_flag(ff)
            .default(true)
            .resolve()
    }
    /// Background workflows (`workflow` tool, `.intellihelper/workflows/*.rhai`,
    ...
    pub(crate) fn resolve_workflows(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("INTELLIHELPER_WORKFLOWS")
    ...
    pub(crate) fn resolve_goal_classifier_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("INTELLIHELPER_GOAL_CLASSIFIER")
    ...
    pub(crate) fn resolve_goal_planner_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("INTELLIHELPER_GOAL_PLANNER")
    ...
    pub(crate) fn resolve_goal_summary_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("INTELLIHELPER_GOAL_SUMMARY")
    ...
    pub(crate) fn resolve_goal_classifier_max_runs(&self) -> Resolved<u32> {
        ...
        Self::resolve_goal_u32(
            "INTELLIHELPER_GOAL_CLASSIFIER_MAX",
    ...
    pub(crate) fn resolve_goal_strategist_every(&self, classifier_max_runs: u32) -> Resolved<u32> {
        Self::resolve_goal_u32(
            "INTELLIHELPER_GOAL_STRATEGIST_EVERY",
    ...
    pub(crate) fn resolve_goal_reverify_after(&self) -> Resolved<u32> {
        Self::resolve_goal_u32(
            "INTELLIHELPER_GOAL_REVERIFY_AFTER",
    ...
    pub(crate) fn resolve_goal_use_current_model_only(&self) -> Resolved<bool> {
        BoolFlag::env("INTELLIHELPER_GOAL_USE_CURRENT_MODEL_ONLY")
    ...
    pub(crate) fn resolve_write_file(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("INTELLIHELPER_WRITE_FILE")
    ...
    pub(crate) fn resolve_backend_tools(&self) -> Resolved<bool> {
        BoolFlag::env("INTELLIHELPER_BACKEND_SEARCH")
            .config(self.features.backend_tools)
            .default(true)
            .resolve()
    }
    /// Resolve the mode (env `INTELLIHELPER_COMPACTION_MODE` > config > remote settings >
    ...
    pub(crate) fn resolve_compaction_mode(&self) -> chat_state::CompactionMode {
        resolve_compaction_mode_from(
            env_string("INTELLIHELPER_COMPACTION_MODE").as_deref(),
    ...
    /// Resolve verbatim-input flag: env `INTELLIHELPER_COMPACTION_VERBATIM_INPUT` > config > remote settings > default `true`.
    pub(crate) fn resolve_compaction_verbatim_input(&self) -> bool {
        BoolFlag::env("INTELLIHELPER_COMPACTION_VERBATIM_INPUT")
    ...
    /// Precedence: env `INTELLIHELPER_COMPACTION_DETAIL`, then config
    ...
    fn resolve_compaction_detail(&self) -> chat_state::CompactionDetail {
        resolve_compaction_detail_from(
            env_string("INTELLIHELPER_COMPACTION_DETAIL").as_deref(),
    ...
    pub(crate) fn resolve_cancel_rewind(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("INTELLIHELPER_CANCEL_REWIND")
            .config(self.features.cancel_rewind)
            .feature_flag(ff)
            .default(true)
            .resolve()
    }
    /// Resolve whether to use intellihelper's default OAuth2 (xAI auth.x.ai).
    ///
    ...
    ///
    /// Priority: `--oauth` > INTELLIHELPER_OAUTH_ENABLED env > default (true = OAuth).
    pub(crate) fn resolve_grok_oauth(&self, cli_oidc: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("INTELLIHELPER_OAUTH_ENABLED")
            .cli(cli_oidc)
            .default(true)
            .resolve()
    }
...
///
/// `requirement > cli > env (INTELLIHELPER_MCP_LIVENESS_WATCHERS) > config >
...
pub(crate) fn resolve_mcp_liveness_watchers(
    ...
    BoolFlag::env("INTELLIHELPER_MCP_LIVENESS_WATCHERS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// `BoolFlag` precedence:
///
/// `requirement > cli > env (INTELLIHELPER_MCP_AUTO_RESTART) > config >
/// managed > feature_flag > default (true)`.
///
...
/// precedence is single-sourced.
///
/// Recovery is on by default; opt out via `INTELLIHELPER_MCP_AUTO_RESTART=false`,
...
pub(crate) fn resolve_mcp_auto_restart(
    ...
    BoolFlag::env("INTELLIHELPER_MCP_AUTO_RESTART")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
///
/// `requirement > cli > env (INTELLIHELPER_MCP_PUSH_SERVER_STATUS) > config >
/// managed > feature_flag > default (true)`.
///
...
pub fn resolve_mcp_push_server_status(
    ...
    BoolFlag::env("INTELLIHELPER_MCP_PUSH_SERVER_STATUS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// [`resolve_mcp_liveness_watchers`]:
///
/// `requirement > cli > env (INTELLIHELPER_MCP_RECURSIVE_CONFIG_WATCH) >
...
/// operators flip this flag (e.g. via `INTELLIHELPER_MCP_RECURSIVE_CONFIG_
...
pub(crate) fn resolve_mcp_recursive_config_watch(
    ...
    BoolFlag::env("INTELLIHELPER_MCP_RECURSIVE_CONFIG_WATCH")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
impl SyncBoolFlag {
    ...
    /// Either-direction env resolver (typically `INTELLIHELPER_*`). Returns
...
pub(crate) fn is_telemetry_disabled_sync() -> bool {
    !SyncBoolFlag::new(telemetry_enabled_from_toml)
        ...
        .enable_env(intellihelper_telemetry_env_enabled)
        .resolve()
}
...
pub fn is_error_reporting_disabled_sync() -> bool {
    !SyncBoolFlag::new(error_reporting_enabled_from_toml)
        ...
        .enable_env(|| env_bool("INTELLIHELPER_ERROR_REPORTING"))
...
/// `INTELLIHELPER_TELEMETRY_ENABLED` resolved through `TelemetryMode::parse` so the
/// extended string forms (e.g. `"session_metrics"`) are accepted.
fn intellihelper_telemetry_env_enabled() -> Option<bool> {
    env_telemetry_mode("INTELLIHELPER_TELEMETRY_ENABLED").map(|m| !m.is_disabled())
}
/// Load `~/.intellihelper/requirements.toml` standalone so the admin pin can beat
/// env vars. The merged config layer can't express that — last-merge-wins
/// loses provenance.
pub(crate) fn read_requirements_toml() -> Option<toml::Value> {
    let path = crate::util::intellihelper_home::intellihelper_home().join("requirements.toml");
    let content = std::fs::read_to_string(&path).ok()?;
    toml::from_str(&content).ok()
}
/// Resolve the external-OTEL master switch exactly the way the external
/// stream's activation does: **requirement pin > `INTELLIHELPER_EXTERNAL_OTEL` env >
...
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        intellihelper_config::load_merged_requirements().as_ref(),
        env_bool("INTELLIHELPER_EXTERNAL_OTEL"),
...
/// by `intellihelper setup`) sit under the env vars, requirements pins are applied on
...
pub fn resolve_external_otel_config(
    client: intellihelper_telemetry::external::config::ExternalClientInfo,
) -> Option<intellihelper_telemetry::external::ExternalOtelConfig> {
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<intellihelper_telemetry::external::ExternalOtelFileConfig> = effective_config
        .and_then(|cfg| cfg.get("telemetry"))
        .map(|t| intellihelper_telemetry::external::ExternalOtelFileConfig {
    ...
    let getenv_pinned = |name: &str| -> Option<String> {
        let pin = match name {
            intellihelper_telemetry::external::config::ENV_MASTER_SWITCH => req_enabled,
    ...
    let mut resolved = intellihelper_telemetry::external::ExternalOtelConfig::resolve_with(
        getenv_pinned,
        file_cfg.as_ref(),
    )?;
...
pub(crate) fn apply_external_otel_remote_policy(
    ...
    let policy = intellihelper_telemetry::external::ExternalOtelRemotePolicy {
    ...
    if policy.force_disable || policy.lock_content_gates {
        intellihelper_telemetry::external::apply_remote_policy(policy);
    }
}
...
pub fn apply_remote_settings_side_effects(settings: Option<&crate::util::config::RemoteSettings>) {
    if let Some(s) = settings {
        ...
        intellihelper_config::signed_policy::apply_remote_managed_config_signature_verification(
...
fn managed_settings_env_flag(key: &str) -> Option<bool> {
    let path = intellihelper_config::claude_managed_settings_path()?;
    let content = std::fs::read_to_string(&path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&content).ok()?;
    intellihelper_workspace::permission::resolution::json_env_flag(json.get("env"), key)
}
...
pub struct ModelEntryConfig {
    ...
    /// Can also be set via the `INTELLIHELPER_MAX_RETRIES` environment variable.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_retries: Option<u32>,
...
pub struct ModelInfo {
    ...
    /// Always has a value; defaults to `"intellihelper-build-plan"` when the server
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<intellihelper_workspace::permission::ClassifierPromptType>,
...
pub struct Features {
    ...
    /// Show a blocking warning when IntelliHelper starts outside a Git repository.
    /// Default: false. Used as the local fallback when the `non_git_warning` remote settings
    /// flag in `intellihelper_build_settings` is absent. When the remote flag is present it takes
    ...
    /// (`image_gen_model_override`) / env / default (`intellihelper-imagine-image-quality`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// env (`INTELLIHELPER_COMPACTION_MODE`). Parsed via `CompactionMode::parse`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compaction_mode: Option<String>,
    /// `none` | `minimal` | `balanced` | `verbose` (default). `None` = defer to
    /// env (`INTELLIHELPER_COMPACTION_DETAIL`). The `segments` verbatim detail level.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// `INTELLIHELPER_MCP_AUTO_RESTART` to opt out).
    ///
    /// Not read through this struct: the live resolver re-reads the
    /// `[features]` key out-of-band from raw TOML in
    /// `util::config::resolve::mcp`. Declared so `serde_ignored`
    ...
    /// `~/.intellihelper/config.toml` will NOT disable the pager's
    /// subscription on a freshly-launched process. To disable the
    /// pager subscription, set `INTELLIHELPER_MCP_PUSH_SERVER_STATUS=0` in
    /// the env before launch.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub mcp_push_server_status: Option<bool>,
    /// Whether the leader's `ConfigFileWatcher` adds the two narrow
    /// non-recursive watches for `<cwd>/` and `<cwd>/.intellihelper/`.
    ///
    /// When `true` (default), edits to `<cwd>/.mcp.json`,
    /// `<cwd>/.intellihelper/config.toml`, or `<cwd>/.claude.json` flow
...
pub(crate) fn enforce_disable_api_key_auth(
        ...
        && crate::util::is_xai_api_url(&creds.base_url)
    {
        ...
        intellihelper_telemetry::unified_log::debug(
...
pub use intellihelper_telemetry::config::deployment_id_from_key;
...
/// BYOK / non-proxy routes for internal slugs like `intellihelper-build`).
...
pub(crate) fn inject_url_derived_headers(
    ...
    if crate::util::is_cli_chat_proxy_url(base_url) {
        headers
            ...
            .or_insert_with(|| "intellihelper-cli".to_string());
        headers
...
mod tests {
    ...
    use intellihelper_test_support::EnvGuard;
    #[test]
    fn main_cli_tools_override_preserves_profile_injection_policy() {
        ...
        let mut cases = vec![(AgentDefinition::default_intellihelper_build(), true)];
    ...
    fn auto_mode_config_parses_from_toml_and_json_equivalently() {
        use intellihelper_workspace::permission::ClassifierPromptType;
        let toml_src = r#"
enabled = true
prompt_type = "no_user_tool_prefix"
classifier_model = "intellihelper-4.5"
...
"#;
        ...
        for cfg in [&from_toml, &from_json] {
            ...
            assert_eq!(cfg.classifier_model.as_deref(), Some("intellihelper-4.5"));
    ...
    fn finalize_image_describe_sampler_none_uses_active_session_model_not_forced_helper() {
        ...
        assert_ne!(cfg.model, "intellihelper-build");
    }
    #[test]
    fn finalize_image_describe_sampler_some_stamps_session_fields() {
        ...
        let aux = SamplerConfig {
            model: "intellihelper-build".into(),
            ..Default::default()
        };
        let (model, cfg) =
            finalize_image_describe_sampler_config(Some(aux), &active, Some("cli".into()), Some(7));
        assert_eq!(model, "intellihelper-build");
        assert_eq!(cfg.model, "intellihelper-build");
        assert_eq!(cfg.client_identifier.as_deref(), Some("cli"));
        assert_eq!(cfg.max_retries, Some(7));
    }
    #[test]
    fn resolve_aux_model_honors_intellihelper_build_override() {
        let endpoints = EndpointsConfig::default();
        let mut catalog = IndexMap::new();
        catalog.insert(
            "intellihelper-build".to_string(),
            test_model_entry(
    ...
    fn session_resolver_is_not_stamped_onto_third_party_samplers() {
        ...
        impl intellihelper_sampler::BearerResolver for SessionResolver {
    ...
    fn shell_environment_policy_known_keys_track_the_policy_struct() {
        let intellihelper_tools::util::ShellEnvironmentPolicy {
            inherit: _,
            ignore_default_excludes: _,
            exclude: _,
            set: _,
            include_only: _,
        } = intellihelper_tools::util::ShellEnvironmentPolicy::default();
    ...
    fn parses_model_api_key() {
        ...
        assert_eq!(model.info.model, "intellihelper-4.5");
    ...
    async fn set_env_key_shadows_warm_provider_at_resolve_time() {
        use intellihelper_test_support::EnvGuard;
        let var = "INTELLIHELPER_TEST_ENVKEY_SHADOW";
    ...
    fn env_keys_resolve_first_set_wins() {
        let keys = EnvKeys::new(["INTELLIHELPER_TEST_ENV_KEY_PRIMARY", "INTELLIHELPER_TEST_ENV_KEY_FALLBACK"]);
        assert_eq!(keys.resolve_value_with(|_| None), None, "none set");
        assert_eq!(
            keys.resolve_value_with(
                |n| (n == "INTELLIHELPER_TEST_ENV_KEY_FALLBACK").then(|| "from-fallback".into())
            ),
            Some("from-fallback".into())
        );
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "INTELLIHELPER_TEST_ENV_KEY_PRIMARY" => Some("from-primary".into()),
                "INTELLIHELPER_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
        ...
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "INTELLIHELPER_TEST_ENV_KEY_PRIMARY" => Some(String::new()),
                "INTELLIHELPER_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
            Some("from-fallback".into())
    ...
    fn env_keys_resolve_skips_whitespace_only_value() {
        let keys = EnvKeys::new(["INTELLIHELPER_TEST_WS_PRIMARY", "INTELLIHELPER_TEST_WS_FALLBACK"]);
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "INTELLIHELPER_TEST_WS_PRIMARY" => Some("   ".into()),
                "INTELLIHELPER_TEST_WS_FALLBACK" => Some("real".into()),
                _ => None,
            }),
            Some("real".into())
        );
        assert_eq!(
            EnvKeys::single("INTELLIHELPER_TEST_WS_ONLY").resolve_value_with(|_| Some("   ".into())),
            None
        );
        assert_eq!(
            EnvKeys::single("INTELLIHELPER_TEST_WS_PAD").resolve_value_with(|_| Some("  tok  ".into())),
            Some("  tok  ".into())
        );
    }
    #[test]
    ...
    fn first_own_credential_empty_api_key_falls_through_to_env_key() {
        use intellihelper_test_support::EnvGuard;
        let var = "INTELLIHELPER_TEST_FIRST_OWN_CRED_ENV";
        let _guard = EnvGuard::set(var, "env-token");
    ...
    fn resolve_credentials_multi_env_key_uses_lc_alias() {
        use chat_state::AuthType;
        let primary = "INTELLIHELPER_TEST_MULTI_ENV_PRIMARY";
        let alias = "INTELLIHELPER_TEST_MULTI_ENV_LC_ALIAS";
        unsafe {
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_session() {
        use chat_state::AuthType;
        use intellihelper_test_support::EnvGuard;
        let primary = "INTELLIHELPER_TEST_EMPTY_ENV_PRIMARY";
        let alias = "INTELLIHELPER_TEST_EMPTY_ENV_LC_ALIAS";
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_global_key() {
        ...
        let primary = "INTELLIHELPER_TEST_EMPTY_ENV_GLOBAL_PRIMARY";
        let alias = "INTELLIHELPER_TEST_EMPTY_ENV_GLOBAL_ALIAS";
        let _primary = EnvGuard::set(primary, "");
        let _alias = EnvGuard::set(alias, "");
    ...
    fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
        ...
        assert_eq!(config.api_backend, ApiBackend::Messages);
        let client = intellihelper_sampler::SamplingClient::new(config).expect("client should build");
    ...
    /// intellihelper-shell/src/agent/models.rs:857 for any BYOK Claude config.
    #[test]
    ...
    #[test]
    fn model_agent_type_defaults_to_intellihelper_build() {
        let raw_config: toml::Value = toml::from_str(
            r#"
            [model.my-model]
            model = "my-model"
    ...
    fn invalid_glob_is_rejected_by_validation() {
        ...
        assert!(ModelGlobSet::compile(Some(&vec!["intellihelper[".to_string()])).is_err());
        let raw: toml::Value = toml::from_str(
            r#"
            [models]
            allowed_models = ["intellihelper["]
            "#,
        )
        .unwrap();
    ...
    #[test]
    fn auth_alias_maps_to_intellihelper_com_config() {
        let raw: toml::Value = toml::from_str(
            r#"
            [auth.oidc]
    ...
    fn config_models_default_is_not_overwritten_by_default_models_json() {
        ...
        let resolved = resolve_string_flag(
            None,
            "INTELLIHELPER_DEFAULT_MODEL_TEST_NONEXISTENT",
            config_default,
    ...
    fn config_models_default_custom_model_is_in_resolved_model_list() {
        ...
        assert!(
            models.contains_key("acme-intellihelper"),
            "user-defined model must be in the resolved model list"
        );
        let model = models.get("acme-intellihelper").unwrap();
        assert_eq!(model.info.model, "intellihelper-4.5");
        assert_eq!(model.info.base_url, "https://inference.example.com/v1");
    }
    #[test]
    ...
    fn e2e_duplicate_model_field_both_entries_survive() {
        ...
        let user = models.get("acme-intellihelper").unwrap();
    ...
    fn e2e_acp_model_info_no_dedup_on_model_field() {
        let mut models = IndexMap::new();
        models.insert(
            "default-intellihelper".to_string(),
            test_model_entry(
                crate::models::default_model(),
        ...
        assert!(
            acp_models.contains_key(&acp::ModelId::new("default-intellihelper")),
            "default entry should be addressable by map key"
        );
        assert!(
            acp_models.contains_key(&acp::ModelId::new("acme-intellihelper")),
    ...
    fn unset_endpoint_env_vars() {
        for k in [
            "INTELLIHELPER_CLI_CHAT_PROXY_BASE_URL",
            "INTELLIHELPER_XAI_API_BASE_URL",
            "INTELLIHELPER_FEEDBACK_BASE_URL",
            "INTELLIHELPER_TRACE_UPLOAD_URL",
            "INTELLIHELPER_MANAGED_CONFIG_URL",
            "INTELLIHELPER_MODELS_BASE_URL",
            "INTELLIHELPER_MODELS_LIST_URL",
            "OTEL_EXPORTER_OTLP_ENDPOINT",
            "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT",
            "OTEL_EXPORTER_OTLP_HEADERS",
            "INTELLIHELPER_INTERNAL_OTLP_TRACES_ENDPOINT",
            "INTELLIHELPER_INTERNAL_OTLP_HEADERS",
            "INTELLIHELPER_EXTERNAL_OTEL",
        ] {
    ...
    fn resolve_feedback_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("INTELLIHELPER_FEEDBACK_ENABLED") };
        unsafe { std::env::remove_var("INTELLIHELPER_TELEMETRY_ENABLED") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("INTELLIHELPER_SESSION_RECAP") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_env_off_overrides_default() {
        unsafe { std::env::set_var("INTELLIHELPER_SESSION_RECAP", "0") };
        let cfg = Config::default();
        let r = cfg.resolve_session_recap();
        assert!(!r.value);
    ...
    fn resolve_turn_summary_precedence() {
        unsafe { std::env::remove_var("INTELLIHELPER_TURN_SUMMARY") };
        ...
        assert_eq!(r.source, ConfigSource::Config);
        unsafe { std::env::set_var("INTELLIHELPER_TURN_SUMMARY", "0") };
        let r = config_over_remote.resolve_turn_summary();
    ...
    fn resolve_two_pass_compaction_precedence() {
        unsafe { std::env::remove_var("INTELLIHELPER_TWO_PASS_COMPACTION") };
        ...
        assert!(r.value);
        assert_eq!(r.source, ConfigSource::Config);
        unsafe { std::env::set_var("INTELLIHELPER_TWO_PASS_COMPACTION", "0") };
        let r = config_over_remote.resolve_two_pass_compaction();
        assert!(!r.value, "env wins over config + remote");
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::remove_var("INTELLIHELPER_TWO_PASS_COMPACTION") };
    ...
    fn resolve_doom_loop_recovery_precedence() {
        ...
        unsafe { std::env::remove_var("INTELLIHELPER_DOOM_LOOP_RECOVERY") };
        let default_cfg = Config::default();
        let p = default_cfg
        ...
        unsafe { std::env::set_var("INTELLIHELPER_DOOM_LOOP_RECOVERY", "0") };
        assert!(
    ...
    fn resolve_feedback_env_overrides_all() {
        unsafe { std::env::set_var("INTELLIHELPER_FEEDBACK_ENABLED", "true") };
        let mut cfg = Config::default();
    ...
    fn resolve_trace_upload_disabled_when_telemetry_off_despite_remote_flag() {
        unsafe { std::env::remove_var("INTELLIHELPER_TELEMETRY_ENABLED") };
        unsafe { std::env::remove_var("INTELLIHELPER_TELEMETRY_TRACE_UPLOAD") };
        let mut cfg = Config::default();
    ...
    fn resolve_goal_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("INTELLIHELPER_GOAL") };
        let cfg = Config::default();
    ...
    fn resolve_goal_env_overrides_config_without_remote_kill_switch() {
        unsafe { std::env::set_var("INTELLIHELPER_GOAL", "1") };
        let mut cfg = Config::default();
    ...
    fn background_workflows_default_on_without_affecting_goal() {
        unsafe { std::env::remove_var("INTELLIHELPER_WORKFLOWS") };
        let cfg = Config::default();
    ...
    fn resolve_workflows_remote_false_kills_local_opt_in() {
        unsafe { std::env::set_var("INTELLIHELPER_WORKFLOWS", "1") };
        let mut cfg = Config::default();
    ...
    fn resolve_workflows_env_wins() {
        unsafe { std::env::set_var("INTELLIHELPER_WORKFLOWS", "0") };
        let cfg = Config::default();
        let r = cfg.resolve_workflows();
        assert_eq!(r.source, ConfigSource::Env);
        assert!(
    ...
    fn resolve_ask_user_question_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("INTELLIHELPER_ASK_USER_QUESTION") };
        let cfg = Config::default();
    ...
    fn resolve_ask_user_question_env_overrides_remote_settings() {
        unsafe { std::env::set_var("INTELLIHELPER_ASK_USER_QUESTION", "1") };
        let cfg = Config {
            remote_settings: Some(crate::util::config::RemoteSettings {
    ...
    fn resolve_image_gen_model_override_remote_settings_or_config() {
        unsafe { std::env::remove_var("INTELLIHELPER_IMAGE_GEN_MODEL_OVERRIDE") };
        ...
        assert_eq!(
            with(None, Some("intellihelper-imagine-image")).resolve_image_gen_model_override(),
            Some("intellihelper-imagine-image".to_owned())
        );
        assert_eq!(
            with(Some("intellihelper-imagine-image-pro"), Some("intellihelper-imagine-image"))
                .resolve_image_gen_model_override(),
            Some("intellihelper-imagine-image-pro".to_owned())
        );
    }
    #[test]
    #[serial]
    fn resolve_image_edit_model_override_remote_settings_or_config() {
        unsafe { std::env::remove_var("INTELLIHELPER_IMAGE_EDIT_MODEL_OVERRIDE") };
        let with = |config: Option<&str>, gb: Option<&str>| Config {
            features: Features {
        ...
        assert_eq!(
            with(None, Some("intellihelper-imagine-image")).resolve_image_edit_model_override(),
            Some("intellihelper-imagine-image".to_owned())
        );
        assert_eq!(
            with(Some("intellihelper-imagine-image-pro"), Some("intellihelper-imagine-image"))
        ...
        let gen_only = Config {
            remote_settings: Some(crate::util::config::RemoteSettings {
                image_gen_model_override: Some("intellihelper-imagine-image".to_owned()),
                ..Default::default()
            }),
            ..Default::default()
        };
    ...
    fn imagine_tools_disabled_gates_image_edit() {
        unsafe { std::env::remove_var("INTELLIHELPER_IMAGE_EDIT") };
        ...
        unsafe { std::env::set_var("INTELLIHELPER_IMAGE_EDIT", "1") };
    ...
    fn resolve_image_gen_gates() {
        unsafe { std::env::remove_var("INTELLIHELPER_IMAGE_GEN") };
        ...
        unsafe { std::env::set_var("INTELLIHELPER_IMAGE_GEN", "1") };
        let denied = Config {
            remote_settings: Some(crate::util::config::RemoteSettings {
    ...
    fn resolve_video_gen_gates() {
        unsafe { std::env::remove_var("INTELLIHELPER_VIDEO_GEN") };
    ...
    fn clear_goal_envs() {
        unsafe {
            std::env::remove_var("INTELLIHELPER_GOAL");
            std::env::remove_var("INTELLIHELPER_GOAL_CLASSIFIER");
            std::env::remove_var("INTELLIHELPER_GOAL_PLANNER");
            std::env::remove_var("INTELLIHELPER_GOAL_SUMMARY");
            std::env::remove_var("INTELLIHELPER_GOAL_VERIFIER_N");
            std::env::remove_var("INTELLIHELPER_GOAL_CLASSIFIER_MAX");
            std::env::remove_var("INTELLIHELPER_GOAL_STRATEGIST_EVERY");
            std::env::remove_var("INTELLIHELPER_GOAL_REVERIFY_AFTER");
        }
    }
    ...
    fn resolve_goal_classifier_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("INTELLIHELPER_GOAL_CLASSIFIER", "0") };
        let r = cfg_with_goal_and_remote(true, remote_classifier(true))
            .resolve_goal_classifier_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("INTELLIHELPER_GOAL_CLASSIFIER", "1") };
    ...
    fn resolve_goal_planner_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("INTELLIHELPER_GOAL_PLANNER", "0") };
        let r =
            cfg_with_goal_and_remote(true, remote_planner(true)).resolve_goal_planner_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("INTELLIHELPER_GOAL_PLANNER", "1") };
    ...
    fn resolve_goal_summary_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("INTELLIHELPER_GOAL_SUMMARY", "0") };
        let r =
...
"#,
    ...
    const GOAL_USE_CURRENT_ENV: &str = "INTELLIHELPER_GOAL_USE_CURRENT_MODEL_ONLY";
    ...
    fn planner_pair() -> crate::util::config::GoalRoleModel {
        crate::util::config::GoalRoleModel {
            model: "intellihelper-4".to_string(),
            agent_type: "general-purpose".to_string(),
        }
    }
    fn strategist_pair() -> crate::util::config::GoalRoleModel {
        crate::util::config::GoalRoleModel {
            model: "intellihelper-4.5".to_string(),
...
[goal]
enabled = true
planner_model = { model = "intellihelper-build", agent_type = "intellihelper-build-plan" }

[goal.strategist_model]
model = "intellihelper-composer-2.5-fast"
agent_type = "cursor"

[[goal.skeptic_models]]
model = "intellihelper-build"
agent_type = "intellihelper-build-plan"
...
"#;
        ...
        let cfg = Config::new_from_toml_cfg(&raw).unwrap();
        assert_eq!(cfg.goal.planner_model.as_ref().unwrap().model, "intellihelper-build");
        assert_eq!(
            cfg.goal.strategist_model.as_ref().unwrap().agent_type,
            "cursor"
        );
        assert_eq!(cfg.goal.skeptic_models.len(), 2);
        assert_eq!(cfg.goal.skeptic_models[0].model, "intellihelper-build");
        assert_eq!(
...
planner_model = { agent_type = "intellihelper-build-plan" }
"#;
        let raw: toml::Value = toml::from_str(toml_str).unwrap();
        let cfg = Config::new_from_toml_cfg(&raw)
...
"#;
        ...
        assert_eq!(cfg.goal.skeptic_models.len(), 2);
        assert_eq!(cfg.goal.skeptic_models[0].model, "intellihelper-build");
        assert_eq!(cfg.goal.skeptic_models[1].model, "intellihelper-composer-2.5-fast");
    }
...
classifier_max_runs = 6
planner_model = { model = "intellihelper-build", agent_type = "intellihelper-build-plan" }
strategist_model = { model = "intellihelper-composer-2.5-fast", agent_type = "cursor" }
...
"#,
        ...
        let intellihelper_build = crate::util::config::GoalRoleModel {
            model: "intellihelper-build".into(),
            agent_type: "intellihelper-build-plan".into(),
        };
        let composer = crate::util::config::GoalRoleModel {
            model: "intellihelper-composer-2.5-fast".into(),
        ...
        assert_eq!(
            planner.value,
            GoalRoleModelChoice::Explicit(intellihelper_build.clone())
        );
        ...
        assert_eq!(
            ...
            vec![
                GoalRoleModelChoice::Explicit(intellihelper_build),
    ...
    fn config_accepts_all_known_sections() {
        let unused = unused_keys_from_toml(
            ...
            command = "/opt/bin/intellihelper-identity"
            [repo_changes_dedup]
            enabled = false
            [relay]
            enabled = false
    ...
    fn internal_otlp_test_config() -> EndpointsConfig {
        EndpointsConfig {
            ...
            intellihelper_internal_otlp_traces_endpoint: None,
            intellihelper_internal_otlp_headers: None,
            external_otel_master_switch: false,
            ..Default::default()
        }
    }
    /// `intellihelper_internal_otlp_traces_endpoint` wins over the legacy `OTEL_*`
    /// fields regardless of the master switch.
    #[test]
    fn internal_otlp_endpoint_grok_internal_wins_regardless_of_switch() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                intellihelper_internal_otlp_traces_endpoint: Some(
            ...
            assert_eq!(
                ...
                "switch={switch}: INTELLIHELPER_INTERNAL_OTLP_TRACES_ENDPOINT must win verbatim (trailing / trimmed)"
            );
        }
    }
    ...
    fn internal_otlp_consumed_standard_vars_cases() {
        ...
        for case in cases {
            let cfg = EndpointsConfig {
                ...
                intellihelper_internal_otlp_traces_endpoint: case
                    .internal_ep
                    .then(|| "https://internal.example/traces".to_string()),
                intellihelper_internal_otlp_headers: case.internal_headers.then(|| "ik=iv".to_string()),
                ..internal_otlp_test_config()
            };
            assert_eq!(
    ...
    /// Headers precedence: `intellihelper_internal_otlp_headers` wins; legacy
    /// `otel_exporter_otlp_headers` only when the master switch is unset.
    #[test]
    fn internal_otlp_headers_precedence() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                intellihelper_internal_otlp_headers: Some("x-debug=1".to_string()),
    ...
    fn ext_client() -> intellihelper_telemetry::external::config::ExternalClientInfo {
        intellihelper_telemetry::external::config::ExternalClientInfo::default()
    }
    #[test]
    fn external_otel_default_off_and_double_opt_in() {
        ...
        assert!(
            resolve_external_otel_config_with(
                None,
                None,
                ext_env(&[("INTELLIHELPER_EXTERNAL_OTEL", "1")]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    fn external_otel_file_table_layered_under_env() {
        ...
        assert!(
            resolve_external_otel_config_with(
                Some(&effective),
                None,
                ext_env(&[("INTELLIHELPER_EXTERNAL_OTEL", "0")]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    fn external_otel_requirements_pin_wins_over_env() {
        ...
        assert!(
            resolve_external_otel_config_with(
                None,
                Some(&req),
                ext_env(&[("INTELLIHELPER_EXTERNAL_OTEL", "1"), ("OTEL_LOGS_EXPORTER", "otlp"),]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    /// (managed config / requirements — no `INTELLIHELPER_EXTERNAL_OTEL` env var) must
    ...
    fn clear_runtime_env_vars() {
        unsafe {
            std::env::remove_var("INTELLIHELPER_SUBAGENTS");
            std::env::remove_var("INTELLIHELPER_RESPECT_GITIGNORE");
            std::env::remove_var("INTELLIHELPER_WEB_SEARCH_MODEL");
            std::env::remove_var("INTELLIHELPER_SESSION_SUMMARY_MODEL");
            std::env::remove_var("INTELLIHELPER_CURSOR_SKILLS_ENABLED");
            std::env::remove_var("INTELLIHELPER_CURSOR_RULES_ENABLED");
            std::env::remove_var("INTELLIHELPER_CURSOR_AGENTS_ENABLED");
            std::env::remove_var("INTELLIHELPER_CLAUDE_SKILLS_ENABLED");
            std::env::remove_var("INTELLIHELPER_CLAUDE_RULES_ENABLED");
            std::env::remove_var("INTELLIHELPER_CLAUDE_AGENTS_ENABLED");
        }
    }
    fn clear_managed_mcp_env_vars() {
        unsafe {
            std::env::remove_var("INTELLIHELPER_MANAGED_MCPS_ENABLED");
            std::env::remove_var("INTELLIHELPER_MANAGED_MCP_GATEWAY_TOOLS_ENABLED");
        }
    }
...
"#,
    ...
    fn resolve_raw_compat_sessions_load_failure_allows_env_override() {
        let _env = isolate_compat_env();
        let _codex = EnvGuard::set("INTELLIHELPER_CODEX_SESSIONS_ENABLED", "true");
        let resolved = resolve_compat_sessions_from_raw(Err(()), None);
        assert!(!resolved.cursor.sessions);
        assert!(!resolved.claude.sessions);
        assert!(resolved.codex.sessions);
    ...
    fn resolve_compat_env_sessions_disable_independently() {
        ...
        for (vendor, env_var) in [
            (CompatVendor::Cursor, "INTELLIHELPER_CURSOR_SESSIONS_ENABLED"),
            (CompatVendor::Claude, "INTELLIHELPER_CLAUDE_SESSIONS_ENABLED"),
            (CompatVendor::Codex, "INTELLIHELPER_CODEX_SESSIONS_ENABLED"),
        ] {
    ...
    fn resolve_compat_precedence_and_reserved_codex_hook() {
        ...
        let _session = EnvGuard::set("INTELLIHELPER_CURSOR_SESSIONS_ENABLED", "true");
        let _hook = EnvGuard::set("INTELLIHELPER_CODEX_HOOKS_ENABLED", "true");
        let resolved = resolve_compat_config(&config, Some(&remote));
        assert!(resolved.cursor.sessions);
        assert!(resolved.codex.hooks);
    }
    ...
    fn resolve_runtime_fields_compat_asymmetric_sources() {
        let _env = isolate_compat_env();
        let _cursor = EnvGuard::set("INTELLIHELPER_CURSOR_SESSIONS_ENABLED", "false");
        let raw: toml::Value =
    ...
    fn resolve_runtime_fields_gitignore_from_env() {
        clear_runtime_env_vars();
        unsafe { std::env::set_var("INTELLIHELPER_RESPECT_GITIGNORE", "0") };
        let raw = empty_config();
        let mut cfg = Config::new_from_toml_cfg(&raw).unwrap();
        cfg.resolve_runtime_fields(&RuntimeResolutionContext {
            raw_config: &raw,
...
"#,
    ...
    fn is_telemetry_explicitly_disabled_sync_env_signals() {
        unsafe { std::env::set_var("INTELLIHELPER_TELEMETRY_ENABLED", "0") };
        unsafe { std::env::remove_var("DISABLE_TELEMETRY") };
        assert!(is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::set_var("INTELLIHELPER_TELEMETRY_ENABLED", "1") };
        assert!(!is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::remove_var("INTELLIHELPER_TELEMETRY_ENABLED") };
...
"#,
        ...
        intellihelper_config::apply_version_overrides(&mut value, &v).unwrap();
        let cfg = Config::new_from_toml_cfg(&value).unwrap();
        assert_eq!(cfg.models.default.as_deref(), Some("intellihelper-4.5"));
    }
    /// Reproduce the enterprise managed config bug: [model.intellihelper-build] sets
    /// context_window=500k for model="intellihelper-4.5", but
    /// [models].default="intellihelper-4.5" resolves to the bare
    /// prefetched entry (256k) because Layer 3 only overrides key
    /// "intellihelper-build", not key "intellihelper-4.5".
    ///
    ...
    fn slug_propagation_enterprise_managed_config_key_mismatch() {
        ...
        prefetched.insert("intellihelper-4.5".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let by_key = resolved
            .get("intellihelper-build")
            .expect("intellihelper-build key must exist");
        assert_eq!(by_key.info.context_window.get(), 500_000);
        assert_eq!(by_key.info.model, "intellihelper-4.5");
        let by_latest = resolved.get("intellihelper-4.5").expect("intellihelper-4.5 key must exist");
        assert_eq!(
            by_latest.info.context_window.get(),
            500_000,
            "BUG: prefetched 'intellihelper-4.5' should inherit 500k from \
             sibling 'intellihelper-build' (same model slug), not stay at {default_cw}"
        );
    }
    ...
    fn slug_propagation_inherits_api_backend_but_not_agent_type() {
        ...
        let raw: toml::Value = toml::from_str(
            ...
            api_backend = "responses"
            agent_type = "intellihelper-build"
            "#,
        )
        .unwrap();
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        let mut prefetched = IndexMap::new();
        let mut entry =
            test_model_entry("intellihelper-4.5", "https://test.example.com/v1", None, None, None);
        entry.info.context_window = NonZeroU64::new(default_cw).unwrap();
        entry.info.agent_type = default_agent_type();
        entry.info.api_backend = ApiBackend::default();
        prefetched.insert("intellihelper-4.5".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let latest = resolved.get("intellihelper-4.5").unwrap();
        assert_eq!(
    ...
    fn resolve_model_list_config_reasoning_efforts_beats_remote() {
        let raw_config: toml::Value = toml::from_str(
            r#"
            [model.intellihelper-x]
            reasoning_efforts = ["low"]
            "#,
        )
        .unwrap();
        let cfg = Config::new_from_toml_cfg(&raw_config).expect("config should parse");
        let mut entry = prefetch_model_entry("intellihelper-x", 200_000, ApiBackend::default());
        entry.info.reasoning_efforts = vec![ReasoningEffortOption {
            id: "high".to_string(),
            value: ReasoningEffort::High,
            label: "High".to_string(),
        ...
        let mut prefetched = IndexMap::new();
        prefetched.insert("intellihelper-x".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let efforts = &resolved
            .get("intellihelper-x")
            .expect("intellihelper-x")
            .info
            .reasoning_efforts;
    ...
    fn mcp_liveness_watchers_default_is_true() {
        unsafe { std::env::remove_var("INTELLIHELPER_MCP_LIVENESS_WATCHERS") };
    ...
    fn mcp_liveness_watchers_requirement_wins_over_everything() {
        unsafe { std::env::set_var("INTELLIHELPER_MCP_LIVENESS_WATCHERS", "true") };
    ...
    fn mcp_liveness_watchers_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("INTELLIHELPER_MCP_LIVENESS_WATCHERS", "false") };
    ...
    fn mcp_auto_restart_default_is_true() {
        unsafe { std::env::remove_var("INTELLIHELPER_MCP_AUTO_RESTART") };
    ...
    fn mcp_auto_restart_requirement_wins_over_everything() {
        unsafe { std::env::set_var("INTELLIHELPER_MCP_AUTO_RESTART", "false") };
    ...
    fn mcp_auto_restart_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("INTELLIHELPER_MCP_AUTO_RESTART", "true") };
    ...
    fn mcp_push_server_status_default_is_true() {
        unsafe { std::env::remove_var("INTELLIHELPER_MCP_PUSH_SERVER_STATUS") };
    ...
    fn mcp_push_server_status_requirement_wins_over_everything() {
        unsafe { std::env::set_var("INTELLIHELPER_MCP_PUSH_SERVER_STATUS", "true") };
    ...
    fn mcp_push_server_status_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("INTELLIHELPER_MCP_PUSH_SERVER_STATUS", "false") };
    ...
    fn mcp_recursive_config_watch_default_is_true() {
        unsafe { std::env::remove_var("INTELLIHELPER_MCP_RECURSIVE_CONFIG_WATCH") };
    ...
    fn mcp_recursive_config_watch_requirement_wins_over_everything() {
        unsafe { std::env::set_var("INTELLIHELPER_MCP_RECURSIVE_CONFIG_WATCH", "true") };
    ...
    fn mcp_recursive_config_watch_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("INTELLIHELPER_MCP_RECURSIVE_CONFIG_WATCH", "false") };
    ...
    fn remote_settings_disarm_managed_config_signatures() {
        ...
        assert!(intellihelper_config::signed_policy::verification_active());
    ...
    fn remote_settings_disarm_requires_prod_proxy_when_keys_embedded() {
        ...
        unsafe {
            std::env::remove_var("INTELLIHELPER_CLI_CHAT_PROXY_BASE_URL");
        }
        apply_remote_settings_side_effects(Some(&settings));
        assert!(
            !intellihelper_config::signed_policy::verification_active(),
