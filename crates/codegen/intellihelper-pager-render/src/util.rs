...
pub use intellihelper_config::intellihelper_home;

/// Path to `$INTELLIHELPER_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    intellihelper_home().join("pager.toml")
}

/// User-facing label for the user intellihelper directory (``~/.intellihelper`` or ``$INTELLIHELPER_HOME``).
///
/// Derived from resolved [`intellihelper_home()`] vs `intellihelper_config::default_intellihelper_home()`,
/// not from whether `INTELLIHELPER_HOME` is set in the environment.
pub fn display_intellihelper_home_prefix() -> String {
    display_intellihelper_home_prefix_for(&intellihelper_home())
}

fn display_intellihelper_home_prefix_for(home: &Path) -> String {
    if home == intellihelper_config::default_intellihelper_home() {
        "~/.intellihelper".to_string()
    } else {
        "$INTELLIHELPER_HOME".to_string()
    }
}

/// User-facing path under [`intellihelper_home()`], e.g. ``~/.intellihelper/config.toml``.
pub fn display_user_intellihelper_path(relative: impl AsRef<Path>) -> String {
    display_user_intellihelper_path_for(&intellihelper_home(), relative)
}

fn display_user_intellihelper_path_for(home: &Path, relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_intellihelper_home_prefix_for(home);
...
/// Abbreviate an absolute path for display: prefer [`intellihelper_home()`], then `$HOME`.
pub fn abbreviate_path(path: &str) -> Cow<'_, str> {
    let path_buf = Path::new(path);
    let intellihelper = intellihelper_home();
    if let Ok(rest) = path_buf.strip_prefix(&intellihelper) {
        let prefix = display_intellihelper_home_prefix();
...
/// True when `path` is under user [`intellihelper_home()`] (not project `{cwd}/.intellihelper`).
pub fn is_under_user_intellihelper_home(path: &Path) -> bool {
    path.starts_with(intellihelper_home())
}
...
mod tests {
    ...
    #[test]
    fn display_intellihelper_home_prefix_default_install() {
        if std::env::var("INTELLIHELPER_HOME").is_ok() {
            return;
        }
        assert_eq!(display_intellihelper_home_prefix(), "~/.intellihelper");
    }

    #[test]
    fn display_user_intellihelper_path_joins_relative() {
        let path = display_user_intellihelper_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".intellihelper") || path.contains("$INTELLIHELPER_HOME"));
    }

    #[test]
    fn display_user_intellihelper_path_for_custom_home_uses_override_label() {
        let custom = std::env::temp_dir().join("intellihelper-home-display-regression");
        assert_eq!(
            display_user_intellihelper_path_for(&custom, "config.toml"),
            "$INTELLIHELPER_HOME/config.toml"
        );
        assert_eq!(
            display_user_intellihelper_path_for(&custom, "sandbox.toml"),
            "$INTELLIHELPER_HOME/sandbox.toml"
        );
    }
    ...
    fn abbreviate_path_uses_home_when_under_default_grok() {
        if let Ok(home) = std::env::var("HOME") {
            ...
            let full = format!("{home}/.intellihelper/memory/MEMORY.md");
