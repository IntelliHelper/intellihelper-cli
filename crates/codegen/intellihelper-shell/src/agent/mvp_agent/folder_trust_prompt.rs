...
//! (`x.ai/folder_trust/request`) that asks a GUI client (intellihelper-desktop) to decide
...
//! verdict is [`intellihelper_workspace::folder_trust::TrustOutcome::Prompt`]. No
...
//! `.intellihelper/lsp.json` servers start on the NEXT session open (the durable grant
...
impl MvpAgent {
    ...
    pub(crate) fn maybe_spawn_interactive_trust_prompt(
        ...
        let key = intellihelper_workspace::trust::workspace_key(cwd);
        ...
        self.session_registry.for_each_resident(|_, h| {
            if intellihelper_workspace::trust::workspace_key(std::path::Path::new(&h.info.cwd)) == key {
...
struct ReloadAfterGrant<'a> {
    ...
    plugin_handle: &'a intellihelper_agent::plugins::SharedPluginRegistryHandle,
    ...
    proxy_url: &'a str,
    compat: &'a intellihelper_tools::types::CompatConfig,
...
async fn reload_project_servers_after_grant(ctx: ReloadAfterGrant<'_>) {
    ...
    for target in ctx.targets {
        ...
        // The session's OWN project hooks (`.intellihelper/hooks`, `.cursor/hooks.json`),
