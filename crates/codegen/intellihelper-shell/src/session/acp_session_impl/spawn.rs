...
fn drop_cli_catchall_allows(
    rules: Vec<intellihelper_workspace::permission::types::PermissionRule>,
    policy_block: Option<&'static str>,
) -> (
    Vec<intellihelper_workspace::permission::types::PermissionRule>,
    Vec<intellihelper_workspace::permission::types::PermissionRule>,
) {
    ...
    for rule in rules {
        if intellihelper_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
///
/// The rlimit is lowered only in a re-exec'd child (the `gix-status`
...
mod runtime_containment_tests {
    ...
    const CHILD_ENV: &str = "INTELLIHELPER_SHELL_RUNTIME_CONTAINMENT_CHILD";
...
mod cli_catchall_drop_tests {
    ...
    use intellihelper_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    use intellihelper_workspace::permission::rules::parse_permission_rule;
    use intellihelper_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
...
pub(crate) async fn spawn_session_actor(
    ...
    attribution_callback: Option<intellihelper_sampler::SharedAttributionCallback>,
    ...
    web_search_sampling_config: Option<intellihelper_sampler::SamplerConfig>,
    ...
    app_builder_deployer_config: intellihelper_tools::implementations::intellihelper_build::deploy_app::AppBuilderDeployerConfig,
    ...
    prompt_audience: intellihelper_agent::prompt::context::PromptAudience,
    ...
    plugin_registry: Option<std::sync::Arc<intellihelper_agent::plugins::PluginRegistry>>,
    plugin_registry_handle: Option<intellihelper_agent::plugins::SharedPluginRegistryHandle>,
    models_manager: crate::agent::models::ModelsManager,
    inherited_permission_handle: Option<intellihelper_workspace::permission::PermissionHandle>,
    api_key_provider: Option<intellihelper_tools::types::SharedApiKeyProvider>,
    image_description_model: String,
    hook_registry_override: Option<std::sync::Arc<intellihelper_hooks::discovery::HookRegistry>>,
    workspace_ops: intellihelper_workspace::WorkspaceOps,
    cli_permission_rules: Vec<intellihelper_workspace::permission::types::PermissionRule>,
    todo_gate: bool,
    ...
    parent_terminal_backend: Option<
        std::sync::Arc<dyn intellihelper_tools::computer::types::TerminalBackend>,
    >,
    parent_scheduler_handle: Option<
        intellihelper_tools::implementations::intellihelper_build::scheduler::types::SchedulerHandle,
    >,
    ...
    intellihelper_agent::AgentBuildError,
> {
    if max_turns == Some(0) {
        return Err(intellihelper_agent::AgentBuildError::InvalidConfig(
        ...
        inherited_permission_handle
    {
        ...
        let mut permission_config =
            intellihelper_workspace::permission::resolution::resolve_permission_config_with_fallback(
                tool_context.cwd.as_path(),
                project_trusted,
            )
            .await;
        let yolo_pin = intellihelper_workspace::permission::resolution::yolo_disabled_by_policy();
            ...
            && !dropped_catchalls.is_empty()
        {
            ...
            if startup_hints.non_interactive {
                eprintln!("intellihelper: --allow catch-all ignored: {reason}");
            }
        }
            ...
            match &mut permission_config {
                ...
                None => {
                    permission_config = Some(
                        intellihelper_workspace::permission::types::PermissionConfig::new(
        ...
        let deny_read_globs = permission_config
            .as_ref()
            .map(intellihelper_workspace::permission::resolution::deny_read_globs_from_config)
            .unwrap_or_default();
        let hub_permission = if intellihelper_workspace::permission::hitl_permission_live_enabled() {
            ...
            let transport = server
                .and_then(|server| {
                    intellihelper_workspace::permission::ToolServerPermissionTransport::from_session_id(
                        server,
                ...
                .map(|t| {
                    std::sync::Arc::new(t)
                        as std::sync::Arc<
                            dyn intellihelper_workspace::permission::PermissionHookTransport,
                        >
                });
        ...
        let (permissions, permission_events_rx) =
            intellihelper_workspace::permission::spawn_permission_manager_with_hub(
    ...
    let web_search_config = if disable_web_search {
        intellihelper_tools::implementations::WebSearchConfig::Disabled
    } else if let Some(cfg) = web_search_sampling_config {
        if let Some(api_key) = cfg.api_key {
            intellihelper_tools::implementations::WebSearchConfig::Enabled {
                api_key,
    ...
    let context_window_override = std::env::var("INTELLIHELPER_DEBUG_CONTEXT_WINDOW")
        .ok()
    ...
    if let Some(cw) = context_window_override {
        tracing::warn!(
            ...
            "INTELLIHELPER_DEBUG_CONTEXT_WINDOW override active"
        );
    }
    let chat_state_sampling_config = intellihelper_sampling_types::SamplingConfig {
    ...
    let task_completion_reservations =
        intellihelper_tools::reminders::task_completion::TaskCompletionReservations::default();
    let task_wake_suppressed =
        intellihelper_tools::reminders::task_completion::TaskWakeSuppressed::default();
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let index_root_for_session =
        intellihelper_workspace::session::git::find_git_root_from_path(tool_context.cwd.as_path())
    ...
    let resolve_search_shadows = || {
        ...
        intellihelper_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
    let resolve_policy = || crate::util::config::resolve_shell_env_policy(effective_cfg.as_ref());
    let terminal_backend: std::sync::Arc<dyn intellihelper_tools::computer::types::TerminalBackend> =
        match terminal_backend_kind {
            ...
            TerminalBackendKind::AcpClient => {
                std::sync::Arc::new(crate::terminal::AcpTerminalAdapter::new(
                    ...
                    as std::sync::Arc<dyn intellihelper_tools::computer::types::TerminalBackend>
            }
    ...
    let fs_backend: std::sync::Arc<dyn intellihelper_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(intellihelper_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
            std::sync::Arc::new(intellihelper_tools::computer::local::LocalFs)
        };
    ...
    let compaction_policy = intellihelper_agent::CompactionPolicy {
    ...
    let (user_question_tx, user_question_rx) = tokio::sync::mpsc::unbounded_channel::<
        intellihelper_tools::implementations::intellihelper_build::ask_user_question::types::UserQuestionRequest,
    >();
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn intellihelper_tools::types::memory_backend::MemoryBackend>,
    > = if let Some(ref storage) = memory_storage_for_session {
        if let Err(e) = storage.ensure_initialized() {
            tracing::warn!(
                target: intellihelper_telemetry::memory_log::TARGET,
                error = %e,
        ...
        let backend: std::sync::Arc<dyn intellihelper_tools::types::memory_backend::MemoryBackend> =
        ...
        intellihelper_telemetry::session_ctx::log_event(
            intellihelper_telemetry::memory_telemetry::MemorySessionInit {
                session_id: session_info.id.to_string(),
                memory_enabled: true,
                watcher_config_enabled: watcher_config.enabled,
                watcher_started,
    ...
    let managed_gateway_tool_client = auth_manager.as_ref().map(|am| {
        intellihelper_tools::types::resources::ManagedGatewayToolClient(Arc::new(
    ...
    agent
        ...
        .await;
    let harness_metrics = if telemetry_enabled || intellihelper_telemetry::external::is_active() {
        ...
        Some(super::telemetry::SessionHarnessMetrics {
            ...
            permission_mode: if session_yolo_mode {
                intellihelper_telemetry::enums::PermissionMode::AlwaysApprove
            } else if session_auto_mode
                && crate::util::config::auto_permission_mode_enabled_from_disk()
            {
                intellihelper_telemetry::enums::PermissionMode::Auto
            } else {
                intellihelper_telemetry::enums::PermissionMode::Ask
            },
    ...
    let resolved_task_output =
        intellihelper_tools::reminders::task_completion::resolve_task_output_tool_name(
            agent.tool_bridge(),
        )
        .await;
    let resolved_read =
        intellihelper_tools::reminders::task_completion::resolve_read_tool_name(agent.tool_bridge())
            .await;
    ...
    tool_context.task_output_tool_name = resolved_task_output.unwrap_or_else(|| {
        intellihelper_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL.to_string()
    });
    let scheduler_handle_for_handle = {
        let toolset = agent.tool_bridge().toolset();
        let res = toolset.resources.lock().await;
        res.get::<intellihelper_tools::implementations::intellihelper_build::scheduler::types::SchedulerHandle>()
            .cloned()
    };
    ...
    let feedback_client_type = match client_type {
        ClientType::IntelliHelperTUI => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Tui,
        ClientType::IntelliHelperWeb => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Web,
        ...
        ClientType::IntelliHelperPager => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Tui,
    };
    ...
    let resolved_workspace_root = intellihelper_workspace::session::git::find_git_root_from_path(
    ...
    let sampler_retry_policy = intellihelper_sampler::RetryPolicy {
    ...
    let (sampler_event_tx, sampler_event_rx) =
        tokio::sync::mpsc::unbounded_channel::<intellihelper_sampler::SamplingEvent>();
    let sampler_handle = intellihelper_sampler::SamplerActor::spawn(
    ...
    let mut hook_discovery_errors: Vec<intellihelper_hooks::error::HookError> = Vec::new();
    let built_hook_registry: Option<Arc<intellihelper_hooks::discovery::HookRegistry>> =
        if let Some(override_reg) = hook_registry_override {
            ...
            let git_root = intellihelper_workspace::session::git::find_git_root_from_path(cwd_path).ok();
    ...
    let (goal_update_tx, goal_update_rx) = tokio::sync::mpsc::unbounded_channel::<
        intellihelper_tools::implementations::intellihelper_build::update_goal::UpdateGoalEnvelope,
    >();
    ...
    let (workflow_launch_tx, mut workflow_launch_rx) = tokio::sync::mpsc::unbounded_channel::<
        intellihelper_tools::implementations::intellihelper_build::workflow::WorkflowLaunchEnvelope,
    >();
    {
        ...
        tokio::spawn(async move {
            ...
            use intellihelper_tools::implementations::intellihelper_build::workflow::WorkflowLaunchAck;
            while let Some((req, ack)) = workflow_launch_rx.recv().await {
                if !background_workflows_enabled {
                    let _ = ack.send(WorkflowLaunchAck::Rejected {
                        ...
                        detail: "Background workflows are disabled for this session \
                                 ([workflows] enabled = false / INTELLIHELPER_WORKFLOWS=0 / remote flag)."
                            .into(),
                    });
                    continue;
                }
                ...
                if input.validate_only {
                    ...
                    let agent_budget = input
                        .agent_budget
                        .unwrap_or(workflow::DEFAULT_AGENT_BUDGET);
                    tokio::spawn(async move {
    ...
    let resolved_tool_overrides: std::sync::Arc<
        arc_swap::ArcSwapOption<intellihelper_sampling_types::ToolOverrides>,
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        max_turns,
        max_retries: intellihelper_sampler::resolve_max_retries(max_retries),
        ...
        vcs_kind: {
            ...
            match intellihelper_workspace::session::git::discover_git_root(root) {
                intellihelper_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                    intellihelper_workspace::session::git::detect_vcs_kind(&git_root)
                }
                _ => intellihelper_workspace::session::git::VcsKind::None,
            }
        },
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(intellihelper_tools::types::tool_index::ToolIndex(
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(intellihelper_tools::types::resources::PlanFilePath(plan_path))
            .await;
    }
    ...
    session
        ...
        .tool_bridge()
        .update_resource(
            intellihelper_tools::implementations::intellihelper_build::workflow::WorkflowLaunchHandle(
    ...
    if !background_workflows_enabled {
        session
            ...
            .tool_bridge()
            .update_resource(
                intellihelper_tools::implementations::intellihelper_build::update_goal::GoalUpdateHandle(
    ...
    if let Some(storage) = session.memory.storage() {
        ...
        tokio::task::spawn_local(async move {
            ...
            ) && let Ok(files) = storage.list_memory_files()
            {
                ...
                intellihelper_telemetry::session_ctx::log_event(
                    intellihelper_telemetry::memory_telemetry::MemoryReindex {
    ...
    }
    {
        ...
        use intellihelper_tools::implementations::intellihelper_build::ask_user_question::{
        ...
        tokio::task::spawn_local(async move {
            while let Some(mut request) = user_question_rx.recv().await {
                use intellihelper_tools::implementations::intellihelper_build::ask_user_question::AskUserQuestionMode;
    ...
    let telemetry_ctx = intellihelper_telemetry::session_ctx::TelemetryCtx::new(
    ...
    if let Some(metrics) = harness_metrics {
        ...
        tokio::spawn(async move {
            ...
            intellihelper_telemetry::session_ctx::log_event_dual(telemetry_enabled, ev);
        });
    }
    tokio::task::spawn_local(async move {
        intellihelper_telemetry::session_ctx::with_session_ctx(
            telemetry_ctx,
            run_session(
                session,
                cmd_rx,
...
> {
    let (init_tx, init_rx) = tokio::sync::oneshot::channel::<
        Result<SessionInitResult, intellihelper_agent::AgentBuildError>,
    >();
        ...
        .spawn(move || {
            ...
            let rt = match build_session_runtime() {
                ...
                Err(e) => {
                    ...
                    let _ = init_tx.send(Err(intellihelper_agent::AgentBuildError::RuntimeBuild(e)));
                    return;
                }
            };
    ...
    conversation.iter().any(|item| {
        ...
        let contains = |needle: &str| {
            u.content.iter().any(|part| {
                matches!(
                    part,
                    intellihelper_sampling_types::conversation::ContentPart::Text { text }
