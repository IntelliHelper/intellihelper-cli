...
use intellihelper_sampler::SamplerConfig;
use intellihelper_tools::implementations::intellihelper_build;
use intellihelper_tools::registry::types::ToolConfig;

/// Production intellihelper-build foreground command-timeout ceiling (seconds). The
...
impl BashToolConfig {
    ...
    pub(crate) fn to_bash_params_json(
        ...
        // production intellihelper-build opts up to 10h by sending it explicitly
...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `INTELLIHELPER_WEB_FETCH_PROXY` env > remote settings > None.
    ...
    /// Resolution: TOML > `INTELLIHELPER_WEB_FETCH_ALLOW_LOCAL` env > false.
...
impl WebFetchToolConfig {
    ...
    pub(crate) fn resolve_params(
    ...
    ) -> intellihelper_tools::implementations::intellihelper_build::web_fetch::WebFetchParams {
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("INTELLIHELPER_WEB_FETCH_PROXY"))
        ...
        let allow_local = self
            .allow_local
            .or_else(|| intellihelper_config::env_bool("INTELLIHELPER_WEB_FETCH_ALLOW_LOCAL"));
...
/// from `intellihelper_tools::registry::types::ToolsetConfig` which holds
...
///
/// Selects between the standard `IntelliHelperBuild` toolset (`read_file`,
/// `search_replace`, `grep`) and the anchor-based `IntelliHelperBuildHashline`
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "IntelliHelperBuildHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "IntelliHelperBuildHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"IntelliHelperBuild:read_file"));
        assert!(ids.contains(&"IntelliHelperBuild:search_replace"));
        assert!(ids.contains(&"IntelliHelperBuild:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"IntelliHelperBuildHashline:hashline_read"));
        assert!(ids.contains(&"IntelliHelperBuildHashline:hashline_edit"));
        assert!(ids.contains(&"IntelliHelperBuildHashline:hashline_grep"));
    }
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            intellihelper_agent::config::AgentDefinition::plan(),
            intellihelper_agent::config::AgentDefinition::explore(),
        ] {
            ...
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "IntelliHelperBuild:search_replace"),
            ...
            assert!(
                ids.contains(&"IntelliHelperBuildHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"IntelliHelperBuild:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"IntelliHelperBuildHashline:hashline_edit"),
    ...
    fn max_timeout_secs_defaults_to_production_10h() {
        ...
        assert_eq!(
            ...
            "production intellihelper-build must set the 10h foreground ceiling"
        );
    }
