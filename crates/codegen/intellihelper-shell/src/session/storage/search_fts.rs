...
impl SessionSearchIndex {
    ...
    fn open_with_journal_mode(
        ...
        // intellihelper generations share this DB (stable vs alpha); an equality check
    ...
    /// so `/resume` search by id returned nothing while `intellihelper --resume <id>`
    ...
    fn test_newer_version_index_is_tolerated_not_dropped() {
        let tmp = TempDir::new().unwrap();
        {
            ...
            index
                .upsert_doc(&test_doc("s1", "Rust debugging", "borrow checker"))
                .unwrap();
            // Simulate an index owned by a newer intellihelper generation that has
