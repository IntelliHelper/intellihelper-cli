...
A plugin bundles skills, slash commands, agents, hooks, and MCP servers into one installable unit. You get plugins from a marketplace, install the ones you want, and IntelliHelper loads what they add. To build and share your own, see [Create your own marketplace](#create-your-own-marketplace).
...
1. **Add the marketplace** so IntelliHelper can show what it offers. Nothing installs yet.
...
```bash
intellihelper plugin marketplace add my-org/team-plugins                  # GitHub shorthand (owner/repo)
intellihelper plugin marketplace add https://gitlab.com/acme/plugins.git  # any git host, include https:// and .git
intellihelper plugin marketplace add ./my-marketplace                     # a local folder
```

List, refresh, and remove sources with `intellihelper plugin marketplace list`, `intellihelper plugin marketplace update [<name>]`, and `intellihelper plugin marketplace remove <url>`.
...
Place this file at `~/.intellihelper/settings.json` or `~/.claude/settings.json`.
...
```bash
intellihelper plugin install deploy-tools --trust
```
...
Run `intellihelper plugin install <source>` without `--trust` and IntelliHelper shows the source, warns that installing activates the plugin's hooks, MCP servers, and skills, then stops. Add `--trust` to go ahead. Only install plugins from sources you trust (see [Trust and security](#trust-and-security)).

A plugin's skills appear in the slash menu. When a skill name is ambiguous, IntelliHelper shows the qualified form prefixed by the plugin name, for example `/deploy-tools:release`. To pick up a newly installed plugin, press `r` in the Plugins tab or start a new session.
...
```bash
intellihelper plugin list [--json] [--available]   # installed plugins (--available requires --json)
intellihelper plugin uninstall <name> [--confirm] [--keep-data]   # aliases: rm, remove
intellihelper plugin update [<name>]               # omit the name to update every plugin
intellihelper plugin enable <name>
intellihelper plugin disable <name>
intellihelper plugin details <name>                # show the plugin's component inventory
```
...
Set these in `~/.intellihelper/config.toml`:
...
Plugins are off by default, so list one in `enabled` to turn it on, or in `disabled` to discover it but skip loading it. Each entry is a plain plugin name (from `intellihelper plugin list`) or a full ID (`<scope>/<hash>/<name>`).

To hide the plugins and hooks interface entirely, set `disable_plugins = true` in `~/.intellihelper/pager.toml`.
...
Enabling a plugin loads its skills, commands, and agents. Trust is separate and controls whether a plugin's code runs: even when enabled, its hooks, MCP servers, and LSP servers stay inactive until you trust it. IntelliHelper trusts plugins in `~/.intellihelper/plugins/` automatically; project plugins in `.intellihelper/plugins/` require trust. Install with `--trust` to grant it:

```bash
intellihelper plugin install <source> --trust
```

Trusted plugin `.mcp.json` servers attach to the session like other MCP config, and child agents inherit them. Plugin agents (`plugin-name:agent-name`) use the parent session's MCP servers by default, the same as user agents under `~/.intellihelper/agents/`; restrict that with the `mcpInheritance` frontmatter (see [Subagents](16-subagents.md#mcp-inheritance)). For safety, plugin agent frontmatter cannot declare `mcpServers` or hooks, or set `permissionMode: bypassPermissions`.
...
3. **List the plugins in `.intellihelper-plugin/marketplace.json`.** This is the index IntelliHelper reads.
...
```
my-org-plugins/
  .intellihelper-plugin/
    marketplace.json      # the index IntelliHelper reads (required)
...
IntelliHelper reads the index from `.intellihelper-plugin/marketplace.json`. It also accepts `.intellihelper-plugin/plugin.json` and the `.claude-plugin/` equivalents.
...
Validate a plugin before publishing with `intellihelper plugin validate [<path>]`, and tag a release from the manifest version with `intellihelper plugin tag [<path>] [--push]`. Then point people at the repository. They add it once and install the plugins they want:

```bash
intellihelper plugin marketplace add my-org/my-org-plugins   # GitHub shorthand, a git URL, or a local path
intellihelper plugin install gdrive --trust
```
...
# Plugins stay off until enabled. List plugin names (from `intellihelper plugin list`)
# or full IDs (`<scope>/<hash>/<name>`).
[plugins]
enabled = ["gdrive"]
```

For a hands-off install with no per-person step, also place the plugin's files where IntelliHelper discovers and trusts them automatically: `~/.intellihelper/plugins/`, or a directory your device-management tool manages that you point to with `[plugins].paths`. Then enable them with `[plugins].enabled`.
...
You can also set `INTELLIHELPER_MARKETPLACE_REQUIRE_SHA=1`. Both only tighten the policy; neither turns it back off. Publish `sha` values in your marketplace's `plugin-index.json` so installs from it satisfy the rule. Plugins vendored directly inside a marketplace repository are copied from that repository's checkout, so pin them the same way, with `sha` values in `plugin-index.json`.
...
Marketplaces distribute IntelliHelper content: skills, commands, agents, hooks, and MCP server configurations. They do not install a program onto a machine. A skill or MCP server that runs a helper binary (for example a custom sign-in tool) still needs that binary delivered separately, bundled with your deployment or pushed through your device-management tool.

---

## Troubleshooting

**A plugin you installed isn't showing up.** Plugins are off until enabled. Check `intellihelper plugin list`, then add the plugin's name or ID to `[plugins].enabled`, or press `Space` on it in the Plugins tab. Reload with `r` in the Plugins tab or start a new session.

**A plugin's hooks or MCP servers don't run.** They stay inactive until the plugin is trusted. Reinstall with `--trust`, or place the plugin under `~/.intellihelper/plugins/` (auto-trusted). See [Trust and security](#trust-and-security).

**A skill or MCP server from a marketplace is missing.** Refresh the source with `intellihelper plugin marketplace update`, confirm the plugin is installed and enabled, and, if your organization restricts sources, check that the marketplace is still allowed (see [Distribute across an organization](#distribute-across-an-organization)). Some MCP servers require a sign-in and will not appear until you authenticate.
...
**See exactly what loaded.** Run `intellihelper inspect` (add `--json` for machine-readable output) to list every discovered plugin and the skills, agents, hooks, and MCP servers it provides, each labeled with its `plugin: <name>` source.
...
An optional `plugin.json` manifest can override paths or add metadata; without one, IntelliHelper discovers components from these standard directories. For example, a `team-tools` plugin might bundle a deploy skill, a code-review agent, pre-commit hooks, and a Linear MCP server, installed together in one step.
...
### Where IntelliHelper looks for plugins

IntelliHelper discovers plugins from these locations, in priority order. The `.claude/plugins/` equivalents also work, and when two plugins share a name the higher-priority one wins:

| Location | Scope | Trust |
|----------|-------|-------|
| `_meta.pluginDirs` (`session/new` / `session/load`) | Session, that session only | Trusted automatically |
| `--plugin-dir` (the `intellihelper agent … stdio` flag) | Process, that agent process only | Trusted automatically |
| `.intellihelper/plugins/` | Project, shared through version control | Requires trust |
| `~/.intellihelper/plugins/` | User, every project | Trusted automatically |
| `[plugins].paths` (config) | Custom directories you add | Depends on location |

The `_meta.pluginDirs` field on the `session/new` and `session/load` requests loads plugins for a single session; because the caller supplies the directory, those plugins are trusted automatically and do not persist after the session. `--plugin-dir` is the process-wide equivalent for a dedicated `intellihelper agent … stdio` process, repeatable (`intellihelper agent --no-leader --plugin-dir A --plugin-dir B stdio`), and ignored in leader mode, where the shared leader discovers its own plugins.
...
|----------|-------------|
| `INTELLIHELPER_PLUGIN_ROOT` | Absolute path to the plugin's installed directory. |
| `INTELLIHELPER_PLUGIN_DATA` | Absolute path to the plugin's writable data directory, for state, caches, and logs. |

IntelliHelper sets these and overrides any same-named value in the hook's `env` map (the `CLAUDE_PLUGIN_ROOT` and `CLAUDE_PLUGIN_DATA` aliases are set too). See the [Hooks guide](10-hooks.md) for every variable passed to hooks.
