...
mod tests {
    ...
    fn with_test_env(f: impl FnOnce()) {
        ...
        theme_cache::set(ThemeKind::IntelliHelperNight);
        f();
        system_appearance::clear_mock();
        theme_cache::reset_for_test();
    }
    ...
    fn suggest_args_explicit_active_when_not_auto() {
        with_test_env(|| {
            ...
            let items = cmd.suggest_args(&ctx, "").expect("should return items");
            let intellihelpernight = items
                .iter()
                .find(|i| i.insert_text == "intellihelpernight")
                .expect("intellihelpernight should be in list");
            assert!(
                intellihelpernight.description.contains("(active)"),
                "explicit theme should show (active), got: {}",
                intellihelpernight.description
            );
        });
    }
    ...
    fn run_explicit_dispatches_set_theme_action() {
        with_test_env(|| {
            ...
            let result = cmd.run(&mut ctx, "intellihelpernight");
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    assert_eq!(name, "intellihelpernight");
                }
                other => panic!("expected Action::SetTheme(\"intellihelpernight\"), got {other:?}"),
            }
        });
    }
    ...
    fn run_toggle_dispatches_set_theme_action() {
        with_test_env(|| {
            ...
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    // available[0] = IntelliHelperNight; next is available[1].
    ...
    fn run_alias_normalises_to_canonical() {
        with_test_env(|| {
            ...
            // "dark" is an alias for IntelliHelperNight.
            let result = cmd.run(&mut ctx, "dark");
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    assert_eq!(name, "intellihelpernight", "alias must normalise to canonical");
                }
                other => panic!("expected Action::SetTheme(\"intellihelpernight\"), got {other:?}"),
            }
        });
    ...
    fn preview_auto_applies_resolved_theme() {
        with_test_env(|| {
            ...
            // Default auto config maps Light -> IntelliHelperDay.
            assert_eq!(Theme::current_kind(), ThemeKind::IntelliHelperDay);
        });
    }
    ...
    fn preview_explicit_theme_applies_directly() {
        with_test_env(|| {
            theme_cache::set(ThemeKind::IntelliHelperNight);
            let cmd = ThemeCommand;
            cmd.preview_arg("intellihelperday");
            assert_eq!(Theme::current_kind(), ThemeKind::IntelliHelperDay);
        });
    }
    ...
    fn cancel_preview_restores_previous_kind() {
        with_test_env(|| {
            ...
            cmd.cancel_preview("intellihelpernight");
            assert_eq!(
                Theme::current_kind(),
                ThemeKind::IntelliHelperNight,
    ...
    fn cancel_preview_unknown_theme_is_no_op() {
        with_test_env(|| {
            theme_cache::set(ThemeKind::IntelliHelperDay);
            let cmd = ThemeCommand;
