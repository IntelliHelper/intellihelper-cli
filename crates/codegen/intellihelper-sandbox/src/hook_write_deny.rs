//! IntelliHelper-owned hook write-deny: plan, identity revalidation, and post-reexec checks.
//! Namespace lockdown is in [`crate::child_net`].

use std::path::{Path, PathBuf};

use intellihelper_config::{
    GlobalHookSource, ensure_intellihelper_hook_slots, missing_configured_sources,
    resolve_global_hook_sources,
};

#[cfg(target_os = "linux")]
use intellihelper_config::unique_ancestors_rootward;
#[cfg(unix)]
use intellihelper_config::validated_hook_json_files_for_sources;
...
impl From<intellihelper_config::GlobalHookSourceError> for HookWriteDenyError {
    fn from(e: intellihelper_config::GlobalHookSourceError) -> Self {
...
fn reject_hardlinked_files(sources: &[GlobalHookSource]) -> Result<(), HookWriteDenyError> {
    use std::os::unix::fs::MetadataExt;
    use intellihelper_config::GlobalHookSourceKind;
    for s in sources {
...
pub fn resolve_hook_write_deny_snapshot() -> Result<Vec<GlobalHookSource>, HookWriteDenyError> {
    ...
    let resolved =
        resolve_global_hook_sources(Some(intellihelper.as_path()), /* reject_symlinks */ true)?;
...
pub fn prepare_hook_write_deny(
    ...
    let intellihelper = intellihelper_home();
    ensure_intellihelper_hook_slots(intellihelper.as_path())?;
...
fn capture_dir_json_snapshot(dir: &Path) -> Result<DirJsonSnapshot, HookWriteDenyError> {
    use intellihelper_config::{list_direct_hook_json_files, validate_direct_hook_json_file};
