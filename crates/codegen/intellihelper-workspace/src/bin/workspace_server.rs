...
//!
//! Reads OIDC credentials from `~/.intellihelper/auth.json`, connects to a
...
use url::Url;
use intellihelper_workspace::config::WorkspaceServerMetadata;
use intellihelper_workspace::daemonize;
use intellihelper_workspace::diag_server::{self, DiagHandle, ErrorClass};
use intellihelper_workspace::error::WorkspaceError;
use intellihelper_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
const SERVICE_NAME: &str = "prod_intellihelper_workspace";
...
struct Args {
    ...
    /// `INTELLIHELPER_WORKSPACE_UPLOAD_QUEUE_ENABLED` env var to `false`) to fall back to
    /// the legacy inline path. Accepts `true`/`false`.
    #[arg(
        long,
        env = "INTELLIHELPER_WORKSPACE_UPLOAD_QUEUE_ENABLED",
    ...
    /// Trust project-scoped LSP servers from `<repo>/.intellihelper/lsp.json`.
    /// Defaults off; sandbox opts in only after workspace trust is established.
    #[arg(
        long,
        env = "INTELLIHELPER_WORKSPACE_PROJECT_LSP_TRUSTED",
    ...
    /// Override with `INTELLIHELPER_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "INTELLIHELPER_WORKSPACE_CONFINE_FS_TO_ROOT",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
...
/// `/usr/local/bin/intellihelper-preview-proxy` child (see `cli.rs` for the proxy's
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    tracing_subscriber::registry()
        ...
        .with(donating.clone())
        .init();
    let direct_otlp = match std::env::var("INTELLIHELPER_WORKSPACE_OTLP_ENDPOINT") {
        Ok(endpoint) if !endpoint.is_empty() => {
            match intellihelper_tracing::init_fastrace(endpoint.clone(), SERVICE_NAME.to_owned(), None) {
                Ok(()) => {
    ...
    {
        use intellihelper_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("INTELLIHELPER_SANDBOX_PROFILE").ok() {
            Some(val) => {
                let parsed = val
                    .parse::<ProfileName>()
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(value = %val,
                        "Unrecognized INTELLIHELPER_SANDBOX_PROFILE, defaulting to workspace");
                    ProfileName::Workspace
                } else {
                    parsed
                }
            }
            None if intellihelper_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
            None => ProfileName::Workspace,
        };
        let profile_name = profile.to_string();
        if profile == ProfileName::Off {
            tracing::info!(profile = %profile_name, "Sandbox explicitly disabled via INTELLIHELPER_SANDBOX_PROFILE=off");
        } else {
            ...
            sandbox.install();
            let active = intellihelper_sandbox::is_active();
            ...
            tracing::info!(
                ...
                active,
                restrict_network_at_known_linux_launches = intellihelper_sandbox::should_restrict_child_network(),
                "{status_msg}"
            );
        }
    }
    let auth_provider = intellihelper_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(
        hub_url = %url,
        cwd = %cwd.display(),
    ...
    let session_id = std::env::var("INTELLIHELPER_SESSION_ID").ok();
    ...
    intellihelper_workspace::init_metrics();
    let ws_handle = match intellihelper_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = intellihelper_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            intellihelper_workspace::handle::DrainReason::Sigterm,
        )
        .await;
...
mod tests {
    ...
    fn project_lsp_trust_defaults_off_and_is_opt_in() {
        unsafe { std::env::remove_var("INTELLIHELPER_WORKSPACE_PROJECT_LSP_TRUSTED") };
        let args = Args::try_parse_from(["xai-workspace-server"]).unwrap();
