...
impl SessionActor {
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch, commit) = tokio::join!(
            intellihelper_workspace::session::git::get_worktree_info(cwd),
            intellihelper_workspace::session::git::get_branch(cwd),
            intellihelper_workspace::session::git::get_current_commit(cwd),
        );
        ...
        let params = intellihelper_workspace::session::git::GitHeadChanged {
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<intellihelper_tools::implementations::intellihelper_build::task::types::SubagentOutstandingReply>
    {
    ...
    pub(super) fn classify_turn_error(err: &acp::Error) -> String {
        use intellihelper_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn stop_failure_error_type(
        err: &acp::Error,
    ) -> intellihelper_hooks::event::StopFailureKind {
        use intellihelper_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn classify_install_error(
        err: &intellihelper_agent::plugins::install_registry::InstallError,
    ) -> String {
