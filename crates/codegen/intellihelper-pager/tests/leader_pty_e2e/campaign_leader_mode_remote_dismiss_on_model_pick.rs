...
#[ignore = "PTY e2e; run with cargo test -p intellihelper-pager --test leader_pty_e2e -- --ignored --test-threads=1"]
async fn campaign_leader_mode_remote_dismiss_on_model_pick() {
    ...
    // socket under the shared INTELLIHELPER_HOME so every spawn elects/attaches to the
    ...
    let socket = intellihelper_home.join("leader-e2e.sock");
    ...
    let state_path = intellihelper_home.join("campaigns_state.json");
    ...
    let config = std::fs::read_to_string(intellihelper_home.join("config.toml")).expect("read config.toml");
    assert!(
