...
use intellihelper_shell::tools::{TodoItem, TodoStatus};
...
mod tests {
    ...
    fn render_per_model_breakdown_shows_each_model() {
        let mut goal = make_goal();
        goal.live_tokens_by_model = vec![("intellihelper-4".into(), 12_300), ("intellihelper-3-mini".into(), 8_000)];
        let text = render_to_text(&goal);
        assert!(text.contains("intellihelper-4"), "first model must render:\n{text}");
        assert!(
            text.contains("intellihelper-3-mini"),
