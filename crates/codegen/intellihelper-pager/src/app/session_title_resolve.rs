...
pub(crate) fn title_miss_hint(arg: &str) -> String {
    format!(
        "no session id or title matched {arg:?} for this directory; \
         try `intellihelper sessions search {arg:?}`"
    )
}
        ...
        _ => {
            ...
            anyhow::bail!(
                "Multiple sessions match title {:?}:\n{listing}\n\
                 Resume by session id instead: intellihelper --resume <session-id>",
                arg.trim()
            );
        }
    }
...
pub(crate) fn presandbox_resume_target(
    ...
    if let Some(local_id) = intellihelper_shell::session::resolve_local_session(arg, cwd) {
        return Ok(PinnedResumeTarget::Id(local_id));
    }
    if intellihelper_shell::session::resolve_local_session_any_cwd(arg).is_some() {
        return Ok(PinnedResumeTarget::Id(arg.to_string()));
    }
    let summaries = intellihelper_shell::session::persistence::local_summaries_for_cwd_sync(cwd)
        .map_err(|e| {
