//! Leader-mode (`intellihelper agent --leader stdio`) test harness.
//!
...
use crate::env::intellihelper_binary;
...
/// (version-skew) test. Falls back to [`intellihelper_binary`]'s resolution.
pub const LEADER_BINARY_ENV: &str = "INTELLIHELPER_BINARY_LEADER";

/// Env var naming the binary for the second (usually newer) client in a
/// two-binary test. Falls back to [`intellihelper_binary`]'s resolution.
pub const CLIENT_BINARY_ENV: &str = "INTELLIHELPER_BINARY_CLIENT";
...
/// A `intellihelper agent --leader stdio` client subprocess speaking ACP over pipes.
...
impl LeaderFixture {
    ...
    pub async fn start(
        ...
        Self::start_with_binary(&intellihelper_binary(), server, cwd, sandbox).await
    }
    ...
    async fn start_with_binary_base_url_timeout(
        ...
        let socket = sandbox.intellihelper_home().join("leader.sock");
        let lock = sandbox.intellihelper_home().join("leader.lock");
        ...
        cmd.envs(tty_utils::pager_env())
            .env("INTELLIHELPER_CLI_CHAT_PROXY_BASE_URL", base_url)
            .env("INTELLIHELPER_XAI_API_BASE_URL", base_url)
            .env("INTELLIHELPER_MODELS_BASE_URL", base_url)
            .env("INTELLIHELPER_FEEDBACK_BASE_URL", base_url)
            .env("INTELLIHELPER_TRACE_UPLOAD_URL", base_url)
            .env("XAI_API_KEY", "test-key-for-ci")
            .env("INTELLIHELPER_LEADER_SOCKET", &socket)
            .env("RUST_LOG", "intellihelper_shell=debug");
        let log_path = sandbox.intellihelper_home().join("leader.log");
        ...
        let pid = child.id();
        let tree = match TestProcessTree::try_attach(pid, "persistent intellihelper test leader") {
            Ok(tree) => tree,
            Err(error) => {
                let _ = child.kill();
    ...
    async fn spawn_with_binary_and_socket(
        ...
        let mut process = TestProcess::spawn(
            ...
            TestProcessConfig::new()
                .label("intellihelper leader stdio client")
                ...
                .env("INTELLIHELPER_TRACE_UPLOAD_URL", base_url)
                .env("XAI_API_KEY", "test-key-for-ci")
                .env("INTELLIHELPER_LEADER_SOCKET", leader_socket)
                .env("RUST_LOG", "intellihelper_shell=debug"),
        )
        .map_err(|error| {
            io::Error::new(
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".intellihelper").join("leader.lock")
}
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".intellihelper").join("leader.log")).unwrap_or_default()
}
