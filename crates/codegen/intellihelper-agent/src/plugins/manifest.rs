...
//! 1. `.intellihelper-plugin/plugin.json`
...
const MANIFEST_PATHS: &[&str] = &[
    "plugin.json",
    ".intellihelper-plugin/plugin.json",
...
///
/// Replaces `${INTELLIHELPER_PLUGIN_ROOT}`, `${CLAUDE_PLUGIN_ROOT}`,
/// `${INTELLIHELPER_PLUGIN_DATA}`, and `${CLAUDE_PLUGIN_DATA}` with the provided values.
///
/// Delegates to [`intellihelper_tools::util::substitute_plugin_tokens`], the single
...
pub fn substitute_env_vars(s: &str, plugin_root: &str, plugin_data: &str) -> String {
    intellihelper_tools::util::substitute_plugin_tokens(s, Some(plugin_root), Some(plugin_data))
}
...
mod tests {
    ...
    fn load_manifest_fallback_paths() {
        ...
        std::fs::create_dir_all(plugin_root.join(".intellihelper-plugin")).unwrap();

        // Write manifest in .intellihelper-plugin/ fallback location
        std::fs::write(
            plugin_root.join(".intellihelper-plugin/plugin.json"),
    ...
    fn substitute_env_vars_replaces_all() {
        let input = "${INTELLIHELPER_PLUGIN_ROOT}/bin:${CLAUDE_PLUGIN_ROOT}/lib:${INTELLIHELPER_PLUGIN_DATA}/cache";
