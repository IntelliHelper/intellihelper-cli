    ...
    pub(super) fn handle_mouse(&mut self, mouse: &MouseEvent) -> InputOutcome {
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if self.hit_session_usage.contains(mouse.column, mouse.row) {
                    // Session `N tokens` badge → full input / output / cached ledger.
                    return InputOutcome::Action(Action::ShowUsage);
                }
                if self.hit_optimize_input.contains(mouse.column, mouse.row) {
                    return InputOutcome::Action(Action::OptimizeInput);
                }
                    ...
                    && !self.pos_occluded(mouse.column, mouse.row)
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        intellihelper_telemetry::events::AnnouncementCtaSurface::Banner,
                    ));
                }
                if self
                    .plugin_cta
                        ...
                        &self.plugin_cta.phase
                {
                    ...
                    if let Err(e) = intellihelper_shell::config::add_dismissed_plugin_cta(&plugin_id) {
                    ...
                    intellihelper_telemetry::session_ctx::log_event(
                        intellihelper_telemetry::events::PluginCtaDismissed {
                ...
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        intellihelper_telemetry::events::AnnouncementCtaSurface::Header,
                    ));
                }
                        ...
                        if let Some(idx) = hit_idx {
                                    ...
                                    .entry_screen_area(idx, self.pane_areas.scrollback)
                            {
                                ...
                                if click_row >= url_row {
                                    ...
                                    intellihelper_telemetry::session_ctx::log_event(intellihelper_telemetry::events::CreditLimitUpsellClicked {
            ...
            MouseEventKind::Moved => {
                ...
                changed |= self.hit_session_usage.update_hover(mouse.column, mouse.row);
                ...
                changed |= self.hit_optimize_input.update_hover(mouse.column, mouse.row);
                changed |= self
                    .hit_plan_approval_status
