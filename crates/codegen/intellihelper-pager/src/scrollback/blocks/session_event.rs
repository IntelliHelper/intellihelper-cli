    ...
    pub fn message(&self) -> String {
        match self {
            ...
            SessionEvent::DiskFull => {
                intellihelper_shell::extensions::notification::DISK_FULL_USER_MESSAGE.to_string()
            }
...
mod tests {
    ...
    fn model_unavailable_with_switch() {
        let event = SessionEvent::ModelUnavailable {
            previous_model_id: "intellihelper-4.5".into(),
            new_model_id: "intellihelper-build".into(),
            reason: "Model \"intellihelper-4.5\" is no longer available.".into(),
        };
        assert_eq!(
            event.message(),
            "Model \"intellihelper-4.5\" is no longer available. Switched to \"intellihelper-build\"."
        );
    }
