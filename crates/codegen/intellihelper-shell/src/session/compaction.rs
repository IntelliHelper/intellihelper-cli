...
use intellihelper_sampling_types::{ApiBackend, ConversationItem};
...
/// Override with `INTELLIHELPER_PREFIRE_LEAD_PERCENT`.
const DEFAULT_PREFIRE_LEAD_PERCENT: u64 = 10;
fn prefire_lead_percent() -> u64 {
    std::env::var("INTELLIHELPER_PREFIRE_LEAD_PERCENT")
        .ok()
...
mod two_pass_prefire_helper_tests {
    ...
    use intellihelper_sampling_types::ConversationItem;
    #[test]
    ...
    fn prefire_lead_percent_defaults_to_10() {
        unsafe { std::env::remove_var("INTELLIHELPER_PREFIRE_LEAD_PERCENT") };
...
impl SessionActor {
    ...
    async fn run_prefire_pass1_inner(self: &Arc<Self>) -> PrefirePass1Run {
        ...
        if std::env::var("INTELLIHELPER_DEBUG_TWO_PASS_FAIL_PASS1")
            .is_ok_and(|v| matches!(v.trim(), "1" | "true" | "yes" | "on"))
        {
            tracing::info!(
                target: "two_pass",
                "two_pass: DEBUG INTELLIHELPER_DEBUG_TWO_PASS_FAIL_PASS1 — prefire pass1 produces no cache"
            );
...
impl SessionActor {
    ...
    pub(crate) async fn run_compact(
        ...
        if let Err(e) = self
            .run_compact_inner(
                user_context,
                None,
                intellihelper_telemetry::events::CompactionTrigger::Manual,
            )
            .await
        {
            let span = tracing::Span::current();
    ...
    async fn suppress_auto_compaction(
            ...
            .is_ok()
        {
            ...
            intellihelper_telemetry::session_ctx::log_event(
                intellihelper_telemetry::events::AutoCompactSuppressed {
    ...
    pub(crate) async fn surface_compact_auth_failure(&self, err: acp::Error) -> acp::Error {
        ...
        acp::Error::auth_required().data(crate::sampling::error::terminal_error_data(
            message,
            Some(401),
            intellihelper_sampler::SamplingErrorKind::Auth,
        ))
    }
    ...
    async fn run_compact_inner(
        ...
        trigger: intellihelper_telemetry::events::CompactionTrigger,
    ) -> Result<(), acp::Error> {
        let (cancel, _cancel_scope) = self.compaction.cancel.enter();
        ...
        let trigger_str = match trigger {
            intellihelper_telemetry::events::CompactionTrigger::Manual => "manual",
            intellihelper_telemetry::events::CompactionTrigger::Auto => "auto",
        };
        let sampling_config = self.chat_state_handle.get_sampling_config().await;
        let context_window = sampling_config
            .as_ref()
        ...
        let compaction = intellihelper_telemetry::events::CompactionScope::begin(
            trigger,
            tokens_before,
            context_window,
            model_id.clone(),
        ...
        self.dispatch_hook(
            intellihelper_hooks::event::HookEventName::PreCompact,
            intellihelper_hooks::event::HookPayload::PreCompact {
        ...
        let effective_tool_defs: Vec<intellihelper_sampling_types::ToolDefinition> = self
        ...
        let compaction_tools: Vec<intellihelper_sampling_types::ToolSpec> = effective_tool_defs
            .into_iter()
            .map(intellihelper_sampling_types::ToolSpec::from)
            .collect();
        let compaction_hosted_tools: Vec<intellihelper_sampling_types::HostedTool> =
        ...
        let auto_trigger = matches!(trigger, intellihelper_telemetry::events::CompactionTrigger::Auto);
        let wall_clock_budget_secs = self
            .agent
            .borrow()
            .compaction_policy()
        ...
        let fr_config = intellihelper_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            match intellihelper_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(intellihelper_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                        acp::Error::internal_error().data("compact failed: nothing to compact"),
                    );
                    break;
                }
                Err(intellihelper_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(intellihelper_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    ...
                    if context_overflow {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            intellihelper_telemetry::session_ctx::log_event(
                                intellihelper_telemetry::events::CompactionRetryDegraded {
                                    trigger,
            ...
            if use_short_prompt {
                ...
                let ctx = {
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                intellihelper_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                intellihelper_tools::computer::types::TaskKind::Bash => {
                    ...
                    let todos = {
                        ...
                        use intellihelper_tools::types::resources::State;
        ...
        let memory_ref: Option<&dyn intellihelper_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn intellihelper_tools::types::memory_backend::MemoryBackend)
            };
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        intellihelper_sampling_types::ContentPart::Text { text } => {
        ...
        if self.memory.is_enabled() {
            tracing::info!(target: intellihelper_telemetry::memory_log::TARGET, "MEMORY_COMPACT: post-compaction reset, next turn re-checks injection (search only if no block persisted)");
        }
        let _ = self
            .notifications
            .persistence_tx
        ...
        self.dispatch_hook(
            intellihelper_hooks::event::HookEventName::PostCompact,
            intellihelper_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
            None,
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &intellihelper_sampler::SamplingErrorInfo,
    ) -> bool {
        if self
            .compaction
            .auto_compact_suppressed
    ...
    pub(crate) async fn run_compact_only(
        ...
        intellihelper_telemetry::session_ctx::log_event(intellihelper_telemetry::events::AutoCompactFired {
        ...
        let result = self
            .run_compact_inner(
                None,
                None,
                intellihelper_telemetry::events::CompactionTrigger::Auto,
            )
            .await;
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<intellihelper_sampling_types::ToolSpec>,
        user_context: Option<&str>,
...
mod inline_auto_compact_flow_tests {
    ...
    use tokio::sync::mpsc;
    use intellihelper_paths::AbsPathBuf;
    use intellihelper_workspace::file_system::MockFs;
    use intellihelper_workspace::permission::PermissionHandle;
    #[derive(Debug)]
    ...
    async fn create_test_actor(
        ...
        let chat_state_handle = chat_state::ChatStateActor::spawn(
            vec![],
            intellihelper_sampling_types::SamplingConfig {
        ...
        SessionActor {
            ...
            vcs_kind: intellihelper_workspace::session::git::VcsKind::Git,
            ...
            sampler_handle: intellihelper_sampler::SamplerHandle::noop(),
    ...
    async fn forked_prefix_released_under_pressure_and_stays_released() {
        use crate::session::compaction_config::SUPPRESS_NONE;
        use std::sync::atomic::Ordering::Relaxed;
        use intellihelper_test_support::MockInferenceServer;
        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) = mpsc::unbounded_channel();
    ...
    fn classify_suppress_reason_maps_error_text() {
        ...
        assert_eq!(
            classify("API error (status 402 Payment Required): IntelliHelper CLI usage balance exhausted"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
            classify("IntelliHelper CLI usage limit reached"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
    ...
    fn api_error_with_context_window(context_window: u64) -> intellihelper_sampler::SamplingErrorInfo {
        intellihelper_sampler::SamplingErrorInfo {
            kind: intellihelper_sampler::SamplingErrorKind::Api,
    ...
    async fn test_compact_on_error_noop_without_model_metadata() {
        ...
        local
            .run_until(async {
                ...
                let err = intellihelper_sampler::SamplingErrorInfo {
                    kind: intellihelper_sampler::SamplingErrorKind::Api,
                    status_code: Some(400),
                    message: "prompt is too long".to_string(),
                    is_retryable: false,
