//! Data APIs for `intellihelper models`. Clients own display.
...
/// Status for the `intellihelper models` banner (display order ≠ sampling priority; see [`AuthStatus::resolve`]).
#[derive(Debug, PartialEq, Eq)]
pub enum AuthStatus {
    ApiKey,
    /// Auth host from `intellihelper_ws_origin` (scheme stripped).
    LoggedIn(String),
...
impl AuthStatus {
    ...
    pub fn resolve(agent_config: &AgentConfig) -> Self {
        ...
        if agent_config.create_auth_manager().current().is_some() {
            let origin = &agent_config.grok_com_config.intellihelper_ws_origin;
            let host = origin
                .strip_prefix("https://")
...
mod tests {
    ...
    ///
    /// Uses `INTELLIHELPER_AUTH_PATH` (not `INTELLIHELPER_HOME`) so a OnceLock-cached real home
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 7]) {
        ...
        let guards = [
            ...
            EnvGuard::unset("INTELLIHELPER_AUTH"),
            EnvGuard::set("INTELLIHELPER_AUTH_PATH", auth_path.to_str().unwrap()),
            EnvGuard::unset("INTELLIHELPER_DEPLOYMENT_KEY"),
            EnvGuard::unset("INTELLIHELPER_WS_ORIGIN"),
            EnvGuard::unset("INTELLIHELPER_DISABLE_API_KEY_AUTH"),
        ];
        (dir, guards)
    }
    ...
    fn resolve_oauth_session() {
        let (_dir, _g) = isolate_auth_sources();
        let token = IntelliHelperAuth {
            key: "session-token".into(),
            auth_mode: AuthMode::WebLogin,
            ..IntelliHelperAuth::test_default()
        };
        let json = serde_json::to_string(&token).unwrap();
        let _auth = EnvGuard::set("INTELLIHELPER_AUTH", &json);

        assert_eq!(
            AuthStatus::resolve(&Config::default()),
            AuthStatus::LoggedIn("intellihelper.com".to_owned())
        );
    }
