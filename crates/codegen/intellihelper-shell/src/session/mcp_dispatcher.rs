...
//!
//! Receives [`intellihelper_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`intellihelper_mcp::liveness`]),
//! - the [`intellihelper_mcp::servers::IntelliHelperClientHandler`] (server-pushed
...
pub struct McpServerStatusPayload {
    ...
    /// or `local` (user `.intellihelper/config.toml`).
...
pub enum McpServerStatus {
    /// Client is in [`intellihelper_mcp::servers::ClientStateKind::Ready`]
...
/// [`intellihelper_mcp::servers::McpClientEventKind`]). Used as the
...
pub(crate) fn build_payload(
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        // NeedsAuth ("visit intellihelper.com"), not a generic Unavailable, so a
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && intellihelper_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
...
///
/// `gateway` is a [`acp_lib::AcpAgentGatewaySender`] (forwarded
...
mod tests {
    ...
    /// intellihelper.com"), NOT a generic `Unavailable`. Keys on the shared
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        ...
        use intellihelper_mcp::servers::{McpClient, McpState};
