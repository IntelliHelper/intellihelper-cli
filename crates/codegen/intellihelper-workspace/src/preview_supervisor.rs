...
//! spawns the unchanged `/usr/local/bin/intellihelper-preview-proxy` binary as a
...
pub const PREVIEW_PROXY_BIN_PATH: &str = "/usr/local/bin/intellihelper-preview-proxy";
...
/// `intellihelper_workspace_preview_proxy_restart_total{reason}` — (re)start events the
...
static PREVIEW_PROXY_RESTART_TOTAL: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "intellihelper_workspace_preview_proxy_restart_total",
...
impl PreviewArgs {
    ...
    /// `intellihelper-preview-proxy/src/cli.rs`). Absent options and a false
...
/// Build the unspawned proxy command. Secrets (`INTELLIHELPER_SERVER_KEY` /
/// `INTELLIHELPER_SESSION_ID`) reach the proxy by env inheritance — never argv.
fn build_preview_command(cfg: &PreviewArgs) -> io::Result<tokio::process::Command> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // Raw pre_exec, NOT tty_utils::detach_command: the proxy must stay in
...
/// `intellihelper-preview-proxy`'s `/__control/activity` route).
...
/// the default in `intellihelper-preview-proxy/src/cli.rs`).
...
mod tests {
    ...
    fn to_argv_maps_every_flag_to_the_proxy_cli_names() {
        // Flag names must match intellihelper-preview-proxy/src/cli.rs exactly.
        assert_eq!(
            sample_cfg().to_argv(),
            vec![
                "--preview-port",
    ...
    #[cfg(target_os = "linux")]
    const PDEATHSIG_HELPER_ENV: &str = "INTELLIHELPER_PDEATHSIG_HELPER";
    #[cfg(target_os = "linux")]
