...
/// Display label for a section header, e.g. `"Managed by intellihelper.com (3)"`.
pub fn section_label(section: &McpSectionId, count: usize) -> String {
    match section {
        McpSectionId::Managed => format!("Managed by intellihelper.com ({count})"),
        McpSectionId::Plugin(name) => format!("Plugin: {name} ({count})"),
        McpSectionId::Local => format!("Local ({count})"),
    }
}

/// Base intellihelper.com connectors URL (no team). Prefer [`managed_connectors_url`] when opening.
...
mod tests {
    ...
    fn section_description_lines_managed_includes_connectors_url() {
        ...
        assert_eq!(lines[1], "[intellihelper.com/connectors]");
        assert!(
        ...
        assert_eq!(with_team[1], "[intellihelper.com/connectors?teamId=team-1]");
    }

    #[test]
    fn managed_connectors_url_display_strips_scheme() {
        assert_eq!(managed_connectors_url_display(None), "intellihelper.com/connectors");
        assert_eq!(
            managed_connectors_url_display(Some("team-uuid-1")),
            "intellihelper.com/connectors?teamId=team-uuid-1"
        );
    }
