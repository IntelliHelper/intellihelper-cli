...
use intellihelper_agent::prompt::skills::{
...
fn discover_auto_sources(cwd: &str, skills: &[SkillInfo]) -> Vec<(String, usize)> {
    ...
    let local_dir_names: &[&str] = if imported {
        &[".intellihelper", ".agents"]
    } else {
        &[".intellihelper", ".agents", ".claude"]
    };
    ...
    for subdir in &subdirs {
        try_add_source(intellihelper_home.join(subdir), None);
    }
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        "x.ai/skills/add" => {
            ...
            .await
            {
                intellihelper_telemetry::session_ctx::log_event(
                    intellihelper_telemetry::events::SkillAdded {
                        added_count: 0,
                        total_skills: 0,
                        success: false,
                    },
            ...
            intellihelper_telemetry::session_ctx::log_event(intellihelper_telemetry::events::SkillAdded {
        ...
        "x.ai/skills/remove" => {
            ...
            {
                intellihelper_telemetry::session_ctx::log_event(
                    intellihelper_telemetry::events::SkillRemoved { success: false },
                );
            ...
            intellihelper_telemetry::session_ctx::log_event(intellihelper_telemetry::events::SkillRemoved {
                success: true,
            });
