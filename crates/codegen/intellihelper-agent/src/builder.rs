...
use std::sync::Arc;
use intellihelper_tools::bridge::ToolBridge;
use intellihelper_tools::computer::types::{AsyncFileSystem, TerminalBackend};
...
/// The IntelliHelper [`ToolKind`] a vendor-compat `tools:` allowlist entry resolves to, so
/// a plugin's upstream allowlist still binds. Backed by the shared vendor-to-IntelliHelper
/// tool registry in `intellihelper-tools` (also used by the hook matcher).
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    intellihelper_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.intellihelper/worktrees/project/fork-...-overlay`) that must stay
    ...
    parent_scheduler_handle:
        Option<intellihelper_tools::implementations::intellihelper_build::scheduler::types::SchedulerHandle>,
    ...
    memory_backend: Option<Arc<dyn intellihelper_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: intellihelper_tools::implementations::web_search::WebSearchConfig,
    ...
    backend_search: bool,
    web_fetch_config: intellihelper_tools::implementations::intellihelper_build::web_fetch::WebFetchConfig,
    lsp: Option<std::sync::Arc<dyn intellihelper_tools::implementations::lsp::LspBackend>>,
    image_gen_config: intellihelper_tools::implementations::intellihelper_build::image_gen::ImageGenConfig,
    video_gen_config: intellihelper_tools::implementations::intellihelper_build::video_gen::VideoGenConfig,
    app_builder_deployer_config:
        intellihelper_tools::implementations::intellihelper_build::deploy_app::AppBuilderDeployerConfig,
    ...
    compat: intellihelper_tools::types::compat::CompatConfig,
    ...
    api_key_provider: Option<intellihelper_tools::types::SharedApiKeyProvider>,
    attribution_callback: Option<intellihelper_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in intellihelper-shell).
    ...
    preloaded_skills: Option<Vec<intellihelper_tools::implementations::skills::types::SkillInfo>>,
}
/// Ensure plan mode tools (`enter_plan_mode`, `exit_plan_mode`,
/// `ask_user_question`) are present in the tool config.
fn ensure_plan_mode_tools(tool_config: &mut intellihelper_tools::registry::types::ToolServerConfig) {
    use intellihelper_tools::implementations::intellihelper_build;
    let existing: std::collections::HashSet<&str> =
        tool_config.tools.iter().map(|tc| tc.id.as_str()).collect();
    let missing_enter = !existing.contains("IntelliHelperBuild:enter_plan_mode");
    let missing_exit = !existing.contains("IntelliHelperBuild:exit_plan_mode");
    let missing_ask = !existing.contains("IntelliHelperBuild:ask_user_question");
    drop(existing);
    if missing_enter {
        tool_config
            .tools
            .push((&intellihelper_build::EnterPlanModeTool).into());
    }
    if missing_exit {
        tool_config
            .tools
            .push((&intellihelper_build::ExitPlanModeTool).into());
    }
    if missing_ask {
        tool_config
            .tools
            .push((&intellihelper_build::AskUserQuestionTool).into());
    }
}
...
fn merge_tool_params(
    tool_config: &mut intellihelper_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(intellihelper_tools::computer::local::LocalFs),
            notification_handle,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<intellihelper_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn intellihelper_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_parent_scheduler_handle(
        mut self,
        handle: intellihelper_tools::implementations::intellihelper_build::scheduler::types::SchedulerHandle,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: intellihelper_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    /// `INTELLIHELPER_WEB_FETCH` env var.
    pub fn with_web_fetch_config(
        mut self,
        config: intellihelper_tools::implementations::intellihelper_build::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn intellihelper_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    pub fn with_image_gen_config(
        mut self,
        config: intellihelper_tools::implementations::intellihelper_build::image_gen::ImageGenConfig,
    ) -> Self {
    ...
    pub fn with_video_gen_config(
        mut self,
        config: intellihelper_tools::implementations::intellihelper_build::video_gen::VideoGenConfig,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: intellihelper_tools::implementations::intellihelper_build::deploy_app::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: intellihelper_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// `intellihelper_sampler::SamplerConfig::attribution_callback` so
    /// all 401s share the same `AuthManager` reference and land in
    /// the same Axiom dataset.
    pub fn with_attribution_callback(
        mut self,
        callback: intellihelper_tools::SharedAttributionCallback,
    ) -> Self {
    ...
    /// Set public model slugs advertised in the IntelliHelperBuild Task description.
    ...
    ///
    /// When disabled, `IntelliHelperBuild:ask_user_question` is stripped from the
    ...
    /// Without this, only auto-discovered skills (cwd/.intellihelper/skills, ~/.intellihelper/skills)
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.web_search_config.is_enabled() {
                use intellihelper_tools::implementations::intellihelper_build;
                tool_config.tools.push((&intellihelper_build::WebSearchTool).into());
            }
            if self.web_fetch_config.is_enabled() {
                use intellihelper_tools::implementations::intellihelper_build;
                tool_config.tools.push((&intellihelper_build::WebFetchTool).into());
            }
            if self.lsp.is_some() {
                tool_config
                    .tools
                    .push((&intellihelper_tools::implementations::intellihelper_build::LspTool).into());
            }
            if self.image_gen_config.image_gen_enabled() {
                tool_config
                    .tools
                    .push((&intellihelper_tools::implementations::intellihelper_build::ImageGenTool).into());
            }
            if self.image_gen_config.image_edit_enabled() {
                tool_config
                    .tools
                    .push((&intellihelper_tools::implementations::intellihelper_build::ImageEditTool).into());
            }
            if self.video_gen_config.is_enabled() {
                tool_config
                    .tools
                    .push((&intellihelper_tools::implementations::intellihelper_build::ImageToVideoTool).into());
                tool_config.tools.push(
                    (&intellihelper_tools::implementations::intellihelper_build::ReferenceToVideoTool).into(),
                );
            }
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&intellihelper_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
            ensure_plan_mode_tools(&mut tool_config);
        }
        if self.memory_backend.is_none() {
            let intellihelper_build_ns = intellihelper_tools::types::tool::ToolNamespace::IntelliHelperBuild.to_string();
            let mem_search_id = format!(
                "{intellihelper_build_ns}:{}",
                intellihelper_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{intellihelper_build_ns}:{}",
                intellihelper_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if !self.ask_user_question_enabled {
            let ask_user_id = format!(
                "{}:ask_user_question",
                intellihelper_tools::types::tool::ToolNamespace::IntelliHelperBuild,
            );
        ...
        if task_stripped {
            use intellihelper_tools::types::tool::ToolNamespace;
            ...
            if !has_satisfier(ToolNamespace::IntelliHelperBuild, "run_terminal_cmd", true)
                && !has_satisfier(ToolNamespace::IntelliHelperBuildConcise, "run_terminal_cmd", true)
        ...
        if let intellihelper_tools::implementations::intellihelper_build::web_fetch::WebFetchConfig::Enabled {
            ref params,
        } = self.web_fetch_config
            && let Ok(params_value) = serde_json::to_value(params)
            && let Some(obj) = params_value.as_object()
        {
            merge_tool_params(&mut tool_config, &["IntelliHelperBuild:web_fetch"], obj);
        }
        if let Some(ref bash_params) = self.bash_params_json {
            merge_tool_params(
                ...
                &[
                    ...
                    "IntelliHelperBuildConcise:run_terminal_cmd",
                ],
                bash_params,
            );
        }
        if let Some(ref ask_params) = self.ask_user_question_params_json {
            merge_tool_params(
                &mut tool_config,
                &["IntelliHelperBuild:ask_user_question"],
                ask_params,
            );
        }
        ...
        if !definition.tools.is_empty() {
            ...
            if unresolved.is_empty() {
                ...
                tracing::warn!(
                    ...
                    "tools allowlist had unmappable entries; keeping full intellihelper toolset"
                );
            }
        }
        tool_config
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                intellihelper_tools::types::resources::TruncationCfg(
                    intellihelper_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(intellihelper_sampling_types::HostedTool::WebSearch { options: None });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(intellihelper_sampling_types::HostedTool::XSearch { options: None });
            }
            intellihelper_sampling_types::apply_tool_overrides(
                &mut hosted_tools,
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        intellihelper_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: intellihelper_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: intellihelper_tools::types::config_source::ConfigSource::Project {
    ...
    fn build_task_description_lists_public_model_slugs() {
        ...
        assert!(!desc.contains(concat!("intellihelper", " models")));
    }
    #[test]
    ...
    fn task_model_guidance_resolves_model_param_override() {
        use intellihelper_tools::types::template_renderer::TemplateRenderer;
        use intellihelper_tools::types::tool::ToolKind;
    ...
    async fn discovery_snapshot_records_gated_and_preloaded_skills() {
        use intellihelper_tools::computer::local::LocalTerminalBackend;
        use intellihelper_tools::notification::ToolNotificationHandle;
        let tmp = tempfile::tempdir().unwrap();
        let write_skill = |dir: &str, content: &str| {
            let d = tmp.path().join(".intellihelper/skills").join(dir);
        ...
        let mut definition = crate::config::AgentDefinition::default_intellihelper_build();
    ...
    async fn pager_flag_combinations_satisfy_tool_invariants() {
        ...
        let cases: &[PagerFlagCase] = &[
            PagerFlagCase {
                label: "intellihelper-build / subagents+ask_user",
                profile: AgentDefinition::default_intellihelper_build,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "intellihelper-build / subagents / no-ask-user",
                profile: AgentDefinition::default_intellihelper_build,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "intellihelper-build / no-subagents / ask_user",
                profile: AgentDefinition::default_intellihelper_build,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "intellihelper-build / no-subagents / no-ask-user",
                profile: AgentDefinition::default_intellihelper_build,
                subagents: false,
                ask_user: false,
            },
            PagerFlagCase {
                label: "intellihelper-build-ask-user / subagents",
                profile: AgentDefinition::intellihelper_build_ask_user,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "intellihelper-build-ask-user / no-subagents",
                profile: AgentDefinition::intellihelper_build_ask_user,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "intellihelper-build-plan",
                profile: AgentDefinition::intellihelper_build_plan,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "intellihelper-build-plan / no-ask-user",
                profile: AgentDefinition::intellihelper_build_plan,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "intellihelper-build-plan-no-subagents",
                profile: AgentDefinition::intellihelper_build_plan_no_subagents,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "intellihelper-build-plan-no-subagents / no-ask-user",
                profile: AgentDefinition::intellihelper_build_plan_no_subagents,
                subagents: false,
                ask_user: false,
            },
    ...
    async fn curated_empty_toolset_fails_agent_build() {
        use intellihelper_tools::computer::local::LocalTerminalBackend;
        use intellihelper_tools::notification::ToolNotificationHandle;
        let mut profile = crate::config::AgentDefinition::default_intellihelper_build();
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        use intellihelper_tools::computer::local::LocalTerminalBackend;
        use intellihelper_tools::implementations::intellihelper_build::ask_user_question::AskUserQuestionParams;
        use intellihelper_tools::notification::ToolNotificationHandle;
        use intellihelper_tools::types::resources::Params;
        let profile = crate::config::AgentDefinition::default_intellihelper_build();
        assert!(
            !profile
                .tool_config
                .tools
                .iter()
                .any(|tc| tc.id == "IntelliHelperBuild:ask_user_question"),
    ...
    async fn build_with_tools(tools: Vec<String>, disallowed: Vec<String>) -> crate::agent::Agent {
        use intellihelper_tools::computer::local::LocalTerminalBackend;
        use intellihelper_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_intellihelper_build();
        def.tools = tools;
    ...
    fn session_tools_allowed_clamp() {
        ...
        assert!(def.session_tools_allowed("IntelliHelperBuild:read_file"));
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use intellihelper_tools::computer::local::LocalTerminalBackend;
        use intellihelper_tools::implementations::intellihelper_build::bash::BashParams;
        use intellihelper_tools::notification::ToolNotificationHandle;
        use intellihelper_tools::types::resources::Params;
        let mut definition = crate::config::AgentDefinition::default_intellihelper_build();
    ...
    /// Compat allowlist names (`Read`, `Bash`, `Grep`) map to their IntelliHelper
    /// equivalents by `ToolKind` — a real restricted toolset, not zero tools.
    #[tokio::test]
    async fn claude_tool_names_map_to_intellihelper_equivalents() {
    ...
    /// Shell, LSP, ask, and task-lifecycle tool names resolve to their intellihelper
    ...
    /// treat `mcp__*` as always-on, so intellihelper keeps the MCP meta-tools
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        use intellihelper_tools::computer::local::LocalTerminalBackend;
        use intellihelper_tools::implementations::intellihelper_build::web_fetch::WebFetchConfig;
        use intellihelper_tools::implementations::web_search::WebSearchConfig;
        use intellihelper_tools::notification::ToolNotificationHandle;
        let mut definition = crate::config::AgentDefinition::default_intellihelper_build();
    ...
    /// intellihelper-build toolsets have no Skill tool — skills are read from
    ...
    /// Compat `ToolSearch` meta-tool maps to intellihelper's `search_tool` (MCP
    ...
    async fn build_with_web_search(
        ...
        tool_overrides: Option<intellihelper_sampling_types::ToolOverrides>,
    ) -> crate::agent::Agent {
        use intellihelper_tools::computer::local::LocalTerminalBackend;
        use intellihelper_tools::implementations::web_search::WebSearchConfig;
        use intellihelper_tools::notification::ToolNotificationHandle;
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, intellihelper_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, intellihelper_sampling_types::HostedTool::XSearch { .. })),
    ...
    async fn hosted_tools_bake_definition_tool_overrides_into_options() {
        let x_search = intellihelper_sampling_types::XSearchOptions {
            date_bound: Some(
                intellihelper_sampling_types::SearchDateBound::new(None, Some("2024-03-15".into()))
                    .unwrap(),
            ),
        };
        let agent = build_with_web_search(
            ...
            true,
            &[],
            Some(intellihelper_sampling_types::ToolOverrides {
        ...
        assert!(
            agent
                .hosted_tools()
                .contains(&intellihelper_sampling_types::HostedTool::XSearch {
