//! `intellihelper plugin` CLI subcommand — manage plugins and marketplace sources.
//!
...
//!
//! Business logic lives in `intellihelper_shell::plugin` (shared orchestration)
//! and lower crates (`intellihelper-agent`, `intellihelper-plugin-marketplace`). This
...
use intellihelper_agent::plugins::install_registry::{InstallKind, InstallRegistry};
use intellihelper_agent::plugins::manifest::{ManifestLoadResult, PluginManifest, load_manifest};
use intellihelper_plugin_marketplace::SourceKind;
use intellihelper_plugin_marketplace::git::SourceCacheLease;
use intellihelper_shell::plugin::{self, RepoUpdateOutcome, UninstallError};

// ── JSON output types ───────────────────────────────────────────────

/// Typed entry for `intellihelper plugin list --json`. The `status` field acts as a
...
/// Typed entry for `intellihelper plugin marketplace list --json`.
#[derive(Serialize)]
...
pub enum PluginCommand {
    ...
    Uninstall {
        /// Plugin name (as shown by `intellihelper plugin list`).
        name: String,
...
fn trust_prompt(subject: &str, source_arg: &str) -> String {
    format!(
        "Installing {subject} requires confirmation.\n\
         ...
         \n\
         To proceed, re-run with --trust:\n  intellihelper plugin install {source_arg} --trust"
    )
}
...
fn cmd_list(json: bool, available: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No plugins installed. Run `intellihelper plugin install --help` to get started.");
    } else {
...
fn installed_plugins(
    repos: &[(
        &str,
        &intellihelper_agent::plugins::install_registry::InstalledRepo,
    )],
) -> Vec<PluginEntry> {
    repos
        .iter()
...
fn available_plugins(registry: &InstallRegistry) -> Vec<PluginEntry> {
    let config = intellihelper_shell::config::load_effective_config()
        .ok()
        .unwrap_or(toml::Value::Table(toml::map::Map::new()));
    let mut sources = intellihelper_plugin_marketplace::load_sources(&config);
    sources.extend(intellihelper_plugin_marketplace::load_extra_sources_from_settings(&sources));
    ...
    for source in &sources {
        ...
        for plugin in intellihelper_plugin_marketplace::scan_marketplace(&root).entries {
            let already_installed =
                intellihelper_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    registry,
                    &identity,
...
fn source_identity(source: &intellihelper_plugin_marketplace::MarketplaceSource) -> String {
    match &source.kind {
...
fn resolve_marketplace_root(
    ...
    match &source.kind {
        ...
        SourceKind::Git { url, branch } => {
            let cache = intellihelper_plugin_marketplace::git::default_cache_root();
            intellihelper_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache,
...
fn install_kind(is_git: bool) -> intellihelper_telemetry::events::InstallKind {
    if is_git {
        intellihelper_telemetry::events::InstallKind::Git
    } else {
        intellihelper_telemetry::events::InstallKind::Local
    }
}

fn log_plugin_installed(
    install_kind: intellihelper_telemetry::events::InstallKind,
    success: bool,
    error_category: Option<String>,
) {
    intellihelper_telemetry::session_ctx::log_event(intellihelper_telemetry::events::PluginInstalled {
        install_kind,
        success,
        trust: true,
        error_category,
...
fn cmd_install(source: &str, trust: bool) -> Result<()> {
    if let Some(mref) = intellihelper_plugin_marketplace::install_resolve::parse_marketplace_ref(source)
    {
    ...
    if !trust {
        use intellihelper_agent::plugins::git_install::{self, InstallSource};
...
fn cmd_install_marketplace(
    source: &str,
    mref: &intellihelper_plugin_marketplace::install_resolve::MarketplaceRef,
    trust: bool,
) -> Result<()> {
    if !trust {
            ...
            if outcome.already_installed {
                ...
                println!(
                    "Plugin \"{}\" is already installed from {}. \
                     Run `intellihelper plugin update {}` to update it.",
    ...
    match plugin::uninstall_plugin(name, confirm, keep_data) {
        Ok(outcome) => {
            intellihelper_telemetry::session_ctx::log_event(
                intellihelper_telemetry::events::PluginUninstalled {
                    confirmed: true,
                    success: true,
                },
            );
        ...
        Err(UninstallError::NeedsConfirm {
            ...
            "Plugin \"{name}\" belongs to repo \"{repo_key}\" which also contains:\n\
             ...
             Uninstalling will remove all {total} plugin(s). To proceed:\n\
               intellihelper plugin uninstall {name} --confirm",
            other_plugins
                .iter()
                .map(|p| format!("  - {p}"))
                .collect::<Vec<_>>()
    ...
    if registry.find_plugin(name).is_none() {
        bail!(
            "Plugin \"{name}\" not found.\n\
               Run `intellihelper plugin list` to see installed plugins."
        );
    }
    if let Err(e) = intellihelper_shell::config::remove_disabled_plugin(name) {
        tracing::warn!("failed to remove from disabled list: {e}");
    }
    intellihelper_shell::config::add_enabled_plugin(name)
...
fn cmd_disable(name: &str) -> Result<()> {
    ...
    if let Err(e) = intellihelper_shell::config::remove_enabled_plugin(name) {
        tracing::warn!("failed to remove from enabled list: {e}");
    }
    intellihelper_shell::config::add_disabled_plugin(name)
...
fn cmd_validate(path: &str) -> Result<()> {
    ...
    match load_manifest(&root) {
        ...
        Ok(ManifestLoadResult::NotFound) => {
            println!(
                "No plugin.json found. IntelliHelper discovers skills, agents, and hooks \
...
fn cmd_tag(path: &str, push: bool, force: bool, dry_run: bool) -> Result<()> {
    ...
    let version = match load_manifest(&root) {
        Ok(ManifestLoadResult::Found(m)) => m.version.ok_or_else(|| {
            anyhow::anyhow!(
                "No `version` field in plugin.json. Set a version to use `intellihelper plugin tag`."
            )
        })?,
    ...
    if json {
        ...
        println!(
            "No marketplace sources configured.\n\
             Run `intellihelper plugin marketplace add --help` to get started."
        );
    } else {
        for s in sources {
            let id = match &s.kind {
...
fn marketplace_add(
    ...
    use intellihelper_shell::plugin::MarketplaceAddInput;
    ...
    if !force && let MarketplaceAddInput::GitUrl(git_url) = &input {
        intellihelper_plugin_marketplace::git::probe_git_remote(git_url).map_err(|e| {
            anyhow::anyhow!(
...
fn find_removal_source<'a>(
    sources: &'a [intellihelper_plugin_marketplace::MarketplaceSource],
    input: &str,
    cwd: &Path,
) -> Result<&'a intellihelper_plugin_marketplace::MarketplaceSource, String> {
    ...
    let local_input = match plugin::classify_marketplace_add_input(input, cwd) {
        intellihelper_shell::plugin::MarketplaceAddInput::LocalPath(p) => Some(p),
        _ => None,
    };
...
fn marketplace_update(
    ...
    marketplace_update_with_cache_root(
        sources,
        name,
        &intellihelper_plugin_marketplace::git::default_cache_root(),
    )
}
...
fn marketplace_update_with_cache_root(
    ...
    for source in sources {
        ...
        if let SourceKind::Git { url, branch } = &source.kind {
            match intellihelper_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                cache_root,
            ) {
...
mod tests {
    use super::*;
    use intellihelper_plugin_marketplace::MarketplaceSource;
    ...
    fn trust_prompt_marketplace_has_no_error_framing() {
        ...
        assert!(msg.contains(
            "To proceed, re-run with --trust:\n  intellihelper plugin install sentry@xai-org/plugin-marketplace --trust"
        ));
    ...
    fn trust_prompt_git_and_local_subjects() {
        ...
        assert!(git.ends_with("  intellihelper plugin install u/r --trust"), "{git}");
    ...
    fn marketplace_update_force_syncs_fresh_git_cache() {
        ...
        let cache_dir = intellihelper_plugin_marketplace::git::sync_source_cache(
            &url,
            Some("main"),
            cache_root.path(),
        )
    ...
    fn current_head(repo: &Path) -> String {
        let output = intellihelper_plugin_marketplace::git::git_command()
            .current_dir(repo)
            .args(["rev-parse", "HEAD"])
            .output()
            .unwrap();
