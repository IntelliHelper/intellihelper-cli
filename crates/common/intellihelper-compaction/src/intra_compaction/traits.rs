...
pub enum CompactionTarget {
    ...
    /// with a single summary — intellihelper-build's full-replace strategy. No tail is
...
/// stream processor. Implemented by IntelliHelper chat's
/// `StreamProcessor` (`Item = Arc<IntelliHelperTurn>`).
///
...
pub trait CompactionStreamProc: Send + Sync {
    ...
    /// item type (`Arc<IntelliHelperTurn>`) already satisfies it.
