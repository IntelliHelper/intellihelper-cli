...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.intellihelper/agents/` or `~/.intellihelper/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.intellihelper/personas/`; applied during subagent resolution |
...
| **Examples** | `intellihelper-build`, `explore`, `plan` | `researcher`, `concise` |
...
IntelliHelper CLI discovers file-based personas from these locations, in priority order:

- `.intellihelper/personas/*.toml` (project)
- `~/.intellihelper/personas/*.toml` (user)
...
> **Note:** IntelliHelper CLI applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, IntelliHelper CLI resolves the effective model and reasoning effort in this order, highest priority first:
...
IntelliHelper CLI manages worktrees through the `x.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
[subagents.models]
explore = "intellihelper-build"               # route explore to a specific model
```
...
# instructions_file = ".intellihelper/personas/concise.md"  # or load from a file
```

IntelliHelper CLI also discovers roles from `.intellihelper/roles/*.toml` and personas from `.intellihelper/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

IntelliHelper CLI shows running and finished work in side panes on the agent screen:
...
- `Subagent running: "do the thing" (Implementer · intellihelper-3) — Thinking`
