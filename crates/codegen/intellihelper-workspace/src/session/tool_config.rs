...
use std::sync::Arc;
use intellihelper_tools::registry::types::{
...
pub(crate) fn resolve_session_toolset(
    ...
    notification_handle: Option<intellihelper_tools::notification::types::ToolNotificationHandle>,
) -> WorkspaceResult<(
    ToolServerConfig,
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<intellihelper_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn intellihelper_tools::computer::types::TerminalBackend>,
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            intellihelper_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
/// enabled (`INTELLIHELPER_WORKSPACE_TOOL_STATE_ENABLED=true`; any other value keeps
/// legacy behavior).
pub fn tool_state_enabled() -> bool {
    std::env::var("INTELLIHELPER_WORKSPACE_TOOL_STATE_ENABLED").as_deref() == Ok("true")
}
...
/// `INTELLIHELPER_WORKSPACE_TOOL_STATE_ENABLED`. Aliased to the crate-wide
...
/// [`LocalTerminalBackend`]: intellihelper_tools::computer::local::LocalTerminalBackend
pub struct WorkspaceSessionContextFactory {
    auth: Option<computer_hub_sdk::SharedAuthProvider>,
    api_base_url: Option<String>,
    /// Resolved `$INTELLIHELPER_WORKSPACE_HOME` when tool-state persistence is enabled;
...
impl WorkspaceSessionContextFactory {
    ...
    /// (`$INTELLIHELPER_WORKSPACE_HOME`). Callers should only invoke this when
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let fs = Arc::new(intellihelper_tools::computer::local::LocalFs)
            as Arc<dyn intellihelper_tools::computer::types::AsyncFileSystem>;
        let notification_handle = intellihelper_tools::notification::ToolNotificationHandle::noop();
    ...
    fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
        crate::config::SessionTerminalBackend::local(
            intellihelper_tools::computer::local::LocalTerminalBackend::new(),
        )
    }
...
fn build_proxy_headers(base_url: &str) -> indexmap::IndexMap<String, String> {
    ...
    let version = intellihelper_version::VERSION;
    headers.insert(
        "user-agent".to_string(),
        format!("intellihelper-workspace/{version}"),
    );
    headers.insert("x-intellihelper-client-version".to_string(), version.to_string());
    headers.insert(
        "x-intellihelper-client-identifier".to_string(),
        std::env::var("INTELLIHELPER_CLIENT_NAME").unwrap_or_else(|_| "intellihelper-shell".to_string()),
    );
    if base_url.contains("cli-chat-proxy") || base_url.contains("chat-proxy") {
        headers.insert("X-XAI-Token-Auth".to_string(), "intellihelper-cli".to_string());
        headers.insert(
...
/// `INTELLIHELPER_DISABLE_WEB_FETCH=1` is set.
fn build_web_fetch_config() -> intellihelper_tools::implementations::intellihelper_build::web_fetch::WebFetchConfig
{
    use intellihelper_tools::implementations::intellihelper_build::web_fetch::{WebFetchConfig, WebFetchParams};
    if std::env::var("INTELLIHELPER_DISABLE_WEB_FETCH").is_ok_and(|v| v == "1" || v == "true") {
        return WebFetchConfig::Disabled;
    }
    let mut params = WebFetchParams::default();
    if let Ok(proxy) = std::env::var("INTELLIHELPER_WEB_FETCH_PROXY") {
        params.proxy_endpoint = Some(proxy);
    }
    if intellihelper_config::env_bool("INTELLIHELPER_WEB_FETCH_ALLOW_LOCAL") == Some(true) {
        params.allow_local = Some(true);
    }
    WebFetchConfig::Enabled { params }
}
fn default_web_search_model() -> String {
    std::env::var("INTELLIHELPER_WEB_SEARCH_MODEL").unwrap_or_else(|_| "intellihelper-4.5".to_string())
}
    ...
    pub fn baseline_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("IntelliHelperBuild:search_replace", Some(ToolKind::Edit)),
                tc("IntelliHelperBuild:grep", Some(ToolKind::Search)),
...
mod tests {
    ...
    async fn resolve_session_toolset_mcp_merge_dedup_by_id_baseline_wins() {
        ...
        let mut mcp_dup = test_support::tc("IntelliHelperBuild:read_file", Some(ToolKind::Read));
    ...
    fn backfill_tool_kinds_fills_known_kindless_ids_only() {
        let kinds = HashMap::from([
            ("IntelliHelperBuild:search_replace".to_owned(), ToolKind::Edit),
            ("IntelliHelperBuild:read_file".to_owned(), ToolKind::Read),
        ]);
        let config = ToolServerConfig {
            tools: vec![
                test_support::tc("IntelliHelperBuild:search_replace", None),
                test_support::tc("adhoc.opaque", None),
                // Pre-set kinds must never be overwritten by the registry.
                test_support::tc("IntelliHelperBuild:read_file", Some(ToolKind::Search)),
            ],
        ...
        assert_eq!(kind_of("IntelliHelperBuild:search_replace"), Some(ToolKind::Edit));
        assert_eq!(
        ...
        assert_eq!(
            kind_of("IntelliHelperBuild:read_file"),
    ...
    async fn resolve_session_toolset_readonly_filters_kindless_pinned_tools() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("IntelliHelperBuild:read_file", None),
                test_support::tc("IntelliHelperBuild:grep", None),
                test_support::tc("IntelliHelperBuild:list_dir", None),
                test_support::tc("IntelliHelperBuild:search_replace", None),
                test_support::tc("IntelliHelperBuild:run_terminal_cmd", None),
            ],
            behavior_preset: None,
        };
        let (eff, ts, _backend) = resolve_session_toolset(
    ...
    async fn resolve_session_toolset_mcp_kind_none_dropped_under_readonly() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("IntelliHelperBuild:read_file", Some(ToolKind::Read)),
        ...
        assert!(
            kept_ids.contains(&"IntelliHelperBuild:read_file"),
    ...
    fn tool_state_enabled_only_true_enables() {
        ...
        let var = "INTELLIHELPER_WORKSPACE_TOOL_STATE_ENABLED";
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use intellihelper_tools::types::resources::{State, WebCitationCounter};
