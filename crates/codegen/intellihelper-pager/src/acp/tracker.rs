...
use intellihelper_tools::types::output::{ReadFileOutput, SearchToolOutput, WebFetchOutput};
use intellihelper_tools::util::strip_redundant_session_cd;
...
impl AcpUpdateTracker {
    ...
    fn handle_user_message(
        ...
        let mut block = if let Some(dt) = display_override {
            ...
            let skill_display =
                intellihelper_tools::implementations::skills::skill::extract_skill_display_text(&text);
...
fn user_message_hidden_from_scrollback(
    ...
    if let Some(pid) = meta.prompt_id.as_deref()
        && intellihelper_shell::session::PromptOrigin::from_prompt_id(pid)
...
fn extract_edit_error(tc: &acp::ToolCall) -> String {
    use intellihelper_tools::types::output::SearchReplaceOutput;
    if let Some(ref raw) = tc.raw_output
...
fn extract_listdir_content(raw: &Option<serde_json::Value>) -> Option<String> {
    let val = raw.as_ref()?;
    match serde_json::from_value::<ToolOutput>(val.clone()) {
        Ok(ToolOutput::ListDir(intellihelper_tools::types::output::ListDirOutput::Content(c))) => {
            Some(c.content)
        }
        _ => None,
    }
...
fn extract_use_tool_output(raw: &Option<serde_json::Value>) -> Option<String> {
    ...
    if let Ok(output) = serde_json::from_value::<ToolOutput>(val.clone()) {
        let text = match output {
            ToolOutput::MCP(mcp) => {
                use intellihelper_tools::types::output::MCPOutputDetails;
                match mcp.output() {
...
mod tests {
    use super::*;
    use std::sync::Arc;
    /// Default meta with no timestamps (simulates old intellihelper-shell or tests that
    ...
    fn test_search_tool_call_flow() {
        use intellihelper_tools::types::output::{GrepFileMatch, GrepLineMatch, GrepSearchOutput};
        let mut tracker = AcpUpdateTracker::new();
    ...
    /// No stream_start_ms (old intellihelper-shell) should not break anything.
    #[test]
    ...
    #[test]
    fn is_task_tool_recognizes_intellihelper_build_variant() {
...
[CI] IN_PROGRESS=5 SUCCESS=38\n\
    ...
    fn video_tool_variants_use_typed_path_not_generic_scrape() {
        ...
        let cases: &[(&str, ToolOutput)] = &[
            (
                "ImageToVideo",
                ToolOutput::ImageToVideo(intellihelper_tools::types::output::MediaGenOutput::new(
                    video_path.clone(),
                )),
            ),
            (
                "ReferenceToVideo",
                ToolOutput::ReferenceToVideo(intellihelper_tools::types::output::MediaGenOutput::new(
                    video_path.clone(),
                )),
            ),
        ];
    ...
    fn media_gen_ref_skips_uploaded_only_video() {
        let output =
            ToolOutput::ImageToVideo(intellihelper_tools::types::output::MediaGenOutput::uploaded(
    ...
    fn tier_restricted_media_shows_upsell_text_not_error() {
        let upsell = "Image generation is a SuperGrok feature. Upgrade at \
             https://grok.com/__PROTECT_4__?referrer=intellihelper-build";
        let output = ToolOutput::Text(intellihelper_tools::types::output::TextOutput::from(upsell));
        let tc = acp::ToolCall::new(
