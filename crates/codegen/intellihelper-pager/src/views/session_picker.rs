...
pub(crate) fn pending_delete_from_selection(
    ...
    content_results: Option<&[intellihelper_shell::extensions::session_search::SearchSessionHit]>,
...
/// matters because the fast foreign scan can land rows the default IntelliHelper view
...
///
/// Default is [`Self::IntelliHelper`]: native IntelliHelper sessions only (local / remote /
/// conversation), so `/resume` does not mix Claude/Codex/Cursor foreign
/// sessions into the list. `f` cycles IntelliHelper → External → All → Local →
/// Remote — External first so one press from the default reveals foreign
/// sessions.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum SourceFilter {
    /// Native IntelliHelper sessions only — excludes Claude/Codex/Cursor foreign rows.
    #[default]
    IntelliHelper,
    Local,
    Remote,
...
impl SourceFilter {
    pub fn label(self) -> &'static str {
        match self {
            Self::IntelliHelper => "IntelliHelper",
            Self::Local => "Local",
            Self::Remote => "Remote",
    ...
    pub fn next(self) -> Self {
        match self {
            Self::IntelliHelper => Self::External,
            Self::External => Self::All,
            Self::All => Self::Local,
            Self::Local => Self::Remote,
            Self::Remote => Self::IntelliHelper,
        }
    }

    /// Returns `true` when a non-default filter is selected.
    pub fn is_active(self) -> bool {
        self != Self::IntelliHelper
    }

    /// Returns `true` if a session with the given `source` string passes the filter.
    ///
    /// intellihelper.com conversations carry `source == "conversation"` and live remotely,
    /// so they pass the `Remote` filter (and `IntelliHelper` / `All`) but not `Local`.
    /// Foreign sources (`claude` / `codex` / `cursor`) only pass `External` and
    /// `All`.
    pub fn matches(self, source: &str) -> bool {
        match self {
            Self::IntelliHelper => !crate::app::is_foreign_picker_source(source),
...
pub(crate) fn build_content_entry_data(
    hits: &[intellihelper_shell::extensions::session_search::SearchSessionHit],
    entries_data: &[SessionPickerEntry],
    filtered_indices: &[usize],
    state: &PickerState,
    content_start: usize,
...
/// Hint shown on the default `IntelliHelper` view when the foreign-session scan loaded
/// Claude/Codex/Cursor entries it hides. IntelliHelper-only: `next(IntelliHelper) == External`
...
pub(crate) fn hidden_external_hint(
    ...
    if source_filter != SourceFilter::IntelliHelper {
        return None;
    }
    let hidden = entries?
        .iter()
...
mod tests {
    ...
    fn source_filter_matches() {
        // Default IntelliHelper filter: native only (not Claude/Codex/Cursor).
        assert!(SourceFilter::IntelliHelper.matches("local"));
        assert!(SourceFilter::IntelliHelper.matches("remote"));
        assert!(SourceFilter::IntelliHelper.matches("both"));
        assert!(SourceFilter::IntelliHelper.matches("conversation"));
        assert!(!SourceFilter::IntelliHelper.matches("claude"));
        assert!(!SourceFilter::IntelliHelper.matches("codex"));
        assert!(!SourceFilter::IntelliHelper.matches("cursor"));
        ...
        // intellihelper.com conversations are remote: visible under IntelliHelper + All + Remote, not Local.
    ...
    fn source_filter_cycles() {
        ...
        assert_eq!(SourceFilter::IntelliHelper.next(), SourceFilter::External);
        ...
        assert_eq!(SourceFilter::Remote.next(), SourceFilter::IntelliHelper);
        assert_eq!(SourceFilter::IntelliHelper.label(), "IntelliHelper");
        assert_eq!(SourceFilter::External.label(), "External");
        assert_eq!(SourceFilter::default(), SourceFilter::IntelliHelper);
    }
    ...
    fn source_filter_filters_entries() {
        ...
        let intellihelper = filter_session_entries(Some(&entries), "", SourceFilter::IntelliHelper);
        assert_eq!(intellihelper, vec![0, 1, 2]); // local + remote + both, no foreign
    ...
    fn source_filter_empty_and_unknown_source() {
        ...
        // foreign, so it passes IntelliHelper + All but never Local, Remote, or External.
        assert!(SourceFilter::IntelliHelper.matches(""));
        ...
        assert!(SourceFilter::IntelliHelper.matches("unknown"));
    ...
    fn source_filter_is_active() {
        assert!(!SourceFilter::IntelliHelper.is_active());
    ...
    fn hidden_external_hint_visibility() {
        ...
        // Only the default IntelliHelper view surfaces the hint (with the count).
        assert_eq!(
            hidden_external_hint(Some(&entries), SourceFilter::IntelliHelper).as_deref(),
        ...
        assert_eq!(
            hidden_external_hint(Some(&one), SourceFilter::IntelliHelper).as_deref(),
        ...
        assert!(hidden_external_hint(Some(&native), SourceFilter::IntelliHelper).is_none());
        assert!(hidden_external_hint(None, SourceFilter::IntelliHelper).is_none());
    }
