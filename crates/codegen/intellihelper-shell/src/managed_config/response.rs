...
impl ManagedConfigSource {
    ...
    /// check `INTELLIHELPER_DEPLOYMENT_KEY`).
...
pub enum ManagedConfigError {
    ...
    #[error(
        "The deployment key was rejected. Confirm that INTELLIHELPER_DEPLOYMENT_KEY is set correctly and hasn't expired."
    )]
    DeploymentKeyRejected,
    #[error(
        "Your team sign-in was rejected. It may have expired or lack access. Run `intellihelper login` to sign in again."
    )]
    TeamAuthRejected,
    ...
    SignatureRejected,
    #[error(
        "Can't save the configuration to ~/.intellihelper. Make sure the directory exists and is writable.\n  ({0})"
    )]
...
pub(super) struct ManagedConfigResponse {
    ...
    #[serde(default)]
    pub(super) signatures: Option<Vec<intellihelper_config::signed_policy::SignatureEnvelope>>,
    ...
    pub(super) managed_identity_signatures:
        Option<Vec<intellihelper_config::signed_policy::SignatureEnvelope>>,
}
...
impl ManagedConfigResponse {
    ...
    pub(super) fn signature_sidecar(
        ...
        pick_trusted_envelope(
            ...
            intellihelper_config::signed_policy::embedded_key_id_trusted,
        )
    }
...
fn pick_trusted_envelope(
    envelopes: Option<&[intellihelper_config::signed_policy::SignatureEnvelope]>,
...
pub(super) struct VerifiedEnvelope {
    pub(super) sidecar: intellihelper_config::signed_policy::SignatureEnvelope,
    pub(super) payload: intellihelper_config::signed_policy::SignedPayload,
}
...
pub(super) fn verify_signed_envelope(
    ...
    use intellihelper_config::signed_policy;
...
mod tests {
    ...
    fn pick_trusted_envelope_prefers_trusted_then_falls_back() {
        use intellihelper_config::signed_policy::SignatureEnvelope;
