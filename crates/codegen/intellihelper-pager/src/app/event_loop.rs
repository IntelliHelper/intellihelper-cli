...
/// [`decide`](intellihelper_workspace::folder_trust::decide) precedence.
///
...
pub(crate) async fn run(
    ...
    if args.log_sampling {
        ...
        unsafe { std::env::set_var("INTELLIHELPER_LOG_SAMPLING", "1") };
    }
    ...
    let launch_effective_ui = intellihelper_shell::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(intellihelper_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = intellihelper_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match intellihelper_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.privacy_notice_rollout = intellihelper_config::env_bool("INTELLIHELPER_PRIVACY_NOTICE_ROLLOUT")
        .or_else(|| {
            remote_settings
                .as_ref()
        ...
        .unwrap_or(false);
    app.privacy_banner_reshow_days = std::env::var("INTELLIHELPER_PRIVACY_BANNER_RESHOW_DAYS")
        .ok()
    ...
    app.privacy_banner_acked = intellihelper_shell::config::load_from_disk()
        .ok()
        .and_then(|root| {
            intellihelper_shell::util::config::load_config_from_toml(&root)
                .privacy
                .privacy_banner_acked
        });
    app.plugin_cta_enabled = intellihelper_config::env_bool("INTELLIHELPER_PLUGIN_CTA")
        .or_else(|| remote_settings.as_ref().and_then(|s| s.plugin_cta))
        .unwrap_or(false);
    // Voice is applied after auth_meta so API-key detection is accurate.
    app.session_picker_grouped = std::env::var("INTELLIHELPER_SESSION_PICKER_GROUPED")
        .ok()
    ...
    // Interactive browser login only when the agent advertised an interactive
    // method AND (needs_login or --force-login). Empty methods / API-key-only
    // setups go straight to Done — no "Login with intellihelper.com", no token paste.
    let has_interactive_method = connection.login_method_id.is_some()
        || connection.auth_methods.iter().any(|m| {
            intellihelper_shell::agent::auth_method::AuthMethodKind::from_id(m.id())
                .needs_interactive_login()
        });
    let force_login =
        args.force_login && !connection.auth_methods.is_empty() && has_interactive_method;
    let needs_interactive_login =
        (connection.needs_login && has_interactive_method) || force_login;
    if needs_interactive_login {
        ...
        if connection.needs_login {
            ...
            // --force-login: prefer an interactive method from the advertised list
        ...
        tracing::info!(
            ...
            "auto-triggering interactive login at startup"
        );
    } else {
        // Clear any stale login affordances so the welcome UI never shows
        // "Login with intellihelper.com" for API-key / BYOK-only setups.
        app.login_label = None;
        app.login_method_id = None;
        app.auth_state = super::app_view::AuthState::Done;
    }
    // Effects stashed until after the initial render.
    let mut post_render_effects = if needs_interactive_login && app.login_method_id.is_some() {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<intellihelper_shell::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app.auth_methods.iter().any(|m| {
            m.id().0.as_ref() == intellihelper_shell::agent::auth_method::XAI_API_KEY_METHOD_ID
        });
    ...
    app.hidden_announcement_ids = intellihelper_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = intellihelper_shell::config::load_merged_requirements();
    let user_config = intellihelper_shell::config::load_from_disk().ok();
    let managed_config = intellihelper_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match intellihelper_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = intellihelper_shell::agent::config::resolve_compat_sessions_from_raw(
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            ...
            // (config > INTELLIHELPER_XAI_API_BASE_URL env > default).
            let endpoints_base =
                intellihelper_shell::agent::config::EndpointsConfig::from_config_value(raw)
                    .xai_api_base_url;
            app.voice_config =
                intellihelper_voice::VoiceConfig::from_config_table(table, Some(&endpoints_base));
        }
    }
    // Stamp request-identity headers so the STT handshake attributes voice usage
    // to intellihelper-cli server-side (mirrors sampler / imagine). Done after
    ...
    app.zdr_access_enabled = intellihelper_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        intellihelper_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        intellihelper_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        intellihelper_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    app.scheduler_background_loops_seed =
        intellihelper_shell::util::config::resolve_scheduler_background_loops(
            remote_settings
                .as_ref()
...

    {
        ...
        app.active_announcements = intellihelper_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            let intellihelper_home = intellihelper_tools::util::intellihelper_home::intellihelper_home();
            app.tip = intellihelper_shell::util::tips::pick_and_advance(&app.tips, &intellihelper_home);
        }
    ...
    let hints = intellihelper_shell::util::config::resolve_hints(
    ...
    // Apply initial config (may come from existing ~/.intellihelper/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        intellihelper_shell::util::config::clamped_display_permission_mode(
            intellihelper_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        intellihelper_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = intellihelper_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = intellihelper_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    // INTELLIHELPER_SCROLL_SPEED/_MODE/_LINES + INTELLIHELPER_INVERT_SCROLL env overrides
    // apply on first load).
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<intellihelper_voice::VoiceEvent>>;
    ...
    // Initial prompt from the CLI positional (`intellihelper "fix the bug"`). When
    ...
    // `intellihelper dashboard` startup: open the dashboard view immediately. The
    // CLI subcommand wrote a `INTELLIHELPER_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.
    if std::env::var("INTELLIHELPER_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
    ...
    loop {
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(intellihelper_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
                ...
                app.voice_state = VoiceState::Idle;
                app.voice_ui_active = false;
                app.show_toast("Voice could not start. Restart IntelliHelper.");
            } else {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.intellihelper/config.toml [ui]`
                // rather than `~/.intellihelper/pager.toml`. The watcher only knows
                        ...
                        let join_handle = tokio::spawn(async move {
                            ...
                            let ok = tokio::time::timeout(timeout, async {
                                ...
                                for (agent_id, plan) in load_plans {
                                    ...
                                    let mcp_servers = intellihelper_shell::util::config::load_mcp_servers(
                                        &plan.cwd,
...
pub(crate) fn load_initial_ui_config() -> intellihelper_shell::agent::config::UiConfig {
    use intellihelper_shell::agent::config::UiConfig;
    let Ok(root) = intellihelper_shell::config::load_effective_config() else {
...
async fn drain_and_process(
    ...
    let mut handle_one = |routed: &RoutedInputEvent| -> bool {
        ...
        if let Event::Key(ke) = ev
            ...
            && intellihelper_voice::AUDIO_SUPPORTED
            && is_voice_chord(ke)
