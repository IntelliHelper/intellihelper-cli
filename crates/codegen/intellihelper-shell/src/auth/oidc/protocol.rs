...
use super::super::{AuthMode, IntelliHelperAuth};
use base64::Engine;
...
pub(super) fn build_intellihelper_auth(
...
async fn refresh_tokens_once(
    ...
    if !resp.status().is_success() {
        ...
        tracing::warn!(
            ...
            rt_prefix = intellihelper_auth::bearer_suffix(refresh_token),
            client_id = %client_id,
            principal_type = ?principal_type,
...
mod tests {
    ...
    fn authorize_url_includes_required_oidc_params() {
        let config = OidcAuthConfig {
            ...
            audience: Some("api://intellihelper".into()),
        };
    ...
    fn authorize_url_includes_team_principal_params() {
        let config = OidcAuthConfig {
            ...
            client_id: TEST_CLIENT_ID.into(),
            scopes: vec!["offline_access".into(), "intellihelper-cli:access".into()],
            audience: None,
        };
        ...
        assert!(url.contains("referrer=intellihelper-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
            "expected exactly one referrer param, got: {url}"
    ...
    fn authorize_url_uses_oauth2_referrer_override_once() {
        ...
        let oauth2 = OAuth2ProviderConfig {
            ...
            principal_type: None,
            principal_id: None,
            referrer: Some("intellihelper-desktop".into()),
        };
        let discovery = Discovery {
            authorization_endpoint: "https://auth.x.ai/authorize".into(),
            token_endpoint: "https://auth.x.ai/token".into(),
        ...
        assert!(url.contains("referrer=intellihelper-desktop"));
        assert!(!url.contains("referrer=intellihelper-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
    ...
    fn peek_access_token_principal_matrix() {
        ...
        let team_jwt = make_jwt(serde_json::json!({
            ...
            "exp": 9999999999u64,
            "iat": 1000000000u64,
            "scope": "offline_access intellihelper-cli:access api:access",
            "principal_type": "Team",
