...
//! Dark themes (IntelliHelperNight, TokyoNight) share `intellihelper-night.tmTheme`;
//! IntelliHelperDay uses `intellihelper-day.tmTheme` with deepened colors for light backgrounds.
//!
...
//! `current_kind()` is a nominal `IntelliHelperNight` (so leftover kind-keyed paths
...
pub use intellihelper_markdown::Syntect;
...
static SYNTECT_INTELLIHELPERNIGHT: OnceLock<Syntect> = OnceLock::new();
static SYNTECT_TOKYONIGHT: OnceLock<Syntect> = OnceLock::new();
static SYNTECT_INTELLIHELPERDAY: OnceLock<Syntect> = OnceLock::new();
...
/// reports a nominal `IntelliHelperNight`, so this returns the night theme. Token
...
pub fn get_syntect() -> &'static Syntect {
    match crate::theme::Theme::current_kind() {
        ...
        | ThemeKind::Auto => SYNTECT_INTELLIHELPERNIGHT
            .get_or_init(|| Syntect::new(include_bytes!("../assets/intellihelper-night.tmTheme"))),
        ThemeKind::TokyoNight => SYNTECT_TOKYONIGHT
            .get_or_init(|| Syntect::new(include_bytes!("../assets/tokyo-night.tmTheme"))),
        ThemeKind::IntelliHelperDay => SYNTECT_INTELLIHELPERDAY
            .get_or_init(|| Syntect::new(include_bytes!("../assets/intellihelper-day.tmTheme"))),
    }
}
