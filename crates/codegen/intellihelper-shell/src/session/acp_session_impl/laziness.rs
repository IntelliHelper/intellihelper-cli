...
impl SessionActor {
    ...
    /// sees "IntelliHelper is thinking", streaming token chunks, or any other
    ...
    pub(crate) async fn maybe_fire_laziness_check(self: Arc<Self>) {
        ...
        let items = vec![
            ConversationItem::System(intellihelper_sampling_types::SystemItem {
        ...
        let request = ConversationRequest {
            ...
            // Don't pass `reasoning_effort` — `intellihelper-4.5` (and
            // other tool-flavoured IntelliHelper variants) reject the field at
            ...
            x_intellihelper_conv_id: Some(format!("trace-classifier-{}", uuid::Uuid::new_v4())),
            x_intellihelper_req_id: Some(format!("{LAZINESS_REQ_ID_PREFIX}{}", uuid::Uuid::new_v4())),
            x_intellihelper_session_id: Some(session_id_str),
