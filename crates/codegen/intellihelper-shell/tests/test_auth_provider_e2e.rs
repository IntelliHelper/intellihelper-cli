...
//!
//! Runs the built intellihelper binary headless against the mock inference server with
...
//! the release artifact via `INTELLIHELPER_BINARY`; run locally (auto-builds the pager):
//! ```bash
//! cargo test -p intellihelper-shell --test test_auth_provider_e2e -- --ignored
//! ```
//!
...
async fn provider_backed_model_sends_minted_token_on_the_wire() {
    ...
    sandbox.remove_env("INTELLIHELPER_LEADER_SOCKET");
    ...
    let counter = intellihelper_home.join("mint-count");
    let helper = intellihelper_home.join("mock-auth.sh");
    std::fs::write(
        &helper,
        format!(
...
async fn provider_with_args_and_json_output_sends_minted_token() {
    ...
    let seen_args = intellihelper_home.join("seen-args");
    let helper = intellihelper_home.join("mock-auth-json.sh");
    std::fs::write(
        &helper,
        format!(
            "#!/bin/sh\nprintf '%s' \"$*\" > {}\n\
