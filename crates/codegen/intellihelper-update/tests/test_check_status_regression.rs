...
//! exact JSON shape produced by `intellihelper update --check --json` for the failure
...
use intellihelper_update::auto_update::check_update_status;

/// Set up a fake `npm` on PATH, set `INTELLIHELPER_INSTALLER=npm` so the auto-update
