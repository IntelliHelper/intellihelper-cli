...
fn messages_per_response_model_reflects_mid_session_switch() {
    let mut r = messages(false);
    let mut out = Vec::new();
    out.extend(r.reduce(response_started("msg_a", Some("intellihelper-4"), 5)));
    out.extend(r.reduce(StreamEvent::AgentMessage("from A".into())));
    out.extend(r.reduce(response_completed("msg_a", "end_turn")));
    out.extend(r.reduce(response_started("msg_b", Some("intellihelper-4-fast"), 6)));
    ...
    assert_eq!(frames[0]["message"]["model"], "intellihelper-4");
    assert_eq!(frames[1]["message"]["id"], "msg_b");
    assert_eq!(frames[1]["message"]["model"], "intellihelper-4-fast");
}
...
fn messages_duplicate_response_started_does_not_merge_content() {
    ...
    out.extend(r.reduce(response_started("msg_b", Some("intellihelper-4"), 6)));
...
fn messages_late_response_completed_for_flushed_response_is_dropped() {
    let mut r = messages(false);
    let mut out = Vec::new();
    out.extend(r.reduce(response_started("msg_a", Some("intellihelper-4"), 1)));
    out.extend(r.reduce(StreamEvent::AgentMessage("a-text".into())));
    out.extend(r.reduce(response_started("msg_b", Some("intellihelper-4"), 2)));
