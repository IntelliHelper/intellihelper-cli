...
//!
//! Searches from cwd to repo root, plus `~/.intellihelper/`. Also discovers
//! `*.md` files in rules directories: vendor-prefixed `.intellihelper/rules/`,
...
//! (`~/.intellihelper/rules/`, `~/.claude/rules/`, `~/.cursor/rules/`).
...
/// Find `*.md` files in `.intellihelper/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.intellihelper/, git repo root, and session cwd.
...
async fn read_agents_config_with_roots(
    ...
    add_discovery_root(&mut home_roots, intellihelper_home, true, HOME_RULES_DIRS);
    ...
    candidates
        ...
        .filter_map(|candidate| {
            ...
            let content = if candidate.is_rule {
                intellihelper_tools::implementations::skills::skill::extract_skill_body(&content)
            } else {
                content
            };
...
/// Used by `intellihelper-shell` to structurally detect legacy untagged AGENTS.md
...
/// Open/close `system-reminder` (IntelliHelper) or `system_reminder` (Cursor/IDE), case-insensitive.
...
mod tests {
    ...
    async fn home_and_project_rules_have_stable_order_without_doubled_paths() {
        let tmp = tempfile::tempdir().unwrap();
        let intellihelper_home = tmp.path().join("custom-intellihelper-home");
        let home = tmp.path().join("home");
        let repo = tmp.path().join("repo");
        fs::create_dir_all(intellihelper_home.join("rules")).unwrap();
        fs::create_dir_all(home.join(".claude/rules")).unwrap();
        fs::create_dir_all(home.join(".cursor/rules")).unwrap();
        fs::create_dir_all(repo.join(".intellihelper/rules")).unwrap();
        fs::create_dir_all(repo.join(".claude/rules")).unwrap();
        fs::create_dir_all(repo.join(".cursor/rules")).unwrap();
        init_git_repo(&repo);

        for (path, content) in [
            (intellihelper_home.join("rules/b.md"), "intellihelper-b"),
            (intellihelper_home.join("rules/a.md"), "intellihelper-a"),
            (home.join(".claude/rules/a.md"), "claude-a"),
            (home.join(".cursor/rules/a.md"), "cursor-a"),
            (repo.join("AGENTS.md"), "repo-named"),
            (repo.join(".intellihelper/rules/a.md"), "repo-intellihelper"),
        ...
        for path in [
            intellihelper_home.join(".intellihelper/rules/doubled.md"),
    ...
    async fn vendor_home_agents_and_rules_cells_are_independent() {
        let tmp = tempfile::tempdir().unwrap();
        let intellihelper_home = tmp.path().join("intellihelper-home");
        let home = tmp.path().join("home");
    ...
    #[tokio::test]
    async fn nested_intellihelper_home_keeps_project_role_in_repo_order() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = tmp.path().join("repo");
        let nested = repo.join("nested");
        fs::create_dir_all(nested.join("rules")).unwrap();
        fs::create_dir_all(nested.join(".intellihelper/rules")).unwrap();
        init_git_repo(&repo);
        ...
        fs::write(nested.join(".intellihelper/rules/project.md"), "nested-project-rule").unwrap();
    ...
    #[tokio::test]
    async fn overlapping_intellihelper_home_and_project_root_merges_roles() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = tmp.path().join("repo");
        ...
        fs::write(repo.join(".intellihelper/rules/project.md"), "project-intellihelper-rule").unwrap();
        fs::write(repo.join(".claude/rules/project.md"), "project-claude-rule").unwrap();
        fs::create_dir_all(repo.join(".intellihelper/.intellihelper/rules")).unwrap();
        fs::write(repo.join(".intellihelper/.intellihelper/rules/doubled.md"), "doubled").unwrap();
        ...
        .await;
        for expected in ["home-rule", "project-intellihelper-rule", "project-claude-rule"] {
            assert_eq!(
                configs
                    .iter()
    ...
    async fn rule_frontmatter_is_stripped_but_named_frontmatter_is_preserved() {
        ...
        for (path, body) in [
            (intellihelper_home.join("rules/global.md"), "custom-home-body"),
            (home.join(".claude/rules/global.md"), "claude-body"),
            (home.join(".cursor/rules/global.md"), "cursor-body"),
            (repo.join(".intellihelper/rules/project.md"), "intellihelper-project-body"),
