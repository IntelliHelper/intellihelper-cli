...
use intellihelper_mcp::servers::McpState;
use intellihelper_tools::notification::types::{ToolNotification, ToolNotificationHandle};
use intellihelper_tools::registry::types::{FinalizedToolset, ToolConfig, ToolServerConfig};
...
impl WorkspaceSession {
    ...
    pub(crate) async fn toolset_terminal_is_session_owned(&self) -> bool {
        ...
        match res.get::<intellihelper_tools::types::resources::Terminal>() {
...
pub struct WorkspaceShared {
    ...
    pub(crate) events: tokio::sync::broadcast::Sender<intellihelper_workspace_types::WorkspaceEvent>,
    pub(crate) respect_gitignore: bool,
    pub(crate) memory_config: Option<MemoryConfig>,
    pub(crate) hook_registry: Arc<parking_lot::RwLock<intellihelper_hooks::discovery::HookRegistry>>,
    pub(crate) hook_load_errors: Vec<intellihelper_hooks::error::HookError>,
    ...
    pub(crate) activity_notify_handle:
        arc_swap::ArcSwap<Option<intellihelper_tools::notification::types::ToolNotificationHandle>>,
    ...
    pub(crate) lsp: Option<std::sync::Arc<dyn intellihelper_tools::implementations::lsp::LspBackend>>,
    ...
    /// (from `INTELLIHELPER_WORKSPACE_REWIND_ALL_OUTCOMES`, default off).
    pub(crate) workspace_rewind_all_outcomes: bool,
    /// Resolved `$INTELLIHELPER_WORKSPACE_HOME` — the workspace-owned on-disk state root
    /// (`<intellihelper_home>/workspace` by default). The upload queue spills here.
    ...
    /// (`INTELLIHELPER_WORKSPACE_EVENTS_ENABLED=true`). When `false`, every
    /// [`session_event_writer`](Self::session_event_writer) hands back an
    /// [`EventWriter::noop()`](file_utils::events::EventWriter::noop) and
    ...
    /// enabled (`INTELLIHELPER_WORKSPACE_TOOL_DEFS_ENABLED=true`).
    ...
    pub(crate) session_event_writers:
        Arc<dashmap::DashMap<String, file_utils::events::EventWriter>>,
...
impl WorkspaceShared {
    ...
    /// [`EventWriter::noop()`](file_utils::events::EventWriter::noop)
    ...
    pub(crate) fn session_event_writer_cached(
        &self,
        session_id: &str,
    ) -> Option<file_utils::events::EventWriter> {
    ...
    ///
    /// Returns a clone of the [`ToolServer`](computer_hub_sdk::ToolServer)
    ...
    pub fn subscribe_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<intellihelper_workspace_types::WorkspaceEvent> {
...
pub(crate) fn get_or_open_session_writer(
    enabled: bool,
    writers: &dashmap::DashMap<String, file_utils::events::EventWriter>,
    workspace_home: &Path,
    session_id: &str,
) -> file_utils::events::EventWriter {
...
mod tests {
    ...
    use dashmap::DashMap;
    use file_utils::events::{Event, EventWriter};
