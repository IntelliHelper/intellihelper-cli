//! Permission resolution engine: merges native `.intellihelper/config.toml`,
...
/// (fail-safe) with a warn + skip record for `intellihelper inspect`.
...
/// user `~/.intellihelper` → `Requirements`, so [`is_admin_source`] trusts only the root tier.
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    intellihelper_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
/// Load `[permission]` rules from native IntelliHelper TOML config files:
///
///   * `~/.intellihelper/config.toml` (lowest priority)
///   * Each `.intellihelper/config.toml` from the git repo root down to `cwd`
...
fn load_config_toml_permissions(cwd: &Path, project_trusted: bool) -> Vec<Sourced<PermissionRule>> {
    ...
    // Global `~/.intellihelper/config.toml` first (lowest priority within this layer).
    // Gated on user_intellihelper_home() so a project's .intellihelper/config.toml is never read as
    // global permissions when neither INTELLIHELPER_HOME nor a home dir resolves.
    if let Some(global_path) = intellihelper_config::user_intellihelper_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
        match intellihelper_config::load_config_file(&global_path) {
    ...
    // `.intellihelper/config.toml` (same gate as project `.claude/settings.json`).
    if project_trusted {
        for path in crate::project_config::find_project_configs(cwd) {
            match intellihelper_config::load_config_file(&path) {
                Ok(value) => rules.extend(extract_toml_permissions(&value, || {
...
fn managed_config_permissions(
    layers: &[intellihelper_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
...
/// Resolve permission config, merging native IntelliHelper and Claude sources.
...
/// `.intellihelper/config.toml` permission rules (mirrors [`load_claude_env_with_project`]).
...
/// blocked `--yolo`); keep admin-tier ones. Records each drop for `intellihelper inspect`.
...
struct ResolveInputs<'a> {
    ...
    /// `.claude/settings.json` / `.intellihelper/config.toml` permission rules are dropped
    ...
    fn live(project_trusted: bool) -> Self {
        Self {
            ...
            managed_config_rules: managed_config_permissions(
                &intellihelper_config::managed_config_layers(),
            ),
            project_trusted,
        }
    }
...
/// bypass is pinned off via intellihelper `requirements.toml`
...
///
/// `project_trusted` gates project-tier Claude settings and `.intellihelper/config.toml`
...
async fn resolve_permissions_with_provenance_inner(
    ...
    // Keep skip-only resolutions alive so the drop reaches `intellihelper inspect`; zero
...
fn resolve_claude_settings_inner(
    ...
    for path in claude_settings_paths_for_trust(cwd, project_trusted) {
        ...
        if let Some(perms) = settings.permissions {
            ...
            // `intellihelper inspect` (all-invalid allow/deny/ask must not leave
    ...
    // `intellihelper inspect` via the outer resolver.
...
/// [`intellihelper_config::claude_managed_settings_path`] (platform path such as
...
fn load_managed_settings() -> ManagedSettings {
    let Some(path) = intellihelper_config::claude_managed_settings_path() else {
...
/// not consulted: intellihelper must not inherit a host-wide always-approve lockdown from
/// that file. intellihelper still honors that file's permission rules / MCP / marketplace
/// allowlists, and the user's own `--yolo` / `[ui] permission_mode` / runtime
/// toggle drive always-approve; to disable it in intellihelper use a root-owned
/// `requirements.toml`. Fails open on user-writable layers.
pub fn yolo_disabled_by_policy() -> Option<&'static str> {
    let layers = intellihelper_config::requirements_layers();
...
/// without `~/.intellihelper`); `path` only names the layer in a non-bool warning.
...
/// Namespace prefix for managed (intellihelper.com-injected) MCP server names. Defined
...
mod tests {
    ...
    // Shared so `INTELLIHELPER_HOME`/`HOME` mutations here also serialize against the
    ...
    fn load_claude_env_merges_with_precedence() {
        // INTELLIHELPER_HOME-isolate so the claude-import marker reads clean (an imported
        ...
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home_guard = EnvVarGuard::set("INTELLIHELPER_HOME", home.path());
        let _marker_guard = EnvVarGuard::unset("_INTELLIHELPER_CLAUDE_MARKER_OVERRIDE");
        let tmp = tempfile::tempdir().unwrap();
        let claude_dir = tmp.path().join(".claude");
        std::fs::create_dir_all(&claude_dir).unwrap();
    ...
    fn load_claude_env_empty_when_no_settings() {
        // Isolate INTELLIHELPER_HOME (claude-import marker) AND HOME (global `~/.claude`)
    ...
    fn load_claude_env_with_project_drops_repo_env_when_untrusted() {
        ...
        // drop it. Isolate INTELLIHELPER_HOME so the claude-import marker reads clean (an
    ...
    fn untrusted_project_claude_permissions_are_not_honored() {
        ...
        let home = tempfile::tempdir().unwrap();
        let _home_guard = EnvVarGuard::set("HOME", home.path());
        let _intellihelper_guard = EnvVarGuard::set("INTELLIHELPER_HOME", home.path());
        let _marker_guard = EnvVarGuard::unset("_INTELLIHELPER_CLAUDE_MARKER_OVERRIDE");
    ...
    /// Untrusted clone must not contribute project `.intellihelper/config.toml` [permission].
    ///
    /// Sync + `block_on` so `ENV_LOCK` is not held across `.await` (clippy
    /// `await_holding_lock`). Does not assert exact global rule counts:
    /// `intellihelper_config::intellihelper_home()` is a process-wide `OnceLock`, so under
    /// single-process `cargo test` an earlier test may have already pinned
    /// `INTELLIHELPER_HOME`. Project-rule filtering is independent of that; global
    /// survival is checked only when our temp home is the live `user_intellihelper_home()`.
    #[test]
    fn untrusted_project_config_toml_permissions_are_not_honored() {
        ...
        // Global allow (survives untrusted project when INTELLIHELPER_HOME resolves here).
        std::fs::write(
...
"#,
        ...
        let rt = tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .expect("test runtime");
        // Untrusted may be None when no global rules load (INTELLIHELPER_HOME OnceLock
        ...
        let global_live = intellihelper_config::user_intellihelper_home()
            .is_some_and(|g| g == home.path() || g.starts_with(home.path()));
        if global_live {
            let untrusted = untrusted.expect("global rules present when INTELLIHELPER_HOME is live");
            assert!(
                untrusted
                    .config
                    .rules
    ...
    fn non_bool_lock_key_warns_and_does_not_lock() {
        ...
        let p = Path::new("/etc/intellihelper/requirements.toml");
        let bad = layer("[ui]\ndisable_bypass_permissions_mode = \"true\"\n");
        ...
        assert!(
            out.contains("/etc/intellihelper/requirements.toml"),
    ...
    fn admin_source_trusts_only_root_owned_tiers() {
        ...
        // the user-writable `~/.intellihelper/requirements.toml` is not, despite its path.
        ...
        assert!(is_admin_source(&RequirementSource::SystemRequirements {
            path: "/etc/intellihelper/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::Requirements {
            path: "/home/u/.intellihelper/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::ManagedConfig {
            path: "/etc/intellihelper/managed_config.toml".into(),
        }));
    ...
    fn drop_untrusted_catchall_allows_is_source_aware() {
        ...
        let rules = vec![
            ...
            // Managed config: defaults tier, untrusted even from /etc/intellihelper.
            sourced(
                allow_any(Some("*")),
    ...
    fn unrecognized_project_mode_claims_scope_over_global_accept_edits() {
        ...
        assert!(
            ...
            "typo should be recorded for intellihelper inspect"
        );
    }
    ...
    async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
        ...
        let layers =
            intellihelper_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
        ...
        .unwrap();
        assert_eq!(
            intellihelper_config::managed_config_layers_at(Some(system.path()), Some(user.path())).len(),
            1
        );
