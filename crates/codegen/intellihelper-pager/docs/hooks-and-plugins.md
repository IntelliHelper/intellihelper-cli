...
IntelliHelper CLI supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.intellihelper/hooks/`
- **Project hooks** — from `.intellihelper/hooks/` in your repo
...
Plugin hooks automatically receive `INTELLIHELPER_PLUGIN_ROOT` and `INTELLIHELPER_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.intellihelper/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `intellihelper plugin marketplace add <source>`)
...
Sources land in `~/.intellihelper/config.toml`:
...
Or in `~/.intellihelper/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../../intellihelper-hooks/examples/README.md) — ready-to-use sample hooks
