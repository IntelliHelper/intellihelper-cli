...
use intellihelper_tools::util::ProcessGroup;
...
pub async fn run_command_hook(
    ...
    let debug_payloads = std::env::var("INTELLIHELPER_HOOK_DEBUG").is_ok_and(|v| v == "1");
    if debug_payloads {
        tracing::trace!(
            hook_name = %spec.name,
    ...
    let mut cmd = if is_shell_command {
        ...
        #[cfg(not(unix))]
        {
            let inv = intellihelper_config::shell::shell_command_argv(&command_str);
    ...
    // plugin) can spoof `INTELLIHELPER_HOOK_EVENT`, `INTELLIHELPER_HOOK_NAME`, `INTELLIHELPER_SESSION_ID`,
    // `INTELLIHELPER_WORKSPACE_ROOT`, or `CLAUDE_PROJECT_DIR`, which are the
    ...
    let mut child = match cmd
        ...
        .env("INTELLIHELPER_HOOK_EVENT", envelope.hook_event_name.to_string())
        .env("INTELLIHELPER_HOOK_NAME", &spec.name)
        .env("INTELLIHELPER_SESSION_ID", ctx.session_id)
        .env("INTELLIHELPER_WORKSPACE_ROOT", ctx.workspace_root)
        // Compatibility alias for external hooks that read this env name.
        // Same value as `INTELLIHELPER_WORKSPACE_ROOT`; native `.intellihelper` hooks should use
        // `INTELLIHELPER_WORKSPACE_ROOT`.
...
pub(crate) const RUNNER_ALWAYS_SET_ENV: &[&str] = &[
    "INTELLIHELPER_HOOK_EVENT",
    "INTELLIHELPER_HOOK_NAME",
    "INTELLIHELPER_SESSION_ID",
    "INTELLIHELPER_WORKSPACE_ROOT",
    "CLAUDE_PROJECT_DIR",
];
...
///   hooks),
/// * the IntelliHelper process's own environment (which is inherited by the child),
...
mod tests {
    ...
    fn resolve_command_path_variants() {
        ...
        assert_eq!(
            resolve_command_path(&spec(
                HandlerType::Command,
                Some("bin/check.sh"),
                "/project/.intellihelper/hooks"
            )),
            Some(std::path::PathBuf::from(
                "/project/.intellihelper/hooks/bin/check.sh"
            ))
        );
        assert_eq!(
    ...
    fn make_scoped_ctx(scope: intellihelper_tools::util::ProcessScope) -> RunContext<'static> {
        RunContext {
    ...
    fn find_unresolved_skips_resolvable_vars() {
        ...
        let v = find_unresolved_env_vars(
            "${INTELLIHELPER_HOOK_EVENT}/${CLAUDE_PROJECT_DIR}/${INTELLIHELPER_SESSION_ID}/${CLAUDE_PLUGIN_ROOT}/foo",
            &env,
        );
        assert!(
            v.is_empty(),
    ...
    async fn test_tilde_expansion_runs_via_shell() {
        let tmp = tempfile::tempdir().unwrap();
        // Create the script at <tmp>/.intellihelper-test-hooks-gb856/tilde-test.sh
        let hook_dir = tmp.path().join(".intellihelper-test-hooks-gb856");
        ...
        // Inject HOME via extra_env so `sh -c "~/.intellihelper-test-hooks-gb856/..."`
        ...
        let spec = HookSpec {
            ...
            enabled: true,
            command: Some(std::path::PathBuf::from(
                "~/.intellihelper-test-hooks-gb856/tilde-test.sh",
            )),
            command_raw: Some("~/.intellihelper-test-hooks-gb856/tilde-test.sh".to_string()),
            url: None,
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::env::temp_dir(),
    ...
    async fn command_hook_session_close_reaps_whole_group() {
        ...
        let envelope = make_envelope();
        let scope = intellihelper_tools::util::ProcessScope::new();
