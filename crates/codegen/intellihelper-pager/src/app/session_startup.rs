...
pub enum DeferredSessionStartup {
    ...
    /// Fresh plain IntelliHelper session whose first prompt resumes a foreign tool session.
    ForeignResume {
        tool: intellihelper_workspace::foreign_sessions::ForeignSessionTool,
        native_id: String,
    },
}
...
pub fn fork_session_params(
    ...
    let source_cwd = intellihelper_shell::session::resolve_local_session_any_cwd(parent_session_id)
...
pub fn parent_session_is_worktree(session_id: &str, cwd: &Path) -> bool {
    ...
    let sessions_root = intellihelper_shell::util::intellihelper_home::intellihelper_home().join("sessions");
    let encoded = intellihelper_shell::util::intellihelper_home::encode_cwd_dirname(&cwd_str);
...
/// unless `INTELLIHELPER_CHAT_LOCAL_WORKSPACE_MODE` / attach server id is set.
#[cfg(feature = "local-workspace")]
pub const INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ENV: &str = "INTELLIHELPER_CHAT_LOCAL_WORKSPACE";
#[cfg(feature = "local-workspace")]
pub const INTELLIHELPER_CHAT_LOCAL_WORKSPACE_CWD_ENV: &str = "INTELLIHELPER_CHAT_LOCAL_WORKSPACE_CWD";
#[cfg(feature = "local-workspace")]
pub const INTELLIHELPER_CHAT_LOCAL_WORKSPACE_MODE_ENV: &str = "INTELLIHELPER_CHAT_LOCAL_WORKSPACE_MODE";
#[cfg(feature = "local-workspace")]
pub const INTELLIHELPER_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV: &str = "INTELLIHELPER_CHAT_LOCAL_WORKSPACE_SERVER_ID";
#[cfg(feature = "local-workspace")]
pub const INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ALLOW_HOME_ENV: &str = "INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ALLOW_HOME";
/// Skip interactive first-run confirm (still prints the banner).
#[cfg(feature = "local-workspace")]
pub const INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ACK_ENV: &str = "INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ACK";
...
pub const LOCAL_WORKSPACE_ATTACH_NEEDS_SERVER_ID: &str = "local-workspace attach requires --local-workspace-attach=<server_id> \
     (or INTELLIHELPER_CHAT_LOCAL_WORKSPACE_SERVER_ID)";
#[cfg(feature = "local-workspace")]
...
pub const LOCAL_WORKSPACE_HOME_DENIED: &str =
    "local-workspace cwd may not be / or $HOME unless INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ALLOW_HOME=1";
#[cfg(feature = "local-workspace")]
...
pub const LOCAL_WORKSPACE_ACK_REQUIRED: &str =
    "local-workspace requires interactive confirm, INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ACK=1, or an ack file";
/// Declared advertised tool ids for attach FS-only check (comma-separated).
#[cfg(feature = "local-workspace")]
pub const INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV: &str =
    "INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS";
#[cfg(feature = "local-workspace")]
...
pub fn resolve_local_workspace_config(
    ...
    let env_enable = env_truthy(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ENV);
    let env_mode = env_nonempty(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_MODE_ENV);
    let env_server_id = env_nonempty(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV);
    let env_cwd = env_nonempty(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_CWD_ENV).map(std::path::PathBuf::from);
    ...
    let mode = if cli_attach.is_some() {
        ...
        match m.as_str() {
            ...
            other => {
                anyhow::bail!(
                    "invalid {INTELLIHELPER_CHAT_LOCAL_WORKSPACE_MODE_ENV}={other:?}; expected own|attach"
                )
            }
        }
...
pub fn validate_local_workspace_cwd(path: &std::path::Path) -> anyhow::Result<std::path::PathBuf> {
    ...
    if env_truthy(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ALLOW_HOME_ENV) {
        return Ok(canon);
    }
...
///
/// Skip confirm only with `INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ACK=1` or a prior ack file.
...
pub fn local_workspace_ack_satisfied() -> bool {
    if env_truthy(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ACK_ENV) {
        return true;
    }
...
/// via `INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS` (comma-separated ids).
/// Unset / empty → refuse.
#[cfg(feature = "local-workspace")]
...
pub fn probe_advertised_tool_ids() -> Option<Vec<String>> {
    let raw = env_nonempty(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV)?;
...
fn local_workspace_ack_path() -> Option<std::path::PathBuf> {
    let home = std::env::var("INTELLIHELPER_HOME")
        .ok()
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    intellihelper_shell::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
pub struct MaterializeCtx {
    ...
    /// Process-wide flag: resume targets are intellihelper.com conversations, not
...
async fn most_recent_session_id(cwd: &str) -> anyhow::Result<(String, Option<String>)> {
    let summaries = intellihelper_shell::session::persistence::list_summaries(Some(cwd)).await?;
    let first = summaries.first().ok_or_else(|| {
        anyhow::anyhow!(
            "No session found for current directory. \
             Use 'intellihelper' to start a new session."
        )
    })?;
    Ok((first.info.id.to_string(), first.display_title_opt()))
}
/// `AuthManager` for direct intellihelper.com calls made outside the agent (pre-ACP
...
pub(crate) fn pre_acp_auth_manager(
    agent_config: &intellihelper_shell::agent::config::Config,
) -> std::sync::Arc<intellihelper_shell::auth::AuthManager> {
    let auth = std::sync::Arc::new(intellihelper_shell::auth::AuthManager::new(
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if intellihelper_shell::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = intellihelper_shell::session::resolve_local_session(session_id, cwd) {
    ...
    if let Some(original_cwd) = intellihelper_shell::session::resolve_local_session_any_cwd(session_id) {
        tracing::info!(
...
async fn restore_session_from_remote(
    ...
    if let Some((false, source)) =
        intellihelper_shell::util::config::session_registry_local_override_sourced(Some(&raw_config))
    {
        anyhow::bail!(
    ...
    let agent_config = intellihelper_shell::agent::config::Config::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {}", e))?;
    use intellihelper_shell::agent::session_registry_client::SessionRegistryClient;
    use intellihelper_shell::auth::{AuthManager, ensure_authenticated_or_noninteractive};
    use intellihelper_shell::session::restore::{RestoreSessionOpts, restore_session_with_storage};
    use intellihelper_shell::util::intellihelper_home::intellihelper_home;
    ...
    let storage_client = intellihelper_shell::auth::credential_provider::build_storage_client_for_proxy(
    ...
    let progress: intellihelper_shell::session::restore::ProgressCallback = Box::new(move |event| {
    ...
    let recovered_local_id = intellihelper_shell::session::find_local_child_for_remote(session_id, cwd);
...
mod tests {
    ...
    fn deferred_startup_owner_take_is_atomic() {
        let mut actions = DeferredStartupActions {
            session: Some(DeferredSessionStartup::ForeignResume {
                tool: intellihelper_workspace::foreign_sessions::ForeignSessionTool::Cursor,
    ...
    fn intent_default_is_new_auto() {
        assert_eq!(
            parse(&["intellihelper"]).session_startup_intent().unwrap(),
    ...
    fn intent_resume_id() {
        assert_eq!(
            parse(&["intellihelper", "--resume", "abc"])
    ...
    fn intent_continue() {
        assert_eq!(
            parse(&["intellihelper", "-c"]).session_startup_intent().unwrap(),
            SessionStartupIntent::Resume {
                session_id: None,
                most_recent_for_cwd: true,
            }
    ...
    fn intent_session_id_alone_is_new_with_id() {
        assert_eq!(
            parse(&["intellihelper", "--session-id", "my-id"])
                .session_startup_intent()
                .unwrap(),
    ...
    fn intent_session_id_with_resume_without_fork_errors() {
        let err = parse(&["intellihelper", "-r", "a", "-s", "b"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_fork_with_resume() {
        assert_eq!(
            parse(&["intellihelper", "-r", "old", "--fork-session"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
    ...
    fn intent_fork_with_resume_and_new_id() {
        assert_eq!(
            parse(&["intellihelper", "-r", "old", "--fork-session", "-s", "new"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
                source_session_id: Some("old".into()),
    ...
    fn intent_fork_alone_errors() {
        let err = parse(&["intellihelper", "--fork-session"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_fork_with_worktree_errors() {
        let err = parse(&["intellihelper", "-r", "a", "--fork-session", "-w"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_from_flags_matches_pager_args() {
        let args = parse(&["intellihelper", "-r", "old", "--fork-session", "-s", "new"]);
    ...
    fn materialize_ctx_chat_mode_from_args() {
        assert!(!MaterializeCtx::from_pager_args(&parse(&["intellihelper"])).chat_mode);
    }
    /// hardcoded `false` here once disabled it everywhere.
    #[test]
    fn remote_restore_follows_compiled_restore_stack() {
        assert_eq!(
            MaterializeCtx::from_pager_args(&parse(&["intellihelper"])).allow_remote_restore,
            false
        );
    }
    #[test]
    fn from_pager_args_does_not_probe_tty_for_progress() {
        assert!(
            !MaterializeCtx::from_pager_args(&parse(&["intellihelper"])).restore_progress_on_stdout,
            "stdout vs stderr is decided at the composition root, not from_pager_args"
        );
    }
    #[test]
    fn materialize_ctx_restore_code_follows_cli_flag() {
        assert!(!MaterializeCtx::from_pager_args(&parse(&["intellihelper"])).restore_code);
        assert!(!MaterializeCtx::from_pager_args(&parse(&["intellihelper", "-r", "abc"])).restore_code);
        assert!(
            MaterializeCtx::from_pager_args(&parse(&["intellihelper", "-r", "abc", "--restore-code"]))
                .restore_code
        );
    ...
    #[serial_test::serial(INTELLIHELPER_HOME)]
    #[tokio::test]
    async fn chat_resume_passthrough_keeps_cwd_collision_refusal() {
        let home = tempfile::tempdir().expect("home tempdir");
        unsafe { std::env::set_var("INTELLIHELPER_HOME", home.path()) };
        let cwd = tempfile::tempdir().expect("cwd tempdir");
        let cwd_str = cwd.path().to_string_lossy().to_string();
        let id = "aaaaaaaa-1111-2222-3333-444444444444";
        let encoded = intellihelper_shell::util::intellihelper_home::encode_cwd_dirname(&cwd_str);
        let sessions_cwd_dir = intellihelper_shell::util::intellihelper_home::intellihelper_home()
            .join("sessions")
            .join(&encoded);
    ...
    mod resume_by_title {
        use super::*;
        use crate::test_util::IntelliHelperHomeFixture;
        ...
        async fn title_fallback_resumes_single_match_case_insensitively() {
            let mut fx = IntelliHelperHomeFixture::new();
    ...
    #[cfg(feature = "local-workspace")]
    fn advertised_tools_env() -> intellihelper_test_support::EnvGuard {
        intellihelper_test_support::EnvGuard::set(
            INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV,
    ...
    #[cfg(feature = "local-workspace")]
    #[serial_test::serial(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS)]
    #[test]
    ...
    #[cfg(feature = "local-workspace")]
    #[serial_test::serial(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS)]
    #[serial_test::serial(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_SERVER_ID)]
    #[test]
    fn resolve_local_workspace_empty_cli_attach_falls_back_to_env() {
        let _env = advertised_tools_env();
        let _sid = intellihelper_test_support::EnvGuard::set(
            INTELLIHELPER_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV,
            "srv-from-env",
        );
        let tmp = tempfile::tempdir().unwrap();
    ...
    #[cfg(feature = "local-workspace")]
    #[serial_test::serial(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_CWD)]
    #[serial_test::serial(INTELLIHELPER_CHAT_LOCAL_WORKSPACE)]
    #[serial_test::serial(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_MODE)]
    #[serial_test::serial(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_SERVER_ID)]
    #[test]
    fn resolve_local_workspace_cwd_only_is_not_a_request() {
        let tmp = tempfile::tempdir().unwrap();
        let _cwd = intellihelper_test_support::EnvGuard::set(
            INTELLIHELPER_CHAT_LOCAL_WORKSPACE_CWD_ENV,
            tmp.path().to_str().unwrap(),
        );
        let _enable = intellihelper_test_support::EnvGuard::unset(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ENV);
        let _mode = intellihelper_test_support::EnvGuard::unset(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_MODE_ENV);
        let _sid = intellihelper_test_support::EnvGuard::unset(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV);
    ...
    fn resolve_local_workspace_own_env_defaults() {
        let _env = advertised_tools_env();
        let _enable = intellihelper_test_support::EnvGuard::set(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ENV, "1");
        let _mode = intellihelper_test_support::EnvGuard::unset(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_MODE_ENV);
        let _sid = intellihelper_test_support::EnvGuard::unset(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV);
    ...
    #[cfg(feature = "local-workspace")]
    #[serial_test::serial(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS)]
    #[serial_test::serial(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ALLOW_HOME)]
    #[serial_test::serial(HOME)]
    #[serial_test::serial(USERPROFILE)]
    #[test]
    fn resolve_local_workspace_defaults_cwd_and_denies_home() {
        let _tools = intellihelper_test_support::EnvGuard::set(
            INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV,
            "workspace.fs_list",
        );
        let _allow =
            intellihelper_test_support::EnvGuard::unset(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ALLOW_HOME_ENV);
        let home = tempfile::tempdir().unwrap();
        let home_str = home.path().to_str().unwrap();
        let _home = intellihelper_test_support::EnvGuard::set("HOME", home_str);
        let _userprofile = intellihelper_test_support::EnvGuard::set("USERPROFILE", home_str);
        let err =
    ...
    fn resolve_local_workspace_refuses_uncheckable_toolset() {
        let _tools =
            intellihelper_test_support::EnvGuard::unset(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV);
        let tmp = tempfile::tempdir().unwrap();
        let err =
    ...
    #[cfg(feature = "local-workspace")]
    #[serial_test::serial(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ACK)]
    #[serial_test::serial(INTELLIHELPER_HOME)]
    #[test]
    fn local_workspace_non_tty_requires_ack() {
        let _ack = intellihelper_test_support::EnvGuard::unset(INTELLIHELPER_CHAT_LOCAL_WORKSPACE_ACK_ENV);
        let home = tempfile::tempdir().unwrap();
        let _home =
            intellihelper_test_support::EnvGuard::set("INTELLIHELPER_HOME", home.path().to_str().unwrap());
