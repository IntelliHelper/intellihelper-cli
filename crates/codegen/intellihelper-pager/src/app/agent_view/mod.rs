...
pub struct BannerSlotParams<'a> {
    ...
    pub(crate) announcements: &'a [intellihelper_announcements::RemoteAnnouncement],
...
pub struct AgentView {
    ...
    pub context_state: Option<intellihelper_shell::session::ContextInfo>,
    /// Cumulative session token spend (input + output) for the top-right
    /// status badge. Updated from `PromptResponse` `_meta.usage` and
    /// `/usage` session ledger fetches. Distinct from context-window fill.
    pub session_usage_total_tokens: u64,
    /// Click target for the session token badge (`N tokens` → `/usage`).
    pub hit_session_usage: HitArea,
    ...
    /// Clickable optimize-sparkle icon on the prompt info line.
    pub hit_optimize_input: HitArea,
    /// In-flight Optimize generation id (`None` when idle).
    pub optimize_generation: Option<u64>,
    /// Draft text before the last successful Optimize (for Undo).
    pub optimize_undo: Option<String>,
    ...
    pub(crate) deferred_session_mode: Option<intellihelper_tools::types::SessionMode>,
    ...
    match kind {
        ...
        LocalQuestionKind::CreditLimitUpsell { choices } => {
            ...
            let choice = choices
                .get(*idx)
                .copied()
                .unwrap_or(intellihelper_telemetry::events::CreditLimitChoice::PayAsYouGo);
            intellihelper_telemetry::session_ctx::log_event(
                intellihelper_telemetry::events::CreditLimitUpsellClicked {
        ...
        LocalQuestionKind::FreeUsageUpsell { source } => {
            ...
            intellihelper_telemetry::session_ctx::log_event(
                intellihelper_telemetry::events::SuperGrokUpsellClicked {
                    source,
                    auth_method: None,
                },
            );
...
fn remember_mode_enabled() -> bool {
    let path = intellihelper_tools::util::intellihelper_home::intellihelper_home().join("config.toml");
...
fn resolve_action(action_id: Option<ActionId>) -> Option<InputOutcome> {
    let action = match action_id? {
        ...
        ActionId::OptimizeInput => Action::OptimizeInput,
        ActionId::UndoOptimizeInput => Action::UndoOptimizeInput,
...
fn question_visible_h(
    ...
    prompt_height: u16,
    question: &intellihelper_tools::implementations::intellihelper_build::ask_user_question::Question,
    content_w: usize,
    preview: Option<&str>,
    fullscreen: bool,
    desc_cap: u16,
