...
//!
//! External-auth deployments (`auth_provider_command`) never reach intellihelper.com
...
fn auth_provider_env_set() -> bool {
    std::env::var("INTELLIHELPER_AUTH_PROVIDER_COMMAND")
        .ok()
        .is_some_and(|s| !s.trim().is_empty())
}

fn auth_provider_config_set() -> bool {
    let Ok(raw) = intellihelper_shell::config::load_effective_config() else {
        return false;
    };
    let Ok(cfg) = intellihelper_shell::agent::config::Config::new_from_toml_cfg(&raw) else {
        return false;
    };
    cfg.grok_com_config
