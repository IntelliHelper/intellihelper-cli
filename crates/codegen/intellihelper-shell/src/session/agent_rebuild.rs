...
//! [`intellihelper_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`intellihelper_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`intellihelper_agent::Agent`] owns an [`intellihelper_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`intellihelper_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use intellihelper_agent::config::AgentDefinition;
use intellihelper_agent::error::AgentBuildError;
use intellihelper_agent::prompt::context::PromptAudience;
use intellihelper_agent::prompt::skills::SkillsConfig;
use intellihelper_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
use intellihelper_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use intellihelper_tools::implementations::intellihelper_build::ask_user_question::types::UserQuestionRequest;
...
use intellihelper_tools::implementations::intellihelper_build::task::types::{SubagentEvent, TaskModelValidator};
...
use intellihelper_tools::implementations::lsp::LspBackend;
...
use intellihelper_tools::types::SharedApiKeyProvider;
use intellihelper_tools::types::compat::CompatConfig;
use intellihelper_tools::types::memory_backend::MemoryBackend;
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<intellihelper_tools::SharedAttributionCallback>,
    ...
    pub managed_gateway_tool_client:
        Option<intellihelper_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        if let Some(event_tx) = subagent_event_tx.clone() {
            use intellihelper_tools::implementations::intellihelper_build::task::backend::{
        ...
        agent
            .tool_bridge()
            .update_resource(intellihelper_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(intellihelper_tools::types::resources::SchedulerBackgroundLoops(
                *scheduler_background_loops,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(intellihelper_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        ...
        {
            use intellihelper_tools::implementations::intellihelper_build::ask_user_question::UserQuestionSender;
            agent
                .tool_bridge()
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            intellihelper_tools::computer::local::LocalTerminalBackend::new_local(
                intellihelper_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
        ...
        subagent_depth: 0,
        subagents_max_depth: intellihelper_tools::implementations::intellihelper_build::task::MAX_SUBAGENT_DEPTH,
        ...
        system_prompt_label: intellihelper_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(intellihelper_tools::types::tool::ToolKind::Task)
            .expect("IntelliHelperBuild Task tool should be present");
        toolset
            .tool_definitions()
            .into_iter()
            .find(|definition| definition.function.name == task_name)
            .and_then(|definition| definition.function.description)
            .expect("IntelliHelperBuild Task description should be present")
    }
    ...
    async fn rebuild_projects_fresh_public_model_keys_into_task_description() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let first = spec
                    .build_agent(AgentDefinition::default_intellihelper_build())
                    .await
