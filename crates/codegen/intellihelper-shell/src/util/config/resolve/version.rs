...
///
/// Reads `stable_version` from `~/.intellihelper/version.json` (written by the
...
///
/// This is a lightweight duplicate of `intellihelper_update::channel_name()` for
/// use in `intellihelper-shell` which cannot depend on `intellihelper-update`.
pub(crate) fn channel_name_from_cache() -> Option<&'static str> {
    use std::sync::OnceLock;
    static NAME: OnceLock<Option<&'static str>> = OnceLock::new();
    *NAME.get_or_init(|| {
        let version_path = crate::util::intellihelper_home::intellihelper_home().join("version.json");
        let content = std::fs::read_to_string(&version_path).ok()?;
        let parsed: serde_json::Value = serde_json::from_str(&content).ok()?;
        let stable = parsed.get("stable_version")?.as_str()?;
        let current = semver::Version::parse(intellihelper_version::VERSION).ok()?;
...
impl VersionKnob {
    ...
    pub(crate) fn env_var(self) -> &'static str {
        match self {
            VersionKnob::Minimum => "INTELLIHELPER_MINIMUM_VERSION",
            VersionKnob::Maximum => "INTELLIHELPER_MAXIMUM_VERSION",
            VersionKnob::RequiredMinimum => "INTELLIHELPER_REQUIRED_MINIMUM_VERSION",
            VersionKnob::RequiredMaximum => "INTELLIHELPER_REQUIRED_MAXIMUM_VERSION",
        }
    }
...
mod tests {
    ...
    fn env_tightens_but_cannot_loosen() {
        ...
        let tighten = |var: &str| match var {
            "INTELLIHELPER_MINIMUM_VERSION" => Some("0.2.150".to_string()),
            "INTELLIHELPER_MAXIMUM_VERSION" => Some("0.2.180".to_string()),
            _ => None,
        };
        ...
        let loosen = |var: &str| match var {
            "INTELLIHELPER_MINIMUM_VERSION" => Some("0.2.1".to_string()),
            "INTELLIHELPER_MAXIMUM_VERSION" => Some("0.2.999".to_string()),
            _ => None,
        };
    ...
    fn a_user_bound_cannot_cancel_a_managed_hard_bound() {
        ...
        let low_ceiling =
            |var: &str| (var == "INTELLIHELPER_REQUIRED_MAXIMUM_VERSION").then(|| "0.2.50".to_string());
        ...
        let high_floor =
            |var: &str| (var == "INTELLIHELPER_REQUIRED_MINIMUM_VERSION").then(|| "0.2.200".to_string());
        ...
        let both = |var: &str| match var {
            "INTELLIHELPER_REQUIRED_MINIMUM_VERSION" => Some("99.0.0".to_string()),
            "INTELLIHELPER_REQUIRED_MAXIMUM_VERSION" => Some("0.0.1".to_string()),
            _ => None,
        };
