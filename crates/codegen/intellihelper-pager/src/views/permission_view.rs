...
use intellihelper_workspace::permission::bash_command_splitting::{
...
pub struct PermissionViewState {
    ...
    /// permissions). Imported from `intellihelper-shell`, NOT duplicated locally.
...
pub(crate) fn preview_command_text(state: &PermissionViewState) -> String {
    match state.bash_highlights.as_ref() {
        Some(h) => intellihelper_workspace::permission::bash_command_splitting::unwrap_command_wrappers(
            &h.highlighted_words,
        )
        .join(" "),
...
fn render_pattern_preview_line(
    ...
    match edit.trimmed() {
        ...
        Some(pattern) => {
            if intellihelper_workspace::permission::bash_pattern_matches_command(pattern, command) {
                spans.push(Span::styled(
            ...
            if intellihelper_workspace::permission::bash_pattern_is_broad(pattern) {
...
/// ```text
///  3 [ ] Tell IntelliHelper what to do differently
...
mod tests {
    ...
    fn render_shows_planned_mcp_args() {
        ...
        state.description = vec![
            ...
            "  \"body\": \"hello from intellihelper\"".to_string(),
            "}".to_string(),
        ];
        ...
        assert!(
            text.contains("\"body\": \"hello from intellihelper\""),
            "args JSON not rendered:\n{text}"
        );
    ...
    ///
    /// ```text
    /// PERMISSION_UI_RENDER_REVIEW=1 cargo test -p intellihelper-pager --lib \
    ...
    fn historic_bash_fixture_path() -> std::path::PathBuf {
        // CARGO_MANIFEST_DIR = crates/codegen/intellihelper-pager
