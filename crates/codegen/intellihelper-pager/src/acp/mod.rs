...
use intellihelper_shell::agent::auth_method::AuthMethodKind;
use intellihelper_shell::agent::config::Config as AgentConfig;
use intellihelper_shell::sampling::types::ReasoningEffort;
...
pub struct AcpConnection {
    ...
    /// Whether the agent is a intellihelper-shell instance.
    pub is_intellihelper_shell: bool,
    ...
    // When available, add field: startup_announcements: Option<Vec<intellihelper_announcements::RemoteAnnouncement>>
    /// Whether interactive login is required (deferred auth for `intellihelper.com`).
    ...
    pub auth_manager: std::sync::Arc<intellihelper_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<intellihelper_shell::util::config::RemoteSettings>,
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let spawned = spawn::spawn_intellihelper_shell(agent_config, cancel, memory_config).await?;
...
pub async fn connect_via_leader(
    ...
    let auth_manager = std::sync::Arc::new(intellihelper_shell::auth::AuthManager::new(
    ...
    intellihelper_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = intellihelper_shell::util::intellihelper_home::intellihelper_home().join("config.toml");
...
async fn initialize(
    ...
    // Check if this is a intellihelper-shell agent
    let is_intellihelper_shell = resp
        .meta
        .as_ref()
...
///
/// Matches TUI startup behavior: if the first method is `intellihelper.com`, defer auth
...
/// for a `intellihelper.com` or `oidc` method — these are the ones that can trigger
...
async fn eager_auth_or_login_fallback(
    ...
    if auth_methods.is_empty() {
        // No advertised methods (browser OAuth off, no global key yet). Do not
        // block the welcome screen — per-model api_key from config.toml is used
        // at request time. Prefer fail-open for open-source BYOK setups.
...
mod tests {
    ...
    fn startup_auth_grok_com_no_provider_needs_login_pending() {
        let methods = vec![make_auth_method("intellihelper.com", "intellihelper.com", None)];
        let (needs, label, method_id, mode) = startup_auth_metadata(&methods);
        assert!(needs);
        assert_eq!(label.as_deref(), Some("intellihelper.com"));
        assert_eq!(method_id.as_ref().unwrap().0.as_ref(), "intellihelper.com");
        assert_eq!(mode, AuthStartMode::Pending);
    }
    ...
    fn startup_auth_grok_com_with_external_provider_command() {
        ...
        let methods = vec![make_auth_method("intellihelper.com", "Acme Corp", Some(meta))];
        let (needs, label, method_id, mode) = startup_auth_metadata(&methods);
        assert!(needs);
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.intellihelper/config.toml` with a
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use intellihelper_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};

        let built = build_auth_methods(AuthMethodsBuildInputs {
            // enterprise-style: model has `env_key` set and the env var resolves,
            // so the shell-side predicate returns true.
            has_external_api_key: true,
            // Realistic enterprise user: no cached session token, no browser OAuth.
    ...
    fn startup_auth_xai_api_key_not_first_still_requires_login() {
        use intellihelper_shell::agent::auth_method::{INTELLIHELPER_COM_METHOD_ID, XAI_API_KEY_METHOD_ID};

        let methods = vec![
            make_auth_method(INTELLIHELPER_COM_METHOD_ID, "IntelliHelper", None),
            make_auth_method(XAI_API_KEY_METHOD_ID, "xai.api_key", None),
        ];
        let (needs, _, _, _) = startup_auth_metadata(&methods);
        assert!(
            needs,
            "with intellihelper.com first, the pager must require login -- pinning \
    ...
    fn startup_auth_method_id_is_copied_not_synthesized() {
        let methods = vec![make_auth_method("intellihelper.com", "My Login", None)];
    ...
    fn startup_auth_external_provider_false_is_pending() {
        ...
        let methods = vec![make_auth_method("intellihelper.com", "intellihelper.com", Some(meta))];
