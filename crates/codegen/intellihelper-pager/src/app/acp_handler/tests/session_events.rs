#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn retry_exhausted_rate_limited_empty_reason_uses_oauth_fallback() {
        use intellihelper_shell::sampling::error::RATE_LIMITED_USER_MESSAGE_OAUTH;
    ...
    fn retry_exhausted_api_key_rewrites_consumer_subscription_upsell() {
        use intellihelper_shell::sampling::error::RATE_LIMITED_USER_MESSAGE_API_KEY;
        ...
        match last_session_event(&scrollback) {
            Some(SessionEvent::RetryFailed { error, .. }) => {
                ...
                assert!(!error.contains("intellihelper.com/supergrok"));
            }
    ...
    fn apply_retry_state_disk_full_pushes_session_event() {
        use intellihelper_shell::extensions::notification::{
    ...
    /// Legacy WebLogin auth keeps its verbose message (with `intellihelper logout` /
    /// `intellihelper login` guidance), not the generic re-auth prompt.
    #[test]
        ...
        apply_retry_state(
            &RetryState::Failed {
                ...
                message: "Unauthorized (401) ... deprecated authentication method (WebLogin) ... \
                          run `intellihelper logout` then `intellihelper login`"
                    .into(),
            },
            &mut session,
            &mut scrollback, false);
    ...
    fn child_compact_started_does_not_reset_context_used() {
        ...
        child_view.context_state = Some(intellihelper_shell::session::ContextInfo::from_notification(
            90_000, 131_072,
        ));
        agent
            .subagent_views
