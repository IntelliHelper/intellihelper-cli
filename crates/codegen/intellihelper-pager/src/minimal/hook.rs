...
//! render mode.
//!
//! `intellihelper-pager` cannot depend on `intellihelper-pager-minimal`: that crate reads
...
//!
//! The composition-root binary (`intellihelper-pager-bin`) wires it up once at
//! startup by calling `intellihelper_pager_minimal::install()`. When nothing has
...
/// (`intellihelper_pager_minimal::draw`).
