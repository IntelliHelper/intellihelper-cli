...
impl crate::types::tool_metadata::ToolMetadata for TaskTool {
    ...
    fn description_template(&self) -> &str {
        // IntelliHelper CLI normally supplies the description via
        // `ToolConfig::with_description(...)` using `build_task_description()`
        // in intellihelper-agent/src/builder.rs (live subagent roster). But a
    ...
    async fn run(
        ...
        if result.backgrounded {
            ...
            if detect_continue_parent_work(&resources, &input.description, &input.prompt).await {
                text.push_str("\n\n");
                text.push_str(tool_types::BACKGROUND_SUBAGENT_CONTINUE_PARENT_WORK);
            }
