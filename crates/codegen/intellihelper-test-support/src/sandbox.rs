//! Hermetic filesystem and child-environment owner for intellihelper integration tests.
...
/// intellihelper network kill switches, and explicit overrides.
pub struct TestSandbox {
    root: TempDir,
    home: PathBuf,
    intellihelper_home: PathBuf,
    workspace: PathBuf,
    temp: PathBuf,
...
impl TestSandbox {
    ...
    /// Explicit intellihelper state root.
    pub fn intellihelper_home(&self) -> &Path {
        &self.intellihelper_home
    }
    ...
    pub fn diagnostic_summary(&self) -> String {
        let mut summary = format!(
            "root={} home={} intellihelper_home={} workspace={} temp={}",
            self.root().display(),
            self.home.display(),
            self.intellihelper_home.display(),
...
impl TestSandboxBuilder {
    /// Wire intellihelper API, models, feedback, trace, conversation, and web traffic to
    ...
    pub fn build(self) -> TestSandbox {
        ...
        for path in [&home, &intellihelper_home, &workspace, &temp] {
        ...
        let mut env = baseline_env(&home, &intellihelper_home, &temp, &parent_cwd);
...
impl TestSandbox {
    fn init_git_workspace(&self) {
        ...
        run_git(self, &["config", "user.name", "IntelliHelper Test"]);
...
fn apply_mock_url(env: &mut BTreeMap<OsString, OsString>, url: String) {
    for key in [
        ...
        "INTELLIHELPER_CODE_WEB_URL",
        "INTELLIHELPER_CONVERSATIONS_BASE_URL",
    ] {
...
fn baseline_env(
    ...
    baseline_env_from_parent(home, intellihelper_home, temp, parent_cwd, &parent_env)
}
...
fn baseline_env_from_parent(
    ...
    for (key, value) in [
        ("INTELLIHELPER_TELEMETRY_ENABLED", "false"),
        ("INTELLIHELPER_TELEMETRY_TRACE_UPLOAD", "false"),
        ("INTELLIHELPER_FEEDBACK_ENABLED", "false"),
        ("INTELLIHELPER_TRACE_UPLOAD", "false"),
        ("INTELLIHELPER_INSTRUMENTATION", "disabled"),
        ("OTEL_SDK_DISABLED", "true"),
        ("DISABLE_TELEMETRY", "1"),
        ("DISABLE_FEEDBACK_COMMAND", "1"),
        ("INTELLIHELPER_DISABLE_AUTOUPDATER", "1"),
        ("INTELLIHELPER_PROMPT_SUGGESTIONS", "false"),
        // Post-turn summary side-calls would add unscripted requests to the
        // mock server and break exact wire-traffic assertions.
        ("INTELLIHELPER_TURN_SUMMARY", "0"),
    ...
    env.insert(
        ...
        intellihelper_home.join("gitconfig").into_os_string(),
    );
    env
}
...
fn diagnostic_value_is_sensitive(key: &OsStr) -> bool {
    ...
    is_secret_key(&key)
        ...
        || matches!(
            ...
            "HOME" | "USERPROFILE" | "INTELLIHELPER_HOME" | "TMPDIR" | "TMP" | "TEMP" | "GIT_CONFIG_GLOBAL"
        )
}
...
mod tests {
    ...
    fn owns_distinct_isolated_paths() {
        ...
        assert_eq!(sandbox.intellihelper_home(), sandbox.home().join(".intellihelper"));
    }
    ...
    fn resolved_baseline_env(
        ...
        baseline_env_from_parent(
            &root.path().join("home"),
            &root.path().join("home/.intellihelper"),
            &root.path().join("tmp"),
            parent_cwd,
            &parent_env,
        )
    ...
    fn git_command_uses_sandbox_state_without_process_global_mutation() {
        ...
        let sandbox = TestSandbox {
            ...
            intellihelper_home: root.path().join("home/.intellihelper"),
    ...
    fn baseline_is_hermetic_and_network_quiet() {
        ...
        assert_eq!(
            env_value(&sandbox, "INTELLIHELPER_HOME"),
            Some(sandbox.intellihelper_home().into())
        );
        assert_eq!(
        ...
        assert_eq!(
            env_value(&sandbox, "INTELLIHELPER_DISABLE_AUTOUPDATER").as_deref(),
            Some(OsStr::new("1"))
        );
        assert_eq!(
            env_value(&sandbox, "INTELLIHELPER_TELEMETRY_TRACE_UPLOAD").as_deref(),
        ...
        assert_eq!(env_value(&sandbox, "INTELLIHELPER_LEADER_SOCKET"), None);
        assert_eq!(env_value(&sandbox, "INTELLIHELPER_DISABLE_WEB_FETCH"), None);
        assert_eq!(env_value(&sandbox, "INTELLIHELPER_WEB_FETCH"), None);
    }
    ...
    fn command_application_clears_ambient_env_and_command_override_wins() {
        ...
        cmd.env("AMBIENT_SECRET", "must-disappear")
            .env("INTELLIHELPER_PROMPT_SUGGESTIONS", "ambient");
        sandbox.apply_to_std_command(&mut cmd);
        cmd.env("INTELLIHELPER_PROMPT_SUGGESTIONS", "command");
        ...
        assert_eq!(
            env.get(OsStr::new("INTELLIHELPER_PROMPT_SUGGESTIONS"))
                .map(OsString::as_os_str),
    ...
    fn explicit_overrides_win_and_can_remove_baseline_entries() {
        ...
        sandbox
            ...
            .set_env("INTELLIHELPER_PROMPT_SUGGESTIONS", "true")
            .set_env("NO_PROXY", "override.invalid")
            .remove_env("INTELLIHELPER_DISABLE_AUTOUPDATER");
        assert_eq!(
            env_value(&sandbox, "TERM_PROGRAM").as_deref(),
            Some(OsStr::new("vscode"))
        );
        assert_eq!(
            env_value(&sandbox, "INTELLIHELPER_PROMPT_SUGGESTIONS").as_deref(),
            Some(OsStr::new("true"))
        );
        assert_eq!(
            env_value(&sandbox, "NO_PROXY").as_deref(),
        ...
        assert_eq!(env_value(&sandbox, "INTELLIHELPER_DISABLE_AUTOUPDATER"), None);
    }
    ...
    fn diagnostics_fail_closed_for_credential_keys() {
        ...
        for (key, value) in [
            ...
            ("INTELLIHELPER_DEPLOYMENT_KEY", "deployment-key-do-not-print"),
            ("INTELLIHELPER_EXTRA_AUTH_KEY", "alpha-test-key-do-not-print"),
