...
pub enum SideQuestionError {
    ...
    Sampling(#[from] intellihelper_sampling_types::SamplingError),
...
pub enum SessionCommand {
    ...
    RestorePlanApproval,
    GetToolOverrides {
        respond_to: oneshot::Sender<Option<intellihelper_sampling_types::ToolOverrides>>,
    },
    ...
    SetToolOverrides {
        overrides: intellihelper_sampling_types::ToolOverrides,
    },
    Prompt {
        prompt_id: String,
        ...
        tool_overrides_update: Option<intellihelper_sampling_types::ToolOverridesUpdate>,
    ...
    SetSessionModel {
        sampling_config: intellihelper_sampler::SamplerConfig,
        use_concise: bool,
    ...
    /// rather than the agent-level default (e.g. `intellihelper-4.5`).
    ///
    /// Keeps the existing base_url, api_key, and other config — only changes
    /// the `model` field sent in the `x-intellihelper-model-override` header and merges
    ...
    ReloadPlugins {
        registry: Option<std::sync::Arc<intellihelper_agent::plugins::PluginRegistry>>,
    },
    /// Re-discover the session's own project hooks (`.intellihelper/hooks`,
    ...
    SnapshotToolDefinitions {
        respond_to: oneshot::Sender<Vec<intellihelper_sampling_types::ToolSpec>>,
    },
    ...
    KillBackgroundTask {
        task_id: String,
        respond_to: oneshot::Sender<Result<intellihelper_tools::types::KillOutcome, String>>,
    },
    DeleteScheduledTask {
        task_id: String,
    ...
    ListTasks {
        respond_to: oneshot::Sender<Option<Vec<intellihelper_tools::types::TaskSnapshot>>>,
    },
    ...
    PluginsList {
        respond_to:
            oneshot::Sender<Option<std::sync::Arc<intellihelper_agent::plugins::PluginRegistry>>>,
    },
    ...
    /// Rewrite a composer draft into clearer natural-language prompt text
    /// (Optimize input sparkle). Tool-free model call with workspace context;
    /// does not mutate the conversation or send a user turn.
    OptimizePrompt {
        draft: String,
        respond_to: oneshot::Sender<Result<String, SideQuestionError>>,
    },
    ...
    /// tool-free model call (default `intellihelper-build-0.1` when available via
    ...
    /// `intellihelper-build` model, low temperature, and capped output tokens.
    RewriteMemoryNote {
        raw_text: String,
    ...
    TakeHarnessTraceTurns {
        respond_to:
            oneshot::Sender<Vec<Vec<intellihelper_sampling_types::conversation::ConversationItem>>>,
    },
