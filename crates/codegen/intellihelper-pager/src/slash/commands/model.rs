...
use intellihelper_shell::sampling::types::supports_reasoning_effort_meta;
...
impl SlashCommand for ModelCommand {
    ...
    fn run(&self, ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        ...
        // often contain spaces ("IntelliHelper 4.5"); if we split on the last token
        // first, a shorter catalog entry ("IntelliHelper") would steal the prefix and
        ...
        // level (e.g. `none` on intellihelper-4.5) surfaces the effort error with the
...
mod tests {
    ...
    fn empty_query_returns_one_row_per_logical_model() {
        ...
        let (pid, pinfo) = plain_model("intellihelper-4.5", "IntelliHelper 4.5");
        ...
        let plain = items.iter().find(|i| i.match_text == "IntelliHelper 4.5").unwrap();
        assert_eq!(plain.insert_text, "IntelliHelper 4.5");
    }
    ...
    fn run_prefers_full_multi_word_model_name_over_prefix_plus_effort() {
        // Catalog has both "IntelliHelper" (reasoning) and "IntelliHelper 4.5". `/model IntelliHelper 4.5`
        // must select the full name, not treat "4.5" as an effort on "IntelliHelper".
        let mut state = ModelState::default();
        let (short_id, short_info) = model_with_reasoning("intellihelper", "IntelliHelper");
        let (long_id, long_info) = model_with_reasoning("intellihelper-4.5", "IntelliHelper 4.5");
        state.available.insert(short_id, short_info);
        state.available.insert(long_id.clone(), long_info);
        let mut ctx = dummy_exec_ctx(&state);
        let result = ModelCommand.run(&mut ctx, "IntelliHelper 4.5");
        match result {
            CommandResult::Action(Action::SetDefaultModel(resolved_id)) => {
                assert_eq!(resolved_id, long_id);
            }
            other => panic!("expected SetDefaultModel(IntelliHelper 4.5), got {other:?}"),
        }
    }
    ...
    fn run_rejects_effort_for_non_reasoning_model() {
        let mut state = ModelState::default();
        let (id, info) = plain_model("intellihelper-4.5", "IntelliHelper 4.5");
        state.available.insert(id, info);
        let mut ctx = dummy_exec_ctx(&state);
        let result = ModelCommand.run(&mut ctx, "IntelliHelper 4.5 high");
    ...
    /// Case-insensitive matching against the catalog: `/model intellihelper 4.5`
    /// resolves to the same `ModelId` as `/model IntelliHelper 4.5`.
    #[test]
