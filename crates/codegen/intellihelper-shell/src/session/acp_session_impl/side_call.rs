...
pub(crate) fn log_prompt_cache_hit(
    ...
    response: &intellihelper_sampling_types::ConversationResponse,
) {
...
pub(crate) struct AuxCall {
    ...
    pub(crate) hosted_tools: Vec<intellihelper_sampling_types::HostedTool>,
    pub(crate) model: String,
    /// Must match the main turn's, or the prompt differs before the conversation history even starts.
    pub(crate) reasoning_effort: Option<intellihelper_sampling_types::ReasoningEffort>,
...
pub(crate) struct SideCallSetup {
    pub(crate) client: intellihelper_sampler::SamplingClient,
...
impl SessionActor {
    ...
    pub(crate) fn parent_cached_request(&self, call: AuxCall) -> ConversationRequest {
        ...
        // so it has to stay the parent session id; the `btw-`/`recap-` label still shows up in `x_intellihelper_req_id`.
        ...
        ConversationRequest {
            ...
            x_intellihelper_conv_id: Some(conv_id),
            x_intellihelper_req_id: Some(call.req_id),
    ...
    pub(crate) async fn side_call_request(
        ...
        x_intellihelper_conv_id: String,
        x_intellihelper_req_id: String,
    ) -> ConversationRequest {
